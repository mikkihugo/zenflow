...
export function createIsolatedCache() {
  ...
  // Use process.env if available (node environment), otherwise use process.env (Node.js environment)
  const baseEnv = typeof node !== 'undefined' && process.env ? process.env.toObject() : process.env;
