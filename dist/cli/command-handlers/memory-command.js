...
import { printSuccess, printError, printWarning } from '../cli-utilities.js';
...
export async function memoryCommand(subArgs, flags) {
  ...
  async function saveMemory(data) {
    await node.mkdir('./memory', { recursive: true });
    await node.writeTextFile(memoryStore, JSON.stringify(data, null, 2));
  }
...
async function importMemory(subArgs, saveMemory) {
  ...
  try {
    const importContent = await node.readTextFile(filename);
