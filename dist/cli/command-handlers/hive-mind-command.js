...
import { parseFlags, normalizeFlags, applySmartDefaults, validateFlags } from '../cli-utilities.js';
...
import { CollectiveMemory } from './hive-mind-handlers/hive-mind/memory.js';
import { SwarmCommunication } from './hive-mind-handlers/hive-mind/communication.js';
import { HiveMindSessionManager } from './hive-mind-handlers/hive-mind/session-manager.js';
import { createAutoSaveMiddleware } from './hive-mind-handlers/hive-mind/auto-save-middleware.js';
...
${chalk.bold('EXAMPLES:')}
  ...
  claude-zen hive-mind spawn "Build REST API" --claude

  ${chalk.gray('# Auto-spawn coordinated Claude Code instances')}
  claude-zen hive-mind spawn "Research AI trends" --auto-spawn --verbose
...
${chalk.blue('https://github.com/ruvnet/claude-zen/tree/main/docs/hive-mind')}
`);
}
...
async function initHiveMind(flags) {
  ...
  try {
    ...
    console.log(
      '  1. Run ' + chalk.cyan('claude-zen hive-mind spawn') + ' to create your first swarm',
    );
    console.log(
      '  2. Use ' + chalk.cyan('claude-zen hive-mind wizard') + ' for interactive setup',
    );
  } catch (error) {
...
async function spawnSwarm(args, flags) {
  ...
  if (!objective && !flags.wizard) {
    ...
    console.log('Example: claude-zen hive-mind spawn "Build REST API"');
    return;
  }
  ...
  try {
    ...
    try {
      ...
      hiveMind = new HiveMindCore({
        ...
        namespace: finalFlags.namespace || 'default',
    ...
    console.log(chalk.gray('Use "claude-zen hive-mind status" to view swarm activity'));
    ...
    console.log(chalk.blue('💡 To resume:') + ' claude-zen hive-mind resume ' + sessionId);
    ...
    const sigintHandler = async () => {
      ...
      try {
        ...
        console.log(chalk.bold(`  claude-zen hive-mind resume ${sessionId}`));
        console.log();
    ...
    if (flags.claude || flags.spawn) {
      ...
      console.log(chalk.gray('   claude-zen hive-mind spawn "objective" --claude'));
    }
  } catch (error) {
...
async function showStatus(flags) {
  try {
    ...
    if (!existsSync(dbPath)) {
      ...
      console.log('Run "claude-zen hive-mind init" first');
      return;
    }
...
export async function hiveMindCommand(args, flags) {
  ...
  switch (subcommand) {
    ...
    default:
      ...
      console.log('Run "claude-zen hive-mind help" for usage information');
      exit(1);
  }
}
...
async function spawnClaudeCodeInstances(swarmId, swarmName, objective, workers, flags) {
  ...
  try {
    ...
    console.log('• Monitor with: claude-zen hive-mind status');
...
1️⃣ **COLLECTIVE INTELLIGENCE**
   mcp__claude-zen__consensus_vote    - Democratic decision making
   mcp__claude-zen__memory_share      - Share knowledge across the hive
   mcp__claude-zen__neural_sync       - Synchronize neural patterns
   mcp__claude-zen__swarm_think       - Collective problem solving

2️⃣ **QUEEN COORDINATION**
   mcp__claude-zen__queen_command     - Issue directives to workers
   mcp__claude-zen__queen_monitor     - Monitor swarm health
   mcp__claude-zen__queen_delegate    - Delegate complex tasks
   mcp__claude-zen__queen_aggregate   - Aggregate worker results

3️⃣ **WORKER MANAGEMENT**
   mcp__claude-zen__agent_spawn       - Create specialized workers
   mcp__claude-zen__agent_assign      - Assign tasks to workers
   mcp__claude-zen__agent_communicate - Inter-agent communication
   mcp__claude-zen__agent_metrics     - Track worker performance

4️⃣ **TASK ORCHESTRATION**
   mcp__claude-zen__task_create       - Create hierarchical tasks
   mcp__claude-zen__task_distribute   - Distribute work efficiently
   mcp__claude-zen__task_monitor      - Track task progress
   mcp__claude-zen__task_aggregate    - Combine task results

5️⃣ **MEMORY & LEARNING**
   mcp__claude-zen__memory_store      - Store collective knowledge
   mcp__claude-zen__memory_retrieve   - Access shared memory
   mcp__claude-zen__neural_train      - Learn from experiences
   mcp__claude-zen__pattern_recognize - Identify patterns
...
1. **INITIALIZE THE HIVE** (Single BatchTool Message):
   [BatchTool]:
   ${workerTypes.map((type) => `   mcp__claude-zen__agent_spawn { "type": "${type}", "count": ${workerGroups[type].length} }`).join('\n')}
   mcp__claude-zen__memory_store { "key": "hive/objective", "value": "${objective}" }
   mcp__claude-zen__memory_store { "key": "hive/queen", "value": "${queenType}" }
   mcp__claude-zen__swarm_think { "topic": "initial_strategy" }
...
async function showSessions(flags) {
  try {
    ...
    console.log('  • Resume a session: claude-zen hive-mind resume <session-id>');
    console.log('  • View session details: claude-zen hive-mind status');
...
async function resumeSession(args, flags) {
  ...
  if (!sessionId) {
    ...
    console.log('Usage: claude-zen hive-mind resume <session-id>');
    console.log('Run "claude-zen hive-mind sessions" to see available sessions');
    return;
  }
  ...
  try {
    ...
    if (!session) {
      ...
      console.log('\nRun "claude-zen hive-mind sessions" to see available sessions');
      sessionManager.close();
      return;
    }
    ...
    if (flags.claude || flags.spawn) {
      ...
      console.log(chalk.gray('   claude-zen hive-mind resume ' + sessionId + ' --claude'));
    }
  } catch (error) {
...
async function stopSession(args, flags) {
  ...
  if (!sessionId) {
    console.error(chalk.red('Error: Please provide a session ID'));
    console.log('Usage: claude-zen hive-mind stop <session-id>');
    console.log('Run "claude-zen hive-mind sessions" to see available sessions');
    return;
  }
  ...
  try {
    ...
    console.log(
      chalk.gray('To resume this session later, use: claude-zen hive-mind resume ' + sessionId),
    );
...
3. **COORDINATION**:
   - Use mcp__claude-zen__memory_retrieve to access shared knowledge
