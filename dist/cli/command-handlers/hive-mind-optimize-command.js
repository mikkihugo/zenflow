...
} from './hive-mind-handlers/hive-mind/db-optimizer.js';
...
${chalk.bold('USAGE:')}
  claude-zen hive-mind-optimize [options]
...
${chalk.bold('EXAMPLES:')}
  ...
  claude-zen hive-mind-optimize --clean-memory --memory-days 60 --task-days 14
...
async function runOptimization(dbPath, options) {
  ...
  console.log('  • Monitor performance with: claude-zen hive-mind metrics');
  console.log('  • Schedule regular maintenance: claude-zen hive-mind-optimize --auto');
  console.log('  • Check swarm status: claude-zen hive-mind status');
}
...
async function generateReport(dbPath) {
  ...
  if (report.schemaVersion >= 1.5) {
    ...
    console.log(chalk.gray('    Run: claude-zen hive-mind-optimize'));
  }
