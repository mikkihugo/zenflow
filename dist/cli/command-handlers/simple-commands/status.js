...
function displayStatus(status, verbose) {
  ...
  if (!status.orchestrator.running) {
    console.log('   Run "claude-zen start" to begin orchestration');
  }
  if (status.agents.active === 0) {
    console.log('   Run "claude-zen agent spawn researcher" to create an agent');
  }
  if (status.memory.entries === 0) {
    console.log('   Run "claude-zen memory store key value" to test memory');
  }
}
