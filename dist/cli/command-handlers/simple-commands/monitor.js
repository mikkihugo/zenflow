...
async function getOrchestratorMetrics() {
  try {
    ...
    const metricsPath = path.join(process.cwd(), '.claude-zen', 'metrics.json');
...
async function checkOrchestratorRunning() {
  try {
    const pidPath = path.join(process.cwd(), '.claude-zen', 'orchestrator.pid');
...
async function getResourceMetrics() {
  try {
    ...
    const memoryDbPath = path.join(process.cwd(), '.claude-zen', 'memory.db');
...
async function countTerminalSessions() {
  try {
    const sessionsPath = path.join(process.cwd(), '.claude-zen', 'sessions');
...
export function showMonitorHelp() {
  ...
  console.log('  claude-zen monitor              # Show current metrics');
  console.log('  claude-zen monitor --watch      # Continuous monitoring');
  console.log('  claude-zen monitor --interval 1000 --watch  # Fast updates');
  console.log('  claude-zen monitor --format json            # JSON output');
}
