...
class SwarmUI {
  ...
  async createObjective(description) {
    try {
      ...
      const process = spawn('claude-zen', args, {
        detached: true,
        stdio: 'ignore',
      });
  ...
  async stopOrphanedProcesses() {
    ...
    if (os.platform() === 'win32') {
      ...
      exec(
        'wmic process where "commandline like \'%claude-zen swarm%\'" get processid',
        (error, stdout) => {
          if (!error && stdout) {
            const pids = stdout
              .split('\n')
      ...
      exec('ps aux | grep "claude-zen swarm" | grep -v grep', (error, stdout) => {
        if (!error && stdout) {
