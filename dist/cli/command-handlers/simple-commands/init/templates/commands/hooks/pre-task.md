...
```bash
npx claude-zen hook pre-task --description "Implement user authentication"
```

### With memory loading

```bash
npx claude-zen hook pre-task -d "Continue API development" --load-memory
```

### Manual agent control

```bash
npx claude-zen hook pre-task -d "Debug issue #123" --auto-spawn-agents false
```

### Full optimization

```bash
npx claude-zen hook pre-task -d "Refactor codebase" --optimize-topology --estimate-complexity
```
...
npx claude-zen hook pre-task --description "Your task here"
```
