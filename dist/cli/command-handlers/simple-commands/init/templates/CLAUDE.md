...
- Use the `swarm_init` tool to set up coordination topology
- Choose: mesh, hierarchical, ring, or star
- This creates a coordination framework for Claude Code's work

**Spawn agents:**

- Use `agent_spawn` tool to create specialized coordinators  
- Agent types: coordinator, researcher, coder, analyst, architect, tester, reviewer, optimizer
- They help Claude Code approach problems from different angles

**Orchestrate tasks:**

- Use `task_orchestrate` tool to coordinate complex workflows
...
- `swarm_init` - Set up coordination topology for Claude Code
- `agent_spawn` - Create cognitive patterns to guide Claude Code  
- `task_orchestrate` - Break down and coordinate complex tasks

### Monitoring Tools:

- `swarm_status` - Monitor coordination effectiveness
- `agent_list` - View active cognitive patterns
- `agent_metrics` - Track coordination performance
- `task_status` - Check workflow progress  
- `task_results` - Review coordination outcomes

### Memory & Neural Tools:

- `memory_usage` - Persistent memory across sessions
- `neural_status` - Neural pattern effectiveness
- `neural_train` - Improve coordination patterns
- `neural_patterns` - Analyze thinking approaches
...
2. Each agent MUST use Claude Flow hooks for coordination:
   ...
   - `npx claude-zen@alpha hooks post-task` to share decisions
...
npx claude-zen@alpha memory query "[previous work]" --namespace "swarm-[id]"
```
...
npx claude-zen@alpha memory store "swarm/[agent]/decision" "[what was decided]" --namespace "swarm-[id]"

# Lifecycle hook for task completion
npx claude-zen@alpha hooks post-task --task-id "[task-step]" --analyze-performance true
```
...
3. MEMORY: Store ALL decisions using `npx claude-zen@alpha memory store "[decision-key]" "[decision]" --namespace "swarm"`
...
[BatchTool]:
  ...
  - agent_spawn { type: "coder", name: "API Developer" }
  - agent_spawn { type: "coder", name: "Frontend Dev" }
  - agent_spawn { type: "analyst", name: "DB Designer" }
  - agent_spawn { type: "tester", name: "QA Engineer" }
  - agent_spawn { type: "researcher", name: "Tech Lead" }
  - agent_spawn { type: "coordinator", name: "PM" }
  - TodoWrite { todos: [multiple todos at once] }

STEP 2: PARALLEL TASK EXECUTION (Single Message!)
[BatchTool]:
  - task_orchestrate { task: "main task", strategy: "parallel" }
  - memory_usage { action: "store", key: "init", value: {...} }
...
[BatchTool - Message 1]:
  ...
  swarm_init { topology: "hierarchical", maxAgents: 8, strategy: "parallel" }
  agent_spawn { type: "architect", name: "System Designer" }
  agent_spawn { type: "coder", name: "API Developer" }
  agent_spawn { type: "coder", name: "Auth Expert" }
  agent_spawn { type: "analyst", name: "DB Designer" }
  agent_spawn { type: "tester", name: "Test Engineer" }
  agent_spawn { type: "coordinator", name: "Lead" }
  ...
  // Start orchestration
  task_orchestrate { task: "Build REST API", strategy: "parallel" }

  // Store initial memory
  memory_usage { action: "store", key: "project/init", value: { started: Date.now() } }
...
Message 1: swarm_init
Message 2: agent_spawn (just one agent)
Message 3: agent_spawn (another agent)
Message 4: TodoWrite (single todo)
...
Remember: **Claude Flow coordinates, Claude Code creates!** Start with `swarm_init` to enhance your development workflow.
