...
export class PostInitValidator {
  ...
  async checkFileIntegrity() {
    ...
    const expectedFiles = [
      ...
      { path: 'memory/claude-zen-data.json', minSize: 10 },
      { path: 'memory/agents/README.md', minSize: 10 },
      { path: 'memory/sessions/README.md', minSize: 10 },
      { path: 'claude-zen', minSize: 50, executable: true },
    ];
    ...
    for (const file of expectedFiles) {
      ...
      try {
        ...
        if (file.executable && node.build.os !== 'windows') {
        ...
        try {
          await node.readTextFile(filePath);
  ...
  async checkPermissions() {
    ...
    const itemsToCheck = [
      ...
      { path: 'claude-zen', type: 'file', requiredMode: 0o755 },
  ...
  async validateSparcStructure() {
    ...
    // Check .roomodes file
    try {
      const stat = await node.stat(`${this.workingDir}/.roomodes`);
