...
export class BackupManager {
  constructor(workingDir) {
    ...
    this.backupDir = `${workingDir}/.claude-zen-backups`;
  }
  ...
  async createBackup(type = 'manual', description = '') {
    ...
    try {
      ...
      await node.mkdir(backupPath, { recursive: true });
      ...
      await node.writeTextFile(`${backupPath}/manifest.json`, JSON.stringify(manifest, null, 2));
      ...
      await node.writeTextFile(`${backupPath}/metadata.json`, JSON.stringify(metadata, null, 2));
  ...
  async deleteBackup(backupId) {
    ...
    try {
      const backupPath = `${this.backupDir}/${backupId}`;
      await node.remove(backupPath, { recursive: true });
  ...
  async ensureBackupDir() {
    try {
      await node.mkdir(this.backupDir, { recursive: true });
    } catch (error) {
      if (!(error instanceof node.errors.AlreadyExists)) {
        throw error;
      }
    }
  }
