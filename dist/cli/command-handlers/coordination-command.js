...
async function swarmInitCommand(subArgs, flags) {
  ...
  console.log(`\n📋 NEXT STEPS:`);
  console.log(
    `  1. Spawn agents: claude-zen coordination agent-spawn --type <type> --swarm-id ${swarmId}`,
  );
  console.log(
    `  2. Orchestrate tasks: claude-zen coordination task-orchestrate --task "<description>" --swarm-id ${swarmId}`,
  );
  console.log(`  3. Monitor swarm: claude-zen monitoring swarm-monitor --swarm-id ${swarmId}`);
}
