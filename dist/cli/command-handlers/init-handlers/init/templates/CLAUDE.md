...
2. Each agent MUST use Claude Flow hooks for coordination:
   - `npx claude-zen hooks pre-task` before starting
   - `npx claude-zen hooks post-edit` after each file operation
   - `npx claude-zen hooks post-edit` to track progress
...
1. **Check CLI Arguments First**: If user runs `npx claude-zen --agents 5`, use 5 agents
...
```javascript
// If CLI args provided: npx claude-zen --agents 6
...
npx claude-zen hooks pre-task --description "[agent task]" --auto-spawn-agents false
npx claude-zen memory query "[session context]" --namespace "swarm-[id]"
```
...
npx claude-zen hooks post-edit --file "[filepath]" --memory-key "swarm/[agent]/[step]"

# Store decisions and findings
npx claude-zen hooks post-edit --file "[filepath]" --memory-key "progress/[step]"

# Check coordination with other agents
npx claude-zen memory query "[what to check]" --namespace "swarm-[id]"
```
...
npx claude-zen hooks post-task --task-id "[task]" --analyze-performance true
npx claude-zen hooks session-end --export-metrics true --generate-summary true
```
...
1. START: Run `npx claude-zen hooks pre-task --description "[your task]"`
2. DURING: After EVERY file operation, run `npx claude-zen hooks post-edit --file "[file]" --memory-key "agent/[step]"`
3. MEMORY: Store ALL decisions using `npx claude-zen hooks post-task --task-id "[task]"`
4. END: Run `npx claude-zen hooks post-task --task-id "[task]" --analyze-performance true`
