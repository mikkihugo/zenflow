...
- \`./claude-zen start --parallel\` - Start with enhanced parallel processing
- \`./claude-zen status\` - Check system status
- \`./claude-zen status --concurrent\` - Check status with parallel monitoring
- \`./claude-zen monitor\` - Real-time monitoring
- \`./claude-zen monitor --performance\` - Enhanced performance monitoring
- \`./claude-zen stop\` - Stop orchestration

### 🤖 Agent Management (Parallel)
- \`./claude-zen agent spawn <type>\` - Create new agent
- \`./claude-zen agent batch-spawn <config>\` - Create multiple agents in parallel
- \`./claude-zen agent list\` - List active agents
- \`./claude-zen agent parallel-status\` - Check all agent status concurrently
- \`./claude-zen agent info <id>\` - Agent details
- \`./claude-zen agent terminate <id>\` - Stop agent
- \`./claude-zen agent batch-terminate <ids>\` - Stop multiple agents in parallel

### 📋 Task Management (Concurrent)
- \`./claude-zen task create <type> "description"\` - Create task
- \`./claude-zen task batch-create <tasks-file>\` - Create multiple tasks in parallel
- \`./claude-zen task list\` - List all tasks
- \`./claude-zen task parallel-status\` - Check all task status concurrently
- \`./claude-zen task status <id>\` - Task status
- \`./claude-zen task cancel <id>\` - Cancel task
- \`./claude-zen task batch-cancel <ids>\` - Cancel multiple tasks in parallel
- \`./claude-zen task workflow <file>\` - Execute workflow
- \`./claude-zen task parallel-workflow <files>\` - Execute multiple workflows concurrently

### 🧠 Memory Operations (Batch Enhanced)
- \`./claude-zen memory store "key" "value"\` - Store data
- \`./claude-zen memory batch-store <entries-file>\` - Store multiple entries in parallel
- \`./claude-zen memory query "search"\` - Search memory
- \`./claude-zen memory parallel-query <queries>\` - Execute multiple queries concurrently
- \`./claude-zen memory stats\` - Memory statistics
- \`./claude-zen memory stats --concurrent\` - Parallel memory analysis
- \`./claude-zen memory export <file>\` - Export memory
- \`./claude-zen memory concurrent-export <namespaces>\` - Export multiple namespaces in parallel
- \`./claude-zen memory import <file>\` - Import memory
- \`./claude-zen memory batch-import <files>\` - Import multiple files concurrently

### ⚡ SPARC Development (Optimized)
- \`./claude-zen sparc "task"\` - Run SPARC orchestrator
- \`./claude-zen sparc parallel "tasks"\` - Run multiple SPARC tasks concurrently
- \`./claude-zen sparc modes\` - List all 17+ SPARC modes
- \`./claude-zen sparc run <mode> "task"\` - Run specific mode
- \`./claude-zen sparc batch <modes> "task"\` - Run multiple modes in parallel
- \`./claude-zen sparc tdd "feature"\` - TDD workflow
- \`./claude-zen sparc concurrent-tdd <features>\` - Parallel TDD for multiple features
...
- \`./claude-zen swarm batch <tasks-config>\` - Multiple swarms in parallel
- \`./claude-zen swarm concurrent "tasks" --parallel\` - Concurrent swarm execution

### 🌍 MCP Integration (Parallel)
- \`./claude-zen mcp status\` - MCP server status
- \`./claude-zen mcp parallel-status\` - Check all MCP servers concurrently
- \`./claude-zen mcp tools\` - List available tools
- \`./claude-zen mcp config\` - Show configuration
- \`./claude-zen mcp logs\` - View MCP logs
- \`./claude-zen mcp batch-logs <servers>\` - View multiple server logs in parallel

### 🤖 Claude Integration (Enhanced)
- \`./claude-zen claude spawn "task"\` - Spawn Claude with enhanced guidance
- \`./claude-zen claude batch-spawn <tasks>\` - Spawn multiple Claude instances in parallel
- \`./claude-zen claude batch <file>\` - Execute workflow configuration

### 🚀 Batchtools Commands (New)
- \`./claude-zen batchtools status\` - Check batchtools system status
- \`./claude-zen batchtools monitor\` - Real-time performance monitoring
- \`./claude-zen batchtools optimize\` - System optimization recommendations
- \`./claude-zen batchtools benchmark\` - Performance benchmarking
- \`./claude-zen batchtools config\` - Batchtools configuration management
...
\`\`\`bash
./claude-zen swarm "Build REST API" --strategy development --monitor --review --parallel
\`\`\`

### Run concurrent TDD workflow:
\`\`\`bash
./claude-zen sparc concurrent-tdd "user authentication,payment processing,notification system"
\`\`\`

### Store project context with batch operations:
\`\`\`bash
./claude-zen memory batch-store "project-contexts.json" --namespace project --parallel
\`\`\`

### Spawn specialized agents in parallel:
\`\`\`bash
./claude-zen agent batch-spawn agents-config.json --parallel --validate
\`\`\`
...
./claude-zen monitor --performance --real-time

# Check resource utilization
./claude-zen batchtools monitor --resources --detailed

# Analyze operation bottlenecks
./claude-zen performance analyze --bottlenecks --optimization
\`\`\`

### Optimization Commands
\`\`\`bash
# Auto-optimize system configuration
./claude-zen batchtools optimize --auto-tune

# Performance benchmarking
./claude-zen batchtools benchmark --detailed --export

# System resource analysis
./claude-zen performance report --system --recommendations
\`\`\`

For comprehensive documentation and optimization guides, see the resources above.
`;
  
  await node.writeTextFile(`${workingDir}/.claude/commands/claude-zen-help.md`, helpCommand);

  await node.writeTextFile(`${workingDir}/.claude/commands/claude-zen-help.md`, helpCommand);
  console.log('  ✓ Created optimized slash command: /claude-zen-help (Batchtools enhanced)');
...
./claude-zen memory batch-store entries.json --parallel

# Store with concurrent validation
./claude-zen memory concurrent-store "multiple_keys" "values" --namespace arch --validate

# Bulk storage with optimization
./claude-zen memory bulk-store project-data/ --recursive --optimize --parallel
\`\`\`
...
./claude-zen memory parallel-query "auth,api,database" --concurrent

# Search across multiple namespaces simultaneously
./claude-zen memory concurrent-search "authentication" --namespaces arch,impl,test --parallel

# Batch query processing
./claude-zen memory batch-query queries.json --optimize --results-parallel
\`\`\`
...
./claude-zen memory stats --real-time --performance

# Concurrent analysis across all namespaces
./claude-zen memory concurrent-stats --all-namespaces --detailed

# Batch performance analysis
./claude-zen memory performance-stats --optimization --benchmarks
\`\`\`
...
./claude-zen memory concurrent-export namespaces.json --parallel --compress

# Batch import with validation
./claude-zen memory batch-import backups/ --validate --parallel

# Incremental export with optimization
./claude-zen memory incremental-export --since yesterday --optimize --concurrent
\`\`\`
...
./claude-zen memory concurrent-cleanup --namespaces temp,cache --days 7 --parallel

# Smart cleanup with optimization
./claude-zen memory smart-cleanup --auto-optimize --performance-based

# Batch maintenance operations
./claude-zen memory batch-maintenance --compress --reindex --parallel
\`\`\`
...
./claude-zen memory batch-store sparc-contexts.json --namespace sparc --parallel

# Concurrent storage across multiple namespaces
./claude-zen memory concurrent-store spec,arch,impl "project data" --parallel --validate

# Performance-optimized bulk storage
./claude-zen memory bulk-store project-data/ --optimize --concurrent --compress
\`\`\`

### Query project decisions with concurrent processing:
\`\`\`bash
# Parallel queries across multiple namespaces
./claude-zen memory parallel-query "authentication" --namespaces arch,impl,test --concurrent

# Batch query processing with optimization
./claude-zen memory batch-query project-queries.json --optimize --results-concurrent

# Real-time search with performance monitoring
./claude-zen memory concurrent-search "API design" --real-time --performance
\`\`\`

### Backup project memory with parallel processing:
\`\`\`bash
# Concurrent export with compression
./claude-zen memory concurrent-export project-$(date +%Y%m%d).json --namespace project --compress --parallel

# Batch backup with incremental processing
./claude-zen memory batch-backup --incremental --all-namespaces --optimize

# Performance-optimized full backup
./claude-zen memory parallel-backup --full --compress --validate --concurrent
\`\`\`
...
./claude-zen memory debug --performance --concurrent

# Analyze batch operation efficiency
./claude-zen memory analyze --batchtools --optimization

# Check parallel processing status
./claude-zen memory status --parallel --detailed
\`\`\`

### Optimization Commands
\`\`\`bash
# Optimize memory configuration
./claude-zen memory optimize --auto-tune --performance

# Benchmark memory operations
./claude-zen memory benchmark --all-operations --detailed

# Performance report generation
./claude-zen memory performance-report --detailed --recommendations
\`\`\`
...
`;
  ...
  await node.writeTextFile(`${workingDir}/.claude/commands/claude-zen-memory.md`, memoryCommand);

  await node.writeTextFile(`${workingDir}/.claude/commands/claude-zen-memory.md`, memoryCommand);
  console.log('  ✓ Created optimized slash command: /claude-zen-memory (Batchtools enhanced)');
...
`;
  ...
  await node.writeTextFile(`${workingDir}/.claude/commands/claude-zen-swarm.md`, swarmCommand);
  console.log('  ✓ Created optimized slash command: /claude-zen-swarm (Batchtools enhanced)');
}
