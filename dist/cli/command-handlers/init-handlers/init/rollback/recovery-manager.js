...
export class RecoveryManager {
  ...
  async recoverFromMemorySetupFailure(context) {
    ...
    try {
      ...
      for (const dir of memoryDirs) {
        try {
          await node.mkdir(`${this.workingDir}/${dir}`, { recursive: true });
      ...
      try {
        await node.writeTextFile(memoryDataPath, JSON.stringify(initialData, null, 2));
      ...
      for (const readme of readmeFiles) {
        try {
          await node.writeTextFile(`${this.workingDir}/${readme.path}`, readme.content);
  ...
  async cleanupTemporaryFiles() {
    ...
    const tempPatterns = ['*.tmp', '*.temp', '.claude-zen-*-test*'];
  ...
  async cleanupOldBackups() {
    ...
    try {
      const backupDir = `${this.workingDir}/.claude-zen-backups`;
  ...
  async verifyDependencies(dependencies) {
    ...
    for (const dep of dependencies) {
      try {
        const command = new node.Command(dep, {
          args: ['--version'],
          stdout: 'piped',
          stderr: 'piped',
        });
...
`;
    ...
    try {
      await node.writeTextFile(`${this.workingDir}/claude-zen`, executableContent);
    } catch {
      result.success = false;
    }
