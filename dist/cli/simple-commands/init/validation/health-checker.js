...
export class HealthChecker {
  ...
  async checkTemplateDirectory(dirPath) {
    ...
    try {
      for await (const entry of node.readDir(dirPath)) {
        if (entry.isFile) {
  ...
  async checkMemoryUsage() {
    ...
    try {
      ...
      const command = new node.Command('free', {
        args: ['-m'],
        stdout: 'piped',
      });
  ...
  async checkFileDescriptors() {
    ...
    try {
      ...
      const command = new node.Command('sh', {
        args: ['-c', 'lsof -p $$ | wc -l'],
        stdout: 'piped',
      });
  ...
  async checkProcessLimits() {
    ...
    try {
      const command = new node.Command('ulimit', {
        args: ['-a'],
        stdout: 'piped',
      });
