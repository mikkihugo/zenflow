...
#### Using CLI (Fallback)
```bash
npx claude-zen swarm "research AI trends" \
  --strategy research \
  --mode distributed \
  --max-agents 6 \
  --parallel
...
#### Using CLI (Fallback)
```bash
npx claude-zen swarm "build REST API" \
...
#### Using CLI (Fallback)
```bash
npx claude-zen swarm "analyze codebase" \
  --strategy analysis \
  --mode mesh \
  --parallel \
  --timeout 300
...
try {
  await mcp__claude-zen__task_orchestrate({
    "task": "complex operation",
    "strategy": "parallel"
  })
} catch (error) {
  // Check swarm health
  const status = await mcp__claude-zen__swarm_status({})
  
  // Log error patterns
  await mcp__claude-zen__error_analysis({
    "logs": [error.message]
  })
}
```
