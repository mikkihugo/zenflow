...
class DocumentValidator {
  ...
  async findDocumentFiles() {
    ...
    return files.filter(file => {
      ...
      return !relativePath.includes('README.md') && 
  ...
  validateMetadataHeader(filePath, content, docType) {
    ...
    if (statusMatch) {
      ...
      if (!validStatuses.includes(status)) {
        this.errors.push(`${filePath}: Invalid status '${status}'. Valid values: ${validStatuses.join(', ')}`);
      }
    }
  }
  ...
  validateVersionFormat(filePath, content) {
    ...
    if (versionMatch) {
      ...
      if (!semverPattern.test(version)) {
        this.errors.push(`${filePath}: Invalid version format '${version}'. Use semantic versioning (x.y.z)`);
      }
    }
  }
