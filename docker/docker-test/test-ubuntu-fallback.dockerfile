...
RUN echo "=== Testing Claude Flow SQLite Fallback in Ubuntu ===" && \
    ...
    npx -y claude-zen@2.0.0-alpha.51 --version && \
    echo "" && \
    echo "2. Testing init command:" && \
    npx -y claude-zen@2.0.0-alpha.51 init --force --project-name test && \
    echo "" && \
    echo "3. Checking created files:" && \
    ls -la && \
    echo "" && \
    echo "4. Testing memory operations:" && \
    npx -y claude-zen@2.0.0-alpha.51 memory store test-key "test-value" && \
    npx -y claude-zen@2.0.0-alpha.51 memory retrieve test-key && \
    echo "" && \
    echo "=== Test Complete ==="
