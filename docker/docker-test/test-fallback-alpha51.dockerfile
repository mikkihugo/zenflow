...
RUN echo "=== Testing Claude Flow alpha.51 SQLite Fallback ===" && \
    ...
    npx -y claude-zen@alpha init --force --project-name test-fallback && \
    echo "" && \
    ...
    npx -y claude-zen@alpha memory store test-key "test-value" --namespace test && \
    npx -y claude-zen@alpha memory retrieve test-key --namespace test && \
    echo "" && \
    echo "4. Testing hive-mind commands:" && \
    npx -y claude-zen@alpha hive-mind init && \
    echo "" && \
    echo "=== Test Complete ==="
