...
export class FACTIntegration extends EventEmitter {
     ...
     * @param version
     */
    async getDocumentation(framework, version) {
        const queries = {
        ...
        const query = queries[framework.toLowerCase()] ||
     ...
     * @param endpoint
     */
    async getAPIReference(api, endpoint) {
     ...
     * @param version
     */
    async getChangelog(project, version) {
        const query = version
     ...
     * @param tags
     */
    async searchCommunityKnowledge(topic, tags) {
    ...
    async verifyFACTRepository() {
        try {
            ...
            throw new Error(`FACT repository not found at ${this.config.factRepoPath}. Please clone it first: git clone https://github.com/ruvnet/FACT.git`);
        }
    }
    /**
     ...
     * @param factQuery
     */
    async executeFACTQuery(queryId, factQuery) {
        const command = {
            action: 'query',
            query_id: queryId,
 ...
 * @example
 */
export async function initializeFACT(config) {
 ...
 * @example
 */
export async function shutdownFACT() {
    if (globalFACTInstance) {
...
export const FACTHelpers = {
     ...
     * @param version
     */
    async getTypeScriptDocs(version) {
        const fact = getFACT();
        if (!fact)
            throw new Error('FACT not initialized');
     ...
     * @param tags
     */
    async searchSolutions(problem, tags) {
        const fact = getFACT();
        if (!fact)
            throw new Error('FACT not initialized');
