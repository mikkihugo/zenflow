...
export class ProjectContextAnalyzer extends EventEmitter {
     ...
     * @param context
     */
    async analyzeMonorepoStructure(context) {
        if (context.monorepo.type === 'none')
            return;
        try {
            switch (context.monorepo.type) {
                case 'lerna': {
                    const lernaConfig = JSON.parse(await readFile(path.join(context.rootPath, 'lerna.json'), 'utf-8'));
                    context.monorepo.version = lernaConfig?.version;
                    context.monorepo.packages = lernaConfig?.packages || ['packages/*'];
                    break;
                }
                case 'nx': {
                    const nxConfig = JSON.parse(await readFile(path.join(context.rootPath, 'nx.json'), 'utf-8'));
                    // NX has a more complex structure, we'd need to analyze workspace.json too
                    context.monorepo.version = nxConfig?.version;
                    break;
                }
                case 'pnpm': {
                    const pnpmWorkspace = await readFile(path.join(context.rootPath, 'pnpm-workspace.yaml'), 'utf-8');
                ...
                case 'rush': {
                    const rushConfig = JSON.parse(await readFile(path.join(context.rootPath, 'rush.json'), 'utf-8'));
                    context.monorepo.version = rushConfig?.rushVersion;
                    context.monorepo.packages = rushConfig?.projects?.map((p) => p.projectFolder) || [];
                    break;
                }
                case 'bazel': {
     ...
     */
    async analyzeBazelWorkspace(context) {
        try {
            ...
            context.monorepo.bazelMetadata = {
            ...
            logger.info('Bazel workspace analyzed', {
                ...
                queryUsed: context.monorepo.bazelMetadata.queryUsed,
            });
        }
        catch (error) {
     ...
     * @param rootPath
     */
    async parseBazelWorkspace(rootPath) {
        const result = {
     ...
     * @param rootPath
     */
    async discoverBazelTargets(rootPath) {
        const targets = [];
        try {
     ...
     * @param packagePath
     */
    parseBazelBuildFile(content, packagePath) {
        const targets = [];
     ...
     * @param startPos
     */
    extractBazelRuleBlock(content, startPos) {
        let depth = 0;
        let i = startPos;
     ...
     * @param packagePath
     */
    parseTargetAttributes(ruleBlock, ruleType, packagePath) {
        const target = {
            package: packagePath,
            name: '',
            type: ruleType,
     ...
     * @param targets
     */
    async analyzeBazelDependencies(rootPath, targets) {
        const dependencies = {};
        for (const target of targets) {
     ...
     * @param rootPath
     */
    async discoverBazelPackagesBasic(rootPath) {
        try {
            const { stdout } = await execAsync(`find "${rootPath}" -name "BUILD" -o -name "BUILD.bazel" 2>/dev/null`);
            const buildFiles = stdout.trim().split('\n').filter(Boolean);
    ...
    async runBazelQuery(rootPath) {
        try {
            ...
            const { stdout: targetsOutput } = await execAsync('bazel query "//..." --output=build 2>/dev/null', { cwd: rootPath, timeout: 30000 } // 30 second timeout
            );
            // Get dependency information
            const { stdout: depsOutput } = await execAsync('bazel query "deps(//...)" --output=graph 2>/dev/null', { cwd: rootPath, timeout: 30000 } // 30 second timeout
            );
     ...
     * @param buildOutput
     */
    parseBazelQueryTargets(buildOutput) {
        const targets = [];
        ...
        for (const block of ruleBlocks) {
            try {
                ...
                if (target.name) {
                    ...
                    const packagePath = packageMatch && packageMatch[1] ? packageMatch[1].replace('//', '') : '';
     ...
     * @param graphOutput
     */
    parseBazelQueryDependencies(graphOutput) {
        const dependencies = {};
     ...
     * @param targetLabel
     */
    extractPackageFromTarget(targetLabel) {
     ...
     * @param rootPath
     */
    async analyzeBzlFiles(rootPath) {
        const result = {
            bzlFiles: [],
            customRules: [],
            macros: [],
     ...
     * @param startIndex
     */
    extractStarlarkFunctionBlock(content, startIndex) {
     ...
     * @param functionBlock
     */
    extractStarlarkParameters(functionBlock) {
     ...
     * @param moduleContent
     */
    parseModuleBazel(moduleContent) {
        const result = {
            name: '',
            dependencies: [],
        };
     ...
     */
    extractLanguagesFromModuleDeps(dependencies, result) {
        const moduleLanguageMap = {
            rules_java: 'java',
            rules_python: 'python',
            rules_go: 'go',
            rules_kotlin: 'kotlin',
     ...
     */
    async generateKnowledgeMissions() {
     ...
     * @param priority
     */
    async executeMissions(priority) {
        ...
        try {
            ...
            switch (mission.type) {
                ...
                case 'api':
                    ...
                    break;
                case 'security':
                    result = await this.knowledgeSwarm.getSecurityGuidance(mission.target, mission.context.join(', '));
                    break;
                case 'performance':
                    result = await this.knowledgeSwarm.getPerformanceOptimization(mission.context.join(', '));
                    break;
                case 'best-practices':
                    result = await this.knowledgeSwarm.researchProblem(`Best practices for ${mission.target}`, mission.context);
                    break;
                default:
     ...
     * @param context
     */
    async analyzeLanguages(context) {
        try {
            // Use basic file counting - could be enhanced with more sophisticated analysis
            const { stdout } = await execAsync(`find "${context.rootPath}" -name "*.ts" -o -name "*.js" -o -name "*.tsx" -o -name "*.jsx" | wc -l`, { cwd: context.rootPath });
     ...
     * @param context
     */
    async analyzeCargoToml(context) {
        try {
     ...
     * @param context
     */
    async analyzeRequirementsTxt(context) {
        try {
            ...
            logger.warn('Error analyzing requirements.txt:', error instanceof Error ? error.message : error);
        }
    }
    /**
     ...
     * @param context
     */
    async detectAPIs(context) {
        // This could be enhanced to actually scan code for API usage patterns
        // For now, make educated guesses based on dependencies
        const apiDeps = context.dependencies.filter((dep) => dep.name.includes('api') || dep.name.includes('client') || dep.name.includes('sdk'));
     ...
     * @param framework
     */
    createFrameworkMission(framework) {
     ...
     * @param vulnDeps
     */
    createSecurityMission(vulnDeps) {
        return {
     ...
     * @param context
     */
    createAdHocMission(query, context) {
        return {
            id: `adhoc-${Date.now()}`,
            priority: 'high',
     ...
     * @param context
     */
    async searchCachedKnowledge(query, context) {
        try {
            const results = [];
            ...
            for (const [_key, cached] of this.contextCache.entries()) {
                ...
                if (context && cached.context) {
                    const hasMatchingContext = context.some((ctx) => cached.context.some((c) => c.toLowerCase().includes(ctx.toLowerCase())));
     ...
     * @param knowledge
     */
    formatCachedKnowledge(knowledge) {
     ...
     * @param result
     */
    async storeKnowledgeInCache(mission, result) {
        try {
     ...
     */
    isMonorepo(confidenceThreshold = 0.7) {
        const monorepo = this.getMonorepoInfo();
        return (monorepo !== null && monorepo.type !== 'none' && monorepo.confidence >= confidenceThreshold);
    }
    /**
     * Shutdown the system.
     */
