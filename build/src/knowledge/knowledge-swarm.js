...
export class KnowledgeSwarm extends EventEmitter {
    ...
    queryCounter = 0;
    vectorRepository;
    vectorDAO; // TODO: Add proper type
    ...
    selectIntelligent(candidates, query) {
        const scores = candidates.map((agent) => {
            ...
            const domainMatch = query.domains?.some((domain) => agent.specialization.domains.some((agentDomain) => domain.includes(agentDomain) || agentDomain.includes(domain))) || false;
            if (domainMatch)
                score += 50;
            // Priority weight
