...
export class ServiceManager extends EventEmitter {
    ...
    initializeMonitoring() {
        ...
        this.systemStatusInterval = setInterval(() => this.reportSystemStatus(), 60000 // Every minute
        );
    ...
    generateHealthAlerts(healthResults, metrics) {
        ...
        metrics.aggregatedMetrics.forEach((metric) => {
            if (metric.averageLatency > this.config.monitoring.performanceThresholds.responseTime) {
                alerts.push({
                    type: 'high-latency',
                    severity: metric.averageLatency > this.config.monitoring.performanceThresholds.responseTime * 2
                        ? 'error'
                        : 'warning',
