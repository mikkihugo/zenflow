...
export class InfrastructureServiceAdapter {
    ...
    async getEventStats() {
        const recentEvents = this.eventQueue.filter((entry) => Date.now() - entry.timestamp.getTime() < 300000 // Last 5 minutes
        );
        return {
    ...
    calculateEventProcessingRate() {
        const recentEvents = this.eventQueue.filter((entry) => Date.now() - entry.timestamp.getTime() < 60000 // Last minute
        );
