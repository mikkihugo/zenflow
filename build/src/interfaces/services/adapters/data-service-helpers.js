...
export class DataServiceHelper {
        ...
        return data.filter((item) => {
            return Object.entries(filters).every(([key, value]) => {
                ...
                if (Array.isArray(value)) {
                    ...
                    if (rangeValue.min !== undefined && typedItem[key] < rangeValue.min)
                        return false;
                    if (rangeValue.max !== undefined && typedItem[key] > rangeValue.max)
                        return false;
                    return true;
                }
                else {
    ...
    groupByMultipleFields(data, fields) {
        ...
        return Object.entries(groups).map(([key, items]) => {
            ...
            const groupData = { items: typedItems, count: typedItems.length };
