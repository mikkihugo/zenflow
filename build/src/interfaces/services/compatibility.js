...
export class USLCompatibilityLayer {
    ...
    logger;
    config;
    uslInstance; // USL instance for fallback operations
    migrationLog = [];
    constructor(config) {
    ...
    async migratePatternIntegrationToUSL() {
        ...
        try {
            ...
            const patternService = await this.serviceManager.createPatternIntegrationService('enhanced-pattern-integration', 'production', // Use production profile for enhanced features
            {
                patternIntegration: {
                    enabled: true,
