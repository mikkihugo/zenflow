...
export class UnifiedPerformanceDashboard extends EventEmitter {
    ...
    assessSystemHealth(mcpMetrics, memoryStats, dbStats, clientMetrics) {
        ...
        // Check memory health
        const memoryHealth = this.assessComponentHealth(0, // No latency for memory
        ...
        const dbHealth = this.assessComponentHealth(dbStats.averageSearchTime, 0, // No error rate available
        'database');
    ...
    displayConsoleStatus(status) {
        // Overall health.
        const _healthEmoji = status.health.overall === 'healthy' ? '✅' : status.health.overall === 'warning' ? '⚠️' : '❌';
        // Alerts
