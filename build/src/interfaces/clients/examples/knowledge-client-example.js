...
export async function example1_CreateFACTClient() {
    try {
        ...
        const knowledgeClient = await createFACTClient('./FACT', // FACT repository path
        (() => {
...
export async function example2_CreateWithFactory() {
    try {
        // Create factory
        const factory = new UACLFactory(console, // Simple logger
        {} // Config
        );
...
export async function example6_ErrorHandling() {
    try {
        ...
        const knowledgeClient = await createFACTClient('./INVALID_FACT_PATH', // Invalid path
