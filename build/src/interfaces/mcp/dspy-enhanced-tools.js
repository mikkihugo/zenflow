...
export class DSPyEnhancedMCPTools {
     ...
     */
    getToolStats() {
        const recentUsage = this.toolUsageHistory.filter((usage) => Date.now() - usage.timestamp.getTime() < 3600000 // Last hour
        );
    ...
    assessErrorSeverity(errorMessage) {
        if (errorMessage.includes('Cannot find module') || errorMessage.includes('ReferenceError'))
            return 'high';
        if (errorMessage.includes('TypeError') || errorMessage.includes('SyntaxError'))
            return 'medium';
        if (errorMessage.includes('Property') && errorMessage.includes('does not exist'))
            return 'medium';
        return 'low';
