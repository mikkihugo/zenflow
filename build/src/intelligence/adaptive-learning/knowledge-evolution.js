...
export class KnowledgeEvolution extends EventEmitter {
    ...
    async detectAntiPatterns(patterns, behaviors) {
        ...
        const failurePatterns = patterns.filter((p) => p.metadata.success === false || p.confidence < this.config.antiPatternThreshold);
    ...
    async detectBestPractices(patterns, _behaviors) {
        ...
        const highPerformancePatterns = patterns.filter((p) => p.confidence >= this.config.bestPracticeThreshold && p.metadata.success === true);
    ...
    extractEmergenceContext(patterns) {
        ...
        if (patterns.length > 0) {
            ...
            const avgComplexity = patterns.reduce((sum, p) => sum + (p.context.complexity || 0), 0) /
                patterns.length;
    ...
    recalculateConfidence(item) {
        const avgEvidenceStrength = item?.evidence.reduce((sum, e) => sum + e.strength, 0) / item?.evidence.length;
