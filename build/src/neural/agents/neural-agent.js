...
class NeuralAgent extends EventEmitter {
    ...
    constructor(agent, agentType, memoryOptimizer = null) {
        ...
        this.memoryOptimizer =
            memoryOptimizer || (MemoryOptimizer ? new MemoryOptimizer() : null);
    ...
    _taskToVector(task) {
        ...
        vector.push(description.length / 1000, // Length normalized
        ...
        const inputSize = this.neuralNetwork.layers?.[0] || 10; // Fallback to 10 if layers not accessible
    ...
    _updateCognitiveState(analysis) {
        ...
        if (this.learningHistory.length > 0) {
            const recentPerformance = this.learningHistory.slice(-5).reduce((sum, h) => sum + h.performance, 0) /
...
class NeuralAgentFactory {
    ...
    static async initializeFactory() {
        if (!NeuralAgentFactory.memoryOptimizer) {
            NeuralAgentFactory.memoryOptimizer = MemoryOptimizer ? new MemoryOptimizer() : null;
            if (NeuralAgentFactory.memoryOptimizer) {
