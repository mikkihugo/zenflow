...
export class MetricsCollector extends EventEmitter {
    ...
    async collectMcpToolMetrics() {
        ...
        return {
            ...
            performance: {
                ...
                overallSuccessRate: Object.values(tools).reduce((sum, tool) => sum + tool.successRate, 0) /
                    Object.keys(tools).length,
                averageResponseTime: Object.values(tools).reduce((sum, tool) => sum + tool.averageLatency, 0) /
                    Object.keys(tools).length,
    ...
    getHistory(timeRange) {
        ...
        return this.metricsHistory.filter((metrics) => metrics.system.timestamp >= timeRange.start && metrics.system.timestamp <= timeRange.end);
    }
    /**
     * Get latest metrics.
     */
