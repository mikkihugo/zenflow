...
export class PerformanceAnalyzer extends EventEmitter {
    ...
    detectAnomalies(metrics) {
        ...
        this.checkAnomaly(anomalies, 'memory_percentage', metrics.system.memory.percentage, 'System memory usage');
        // FACT anomalies
        this.checkAnomaly(anomalies, 'fact_cache_hit_rate', metrics.fact.cache.hitRate, 'FACT cache hit rate');
        this.checkAnomaly(anomalies, 'fact_query_time', metrics.fact.queries.averageQueryTime, 'FACT query time');
        // RAG anomalies
        this.checkAnomaly(anomalies, 'rag_query_latency', metrics.rag.vectors.queryLatency, 'RAG query latency');
        this.checkAnomaly(anomalies, 'rag_retrieval_time', metrics.rag.retrieval.averageRetrievalTime, 'RAG retrieval time');
        // Swarm anomalies
        this.checkAnomaly(anomalies, 'swarm_consensus_time', metrics.swarm.coordination.consensusTime, 'Swarm consensus time');
        this.checkAnomaly(anomalies, 'swarm_task_time', metrics.swarm.tasks.averageTaskTime, 'Swarm task time');
        // MCP anomalies
        this.checkAnomaly(anomalies, 'mcp_success_rate', metrics.mcp.performance.overallSuccessRate, 'MCP success rate');
        this.checkAnomaly(anomalies, 'mcp_response_time', metrics.mcp.performance.averageResponseTime, 'MCP response time');
        return anomalies;
    }
    /**
     ...
     * @param description
     */
    checkAnomaly(anomalies, metricName, value, description) {
    ...
    analyzeTrends() {
        ...
        trends.push(this.analyzeTrend('cpu_usage', recentMetrics.map((m) => m.system.cpu.usage)));
        // Analyze memory usage trend
        trends.push(this.analyzeTrend('memory_percentage', recentMetrics.map((m) => m.system.memory.percentage)));
        // Analyze FACT cache hit rate trend
        trends.push(this.analyzeTrend('fact_cache_hit_rate', recentMetrics.map((m) => m.fact.cache.hitRate)));
        // Analyze RAG query latency trend
        trends.push(this.analyzeTrend('rag_query_latency', recentMetrics.map((m) => m.rag.vectors.queryLatency)));
        // Analyze swarm performance trend
        trends.push(this.analyzeTrend('swarm_active_agents', recentMetrics.map((m) => m.swarm.agents.activeAgents)));
        return trends;
    }
    /**
    ...
    calculateHealthScore(metrics) {
        ...
        // System health (0-100)
        const systemHealth = Math.max(0, 100 - metrics.system.cpu.usage) *
            Math.max(0, (100 - metrics.system.memory.percentage) / 100);
        // FACT health (0-100)
        const factHealth = metrics.fact.cache.hitRate * 100 * Math.max(0, 1 - metrics.fact.queries.errorRate);
        // RAG health (0-100)
        const ragHealth = Math.max(0, 100 - metrics.rag.vectors.queryLatency) *
            metrics.rag.retrieval.retrievalAccuracy *
            100;
        // Swarm health (0-100)
        const swarmHealth = (metrics.swarm.agents.activeAgents / metrics.swarm.agents.totalAgents) *
            100 *
            metrics.swarm.load.balancingEfficiency;
        // MCP health (0-100)
        const mcpHealth = metrics.mcp.performance.overallSuccessRate * 100;
        const overallHealth = systemHealth * weights.system +
    ...
    generatePredictions() {
        ...
        const cpuTrend = this.analyzeTrend('cpu_usage', recentMetrics.map((m) => m.system.cpu.usage));
        if (cpuTrend.direction === 'increasing' && cpuTrend.rate > 0) {
            const timeToCapacity = (90 - recentMetrics[recentMetrics.length - 1]?.system?.cpu?.usage) / cpuTrend.rate;
        ...
        const memoryTrend = this.analyzeTrend('memory_percentage', recentMetrics.map((m) => m.system.memory.percentage));
        if (memoryTrend.direction === 'increasing' && memoryTrend.rate > 0) {
            const timeToCapacity = (90 - recentMetrics[recentMetrics.length - 1]?.system?.memory?.percentage) /
                memoryTrend.rate;
        ...
        const capacityUtilization = Math.max(latestMetrics.system.cpu.usage, latestMetrics.system.memory.percentage);
        return {
            capacityUtilization,
    ...
    getHistoricalInsights(timeRange) {
        const relevantMetrics = timeRange
            ? this.metricsHistory.filter((m) => m.system.timestamp >= timeRange.start && m.system.timestamp <= timeRange.end)
            : this.metricsHistory;
    ...
    updateBaselines() {
        ...
        const avgMemory = recentMetrics.reduce((sum, m) => sum + m.system.memory.percentage, 0) / recentMetrics.length;
