...
export class AutoSwarmFactory extends EventEmitter {
    ...
    async analyzeDomainCharacteristics(domain) {
        ...
        const hasNestedStructure = concepts.some((c) => c.includes('nested') || c.includes('hierarchical') || c.includes('tree')) || fileCount > 50;
    ...
    async validateResourceConstraints(configs) {
        const totalAgents = configs.reduce((sum, config) => sum + config?.agents.reduce((agentSum, agent) => agentSum + agent.count, 0), 0);
    ...
    getSwarmStatistics() {
        ...
        const totalAgents = swarms.reduce((sum, swarm) => sum + swarm.agents.reduce((agentSum, agent) => agentSum + agent.count, 0), 0);
