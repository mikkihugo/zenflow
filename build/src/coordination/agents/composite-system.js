...
export class Agent extends EventEmitter {
    ...
    constructor(id, name, initialCapabilities = [], resourceLimits = { cpu: 1.0, memory: 1024, network: 100, storage: 1024 }) {
        super();
        this.id = id;
        this.name = name;
    ...
    canHandleTask(task) {
        ...
        const hasCapabilities = task.requiredCapabilities.every((reqCap) => this.capabilities.has(reqCap));
        if (!hasCapabilities)
            return false;
...
export class AgentGroup extends EventEmitter {
    ...
    selectByCapability(eligibleMembers, task) {
        ...
        return eligibleMembers.reduce((best, current) => {
            ...
            const bestMatches = task.requiredCapabilities.filter((req) => bestCapabilities.some((cap) => cap.name === req)).length;
            const currentMatches = task.requiredCapabilities.filter((req) => currentCapabilities?.some((cap) => cap.name === req)).length;
    ...
    aggregateResources(resourcesList) {
        return resourcesList.reduce((total, resources) => ({
