...
export class ConnectionStateManager extends EventEmitter {
    options;
    connections;
    connectionStats; // Initialized in constructor
    healthChecks; // Initialized in constructor
    persistenceManager;
    fallbackManager;
    logger;
    connectionHealth;
    ...
    async initialize() {
        try {
            ...
            const managerError = ErrorFactory.createError('resource', 'Failed to initialize connection state manager', {
    ...
    scheduleReconnection(connectionId) {
        ...
        const delay = Math.min((this.options.reconnectDelay || 1000) * 2 ** connection.reconnectAttempts, this.options.maxReconnectDelay || 30000);
    ...
    getConnectionStats() {
        return {
            ...
            healthyConnections: Array.from(this.connections.values()).filter((c) => c.health.status === 'healthy').length,
    ...
    async restorePersistedConnections() {
      ...
      `);
            const connections = await this.persistence.pool.read('SELECT * FROM mcp_connections WHERE status IN (?, ?)', ['connected', 'reconnecting']);
    ...
    async shutdown() {
        ...
        const disconnectPromises = Array.from(this.connections.keys()).map((connectionId) => this.disconnectConnection(connectionId, 'System shutdown').catch((error) => this.logger.warn(`Error disconnecting ${connectionId}`, { error: error.message })));
