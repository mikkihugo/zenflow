 ...
 * @example
 */
export function getDefaultCognitiveProfile(type) {
 ...
 * @example
 */
export function calculateCognitiveDiversity(profile1, profile2) {
 ...
 * @example
 */
export function recommendTopology(agentCount, taskComplexity, coordinationNeeds) {
    if (agentCount <= 5) {
        return 'mesh';
    }
 ...
 * @example
 */
export function validateSwarmOptions(options) {
    const errors = [];
    ...
    if (options?.connectionDensity !== undefined) {
        if (typeof options?.connectionDensity !== 'number' ||
