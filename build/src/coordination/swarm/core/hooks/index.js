...
`;
    ...
    calculateSessionMetrics() {
        ...
        return {
            ...
            learning: {
                ...
                average_improvement: (this.sessionData.learnings.reduce((acc, l) => acc + l.improvement, 0) /
    ...
    async updateKnowledgeGraph(file, operation) {
        ...
        relatedFiles.forEach((related) => {
            if (!graph.edges.find((e) => (e.from === nodeId && e.to === related) || (e.from === related && e.to === nodeId))) {
                graph.edges.push({
                    from: nodeId,
                    to: related,
                    type: 'related',
    ...
    async getSwarmStatus() {
        ...
        return {
            ...
            activeTasks: this.sessionData.operations.filter((op) => Date.now() - op.timestamp < 300000 // Last 5 minutes
            ).length,
            health: 'operational',
        };
    }
