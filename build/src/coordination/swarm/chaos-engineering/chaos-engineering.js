...
export class ChaosEngineering extends EventEmitter {
    ...
    async shutdown() {
        ...
        // Cancel all active experiments
        const cancelPromises = Array.from(this.activeExperiments.keys()).map((executionId) => this.cancelExperiment(executionId, 'System shutdown').catch((error) => this.logger.warn(`Error cancelling experiment ${executionId}`, { error: error.message })));
        await Promise.allSettled(cancelPromises);
        // Clear all data
