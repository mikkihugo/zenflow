...
class EnhancedMCPTools {
    ...
    async swarm_init(params) {
        ...
        try {
            ...
            if (error.message.includes('WASM') || error.message.includes('module')) {
                handledError = ErrorFactory.createError('wasm', `WASM module error during swarm initialization: ${error.message}`, { operation: 'swarm_init', topology: params?.topology, originalError: error });
            }
            else if (error.message.includes('memory') || error.message.includes('allocation')) {
                handledError = ErrorFactory.createError('resource', `Insufficient resources for swarm initialization: ${error.message}`, { resourceType: 'memory', operation: 'swarm_init', maxAgents: params?.maxAgents });
            }
    ...
    async agent_spawn(params) {
        ...
        try {
            ...
            if (error.message.includes('neural') || error.message.includes('network')) {
                handledError = ErrorFactory.createError('neural', `Neural network error during agent spawn: ${error.message}`, { operation: 'agent_spawn', agentType: params?.type, originalError: error });
            }
            else if (error.message.includes('capabilities') || error.message.includes('mismatch')) {
                handledError = ErrorFactory.createError('agent', `Agent capability error: ${error.message}`, { operation: 'agent_spawn', agentType: params?.type, capabilities: params?.capabilities });
            }
            else if (error.message.includes('database') || error.message.includes('persistence')) {
                handledError = ErrorFactory.createError('persistence', `Database error during agent creation: ${error.message}`, { operation: 'create_agent', agentType: params?.type, originalError: error });
            }
            else if (!(error instanceof ValidationError || error instanceof ZenSwarmError)) {
    ...
    async task_orchestrate(params) {
        ...
        try {
            ...
            if (error.message.includes('swarm') && error.message.includes('not found')) {
                handledError = ErrorFactory.createError('swarm', `Swarm not found for task orchestration: ${error.message}`, { operation: 'task_orchestrate', swarmId: params?.swarmId, originalError: error });
            }
                ...
                handledError = ErrorFactory.createError('task', `Task orchestration failed: ${error.message}`, { operation: 'task_orchestrate', task: params?.task, originalError: error });
            }
    ...
    async benchmark_run(params) {
        const startTime = performance.now();
        try {
            const { type = 'all', iterations = 10, 
            // includeWasmBenchmarks = true,
            includeNeuralBenchmarks = true, includeSwarmBenchmarks = true, } = params;
            const benchmarks = {};
    ...
    async neural_train(params) {
        ...
        try {
            ...
            if (!agentId || typeof agentId !== 'string') {
                throw ErrorFactory.createError('validation', 'agentId is required and must be a string. The agentId parameter identifies which agent to train.', { parameter: 'agentId', providedValue: String(agentId), expectedType: 'string' });
            }
            let iterations;
            try {
                ...
                if (iterations > 100) {
                    throw ErrorFactory.createError('validation', 'iterations must not exceed 100 to prevent excessive training time', { parameter: 'iterations', providedValue: String(iterations), validRange: '1-100' });
                }
            }
            catch (parseError) {
    ...
    async runWasmBenchmarks(iterations) {
        ...
        results.neural_networks = {
            ...
            operations_per_second: neuralNetworkTimes.length > 0
                ? Math.round(1000 / (neuralNetworkTimes.reduce((a, b) => a + b, 0) / neuralNetworkTimes.length))
                : 0,
        };
        results.forecasting = {
            ...
            predictions_per_second: forecastingTimes.length > 0
                ? Math.round(1000 / (forecastingTimes.reduce((a, b) => a + b, 0) / forecastingTimes.length))
                : 0,
        };
        results.swarm_operations = {
            ...
            operations_per_second: swarmOperationTimes.length > 0
                ? Math.round(1000 / (swarmOperationTimes.reduce((a, b) => a + b, 0) / swarmOperationTimes.length))
                : 0,
        };
    ...
    async agent_metrics(params) {
        ...
        try {
            ...
            const result = {
                ...
                agents: metricsData,
                summary: {
                    avg_performance: metricsData.reduce((sum, a) => sum + (a.performance?.accuracy_score || 0), 0) /
                        metricsData.length,
            ...
            return {
                ...
                persistence_metrics: {
                    ...
                    error_rate: persistenceStats.totalOperations > 0
                        ? `${((persistenceStats.totalErrors / persistenceStats.totalOperations) * 100).toFixed(2)}%`
                        : '0%',
                },
                health_status: {
