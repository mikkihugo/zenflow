...
export class TaskDistributionEngine extends EventEmitter {
    ...
    async processQueue() {
        const availableAgents = Array.from(this.agentCapabilities.values()).filter((agent) => agent.availability.currentStatus === 'available' && agent.currentLoad < agent.maxLoad);
        ...
        for (const task of tasksToAssign) {
            try {
                ...
                await this.handleTaskFailure({ taskId: task.id, error: error });
            }
        }
    }
    ...
    isAgentSuitable(agent, task) {
        ...
        if (agent.trustScore < 0.5)
            return false; // Minimum trust threshold
        // Check resource requirements (simplified)
        const canHandleResources = task.requirements.resourceRequirements.cpu <= 1.0 && // Assume normalized values
    ...
    async findAlternativeAgents(task, primaryAgent) {
        const alternatives = Array.from(this.agentCapabilities.values())
            .filter((agent) => agent.agentId !== primaryAgent.agentId && this.isAgentSuitable(agent, task))
            .sort((a, b) => {
    ...
    calculateAgentScore(task, agent) {
        ...
        return (capabilityScore * 0.3 + performanceScore * 0.3 + availabilityScore * 0.2 + trustScore * 0.2);
    }
    ...
    calculateLoadBalance() {
        ...
        const variance = utilizations.reduce((sum, util) => sum + (util - avg) ** 2, 0) / utilizations.length;
    ...
    detectLoadImbalance() {
        ...
        const avg = utilizations.reduce((sum, { utilization }) => sum + utilization, 0) / utilizations.length;
