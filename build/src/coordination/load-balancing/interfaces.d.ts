 ...
 */
import type { Agent, CapacityMetrics, LoadMetrics, PredictionModel, QoSRequirement, RoutingResult, Task } from './types.ts';
...
export interface ResourceMonitor {
    ...
    getHistoricalMetrics(agentId: string, timeRange: {
        start: Date;
        end: Date;
    }): Promise<LoadMetrics[]>;
...
export interface HealthChecker {
    ...
    getHealthStatus(agentId: string): Promise<{
        healthy: boolean;
        lastCheck: Date;
        details?: string;
    }>;
...
export interface ConnectionPool {
    ...
    getPoolStats(agentId: string): Promise<{
        active: number;
        idle: number;
        total: number;
    }>;
...
export interface AutoScaler {
    ...
    getScalingHistory(): Promise<Array<{
        timestamp: Date;
        action: string;
        reason: string;
    }>>;
...
export interface MetricsAggregator {
    ...
    calculateTrends(metrics: LoadMetrics[], timeWindow: number): Promise<Record<string, 'up' | 'down' | 'stable'>>;
    detectAnomalies(metrics: LoadMetrics[]): Promise<Array<{
        timestamp: Date;
        metric: string;
        value: number;
        expected: number;
