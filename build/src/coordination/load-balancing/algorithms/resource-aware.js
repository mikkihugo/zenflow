...
export class ResourceAwareAlgorithm {
    ...
    canHandleTask(profile, requirements) {
        ...
        const networkAvailable = profile.capacityLimits.maxNetworkUtilization - profile.network.utilization;
    ...
    applyConstraintPenalties(score, profile) {
        ...
        if (profile.cpu.constraint && profile.cpu.constraint.severity === 'critical')
            penalty *= 0.1;
    ...
    countConstraintViolations(profiles) {
        ...
        for (const profile of profiles) {
            if (profile.cpu.constraint && profile.cpu.constraint.severity !== 'low')
                violations++;
            if (profile.memory.constraint && profile.memory.constraint.severity !== 'low')
                violations++;
            if (profile.disk.constraint && profile.disk.constraint.severity !== 'low')
                violations++;
            if (profile.network.constraint && profile.network.constraint.severity !== 'low')
                violations++;
        }
        return violations;
    }
