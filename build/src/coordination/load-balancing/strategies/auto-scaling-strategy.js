...
export class AutoScalingStrategy extends EventEmitter {
    ...
    async makeScalingDecision(metrics) {
        ...
        if (avgUtilization > this.config.scaleUpThreshold || maxUtilization > 0.95) {
            if (agentCount < this.config.maxAgents) {
                const targetCount = Math.min(this.config.maxAgents, Math.ceil(agentCount * 1.5) // Scale by 50%
                );
                return {
                    action: 'scale_up',
                    targetCount,
        ...
        if (avgUtilization < this.config.scaleDownThreshold && maxUtilization < 0.6) {
            if (agentCount > this.config.minAgents) {
                const targetCount = Math.max(this.config.minAgents, Math.floor(agentCount * 0.8) // Scale down by 20%
                );
                return {
                    action: 'scale_down',
                    targetCount,
    ...
    calculateSystemLoad(metrics) {
        ...
        const avgUtilization = cpuUtilizations.reduce((sum, cpu) => sum + cpu, 0) / cpuUtilizations.length;
