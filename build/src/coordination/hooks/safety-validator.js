 ...
 */
export class BashSafetyValidator {
    DANGEROUS_PATTERNS = [
        {
            pattern: /rm\s+-rf\s+\//,
    ...
    async suggestSaferAlternative(command) {
        const alternatives = [];
        ...
        if (command.includes('rm -rf')) {
            alternatives.push('Use rm with specific files instead of -rf', 'Use trash command for recoverable deletion', 'Use find with -delete for controlled deletion');
        }
        if (command.includes('curl') && command.includes('| sh')) {
            alternatives.push('Download script first: curl -O <url>', 'Review downloaded script before execution', 'Use package manager instead if available');
        }
        if (command.includes('chmod 777')) {
            alternatives.push('Use chmod 755 for executables', 'Use chmod 644 for regular files', 'Set specific user/group permissions');
        }
        if (command.includes('sudo') && command.includes('--preserve-env')) {
            alternatives.push('Use sudo with specific -E variables', 'Use sudo without environment preservation', 'Configure sudoers file for specific permissions');
        }
        return alternatives;
    }
    async assessRiskLevel(operation) {
    ...
    generateMitigations(risks) {
        return risks
            ...
            .filter((mitigation) => mitigation !== undefined);
    }
}
// File Operation Validator
export class FileOperationValidator {
    RESTRICTED_EXTENSIONS = ['.exe', '.bat', '.cmd', '.com', '.scr', '.pif'];
    SENSITIVE_FILENAMES = ['passwd', 'shadow', 'hosts', 'fstab', 'sudoers'];
    async validateFileAccess(path, operation) {
        const risks = [];
...
export class SecurityRiskAssessment {
    ...
    static generateSecurityReport(risks) {
        if (risks.length === 0) {
