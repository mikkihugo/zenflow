...
export class IntelligentAgentAssignor {
    ...
    async getHistoricalPerformanceScore(agentId, analysis) {
        ...
        const successRate = relevantHistory.filter((record) => record.success).length / relevantHistory.length;
        return successRate;
    }
    ...
    calculateProjectedEfficiency(workloads, recommendations) {
        ...
        const currentEfficiency = workloads.reduce((sum, wl) => sum + Math.min(wl.utilization, 1), 0) / workloads.length;
        const improvementPotential = recommendations.reduce((sum, rec) => sum + Math.abs(rec.impact), 0) / recommendations.length;
