...
export class DynamicResourceManager extends EventEmitter {
    ...
    calculateExperienceMatch(agent, demand) {
        ...
        if (relevantHistory.length === 0)
            return 0.3;
        const avgQuality = relevantHistory.reduce((sum, perf) => sum + perf.quality, 0) / relevantHistory.length;
        const avgEfficiency = relevantHistory.reduce((sum, perf) => sum + perf.efficiency, 0) / relevantHistory.length;
    ...
    createResourceTransfer(agent, fromLevel, toLevel, demand, rule) {
        ...
        return {
            ...
            startTime: new Date(),
            expectedReturnTime: new Date(Date.now() + Math.min(demand.duration, rule.durationLimit) * 60 * 60 * 1000),
    ...
    async allocateBySkills(demand) {
        try {
            ...
            if (matchedAgents.length === 0) {
                ...
                skillAllocation.learningOpportunities = this.generateLearningOpportunities(skillAllocation.skillGapAnalysis);
    ...
    calculateBufferAdjustments(buffers) {
        return buffers
            ...
            .map((buffer) => {
            const adjustmentType = buffer.status === 'critical' || buffer.status === 'low' ? 'increase' : 'decrease';
