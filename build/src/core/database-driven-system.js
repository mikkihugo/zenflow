...
export class DatabaseDrivenSystem extends EventEmitter {
     ...
     * @param options
     */
    async processDocumentEntity(workspaceId, document, options = {}) {
        ...
        if (options?.startWorkflows !== false) {
            const workflowIds = await this.workflowEngine.processDocumentEvent('document:created', document);
     ...
     * @param options
     */
    async createVisionDocument(workspaceId, visionSpec, options = {}) {
        const context = this.workspaces.get(workspaceId);
        if (!context) {
            throw new Error(`Workspace not found: ${workspaceId}`);
        }
     ...
     * @param options
     */
    async generateDocumentsFromSource(workspaceId, sourceDocumentId, targetType, options = {}) {
        const context = this.workspaces.get(workspaceId);
        if (!context) {
            throw new Error(`Workspace not found: ${workspaceId}`);
        }
        ...
        const generatedDocs = [];
        if (targetType === 'prd') {
            generatedDocs.push(...(await this.generatePRDsFromVision(sourceDoc, context)));
        }
        else if (targetType === 'epic') {
            generatedDocs.push(...(await this.generateEpicsFromPRD(sourceDoc, context)));
        }
        else if (targetType === 'feature') {
            generatedDocs.push(...(await this.generateFeaturesFromEpic(sourceDoc, context)));
        }
        else if (targetType === 'task') {
            generatedDocs.push(...(await this.generateTasksFromFeature(sourceDoc, context)));
        }
        else {
        ...
        logger.info(`🔧 Generated ${generatedDocs.length} ${targetType} documents from ${sourceDoc.type}`);
    ...
    async getWorkspaceStatus(workspaceId) {
        ...
        const completedDocs = allDocs.filter((doc) => doc.status === 'approved' || doc.completion_percentage === 100).length;
        const overallProgress = allDocs.length > 0 ? Math.round((completedDocs / allDocs.length) * 100) : 0;
        return {
            workspace: context,
            project: project,
            documents: {
     ...
     * @param _format
     */
    async exportWorkspaceToFiles(workspaceId, outputPath, _format = 'markdown') {
        const context = this.workspaces.get(workspaceId);
        if (!context) {
            throw new Error(`Workspace not found: ${workspaceId}`);
        }
        ...
        logger.info(`📊 Documents to export: ${JSON.stringify({
    ...
    async handleWorkspaceCreated(event) {
        logger.debug(`Workspace created: ${event.workspaceId}`);
    }
    async handleWorkspaceLoaded(event) {
    ...
    generateVisionContent(spec) {
        return `# ${spec.title}

## Business Objectives
${spec.businessObjectives.map((obj) => `- ${obj}`).join('\n')}

## Success Criteria
${spec.successCriteria.map((crit) => `- ${crit}`).join('\n')}

## Stakeholders
${spec.stakeholders.map((sh) => `- ${sh}`).join('\n')}
...
### Milestones
${spec.timeline.milestones?.map((m) => `- **${m.name}** (${m.date.toISOString().split('T')[0]}): ${m.description}`).join('\n') || 'No milestones defined'}
`
...
*Generated by Database-Driven Development System*`;
    ...
    async generatePRDsFromVision(vision, context) {
        const prdDoc = {
            type: 'prd',
...
*Generated from vision: ${vision.title}*`,
    ...
    async generateEpicsFromPRD(prd, context) {
        // Group functional requirements into epics
        const epics = [
            {
                title: 'User Management Epic',
                requirements: prd.functional_requirements.slice(0, Math.ceil(prd.functional_requirements.length / 2)),
                businessValue: 'Enable user registration, authentication, and profile management',
            },
            {
                title: 'Core Functionality Epic',
                requirements: prd.functional_requirements.slice(Math.ceil(prd.functional_requirements.length / 2)),
...
*Generated from PRD: ${prd.title}*`,
                ...
                timeline: {
                    start_date: new Date(),
                    estimated_completion: new Date(Date.now() + Math.ceil(epicSpec.requirements.length / 2) * 7 * 24 * 60 * 60 * 1000),
                },
    ...
    async generateFeaturesFromEpic(epic, context) {
...
*Generated from epic: ${epic.title}*`,
    ...
    async generateTasksFromFeature(feature, context) {
        const taskSpecs = [
...
*Generated from feature: ${feature.title}*`,
    ...
    calculatePhaseProgress(documents) {
        if (documents.length === 0)
            return 0;
        const completed = documents.filter((doc) => doc.status === 'approved' || doc.completion_percentage === 100).length;
...
export function createDatabaseDrivenSystem(documentService, workflowEngine) {
