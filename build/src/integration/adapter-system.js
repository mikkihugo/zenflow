...
export class MCPAdapter {
    ...
    async connect(config) {
        ...
        try {
            ...
            throw new Error(`Failed to connect via ${this.protocol}: ${error.message}`);
        }
    }
    async disconnect() {
        if (!this.connected)
    ...
    async makeHTTPRequest(_endpoint, _method, body) {
        // HTTP request implementation placeholder
        return { success: true, data: body };
    }
    mapMessageTypeToEndpoint(messageType) {
        const endpointMap = {
    ...
    emitToHandlers(eventType, message) {
        const handlers = this.eventHandlers.get(eventType) || [];
...
export class LegacySystemAdapter {
    ...
    async connect(config) {
        ...
        try {
            ...
            throw new Error(`Legacy system connection failed: ${error.message}`);
        }
    }
    async disconnect() {
        if (this.connection) {
    ...
    transformToLegacyFormat(message) {
    ...
    transformFromLegacyFormat(response) {
    ...
    async sendLegacyMessage(_message) {
...
export class AdapterFactory {
    static adapterRegistry = new Map();
    static {
    ...
    static registerAdapter(protocol, adapterFactory) {
        AdapterFactory.adapterRegistry.set(protocol.toLowerCase(), adapterFactory);
    }
    static createAdapter(protocol) {
...
export class ProtocolManager extends EventEmitter {
    ...
    async addProtocol(name, protocol, config) {
    ...
    async sendMessage(message, protocolName) {
    ...
    async broadcast(message, excludeProtocols) {
        const results = [];
    ...
    setRoute(messageType, protocolName) {
    ...
    async shutdown() {
        ...
        const disconnections = Array.from(this.adapters.values()).map((adapter) => adapter.disconnect().catch((error) => logger.error('Error disconnecting adapter:', error)));
