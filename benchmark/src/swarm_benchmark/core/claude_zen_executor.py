"""
Claude-Flow Executor - Integration layer for executing claude-zen commands.

This module provides a robust interface to execute claude-zen commands with:
...
class ExecutionResult:
    """Result of a claude-zen execution."""
    success: bool
    command: List[str]
    stdout: str
    stderr: str
...
class ClaudeFlowExecutor:
    """Executor for claude-zen commands with robust error handling."""
    ...
    def _find_claude_flow(self) -> str:
        """Find the claude-zen executable."""
        # Check common locations
        locations = [
            Path(__file__).parent.parent.parent.parent.parent / "bin" / "claude-zen",
            Path(__file__).parent.parent.parent.parent.parent / "claude-zen",
            Path.cwd() / "bin" / "claude-zen",
            Path.cwd() / "claude-zen",
        ]
        ...
        # Try system path
        result = shutil.which("claude-zen")
        if result:
            return result
            
        raise RuntimeError("Could not find claude-zen executable")
    ...
    def validate_installation(self) -> bool:
        """Validate claude-zen installation."""
        try:
