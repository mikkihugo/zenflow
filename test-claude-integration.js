...
import { ClaudeCodeProvider, createClaudeCodeProvider } from './src/cli/claude-code-provider.js';
import { printSuccess, printError, printInfo, printWarning } from './src/cli/utils.js';
...
async function testClaudeIntegration() {
  ...
  try {
    ...
    try {
      const simpleResponse = await provider.generateText('Say "Hello from Claude!" and nothing else.');
    ...
    const taskResponse = await provider.generateForTask('analyze-complexity', 
    ...
    const jsonPrompt = 'Output a JSON object with these exact fields: {"name": "Claude Flow", "version": "1.0.0", "features": ["AI coordination", "Task management", "Swarm orchestration"]}. Return only the JSON, no other text.';
