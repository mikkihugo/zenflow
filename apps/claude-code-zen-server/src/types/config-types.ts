...
export interface ConfigHealthReport {
  ...
  timestamp: number;
  environment?: string;
  status: 'healthy' || warning || 'critic'a'l')';
  score: number;
...
export interface ValidationResult {
  isValid: boolean;
  valid?: boolean;
  // Alias for isValid for compatibility errors: string[];
  warnings: string[];
  message?: string;
...
export interface ConfigChangeEvent {
  type: 'created' '|| updated | delet'e'd')';
  path: string;
  oldValue?: any;
  newValue?: any;
  timestamp: number;
...
export interface ConfigurationSource {
  type: file  || environment | defau'l't | override || 'defaults  || env || 'c'l'i')';
  path?: string;
  data?: any;
  priority: number
...
export interface DatabaseConfig {
  type: 'sqlite  |postgresql | | my's'q'l')';
  host?: string;
  port?: number;
  database: string;
  username?: string;
...
export interface StorageConfiguration {
  database: { provider: sqlite | postgresql | m'y's'q'l')';
  connection: string;
  pool?: {
  min: number;
  max: number
  ...
  provider: 'memory' || redis)'; connection?: string; ttl: number

}; files: { uploads: string; temp: string; logs: string
}
}
...
export const DEFAULT_API_CONFIG: APIConfig = { version: '1..0'; prefix: '/api/v1'; rateLimit: {
  windowMs: 900000,
  // 15 minutes max: 100
