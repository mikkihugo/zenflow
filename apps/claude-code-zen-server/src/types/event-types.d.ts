...
export interface CollectiveEvents {
  'collective:initialized': CollectiveInitialize'Payload';
...
export interface CollectiveInitialize'Payload extends anyPayload {
...
export interface CubeStatusPayload extends anyPayload {
  readonly type: string; readonly status: string; readonly borgRating: string
...
export interface CoordinationEvents {
  ...
  'agent:spawn:request': Agen'SpawnRequestPayload';
  ...
  'hive:request:agent_states': HiveReque'tAgentStatesPayload';
  ...
  'swarm:register': Swa'mRegisterPayload';
  ...
  'workload:demand-change': WorkloadD'mandChangePayload';
  'load:spike': LoadSpik'Payload';
  'node:joined': No'eJoinedPayload';
  'node:left': NodeLef'Payload';
  'node:metrics-updated': No'eMetricsUpdatedPayload';
  'heartbeat:sent': Hear'beatSentPayload';
  'connection:quality-changed': ConnectionQualityChange'Payload';
  'network:fault-detected': NetworkFaultDetecte'Payload';
  'workload:pattern-changed': Workloa'PatternChangedPayload';
  'swarm:sync:broadcast': SwarmSyncBroadcas'Payload';
  'swarm:sync:response': SwarmSyncR'sponsePayload';
  'agent:state:updated': AgentStateUp'atedPayload'
...
export interface SwarmInitializePayload extends anyPayload { readonly swarmId: string; readonly agents: readonly {
  id: string;
  type: AgentType
}[]; readonly task: {
  readonly id: string;
...
export interface SwarmCreatedPayload extends anyPayload { readonly swarmId: string; readonly topology: SwarmTopology; readonly maxAgents: number; readonly purpose: string; readonly createdBy: string; readonly agentTypes?: readonly AgentType[]; readonly configuration: {
...
export interface SwarmCompletedPayload extends anyPayload { readonly swarmId: string; readonly duration: number; readonly tasksCompleted: number; readonly tasksFailed: number; readonly finalState: 'success'| partial-success   | failure')'; readonly results?: readonly unknown[]; readonly performance: {
...
export interface NodeJoinedPayload extends anyPayload { readonly nodeId: string; readonly nodeTyp'e': || ag'n't || coordi'at'o'r | st'rage | compute  || load-balancer)'; readonly capabilities: readonly string[]; readonly resources: {
...
export interface SwarmSyncBroadcastPayload extends anyPayload { readonly swarmId: string; readonly sourceSwarmId?: string; readonly syncId: string; readonly syncType: 'state' |config | 'knowledge'| healt'h')'; readonly broadcastScope: || all-agen't's | specific-agen's  || coordinator-on'l'y')'; readonly targetAgents?: readonly string[]; readonly payload?: {
  readonly type: string; readonly data: any; readonly checksum: string

}; readonly state?: any; readonly priority?: 'low' || medium || '''high  ''|| critical)'; readonly acknowledgeRequired?: boolean
}
export interface SwarmSyncResponsePayload extends anyPayload { readonly swarmId: string; readonly syncId: string; readonly respondingAgentId: string; readonly responseType':' 'ack'| nack | partial  || err'o'r')'; readonly responseData?: any; readonly processingTime: number; readonly sourceSwarmId?: string; readonly error?: {
