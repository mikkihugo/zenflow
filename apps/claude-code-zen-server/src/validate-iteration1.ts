...
async function validateIteration1(): Promise<void> {
  ...
  try {
    const { LLMProvider, getGlobalLLM, executeClaudeTask } = await import('@claude-zen/foundation');
  ...
  try {
    const { createSafeSparcWorkflow, SafeSparcWorkflow } = await import('./workflows/safe-sparc-standalone');
