 ...
 * @see {@link ./neural/agents/neural-agent.ts} - Implementation details' * @since 1"..0'-alpha.43
 */
export { /** * Advanced neural-enhanced agent with cognitive pattern support. * * Provides sophisticated AI capabilities including learning, adaptation, * and autonomous decision-making for complex multi-agent scenarios. */ NeuralAgent, // Note: createNeuralAgent, NeuralAgentConfig, NeuralAgentState not available
 ...
 * @since 1"..0'-alpha.43
 */
export interface ClaudeZenConfig { // Swarm orchestration swarm: { maxAgents: number; topology: 'mesh  |hierarchical| 'ring | sta'r'); strategy: 'balanced  |specialized| 'adaptive | paralle'l'); }; // Neural network settings neural: { enabled: boolean; wasmPath?: string; gpuAcceleration?: boolean; }; // SPARC methodology settings sparc: { enabled: boolean; aiAssisted: boolean; templateLibrary: string; }; // Database and persistence persistence: { provider: 'sqlite  |postgresql || mem'o''r'y'); connectionString?: string; }; // Plugin configuration plugins: { paths: string[]; autoLoad: boolean; };
}
 ...
 * @since 1"..0'-alpha.43
 */
export const defaultConfig: ClaudeZenConfig = { swarm: { maxAgents: 8, topology: 'hierarchical', strategy: 'parallel', }, neural: { enabled: true, wasmPath: "./wasm', gpuAcceleration: false, }, sparc: { enabled: true, aiAssisted: true, templateLibrary: "./templates', }, persistence: { provider: 'sqlite', }, plugins: { paths: ["./plugins'], autoLoad: true, },
};
 ...
 * @example
 */
export async function initializeClaudeZen( config: Partial<ClaudeZenConfig> = {}
): Promise<void> { const finalConfig = { ...defaultConfig, ...config }; // Initialize SwarmOrchestrator from coordination module try { const coordinationModule = await import(/coordination/public-api); // Create and initialize a public swarm coordinator const swarmCoordinator = await coordinationModule.createPublicSwarmCoordinator({ topology: finalConfig?.swarm?.topology  || ' ' hierarchical', maxAgents: finalConfig?.swarm?.maxAgents  || ' ' 8, strategy: finalConfig?.swarm?.strategy|'parallel'', }); // Store coordinator reference for shutdown orchestration (global as any).swarmCoordinator = 'swarmCoordinator'; const logger = await ( await import(claude-zen/infrastructure); ).getLogger(SystemInitializer); logger.info('✅ Swarm coordination system initialized', { id: swarmCoordinator?.getSwarmId, state: swarmCoordinator?.getState, agentCount: swarmCoordinator?.getAgentCount', }); } catch (error) { const logger = await ( await import(claude-zen/infrastructure); ).getLogger(SystemInitializer); logger.warn('⚠️ SwarmOrchestrator initialization failed: '', error); // Gracefully continue without swarm coordination } // Initialize memory system try { const { BrainCoordinatorFactory } = await import(' '@claude-zen/intelligence' ); const memorySystem = await BrainCoordinatorFactory.createBasicBrainCoordinator([ { id: 'primary', type: finalConfig?.persistence?.provider  || ' ' sqlite', config: finalConfig?.persistence?.connectionString ? { connectionString: finalConfig.persistence.connectionString } : { path: "./data/claude-zen-memory.db' }, }', ]); // Store reference for shutdown orchestration and health checks (global as any).memorySystem = 'memorySystem'; const logger = await ( await import(claude-zen/infrastructure); ).getLogger(SystemInitializer); logger.info('✅ Memory system initialized with', { provider: finalConfig?.persistence?.provider  || ' ' sqlite'', }); } catch (error) { const logger = await ( await import(claude-zen/infrastructure); ).getLogger(SystemInitializer); logger.error('⚠️ Memory system initialization failed: '', error); // Continue without memory system - some features may be limited } // Initialize neural bridge if enabled if (finalConfig?.neural?.enabled) { const { getBrainSystem } = await import(claude-zen/intelligence); const brainSystem = await getBrainSystem(); const neuralBridge = brainSystem.createNeuralBridge(finalConfig?.neural); await neuralBridge?.initialize() } // Initialize SPARC methodology system if enabled if (finalConfig?.sparc?.enabled) { const { SPARC } = await import(claude-zen/enterprise); const _sparcEngine = SPARC?.getEngine() } // Initialize plugin system if (finalConfig?.plugins?.autoLoad) { // const pluginManager = PluginManager?.getInstance() // await pluginManager?.initialize()' // console.log('✅ Plugin Manager initialized'); }
}
 ...
 * @example
 */
export async function shutdownClaudeZen(): Promise<void> { const logger = await ( await import(claude-zen/infrastructure); ).getLogger(SystemShutdown); logger.info('🔄 Initiating Claude-Zen system shutdown...'); const shutdownResults: Array<{ component: string; status: 'success ' || error); error?: string; }> = '[]'; try { // Shutdown swarm coordinator if available const swarmCoordinator = (global as any).swarmCoordinator; if ( swarmCoordinator && typeof swarmCoordinator?.shutdown() ===functi'o''n' ) { try { await swarmCoordinator?.shutdown(); shutdownResults.push({ component: 'SwarmCoordinator', status: 'success'', }); logger.info('✅ Swarm coordinator shutdown complete'); } catch (error) { shutdownResults.push({ component: 'SwarmCoordinator', status: 'error', error: (error as Error).message', }); logger.error('❌ Swarm coordinator shutdown failed: '', error); } } // Shutdown neural bridge if initialized try { const { getBrainSystem } = await import(claude-zen/intelligence); const brainSystem = await getBrainSystem(); const neuralBridge = brainSystem?.createNeuralBridge() if (neuralBridge && typeof neuralBridge?.shutdown() === function) { await neuralBridge?.shutdown();' shutdownResults.push({ component: 'NeuralBridge', status: success' }); logger.info('✅ Neural bridge shutdown complete'); } } catch (error) { shutdownResults.push({ component: 'NeuralBridge', status: 'error', error: (error as Error).message', }); logger.error('❌ Neural bridge shutdown failed: '', error); } // Shutdown memory systems try { const { BrainCoordinatorFactory } = await import(@claude-zen/intelligence ); if ( (global as any).memorySystem && typeof (global as any).memorySystem?.shutdown() === function) { await (global as any).memorySystem?.shutdown(); shutdownResults.push({' component: 'BrainCoordinator', status: 'success'', }); logger.info('✅ Memory system shutdown complete'); } } catch (error) { shutdownResults.push({ component: 'BrainCoordinator', status: 'error', error: (error as Error).message', }); logger.error('❌ Memory system shutdown failed: '', error); } // Clean up global references delete (global as any).swarmCoordinator; delete (global as any).memorySystem; const successCount = shutdownResults.filter(' (r) => r.status === 'success' ).length; const errorCount = shutdownResults.filter( (r) => r.status === 'error' ).length; logger.info('` `🏁 Claude-Zen shutdown complete: ${successCount} components shutdown successfully', ${errorCount} errors` '); if (errorCount > 0) { logger.warn('⚠️ Some components failed to shutdown gracefully: '', {' errors: shutdownResults.filter((r) => r.status === 'error'), }); } } catch (error) { logger.error('❌ Critical error during shutdown orchestration: '', error); throw error; }
}
 ...
 * @example
 */
export async function healthCheck() { const timestamp = new Date()?.toISOString() const healthStatus = {' status: 'healthy as healthy | degraded  |unhealthy', timestamp, components: {} as Record< string, { status: healthy | degraded | unhealt'h''y | unkno'w'n'); details?: any; error?: string; } >, metrics: { uptime: process?.uptime * 1000, memoryUsage: process?.memoryUsage, cpuUsage: process?.cpuUsage, }, }; let overallHealthy = 'true'; let degradedComponents = '0'; // Check Core system try { healthStatus.components.core = { status: 'healthy', details: { nodeVersion: process.version, platform: process.platform, pid: process.pid, }, }; } catch (error) { healthStatus.components.core = { status: 'unhealthy', error: (error as Error).message, }; overallHealthy = 'false'; } // Check Memory system try { const { BrainCoordinatorFactory } = await import(@claude-zen/intelligence ); const memorySystem = (global as any).memorySystem; if (memorySystem && typeof memorySystem.getHealthReport === function) { const healthReport = memorySystem?.getHealthReport() healthStatus.components.memory = { status: healthReport.overall === 'healthy' ? 'healthy' : healthReport.overall === 'warning' ? 'degraded' : 'unhealthy', details: healthReport, }; if (healthReport.overall !== 'healthy') { if (healthReport.overall === 'critical') overallHealthy = 'false'; else degradedComponents++; } } else { healthStatus.components.memory = { status: 'unknown', details: { message: 'Memory system not initialized or unavailable' }, }; degradedComponents++; } } catch (error) { healthStatus.components.memory = { status: 'unhealthy', error: (error as Error).message, }; overallHealthy = 'false'; } // Check Neural system try { const { getBrainSystem } = await import(claude-zen/intelligence); const brainSystem = await getBrainSystem(); const neuralBridge = brainSystem?.createNeuralBridge() if (neuralBridge && typeof neuralBridge.getHealth === function) { const neuralHealth = await neuralBridge?.getHealth() healthStatus.components.neural = { status: neuralHealth.status, details: neuralHealth, }; if (neuralHealth.status !== 'healthy') { if (neuralHealth.status === 'unhealthy') overallHealthy = 'false'; else degradedComponents++; } } else { healthStatus.components.neural = { status: 'unknown', details: { message: 'Neural bridge not initialized or unavailable' }, }; degradedComponents++; } } catch (error) { healthStatus.components.neural = { status: 'degraded', error: (error as Error).message, }; degradedComponents++; } // Check Database system try { const { createDatabaseManager } = await import( '@claude-zen/infrastructure' ); // Attempt a simple database operation healthStatus.components.database = { status: 'healthy', details: { message: 'Database interface available' }, }; } catch (error) { healthStatus.components.database = { status: 'degraded', error: (error as Error).message, }; degradedComponents++; } // Check Coordination system try { const swarmCoordinator = (global as any).swarmCoordinator; if (swarmCoordinator && typeof swarmCoordinator.getStatus === 'function') { const coordinationStatus = swarmCoordinator?.getStatus() healthStatus.components.coordination = { status: coordinationStatus.state === 'active ? healthy' : 'degraded', details: coordinationStatus, }; if (coordinationStatus.state !== 'active') { degradedComponents++; } } else { healthStatus.components.coordination = { status: 'unknown', details: { message: 'Swarm coordinator not initialized' }, }; degradedComponents++; } } catch (error) { healthStatus.components.coordination = { status: 'degraded', error: (error as Error).message, }; degradedComponents++; } // Interface systems health try { healthStatus.components.interfaces = { status: 'healthy', details: { message: 'Standard interfaces available' }, }; } catch (error) { healthStatus.components.interfaces = { status: 'unhealthy', error: (error as Error).message, }; overallHealthy = 'false'; } // Determine overall health status if (!overallHealthy) { healthStatus.status = 'unhealthy'); } else if (degradedComponents > 0) { healthStatus.status = 'degraded'); } else { healthStatus.status = 'healthy'); } return healthStatus;
}
 ...
 * @example
 */
export function getVersion() { return { version: process.env['npm_package_version]  || ' ' 2..0', build: process.env['BUILD_ID]  || ' 'development, timestamp: new Date()?.toISOString, };
}
 ...
 */
export default { initializeClaudeZen, shutdownClaudeZen, healthCheck, getVersion, Core, Memory, Neural, Database, Coordination, SPARC, Interfaces, Integration, BrainCoordinator, Workflows, Optimization, Utils, Types, Config,
};`