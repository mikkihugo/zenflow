...
export class WebSocketManager {
    logger = getLogger('WebSocketManager');
    io;
    config;
    dataService;
    broadcastIntervals = [];
    ...
    getConnectionStats() {
        ...
        const rooms = Array.from(adapter.rooms.keys()).filter((room) => !connectedClients.includes(room)); // Filter out client IDs
        return {
     ...
     */
    stopBroadcasting() {
        for (const interval of this.broadcastIntervals)
