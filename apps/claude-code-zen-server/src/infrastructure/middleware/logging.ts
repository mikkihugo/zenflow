...
export enum LogLevel {
  DEBUG = 'DEBUG',  INFO = 'INFO',  NOTICE = 'NOTICE',  WARNING = 'WARNING',  ERROR = 'ERROR',  CRITICAL = 'CRITICAL',  ALERT = 'ALERT',  EMERGENCY = 'EMERGENCY',}
...
const sanitizeData = (data:unknown): Record<string, unknown> => {
  ...
  const sensitiveFields = [
    'password',    'token',    'key',    'secret',    'auth',    'authorization',    'cookie',    'session',    'csrf',    'api_key',    'apikey',];
...
const createLogEntry = ({
  ...
  const logEntry:LogEntry = {
    ...
    operation:{
      ...
      producer: 'claude-zen-flow-api',      first:!res,
