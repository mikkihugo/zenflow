...
export class RestAdapter extends BaseAdapter {
  ...
  async connect(): Promise<void> {
    try {
      ...
      this.recordRequest(false, `Connection failed: ${error}`);
      throw error;
    }
  }
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      await this.request('GET', null, endpoint);
  ...
  async request(method: string, data?: any, endpoint?: string): Promise<any> {
    return this.retryOperation(async () => {
      try {
        ...
        this.recordRequest(false, `Request failed: ${error}`);
        throw error;
      }
    });
  }
...
export class AdapterSystem extends TypedEventBase {
  ...
  registerAdapter(adapter: BaseAdapter): void {
    ...
    adapter.on('connected', (data) => this.emit('adapter:connected', data));
    adapter.on('disconnected', (data) =>
      this.emit('adapter:disconnected', data)
    );
  ...
  async connectAll(): Promise<void> {
    const connections = Array.from(this.adapters.values()).map((adapter) =>
      adapter.connect().catch((error) => {
        this.logger.error(
          `Failed to connect adapter ${adapter.getConfig().name}:`,
          error
        );
        return error;
      })
  ...
  async disconnectAll(): Promise<void> {
    const disconnections = Array.from(this.adapters.values()).map((adapter) =>
      adapter.disconnect().catch((error) => {
        this.logger.error(
          `Failed to disconnect adapter ${adapter.getConfig().name}:`,
          error
        );
        return error;
      })
  ...
  getAllStatuses(): Record<string, AdapterStatus> {
  ...
  getSystemStatus(): {
    ...
    const connectedAdapters = Object.values(adapters).filter(
      (s) => s.connected
    ).length;
    const healthyAdapters = Object.values(adapters).filter(
      (s) =>
        s.connected &&
        s.lastHealthCheck &&
