...
import { getLogger, Logger } from '@claude-zen/foundation';
...
export class LogTapeSyslogBridge {
  ...
  private setupSyslogLogging(): void {
    try {
      ...
      console.error('Failed to initialize syslog bridge:', error);
  ...
  public logToSyslog(entry: SyslogEntry): void {
    ...
    try {
      ...
      console.error('Syslog bridge error:', error);
    }
  }
  ...
  private sendToSystemLog(level: string, message: string): void {
    try {
      ...
      console.log(`SYSLOG[${level.toUpperCase()}] ${this.componentName}: ${message}`);
    }
  }
