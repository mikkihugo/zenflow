...
class MockMCPClient extends TypedEventBase {
  ...
  async callTool(params: {
    ...
    logger.debug('Calling MCP tool:', params);
    // Mock tool execution
  ...
  implements McpClient<McpRequest>
{
  ...
  async health(): Promise<boolean> {
    try {
      ...
      logger.warn('MCP client health check failed:', error);
      return false;
    }
  }
 ...
 * const client = await factory.create('https', {
 *   protocol: 'https',
 *   url: 'https://mcp.example.com',
 *   clientName: 'claude-zen-client'
 * });
...
export class McpClientFactory implements ClientFactory {
  ...
  getSupportedProtocols(): ProtocolType[] {
    return [ProtocolTypes.HTTP, ProtocolTypes.HTTPS, ProtocolTypes.STDIO];
  }
 ...
 * @example
 * ```typescript
 * const client = await createStdioMCPClient('npx', ['@context7/mcp-server']);
 * ```
 */
export async function createStdioMCPClient(
  ...
  const config: McpClientConfig = {
    ...
    url: 'stdio://' + command + (args.length > 0 ? ' ' + args.join(' ') : ''),
