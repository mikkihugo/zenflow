...
export class USL extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      const { createServiceRegistry, PerformanceTracker } = await import('@claude-zen/foundation');
      ...
      this.logger.info('USL initialized successfully with @claude-zen package delegation');
  ...
  async createUnifiedDataService(
    ...
    const timer = this.performanceTracker.startTimer('create_unified_data_service');
   ...
   */
  async createWebService(name: string, port: number, options: any = {}): Promise<Service> {
    if (!this.initialized) await this.initialize();
   ...
   */
  async createCoordinationService(name: string, options: any = {}): Promise<Service> {
    if (!this.initialized) await this.initialize();
   ...
   */
  async createDatabaseService(name: string, options: any = {}): Promise<Service> {
    if (!this.initialized) await this.initialize();
   ...
   */
  async createIntegrationService(name: string, options: any = {}): Promise<Service> {
    if (!this.initialized) await this.initialize();
   ...
   */
  async createMonitoringService(name: string, options: any = {}): Promise<Service> {
    if (!this.initialized) await this.initialize();
  ...
  discoverServices(criteria: { type?: ServiceType; capabilities?: string[]; tags?: string[] }): Service[] {
    return Array.from(this.services.values()).filter(service => {
      if (criteria.type && service.type !== criteria.type) return false;
      if (criteria.capabilities) {
        const serviceCapabilities = service.getCapabilities?.() || [];
        return criteria.capabilities.every(cap => serviceCapabilities.includes(cap));
      }
      return true;
    });
  }
  ...
  getSystemMetrics(): { totalServices: number; servicesByType: Record<string, number>; performance: any } {
