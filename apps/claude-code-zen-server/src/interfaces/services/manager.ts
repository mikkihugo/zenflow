...
export class ServiceManager extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.agentManager = new AgentManager({
        parallelStartup: this.config.lifecycle.parallelStartup,
        dependencyResolution: this.config.lifecycle.dependencyResolution
      });
      ...
      this.logger.info('Service Manager facade initialized successfully with @claude-zen delegation');

    } catch (error) {
      this.logger.error('Failed to initialize Service Manager facade:', error);
      throw error;
    }
  }
  ...
  async createService(request: ServiceRequest): Promise<Service> {
    ...
    const timer = this.performanceTracker?.startTimer('service_creation');
    ...
    try {
      ...
      this.logger.info(`Service ${request.name} created successfully via delegation`);
      this.emit('service-created', { service, timestamp: new Date() });

      return service;

    } catch (error) {
      this.logger.error(`Failed to create service ${request.name}:`, getErrorMessage(error));
      throw error;
    } finally {
  ...
  async createServices(request: BatchServiceRequest): Promise<Service[]> {
    ...
    try {
      ...
      if (request.parallel && this.config.lifecycle.parallelStartup) {
        // Delegate parallel creation to workflow engine
        createdServices = await this.workflowEngine.createServicesParallel(request.services);
      } else {
        ...
        for (const serviceRequest of request.services) {
          try {
            ...
            this.logger.error(`Failed to create service ${serviceRequest.name}:`, getErrorMessage(error));
      ...
      this.logger.info(`Successfully created ${createdServices.length} services via delegation`);
      return createdServices;

    } catch (error) {
      this.logger.error('Failed to create service batch:', getErrorMessage(error));
      throw error;
    }
  }
  ...
  getServiceMetrics(serviceName?: string): ServiceMetrics[] {
    ...
    return serviceName 
      ? [this.performanceTracker.getMetrics(serviceName)]
  ...
  private setupEventForwarding(): void {
    ...
    this.workflowEngine?.on('service-started', (data) => this.emit('service-started', data));
    this.workflowEngine?.on('service-stopped', (data) => this.emit('service-stopped', data));
    this.workflowEngine?.on('service-error', (data) => this.emit('service-error', data));

    // Forward health monitor events
    this.healthMonitor?.on('service-health-changed', (data) => this.emit('service-health-changed', data));
    this.healthMonitor?.on('service-health-degraded', (data) => this.emit('service-health-degraded', data));

    // Forward load balancer events
    this.loadBalancer?.on('service-recovered', (data) => this.emit('service-recovered', data));
    this.loadBalancer?.on('load-balanced', (data) => this.emit('load-balanced', data));

    // Forward coordination events
    this.serviceCoordinator?.on('coordination-event', (data) => this.emit('coordination-event', data));
  }
  ...
  async shutdown(): Promise<void> {
    try {
      ...
      this.logger.error('Error during Service Manager shutdown:', getErrorMessage(error));
      throw error;
    }
  }
}
...
export function createServiceManager(config?: Partial<ServiceManagerConfig>): ServiceManager {
  ...
  return new ServiceManager({ ...defaultConfig, ...config });
}
