...
export interface USLFactoryConfig {
  ...
  /** Service health monitoring and alerting configuration */ healthMonitoring?: {
    /** Enable health monitoring system */ enabled: boolean;
    /** Health check interval in milliseconds (default: 30000) */ interval?: number;
    /** Alert thresholds for automated notifications */ alertThresholds?: {
      /** Error rate percentage threshold for alerts (e.g., 5.0 for 5%) */ errorRate: number;
  ...
  /** Performance metrics collection and storage configuration */ metricsCollection?: {
    /** Enable metrics collection system */ enabled: boolean;
    /** Metrics collection interval in milliseconds (default: 60000) */ interval?: number;
    /** Metrics retention period in milliseconds (default: 86400000 - 24 hours) */ retention?: number;
  };
  /** Automatic service recovery and restart configuration */ autoRecovery?: {
    /** Enable automatic recovery for failed services */ enabled: boolean;
    /** Maximum number of recovery attempts (default: 3) */ maxRetries?: number;
    /** Backoff multiplier for retry delays (default: 2) */ backoffMultiplier?: number;
  };
}
...
export class USLFactory implements ServiceFactory {
  ...
  /** * Create a service instance based on configuration. * * @param config */
  async create<T extends ServiceConfig = ServiceConfig>(
    ...
    if (this.initializationQueue.has(config?.name)) {
      this.logger.debug(
        'Service ' + config?.name + ' is already being initialized, waiting...'
      );
    ...
    try {
      ...
      this.initializationQueue.delete(config?.name);
      this.logger.error(
        'Failed to create service ' + config?.name + ':',
        error
      );
      throw new ServiceInitializationError(config?.name, error as Error);
    }
  }
  /** * Create multiple services concurrently. * * @param configs */
  async createMultiple<T extends ServiceConfig = ServiceConfig>(
    ...
    const batchSize = this.configuration.maxConcurrentInits;
  ...
  /** * Get service by name. * * @param name */
  ...
  /** * Check if service exists. * * @param name */
  has(name: string): boolean {
    return this.services.has(name);
  }
  /** * Remove and destroy service. * * @param name */
  ...
  /** * Check if service type is supported. * * @param type */
    ...
    for (const priority of [
      ...
      const startPromises = priorityServices.map(async (service) => {
        try {
          ...
          if (this.configuration.autoRecovery.enabled) {
    ...
    for (const priority of [
      ...
      const stopPromises = priorityServices.map(async (service) => {
        try {
          ...
          this.logger.error(
            'Failed to stop service ' + service.name + ':',
            error
          );
        }
      });
    ...
    try {
      ...
      const destroyPromises = this.list().map(async (service) => {
        try {
          ...
          this.logger.error(
            'Failed to destroy service ' + service.name + ':',
            error
          );
        }
      });
  ...
  async validateConfig(config: ServiceConfig): Promise<boolean> {
    try {
      ...
      this.logger.error(
        'Error validating service configuration for ' + config?.name + ':',
        error
      );
      return false;
    }
  }
  /** * Get configuration schema for service type. * * @param type */
  ...
  private async createServiceInstance(config: ServiceConfig): Promise<Service> {
    ...
    try {
      ...
      this.logger.info(
        'Service ' + config?.name + ' initialized in ' + duration + 'ms'
      );
      return service;
    } catch (error) {
      const duration = Date.now() - startTime;
  ...
  private validateTypeSpecificConfig(config: ServiceConfig): {
    ...
    try {
      if (isDataServiceConfig(config)) {
        ...
        if (
          config?.dataSource &&
          !['database', 'memory', 'file', 'api'].includes(
        ...
        isCoordinationServiceConfig(config) &&
  ...
  private setupServiceEventHandling(service: Service): void {
    ...
    eventTypes.forEach((eventType) => {
      service.on(eventType, (event: ServiceEvent) => {
        ...
        if (eventType === 'error' && this.configuration.autoRecovery.enabled) {
          this.scheduleServiceRecovery(service);
        }
      });
    });
  ...
  private groupServicesByPriority(
    ...
    services.forEach((service) => {
      const priority =
        (service.config as any).priority ?? ServicePriority.NORMAL;
  ...
  private async performServiceRecovery(service: Service): Promise<Service> {
    const maxRetries = this.configuration.autoRecovery.maxRetries;
    const backoffMultiplier = this.configuration.autoRecovery.backoffMultiplier;
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        this.logger.info(
          'Attempting service recovery for ' +
            service.name +
            ' (attempt ' +
            attempt +
            '/' +
        ...
        if (attempt < maxRetries) {
          ...
          this.logger.info(
            'Waiting ' +
              delay +
              'ms before next recovery attempt for ' +
              service.name
          );
  ...
  private initializeSystemServices(): void {
    ...
    if (this.configuration.discovery.enabled) {
    ...
    setInterval(async () => {
      try {
        ...
        if (slowServices.length > 0) {
          this.logger.warn(
            'Slow services detected: ' + slowServices.join(', ')
          );
...
export class ServiceRegistry implements ServiceRegistry {
  ...
  getServicesByStatus(status: ServiceLifecycleStatus): Service[] {
    ...
    return Array.from(allServices?.values()).filter((service) => {
      try {
  ...
  async startAllServices(): Promise<void> {
    ...
    const startPromises = Array.from(this.factories?.values()).map((factory) =>
      factory?.startAll()
    );
    await Promise.allSettled(startPromises);
  }
  async stopAllServices(): Promise<void> {
    ...
    const stopPromises = Array.from(this.factories?.values()).map((factory) =>
      factory?.stopAll()
    );
    await Promise.allSettled(stopPromises);
  }
  async healthCheckAll(): Promise<Map<string, ServiceStatus>> {
    ...
    const healthCheckPromises = Array.from(this.factories?.values()).map(
      async (factory) => {
        try {
  ...
  async getSystemMetrics(): Promise<{
    ...
    const metricsPromises = Array.from(this.factories?.values()).map(
      async (factory) => {
        try {
  ...
  async validate(config: ServiceConfig): Promise<{
    ...
    try {
      ...
      if (config?.type) {
        ...
        if (!typeValid) {
          errors.push(
            'Invalid configuration for service type: ' + config?.type
          );
        }
      }
...
export class ServiceCapabilityRegistry implements ServiceCapabilityRegistry {
  ...
  register(serviceName: string, capability: ServiceCapability): void {
    ...
    const existingIndex = serviceCapabilities.findIndex(
      (cap) => cap.name === capability.name
    );
    if (existingIndex >= 0) {
      // Update existing capability
      serviceCapabilities[existingIndex] = capability;
      this.logger.debug(
        'Updated capability ' + capability.name + ' for service ' + serviceName
      );
    } else {
      // Add new capability
  ...
  unregister(serviceName: string, capabilityName: string): void {
    ...
    if (serviceCapabilities) {
      const filteredCapabilities = serviceCapabilities.filter(
