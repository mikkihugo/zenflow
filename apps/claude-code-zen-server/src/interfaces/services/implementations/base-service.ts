...
export abstract class BaseService extends TypedEventBase implements Service {
  ...
  async initialize(config?: Partial<ServiceConfig>): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        'Service ' + this.name + ' initialization failed:',
        error
      );
  ...
  async getStatus(): Promise<ServiceStatus> {
    ...
    try {
      ...
      health = 'unknown';
      this.logger.debug(
        'Health check failed for service ' + this.name + ':',
        error
      );
    }
    ...
    for (const [depName, _depConfig] of this.dependencies) {
      try {
        ...
        dependencies[depName] = { status: 'healthy', lastCheck: new Date() };
      } catch (_error) {
        dependencies[depName] = { status: 'unhealthy', lastCheck: new Date() };
      }
    }
    return {
      name: this.name,
    ...
    const dependencyChecks = Array.from(this.dependencies.entries()).map(
      async ([depName, depConfig]) => {
        try {
          ...
          this.logger.warn(
            'Dependency check failed for ' + depName + ':',
            error
          );
          return {
            name: depName,
