...
export class InfrastructureServiceFactory extends TypedEventBase {
  ...
  constructor(config: InfrastructureServiceFactoryConfig = {}) {
    ...
    this.logger = getLogger(InfrastructureServiceFactory as unknown as string);
    this.metrics = {
  ...
  async createService(
    ...
    this.logger.info('Creating new infrastructure service', {
      name,
      options,
    });
    ...
    try {
      ...
      this.emit('service-creation-failed', {
        name,
        options,
        error,
      });
  ...
  async removeService(name: string): Promise<void> {
    ...
    try {
      ...
      this.emit('service-removal-failed', {
        serviceName: name,
        error,
      });
      throw error;
    ...
    for (const [serviceName, entry] of this.serviceRegistry.entries()) {
      try {
        ...
        allStatus[serviceName] = {
          name: serviceName,
          type: (entry.service as any).type,
          lifcycle: 'error',
          health: 'unhealthy',
      ...
      for (const [name, entry] of this.serviceRegistry.entries()) {
        shutdownPromises.push(
          entry.service?.destroy().catch((error) => {
            this.logger.error(
              'Failed to shutdown service ' + name + ':',
              error
            );
          }) as Promise<void>
        );
  ...
  private createServiceConfig(
    ...
    return {
      ...
      ...overrides,
      name, // Ensure name is preserved
    } as InfrastructureServiceAdapterConfig;
  }
  ...
  private setupServiceEventHandlers(
    ...
    if (this.configuration.eventCoordination?.crossServiceEvents) {
      service.on('error', (event: unknown) => {
      ...
      service.on('operation', (_event: unknown) => {
      ...
      for (const eventType of eventTypes) {
        service.on(eventType, (event: unknown) => {
  ...
  private setupEventHandlers(): void {
    ...
    this.on('service-error', (data: any) => {
      this.logger.warn(
        'Service error in ' + data?.serviceName + ':',
        data?.event
      );
    });

    this.on('service-unhealthy', (data: any) => {
