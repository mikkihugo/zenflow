...
export async function createIntelligentAgent(
  ...
  const enhancedConfig = {
    ...
    specialization: (config?.specialization as any) || (config?.type as any),
...
export async function createAgentBatch(
  ...
  logger.info(
    'Agent batch creation completed: ' +
      successCount +
      '/' +
      configs.length +
      ' successful'
  ...
  for (const metrics of targetAgents) {
    ...
    for (const issue of issues) {
      recommendations.push({
        agentId: (metrics as any).agentId,
        issue: issue.problem,
      ...
      if (issue.priority === 'high' && issue.autoFix) {
        try {
          await issue.autoFix(adapter, (metrics as any).agentId);
          optimized++;
          logger.info(
            'Auto-optimized agent ' +
              (metrics as any).agentId +
              ': ' +
              issue.problem
          );
        } catch (error) {
          logger.warn(
            'Failed to auto-optimize agent ' +
              (metrics as any).agentId +
              ': ' +
              error
          );
...
export async function createManagedSession(
  ...
  if (!sessionResult?.success) {
    throw new Error(
      'Failed to create session: ' + (sessionResult as any)?.error?.message
    );
  }
    ...
    setTimeout(async () => {
      try {
        logger.info(
          'Session ' +
            sessionId +
            ' reached max duration, creating final checkpoint'
        );
        await adapter.execute('session-checkpoint', {
          sessionId,
        ...
        logger.error(
          'Failed to create final checkpoint for session ' +
            sessionId +
            ': ' +
            error
        );
...
export async function coordinateIntelligentSwarm(
  ...
  logger.info(
    'Coordinating intelligent swarm with ' + agents.length + ' agents'
  );
  ...
  const topologies: SwarmTopology[] = adaptiveTopology ? ['mesh'] : ['mesh'];
  for (const topology of topologies) {
    try {
      ...
      if (coordinationResult?.success) {
        const coordination = (coordinationResult as any)?.data;
        const performance = {
  ...
  if (!bestCoordination) {
    ...
    if (coordinationResult?.success) {
      bestCoordination = (coordinationResult as any)?.data;
      bestTopology = 'mesh';
      bestPerformance = {
...
export async function distributeSwarmTasks(
  ...
  unassigned: string[];
  loadBalance: { [agentId: string]: number };
}> {
  ...
  logger.info(
    'Distributing ' + tasks.length + ' tasks using strategy: ' + strategy
  );
  ...
  availableAgents.forEach((agent) => {
    if (agent.id) agentTaskCounts[agent.id] = 0;
  });
  ...
  for (const task of sortedTasks) {
    ...
    if (selectedAgent) {
      try {
        // CLAUDE-NEEDED: Confirm parameters for 'swarm-assign-task' execute call; using { task } placeholder
        ...
        if (assignResult?.success && selectedAgent?.id) {
          ...
          logger.debug(
            'Assigned task ' + task.id + ' to agent ' + selectedAgent.id
          );
        } else {
          ...
          logger.warn(
            'Failed to assign task ' +
              task.id +
              ': ' +
              (assignResult as any)?.error?.message
          );
        }
      } catch (error) {
        unassigned.push(task.id);
...
export async function analyzeCoordinationPerformance(
  ...
  logger.info(
    'Performance analysis completed: Overall grade ' +
      overall.grade +
      ', ' +
      agents.active +
      '/' +
...
function analyzeAgentPerformance(metrics: any): Array<{
  ...
  if (metrics.learningProgress < 0.3) {
    issues.push({
      problem:
        'Low learning progress: ' +
        (metrics.learningProgress * 100).toFixed(1) +
        '%',
      ...
      autoFix: async (adapter, agentId) => {
        await adapter.execute('cognitive-set', {
          agentId,
          pattern: {
            ...(getDefaultCognitivePattern(metrics.type) as any),
            learningRate: 0.2,
          },
        });
      },
  ...
  if (inactiveTime > 3600000) {
    ...
    issues.push({
      problem:
        'Agent inactive for ' + Math.round(inactiveTime / 60000) + ' minutes',
...
function findBestCapabilityMatch(
  ...
  const suitableAgents = agents.filter((agent) => {
    ...
    const hasCapabilities = (task.requirements || []).every((req: string) =>
      (agent as any).capabilities?.includes(req)
    );
...
function calculateAgentScore(agent: SwarmAgent): number {
  const completionRate = (agent as any).performance?.tasksCompleted;
  const errorPenalty = (agent as any).performance?.errorRate * 100;
  const responsePenalty =
    (agent as any).performance?.averageResponseTime / 1000;
