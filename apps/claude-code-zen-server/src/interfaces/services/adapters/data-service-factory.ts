  ...
  async getMetricsAll(): Promise<Map<string, ServiceMetrics>> {
    ...
    const metricsPromises = services.map(async (service) => {
      try {
        ...
        this.logger.error(
          'Failed to get metrics for data service adapter ' +
            service.name +
            ':',
          error
        );
  ...
  async createUnifiedDataAdapter(
    ...
    const unifiedConfig = createDefaultDataServiceAdapterConfig(name, {
      ...
      performance: {
        enableRequestDeduplication: true,
        maxConcurrency: 15,
        // Higher concurrency for unified adapter
      ...
      cache: {
        ...
        strategy: 'memory',
        defaultTTL: 600000,
        // Longer cache for unified adapter
        maxSize: 2000,
        // Larger cache for unified adapter
    ...
    this.logger.info(
      'Created unified data adapter: ' + name + ' ' + databaseType
    );
    return adapter;
  }
  /**
      ...
      this.healthCheckTimer = setInterval(async () => {
        try {
          ...
          if (unhealthyServices.length > 0) {
            this.logger.warn(
              'Unhealthy data service adapters detected: ' +
