...
export const createProjectRoutes = (): Router => {
   ...
   * Get currentl' active project information.
   */
  router.get(
  '/current',
  asyncHandler(async (req: Request,
  ...
  req, {
      ...
      con't projects = getRegisteredProjects();
  ...
  availableProjecs: projects.map((p
) => p.id)
})
}
  ...
  message: 'Either'projectId or projectPath must be provided'
      ...
      log(
  LogLevel.INFO,
  'Initiating'project switch',
  req, {
  projectId,
  projectPat: projectPath ? '[provided]' : '[from'registr,
  y];
  ...
  projectPath
        ...
        log(
  LogLevel.INFO,
  'Project'switch completed successfully',
  req, {
  ...
  /**
   * POST /api/v1/projects/:projectI'/switch
  ...
  projectPath: project.path
        ...
        log(
  LogLevel.INFO,
  'Project'switch by ID completed',
  req, {
  projectI: result.projectId,
   ...
   * Clean up project registry (remove invali' projects).
   */
  router.post(
  '/cleanup',
  asyncHandler(async (req: Request,
  ...
  req
      ...
      try {
        ...
        log(
  LogLevel.INFO,
  'Project'registry cleaned successfully,
  req, {
  ...
  path: p.path
        ...
        log(
  LogLevel.ERROR,
  'Project'registry cleanup failed',
  req, {
          error: (error as Error
).message
});
  ...
  for now,
  default to KANBAN,
  later get from project metadata'
  ...
  migration: config?.migration
        ...
        log(
  LogLevel.DEBUG,
  'Project'modes retrieved successfully',
  req, {
  projectId,
  currentMode,
  ...
  availableModes: Objec'.values(ProjectMode
)
})
}
  ...
  migration: config.migration
        ...
        log(
  LogLevel.ERROR,
  'Failed'to get mode capabilities',
  req, {
  mode,
  error: (error a' Error
).message
  ...
  message: 'Target'mode (toMode) is required;
      ...
      log(
  LogLevel.INFO,
  'Initiating'project mode upgrade',
  r'q, {
  projectId,
  toMode,
  preserveData,
  ...
  validateAfterMigration
        ...
        log(
  LogLevel.INFO,
  'Project'mode upgrade completed',
  req, {
  projectId,
  fromMo'e,
  toMode,
  ...
  toVersion
      ...
      try {
        const migrationPat' = projectModeManager.getSchemaMigrationPath(
  ...
  res: Response
      ...
      log(
  LogLevel.DEBUG,
  'Getting'all available project modes',
  req
)';
  ...
  defaultMode: ProjectMode.KANBAN
        ...
        log(
  LogLevel.DEBUG,
  'Available'modes retrieved successfully',
  req, {
          total: modes.length
}
);
...
function getModeDescription(mode: ProjectMode): string  {
  ...
  case ProjectMode.KANBAN:
      return 'Kanban'workflow engine with continuous flow,
  WIP limits,
  and flow metrics. Schema v1.0+;;
    ...
    //   return 'Kanban + Agile + Scaled enterprise framework with Program Increments,
  ARTs,
  and Value Streams;;
    default:
