...
export class WorkflowAGUIAdapter extends TerminalAGUI {
  ...
  notifyOnTimeout: true
    ...
    this.logger.info(
  'WorkflowAGUIAdapter initialized',
  {
      config: this.config
}
)
  ...
  // ============================================================================
  // ENHANCED VALIDATION METHODS - Workflow-aware question han'ling
  // ============================================================================
  ...
  return this.processWorkflowGate(question,
  correlationId)
   ...
   */
  public async processWorkflowGate(gateRequest: WorkflowGateRequest,
    correlationId?: string
  ): Promise<string>  {
    const startTime = Date.now();
    ...
    this.logger.info(
  'Processing workflow gate',
  {
  gatId: gateRequest.id,
  ...
  this.registerActiveGate(gateRequest,
  gateCorrelationId)
  ...
  gateCorrelationId
      ...
      this.logger.info(
  'Workflow gate processed successfully',
  {
  ...
  correlationId: gateCorrelationId
      ...
      this.logger.error(
  'Workflow gate processing failed',
  {
  gateI: gateRequest.id,
 ...
 + '')'
  ...
  this.handleGateTimeout(gateRequest.id,
  'initial)
    ...
    this.logger.debug(
  'Registered active gate for timeout management',
  {
  gaeId: gateRequest.id,
   ...
   * Handle gate timeout
   */
  private async handleGateTimeout(gateId: string, timeoutType: string): Promise<void>  {
  ...
  private cleanupActiveGate(gateId: string: void {
    ...
    this.logger.debug('Cleaned up active gate', { gat'Id })'
}
  ...
  correlationId
    ...
    this.logger.info(
  'Workflow decision logged to audit trail',
  {
  gateId: gateRequest.id,
  workfowId: gateRequest.workflowContext.workflowId,
  decision: response,
   ...
   */
  private isWorkflowGateRequest(question: ValidationQuestion
  ): question is WorkflowGateRequest  {
  return 'workflowContext' in ques'ion && 'gateType' in qu'stion;
  ...
  error?: Error
    ...
    try {
      if (!gateRequest.workflowCo'text.workflowId) {
        return {
  success: false,
  ...
  error: new Error('Step name is required)
      ...
      if (
        gateRequest.workflowContext.stakehol'ers.length === 0 &&
   ...
   */
  private async getWorkflowInput(gateRequest: WorkflowGateRequest): Promise<string>  {
   ...
   */
  private processWorkflowResponse(gateRequest: WorkflowGateRequest,
    response: string
  ): string  {
    switch (gateRequest.gateType) {
  case approval:
        return this.processApprova'Response(response);
      case checkpoint:
        return this.processCheckpointResponse(response);
      case decision:
        retur' this.processDecisionResponse(response);
      case emergency:
        return this.processEmergenc'Response(response);
      default:
        return response
  ...
  private processApprovalResponse(response: string): string  {
    const approvalKeywords = ['approve',
      'approved',
      'yes',
      'accept',
      'ok',
      'continue', ];
    const r'jectionKeywords = ['reject',
      'rejected',
      'no',
      'deny',
      'stop',
    ...
    const 'owerResponse = response.toLowerCase();
 ...
 */
export function createWorkflowAGUIAdapter(config?: Partial<WorkflowAGUIConfig>
): WorkflowAGUIAdapter  {
