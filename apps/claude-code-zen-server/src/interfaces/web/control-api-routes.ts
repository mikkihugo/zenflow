...
import { getTelemetry, recordMetric, getDatabaseAccess , EventBus, createEventBus } from '@claude-zen/infrastructure';
...
export class ControlApiRoutes {
  ...
  setupRoutes(app: Express, httpServer?: Server): void {
    ...
    syslogBridge.info('control-api', 'Complete control API routes configured', {
      ...
      components: ['logtape', 'telemetry', 'monitoring', 'neural', 'sparc', 'git', 'config', 'process', 'project-mgmt']
    });
  ...
  private setupLogTapeApis(app: Express, api: string): void {
    ...
    app.get(`${api}/logs/stats`, async (req: Request, res: Response) => {
      try {
        ...
        const totalResult = await this.database.query('SELECT COUNT(*) as total FROM system_logs');
        ...
        const componentStatsResult = await this.database.query(`
          ...
          GROUP BY component
        `);
        const logsByComponent = componentStatsResult.reduce((acc: any, row: any) => {
        ...
        res.status(500).json({ error: 'Failed to get log stats from database' });
      }
    });
    ...
    app.delete(`${api}/logs`, async (req: Request, res: Response) => {
      try {
        const beforeCountResult = await this.database.query('SELECT COUNT(*) as total FROM system_logs');
    ...
    app.get(`${api}/logs/export`, async (req: Request, res: Response) => {
      try {
        ...
        const logs = await this.database.query('SELECT * FROM system_logs ORDER BY timestamp DESC');

        if (format === 'csv') {
          const csv = 'timestamp,level,component,message,meta\n' +
            logs.map((log: any) => 
              `${log.timestamp},${log.level},${log.component},"${log.message.replace(/"/g, '""')}","${(log.meta || '').replace(/"/g, '""')}"`
            ).join('\n');
          
          res.setHeader('Content-Type', 'text/csv');
          res.setHeader('Content-Disposition', 'attachment; filename="claude-zen-logs.csv"');
          res.send(csv);
        } else {
          res.setHeader('Content-Type', 'application/json');
          res.setHeader('Content-Disposition', 'attachment; filename="claude-zen-logs.json"');
          res.json({ 
            logs: logs.map((log: any) => ({
              ...log,
              meta: log.meta ? JSON.parse(log.meta) : null
  ...
  private setupTelemetryApis(app: Express, api: string): void {
    ...
    app.get(`${api}/traces`, async (req: Request, res: Response) => {
      try {
        ...
        const traces = Array.from({ length: parseInt(limit as string) }, (_, i) => ({
  ...
  private setupNeuralControlApis(app: Express, api: string): void {
    ...
    app.get(`${api}/neural/status`, async (req: Request, res: Response) => {
      try {
        ...
        const behavioralIntelligence = (global as any).claudeZenSwarm?.behavioralIntelligence;
        ...
        const status = {
          ...
          behavioralIntelligence: {
            ...
            learningRate: behavioralIntelligence?.getLearningRate?.() || 0.3,
    ...
    app.post(`${api}/neural/training/start`, async (req: Request, res: Response) => {
      try {
        const { model, trainingData, epochs = 10 } = req.body;
        const behavioralIntelligence = (global as any).claudeZenSwarm?.behavioralIntelligence;

        if (!behavioralIntelligence) {
          return res.status(503).json({ error: 'Behavioral intelligence not available' });
        }
    ...
    // Get training status
    app.get(`${api}/neural/training/:trainingId`, async (req: Request, res: Response) => {
      try {
    ...
    app.post(`${api}/neural/predict`, async (req: Request, res: Response) => {
      try {
        ...
        const prediction = {
          result: Math.random() * 0.8 + 0.1, // 0.1 to 0.9
  ...
  private setupSparcControlApis(app: Express, api: string): void {
    this.logger.info('🎯 Setting up comprehensive SPARC workflow control APIs...');
        ...
        const projects = [
          {
            ...
            phases: {
              specification: { status: 'completed', completedAt: new Date(Date.now() - 3600000).toISOString() },
              pseudocode: { status: 'completed', completedAt: new Date(Date.now() - 1800000).toISOString() },
              architecture: { status: 'in-progress', startedAt: new Date(Date.now() - 900000).toISOString() },
    ...
    app.post(`${api}/sparc/projects`, async (req: Request, res: Response) => {
      try {
        const { name, description, requirements, epic, priority = 'medium' } = req.body;
        const sparcCommander = (global as any).claudeZenSwarm?.sparcCommander;
    ...
    // Advance SPARC phase
    app.post(`${api}/sparc/projects/:projectId/advance`, async (req: Request, res: Response) => {
      try {
        const { projectId } = req.params;
        const { phaseResults } = req.body;

        // Mock advancing to next phase
        const nextPhase = this.getNextSparcPhase(req.query.currentPhase as string);
  ...
  private setupGitControlApis(app: Express, api: string): void {
    ...
    // Create Git sandbox
    app.post(`${api}/git/sandbox/create`, async (req: Request, res: Response) => {
      try {
    ...
    // Git operations
    app.post(`${api}/git/operations/:operation`, async (req: Request, res: Response) => {
      try {
        const { operation } = req.params;
        const { params } = req.body;

        const validOperations = ['add', 'commit', 'push', 'pull', 'merge', 'branch', 'checkout'];
        ...
        this.logger.error(`Failed to execute Git ${req.params.operation}:`, error);
        res.status(500).json({ error: `Failed to execute Git ${req.params.operation}` });
      }
    });
  ...
  private setupProcessControlApis(app: Express, api: string): void {
      ...
      try {
        const services = {
          ...
          brain: {
            ...
            status: (global as any).claudeZenSwarm?.brainCoordinator ? 'running' : 'stopped',
            active: !!(global as any).claudeZenSwarm?.brainCoordinator
          },
          swarm: {
            name: 'Swarm Commander',
            status: (global as any).claudeZenSwarm?.swarmCommander ? 'running' : 'stopped',
            agents: (global as any).claudeZenSwarm?.swarmCommander?.getAgentCount?.() || 0
          },
          safety: {
            name: 'AI Safety',
            status: (global as any).claudeZenSwarm?.aiSafetyOrchestrator ? 'running' : 'stopped',
    ...
    app.post(`${api}/services/:service/restart`, async (req: Request, res: Response) => {
      try {
        ...
        this.logger.error(`Failed to restart service ${req.params.service}:`, error);
        res.status(500).json({ error: `Failed to restart service ${req.params.service}` });
      }
    });
        ...
        const prds = [
          {
            ...
            sections: {
              ...
              acceptance: ['Security audit passed', 'Performance benchmarks met']
            },
  ...
  private async cleanupOldLogs(): Promise<void> {
    try {
      ...
      if (total > keepCount) {
        ...
        this.logger.info(`Cleaned up old logs, kept latest ${keepCount} entries`);
      }
    } catch (error) {
