...
export class System extends TypedEventBase {
  ...
  constructor(config: SystemConfig = {}) {
    ...
    // this.setMaxListeners(20); // Handled by TypedEventBase configuration
  ...
  private initializeComponents(): void {
    ...
    this.documentProcessor = new DocumentProcessor(
      ...
      this.workflowEngine,
      {
        autoWatch: this.configuration?.documents?.autoWatch !== false,
        enableWorkflows:
          this.configuration?.documents?.enableWorkflows !== false,
      }
    );
  ...
  private setupEventHandlers(): void {
    ...
    this.documentProcessor?.on('document:created', async (event) => {
      ...
      if (this.configuration?.documents?.enableWorkflows !== false) {
  ...
  private async initializeAISystemsAsync(): Promise<void> {
    try {
      await withTrace('ai-systems-initialization', async () => {
        await this.systemCircuitBreaker(this as any)?.fire(async () => {
    ...
    this.aiEventBus?.on('workflow:performance', async (data: any) => {
      if (this.neuralML) {
        try {
          ...
          logger?.debug(
            `Neural workflow optimization: ${optimization?.improvementFactor}x faster`
          );
        } catch (error) {
