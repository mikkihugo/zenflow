...
const getCurrentProject = ():{ id: string; path: string; name: string} | null => {
...
const getRegisteredProjects = ():{ id: string; path: string; name: string}[] => 
...
export class ProjectSwitcher extends EventEmitter {
  ...
  private handleSwitchError(
    ...
    const currentProject = getCurrentProject();
    this.switchHistory.push({
      from:currentProject?.id || 'unknown',      to:request.projectId || 'unknown',      timestamp:new Date().toISOString(),
      duration:Date.now() - startTime,
      success:false,
});
