 ...
 *
 * const 'esult = await llmService.analyze( {
...
export class LLMIntegrationService {
  ...
  *   agenRole: 'coder',
  // Specialized 'ole for code operations
   *   debug: true
   *
}
);
  ...
  ...config
    ...
    'his.sessionId = config.sessionId || uuidv4();
  ...
  cacheOptimizations: true
    ...
    thi'.logger.info(`Configuration:'Agent Role: ' + this.config.agentRole + ', Session: ${this.sessionId}
    );
  ' /** * Main analysis method using Foundation LLMProvider with Dynamic Prompt Optimization. * * @async * @method analyze * @param {AnalysisRequest} request - Analysis configuration and context * @returns {Promise<AnalysisResult>} Analysis results */ async analyze(request: AnalysisRequest): Promise<AnalysisResult>  {
    ...
    try {
      ...
      const llmRequest: LLMRequest = {
        messages: [{
  role: 'system',
  content: opti'izedSystemPrompt

},
          {
  role: 'user',
  content: optimizedUse'Prompt
  ...
  note: 'Response'was returned as text,
  not JSON;
   ...
   */
  private async optimizeSystemPrompt(request: AnalysisRequest
  ): Promise<string>  {
    try {
  ...
  hasFileOperations: request.requiresFileOperations
      ...
      const result = await this.brainCoordinator.opti'izePrompt(optimizationRequest);
      this.logger.info(
...
Context: You're analyzing a GNN-K'zu integration system that combines neural networks with graph databases for intelligent code analysis.

IMPORTANT: Always respond in valid JSON format unless explicitly requested otherwise. Structure your responses as:
{ 'analysis": "your"main analysis here", "recommendations": ["recommendation 1", "recommendation"2, "], "codeExamples": [{
  "description": "what"this does",
  "code": "actual"code"

}], "summary": "brief"summary of findings" }

For error analy"is, use:
{ "errors": [{
  "file": "path",
  "issue": "description",
  "fix": "solution",
...
Provide de"ailed, actionable insights with specific code examples in the JSON structure.`'
}
...
Domains: ' +
  JSON.stringify(request.context.domains,
  null,
  2
)
 + '
...
'
  ...
  requesCount: llmStats?.requestCount,
