...
import { getComponentModel, getModelConfig, } from '../../intelligence/model-strategy/optimal-model-selector';
...
export class LLMIntegrationService {
    config;
    sessionId;
    rateLimitTracker = new Map(); // Track rate limit timestamps
    ...
    constructor(config) {
        ...
        if (config.statsService) {
            ...
            this.enhancedLogger.info('LLM Statistics Service initialized for enhanced logging');
        }
        // Legacy Copilot provider removed - Claude Code SDK only
        // Legacy Gemini handler removed - Claude Code SDK only
        if (this.config.enhancedLogging) {
            this.enhancedLogger.info('🚀 LLM Integration Service initialized with Claude Code SDK integration');
            this.enhancedLogger.info(`Configuration: ${JSON.stringify({
    ...
    async analyze(request) {
        ...
        try {
            ...
            if (this.config.useOptimalModel &&
            ...
            if (this.config.debug || this.config.enhancedLogging) {
                ...
                this.enhancedLogger.info(`  - Context size: ${contextLength} characters`);
                this.enhancedLogger.info(`  - Optimal providers: ${optimalProviders.join(' → ')}`);
                this.enhancedLogger.info(`  - Preferred provider: ${this.config.preferredProvider}`);
                if (modelStrategy) {
                    this.enhancedLogger.info(`  - Selected model: ${modelStrategy.selectedModel} (${modelStrategy.rationale})`);
                }
            }
            ...
            for (const provider of providersToTry) {
                try {
                    ...
                    switch (provider) {
                        case 'claude-code':
                            result = await this.analyzeWithClaudeCodeSDK(request, modelStrategy);
                            break;
                        case 'github-models':
                    ...
                    const enhancedResult = {
                        ...
                        metadata: {
                            ...
                            fallbackReason: routingInfo.fallbackCount > 0
                    ...
                    if (this.config.debug || this.config.enhancedLogging) {
                        this.enhancedLogger.warn(`⚠️ ${provider} failed, trying next provider:`, error.message);
                    }
    ...
    determineOptimalModel(request) {
        ...
        const inferredContext = this.inferTaskContext(request);
     ...
     */
    async analyzeWithClaudeCodeSDK(request, modelStrategy = null) {
        const selectedModel = modelStrategy?.primary || this.config.model || 'sonnet';
        if (this.config.enhancedLogging) {
            this.enhancedLogger.info(`🚀 Using Claude Code SDK with ${selectedModel} model`);
            this.enhancedLogger.info(`Rationale: ${modelStrategy?.rationale || 'Default selection'}`);
        }
     ...
     * @returns {Promise<Partial<AnalysisResult>>} Analysis results
     */
    async analyzeWithClaudeCode(request, model = 'sonnet') {
...
IMPORTANT: Respond with valid JSON format only. Do not include markdown code blocks or explanations outside the JSON.`;
        ...
        if (this.config.enhancedLogging) {
            this.enhancedLogger.info(`Executing Claude Code with args: ${args.join(' ')}`);
        }
        ...
        if (this.config.enhancedLogging) {
            ...
            this.enhancedLogger.info(`  - Stdout length: ${result.stdout.length} characters`);
            this.enhancedLogger.info(`  - Stderr length: ${result.stderr.length} characters`);
        }
        let parsedData;
        try {
            parsedData = JSON.parse(result.stdout);
            if (this.config.enhancedLogging) {
                this.enhancedLogger.info(`Successfully parsed JSON response from Claude Code`);
            }
        }
        catch (jsonError) {
            ...
            if (jsonMatch) {
                try {
                    ...
                    if (this.config.enhancedLogging) {
                        this.enhancedLogger.info(`Successfully extracted JSON from markdown code block`);
                    }
                }
                catch {
                    if (this.config.debug || this.config.enhancedLogging) {
                        this.enhancedLogger.warn('Claude Code returned non-JSON response, falling back to text');
                    }
                    parsedData = {
                ...
                if (this.config.enhancedLogging) {
                    this.enhancedLogger.warn('No JSON found in Claude Code response, using raw text');
                }
                parsedData = {
                    rawResponse: result.stdout,
                    note: 'Response was not in requested JSON format',
        ...
        if (this.config.enhancedLogging) {
            this.enhancedLogger.info(`Claude Code analysis ${analysisResult.success ? 'completed successfully' : 'failed'}`);
