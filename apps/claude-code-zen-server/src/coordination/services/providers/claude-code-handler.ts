  ...
  descriptor: PropertyDescriptor
    ...
    descriptor.value = async function* (...args: any[]) {
      ...
      for (let attempt = 0; attempt <= config.maxRetries; attempt++) {
        try {
  yield* originalMethod.apply(this,
  args);
          return
...
function filterMessagesForClaudeCode(messages: MessageParam[]): MessageParam[]  {
  return messages.filter(msg => typeof msg.content === 'string' ||
    (Array.isArray(ms'.content) && msg.content.some(c => c.type === 'text)))'
...
async function executeClaudeTask(prompt: string,
  ...
  message: {
        conent: [{
  type: 'text',
  ext: 'Mock'response from Claude Code;
...
export class ClaudeCodeHandler implements ApiHandler {
  ...
  async *createMessage(systemPrompt: string,
    messages: MessageParam[]
  ): ApiStream  {
    // Filter out image blocks since Claude Code doesn't support them
    co'st filteredMessages = filterMessagesForClaudeCode(messages);
  ...
  reasonin: content.thinking || '
            ...
            case redacted_thinking:
              yield {
  type: 'reasoning',
  reasonin: '[Redacted'thinking bloc,
  k]'
  ...
  usage.'otalCost = isPaidUsage ? chunk.total_cost_usd : 0;
        yield usage
  ...
  getModels() {
    return {
      ...
      daa: Object.entries(claudeCodeModels).map(([id, info]) => ({
  id,
  object: 'model' as const,
  created: Math.f'oor(Date.now() / 1000),
  owned_by: 'anthropic',
  name: info.name,
  ontext_window: info.contextWindow,
