...
class DocumentManager {
  async searchDocuments(
    query: any
  ): Promise<{ success: boolean; data?: { documents: BaseDocumentEntity[] } }> {
...
export class StrategicVisionService {
  ...
  async createVisionDocument(
    ...
    try {
      ...
      logger.info(
        `${type} document created successfully with ID ${documentId}${
  ...
  private async importFromFiles(
    ...
    try {
      logger.info(
        `Would import files from ${projectPath} for project ${projectId}`
      );
