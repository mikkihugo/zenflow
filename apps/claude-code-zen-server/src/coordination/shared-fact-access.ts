...
// Define minimal fact types locally
interface FactEntry {
  id: string;
  type: string;
  data: any;
...
export interface CoordinationFactAccess {
  ...
  agentId?: string
  ': Promise<string>;
  getCoordinationHistory(limit?: number
): Promise<FactEntry[]>;
  ...
  getAgentInteractions(agentId?: string,
  limit?: number): Promise<FactEntry[]>;
  recordSwarmState(swarmId: string,
  state: any): Promise<string>;
  getSwarmHistory(swarmId: string,
  limit?: number): Promise<FactEntry[]>;
  recordDecision(
  decision: string,
  reasoning: string,
...
export class SharedFactAccess implements CoordinationFactAccess {
  ...
  logger.error('Failed to get coordination history:','
  error);;
      return []
   ...
   */
  async getAgentInteractions(agentId?: string,
    limit = 30
  ): Promise<FactEntry[]>  {
    try {
  ...
  logger.error('Failed to get agent interactions:','
  error);;
      return []
  ...
  lo'ger.error('Failed to record coordination decision:','
  error);;
      throw error
  ...
  metrics
        ...
        ource: metrics:' + component + '',
        confidence: 1.0,
        tags: ['performance', 'metrics', componen, t]
}
)
} catch (error) {
      logger.error(
   ...
   */
  async getPerformanceMetrics(component?: string,
    limit = 15
  ): Promise<FactEntry[]>  {
    try {
      const query: FactQuery = {
   ...
   */
  async searchExternalFacts(query: string,
  sources?: string[],
  limit = 10' {
    try {
      logger.debug('Searching external facts: ' + query + '
);;
      return await searchExternalFacts(
  query,
  sources,
  limit
)
...
export const recordEvent = (event: string, data: any, agentId?: string' =>
  coordinationFactAccess.recordCoordinationEvent(
  event,
  data,
  agentId
);
...
export const recordInteraction = (from: string, to: string, interaction: any) =>
  coordinationFactAccess.recordAgentInteraction(
  from,
  to,
  interaction
);
...
export const recordDecision = (
  ...
  reasoning: string,
  context?: any
) => coordinationFactAccess.recordDecision(
  decision,
  reasoning,
  context
);
...
export const searchExternal = (
  ...
  sources?: string[],
  limit?: number
) => coordinationFactAccess.searchExternalFacts(
  query,
  sources,
  limit
);
