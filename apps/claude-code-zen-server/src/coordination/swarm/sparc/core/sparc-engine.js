...
export class SPARCEngineCore {
    ...
    workflowEngine; // CIRCULAR DEPENDENCY FIX: Use dependency injection
    swarmCoordinator;
    memorySystem;
    taskCoordinator;
    taskAPI;
    ...
    async executePhase(project, phase) {
        ...
        try {
            ...
            const metrics = this.calculatePhaseMetrics(project, phase, deliverables, duration);
    ...
    async executePhaseLogic(project, phase) {
        ...
        switch (phase) {
            case 'specification': {
                ...
                const functionalReqs = specification.filter(req => 'testCriteria' in req && req.type === 'functional');
                const nonFunctionalReqs = specification.filter(req => 'metrics' in req && req.type === 'non-functional');
     ...
     * @since 1.0.0
     */
    calculatePhaseMetrics(project, phase, deliverables, duration) {
    ...
    calculateSpecificationQuality(project) {
        let score = 0.0;
        ...
        if (project.specification?.constraints?.length > 0)
            score += 0.1;
        if (project.specification?.assumptions?.length > 0)
            score += 0.1;
        if (project.specification?.dependencies?.length > 0)
            score += 0.1;
        return Math.min(score, 1.0);
    }
    calculateSpecificationCompleteness(project) {
    ...
    calculateArchitectureQuality(project) {
        let score = 0.0;
        ...
        if (project.architecture?.dataFlow?.length > 0)
            score += 0.2;
        if (project.architecture?.integrationPoints?.length > 0)
            score += 0.2;
        return Math.min(score, 1.0);
    }
    calculateArchitectureCompleteness(project) {
        const requiredSections = [
    ...
    calculateImplementationQuality(project) {
        let score = 0.0;
        if (project.implementation?.sourceCode?.length > 0) {
            score += 0.4;
            // Quality bonus for multiple files
            if (project.implementation.sourceCode.length > 3)
                score += 0.1;
        }
        ...
        if (project.implementation?.documentation?.length > 0)
            score += 0.1;
        return Math.min(score, 1.0);
    }
    calculateImplementationCompleteness(project) {
        const requiredSections = [
