...
export class SpecificationPhaseEngine {
    templateEngine;
    constructor() {
    ...
    async generateSpecificationDocument(analysis) {
        ...
        const specification = {
            ...
            domain: 'general', // Default domain since analysis doesn't have domain
            functionalRequirements,
            nonFunctionalRequirements,
            constraints,
            assumptions,
    ...
    deriveSystemConstraints(requirements) {
        ...
        const hasPerformanceReqs = requirements.some((req) => 'metrics' in req &&
            Object.keys(req.metrics).some((key) => key.includes('performance') ||
                key.includes('speed') ||
    ...
    extractConstraintsFromAnalysis(analysis) {
        return analysis.filter((item) => 'type' in item &&
    ...
    validateHighPriorityRequirements(spec) {
        ...
        return highPriorityReqs.every((req) => spec.acceptanceCriteria.some((ac) => ac.requirement === req.id));
    }
    calculateHighPriorityCompleteness(spec) {
        const highPriorityReqs = spec.functionalRequirements.filter((req) => req.priority === 'HIGH');
        if (highPriorityReqs.length === 0)
            return 1.0;
        const completedHighPriority = highPriorityReqs.filter((req) => spec.acceptanceCriteria.some((ac) => ac.requirement === req.id));
...
//# sourceMappingURL=specification-engine.js.map