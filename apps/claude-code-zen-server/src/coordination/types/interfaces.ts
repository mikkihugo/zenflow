...
export interface AgentMetrics {
  ...
  cpuUsage?: number;
  // CPU usage for queen-coordinator
  memoryUsage?: number;
  // Memory usage for queen-coordinator
  diskUsage?: number;
...
export interface QueenPool {
  available: QueenId[] | number;
  // Can be array or count
  busy: QueenId[];
  offline: QueenId[];
...
export interface QueenHealth {
  ...
  issues: string[];
  queenId?: string;
  // Queen ID for queen-coordinator
  metrics?: Record<string, unknown>;
  // Health metrics for queen-coordinator
  components?: Record<
    string,
    {
  status: 'healthy' | 'degraded' | 'critical' | 'unhealthy';
