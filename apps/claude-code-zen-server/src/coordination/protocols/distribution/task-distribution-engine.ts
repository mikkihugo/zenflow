...
export class TaskDistributionEngine extends TypedEventBase {
  ...
  private async assignTask(
    ...
    this.eventBus.emit('task:assign', {
      ...
      priority:
        task.priority === TaskPriority.MEDIUM ? 'medium' : task.priority,
      dependencies: [],
  ...
  private subtaskToTask(subtask: SubTask, parentId: string): TaskDefinition {
    return {
      ...
      type: subtask.type,
      priority: TaskPriority.MEDIUM, // Inherit from parent or adjust
      complexity: TaskComplexity.SIMPLE, // Subtasks are typically simpler
  ...
  private async handleStuckTask(taskId: string): Promise<void> {
    ...
    await this.reassignTask(taskId, 'task-stuck');
  }
  ...
  private async handleAgentUnavailable(data: any): Promise<void> {
    ...
    if (agent) {
      ...
      for (const [taskId] of affectedAssignments) {
        await this.reassignTask(taskId, 'agent-unavailable');
      }
    }
  }
