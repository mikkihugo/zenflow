...
export class CommunicationProtocols extends TypedEventBase {
  ...
  async sendMessage(message: Partial<Message>): Promise<string> {
    const fullMessage: Message = {
      ...
      encryption: message.encryption || {
        enabled: this.configuration.encryptionEnabled,
      },
      ...
      routing: message.routing || {
        ...
        acknowledgment: true,
        timeout: this.configuration.messageTimeout,
      },
      qos: message.qos || {
        bandwidth: 0,
        latency: 0,
  ...
  private async cleanupExpiredMessages(): Promise<void> {
    ...
    for (const [proposalId, proposal] of this.consensusProposals) {
      if (
        now - proposal.timestamp?.getTime >
        this.configuration.consensusTimeout
      ) {
        this.consensusProposals.delete(proposalId);
        this.consensusVotes.delete(proposalId);
      }
  ...
  private cleanupMessageHistory(): void {
    if (this.messageHistory.size > this.configuration.maxMessageHistory) {
      ...
      const toDelete = sortedMessages.slice(
        0,
        sortedMessages.length - this.configuration.maxMessageHistory
      );
