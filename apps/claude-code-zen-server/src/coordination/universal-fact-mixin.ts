...
export interface FactCapable {
  ...
  queryFacts(query?: any): Promise<any[]>;
  getMyFacts(type?: string, limit?: number): Promise<any[]>;
...
export function withFactCapabilities<TBase extends FactMixinConstructor<{ agentId: string }>>(
  ...
  return class FactEnhanced extends Base implements FactCapable {
     ...
     */
    async getRecentDecisions(limit = 10): Promise<any[]> {
     ...
     */
    async getObservations(category?: string, limit = 20): Promise<any[]> {
      const query: any = {
        type: 'observation',
        source: `agent:${this.agentId}`,
        limit
     ...
     */
    async getLearnings(context?: string, limit = 15): Promise<any[]> {
      const query: any = {
        type: 'learning',
        source: `agent:${this.agentId}`,
        limit
     ...
     */
    onNewFacts(callback: (fact: any) => void): () => void {
      return sharedFactSystem.onFactAdded((fact: any) => {
...
export class FactCapableAgent implements FactCapable {
  ...
  async getMyFacts(type?: string, limit = 100): Promise<any[]> {
    return await queryAgentFacts(
      this.agentId,
      type,
      limit
    );
