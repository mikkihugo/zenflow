...
export class HealthMonitor extends TypedEventBase {
  ...
  unregisterHealthCheck(name: string): boolean {
    ...
    if (removed) {
      logger.info('🗑️ Removed health check: ' + name);
    }
    return removed;
  }
  ...
  async runHealthChecks(): Promise<HealthReport> {
    ...
    checkResults.forEach((result, index) => {
      ...
      if(result.status === 'fulfilled') {
        const { score, status, details, metrics, duration } = result.value;
    ...
    logger.debug('✅ Health check completed: ' + overallScore + '% (${duration.toFixed(1)}ms)');
  ...
  private initializeSystemChecks(): void {
    ...
    this.registerHealthCheck(
      ...
      {
        weight: 1,
        description: 'Event loop performance monitoring'
      }
    );
  ...
  private async processHealthAlerts(healthReport: HealthReport): Promise<void> {
    ...
    if(status === 'critical') {
      ...
      logger.warn('⚠️ WARNING: System health at ' + overallScore + '%');
    }
