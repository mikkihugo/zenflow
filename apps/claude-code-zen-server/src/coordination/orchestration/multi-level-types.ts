...
export interface StreamConfiguration {
  ...
  readonly gateConfiguration: GateConfiguration;
...
export interface GateConfiguration {
  readonly enableBusinessGates: boolean;
  readonly enableTechnicalGates: boolean;
...
export interface AutoScalingConfig {
  ...
  readonly scalingCooldown: number;
}
 ...
 */
export type PortfolioItemStatus ='' | '''proposed | evaluating' | 'approved''' | '''in_progress | on_hold' | 'completed''' | '''cancelled';

/**
 * Portfolio priority levels
 */
export enum PortfolioPriority {
  STRATEGIC = 'strategic', // Critical for business strategy
  HIGH = 'high', // High business value
  MEDIUM = 'medium', // Standard priority
  LOW = 'low', // Nice to have
  EXPERIMENTAL = 'experimental', // Innovation/research
}
...
export interface PortfolioTimeline {
  ...
  readonly phases: PortfolioPhase[];
}
...
export interface PortfolioGate {
  readonly id: string;
  readonly type: PortfolioGateType;
  readonly name: string;
  readonly description: string;
...
export interface GateCriteria {
  ...
  readonly threshold: number; // Minimum score required
...
/**
 * Program-level work item (Epic with coordination context)
 */
 ...
 * Program item status
 */
export type ProgramItemStatus ='' | '''planned | designing' | 'development''' | '''testing | integration' | 'completed''' | '''blocked';
...
export enum ComplexityLevel {
  ...
  COMPLEX = 'complex', // Significant technical challenges
  HIGHLY_COMPLEX = 'highly_complex', // Research/innovation required
}
...
export interface ProgramTimeline {
  readonly startDate: Date;
  readonly endDate: Date;
  readonly phases: ProgramPhase[];
  readonly checkpoints: ProgramCheckpoint[];
}
...
export interface ArchitectureSpec {
  ...
  readonly dataFlow: DataFlowSpec[];
...
/**
 * Program-level gates (technical decisions)
 */
export interface ProgramGate {
  readonly id: string;
  readonly type: ProgramGateType;
  readonly name: string;
  readonly description: string;
  readonly requiredApprovers: string[];
  readonly technicalCriteria: TechnicalCriteria[];
  readonly status: GateStatus;
...
export interface AIRecommendation {
  ...
  readonly risks: string[];
  readonly mitigations: string[];
  readonly conditions?: string[]; // If conditional approval
}
...
export interface CoordinationInfo {
  ...
  readonly aiAssistance: AIAssistanceLevel;
...
export interface SwarmExecutionItem {
  ...
  readonly metrics: SwarmExecutionMetrics;
  readonly createdAt: Date;
  readonly updatedAt: Date;
}
 ...
 */
export type SwarmExecutionStatus ='' | '''queued | analyzing' | 'designing''' | '''implementing | testing' | 'reviewing' | 'completed' | 'failed';

/**
 * Swarm execution priority
 */
export enum SwarmExecutionPriority {
  URGENT = 'urgent',
  HIGH = 'high',
  NORMAL = 'normal',
  LOW = 'low',
...
export interface SwarmTimeline {
  readonly startDate: Date;
  readonly endDate: Date;
  readonly sparcPhases?: SPARCPhaseTimeline[];
  readonly testingWindows: TestingWindow[];
  readonly deploymentWindows: DeploymentWindow[];
}

/**
 * SPARC phase timeline
 */
export interface SPARCPhaseTimeline {
...
export interface QualityGate {
  ...
  readonly results?: QualityResults;
}
...
export interface MultiLevelOrchestratorState {
  readonly portfolioStreams: WorkflowStream<PortfolioItem>[];
  readonly programStreams: WorkflowStream<ProgramItem>[];
  readonly executionStreams: WorkflowStream<SwarmExecutionItem>[];
 ...
 */
export interface ProductWorkflowIntegration {
  readonly workflowId: string;
  readonly orchestrationLevel: OrchestrationLevel;
  readonly streamId: string;
  readonly workItemId: string;
  readonly sparcIntegration?: SPARCIntegration;
  readonly aguiIntegration?: AGUIIntegration;
}

/**
 * SPARC methodology integration
 */
export interface SPARCIntegration {
  readonly projectId: string;
  readonly currentPhase: SPARCPhase;
  readonly automation: boolean;
  readonly humanOversight: boolean;
...
export interface SystemPerformanceMetrics {
  ...
  readonly levelThroughput: Record<OrchestrationLevel, number>;
...
export interface ResourceUtilization {
  ...
  readonly agents: AgentUtilization;
  readonly swarms: SwarmUtilization;
  readonly humans: HumanUtilization;
}
 ...
 * Human utilization
 */
export interface HumanUtilization {
