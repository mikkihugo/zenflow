...
export class CollectiveFACTSystem extends EventEmitter {
            ...
            switch (type) {
                ...
                case 'github-repo':
                    ...
                    try {
                        ...
                        const githubProcess = spawn(factToolsPath, ['github', '--repo', subject, '--format', 'json', '--verbose'], {
                            cwd: __dirname,
     ...
     * @param subject
     */
    async gatherFactFallback(type, subject) {
        try {
            let result = null;
            let sources = [];
            let confidence = 0.5;
     ...
     */
    async fetchNPMDownloads(packageName) {
        try {
     ...
     */
    async fetchAPIDocumentation(apiName) {
        try {
