...
export const ERROR_MESSAGES = {
    SWARM_NOT_FOUND: (id) => `Swarm not found: ${id}`,
    AGENT_NOT_FOUND: (id) => `Agent not found: ${id}`,
...
export const isValidAgentStatus = (status) => {
    return Object.values(STATUS_CONSTANTS.AGENT_STATUS).includes(status);
};
export const isValidTaskStatus = (status) => {
    return Object.values(STATUS_CONSTANTS.TASK_STATUS).includes(status);
};
export const isValidCognitivePattern = (pattern) => {
    return pattern in COGNITIVE_PATTERNS;
};
//# sourceMappingURL=swarm-constants.js.map