...
export interface SPARCDocumentEvents {
  ...
  'sparc:project:created': { story: StoryDocumentEntity; feature: FeatureDocumentEntity; sparcProject: SPARCProject; workAssignment: WorkAssignment };
  'sparc:phase:completed': { story: StoryDocumentEntity; feature: FeatureDocumentEntity; phase: string; deliverables: any[]; metrics: any };
  'sparc:tasks:generated': { story: StoryDocumentEntity; feature: FeatureDocumentEntity; tasks: TaskDocumentEntity[]; methodologyResult: MethodologyResult };
  'sparc:project:completed': { story: StoryDocumentEntity; feature: FeatureDocumentEntity; result: MethodologyResult };
  'sparc:project:failed': { story: StoryDocumentEntity; feature: FeatureDocumentEntity; error: Error };
  
  // Bridge events
  'sparc:workflow:initialized': { workflowId: string; aiOrchestration: AIOrchestrationConfig };
  'sparc:neural:coordination': { agents: ClaudeZenAgentProfile[]; consensusResult: any };
  'sparc:swarm:intelligence': { swarmMetrics: any; emergentBehaviors: any };
  'sparc:quality:gates': { qualityGates: ClaudeZenQualityGate[]; validationResults: any };
}
 ...
 */
export class SPARCDocumentIntegration extends EventEmitter<SPARCDocumentEvents> {
  ...
  private config: SPARCIntegrationConfig;
  ...
  constructor(
    ...
    this.config = {
      ...
      claudeZenIntegration: {
        ...
        escalationSettings: {
          ...
          escalationThresholds: [{ metric: 'quality-gate-failure-rate', threshold: 0.3, timeWindow: 3600000, severity: 'high' }],
    ...
    this.sparcCommander = new SPARCCommander(this.config);
    ...
    this.logger.info('SPARC Document Integration initialized for Story-level execution');
  }
  ...
  async createSPARCProjectFromStory(
    ...
    try {
      ...
      if (this.activeProjects.size >= this.config.maxConcurrentProjects) {
        throw new Error(`Maximum concurrent SPARC projects (${this.config.maxConcurrentProjects}) reached`);
      }
      ...
      const requirements = [
        ...
        ...(story.definition_of_done ? [`Definition of Done: ${story.definition_of_done}`] : []),
        `Feature Context: ${feature.title} - ${feature.description || feature.content}`,
        ...(feature.technical_approach ? [`Technical Approach: ${feature.technical_approach}`] : [])
      ];

      // Determine domain from Story and Feature context
      const domain = this.mapStoryTypeToSPARCDomain(story.story_type || 'user_story', feature.feature_type || 'general');
      ...
      const methodologyResult = await this.sparcCommander.executeMethodology(sparcProject);

      // Assign to database bridge for integration
      const workAssignmentId = await this.databaseSPARCBridge.assignFeatureToSparcs(feature);
      ...
      this.activeProjects.set(story.id, { 
        ...
        workAssignment: {
          ...
          priority: story.priority as 'low' | 'medium' | 'high' | 'critical' || 'medium',
          requirements: requirements,
          context: {
      ...
      this.logger.info(`Created SPARC project ${sparcProject.id} for story "${story.title}" with feature context "${feature.title}" using direct Story-level execution`);
   ...
   */
  async executeSPARCMethodology(story: StoryDocumentEntity): Promise<MethodologyResult> {
    try {
      ...
      if (methodologyResult && methodologyResult.success) {
        ...
        const implementation = workStatus.completed.find(impl => 
          impl.workAssignmentId === activeProject.workAssignment.id
        );
        ...
        this.emit('sparc:project:completed', { story, feature, result: methodologyResult });
        this.emit('sparc:tasks:generated', { story, feature, tasks, methodologyResult });
        
        // Clean up active project
        this.activeProjects.delete(story.id);
        
        this.logger.info(`SPARC methodology completed successfully for story "${story.title}" in feature "${feature.title}" using direct execution`);
      } else {
        this.emit('sparc:project:failed', { story, feature, error: new Error('SPARC methodology execution failed') });
  ...
  private async generateTasksFromSPARCDeliverables(
    ...
    for (const deliverable of deliverables) {
      const task: TaskDocumentEntity = {
        ...
        author: 'sparc-system',
        tags: ['sparc-generated', deliverable.type, story.story_type, feature.feature_type],
        ...
        implementation_details: {
          ...
          documentation_updates: deliverable.type === 'documentation' ? [deliverable.path] : []
        },
        ...
        sparc_implementation_details: {
          ...
          parent_feature_sparc_id: feature.sparc_implementation?.sparc_project_id,
          sparc_phase_assignment: this.mapDeliverableToSPARCPhase(deliverable.type),
          sparc_deliverable_type: this.mapDeliverableToSPARCDeliverableType(deliverable.type),
          sparc_quality_gates: [{
          ...
          complexity_analysis: {
            ...
            performance_impact: deliverable.metrics.complexity > 0.5 ? 'high' : 'low'
          },
    ...
    this.logger.info(`Generated ${tasks.length} traceable tasks from SPARC deliverables for story "${story.title}" in feature "${feature.title}"`);
    return tasks;
  }
  ...
  private async updateStoryWithSPARCIntegration(
    ...
    const updates = {
      sparc_implementation: {
        ...
        sparc_phases: {
          specification: { status: 'pending' as const, deliverables: [], completion_date: undefined, quality_score: undefined },
          pseudocode: { status: 'pending' as const, deliverables: [], completion_date: undefined, algorithms: [] },
          architecture: { status: 'pending' as const, deliverables: [], completion_date: undefined, components: [] },
          refinement: { status: 'pending' as const, deliverables: [], completion_date: undefined, optimizations: [] },
          completion: { status: 'pending' as const, deliverables: [], completion_date: undefined, artifacts: [] }
        },
  ...
  private async updateStoryWithResults(
    ...
    const completionPercentage = (result.completedPhases / result.totalPhases) * 100;
  ...
  // Helper methods
  private mapStoryTypeToSPARCDomain(storyType: string, featureType: string): string {
    const storyMapping = {
  ...
  private mapDeliverableToTaskType(deliverableType: string): 'development' | 'testing' | 'documentation' | 'deployment' | 'research' {
    const mapping = {
  ...
  private mapDeliverableToSPARCDeliverableType(deliverableType: string): string {
    const mapping = {
  ...
  private estimateCostFromStory(story: StoryDocumentEntity): number {
    ...
    const priorityMultiplier = story.priority === 'high' ? 1.5 : story.priority === 'critical' ? 2.0 : 1.0;
      ...
      sparc_implementation: {
        ...
        sparc_phases: {
          ...
          pseudocode: { 
            ...
            completion_date: sparcResult.phases.pseudocode ? new Date() : undefined, 
            algorithms: [] 
          },
          architecture: { 
          ...
          refinement: { 
            ...
            completion_date: sparcResult.phases.refinement ? new Date() : undefined, 
            optimizations: [] 
          },
          completion: { 
  ...
  private async updateStoryWithBridgeResults(
    ...
    const updates = {
      ...
      'sparc_implementation.sparc_progress_percentage': result.metrics.completeness,
  ...
  private async generateTasksFromSPARCBridge(
    ...
    const phaseMapping = [
      { phase: 'specification', content: sparcResult.specification, type: 'research' },
      { phase: 'architecture', content: sparcResult.architecture, type: 'development' }, 
      { phase: 'implementation', content: sparcResult.implementation, type: 'development' }
    ];
    ...
    for (const { phase, content, type } of phaseMapping) {
      if (content) {
        const task: TaskDocumentEntity = {
          ...
          tags: ['sparc-generated', phase, story.story_type, feature.feature_type, 'bridge-integrated'],
          ...
          keywords: [phase, feature.feature_type, 'sparc-generated', 'bridge-integrated', 'ai-orchestrated'],
          workflow_stage: 'implementation',
          completion_percentage: 0
        };
    ...
    this.logger.info(`Generated ${tasks.length} traceable tasks from SPARC bridge deliverables for story "${story.title}" in feature "${feature.title}"`);
    return tasks;
  }
  ...
  private setupEventHandlers(): void {
    ...
    this.sparcCommander.on('project:initialized', (event) => {
      this.emit('sparc:workflow:initialized', { 
        ...
        aiOrchestration: this.config.aiOrchestration 
      });
    });
   ...
   */
  getActiveProjects(): Array<{ story: StoryDocumentEntity; feature: FeatureDocumentEntity; sparcProject: SPARCProject; methodologyResult?: MethodologyResult }> {
   ...
   */
  getProjectByStory(storyId: string): { story: StoryDocumentEntity; feature: FeatureDocumentEntity; sparcProject: SPARCProject; methodologyResult?: MethodologyResult } | undefined {
   ...
   */
  getProjectByFeature(featureId: string): { story: StoryDocumentEntity; feature: FeatureDocumentEntity; sparcProject: SPARCProject; methodologyResult?: MethodologyResult } | undefined {
    // Find first story project for this feature - in practice should query all stories for feature
    return Array.from(this.activeProjects.values()).find(project => project.feature.id === featureId);
  }
}