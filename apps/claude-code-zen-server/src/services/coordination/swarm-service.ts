...
import type {
  ...
  ServiceResult,
  TranslationError
} from '../types/service-integration-layer';
...
export class SwarmService extends EventEmitter {
  ...
  private readonly config: SwarmServiceConfig;
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.brainCoordinator = new BrainCoordinator({
        ...
        cognitivePatterns: ['neural-adaptive', 'distributed-reasoning', 'collaborative-learning']
      });
      ...
      this.agentManager = new AgentManager({
        maxAgents: this.config.maxSwarms * this.config.maxAgentsPerSwarm,
      ...
      this.logger.info('Swarm Service facade initialized successfully with @claude-zen delegation');
   ...
   */
  async initializeSwarm(config: SwarmConfiguration, context: ServiceContext): Promise<ServiceResult<SwarmInitResult, TranslationError>> {
    ...
    try {
      ...
      const swarm = await this.brainCoordinator.createSwarm({
        ...
        maxAgents: Math.min(config.maxAgents || 10, this.config.maxAgentsPerSwarm),
      ...
      this.logger.info(`Swarm ${swarm.id} initialized via brain coordinator delegation`);
   ...
   */
  async spawnAgent(swarmId: string, config: AgentConfig, context: ServiceContext): Promise<ServiceResult<AgentSpawnResult, TranslationError>> {
    if (!this.initialized) await this.initialize();
    ...
    try {
      ...
      this.logger.info(`Agent ${agent.id} spawned via delegation`, { swarmId, type: config.type });
      this.emit('agent:spawned', { agentId: agent.id, swarmId, context, result });
   ...
   */
  async orchestrateTask(config: TaskOrchestrationConfig, context: ServiceContext): Promise<ServiceResult<TaskOrchestrationResult, TranslationError>> {
    if (!this.initialized) await this.initialize();
    ...
    try {
      ...
      const execution = await this.collaborationEngine.coordinateExecution(task, {
        enableRealTimeSync: true,
        enableConflictResolution: true,
      ...
      const result: TaskOrchestrationResult = {
        ...
        metrics: this.performanceTracker?.getMetrics('task_orchestration') || {}
      };

      this.logger.info(`Task ${task.id} orchestrated via delegation`, { strategy: config.strategy });
  ...
  private setupEventForwarding(): void {
    ...
    this.brainCoordinator?.on('swarm-created', (data) => this.emit('swarm:created', data));
    this.brainCoordinator?.on('swarm-status-changed', (data) => this.emit('swarm:status-changed', data));
    this.brainCoordinator?.on('neural-pattern-learned', (data) => this.emit('neural:pattern-learned', data));

    // Forward agent manager events
    this.agentManager?.on('agent-created', (data) => this.emit('agent:created', data));
    this.agentManager?.on('agent-status-changed', (data) => this.emit('agent:status-changed', data));

    // Forward workflow engine events
    this.workflowEngine?.on('task-created', (data) => this.emit('task:created', data));
    this.workflowEngine?.on('task-completed', (data) => this.emit('task:completed', data));

    // Forward collaboration events
    this.collaborationEngine?.on('coordination-event', (data) => this.emit('coordination:event', data));
    this.collaborationEngine?.on('conflict-resolved', (data) => this.emit('coordination:conflict-resolved', data));

    // Forward monitoring events
    this.healthMonitor?.on('health-status-changed', (data) => this.emit('health:status-changed', data));
    this.performanceTracker?.on('performance-alert', (data) => this.emit('performance:alert', data));
  }
  ...
  async shutdown(): Promise<void> {
    try {
      ...
      this.logger.error('Error during Swarm Service shutdown:', getErrorMessage(error));
      throw error;
    }
  }
}
 ...
 */
export function createSwarmService(config?: Partial<SwarmServiceConfig>): SwarmService {
