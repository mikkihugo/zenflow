...
export class ArchitectureRunwayService extends BaseDocumentService<any> {
  ...
  async createArchitectureRunway(
    ...
    try {
      ...
      this.logger.info(
        `Created Architecture Runway ${runwayId}: ${options.title}`
      );
    ...
    try {
      ...
      await this.updateDocument(oldRunway.id, {
        ...
        ...(this.currentMode === 'safe' && {
          superseded_by: newRunway.runway_id,
        }),
      });
  ...
  async queryRunways(
    ...
    if (options.decisionStatus) {
      filteredRunways = filteredRunways.filter(
        (runway) => runway.decision_status === options.decisionStatus
      );
    }
