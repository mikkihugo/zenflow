...
export class FeatureService extends BaseDocumentService<FeatureEntity> {
  ...
  protected formatDocumentContent(data: Partial<FeatureEntity>): string {
    ...
    content += data.metadata?.enablerType
      ? `*Enabler Feature - ${data.metadata.enablerType}*\n\n`
      : `*Business Feature*\n\n`;
    ...
    if (
      ...
      Array.isArray(data.metadata.teamAssignments)
    ) {
  ...
  async createFeature(options: FeatureCreateOptions): Promise<FeatureEntity> {
    ...
    try {
      ...
      this.logger.info(
        `Created Feature: ${options.title} (${options.enablerType ? 'Enabler : Business'})`
      );
  ...
  async updateBenefitValidation(
    ...
    try {
      ...
      this.logger.info(
        `Updated benefit validation for Feature ${featureId}: ${validation.validationStatus}`
      );
      this.emit('benefitValidationUpdated', {
  ...
  async queryFeatures(
    ...
    if (options.artId) {
      filteredFeatures = filteredFeatures.filter(
        (feature) => feature.metadata?.artId === options.artId
      );
    }
    ...
    if (options.enablerType) {
      filteredFeatures = filteredFeatures.filter(
        (feature) => feature.metadata?.enablerType === options.enablerType
      );
    }
    ...
    if (options.teamId) {
      filteredFeatures = filteredFeatures.filter(
        (feature) =>
          ...
          (feature.metadata.teamAssignments as string[]).includes(
            options.teamId!
          )
      );
    }
    ...
    if (options.implementationStatus) {
      filteredFeatures = filteredFeatures.filter(
        (feature) =>
          feature.metadata?.implementationStatus ===
  ...
  async getFeatureStats(): Promise<FeatureStats> {
    try {
      const { documents: features } = await this.queryDocuments({
        limit: 1000,
      });
      ...
      stats.averageStoriesPerFeature =
        features.length > 0 ? stats.totalStories / features.length : 0;
