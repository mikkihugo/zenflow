...
export const ArchitectureRunwayV3Schema = ArchitectureRunwayV2Schema.extend({
  ...
  architecture_impact: z
    .enum(['foundation, system', 'solution, enterprise'])
    .default('system'),
...
export class DocumentSchemaManager {
  ...
  getSchema(
    ...
    const schema =
      this.schemas[documentType as keyof typeof this.schemas]?.[version];
    if (!schema) {
      throw new Error(
  ...
  createDocument(
    ...
    this.logger.info(
      `Created ${documentType} document with ${mode} schema v${result.schema_version}`
    );
  ...
  migrateDocument(document: any, targetMode:'kanban | agile''' | '''safe'): any {
    ...
    try {
      const migrated = targetSchema.parse({
        ...
        schema_version:
          this.modeVersions[targetMode][
            documentType as keyof (typeof this.modeVersions)[typeof targetMode]
          ],
      });
