...
export const DEFAULT_SERVER_CONFIG:Partial<ServerConfig> = {
  port:3000,
  host: '0.0.0.0',  environment: 'development',  timeout:30000,
  maxConnections:1000,
  enableMetrics:true,
  enableEvents:true,
  database:{
    type: 'sqlite',    path: './data/server.db',},
  security:{
    enableCors:true,
    enableHelmet:true,
    rateLimiting:{
...
export class ClaudeZenServerImpl implements UnifiedClaudeZenServer {
  ...
  shutdown():Promise<Result<void, Error>> {
    ...
    return safeAsync(async () => {
      ...
      if (stopResult.isErr()) {
        logger.error(
          'Error during server stop in shutdown',          stopResult.unwrapErr()
        );
}
  ...
  async getHealth():Promise<HealthCheckResult> {
    ...
    if (this.adapters.http) {
      try {
        ...
        healthChecks.push({
          name: 'http',          status:httpHealth.healthy ? 'ok' : ' error',          message:httpHealth.message,
});
} catch (error) {
        healthChecks.push({
          name: 'http',          status: 'error',          message:`HTTP adapter health check failed: ${error}`,
});
}
}
    ...
    if (this.adapters.database) {
      try {
        ...
        healthChecks.push({
          name: 'database',          status:dbHealth.healthy ? 'ok' : ' error',          message:dbHealth.message,
});
} catch (error) {
        healthChecks.push({
          name: 'database',          status: 'error',          message:`Database adapter health check failed: ${error}`,
});
}
}
...
export class ClaudeZenServerFactory implements ServerFactory {
  async createWebApiServer(
    ...
    const webApiConfig:Partial<ServerConfig> = {
      ...
      environment: 'production',      enableMetrics:true,
      enableEvents:true,
      security:{
        enableCors:true,
        enableHelmet:true,
  ...
  async createCoordinationServer(
    ...
    const coordinationConfig:Partial<ServerConfig> = {
      ...
      environment: 'development',      enableMetrics:true,
      enableEvents:true,
      database:{
        type: 'sqlite',        path: './data/coordination.db',},
};
  ...
  createDevelopmentServer(
    ...
    const devConfig:Partial<ServerConfig> = {
      ...
      host: '127.0.0.1',      environment: 'development',      enableMetrics:false,
