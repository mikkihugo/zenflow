...
import { getLogger, withTimeout } from '@claude-zen/foundation';
...
/**
 * Document listing filters interface
 */
...
/**
 * SPARC project creation response interface
 */
interface SPARCProjectResponse {
  sparcProject: {
    id: string;
    status: string;
    phases: string[];
  };
  storyDocument: StoryDocumentEntity;
  featureDocument: {
    id: string;
    title: string;
...
/**
 * Task entity interface for SPARC-generated tasks
 */
...
export const createDocumentRoutes = (): Router => {
  ...
  const initializeServices = async () => {
    try {
      ...
      const documentManager = new DocumentManager(databaseSystem);
      ...
      logger.error('Failed to initialize document services', { error });
      throw error;
    }
  };
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      try {
        ...
        const filters: DocumentFilters = {
          ...
          ...(search && { search_text: search as string }),
        };
        ...
        const result = await withTimeout(
          services.documentManager.listDocuments(filters, paginationOptions),
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      log(LogLevel.DEBUG, 'Getting document by ID', req, {
        documentId,
        mode,
      });
      ...
      try {
        const services = await initializeServices();
        const document = await services.documentManager.getDocument(documentId);
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      const { mode = 'safe' } = req.query;
      const documentData: DocumentCreationData = req.body;
      ...
      try {
        ...
        const createdDocument =
          await services.documentManager.createDocument(newDocument);
  ...
  router.put(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      log(LogLevel.INFO, 'Updating document', req, {
        documentId,
        mode,
      });
      ...
      try {
        ...
        const existingDocument =
          await services.documentManager.getDocument(documentId);
        ...
        // Save updates
        await services.documentManager.updateDocument(
          documentId,
          finalDocument
        );
  ...
  router.delete(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      try {
        ...
        await services.documentManager.deleteDocument(documentId);
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      try {
        ...
        const featureDocument = featureId
          ? (await services.documentManager.getDocument(featureId)) || null
        ...
        if (!featureDocument || featureDocument.type !== 'feature') {
          return res.status(400).json({
            ...
            provided: {
              featureId,
              documentType: featureDocument?.type,
            },
          });
        }

        const sparcProject =
          await services.sparcIntegration.createSPARCProjectFromStory(
            document as StoryDocumentEntity,
        ...
        const response: SPARCProjectResponse = {
          sparcProject,
          storyDocument: document as StoryDocumentEntity,
          featureDocument: featureDocument as any,
          safeHierarchy: {
            businessEpicId: (featureDocument as any).parent_business_epic_id,
            programEpicId: (featureDocument as any).parent_program_epic_id,
            featureId: featureDocument.id,
            storyId: document.id,
          },
        };
  ...
  router.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      try {
        ...
        const result = await services.sparcIntegration.executeSPARCMethodology(
          document as StoryDocumentEntity
        );
        ...
        const activeProject =
          services.sparcIntegration.getProjectByStory(documentId);
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      try {
        ...
        const generatedTasks: TaskEntity[] =
          (await getTasksGeneratedByStory(
            documentId,
            services.documentManager
          )) || [];
        ...
        res.json({
          ...
          generatedTasks: {
            ...
            tasks: generatedTasks.map((task: TaskEntity) => ({
              id: task.id,
              title: task.title,
              status: task.status,
          ...
          traceabilityChain: {
            ...
            tasks: generatedTasks.map((task) => task.id),
          },
          timestamp: new Date().toISOString(),
        });
      } catch (error) {
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      try {
        const services = await initializeServices();
        const activeProjects = services.sparcIntegration.getActiveProjects();
  ...
  router.get(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      try {
        ...
        const children =
          await services.documentManager.listDocuments(childrenFilter);
