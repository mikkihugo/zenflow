...
export class ApiServer {
  ...
  private setupMiddleware():void {
    ...
    const corsOptions = {
      ...
      allowedHeaders:[
        'Content-Type',        'Authorization',        'X-Requested-With',        'X-Session-D',],
    ...
    const limiter = rateLimit({
      ...
      message: 'Too many requests from this IP, please try again later.',      standardHeaders:true,
      legacyHeaders:false,
});
  ...
  private setupSystemCapabilityRoutes():void {
    ...
    this.app.use(
      '/api/v1/system/capability',      this.systemCapabilityRoutes.getRouter()
    );
  ...
  private async handleReadinessProbe(
    ...
    try {
      ...
      res.status(503).json({
        status: 'not_ready',        error: 'Health check failed',        timestamp:new Date().toISOString(),
});
}
}
  ...
  private handleStartupProbe(req:Request, res:Response): void {
    ...
    res.status(started ? 200:503).json({
      status:started ? 'started' : ' starting',      uptime:process.uptime(),
      timestamp:new Date().toISOString(),
});
}
  ...
  private async buildHealthResponse():Promise<Record<string, unknown>> {
    const health:Record<string, unknown> = {
      ...
      environment:process.env.NODE_ENV || 'development',      checks:{
        filesystem: 'healthy',        memory: 'healthy',        uptime: 'healthy',},
};
  ...
  private setupSystemRoutes():void {
    ...
    this.app.get('/api/info', (req:Request, res:Response) => {
      res.json({
        name: 'Claude Code Zen',        version:getVersion(),
        description:
          'AI swarm orchestration platform with neural networks and web dashboard',        endpoints:{
          health:HEALTH_ENDPOINT,
          status:STATUS_ENDPOINT,
          info: '/api/info',          workspace:WORKSPACE_FILES_ENDPOINT,
          // Control API endpoints
          controlLogs: '/api/v1/control/logs',          controlMetrics: '/api/v1/control/metrics',          controlNeural: '/api/v1/control/neural/status',          controlProjects: '/api/v1/control/sparc/projects',          controlEvents: '/api/events/ws',          controlRegistry: '/api/v1/control/events/registry',          controlGit: '/api/v1/control/git/status',          controlConfig: '/api/v1/control/config',          controlServices: '/api/v1/control/services',          controlRealtime: '/api/v1/control/realtime',},
        features:[
          'Production Middleware',          'Rate Limiting',          'Security Headers',          'Graceful Shutdown',          'CORS Support',          'Request Logging',          'Compression',          'Centralized LogTape Database Storage',          'Lightweight OpenTelemetry Metrics',          'Real-time WebSocket Monitoring',          'Neural System Control',          'Real-time Event System Visualization',          'Git Operations Control',          'System Configuration Management',],
});
});
}
    ...
    this.app.use('/api/*', (req:Request, res:Response) => {
      res.status(404).json({
        ...
        availableEndpoints:[
          HEALTH_ENDPOINT,
          STATUS_ENDPOINT,
          '/api/info',          WORKSPACE_FILES_ENDPOINT,
          '/api/v1/control/* (comprehensive control APIs)',          '/api/events/ws (WebSocket event streaming)',],
});
});
}
  ...
  stop():Promise<void> {
    return new Promise((resolveServerStop) => {
      ...
      const shutdownMonitor:{
        ...
        startTime:Date.now(),
        activeConnections:0,
        shutdownState: 'initiating',        gracefulTimeout:10000, // 10 seconds
};
    ...
    return ():Promise<{
      ...
      switch (type) {
        case 'health':
          ...
          return {
            ...
            status: 'healthy',            checks:{
              process: 'ok',              memory:
                memUsage.heapUsed < memUsage.heapTotal * 0.9 ? 'ok' : ' warning',},
};
        ...
        case 'readyz':{
          ...
          return {
            ...baseInfo,
            status:isReady ? 'ready' : ' starting',            checks:{
              uptime:isReady ? 'ok' : ' starting',              server: 'ok',},
};
}
  ...
  private getCorsOrigins():string[] | boolean {
    ...
    if (nodeEnv === 'development') {
      ...
      return [
        'http://localhost:3000',        'http://localhost:3001',        'http://localhost:3002',        'http://localhost:4000',        'http://localhost:5000',        'http://localhost:8080',        'http://127.0.0.1:3000',        'http://127.0.0.1:3001',        'http://127.0.0.1:3002',        'http://127.0.0.1:4000',        'http://127.0.0.1:5000',        'http://127.0.0.1:8080',];
}
  ...
  private initializeWebSocketService():void {
    ...
    try {
      ...
      this.logger.info('✅ Event registry initialized and broadcasting started');
} catch (error) {
