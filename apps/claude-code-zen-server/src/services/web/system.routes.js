...
import { createHealthDataProviders, getCapabilityScores, getSystemCapabilityData, } from '@claude-zen/foundation/system-capability-data-provider';
...
export class SystemCapabilityRoutes {
     ...
     */
    async handleGetFacades(req, res) {
        try {
     ...
     */
    async handleGetDetailed(req, res) {
        try {
        ...
        try {
            ...
            res.json({
                ...
                data: {
                    scores,
                    summary: {
                        average: Object.values(scores).reduce((sum, score) => sum + score, 0) /
