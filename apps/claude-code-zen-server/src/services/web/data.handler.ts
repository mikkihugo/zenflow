...
import { SafeFramework, TaskMaster, WorkflowEngine} from '@claude-zen/coordination';
...
export class WebDataService {
  ...
  private buildSystemHealthStatus(taskStats:unknown, performance:unknown, swarmStats:unknown, healthMetrics:unknown) {
    return {
      system: 'Claude Code Zen',      version:getVersion(),
      swarms:swarmStats,
      tasks:taskStats,
      performance:{
  ...
  private createSwarmDataFromAgent(agent:AgentData, index:number): SwarmStatusData {
    return {
      ...
      type:(agent.type as 'queen' | ' commander' | ' agent') || ' agent',      status:
  ...
  private getMockSwarmData():SwarmStatusData[] {
    return [
      {
        id: 'queen-001',        name: 'Queen Coordinator',        type: 'queen',        status: 'active',        tasks:{ current: 3, completed:456, failed:8},
        performance:{
          efficiency:0.94,
          responseTime:125,
          successRate:0.96,
      ...
      {
        id: 'cmd-001',        name: 'Primary Commander',        type: 'commander',        status: 'busy',        tasks:{ current: 7, completed:234, failed:3},
        performance:{
          efficiency:0.89,
          responseTime:180,
          successRate:0.93,
      ...
      {
        id: 'cmd-002',        name: 'SPARC Commander',        type: 'commander',        status: 'active',        tasks:{ current: 4, completed:189, failed:2},
        performance:{
          efficiency:0.91,
          responseTime:160,
          successRate:0.95,
  ...
  private getFallbackTaskMetrics():TaskMetricsData {
    return {
      ...
      recommendations:[
        'Consider parallel testing to reduce cycle time',        'Add more reviewers to reduce review bottlenecks',        'Implement automated testing to improve throughput',],
};
}
