        ...
        const fileList = await Promise.all(
          items.map(async (item) => {
            ...
            return {
              ...
              type:itemStats.isDirectory() ? 'directory' : ' file',              size:itemStats.size,
  ...
  private async getProjectInfo(req:Request, res:Response): Promise<void> {
    try {
      ...
      res.json({
        ...
        version:packageInfo?.version || '0.0.0',        description:packageInfo?.description || '',
