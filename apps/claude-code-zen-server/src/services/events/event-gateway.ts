...
export class EventGateway {
  ...
  initialize(): void {
    ...
    // Health/status
    bus.on('system:status:request', async (...args: unknown[]) => {
      const payload = args[0] as CorrelatedPayload;
      try {
        const result = await this.data.getSystemStatus();
        bus.emit('system:status:response', { ...result, correlationId: payload?.correlationId });
      } catch (error) {
        logger.warn('status handler failed', { error });
        bus.emit('service:error', { scope: 'status', error: String(error), correlationId: payload?.correlationId });
      }
    });
  ...
  private registerSwarms(bus: ReturnType<typeof getEventBus>): void {
    bus.on('swarms:list:request', async (...args: unknown[]) => {
      const payload = args[0] as CorrelatedPayload;
      try {
        const result = await this.data.getSwarmStatus();
        bus.emit('swarms:list:response', { swarms: result, correlationId: payload?.correlationId });
      } catch (error) {
        logger.warn('swarms handler failed', { error });
        bus.emit('service:error', { scope: 'swarms', error: String(error), correlationId: payload?.correlationId });
      }
    });
  }

  private registerTaskMetrics(bus: ReturnType<typeof getEventBus>): void {
    bus.on('taskmaster:metrics:request', async (...args: unknown[]) => {
      const payload = args[0] as CorrelatedPayload;
      try {
        const result = await this.data.getTaskMetrics();
        bus.emit('taskmaster:metrics:response', { metrics: result, correlationId: payload?.correlationId });
      } catch (error) {
        logger.warn('tasks handler failed', { error });
        bus.emit('service:error', { scope: 'tasks', error: String(error), correlationId: payload?.correlationId });
      }
    });
  }

  private registerTasks(bus: ReturnType<typeof getEventBus>): void {
    bus.on('taskmaster:list:request', async (...args: unknown[]) => {
      const payload = args[0] as CorrelatedPayload;
      try {
        const tasks = await this.data.getTasks?.();
        bus.emit('taskmaster:list:response', { tasks: tasks ?? [], correlationId: payload?.correlationId });
      } catch (error) {
        logger.warn('tasks list handler failed', { error });
        bus.emit('service:error', { scope: 'tasks:list', error: String(error), correlationId: payload?.correlationId });
      }
    });

    bus.on('taskmaster:create:request', async (...args: unknown[]) => {
      ...
      try {
        ...
        bus.emit('taskmaster:create:response', { task, correlationId: payload?.correlationId });
      } catch (error) {
        logger.warn('task create handler failed', { error });
        bus.emit('service:error', { scope: 'tasks:create', error: String(error), correlationId: payload?.correlationId });
      }
    });
  }

  private registerDocuments(bus: ReturnType<typeof getEventBus>): void {
    bus.on('documents:list:request', async (...args: unknown[]) => {
      const payload = args[0] as CorrelatedPayload;
      try {
        const documents = await this.data.getDocuments();
        bus.emit('documents:list:response', { documents, correlationId: payload?.correlationId });
      } catch (error) {
        logger.warn('documents list handler failed', { error });
        bus.emit('service:error', { scope: 'documents:list', error: String(error), correlationId: payload?.correlationId });
      }
    });
  }

  private registerExecute(bus: ReturnType<typeof getEventBus>): void {
    bus.on('execution:request', async (...args: unknown[]) => {
      const payload = args[0] as CorrelatedPayload<{ input: unknown }>;
      try {
        const input = (payload && 'input' in payload ? (payload as { input: unknown }).input : undefined);
        const result = await this.data.executeCommand(input);
        bus.emit('execution:response', { result, correlationId: payload?.correlationId });
      } catch (error) {
        logger.warn('execute handler failed', { error });
        bus.emit('service:error', { scope: 'execute', error: String(error), correlationId: payload?.correlationId });
      }
    });
  }

  private registerSettings(bus: ReturnType<typeof getEventBus>): void {
    bus.on('config:get:request', async (...args: unknown[]) => {
      const payload = args[0] as CorrelatedPayload;
      try {
        const settings = await this.data.getSettings();
        bus.emit('config:get:response', { settings, correlationId: payload?.correlationId });
      } catch (error) {
        logger.warn('settings get handler failed', { error });
        bus.emit('service:error', { scope: 'settings:get', error: String(error), correlationId: payload?.correlationId });
      }
    });

    bus.on('config:update:request', async (...args: unknown[]) => {
      const payload = args[0] as CorrelatedPayload<{ input: unknown }>;
      try {
        const input = (payload && 'input' in payload ? (payload as { input: unknown }).input : undefined);
        const settings = await this.data.updateSettings(input);
        bus.emit('config:update:response', { settings, correlationId: payload?.correlationId });
      } catch (error) {
        logger.warn('settings update handler failed', { error });
        bus.emit('service:error', { scope: 'settings:update', error: String(error), correlationId: payload?.correlationId });
      }
    });
  }

  private registerLogs(bus: ReturnType<typeof getEventBus>): void {
    bus.on('logs:list:request', async (...args: unknown[]) => {
      ...
      try {
        ...
        bus.emit('logs:list:response', { logs, correlationId: payload?.correlationId });
      } catch (error) {
        logger.warn('logs list handler failed', { error });
        bus.emit('service:error', { scope: 'logs:list', error: String(error), correlationId: payload?.correlationId });
      }
    });
  }
  ...
  private registerBrain(bus: ReturnType<typeof getEventBus>): void {
    // Brain task analysis
    bus.on('brain:analyze:request', async (...args: unknown[]) => {
      ...
      try {
        ...
        bus.emit('brain:analyze:response', { analysis, correlationId: payload?.correlationId });
      } catch (error) {
        logger.warn('brain analyze handler failed', { error });
        bus.emit('service:error', { scope: 'brain:analyze', error: String(error), correlationId: payload?.correlationId });
      }
    });

    // Brain prompt optimization
    bus.on('brain:optimize:request', async (...args: unknown[]) => {
      ...
      try {
        ...
        let input: { task: string; basePrompt: string; context?: Record<string, unknown>; priority?: string };
        if (payload && 'task' in payload) {
          input = { task: payload.task, basePrompt: payload.basePrompt };
          if (payload.context !== undefined) input.context = payload.context;
          if (payload.priority !== undefined) input.priority = payload.priority;
        } else {
          input = { task: '', basePrompt: '' };
        }
        const optimization = await this.data.optimizeBrainPrompt(input);
        bus.emit('brain:optimize:response', { optimization, correlationId: payload?.correlationId });
      } catch (error) {
        logger.warn('brain optimize handler failed', { error });
        bus.emit('service:error', { scope: 'brain:optimize', error: String(error), correlationId: payload?.correlationId });
      }
    });

    // Brain complexity estimation
    bus.on('brain:complexity:request', async (...args: unknown[]) => {
      ...
      try {
        ...
        let input: { taskId: string; task: string; context?: Record<string, unknown> };
        if (payload && 'taskId' in payload) {
          input = { taskId: payload.taskId, task: payload.task };
          if (payload.context !== undefined) input.context = payload.context;
        } else {
          input = { taskId: '', task: '' };
        }
        const complexity = await this.data.estimateBrainComplexity(input);
        bus.emit('brain:complexity:response', { complexity, correlationId: payload?.correlationId });
      } catch (error) {
        logger.warn('brain complexity handler failed', { error });
        bus.emit('service:error', { scope: 'brain:complexity', error: String(error), correlationId: payload?.correlationId });
      }
    });

    // Brain status
    bus.on('brain:status:request', async (...args: unknown[]) => {
      const payload = args[0] as CorrelatedPayload;
      try {
        ...
        bus.emit('brain:status:response', { status, correlationId: payload?.correlationId });
      } catch (error) {
        logger.warn('brain status handler failed', { error });
        bus.emit('service:error', { scope: 'brain:status', error: String(error), correlationId: payload?.correlationId });
      }
    });
  }
}
