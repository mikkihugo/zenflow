...
export interface EventRegistryWebSocketMessage {
  type:
    | typeof MODULE_STATUS
    | 'module-registry'
    | typeof MODULE_LIST
    | ' module-update'
    | typeof EVENT_FLOWS
    | typeof EVENT_METRICS
    | ' event-flow'
    | ' heartbeat';
  data: unknown;
  timestamp: string;
...
export class EventRegistryWebSocketService {
  ...
  private handleConnection(ws: WebSocket): void {
    ...
    this.sendToClient(
      client,
      MODULE_LIST,
      eventRegistryInitializer.getActiveModules()
    );
    this.sendToClient(
      client,
      EVENT_METRICS,
      eventRegistryInitializer.getEventMetrics()
    );
    this.sendToClient(
      client,
      EVENT_FLOWS,
      eventRegistryInitializer.getEventFlows().slice(0, 10)
    );
  ...
  private handleClientMessage(
    ...
    message: { type: string; eventTypes?: string[] }
  ): void {
    switch (message.type) {
      case 'subscribe':
  ...
  private sendToClient(
    client: WebSocketClient,
    type: EventRegistryWebSocketMessage['type'],
    data: unknown
  ): void {
...
// Singleton instance
export const eventRegistryWebSocketService =
  new EventRegistryWebSocketService();
