 ...
 * - Clean Product Flow naming throughout (no generic "document" "erminology)
...
export interface BaseProductEntity {
  ...
  // Processed content for full-text search
 ...
 * @example
 */
export interface VisionProductEntity extends BaseProductEntity {
  type: 'vision';
 ...
 * @example
 */
export interface ADRProductEntity extends BaseProductEntity {
  type: 'adr';
 ...
 * @example
 */
export interface PRDProductEntity extends BaseProductEntity {
  type: 'prd';
 ...
 * @example
 */
export interface EpicProductEntity extends BaseProductEntity {
  type: 'epic';
 ...
 * @example
 */
export interface FeatureProductEntity extends BaseProductEntity {
  type: 'feature';
 ...
 * @example
 */
export interface TaskProductEntity extends BaseProductEntity {
  type: 'task';
...
export interface ProductWorkflowStateEntity {
  ...
  // Feature IDs using SPARC
    sparc_phases_active: Record<string,
  string>;
  // featureId -> current SPARC phase
    sparc_completion_percentage: number;
  // Overall SPARC completion %
...
export interface ProductSearchIndexEntity {
  ...
  // Associated SPARC project IDs for search
  sparc_phase: string;
  // Current SPARC phase if document uses SPARC
  sparc_progress: number;
  // SPARC completion percentage (0-100)
  uses_sparc_methodology: boolean;
  // Whether document/project uses SPARC
  sparc_domain: string;
  // SPARC domain classification
  sparc_complexity: string;
  // SPARC complexity level
...
export const PRODUCT_DATABASE_SCHEMAS = {
  product_documents: 'CREATE'TABLE IF NOT EXISTS product_documents(
  id TEXT PRIMARY KEY,
  type TEXT NOT NULL,
  title TEXT NOT NULL,
  ...
  -- JSON for vision-specific fields
    adr_data TEXT,
  -- JSON for ADR-specific fields
    prd_data TEXT,
  -- JSON for PRD-specific fields
    epic_data TEXT,
  -- JSON for epic-specific fields
    feature_data TEXT,
  -- JSON for feature-specific fields (includes SPARC integration
)
    task_data TEXT,
  -- JSON for task-specific fields (includes SPARC integration)
  ...
  product_relationships: 'CREATE'TABLE IF NOT EXISTS product_relationships(
  id TEXT PRIMARY KEY,
  ...
  product_workflow_states: 'CREATE'TABLE IF NOT EXISTS product_workflow_states(
  id TEXT PRIMARY KEY,
  ...
  -- JSON array
    ...
    sparc_workflow_integration TEXT,
  -- JSON with SPARC workflow state
  ...
  product_projects: 'CREATE'TABLE IF NOT EXISTS product_projects(
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
  description TEXT,
  ...
  -- COMPREHENSIVE SPARC INTEGRATION
    sparc_inte'ration_enabled BOOLEAN DEFAULT FALSE,
  sparc_project_mappings TEXT,
  -- JSON array of feature->SPARC mappings
    sparc_project_ids TEXT,
  -- JSON array of SPARC project IDs
    sparc_workflow_config TEXT,
  -- JSON with SPARC workflow settings
    sparc_integration_health TEXT,
  -- JSON with sync status and metrics
    sparc_benefits_tracking TEXT,
  -- JSON with SPARC benefits metrics
    tags TEXT,
  -- JSON array
    stakeholders TEXT,
  -- JSON array
  ...
  product_search_index: 'CREATE'TABLE IF NOT EXISTS product_search_index(
  ...
  product_sparc_integration: 'CREATE'TABLE IF NOT EXISTS product_sparc_integration(
  id TEXT PRIMARY KEY,
  ...
  -- Foreign key constraints
    FOREIGN KEY (product_project_id
) REFERENCES product_projects(id) ON DELETE CASCADE,
 ...
 * @example
 */
export function isVisionDocument(doc: BaseProductEntity): doc is VisionProductEntity  {
  return doc.type === `vision'
}

export function isADRDocument(doc: BaseProductEntity): doc is ADRProductEntity  {
  return doc.type === 'adr'
}

export function isPRDDocument(doc: BaseProductEntity): doc is PRDProductEntity  {
  return doc.type === 'prd'
}

export function isEpicDocument(doc: BaseProductEntity): doc is EpicProductEntity  {
  return doc.type === 'epic'
}

export function isFeatureDocument(doc: BaseProductEntity): doc is FeatureProductEntity  {
  return doc.type === 'feature'
}

export function isTaskDocument(doc: BaseProductEntity): doc is TaskProductEntity  {
  return doc.type === 'task;
}

// Alias BaseDocumentEntity to BaseProductEntity for compatibility
export type { BaseProductEntity as BaseDocumentEntity };