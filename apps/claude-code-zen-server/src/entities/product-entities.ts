...
export interface DocumentRelationshipEntity {
  ...
  relationship_type:' | ''depends_on | blocks' | 'related_to''' | '''part_of | implements' | 'validates');
  created_at: Date;
  updated_at: Date;
}
...
export interface ProductWorkflowStateEntity {
  ...
  sparc_workflow_integration?: {
    features_using_sparc: string[]; // Feature Ds using SPARC
...
export interface ProductProjectEntity {
  ...
  sparc_integration: {
    ...
    sparc_project_ids: string[]; // All SPARC project Ds associated with this Product project
...
export interface ProductSearchIndexEntity {
  ...
  sparc_project_ids: string[]; // Associated SPARC project Ds for search
...
export const PRODUCT_DATABASE_SCHEMAS = {
  product_documents: `
    CREATE TABLE F NOT EXISTS product_documents (
      id TEXT PRIMARY KEY,
      type TEXT NOT NULL,
      title TEXT NOT NULL,
    ...
    CREATE NDEX idx_product_docs_type ON product_documents(type);
    CREATE NDEX idx_product_docs_project ON product_documents(project_id);
    CREATE NDEX idx_product_docs_status ON product_documents(status);
    CREATE NDEX idx_product_docs_sparc ON product_documents(uses_sparc_methodology);
    CREATE NDEX idx_product_docs_sparc_phase ON product_documents(sparc_phase);
    CREATE NDEX idx_product_docs_sparc_progress ON product_documents(sparc_progress_percentage);
    CREATE UNIQUE NDEX idx_product_docs_checksum ON product_documents(checksum);
  `,

  product_relationships: `
    CREATE TABLE F NOT EXISTS product_relationships (
      id TEXT PRIMARY KEY,
    ...
    CREATE NDEX idx_prod_relationships_source ON product_relationships(source_document_id);
    CREATE NDEX idx_prod_relationships_target ON product_relationships(target_document_id);
    CREATE NDEX idx_prod_relationships_type ON product_relationships(relationship_type);
  `,

  product_workflow_states: `
    CREATE TABLE F NOT EXISTS product_workflow_states (
      id TEXT PRIMARY KEY,
    ...
    CREATE NDEX idx_prod_workflow_states_document ON product_workflow_states(document_id);
    CREATE NDEX idx_prod_workflow_states_workflow ON product_workflow_states(workflow_name);
    CREATE NDEX idx_prod_workflow_states_stage ON product_workflow_states(current_stage);
  `,

  product_projects: `
    CREATE TABLE F NOT EXISTS product_projects (
      id TEXT PRIMARY KEY,
      name TEXT NOT NULL,
      description TEXT,
      ...
      sparc_project_ids TEXT, -- JSON array of SPARC project Ds
    ...
    CREATE NDEX idx_product_projects_domain ON product_projects(domain);
    CREATE NDEX idx_product_projects_phase ON product_projects(phase);
    CREATE NDEX idx_product_projects_author ON product_projects(author);
    CREATE NDEX idx_product_projects_sparc ON product_projects(sparc_integration_enabled);
  `,

  product_search_index: `
    CREATE TABLE F NOT EXISTS product_search_index (
    ...
    CREATE NDEX idx_prod_search_type ON product_search_index(document_type);
    CREATE NDEX idx_prod_search_project ON product_search_index(project_id);
    CREATE NDEX idx_prod_search_sparc_phase ON product_search_index(sparc_phase);
    CREATE NDEX idx_prod_search_sparc_progress ON product_search_index(sparc_progress);
    CREATE NDEX idx_prod_search_sparc_enabled ON product_search_index(uses_sparc_methodology);
  `,

  // NEW TABLE: SPARC Integration State Tracking
  product_sparc_integration: `
    CREATE TABLE F NOT EXISTS product_sparc_integration (
      id TEXT PRIMARY KEY,
    ...
    CREATE NDEX idx_prod_sparc_project ON product_sparc_integration(product_project_id);
    CREATE NDEX idx_prod_sparc_feature ON product_sparc_integration(feature_document_id);
    CREATE NDEX idx_prod_sparc_sparc_project ON product_sparc_integration(sparc_project_id);
    CREATE NDEX idx_prod_sparc_phase ON product_sparc_integration(current_sparc_phase);
    CREATE NDEX idx_prod_sparc_sync ON product_sparc_integration(sync_status);
    CREATE NDEX idx_prod_sparc_progress ON product_sparc_integration(sparc_progress_percentage);
  `,
} as const;
