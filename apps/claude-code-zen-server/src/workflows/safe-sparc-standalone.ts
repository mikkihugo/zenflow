    ...
    try {
      ...
      return {
        ...
        metadata: {
          ...
          calculatedROI:
            (epic.estimatedValue - epic.estimatedCost) / epic.estimatedCost,
          llmDecision: true,
        },
      };
    } catch (error) {
...
class SPARCEngineStandalone {
  ...
  private async executeCompletionPhase(
    ...
    try {
      ...
      const codeExplanationPrompt = this.buildCodeGenerationPrompt(
        project,
        epic
      );
      ...
      this.logger.info(
        `   Simulated files: ${simulatedFiles.files.length} files`
      );
      this.logger.info(
...
`;
  ...
  private buildRefinementPrompt(
    ...
    const performanceTargets = {
      userLoad:
        epic.estimatedValue > 1000000
          ? 'high'
          : epic.estimatedValue > 500000
            ? 'medium'
            : 'standard',
...
`;
  ...
  private parseSpecificationResponse(response: string): any {
    try {
      ...
      return {
        goals: [
          `Implement functionality based on: ${response.substring(0, 100)}...`,
        ],
...
export class SafeSparcWorkflow extends TypedEventBase {
  ...
  async processSafeEpic(epic: EpicProposal): Promise<SafeWorkflowResult> {
    ...
    this.logger.info(
      `Expected ROI: ${(((epic.estimatedValue - epic.estimatedCost) / epic.estimatedCost) * 100).toFixed(1)}%`
    );

    const workflowStartTime = Date.now();
    this.logger.info(
      `🚀 Starting SAFe-SPARC workflow at ${new Date(workflowStartTime)?.toISOString}`
    );
  ...
  private async executeSafeRoles(
    ...
    for (const role of roles) {
      ...
      try {
        ...
        this.logger.info(
          `${role}: ${decision.decision} (${(decision.confidence * 100).toFixed(1)}%)`
        );
  ...
  private determineOverallDecision(
    ...
    const deferredDecisions = decisions.filter(
      (d) => d.decision === 'defer'
    ).length;
    ...
    const highConfidenceDecisions = decisions.filter((d) => d.confidence > .8);
    ...
    this.logger.info(
      `Average confidence: ${(avgConfidence * 100).toFixed(1)}%, High confidence decisions: ${highConfidenceDecisions.length}/${totalDecisions}`
    );
...
export async function testSafeSparcWorkflow(): Promise<void> {
  ...
  try {
    ...
    result.roleDecisions.forEach((decision, index) => {
      console.log(
        `  ${index + 1}. ${decision.roleType}: ${decision.decision?.toUpperCase}`
      );
      console.log(
    ...
    if (result.sparcArtifacts && result.overallDecision === 'approve') {
      ...
      if (result.sparcArtifacts.implementation) {
        ...
        console.log(
          `Generated: ${impl.files.length} files, ${impl.tests.length} tests, ${impl.documentation.length} docs`
        );
