...
export class WebSocketHubManager {
  ...
  private handleMessage(event:MessageEvent): void {
    try {
      const message:HubMessage | ServiceDiscoveryResponse = JSON.parse(event.data);
  ...
  private handleServiceDiscovery(message:ServiceDiscoveryResponse): void {
    ...
    this.subscribe(['taskmaster', 'coordination'], [
      'task_updated',
  ...
  private handleTaskUpdate(message:HubMessage): void {
    ...
    logger.debug("Task update received", { taskCount:message.data.tasks?.length});
}
  ...
  private handleApprovalGateUpdate(message:HubMessage): void {
    ...
    logger.debug("Approval gates updated", { gateCount:message.data.gates?.length});
}
  ...
  private handlePIPlanningUpdate(message:HubMessage): void {
    ...
    logger.debug("PI Planning progress updated", { progress:message.data.percentage});
}
  ...
  private handleAgentCoordination(message:HubMessage): void {
    ...
    logger.debug("Agent coordination updated", { agentCount:message.data.agents?.length});
}
  ...
  private handleGenericEvent(message:HubMessage): void {
    ...
    logger.debug("Generic event received", { type:message.type, source:message.source});
}
  ...
  subscribe(services:string[], messageTypes:string[]): void {
    ...
    this.connectionState.update(state => ({
      ...state,
      subscriptions:Array.from(new Set([...state.subscriptions, ...services, ...messageTypes]))
}));
