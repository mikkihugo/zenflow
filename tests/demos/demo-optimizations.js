...
async function demonstrateMCPOptimizations() {
  ...
  console.warn(`   - Batch size: ${server.stdioOptimizer.batchSize} messages`);
  console.warn(`   - Batch timeout: ${server.stdioOptimizer.batchTimeout}ms`);
  console.warn(`   - Retry attempts: ${server.errorHandler.maxRetries}`);
  console.warn(`   - Circuit breaker threshold: ${server.errorHandler.circuitBreakerThreshold}`);
  ...
  server.stdioOptimizer.on('batch', async (batch) => {
    console.warn(`   Processing batch of ${batch.length} messages...`);
  ...
  console.warn(`   Average latency: ${metrics.requests.avgLatency.toFixed(2)}ms`);
  console.warn(`   Batches processed: ${metrics.batches.total}`);

  // Show stdio optimizer metrics
  console.warn('\n📈 Stdio Optimization Metrics:');
  const stdioMetrics = server.stdioOptimizer.getMetrics();
  console.warn(`   Messages processed: ${stdioMetrics.messagesProcessed}`);
  console.warn(`   Batches processed: ${stdioMetrics.batchesProcessed}`);
  console.warn(`   Buffer overflows: ${stdioMetrics.bufferOverflows}`);
  console.warn(`   Retry attempts: ${stdioMetrics.retryAttempts}`);
  console.warn(`   Connection status: ${stdioMetrics.isConnected ? 'Connected' : 'Disconnected'}`);

  // Show error handling status
  console.warn('\n🛡️ Error Handling Status:');
  const errorStats = server.errorHandler.getErrorStats();
  console.warn(`   Total errors: ${errorStats.totalErrors}`);
  console.warn(`   Circuit breaker state: ${errorStats.circuitState}`);
  console.warn(`   Success count: ${errorStats.successCount}`);

  // Show server status with new optimizations
  console.warn('\n🔍 Server Status:');
  const status = server.getStatus();
  console.warn(`   Version: ${status.version}`);
  console.warn(
    `   Optimizations: ${Object.keys(status.optimization)
      .filter((k) => status.optimization[k])
      .join(', ')}`
  );
  console.warn(
    `   Performance: ${status.performance.throughput.toFixed(1)} msg/s, ${status.performance.avgLatency.toFixed(1)}ms avg latency`
  );
  console.warn(
    `   Stdio: ${status.stdio.queueLength} queued, ${status.stdio.bufferSize} bytes buffered`
  );
  ...
  resources.forEach((resource) => {
    console.warn(`   - ${resource.uri}: ${resource.description}`);
  });
  ...
  try {
    ...
    console.warn(`   Success rate: ${(perfSummary.overview.successRate * 100).toFixed(1)}%`);
    console.warn(`   Throughput: ${perfSummary.overview.throughput.toFixed(1)} messages/second`);
    console.warn(`   Connection healthy: ${perfSummary.health.connectionHealthy}`);
    console.warn(`   Memory usage: ${(perfSummary.health.memoryUsage / 1024 / 1024).toFixed(1)}MB`);
  } catch (error) {
    console.warn(`   Error reading resource: ${error.message}`);
  }
