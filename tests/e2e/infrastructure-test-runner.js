...
export class MockServiceManager {
    ...
    async startMockServices() {
        ...
        const serviceConfigs = [
            ...
            { name: 'development', port: this.basePort + 3, endpoints: this.createDevelopmentEndpoints() }
        ];
    ...
    handleRequest(req, res, config) {
        ...
        const endpoint = config.endpoints.find(e => 
            e.path === url.pathname && e.method === req.method
        );
...
export class InfrastructureTestSuite {
    ...
    async testServiceCommunication() {
        ...
        try {
            ...
            if (successfulAuths >= 3) { // At least 3 out of 4 services
                this.testResults.services = { 
                    passed: true, 
                    successfulServices: successfulAuths,
                    totalServices: authResults.length
                };
                this.logger.info(`✅ Service communication test passed (${successfulAuths}/${authResults.length} services)`);
            } else {
