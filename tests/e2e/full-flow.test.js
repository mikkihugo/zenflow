...
describe('Complete User Journey', () =>
{
  ...
  // await page.waitForSelector('[data-testid="code-preview"]', { timeout: 30000 });
    ...
    const _totalTime = await page.evaluate(() => performance.now());
...
describe('Responsive Design', () =>
{
  ...
  // await page.goto('http://localhost:3000');
      ...
      if (viewport.width < 768) {
        const _mobileMenu = await page.locator('[data-testid="mobile-menu-button"]');
...
describe('Accessibility', () =>
{
  ...
  // await page.goto('http://localhost:3000');
    ...
    const _uploadArea = await page.locator('[data-testid="upload-area"]');
    const _ariaLabel = await uploadArea.getAttribute('aria-label');
    expect(ariaLabel).toBeTruthy();
    // Check form inputs have labels
    const _inputs = await page.$$('input, select');
    for (const input of inputs) {
      const _id = await input.getAttribute('id');
      const _label = await page.$(`label[for="${id}"]`);
  ...
  // await page.addScriptTag({
    ...
    const _violations = await page.evaluate(async () => {
      const _results = await window.axe.run();
