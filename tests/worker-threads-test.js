...
async function testWorkerThreads(): unknown {
...
;
    const _result1 = await pool.executeTask({
      type: 'agent-spawn',;
        agentType: 'coder',;name: 'Test Agent' ,;,;
    }
)
...
console.warn('\n🔧 Test 2: Parallel task execution');
const _parallelStartTime = performance.now();
const _parallelTasks = [;
      pool.executeTask({
...
{
  type: 'code-analysis',;
  codebase: 'test-codebase', analysisOptions;
  :
,
}
),
...
{
  type: 'research-task',;
  topic: 'test-topic', depth;
  : 'basic'
,
}
),
]
const _parallelResults = await Promise.all(parallelTasks);
const _parallelEndTime = performance.now();
console.warn(;
...
📊 Metrics: $
{
  status.metrics.tasksCompleted;
}
completed, $;
{
  status.metrics.averageTaskTime.toFixed(0);
}
ms;
avg;
time`;
...
const _loadTestTasks = Array.from({ length: 6 }, (_, i) =>;
pool.executeTask({
        type: 'testing-task',;
{
  testType: 'unit', target;
  : `;
test -
  $i`
}
...
const _loadTestResults = await Promise.all(loadTestTasks);
console.warn(;
`;
✅ Load balancing test: $loadTestResults.filter((r) => r.success).length/$
{
  loadTestResults.length;
}
successful`;
)
const _finalStatus = pool.getStatus();
...
tasks, $;
{
  stats.averageTaskTime.toFixed(0);
}
ms;
avg`;
  )
