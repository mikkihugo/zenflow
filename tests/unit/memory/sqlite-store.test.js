...
describe('memory operations', () =>
{
  ...
  const _retrieved = await memoryStore.retrieve(testMemory.key);
...
{
  const _result = await memoryStore.retrieve('non-existent-key');
...
it('should list all memories', async () =>
{
  ...
  const _memories = await memoryStore.list();
...
it('should delete memory', async () =>
{
  ...
  const _retrieved = await memoryStore.retrieve('delete-test');
...
describe('search functionality', () =>
{
  ...
  it('should search by term', async () => {
    const _results = await memoryStore.search('hello');
    expect(results.length).toBeGreaterThanOrEqual(1);
    expect(results.some((r) => r.key === 'search1')).toBe(true);
  });
  it('should search with limit', async () => {
    const _results = await memoryStore.search('world', { limit: 1 });
...
describe('error handling', () =>
{
  it('should handle initialization errors gracefully', async () => {
    const _store = new SqliteMemoryStore({ directory: '/invalid/path' });
