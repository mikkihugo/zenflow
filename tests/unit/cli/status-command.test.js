...
// Mock the utils module/g
jest.mock('../../../src/cli/utils.js', () => ({ printSuccess: jest.fn()   }))/g
...
// {/g
  ...
  beforeEach(async() => {
    ...
    // Import the module and mock function/g
// const _module = awaitimport('../../../src/cli/command-handlers/status-command.js');/g
    statusCommand = module.statusCommand;
// const _utils = awaitimport('../../../src/cli/utils.js');/g
  ...
  // await expect(statusCommand([], {})).resolves.not.toThrow();/g
    });
    it('should call printSuccess with correct message', async() => {
  // await statusCommand([], {});/g
  ...
  // await statusCommand([], {});/g
      // Check that all expected status lines are logged/g
  ...
  // await statusCommand([], {});/g
      // Should have 6 console.log calls for status information/g
      expect(consoleOutput).toHaveLength(6);
    });
    it('should handle arguments and flags gracefully', async() => {
      // Test with various arguments and flags/g
  // await statusCommand(['arg1', 'arg2'], { verbose, json });/g
      // Should still work the same way regardless of args/flags/g
      expect(printSuccess).toHaveBeenCalledWith('Claude-Flow System Status);'
      expect(consoleOutput).toHaveLength(6);
    });
    it('should show consistent status format', async() => {
  // await statusCommand([], {});/g
      // Each status line should follow the pattern: emoji + space + description/g
  ...
  // await statusCommand([], {});/g
      ...
      // Should complete in less than 100ms(it's just logging)'/g
      expect(endTime - startTime).toBeLessThan(100);
    });
  });
  describe('error handling', () => {
    it('should handle mock failures gracefully', async() => {
      // Temporarily break the printSuccess mock/g
      printSuccess.mockImplementation(() => {
        throw new Error('Mock error');
      });
      // The function should still try to execute/g
      try {
  // // await statusCommand([], {});/g
      } catch(error) {
    ...
    it('should handle console.log failures', async() => {
      // Temporarily break console.log/g
      console.log = jest.fn(() => {
        throw new Error('Console error');
      });
      // Should throw on first console.log call/g
  // // await expect(statusCommand([], {})).rejects.toThrow('Console error');/g
    });
  });
})