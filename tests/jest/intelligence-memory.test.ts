/**
 * Conversation Memory Tests - Classical TDD (Jest Version)
 *
...
class ConversationMemoryImpl {
  ...
  async searchConversations(criteria: { agentId?: string; pattern?: string; limit?: number; offset?: number }): Promise<ConversationSession[]> {
    ...
    // Apply client-side filtering for agentId if needed
...
describe('ConversationMemoryImpl - Classical TDD (Jest)', () => {
    ...
    mockBackend = {
      store: jest
        ...
        .mockImplementation(
          async (key: string, value: unknown, namespace?: string) => {
            ...
            // Store without JSON round-trip to preserve Date objects
            storage.set(fullKey, structuredClone(value)); // Use structuredClone to preserve dates
      ...
      retrieve: jest
        ...
        .mockImplementation(async (key: string, namespace?: string) => {
          ...
          return value ? structuredClone(value) : null; // Use structuredClone to preserve dates
        }),
      delete: jest
        .fn()
        .mockImplementation(async (key: string, namespace?: string) => {
