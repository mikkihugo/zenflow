...
async function waitForServiceOrStartMock(serviceName, serviceUrl) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      const _response = await axios.get(`${serviceUrl}/api/health`, {
        timeout: 5000,
...
async function createServiceStub(serviceName, _serviceUrl) {
  ...
  console.warn(`📝 Created service stub for ${serviceName}`);
}
