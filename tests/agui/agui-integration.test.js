...
describe('AG-UI Integration Tests', () => {
  ...
  describe('AGUIAdapter', () => {
    ...
    test('should emit tool call events correctly', (done) => {
      let eventsReceived = 0;
      const expectedEvents = ['TOOL_CALL_START', 'TOOL_CALL_ARGS', 'TOOL_CALL_END', 'TOOL_CALL_RESULT'];
...
if (process.argv[1].endsWith('/agui-integration.test.js')) {
  console.log('🧪 Running AG-UI Integration Tests...');
  ...
  async function runBasicTests() {
    ...
    console.log(`✓ Text message flow (${eventCount} events)`);
    ...
    console.log(`✓ Tool call flow (${eventCount - initialCount} events)`);
    ...
    console.log(`✓ Custom events (${eventCount - beforeCustom} events)`);
    
    console.log(`\n✅ Basic tests completed! Total events: ${eventCount}`);
    console.log('📊 Final stats:', adapter.getStats());
  }
