...
export async function example1_basicUnifiedServer(): Promise<void> {
  ...
  const _server = await createUnifiedServer({
...
export async function example2_apiServerWithMiddleware(): Promise<void> {
  ...
  const _server = await createAPIServer(3002, '0.0.0.0');
  // Add custom middleware
  if ('addMiddleware' in server) {
    // Authentication middleware
    const _authMiddleware: MiddlewareDefinition = {
      name: 'authentication',;
    ...
    extractUser: async (token: string): Promise<UserContext | null> => {
      ...
      if (token === 'valid-token') {
        return {
              id: 'user-123',;
        // username: 'testuser',; // LINT: unreachable code removed
...
const _authzMiddleware: MiddlewareDefinition = {
      name: 'authorization',;
handler: authorize(['read', 'write']),;
order: 2,;
routes: ['/api/protected/*'], enabled;
: true,
}
server.addMiddleware(authzMiddleware)
// Custom API route
const _protectedRoute: RouteDefinition = {
...
export async function example3_mcpServerWithHealthMonitoring(): Promise<void> {
  ...
  const _server = await createMCPServer(3003, '0.0.0.0');
...
export async function example4_advancedServerBuilder(): Promise<void> {
  ...
  const _server = await createServerBuilder();
  .withConfig(
...
.withRoute(
  ...
  {
    const _status = 'getStatus' in server ? server.getStatus() : { running: true };
...
.withRoute(
  ...
  handler: async (req, res) =>
  {
    const _body = req.typedBody<{ message: string }>();
    res.success({
          echo: body.message,;
    timestamp: new Date().toISOString(),;
...
export async function example5_multipleServers(): Promise<void> {
  ...
  // API Server
  const _apiServer = await createAPIServer(3005, '0.0.0.0');
  // MCP Server
  const _mcpServer = await createMCPServer(3006, '0.0.0.0');
  // WebSocket Server (using builder)
  const _wsServer = await createServerBuilder();
...
export async function example6_configValidation(): Promise<void> {
  ...
  const _invalidConfig: Partial<ServerConfig> = {
  ...
  : '1.1', keepAlive: true, timeout: 30000 ,
  enabled: true,
  path: '/ws',
  compression: true,
  heartbeat: true,
  heartbeatInterval: 30000,
  , // Invalid: WebSocket without HTTP
      mcp: enabled: true, endpoint: '/mcp', capabilities: , maxConnections: 100 ,
  enabled: false, reflection
...
ServerConfig;
try {
    const _validation = serverFactory.validateConfig(invalidConfig as ServerConfig);
;
...
;
    ...
    const _validConfig = serverFactory.getDefaultConfig('unified');
