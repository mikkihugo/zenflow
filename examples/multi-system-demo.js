...
class MultiSystemDemo {
  ...
  async runLanceDBDemo() {
    ...
    try {
      ...
      console.warn(`     - Found ${searchResults.results.length} similar documents`);
      ...
      console.warn(`     - Inserted ${codeInsertCount} code snippets`);
      ...
      console.warn(`     - Analyzed ${analytics.total_vectors} vectors`);
  ...
  async runKuzuDemo() {
    ...
    try {
      ...
      console.warn(`     - Inserted ${serviceCount} services`);
      ...
      console.warn(`     - Inserted ${relCount} relationships`);
      ...
      console.warn(`     - Computed centrality for ${centrality.scores.length} nodes`);
      console.warn(
      ...
      console.warn(`     - Detected ${communities.communities.length} communities`);
      console.warn(`     - Modularity score: ${communities.modularity.toFixed(3)}`);
      ...
      console.warn(`     - Traversal found ${traversal.data?.length || 0} nodes`);
      console.warn(`     - Execution time: ${traversal.execution_time}ms`);
  ...
  async runVisionDemo() {
    ...
    try {
      ...
      if (result.success) {
        console.warn(`     - Generated ${result.files.size} files`);
        console.warn(`     - Quality score: ${result.qualityScore}/100`);
        console.warn(`     - Processing time: ${result.processingTime}ms`);
        console.warn(`     - Framework: ${result.framework}`);
      } else {
      ...
      for (const framework of frameworks) {
        try {
          const frameworkResult = await this.coordinator.vision.processImage(
          ...
          console.warn(`     - ${framework}: ${frameworkResult.success ? 'Success' : 'Failed'}`);
        } catch (error) {
          console.warn(`     - ${framework}: Failed (${error.message})`);
        }
      }
  ...
  async runIntegrationDemo() {
    ...
    try {
      ...
      for (const test of routingTests) {
        try {
          const result = await this.coordinator.intelligentRoute(test.operation, test.input, {
            limit: 5,
          });
  ...
  async generateReport() {
    ...
    // Display summary
    console.warn('\n📊 Demo Summary:');
    console.warn(`   Duration: ${report.performance_summary.total_duration}ms`);
