...
import { PatternTypeSchema } from './schemas';

// Import ActionPattern type from schemas
import type { ActionPattern } from './schemas';
...
export class LearningCoordinator extends EventEmitter implements ILearningCoordinator {
  ...
  constructor(
    ...
    this._logger = getLogger('adaptive-learning-coordinator');
  ...
  private findCommonPatterns(successes: SuccessPattern[]): ActionPattern[] {
  ...
  private selectPatternType(agent: Agent): PatternType {
    const types: PatternType[] = ['task_completion', 'communication', 'resource_utilization', 'coordination', 'failure'];
    
    // If agent has specializations, try to map them to pattern types
    if (agent.specializations.length > 0) {
      const specialization = agent.specializations[0];
      const result = PatternTypeSchema.safeParse(specialization);
      if (result.success) {
        return result.data;
      }
    }
    
    // Return a random valid pattern type
  ...
  private calculateAverageOutcomes(successes: SuccessPattern[]): { efficiency: number; quality: number; cost: number; satisfaction: number; sustainability: number } {
    const totals = {
      efficiency: 0,
      quality: 0,
      cost: 0,
