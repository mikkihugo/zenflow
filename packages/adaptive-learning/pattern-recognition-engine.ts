...
// Using CommunicationPattern from types.ts instead of local interface
  ...
  implements IPatternRecognitionEngine
{
  ...
  private communicationPatterns: Map<string, ICommunicationPattern> = new Map();
  private failurePatterns: Map<string, IFailurePattern> = new Map();
  ...
  classifyTaskCompletion(task: TaskResult): TaskCompletionPattern {
    ...
    const successfulTasks = similarTasks.filter((t) => {
      try {
        // Use SuccessData schema for safe property access
        const successData = SuccessDataSchema.parse({
          success: true,
          target: t.agentId,
        ...
        const result = t.result as { success?: boolean } | undefined;
        return result?.success !== false;
      } catch {
        // Fallback for non-conforming results
        const result = t.result as { success?: boolean } | undefined;
        return result?.success !== false;
      }
    });
    ...
    const failedTasks = similarTasks.filter((t) => {
      try {
        // Use SuccessData schema for safe property access
        const successData = SuccessDataSchema.parse({
          success: false,
    ...
    for (const [pairKey, pairMessages] of pairGroups) {
      if (pairMessages.length >= this.minPatternFrequency) {
        ...
        const pattern: ICommunicationPattern = {
          source,
          target,
          messageType: this.getMostFrequentMessageTypeFromTraces(source, target, []), // Simplified for now
  ...
  private analyzeCommunicationPatterns(): void {
    ...
    for (const trace of communicationTraces) {
      const params = trace.parameters as { target?: string; messageType?: string } | undefined;
      if (params?.target) {
        const key = `${trace.agentId}->${params.target}`;
        pairwiseCommunication.set(
          key,
          (pairwiseCommunication.get(key) || 0) + 1
        );

        const msgType = params.messageType || 'unknown';
    ...
    for (const [pair, frequency] of pairwiseCommunication) {
      if (frequency >= this.minPatternFrequency) {
        ...
        const source = parts[0] || 'unknown';
        const target = parts[1] || 'unknown';
        const commPattern: ICommunicationPattern = {
          source,
          target,
          ...
          frequency,
          averageLatency: this.calculateAverageLatencyFromTraces(
            source,
            target,
            communicationTraces
          ),
          averageSize: this.calculateAveragePayloadSize(
            source,
            target,
            communicationTraces
          ),
          ...
          efficiency: this.calculateCommunicationEfficiencyFromTraces(
            source,
            target,
            communicationTraces
          ),
  ...
  private analyzeFailurePatterns(): void {
    const failureTraces = this.traces.filter((t) => {
      const result = t.result as { error?: unknown; success?: boolean } | undefined;
    ...
    for (const trace of failureTraces) {
      const result = trace.result as { error?: unknown } | undefined;
    ...
    for (const [errorType, traces] of failureTypes) {
      if (traces.length >= this.minPatternFrequency) {
        const failurePattern: FailurePattern = {
          ...
          severity: traces.length > 10 ? 'high' : traces.length > 5 ? 'medium' : 'low',
          ...
          prevention: this.generatePreventionStrategies(traces),
        };
  ...
  private analyzeCoordinationPatterns(): void {
    ...
    for (const trace of coordinationTraces) {
      const params = trace.parameters as { topology?: string } | undefined;
      const topology = params?.topology || 'unknown';
    ...
    for (const [topology, traces] of topologies) {
      ...
      const coordPattern = pattern as { effectiveness?: number };
      if ((coordPattern.effectiveness || 0) > 0.6) {
        this.patterns.set(`coordination_${topology}`, {
          id: `coordination_${topology}`,
          type: 'coordination',
          pattern,
          frequency: traces.length,
          confidence: coordPattern.effectiveness || 0.5,
   ...
   * @param agentId
   */
  getCommunicationPatterns(agentId?: string): ICommunicationPattern[] {
  ...
  private calculateTaskPattern(
    ...
    const pattern = {
      ...
      successRate: 0.8, // Default, can be calculated from success data
    };
    
    // Validate with schema
    return TaskCompletionStatsSchema.parse(pattern);
  }

  private calculateResourcePattern(
    resourceType: string,
    usages: number[]
  ): ResourceUsageStats & { significance: number } {
    ...
    const pattern = {
      ...
      max: Math.max(...usages),
      percentiles: {
        '25': String(sorted[Math.floor(sorted.length * 0.25)] || 0),
        '50': String(sorted[Math.floor(sorted.length * 0.5)] || 0),
        '75': String(sorted[Math.floor(sorted.length * 0.75)] || 0),
        '90': String(sorted[Math.floor(sorted.length * 0.9)] || 0),
        '95': String(sorted[Math.floor(sorted.length * 0.95)] || 0),
      },
    };
    
    // Validate with schema and add significance
    const validatedPattern = ResourceUsageStatsSchema.parse(pattern);
    const significance = this.calculateSignificance(usages);
    
    return { ...validatedPattern, significance };
  }

  private calculateCoordinationPattern(
    topology: string,
    traces: ExecutionTrace[]
  ): { successRate: number; effectiveness: number; topology: string; averageCoordinationTime: number; agentParticipation: number } {
  ...
  private calculateConfidence(pattern: TaskCompletionStats): number {
  ...
  private getMostFrequentMessageTypeFromTraces(
    ...
    const relevantTraces = traces.filter(
      (t) => t.agentId === source && 
      (t.parameters as { target?: string } | undefined)?.target === target
    );

    const types = new Map<string, number>();
    for (const trace of relevantTraces) {
      const params = trace.parameters as { messageType?: string } | undefined;
      const type = params?.messageType || 'unknown';
  ...
  private calculateReliabilityFromTraces(
    ...
    const successful = relevantTraces.filter((t) => {
      const result = t.result as { success?: boolean } | undefined;
      return result?.success !== false;
    }).length;
  ...
  private extractCoordinationContext(
    ...
    const firstParams = firstTrace?.parameters as { topology?: string } | undefined;
    
    return {
      swarmId: firstTrace?.swarmId || 'unknown',
      agentIds: [...new Set(traces.map((t) => t.agentId))],
      taskType: 'coordination',
      topology: firstParams?.topology || 'unknown',
      environment: 'production',
      resourceConstraints: {
        cpuLimit: 100,
        memoryLimit: 1024,
  ...
  private calculateCoordinationMetadata(
    pattern: { successRate: number; effectiveness: number; topology: string; averageCoordinationTime: number; agentParticipation: number },
    traces: ExecutionTrace[]
  ): PatternMetadata {
    return {
  ...
  private convertToTrace(data: ExecutionData): ExecutionTrace {
    ...
    const trace: ExecutionTrace = {
      ...
      resourceUsage: {
        ...
        network: data?.resourceUsage?.network || 0,
        diskIO: data?.resourceUsage?.diskIO || 0,
        timestamp: (data?.resourceUsage as any)?.timestamp || Date.now(),
        duration: (data?.resourceUsage as any)?.duration || data?.duration || 0,
  ...
  private generateInsights(
    ...
    const lowPerformanceClusters = clusters.filter(
      (c) => {
        const centroid = c.centroid as { successRate?: number; avgDuration?: number };
        return (centroid.successRate || 0) < 0.8 || (centroid.avgDuration || 0) > 5000;
      }
    );
  ...
  private calculateResourceStatistics(values: number[]): ResourceUsageStatistics {
  ...
  private generateResourceOptimizations(
    ...
    const optimizations: ResourceOptimizationSuggestion[] = [];
    const mean = values.reduce((sum, v) => sum + v, 0) / values.length;
    const max = Math.max(...values);
  ...
  private estimateTimeToFailure(pattern: FailurePattern): number {
    // Simple estimation based on frequency
    return Math.max(3600, 86400 / pattern.frequency); // At least 1 hour
  }

  private calculatePredictionConfidence(pattern: FailurePattern): number {
  ...
  private identifyAffectedComponents(pattern: FailurePattern): string[] {
    return pattern.context.slice(0, 3); // First 3 context items as components
  }

  private generatePreventionActions(pattern: FailurePattern): string[] {
  ...
  private identifyCommonFailures(traces: ExecutionTrace[]): string[] {
    ...
    for (const trace of traces) {
      // Safe access to result properties using SuccessData schema
      try {
        const successData = SuccessDataSchema.parse({
          success: false,
          target: trace.agentId,
          messageType: 'error',
          error: (trace.result as { error?: string } | undefined)?.error || 'unknown_error'
        });
        
        if (successData.error) {
          const errorType = this.classifyError(successData.error);
          errorTypes.set(errorType, (errorTypes.get(errorType) || 0) + 1);
        }
      } catch {
        // Fallback for non-conforming results
        const result = trace.result as { error?: unknown; success?: boolean } | undefined;
        if (result?.error) {
          const errorType = this.classifyError(result.error);
          errorTypes.set(errorType, (errorTypes.get(errorType) || 0) + 1);
        }
  ...
  private calculateCommunicationEfficiencyFromTraces(
    source: string,
    target: string,
    traces: ExecutionTrace[]
  ): number {
    const avgLatency = this.calculateAverageLatencyFromTraces(source, target, traces);
    const avgSize = this.calculateAveragePayloadSize(source, target, traces);
    // Simple efficiency calculation: inversely related to latency and size
    return 1 / (1 + avgLatency / 1000 + avgSize / 1000);
  }

  private generatePreventionStrategies(traces: ExecutionTrace[]): PreventionStrategy[] {
    const strategies: PreventionStrategy[] = [];
    
    // Extract common patterns from failure context
    const commonActions = traces.map(t => t.action).filter(Boolean);
    const uniqueActions = [...new Set(commonActions)];
    
    if (uniqueActions.length > 0) {
      strategies.push({
        type: 'validation',
        description: `Implement validation before executing ${uniqueActions[0]} actions`,
        effectiveness: 0.8,
        cost: 0.3,
        implementation: ['Add input validation', 'Implement pre-condition checks']
      });
    }
    ...
    strategies.push({
      ...
      effectiveness: 0.7,
      cost: 0.2,
      implementation: ['Implement retry mechanism', 'Add exponential backoff']
    });
    ...
    strategies.push({
      ...
      effectiveness: 0.9,
      cost: 0.5,
      implementation: ['Add circuit breaker', 'Implement fallback mechanisms']
    });
    ...
    strategies.push({
      ...
      effectiveness: 0.6,
      cost: 0.4,
      implementation: ['Implement monitoring', 'Set up alerting', 'Add dashboards']
    });
