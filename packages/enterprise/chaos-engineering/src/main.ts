...
export class ChaosEngineering extends TypedEventBase {
	...
	private async registerAdvancedInjectors() {
		try {
			...
			Object.entries(advancedInjectors).forEach(([name, injector]) => {
	...
	private async registerExtendedExperiments() {
		try {
			...
			extendedExperiments.forEach((experimentDef) => {
	...
	async checkResourceUsage() {
		...
		const [memUsage, totalMem] = await Promise.all([
			new Promise((resolve) => setTimeout(() => resolve(process.memoryUsage()), 10)),
			new Promise((resolve) => setTimeout(() => resolve(require("node:os").totalmem()), 10))
		]);
	...
	async collectImpactMetrics() {
		...
		const [memoryMetrics, cpuMetrics, connectionStats] = await Promise.all([
			new Promise((resolve) => setTimeout(() => resolve(process.memoryUsage()), 10)),
			new Promise((resolve) => setTimeout(() => resolve(process.cpuUsage()), 10)),
			new Promise((resolve) => setTimeout(() => resolve(
				this.connectionManager ? this.connectionManager.getConnectionStats() : null
			), 10)),
		]);
