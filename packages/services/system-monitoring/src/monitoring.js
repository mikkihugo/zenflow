...
export class SystemMonitor {
    ...
    async getHealthStatus() {
        return await withAsyncTrace('system.health_check', async () => {
            ...
            const checks = {
                cpu: {
                    status: metrics.cpu.usage > this.config.cpuErrorThreshold
                        ? 'error'
                ...
                memory: {
                    status: metrics.memory.usage > this.config.memoryErrorThreshold
                        ? 'error'
...
export class PerformanceTracker {
    operations = new Map();
    logger = getLogger('PerformanceTracker');
    /**
...
export class InfrastructureMetrics {
     ...
     */
    trackResourceUtilization(resource, utilization) {
...
export async function getSystemMonitoring(config) {
    ...
    return {
        ...
        trackPerformance: (operation, duration) => recordHistogram('operation.duration', duration),
        // Lifecycle
        shutdown: async () => {
