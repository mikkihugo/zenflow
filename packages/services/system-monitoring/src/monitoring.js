...
import { getTelemetry, initializeTelemetry, recordGauge, recordMetric, withAsyncTrace, } from '@claude-zen/telemetry';
...
export class SystemMonitor {
    config;
    logger = getLogger('SystemMonitor');
    ';
    ...
    async getHealthStatus() {
        return await withAsyncTrace('system.health_check', async () => {
            ...
            const checks = { ': cpu }, { status: metrics, cpu, usage };
             > this.config.cpuErrorThreshold
                ? 'error' : ;
            ';
            ...
            this.config.cpuWarningThreshold,
                message;
            `System CPU usage: ${metrics.cpu.usage}%`, `
        },
        memory: {
          status:
          ...
          message: `;
            ...
            status: metrics.disk.usage > this.config.diskErrorThreshold
...
';
recordMetric('system.health.checks.errors', Object.values(checks).filter((c) => c.status === 'error').length, ');
recordMetric('system.health.checks.warnings', Object.values(checks).filter((c) => c.status === 'warning').length, ');
return status;
;
...
void {
    ...
    this: .logger.info('Started system metrics collection', { ': interval, this: .config.systemMetricsInterval,
...
export class PerformanceTracker {
    ...
    getMetrics() {
        ...
        const systemAvgResponseTime = allOperations.length > 0
            ? allOperations.reduce((sum, op) => sum + op.avgTime, 0) /
                allOperations.length
            : 0;
        const systemThroughput = allOperations.reduce((sum, op) => sum + (op.throughput || 0), 0);
        return {
            operations,
            system: {
...
if (change < -0.05)
    return 'improving'; // 5% improvement'
if (change > 0.05)
    return 'degrading'; // 5% degradation'
return 'stable';
/**
...
export class InfrastructureMetrics {
     ...
     */
    trackSystemOperation(operation, duration, success) {
        recordHistogram(`infrastructure.$operation.duration`, duration);
        `
    recordMetric(`;
        infrastructure.$operation.calls `, 1, `;
 ...
 */
trackResourceUtilization(resource, string, utilization, number);
void {} `
 ...
 */
export function createSystemMonitor(config) {
 ...
 */
export async function getSystemMonitoring(config) {
    ...
    return {
        ...
        trackOperation: (name, duration) => tracker.recordDuration(name, duration),
        startTimer: (name) => tracker.startTimer(name),
        getPerformanceMetrics: () => tracker.getMetrics(),
        // Infrastructure metrics
        trackSystem: (metrics) => recordMetric('system.health', metrics.cpu + metrics.memory),
        trackPerformance: (operation, duration) => recordHistogram('operation.duration', duration, { operation }),
        // Lifecycle
        shutdown: async () => {
