...
export class EventDrivenSystemMonitor {
  ...
  private async getHealthStatusInternal(): Promise<HealthStatus> {
    ...
    this.emitEvent('telemetry:record-metric', {
      name: 'system.health.checks.total',
      value: Object.keys(checks).length,
      timestamp: Date.now(),
    });
    this.emitEvent('telemetry:record-metric', {
      name: 'system.health.checks.errors',
      value: Object.values(checks).filter(c => c.status === 'error').length,
      timestamp: Date.now(),
    });
  ...
  private startSystemMetricsCollection(): void {
    this.monitoringInterval = setInterval(async () => {
      try {
        ...
        this.emitEvent('telemetry:record-metric', {
          name: 'system.metrics.collection.success',
          value: 1,
          timestamp: Date.now(),
        });
      } catch (error) {
