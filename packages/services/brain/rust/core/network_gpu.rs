...
use crate::webgpu::backend::{
  ActivationFunction as GpuActivationFunction, MatrixDims,
};
#[cfg(feature = "gpu")]
use crate::webgpu::{ComputeBackend, ComputeResult};

#[cfg(feature = "gpu")]
impl<T: Float + Send + Sync + Default + bytemuck::Pod + FromPrimitive>
  Network<T>
{
  ...
  pub async fn run_gpu(&mut self, inputs: &[T]) -> ComputeResult<Vec<T>> {
    ...
    let backend_selector = self.backend_selector.as_ref().ok_or(
      crate::webgpu::error::ComputeError::backend_error(
        "Backend selector not initialized",
      ),
    )?;
    ...
    for layer_idx in 1..self.layers.len() {
      ...
      let mut layer_outputs = gpu_backend
        .matrix_vector_multiply(&weights, &current_activations, dims)
        .await?;
      ...
      for neuron in layer.neurons.iter().filter(|n| !n.is_bias) {
        if bias_idx < layer_outputs.len() {
          ...
          if let Some(bias_conn) = neuron.connections.last() {
            layer_outputs[bias_idx] =
              layer_outputs[bias_idx] + bias_conn.weight;
          }
          bias_idx += 1;
        }
      }
      ...
      let activation =
        if let Some(neuron) = layer.neurons.iter().find(|n| !n.is_bias) {
          convert_activation_function(neuron.activation_function)
        } else {
          GpuActivationFunction::Linear
        };
      layer_outputs = gpu_backend
        .activation_function(&layer_outputs, activation)
        .await?;
    ...
    T: std::iter::Sum + std::ops::Div<Output = T> + FromPrimitive,
  {
    ...
    for (input, target) in data.inputs.iter().zip(data.outputs.iter()) {
      ...
      // Sum squared errors
      let sample_error: T = errors.iter().map(|e| *e * *e).sum();
...
#[cfg(feature = "gpu")]
fn convert_activation_function(
  fann_activation: crate::ActivationFunction,
) -> GpuActivationFunction {
