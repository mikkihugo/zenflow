...
impl OptimizationSelector {
  ...
  pub fn select_strategy(
    ...
    let dspy_benefit_score = 0.4 * complexity_score +    // High complexity benefits from DSPy
    ...
    let similar_tasks: Vec<&PerformanceRecord> = self
      ...
      .filter(|record| {
        self.task_similarity(metrics, &record.metrics) > 0.7
          && record.age_ms() < 86400000 // Within 24 hours
      })
      .collect();
  ...
  fn task_similarity(&self, task1: &TaskMetrics, task2: &TaskMetrics) -> f32 {
    let agent_sim = 1.0
      - ((task1.agent_count as f32 - task2.agent_count as f32).abs() / 100.0)
        .min(1.0);
    let complexity_sim = 1.0 - (task1.complexity - task2.complexity).abs();
    let dimension_sim = 1.0
      - ((task1.dimensions as f32 - task2.dimensions as f32).abs() / 1000.0)
        .min(1.0);
  ...
  fn adapt_thresholds(&mut self) {
    ...
    let recent_records: Vec<&PerformanceRecord> =
      self.performance_history.iter().rev().take(50).collect();
    ...
    for record in &recent_records {
      let perf_score =
        record.accuracy / (record.execution_time_ms as f32 / 1000.0 + 0.1);
    ...
    let avg_age_ms = if !recent_records.is_empty() {
      recent_records.iter().map(|r| r.age_ms()).sum::<u64>()
  ...
  pub fn get_performance_stats(&self) -> (usize, f32, f32) {
    ...
    let weighted_accuracy: f32 = self
      .performance_history
      .iter()
      .map(|r| {
