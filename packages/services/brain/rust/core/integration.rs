...
impl<T: Float + Send + Default> IntegrationTestSuite<T> {
  ...
  pub fn generate_test_datasets(&mut self) -> Result<(), IntegrationError> {
    ...
    for _ in 0..100 {
      let input: Vec<T> = (0..4)
        .map(|_| T::from(rng.r#gen::<f64>()).unwrap())
        .collect();
  ...
  fn run_training_integration_test(
    ...
    */
    ...
    Ok(BenchmarkResult {
      duration,
      memory_mb: 0.0,
      throughput: T::from(data.inputs.len())
        .unwrap_or_default()
        .to_f64()
        .unwrap_or(0.0)
  ...
  fn run_cascade_integration_test(
    ...
    let mut trainer =
      CascadeTrainer::new(config, network, data).map_err(|e| {
        IntegrationError::TestFailed(format!(
          "Cascade trainer creation failed: {e}"
        ))
      })?;

    let result_data = trainer.train().map_err(|e| {
      IntegrationError::TestFailed(format!("Cascade training failed: {e}"))
    })?;
    ...
    if let Some(baseline) = &self.baseline_metrics {
      for (test_name, current) in &result.benchmarks {
        if let Some(baseline_result) = baseline.get(test_name) {
          let ratio = current.duration.as_secs_f64()
  ...
  fn test_parallel_execution(
    ...
    #[cfg(not(feature = "parallel"))]
    {
      result.warnings.push(
        "Parallel features not available - skipping parallel tests".to_string(),
      );
    }
...
mod tests {
  ...
  fn test_fann_compatibility_validator() {
    let mut validator: FannCompatibilityValidator<f32> =
      FannCompatibilityValidator::new();
