...
impl WasmNetworkCore {
  ...
  fn train_batch(
    ...
    self
      .network
      .train(&training_inputs, &training_outputs, 0.1, epochs as usize)
      .map_err(|e| {
        NeuralError::TrainingFailed(format!("FANN training failed: {:?}", e))
      })?;
...
#[cfg(feature = "wasm")]
static NETWORKS: LazyLock<Mutex<HashMap<u32, WasmNetworkCore>>> =
...
impl WasmNetwork {
  ...
  pub fn train(
    ...
    log::info!(
      "Training completed for network {}. Final error: {:.6}",
      self.handle,
      final_error
    );
    Ok(final_error)
  ...
  pub fn predict(&self, inputs: &[f32]) -> Result<Vec<f32>, JsValue> {
    ...
    let outputs = network.forward(inputs).map_err(JsValue::from)?;
  ...
  pub fn set_learning_rate(&mut self, rate: f32) -> Result<(), JsValue> {
    ...
    log::info!(
      "Learning rate {} noted for network {} (will be used in training)",
      rate,
      self.handle
    );
    Ok(())
