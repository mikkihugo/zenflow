...
use crate::{
  ActivationFunction, Network, TrainingData, cascade_error,
...
impl<T: Float> CandidateNeuron<T> {
  ...
  pub fn new(
    ...
    let weights: Vec<T> = (0..num_inputs)
      .map(|_| {
        let w: f64 = rng.gen_range(
          min_weight.to_f64().unwrap()..=max_weight.to_f64().unwrap(),
        );
        T::from(w).unwrap()
      })
      .collect();

    let bias_val: f64 = rng
      .gen_range(min_weight.to_f64().unwrap()..=max_weight.to_f64().unwrap());
  ...
  pub fn activation_derivative(&self, output: T) -> T {
    match self.activation {
      ActivationFunction::Sigmoid => {
        output * (T::one() - output) * self.steepness
      }
  ...
  pub fn update_weights(
    ...
    for i in 0..self.weights.len() {
      if use_momentum {
        self.weight_momentum[i] = momentum * self.weight_momentum[i]
    ...
    if use_momentum {
      self.bias_momentum =
        momentum * self.bias_momentum + learning_rate * self.bias_gradient;
...
impl<T: Float> CascadeTrainer<T> {
  ...
  pub fn train(&mut self) -> Result<CascadeTrainingResult<T>, RuvFannError> {
    ...
    while self.hidden_count < self.config.max_hidden_neurons {
      ...
      if best_candidate.correlation < self.config.min_correlation_improvement {
        #[cfg(feature = "logging")]
        info!(
          "No candidate meets minimum correlation improvement threshold. Stopping cascade training."
        );
        break;
      }
  ...
  fn generate_candidates(
    &mut self,
  ) -> Result<Vec<CandidateNeuron<T>>, RuvFannError> {
  ...
  fn calculate_correlation(
    ...
    let mut candidate_outputs =
      Vec::with_capacity(self.training_data.inputs.len());
    ...
    for output_idx in 0..num_outputs {
      let residual_values: Vec<T> =
        residuals.iter().map(|r| r[output_idx]).collect();

      let correlation =
        self.pearson_correlation(&candidate_outputs, &residual_values)?;
  ...
  #[cfg(feature = "parallel")]
  pub fn train_single_candidate_parallel(
    &self,
    _candidate: &mut CandidateNeuron<T>,
  ...
  T::FromStrRadixErr: Send + Sync,
{
  ...
  pub fn train_candidates_parallel(
    &mut self,
    candidates: &mut [CandidateNeuron<T>],
  ) -> Result<(), RuvFannError> {
    #[cfg(feature = "parallel")]
  ...
  pub fn train_single_candidate_with_data(
    &self,
    candidate: &mut CandidateNeuron<T>,
    data: &TrainingData<T>,
    config: &CascadeConfig<T>,
    ...
    for _epoch in 0..config.candidate_max_epochs {
      ...
      let correlation = self
        .calculate_candidate_correlation_with_data(candidate, data, network)?;
  ...
  pub fn calculate_candidate_correlation_with_data(
    &self,
    candidate: &CandidateNeuron<T>,
    data: &TrainingData<T>,
    _network: &Network<T>,
  ...
  pub fn update_candidate_weights_simple(
    &self,
    candidate: &mut CandidateNeuron<T>,
