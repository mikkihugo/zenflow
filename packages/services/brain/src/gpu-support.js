...
export async function detectGPUCapabilities() {
    ...
    try {
        ...
        if (GPU) {
            const gpu = new GPU();
            capabilities.hasGPUJS = !!gpu.context;
 ...
 */
export async function initializeGPUAcceleration(options = {}) {
        ...
        switch (selectedBackend) {
            case 'tensorflow-gpu':
                ...
                if (capabilities.hasTensorFlowGPU) {
                    ...
                    logger.debug('TensorFlow GPU module loaded', ', version, tf.version?.['tfjs-core'] || 'unknown', backend, selectedBackend);
    ...
    logger.warn(`, Failed, to, initialize, $, { selectedBackend }, acceleration, `, error);`);
                 ...
                 */
                export async function createAcceleratedNeuralNetwork(architecture, options = {}) {
                        ...
                        try {
                            if (!this.kernels.has('multiply')) {
                                ...
                                const kernel = this.gpu
                                    .createKernel(function (a, b, aWidth) {
                                    let sum = 0;
                            ...
                            ';
                            const result = kernel(a, b, a[0].length);
                ...
                export const isGPUAvailable = async () => {
                    const capabilities = await detectGPUCapabilities();
                    return (capabilities.hasTensorFlowGPU ||
                        capabilities.hasWebGPU ||
                        capabilities.hasGPUJS ||
                        capabilities.hasONNXGPU);
                };
                export const getRecommendedGPUBackend = async () => {
                    const capabilities = await detectGPUCapabilities();
