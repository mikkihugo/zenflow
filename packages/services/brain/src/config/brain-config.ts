...
export function validateBrainConfig(
  ...
  try {
    ...
    if (
      config.maxNetworks  && (config.maxNetworks < 1||config.maxNetworks > 100)
    ) {
      throw new Error('maxNetworks must be between 1 and 100');'

    if (
      config.defaultBatchSize  && (config.defaultBatchSize < 1||config.defaultBatchSize > 1024)
    ) {
      throw new Error('defaultBatchSize must be between 1 and 1024');'

    if (
      config.dspy?.maxTokens  && (config.dspy.maxTokens < 100||config.dspy.maxTokens > 10000)
    ) {
