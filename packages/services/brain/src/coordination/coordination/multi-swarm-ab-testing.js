...
export class MultiSwarmABTesting {
    ...
    testHistory = [];
    constructor(codingPrinciplesResearcher, promptGenerator) {
        ...
        this.promptGenerator =
            promptGenerator || new IntelligentPromptGenerator(undefined, this.codingPrinciplesResearcher);
    }
    /**
     * Execute A/B test with multiple swarm strategies
     */
    async executeABTest(taskDescription, strategies, options = {}) {
  ...
  createStrategySet(
    ...
    getRecommendations(taskType) {
        ...
        const relevantTests = this.testHistory.filter((test) => test.description.toLowerCase().includes(taskType.toLowerCase()));
    ...
    return {
      ...
      reasoning: [
        `, Based, on, $, { relevantTests, : .length }, historical, tests
                ], for: "${taskType}" `,` `Success rate: ${(successRate * 100).toFixed(1)}%`,
            } `
        `;
            Model: $;
            {
  ...
  private async prepareGitWorktrees(
    ...
    for (const strategy of strategiesToProcess) {
      const branchName = `, $, { branchPrefix } - $, { strategy, : .id } - $, {} `;`);
...
logger.info('Git worktree preparation completed', { ': totalWorktrees, Object, : .keys(worktreePaths).length,
    cleanupAfterTest,
...
return results;
recordCleanupMetrics(strategyId, string, path, string);
void {
    // Record cleanup metrics for monitoring and analytics
    logger, : .debug('Recording cleanup metrics', { ': strategyId,
