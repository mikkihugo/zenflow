    ...
    ')      const priorities:Array<'low|medium|high'> = [')        'low',        'medium',        'high',];
      ...
      const results = await Promise.all(
        priorities.map(async (priority) => {
          const _request:NeuralEmbeddingRequest = {
            text:`Priority test ${priority}`,"Fixed unterminated template":`Quality test ${qualityLevel}`,"Fixed unterminated template":`Performance test ${i}`,"Fixed unterminated template":`Quality test for ${qualityLevel} classification`,"Fixed unterminated template" `"Fixed unterminated template" `;"Fixed unterminated template"