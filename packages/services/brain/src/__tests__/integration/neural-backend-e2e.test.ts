  ...
  describe('Performance and Scaling Scenarios', () => {
    ...
    ')      const batchSize = 50;
      const texts = Array.from(
        { length:batchSize},
        (_, i) =>
          `Performance test document ${i + 1} with unique content and identifier ${Math.random()}`"Fixed unterminated template" `Cache pressure test ${i}`"Fixed unterminated template":`quality-test-${testCase.type}`,"Fixed unterminated template"