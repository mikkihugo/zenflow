...
export class AgentPerformancePredictor {
     ...
     */
    async persistPerformanceTrends(agentId, successRates) {
        try {
            ...
            logger.debug(`Persisted performance trends for agent ${agentId}:`, {
                ...
                trend: trendData.trendMetrics.slope > 0 ? 'improving' : trendData.trendMetrics.slope < -0.1 ? 'declining' : 'stable',
    ...
    async performEnhancedBottleneckDetection() {
        const bottlenecks = [];
        const allAgents = Array.from(this.performanceHistory.keys());
    ...
    async detectAgentBottlenecks(agentId, systemLoad) {
    ...
    async loadPersistedMetrics(agentId) {
        try {
    ...
    async enhanceMetricsWithTrends(currentMetrics, persistedMetrics) {
        await new Promise(resolve => setTimeout(resolve, 5));
        if (!persistedMetrics) {
            return {
                ...currentMetrics,
    ...
    async identifyAdvancedPerformanceIssues(metrics, systemLoad, agentId) {
        const issues = [];
    ...
    calculateAgentMetrics(recentData) {
        return {
    ...
    identifyPerformanceIssues(metrics, systemLoad) {
        const issues = [];
    ...
    addSystemWideBottlenecks(bottlenecks, systemLoad) {
        if (systemLoad > 75) {
  ...
  private async applyMLForecast(agentId: string, metrics: Record<string, any>): Promise<number> {
     ...
     * Log comprehensive agent performance snapshot to database
     */
    async logAgentPerformanceSnapshot(agentId, metrics, systemLoad) {
        try {
