...
export async function demoBehavioral: Intelligence(): void {
  ...
  try {
    ...
    logger.info(
      "   • Required skills:$complexity: Analysis.required: Skills.join(',    ')Finding best agent for task...'))    const best: Agent = await behavioral.findBestAgentFor: Task(
      'data-processing',      0.5,
      ['agent-1',    'agent-2']') Behavioral: Intelligence Statistics:'), ')\n🎉 Behavioral: Intelligence Demo: Complete!')\nidea: Key Benefits for claude-code-zen:')   • Real-time agent performance prediction')   • Intelligent task-agent matching')   • Behavioral pattern learning and adaptation')   • Task complexity estimation for better routing')   • Data-driven swarm optimization')Demo failed:', error);
  }
}
