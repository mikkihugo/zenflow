...
try {
  DocumentManager =
    require('./../services/document/document-service').DocumentManager;')} catch {
...
export class StrategicVisionService {
  ...
  async analyzeProjectVision(
    ...
    try {
      ...
      const visionQuery = await this.documentManager.searchDocuments({
        searchType: 'combined',        query: 'vision mission strategy goals objectives',        documentTypes:['vision, prd',    'epic'],
        projectId,
        includeContent:true,
  ...
  async importStrategicDocuments(options:VisionImportOptions): Promise<{
    ...
    try {
      ...
      // Import from files if requested and documents don't exist')      if (options.importFromFiles && options.projectPath) {
  ...
  async createVisionDocument(
    projectId:string,
    type: 'vision|prd|epic|strategy',    content:{
      title:string;
      summary:string;
      content:string;
      goals?:string[];
    ...
    try {
      ...
      const __docData = {
        ...
        author: 'strategic-vision-service',        project_id:projectId,
        ...
        metadata:{
          ...
          created_by: 'strategic-vision-service',          document_source: 'service_generated',},
...
'      );
  ...
  private async analyzeStructuredDocuments(
    ...
    const visionDoc = documents.find((doc) => doc.type === 'vision') as|any|undefined;')    const prdDoc = documents.find((doc) => doc.type ==='prd') as|any|undefined;')    const epicDocs = documents.filter((doc) => doc.type ==='epic') as any[];')
    ...
    return {
      ...
      marketPosition:
        visionDoc?.metadata?.market_position||'Database-driven analysis',      targetOutcome:
        visionDoc?.metadata?.target_outcome||strategicGoals[0]||'Structured outcome delivery',      keyMetrics,
      stakeholders:
        stakeholders.length > 0 ? stakeholders:['Database stakeholders'],
      timeline:
        visionDoc?.metadata?.timeline||'Timeline from structured documents',      risks:risks.length > 0 ? risks : ['Database-identified risks'],
      confidenceScore,
  ...
  private async importFromFiles(
    ...
    try {
      const { access, readFile, readdir} = await import('node:fs/promises');')      const { join, extname} = await import('node:path');')
    ...
    ')            potentialDocFiles.push(file);
        ...
        const docsPath = join(projectPath, 'docs');')        try {
      ...
      for (const file of potentialDocFiles) {
        try {
          ...
          const docData = {
            ...
            content,
            author: 'llm-content-classifier',            project_id:projectId,
            status:'draft' as const,
            priority:classification.suggestedPriority,
            keywords:classification.extractedKeywords,
            tags:[
              classification.documentType,
              'llm-classified',              'content-analyzed',              ...classification.suggestedTags,
],
            metadata:{
              source_file:file,
              import_date:new Date().toISOString(),
              document_source: 'llm_classified_import',              llm_confidence:classification.confidence,
            ...
            version: '1.0',            dependencies:classification.suggestedDependencies,
            related_documents:[],
};
...
'    strategicRelevance:number; // 0-1 score
    ...
    try {
      ...
      let documentType = 'document'; // default')      let confidence = 0.5;
      ...
      if (this.containsVisionKeywords(contentLower)) {
        documentType = 'vision';
        confidence = 0.9;
        themes.push('strategic-planning',    'future-state',    'objectives');')        actions.push('Review strategic alignment with current goals');')        actions.push('Identify measurable outcomes and metrics');')}

      // PRD/Requirements detection
      else if (this.containsRequirementsKeywords(contentLower)) {
        documentType = 'prd';
        confidence = 0.85;
        themes.push('requirements',    'specifications',    'user-needs');')        actions.push('Validate requirements with stakeholders');')        actions.push('Create technical specifications from requirements');')}

      // Epic/Task detection
      else if (this.containsTaskKeywords(contentLower)) {
        documentType = 'epic';
        confidence = 0.8;
        themes.push('tasks, implementation',    'deliverables');')        actions.push('Break down epics into actionable tasks');')        actions.push('Assign priorities and effort estimates');')}

      // Architecture/Technical detection
      else if (this.containsArchitectureKeywords(contentLower)) {
        documentType = 'adr';
        confidence = 0.85;
        themes.push('technical-decisions, architecture',    'system-design');')        actions.push('Document decision rationale and alternatives');')        actions.push('Update architecture diagrams and dependencies');')}

      // Feature/Enhancement detection
      else if (this.containsFeatureKeywords(contentLower)) {
        documentType = 'feature';
        confidence = 0.75;
        themes.push('feature-development, user-experience',    'functionality');')        actions.push('Define user acceptance criteria');')        actions.push('Plan implementation phases and rollout');')}
      ...
      // Default classification based on content structure
        ...
        if (lines.length < 5) {
          documentType = 'note';
          confidence = 0.6;
          actions.push('Expand content with more detailed information');')} else if (this.hasStructuredFormat(content)) {
          documentType = 'specification';
          confidence = 0.7;
          actions.push('Review and validate technical specifications');')} else {
      ...
      const suggestedPriority =
        strategicRelevance > 0.7
          ? 'high')          :strategicRelevance > 0.4
            ? 'medium')            : 'low;
'
      // Add maturity-based actions
      if (documentMaturity === 'draft'||documentMaturity ===' partial') {
    ')        actions.push('Complete missing sections and add more detail');')} else if (documentMaturity === 'outdated') {
    ')        actions.push(
      ...
      return {
        documentType: 'document',        suggestedTitle:filename,
        summary: 'Document classification failed - manual review needed',        confidence:0.1,
        extractedKeywords:[],
        suggestedTags:['needs-classification'],
        suggestedPriority: 'medium',        suggestedActions:['Manually review and classify this document'],
        contentThemes:['unclassified'],
        documentMaturity: 'draft',        strategicRelevance:0.5,
        suggestedDependencies:[],
};
}
  ...
  private extractKeywordsFromContent(content:string): string[] {
    ...
    const words = content
      ?.toLowerCase()
      .replace(/[^sw]/g,' ')')      .split(/s+/)
      .filter((word) => word.length > 3)
      .filter(
        (word) =>
          ![
            'this',            'that',            'with',            'from',            'they',            'will',            'have',            'been',            'were',            'said',            'each',            'which',            'their',            'time',            'would',            'there',            'could',            'other',].includes(word)
      );
  ...
  private assessDocumentMaturity(
    ...
    ')    const lines = content.split('\n').filter((line) => line?.trim().length > 0);')    const totalLength = content.length;
  ...
  private calculateStrategicRelevance(
    ...
    const strategicThemes = [
      'strategic-planning',      'objectives',      'requirements',      'architecture',];
    ...
    const strategicWords = [
      'strategic',      'important',      'critical',      'priority',      'business',      'value',      'impact',      'outcome',];
  ...
  private generateSuggestedTitle(
    ...
    const baseName = filename.replace(/.(md|txt|rst|adoc)$/i,');')    const typePrefix =
      documentType === 'vision')        ? 'Vision: ')'        :documentType === 'prd')          ? 'Requirements: ')'          :documentType === 'adr')            ? 'Architecture: ')'            : ';
'
  ...
  private async loadGitignorePatterns(
    ...
    try {
      ...
      try {
        ...
        // .gitignore doesn't exist, use defaults')        logger.info(
      ...
      logger.error('Error loading .gitignore patterns:', error);')      return new Set([
        '.git',        'node_modules',        '.DS_Store',        '*.log',        'dist',        'build',]);
}
  ...
  private shouldIgnoreFile(
    ...
    try {
      const { relative} = require('node:path');')      const relativePath = relative(projectPath, filePath);
  ...
  private async importFromCodeAnnotations(
    ...
    try {
      const { access, readdir, readFile} = await import('node:fs/promises');')      const { join, extname} = await import('node:path');')
      const srcPath = join(projectPath, 'src');')      await access(srcPath);
      ...
      for (const file of codeFiles.slice(0, 100)) {
        ...
        if (
          typeof file === 'string' &&')          ['.ts',    '.tsx',    '.js',    '.jsx'].includes(extname(file))')        ) 
          try {
      ...
      const annotationDocs = [
        {
          type: 'epic',          content:todoAnnotations,
          title: 'Code TODOs and Tasks',},
        {
          type: 'strategy',          content:strategyAnnotations,
          title: 'Strategic Code Annotations',},
        {
          type: 'vision',          content:visionAnnotations,
      ...
      for (const annotationDoc of annotationDocs) {
        if (
          ...
          const docData = {
            ...
            author: 'code-annotation-service',            project_id:projectId,
            status:'draft' as const,
    ...
    ')        // Don't error if src directory doesn' t exist')        results.errors.push(
  ...
  private calculateConfidenceScore(documents:BaseDocumentEntity[]): number {
    ...
    if (documents.some((doc) => doc.type === 'vision')) score += 0.3;')    if (documents.some((doc) => doc.type === 'prd')) score += 0.2;')    if (documents.some((doc) => doc.type === 'epic')) score += 0.1;')
  ...
  private createDefaultVisionAnalysis(
    ...
    return {
      projectId,
      missionStatement:
        'No structured vision documents found - import documents to get detailed analysis',      strategicGoals:[],
      businessValue:0.3,
      technicalImpact:0.3,
      marketPosition: 'Not analyzed - no vision documents',      targetOutcome: 'Import strategic documents for analysis',      keyMetrics:[],
      stakeholders:[],
      timeline: 'Timeline not available',      risks:['No strategic documentation'],
      confidenceScore:0.1,
      sourceDocuments:[],
      lastAnalyzed:new Date(),
};
  ...
  private createErrorVisionAnalysis(
    ...
    return {
      projectId,
      missionStatement: 'Analysis failed - check system logs',      strategicGoals:[],
      businessValue:0,
      technicalImpact:0,
      marketPosition: 'Analysis error',      targetOutcome: 'Fix analysis errors',      keyMetrics:[],
      stakeholders:[],
      timeline: 'Unknown due to analysis error',      risks:['Analysis system error', error.message],
      confidenceScore:0,
      sourceDocuments:[],
      lastAnalyzed:new Date(),
};
