...
export class DocumentWorkflowService {
  ...
  async registerDocumentWorkflows(): Promise<void> {
    ...
    const documentWorkflows: DocumentWorkflowDefinition[] = [
      ...
      {
        name: 'prds-to-epics',
        description: 'Convert product requirements to epic definitions',
        version: '1.0.0',
        steps: [
          {
            type: 'analyze-requirements',
            name: 'Analyze product requirements for epic extraction',
            params: { outputKey: 'epic_requirements' },
          },
          {
            type: 'create-epic-documents',
            name: 'Create epic definition documents',
            params: { templateKey: 'epic_template', outputKey: 'epic_documents' },
          },
        ],
      },
    ];
      ...
      switch (documentType) {
        ...
        default:
          ...
          return {
            success: true,
            workflowId: 'none',
            results: { message: 'No workflow triggered for document type' }
          };
      }
  ...
  private async createEpicDocuments(
    ...
    for (const [epicName, features] of Object.entries(epicRequirements || {})) {
      const epicDoc: DocumentContent = {
        ...
        type: 'epic',
        title: `Epic: ${epicName.replace(/([A-Z])/g, ' $1').trim()}`,
        content: `
# Epic: ${epicName}
