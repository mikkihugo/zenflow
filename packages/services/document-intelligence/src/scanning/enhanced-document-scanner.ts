...
export class EnhancedDocumentScanner extends TypedEventBase {
  ...
  private async analyzeMarkdownDocument(
    ...
    for (const match of incompleteSections) {
      ...
      results.push({
        id:this.generateId(),
        type: 'documentation_gap',        severity: 'low',        title:`Empty documentation section: $sectionName`,`
        ...
        estimatedEffort: 'medium',        tags:['documentation',    'content'],
});
}
    ...
    for (const match of internalLinks) {
      ...
      results.push({
        id:this.generateId(),
        type: 'documentation_gap',        severity: 'low',        title:`Verify internal link: ${linkText}`,`
        ...
        estimatedEffort: 'small',        tags:['documentation',    'links'],
});
}
}
   ...
   * Generate swarm tasks from analysis results
   */
  private async generateSwarmTasks(
    analysisResults:CodeAnalysisResult[]
  ):Promise<GeneratedSwarmTask[]> {
  ...
  private createSwarmTask(
    ...
    const [patternType, directory] = groupKey.split('-');')    const pattern = patternType as AnalysisPattern;
  ...
  private generateAcceptanceCriteria(results:CodeAnalysisResult[]): string[] {
    ...
    switch (pattern) {
      case 'todo': ')'        criteria.push(
        ...
        break;
      case 'fixme': ')'        criteria.push('All FIXME issues are resolved and tested');')        criteria.push('Code passes all existing tests');')        break;
      case 'empty_function': ')'        criteria.push('All empty functions are implemented with proper logic');')        criteria.push('Functions have appropriate documentation');')        criteria.push('Unit tests are added for new implementations');')        break;
      case 'documentation_gap': ')'        criteria.push('All documentation sections are complete and accurate');')        criteria.push('Links are verified and functional');')        break;
      default:
  ...
  private matchesPattern(path:string, pattern:string): boolean {
    ...
    const regexPattern = pattern
      .replace(/\*\*/g, '.*')')      .replace(/\*/g, '[^/]*')')      .replace(/\?/g, '[^/]');')    return new RegExp(`^${regexPattern}$`).test(path);`
}
  ...
  private getPatternSeverity(
    ...
    ')    const severityMap:Record<AnalysisPattern, CodeAnalysisResult['severity']> =')        todo: 'low',        fixme: 'medium',        hack: 'medium',        deprecated: 'high',        missing_implementation: 'high',        empty_function: 'medium',        code_quality: 'medium',        documentation_gap: 'low',        test_missing: 'medium',        performance_issue: 'medium',        security_concern: 'critical',        refactor_needed: 'medium',;
  ...
  private getPatternEffort(
    ...
    ')    const effortMap:Record<
      AnalysisPattern,
      CodeAnalysisResult['estimatedEffort']')    > = {
      todo: 'small',      fixme: 'medium',      hack: 'medium',      deprecated: 'large',      missing_implementation: 'large',      empty_function: 'medium',      code_quality: 'medium',      documentation_gap: 'small',      test_missing: 'medium',      performance_issue: 'large',      security_concern: 'large',      refactor_needed: 'large',};
  ...
  private getMaxSeverity(
    ...
    ')    const severityOrder = ['low',    'medium',    'high',    'critical'];')    return results.reduce(
      (max, result) => {
  ...
  private severityToPriority(
    severity:CodeAnalysisResult['severity']')  ):GeneratedSwarmTask['priority'] {
