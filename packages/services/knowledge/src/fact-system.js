...
async function _createSQLiteFactClient() {
    return {
        async initialize() {
    ...
    try {
      ...
      logger.info(
        ...
        try { }
            ...
            logger.warn(`Rust bridge NPM lookup failed for ${packageName}, trying foundation fallback:`, `
        error
      );
      ...
      if (this.factClient) {
        try {
          ...
          logger.error(
            `, Foundation, NPM, lookup, also, failed);
            for ($; { packageName }; )
                : `,`;
            fallbackError;
            ;
...
getGitHubRepoInfo(owner, string, repo, string);
{
    ...
    try {
      ...
      logger.warn(
        `, Rust, bridge, GitHub, lookup, failed);
        for ($; { owner } / $; { repo }, trying)
            foundation;
        fallback: `,`;
        error;
        ;
        ...
        if (this.factClient) {
            try {
                ...
                logger.error(`Foundation GitHub lookup also failed for ${owner}/${repo}:`, `
            fallbackError
          );
      } else {
