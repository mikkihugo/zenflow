...
export const COORDINATION_FACT_SCHEMA = z.object({
  id: z.string().uuid(),
  type: z.string().min(1),
  data: z.unknown(),
  timestamp: z.date(),
...
export const COORDINATION_FACT_QUERY_SCHEMA = z.object({
...
export type CoordinationFactSchemaType = z.infer<typeof COORDINATION_FACT_SCHEMA>;
export type CoordinationFactQuerySchemaType = z.infer<typeof COORDINATION_FACT_QUERY_SCHEMA>;
...
class KnowledgeFactSystem {
    ...
    try {
      ...
      if (adapterResult.isOk()) {
        ...
        const dbResult = await adapter.connect({
          type: 'sqlite',
          path: ':memory:' // In-memory for now, can be configured for persistence
        });
  ...
  async storeFact(
    ...
    this.eventBus.emit('fact:stats', {
      operation: 'store',
      totalFacts: this.coordinationFacts.size,
  ...
  private buildQuerySQL(query: CoordinationFactQuery): { sql: string; params: unknown[] } {
    let sql = 'SELECT id, type, data, timestamp, source, confidence, tags FROM coordination_facts WHERE 1=1';
  ...
  async getFact(id: string): Promise<CoordinationFact | null> {
    ...
    try {
      const rows = await this.factDatabase.query<{
        ...
        confidence: number;
        tags: string;
      }>('SELECT id, type, data, timestamp, source, confidence, tags FROM coordination_facts WHERE id = ?', [id]);
  ...
  private async searchDatabaseFacts(query: string, searchType?: string, searchLimit = 10): Promise<CoordinationFact[]> {
    ...
    try {
      ...
      if (query.trim()) {
        sql += ' AND (JSON_EXTRACT(data, \'$\') LIKE ? OR source LIKE ? OR JSON_EXTRACT(tags, \'$\') LIKE ?)';
    ...
    try {
      ...
      for (const extResult of externalResults) {
        results.push({
          ...
          type: searchType || (extResult as { factType?: string }).factType || 'external-knowledge',
          data: (extResult as { metadata?: unknown }).metadata || extResult,
          timestamp: new Date(),
          source: 'rust-fact-bridge',
          confidence: (extResult as { score?: number }).score || 0.8,
      ...
      try {
        ...
        for (const extResult of fallbackResults) {
          results.push({
            id: `ext-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`,
            type: searchType || 'external-knowledge',
            data: extResult as unknown,
            timestamp: new Date(),
  ...
  async getNPMPackageInfo(packageName: string, version?: string) {
    ...
    try {
      ...
      logger.info(` NPM package info retrieved via Rust bridge: ${packageName}`);
      return npmResult;
    } catch (error) {
      logger.warn(`Rust bridge NPM lookup failed for ${packageName}, trying foundation fallback:`, error);
      ...
      if (this.factClient) {
        try {
          ...
          logger.error(`Foundation NPM lookup also failed for ${packageName}:`, fallbackError);
        }
      } else {
  ...
  async getGitHubRepoInfo(owner: string, repo: string) {
    ...
    try {
      ...
      logger.warn(`Rust bridge GitHub lookup failed for ${owner}/${repo}, trying foundation fallback:`, error);
      ...
      if (this.factClient) {
        try {
          ...
          logger.error(`Foundation GitHub lookup also failed for ${owner}/${repo}:`, fallbackError);
        }
      } else {
