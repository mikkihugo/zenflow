  ...
  private async initializeFactDatabases():Promise<void> {
    try {
      ...
      if (adapterResult.isOk()) {
        ...
        if (!adapter) {
          logger.warn(' Database adapter is undefined');
          return;
}
        ...
        if (dbResult.isOk()) {
          ...
          logger.warn(' Fact database connection failed, using in-memory fallback');
}
} else {
        logger.warn(' Fact database adapter unavailable, using in-memory fallback');
}
} catch (error) {
