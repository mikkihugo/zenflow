...
import { ContextError, createCircuitBreaker, createErrorAggregator, createTimestamp, ensureError, err, generateUUID, getLogger, injectable, ok, recordMetric, Storage, safeAsync, validateObject, withRetry, withTrace, } from '@claude-zen/foundation';
        ...
        async initialize() {
            ...
            try {
                ...
                if (!storageResult.success) {
                    const error = new KnowledgeError('Failed to initialize knowledge storage', { operation: 'initialize' }, storageResult.error);
                ...
                ';
                this.logger.info('Knowledge store initialized successfully', ', itemCount, this.items.size, storageConnected, !!this.storage);
                return ok(undefined);
            }
            catch (error) {
                const knowledgeError = new KnowledgeError('Knowledge store initialization failed', { operation: 'initialize' }, ensureError(error));
    ...
    return withTrace('knowledge_store_get', async () => {'
      ...
      try {
        ...
        const storageResult = await withTimeout(
          this.circuitBreaker.execute('get', `, knowledge, $, { id } `),`, 3000));
                    if (storageResult.success && storageResult.data) {
                        const item = JSON.parse(storageResult.data);
                    ...
                    const knowledgeError = new KnowledgeError('Failed to get knowledge item', { id, operation: 'get' }, ensureError(error));
                    this.errorAggregator.addError(knowledgeError);
                    this.performanceTracker.endTimer('knowledge_store_get');
                    ';
            ...
            return withTrace('knowledge_store_query', async () => {
                ...
                try {
                    ...
                    if (query?.contentSearch) {
                        ...
                        results = results.filter((item) => item.content.toLowerCase().includes(searchLower));
                    }
                    ...
                    const knowledgeError = new KnowledgeError('Failed to query knowledge items', { query, operation: 'query' }, ensureError(error));
                    this.errorAggregator.addError(knowledgeError);
                    return err(knowledgeError);
                }
            });
        ...
        async performStorageOperation(operation, ...args) {
            if (!this.storage) {
...
export async function getKnowledgeManagement(config?: any): Promise<any> {
  ...
  return {
    ...
    return FoundationKnowledgeStore = _classThis;
})();
export { FoundationKnowledgeStore };
;
...
export const knowledgeSystem = {
    ...
    createManager: (store) => new EnterpriseKnowledgeManager(store),
    createStore: () => new FoundationKnowledgeStore(),
    // Enhanced with fact-system integration
