...
export class CoreCompetencyFrameworkManager {
  ...
  createCompetencyAssessment(
    ...
    const assessmentId = `competency-${Date.now()}-${Math.random().toString(36).substr(2, 9)}"Fixed unterminated template" `Created competency assessment: ${assessmentId} for organization: ${organizationId}"Fixed unterminated template"(`Competency assessment not found: ${assessmentId}"Fixed unterminated template"(`Updated competency assessment: ${assessmentId}"Fixed unterminated template" `assessment-${Date.now()}-${Math.random().toString(36).substr(2, 9)}"Fixed unterminated template"