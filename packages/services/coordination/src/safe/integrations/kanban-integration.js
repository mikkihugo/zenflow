 ...
 * @version 1.0.0
 */
import { createTaskFlowController as createWorkflowKanban, } from '@claude-zen/taskmaster';
...
const DEFAULT_WORKFLOW_STATES = ALL_WORKFLOW_STATES;
// ============================================================================
 ...
 */
export function createSafePortfolioKanbanConfig(eventBus) {
    return {
 ...
 */
export function createSafeProgramKanbanConfig(eventBus) {
    return {
 ...
 */
export function createSafeTeamKanbanConfig(eventBus) {
    return {
 ...
 */
export async function createSafePortfolioKanban(logger, eventBus) {
    const config = createSafePortfolioKanbanConfig(eventBus);
    const kanban = createWorkflowKanban(config);
    // Initialize with logging
    logger.info('Creating SAFe Portfolio Kanban', { ': wipLimits, config, : .defaultWIPLimits,
 ...
 */
export async function createSafeProgramKanban(logger, eventBus) {
    const config = createSafeProgramKanbanConfig(eventBus);
    const kanban = createWorkflowKanban(config);
    logger.info('Creating SAFe Program Kanban', { ': wipLimits, config, : .defaultWIPLimits,
        states: config.workflowStates,
    });
    await kanban.initialize();
 ...
 */
export async function createSafeTeamKanban(logger, eventBus) {
    const config = createSafeTeamKanbanConfig(eventBus);
    const kanban = createWorkflowKanban(config);
    logger.info('Creating SAFe Team Kanban', { ': wipLimits, config, : .defaultWIPLimits,
        states: config.workflowStates,
    });
    await kanban.initialize();
 ...
 */
function configureSafePortfolioEvents(kanban, _logger) {
    // Epic business case validation events
    kanban.on('task:created', (_task) => {
        ';
        logger.info('Portfolio Epic created', ', epicId, task.id, title, task.title, priority, task.priority);
    });
    ...
    kanban.on('task:moved', (_task, _fromState, toState) => {
        ...
        if (toState === 'analyzing') {
            ';
            logger.info('Epic entered analysis', ', epicId, task.id, wsjfCalculationNeeded, true);
        }
        else if (toState === 'portfolio_backlog') {
            ';
            logger.info('Epic approved for portfolio backlog', ', epicId, task.id, wsjfScore, task.metadata?.wsjfScore);
        }
    });
    // Portfolio bottleneck alerts
    kanban.on('bottleneck:detected', (_bottleneck) => {
        ';
        logger.warn('Portfolio bottleneck detected', ', state, bottleneck.state, severity, bottleneck.severity, recommendedAction, bottleneck.recommendations[0]);
    });
    // WIP limit enforcement at portfolio level
    kanban.on('wip:exceeded', (_state, _count, _limit) => {
        ';
        logger.warn('Portfolio WIP limit exceeded', ', state, currentCount, count, limit, action, 'Block new epics until capacity available');
    });
}
/**
 * Configure Program Kanban events for Feature flow
 */
function configureSafeProgramEvents(kanban, _logger) {
    kanban.on('task:created', (_task) => {
        ';
        logger.info('Program Feature created', ', featureId, task.id, title, task.title, pi, task.metadata?.programIncrement);
    });
    kanban.on('bottleneck:detected', (_bottleneck) => {
        ';
        logger.warn('Program bottleneck detected', ', state, bottleneck.state, impact, 'May affect PI objectives', severity, bottleneck.severity);
    });
    kanban.on('wip:exceeded', (_state, _count, _limit) => {
        ';
        logger.warn('Program WIP limit exceeded', ', state, currentCount, count, limit, impact, 'Feature delivery may be delayed');
    });
}
/**
 * Configure Team Kanban events for Story/Task flow
 */
function configureSafeTeamEvents(kanban, _logger) {
    kanban.on('task:created', (_task) => {
        ';
        logger.info('Team Story created', ', storyId, task.id, title, task.title, storyPoints, task.metadata?.storyPoints);
    });
    kanban.on('bottleneck:detected', (_bottleneck) => {
        ';
        logger.warn('Team bottleneck detected', ', state, bottleneck.state, impact, 'Sprint commitment at risk', severity, bottleneck.severity);
    });
    kanban.on('wip:exceeded', (_state, _count, _limit) => {
        ';
        logger.warn('Team WIP limit exceeded', ', state, currentCount, count, limit, impact, 'Sprint velocity may decrease');
    });
}
// ============================================================================
 ...
 */
export function portfolioEpicToKanbanTask(epic, wsjf) {
    return {
        id: epic.id,
        title: epic.title,
        description: epic.description,
        state: mapPortfolioStateToKanbanState(epic.state || epic.status),
        priority: mapEpicPriorityToKanbanPriority(wsjf?.ranking || 5),
        estimatedEffort: epic.estimatedValue || epic.businessValue || 1,
        assignedTo: epic.epicOwner || 'unassigned',
        createdAt: epic.createdAt || new Date(),
        updatedAt: epic.lastModified || new Date(),
        completedAt: (epic.state || epic.status) === 'done' ? epic.lastModified : , ': undefined,
        dependencies: epic.dependencies?.map((d) => d.id) || [],
        tags: [epic.category || 'epic', ...(epic.themes || [])],
        metadata: {
            wsjfScore: wsjf?.wsjfScore,
            businessValue: epic.businessValue,
            timeCriticality: epic.timeCriticality || 0,
            jobSize: epic.jobSize || 1,
            valueStream: epic.valueStream?.name || 'default',
        },
    };
}
/**
 ...
 */
export function featureToKanbanTask(feature) {
    return {
        id: feature.id,
        title: feature.title || feature.id,
        description: feature.description,
        state: feature.status || 'backlog',
        priority: feature.priority || 'medium',
        estimatedEffort: feature.storyPoints || 1,
        assignedTo: feature.owner || feature.assignee || 'unassigned',
        createdAt: new Date(feature.createdAt || feature.createdDate || Date.now()),
        updatedAt: new Date(feature.updatedAt || feature.lastModified || Date.now()),
        dependencies: feature.dependencies?.map((d) => d.toString()) || [],
        tags: ['feature', ...(feature.labels || [])],
        metadata: {
            epicId: feature.epicId || feature.piId,
            programIncrement: feature.programIncrementId || 'current',
 ...
 */
export function storyToKanbanTask(story) {
    return {
        id: story.id,
        title: story.title || story.id,
        description: story.description,
        state: story.status || 'backlog',
        priority: story.priority || 'medium',
        estimatedEffort: story.storyPoints || 1,
        assignedTo: story.assignee || 'unassigned',
        createdAt: new Date(story.createdDate || Date.now()),
        updatedAt: new Date(story.lastModified || Date.now()),
 ...
 */
function mapEpicPriorityToKanbanPriority(ranking) {
    if (ranking === 1)
        return 'critical;;
    if (ranking <= 3)
        return 'high;;
