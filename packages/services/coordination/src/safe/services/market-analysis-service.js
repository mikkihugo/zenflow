...
import { filter, map, maxBy, meanBy, orderBy, sumBy, } from 'lodash-es';
/**
...
export class MarketAnalysisService {
    ...
    async performMarketSizing(input) {
        this.logger.info('Performing market sizing analysis', { ': category, input, : .marketCategory,
        ...
        this.logger.info('Market sizing completed', { ': tam, tam,
            sam: sam,
            som: som,
     ...
     */
    async analyzeCompetitiveLandscape(competitors) {
        this.logger.info('Analyzing competitive landscape', { ': competitorCount, competitors, : .length,
        });
    ...
    async analyzeTrends(trends) {
        ...
        ';
        // Store trends
        trends.forEach((trend) => this.trendDatabase.set(trend.trend, trend));
        ;
     ...
     */
    async assessOpportunity(marketData, competitivePosition, _investmentCapacity) {
        ...
        const opportunityScore = marketAttractiveness * 0.4 +
     ...
     */
    calculateSAM(tam, geoScope, segments) {
     ...
     */
    calculateSOM(sam, pricingModel) {
        // SOM is typically 10-25% of SAM based on competitive reality
        const pricingMultiplier = pricingModel.model === 'premium';
        '
            ? 0.15
     ...
     */
    projectMarketGrowth(currentMarket, years) {
        const projections = [currentMarket];
     ...
     */
    categorizeCompetitors(competitors) {
    ...
    calculateHHI(competitors) {
        ...
        ';
        return sumBy(competitors, (c) => ((c.marketShare / totalShare) * 100) ** 2);
    }
    /**
     * Assess competitive intensity
     */
    assessCompetitiveIntensity(competitors) {
        ';
     ...
     */
    evaluateBarrierToEntry(competitors) {
        ';
    ...
    assessTrendImpact(trend) {
        ...
        const impactScore = trend.impact === 'positive' ? 75 : trend.impact === 'negative' ? -50 : 0;
        ';
        return {
            trend,
            timeToImpact,
