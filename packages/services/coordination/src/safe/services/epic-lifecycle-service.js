...
export class EpicLifecycleService {
     ...
     */
    async progressEpicState(epicId, targetState, _gateEvidence) {
    ...
    async calculateWSJFScore(input) {
        ...
        const costOfDelay = input.businessValue +
            input.urgency +
            input.riskReduction +
        ...
        if (previousScore) {
            const previousSorted = orderBy(allScores, ([id, score]) => id === input.epicId ? previousScore.wsjfScore : score.wsjfScore, 'desc', ');
            previousRank =
        ...
        this.logger.info('WSJF score calculated', { ': epicId, input, : .epicId,
            score: wsjfScore,
            rank: currentRank,
            rankChange,
        });
 ...
 */
async;
getPortfolioKanbanMetrics();
Promise < PortfolioKanbanMetrics > {
    ...
    // State distribution
    const: stateDistribution = countBy(allEpics, 'status'),
...
const cycleTimeStages = filter(allStages, (s) => s.stage === PortfolioKanbanState.IMPLEMENTING || s.stage === PortfolioKanbanState.DONE);
...
const valueAddedTime = sumBy(filter(allStages, (s) => valueAddedStates.includes(s.stage)), 'duration', ');
...
getCurrentLifecycleStage(epicId, string);
EpicLifecycleStage | null;
{
    const stages = this.lifecycleStages.get(epicId) || [];
    return stages.length > 0 ? stages[stages.length - 1] : null;
}
async;
validateGateCriteria(epicId, string, targetState, PortfolioKanbanState, evidence ?  : Record);
Promise < {
    canProgress: boolean,
