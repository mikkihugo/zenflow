...
export class ValueStreamMapper extends EventBus {
    ...
    optimizationTimer;
    valueTrackingTimer;
    constructor(_eventBus, memory, multilevelOrchestrator, portfolioOrchestrator, programOrchestrator, swarmOrchestrator, config = {}) {
        super();
    ...
    async initialize() {
        ...
        try {
            ...
            const { ValueStreamMappingService } = await import('../services/value-stream/value-stream-mapping-service', ');
            this.valueStreamMappingService = new ValueStreamMappingService(this.logger);
    ...
    async mapWorkflowsToValueStreams() {
        ...
        try {
            this.logger.info('Mapping workflows to value streams via service delegation', ');
            // Delegate to specialized service
            const valueStreams = await this.valueStreamMappingService.mapWorkflowsToValueStreams(this.multilevelOrchestrator);
            // Update local state
            this.state.valueStreams = valueStreams;
            this.performanceTracker.endTimer('map_workflows_to_value_streams');
            ';
            this.telemetryManager.recordCounter('workflows_mapped', valueStreams.size);
            this.logger.info('Workflow to value stream mapping completed', { ': totalValueStreams, valueStreams, : .size,
            });
    ...
    async createValueStreamFromWorkflow(workflowId, context) {
        if (!this.initialized)
            await this.initialize();
        const _timer = this.performanceTracker.startTimer('create_value_stream_from_workflow', ');
        try {
            // Delegate to specialized service
            const valueStream = await this.valueStreamMappingService.createValueStreamFromWorkflow(workflowId, context);
            // Update local state
            ...
            ';
            this.emit('value-stream-created', ', workflowId, valueStreamId, valueStream.id);
            return valueStream;
        }
        catch (error) {
            this.performanceTracker.endTimer('create_value_stream_from_workflow');
     ...
     */
    async getMappingInsights(workflowId) {
        if (!this.initialized)
            await this.initialize();
        try {
            return await this.valueStreamMappingService.getMappingInsights(workflowId);
        }
        catch (error) {
    ...
    async identifyValueDeliveryBottlenecks() {
        ...
        try {
            ...
            ';
            this.telemetryManager.recordCounter('bottlenecks_identified', Array.from(allBottlenecks.values()).flat().length);
