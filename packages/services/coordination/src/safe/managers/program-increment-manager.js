...
export class ProgramIncrementManager extends EventBus {
    ...
    piCompletionService;
    initialized = false;
    constructor(eventBus, memory, logger, config = {}) {
        super();
        this.logger = logger;
    ...
    async initialize() {
        if (this.initialized)
            return;
        this.logger.info('Initializing Program Increment Manager', { ': config, this: .config,
        });
        try {
    ...
    async planProgramIncrement(artId, businessContext, architecturalVision, teamCapacities) {
        ...
        try {
            ...
            const planningEvent = await this.getPIPlanningService().createPIPlanningEvent(artId, businessContext, architecturalVision, teamCapacities.map((tc) => ({
                userId: tc.teamId,
                name: tc.teamId,
                role: 'team-lead',
                required: true,
            ...
            const programIncrement = await this.createProgramIncrement(artId, planningResult, teamCapacities);
            ...
            const _capacityResult = await this.getCapacityPlanningService().implementCapacityPlanning(teamCapacities, [] // Features will be generated from objectives
            );
            ...
            ';
            this.logger.info('PI Planning completed successfully', ', piId, completePIPlan.id, objectiveCount, piObjectives.length, featureCount, features.length, capacityUtilization, capacityResult.allocatedCapacity / capacityResult.totalCapacity);
     ...
     */
    async implementCapacityPlanning(teamCapacities, _piObjectives, features) {
        if (!this.initialized)
            await this.initialize();
        ...
        return await this.getCapacityPlanningService().implementCapacityPlanning(teamCapacities, allocationRequests);
    }
    // ============================================================================
  ...
  async trackPIProgress(piId: string): Promise<any> {
    ...
    if (!pi) {
      throw new Error(`;
        ...
        this.logger.debug('PI progress updated', { ': piId,
...
;
;
generatePIObjectives(piId, string, businessContext, any, teamCapacities, TeamCapacity[]);
PIObjective[];
...
identifyPIDependencies(features, Feature[], objectives, PIObjective[]);
Dependency[];
return [];
assessPIRisks(pi, ProgramIncrement, features, Feature[], deps, Dependency[]);
Risk[];
return [];
async;
updateAllPIMetrics();
