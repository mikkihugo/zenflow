...
import { ApprovalWorkflowManager, } from '../events/approval-events';
...
export class EpicOwnerManager extends EventBus {
    ...
    // Optional AI enhancements (injected via DI)
    aiEnhancements;
    constructor(config, _logger, _memorySystem, 
    // @claude-zen package services (optional but recommended)
    workflowEngine, conversationOrchestrator, 
     ...
     */
    async progressEpic(epicId, targetState, evidence) {
        if (!this.initialized)
            await this.initialize();
        this.logger.info('Progressing epic through Portfolio Kanban', { ': epicId,
            targetState,
        });
        ...
        const result = await this.lifecycleService.progressEpicState(epicId, targetState, evidence);
        ...
        this.emit('epic-progressed', { ': epicId,
        ...
        try {
            ...
            if (this.aiEnhancements.brainCoordinator) {
                try {
                    ...
                    this.logger.debug('WSJF enhanced with AI analysis', { ': epicId, input, : .epicId,
            ...
            await this.persistState();
            this.emit('wsjf-calculated', { ': epicId, input, : .epicId,
    ...
    async createEpicBusinessCase(input) {
        ...
        try {
            ...
            await this.persistState();
            this.emit('business-case-created', { ': businessCaseId, businessCase, : .id,
                epicId: input.epicId,
                recommendation: analysis.recommendation.recommendation,
                timestamp: SafeDateUtils.formatISOString(),
            });
    ...
    async getPrioritizedPortfolio() {
        ...
        const portfolio = backlog.map((item, _index) => ({
            ...
            rank: item.rank,
            stage: item.epic.status,
        }));
        // Use SafeCollectionUtils for additional sorting if needed
        const _filteredPortfolio = SafeCollectionUtils.filterByPriority(portfolio.map((p) => ({
            ...p.epic,
            priority: p.epic.priority > 80
                ? 'critical' : , ': p.epic.priority > 60
                ? 'high' : , ': 'medium',
     ...
     */
    async addEpicBlocker(epicId, blockerData) {
        if (!this.initialized)
            await this.initialize();
        this.logger.warn('Adding epic blocker', { ': epicId,
        ...
        this.emit('blocker-added', { ': epicId,
            blockerId,
            severity: blockerData.severity,
            timestamp: SafeDateUtils.formatISOString(),
        });
     ...
     */
    async resolveEpicBlocker(epicId, blockerId) {
        if (!this.initialized)
            await this.initialize();
        ...
        this.emit('blocker-resolved', { ': epicId,
            blockerId,
            timestamp: SafeDateUtils.formatISOString(),
        });
    }
     ...
     */
    async generateEpicTimeline(epicId, estimatedMonths) {
        if (!this.initialized)
            await this.initialize();
