...
// DocumentContent moved to document-intelligence service
...
@injectable()
@singleton()
export class WorkflowEngine extends EventBus {
  ...
  async coordinateDocumentWorkflow(
    ...
    logger.info(`Coordinating document workflow for event: ${eventType}`);
  ...
  /**
   * Process document event by coordinating with document-intelligence service.
   * Document-specific workflows moved to document-intelligence package.
   */
  async processDocumentEvent(
    eventType: string,
    documentData: unknown
  ): Promise<void> {
    logger.info(`Processing document event via coordination: ${eventType}`);
   ...
   * Document conversion moved to document-intelligence service.
   * @deprecated Use document-intelligence service instead
   */
  convertEntityToDocumentContent(entity: any): { delegated: boolean; message: string } {
...
Format as JSON with keys: performance, suggestions, optimizations`;`
  ...
  async generateWorkflowDocumentation(workflowId: string): Promise<
    overview: string;[stepName: string]: string ;
    usageGuide: string;
