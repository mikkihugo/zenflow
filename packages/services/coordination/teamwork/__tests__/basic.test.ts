...
describe('Teamwork Basic Integration - Hybrid Brain', () => {
  ...
  describe('🎯 Core Functionality', () => {
    ')    it('should create orchestrator with brain coordination', () => ')      expect(orchestrator).toBeDefined();
      expect(typeof orchestrator.createConversation).toBe('function');')      expect(typeof orchestrator.joinConversation).toBe('function');')      expect(typeof orchestrator.sendMessage).toBe('function');')      expect(typeof orchestrator.terminateConversation).toBe('function');');

    it('should create a conversation successfully', async () => {
    ')      const config:ConversationConfig = {
        title: 'Test Conversation',        pattern: 'problem-solving',        context:{
      ...
      expect(session.title).toBe('Test Conversation');')      expect(session.participants).toHaveLength(2);
      expect(session.status).toBe('active');')      expect(session.participants[0]?.id).toBe('agent-1');')      expect(session.participants[1]?.id).toBe('agent-2');')});

    it('should handle brain coordination initialization without errors', () => {
    ')      // Test that the brain coordinator is initialized properly
      // This verifies our hybrid brain system doesn't break the orchestrator')      const sessions = orchestrator.getActiveSessions();
  ...
  describe('🧠 Hybrid Brain Integration', () => {
    ...
    ')      const config:ConversationConfig = {
        title: 'Brain Test Conversation',        pattern: 'code-review',        context:{
