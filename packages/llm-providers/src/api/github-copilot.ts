...
import type { APIProvider, APIRequest, APIResponse, APIResult, APIError, APIProviderCapabilities } from '../types/api-providers';
import { API_ERROR_CODES } from '../types/api-providers';
import { Result, ok, err } from '@claude-zen/foundation';
import { githubCopilotDB, initializeGitHubCopilotDB } from './github-copilot-db';
...
function loadCopilotToken(): string {
  try {
    ...
    if (fs.existsSync(configPath)) {
      ...
      logger.info('✅ Loaded GitHub Copilot OAuth token from ~/.claude-zen/copilot-token.json');
  ...
  if (token && !token.startsWith('gho_')) {
    logger.warn('⚠️ Token may not be a valid GitHub Copilot OAuth token. Expected format: gho_xxx');
  }
  return token;
}
...
export class GitHubCopilotAPI implements APIProvider {
  ...
  async execute(request: APIRequest): Promise<APIResult> {
    try {
      logger.info(`Executing GitHub Copilot API request with model: ${this.options.model}`);
      ...
      if (!response.ok) {
        ...
        logger.error(`GitHub Copilot API error: ${response.status} ${response.statusText} - ${errorText}`);
      ...
      logger.info(`GitHub Copilot API response received: ${content.length} characters`);
  ...
  getCapabilities(): APIProviderCapabilities {
    ...
    const maxContext = Math.max(...chatModels.map(m => m.contextWindow), 128000);
    const maxOutput = Math.max(...chatModels.map(m => m.maxOutputTokens), 16384);
  ...
  async listModels(): Promise<string[]> {
    try {
      ...
      logger.info(`🎯 Primary models: ${primaryModels.map(m => m.id).join(', ')}`);
      logger.info(`🖼️ Vision models: ${stats.vision}`);
      logger.info(`📊 By category: versatile:${stats.byCategory.versatile}, lightweight:${stats.byCategory.lightweight}, powerful:${stats.byCategory.powerful}`);
      logger.info(`🔄 Last updated: ${stats.lastUpdate.toISOString()}`);
      
      return modelIds;
      
    } catch (error) {
      logger.error('Failed to list GitHub Copilot models from database:', error);
      ...
      logger.info(`📋 Using emergency fallback: ${emergencyModels.length} models (high limits)`);
 ...
 */
export function createGitHubCopilotProvider(options?: Partial<GitHubCopilotOptions>): GitHubCopilotAPI {
...
export async function executeGitHubCopilotTask(
  prompt: string,
  options: { token: string; model?: 'gpt-4' | 'gpt-4-turbo' | 'gpt-3.5-turbo' | 'claude-3-sonnet' } = { token: '' }
): Promise<string> {
