...
export class GitHubModelsAPI implements APIProvider {
  ...
  async execute(request: APIRequest): Promise<APIResult> {
    try {
      ...
      const response = await fetch(`${this.options.baseURL}/inference/chat/completions`, {
        method: 'POST',
        headers: {
  ...
  async listModels(): Promise<string[]> {
    try {
      ...
      logger.info(`📊 Categories: low:${stats.byCategory.low}, medium:${stats.byCategory.medium}, high:${stats.byCategory.high}`);
      ...
      logger.info(`📋 Using emergency fallback: ${emergencyModels.length} models (all 8k/4k limits)`);
...
export const gitHubModelsConfig = {
  ...
  modelFormat: 'provider/model-name (e.g., openai/gpt-4.1, anthropic/claude-3-5-sonnet)'
};