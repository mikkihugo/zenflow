...
import { githubCopilotDB, initializeGitHubCopilotDB } from './api/github-copilot-db';
...
async function testGitHubCopilotDatabase() {
  try {
    ...
    const lightweightModels = githubCopilotDB.getModelsByCategory('lightweight');
    ...
    if (visionModels.length > 0) {
      visionModels.forEach(model => {
        ...
        logger.info(`   - ${model.id}: ${limits?.maxImages} images, ${limits?.supportedFormats.join(', ')}`);
      });
    }
    ...
    if (gpt5) {
      ...
      logger.info(`   Structured outputs: ${gpt5.supportsStructuredOutputs ? '✅' : '❌'}`);
    }
    
    // Test Claude models with thinking budget
    const claudeThinking = githubCopilotDB.getModel('claude-3.7-sonnet-thought');
    if (claudeThinking) {
      logger.info(`🤔 Claude 3.7 Sonnet Thinking:`);
      logger.info(`   Context: ${claudeThinking.contextWindow.toLocaleString()} tokens`);
      logger.info(`   Output: ${claudeThinking.maxOutputTokens.toLocaleString()} tokens`);
      if (claudeThinking.thinkingBudget) {
        logger.info(`   Thinking budget: ${claudeThinking.thinkingBudget.min}-${claudeThinking.thinkingBudget.max} tokens`);
      }
    }
    ...
    logger.info('📋 Much higher limits than GitHub Models (128k-200k context, 16k-100k output)');
