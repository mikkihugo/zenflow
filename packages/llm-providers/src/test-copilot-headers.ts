...
async function testCopilotWithHeaders() {
  try {
    logger.info('🚀 Testing GitHub Copilot API with Copilot-Integration-Id header...');
    
    // Create GitHub Copilot API provider with gpt-4.1
    const copilot = await createAPIProvider('github-copilot-api', { model: 'gpt-4.1' });
    ...
    if (result.isOk()) {
      ...
      logger.info(`📊 Metadata: ${JSON.stringify(result.value.metadata, null, 2)}`);
    } else {
