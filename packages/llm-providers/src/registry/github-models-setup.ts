...
import { ModelRegistryService, createModelRegistryService } from './model-registry';
...
export class GitHubModelsContainer {
  ...
  async initialize(): Promise<ModelRegistryService> {
    ...
    if (this.providers.length === 0) {
      throw new Error('No GitHub providers could be initialized. Check your configuration and tokens.');
    }

    // Create registry service
    this.registryService = createModelRegistryService(this.providers, this.config.registry);
    
    // Initialize the service
    await this.registryService.initialize();

    logger.info(`✅ GitHub Models container initialized with ${this.providers.length} providers`);
...
export async function setupGitHubModelsDefault(): Promise<ModelRegistryService> {
  ...
  try {
    ...
    logger.info(`   Providers: ${Object.keys(summary.providerCounts).join(', ')}`);
    logger.info(`   Capabilities: ${summary.capabilities.slice(0, 5).join(', ')}${summary.capabilities.length > 5 ? '...' : ''}`);
