  ...
  describe('Real Provider Calls', () => {
    itIntegration('should handle role-specific tasks', async () => {
      ...
      const codeResponse = await llmProvider.execute({
        messages: [{ role: 'user', content: 'Write a simple hello world function in TypeScript' }]
      });
      ...
      expect(analysisResponse.content.toLowerCase()).toMatch(/(mean|average|sum)/);
...
describe('LLM Provider - Edge Cases', () => {
  ...
  it('should handle rapid role switching', () => {
    const roles: string[] = ['coder', 'analyst', 'researcher', 'architect', 'coordinator'];
