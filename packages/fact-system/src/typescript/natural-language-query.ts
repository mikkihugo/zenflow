...
export class NaturalLanguageQuery {
   ...
   */
  private async convertToStructuredQueries(parsed: ParsedQuery, originalQuery: string): Promise<FactSearchQuery[]> {
   ...
   */
  private generateExplanation(parsed: ParsedQuery, queries: FactSearchQuery[]): string {
    const parts: string[] = [];
    ...
    if (parsed.entities.repoOwners?.length && parsed.entities.repoNames?.length) {
      const repos = parsed.entities.repoOwners.map((owner, i) => 
        `${owner}/${parsed.entities.repoNames![i]}`
      );
      parts.push(`🔗 GitHub repositories: ${repos.join(', ')}`);
    }

    if (parsed.entities.cveIds?.length) {
      parts.push(`🔒 Security advisories: ${parsed.entities.cveIds.join(', ')}`);
    }
    ...
    parts.push(`🔍 Executing ${queries.length} targeted queries to gather comprehensive information`);
   ...
   */
  private calculateConfidence(query: string, intent: QueryIntent, entities: ParsedQuery['entities']): number {
    ...
    const entityCount = Object.values(entities).reduce((count, entityArray) => 
      count + (entityArray?.length || 0), 0
    );
