...
impl<T: Float> DataValidator<T> {
  ...
  pub fn validate_series(&self, data: &TimeSeriesData<T>) -> ValidationReport {
    ...
    if self.config.check_temporal_ordering
      || self.config.check_duplicate_timestamps
    {
  ...
  pub fn validate_dataset(
    ...
    if !series_lengths.is_empty() {
      combined_report.summary.avg_series_length =
        series_lengths.iter().sum::<usize>() as f64
          / series_lengths.len() as f64;
    ...
    if self.config.check_missing_values {
      ...
      for (i, point) in data.data_points.iter().enumerate() {
        if point.value.is_nan() || point.value.is_infinite() {
          missing_count += 1;
          if missing_count <= 5 {
            // Report first 5 missing values
            report.add_error(ValidationError::MissingData {
              field: format!("value[{}]", i),
              details: format!(
                "Invalid value at timestamp {}",
                point.timestamp
              ),
            });
          }
      ...
      if missing_count > 0 {
        ...
        if missing_count > 5 {
          report.add_warning(ValidationWarning::DataQuality {
            ...
            details: format!(
              "Total {} missing/invalid values detected",
              missing_count
            ),
          });
        }
  ...
  fn validate_outliers(
    ...
    let outlier_indices = match self.config.outlier_method {
      ...
      OutlierMethod::ModifiedZScore => {
        self.detect_outliers_modified_zscore(&valid_values)
      }
      OutlierMethod::IsolationForest => {
        self.detect_outliers_isolation_forest(&valid_values)
      }
    };
      ...
      if outlier_indices.len() <= 5 {
        for &idx in &outlier_indices {
          report.add_warning(ValidationWarning::SuspiciousPattern {
            ...
            details: format!(
              "Potential outlier at index {}: {:.6}",
              idx,
              valid_values[idx].to_f64().unwrap_or(0.0)
            ),
          });
        }
      } else {
    ...
    if let Some(ref expected_freq) = self.config.expected_frequency {
      ...
      if irregular_count > 3 {
        report.add_warning(ValidationWarning::DataQuality {
          field: "frequency".to_string(),
          details: format!(
            "Total {} irregular intervals detected",
            irregular_count
          ),
        });
      }
  ...
  fn validate_stationarity(
    ...
    let mean1 = first_half.iter().fold(T::zero(), |acc, &x| acc + x)
      / T::from(first_half.len()).unwrap();
    let mean2 = second_half.iter().fold(T::zero(), |acc, &x| acc + x)
      / T::from(second_half.len()).unwrap();
  ...
  fn validate_cross_series_consistency(
    &self,
    dataset: &TimeSeriesDataset<T>,
    report: &mut ValidationReport,
  ) {
...
pub fn validate_time_series_dataset<T: Float>(
  dataset: &TimeSeriesDataset<T>,
) -> ValidationReport {
  DataValidator::new().validate_dataset(dataset)
}

pub fn quick_data_quality_check<T: Float>(
  data: &TimeSeriesData<T>,
) -> Result<()> {
...
mod tests {
  ...
  fn test_empty_series_validation() {
    ...
    assert!(report
      .errors
      .iter()
      .any(|e| matches!(e, ValidationError::MissingData { .. })));
  }
  ...
  fn test_duplicate_timestamps() {
    ...
    assert!(report
      .errors
      .iter()
      .any(|e| matches!(e, ValidationError::DuplicateData { .. })));
  ...
  fn test_out_of_order_timestamps() {
    ...
    assert!(report
      .errors
      .iter()
      .any(|e| matches!(e, ValidationError::TemporalError { .. })));
  ...
  fn test_outlier_detection() {
    ...
    let validator =
      DataValidator::new().with_outlier_method(OutlierMethod::ZScore);
    let report = validator.validate_series(&data);
