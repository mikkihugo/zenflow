...
impl<T: Float> Scaler<T> for StandardScaler<T> {
  ...
  fn is_fitted(&self) -> bool {
    (!self.with_mean || self.mean.is_some())
      && (!self.with_std || self.std.is_some())
  }
}
  ...
  fn fit(&mut self, data: &[T]) -> Result<()> {
    ...
    match self.output_distribution {
      QuantileDistribution::Uniform => {
        for i in 0..self.n_quantiles {
          let ref_val =
            T::from(i as f64 / (self.n_quantiles - 1) as f64).unwrap();
  ...
  fn transform(&self, data: &[T]) -> Result<Vec<T>> {
    ...
    for &value in data {
      ...
      let transformed_value = if quantiles[lower_idx] == quantiles[upper_idx] {
        ...
        let weight = (value - quantiles[lower_idx])
          / (quantiles[upper_idx] - quantiles[lower_idx]);
        references[lower_idx] * (T::one() - weight)
          + references[upper_idx] * weight
      };
  ...
  fn inverse_transform(&self, data: &[T]) -> Result<Vec<T>> {
    ...
    for &value in data {
      ...
      let original_value = if references[lower_idx] == references[upper_idx] {
        ...
        let weight = (value - references[lower_idx])
          / (references[upper_idx] - references[lower_idx]);
        quantiles[lower_idx] * (T::one() - weight)
          + quantiles[upper_idx] * weight
      };
...
impl<T: Float> DataTransform<T> for LogTransformer<T> {
  fn transform(&self, data: &TimeSeriesData<T>) -> Result<TimeSeriesData<T>> {
    ...
    let mut transformed = data.clone();
    for (point, &new_value) in transformed
      .data_points
      .iter_mut()
  ...
  fn find_optimal_lambda(&self, data: &[T]) -> Result<T> {
    ...
    for &lambda_f64 in lambdas.iter() {
      ...
      let log_likelihood = -n / T::from(2.0).unwrap() * variance.ln()
        + (lambda - T::one())
          * data.iter().fold(T::zero(), |acc, &x| acc + x.ln());
...
impl<T: Float> DataTransform<T> for BoxCoxTransformer<T> {
  fn transform(&self, data: &TimeSeriesData<T>) -> Result<TimeSeriesData<T>> {
    ...
    for &value in &values {
      if value <= T::zero() {
        return Err(DataPipelineError::InvalidFormat {
          message: "Box-Cox transformation requires positive values"
            .to_string(),
        });
      }
    }

    let transformed_values: Vec<T> =
      values.iter().map(|&x| Self::box_cox(x, lambda)).collect();
...
mod tests {
  ...
  fn test_standard_scaler() {
    ...
    let variance: f64 =
      scaled.iter().map(|&x| x * x).sum::<f64>() / scaled.len() as f64;
  ...
  fn test_log_transformer() {
    ...
    let recovered_values =
      transformer.inverse_transform_values(&log_values).unwrap();
