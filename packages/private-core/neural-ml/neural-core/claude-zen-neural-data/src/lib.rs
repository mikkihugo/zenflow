...
pub mod prelude {
  pub use crate::{
    augmentation::*, crossval::*, features::*, loaders::*, preprocessing::*,
    transforms::*, validation::*, DataPoint, TimeSeriesData, TimeSeriesDataset,
  };
  pub use chrono::{DateTime, Utc};
...
impl<T: Float + fmt::Display> fmt::Display for TimeSeriesData<T> {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    write!(
      f,
      "TimeSeriesData(id={}, length={}, frequency={})",
      self.series_id,
      self.len(),
      self.frequency
    )
...
impl<T: Float + fmt::Display> fmt::Display for TimeSeriesDataset<T> {
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    write!(
      f,
      "TimeSeriesDataset(series_count={}, total_points={})",
      self.len(),
      self.total_points()
    )
  }
