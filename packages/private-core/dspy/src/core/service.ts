...
// Foundation imports limited to core utilities only - dspy uses bridge pattern
import {
  getLogger,
  type Logger,
  getConfig,
...
export class DSPyService {
  ...
  /**
   * Shutdown DSPy service and clean up resources
   */
  async shutdown(): Promise<void> {
    this.logger.info('Shutting down DSPy service');
    ...
    try {
      ...
      this.logger.info('DSPy service shutdown completed successfully');
    } catch (error) {
      this.logger.error('Error during DSPy service shutdown:', error);
      throw error;
    }
  }
}
