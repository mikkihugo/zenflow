...
Description: ${task.description}
    ...
    getDomainSpecificBonus(history, domain) {
        ...
        if (domainPerformance) {
            return ((domainPerformance.accuracy.mean + domainPerformance.robustness) / 2 -
                0.5);
        }
        return 0;
    }
    ...
    makeFinalSelection(evaluations, mlRecommendation, task) {
        // Sort by score
        const sortedEvaluations = Array.from(evaluations.entries()).sort(([, a], [, b]) => b - a);
        ...
        const mlAlignment = mlRecommendation.recommendedTeleprompter.includes(selectedVariant.algorithm)
            ? 0.2
            : 0;
