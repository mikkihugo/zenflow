...
export class GEPAML extends Teleprompter {
	 ...
	 */
	async performEvolution(student, _teacher, trainset, valset) {
	...
	async evaluateModulePerformance(_student, parameters, _trainset, _valset) {
	...
	calculateFitnessVariance() {
		...
		const variance =
			population.reduce(
				(sum, ind) => sum + (ind.fitness - meanFitness) ** 2,
				0,
			) / population.length;
		return variance;
	}
	...
	async calculatePopulationStats() {
		...
		const fitnessVariance =
			fitnesses.reduce((sum, f) => sum + (f - averageFitness) ** 2, 0) /
			fitnesses.length;
