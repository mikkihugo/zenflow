...
export class DspyGEPAResult {
    ...
    get best_idx() {
        ...
        return scores.reduce((maxIdx, score, idx) => (score > scores[maxIdx] ? idx : maxIdx), 0);
    }
...
export class GEPA extends Teleprompter {
    ...
    constructor(config) {
        ...
        if (budgetOptions !== 1) {
            throw new Error(`Exactly one of max_metric_calls, max_full_evals, auto must be set. ` +
        ...
        if (!config.reflection_lm) {
            throw new Error('GEPA requires a reflection language model to be provided. ' +
     ...
     */
    auto_budget(num_preds, num_candidates, valset_size, minibatch_size = 35, full_eval_steps = 5) {
        const num_trials = Math.max(2 * (num_preds * 2) * Math.log2(num_candidates), 1.5 * num_candidates);
    ...
    async compile(student, config) {
        ...
        if (this.auto !== null && this.auto !== undefined) {
            this.max_metric_calls = this.auto_budget(student.predictors().length, AUTO_RUN_SETTINGS[this.auto]['n'], valset ? valset.length : trainset.length);
        }
        else if (this.max_full_evals !== null &&
        ...
        console.log(`Running GEPA for approx ${this.max_metric_calls} metric calls of the program. ` +
            `This amounts to ${(this.max_metric_calls /
                (valset === null ? trainset.length : trainset.length + valset.length)).toFixed(2)} full evals on the ${valset === null ? 'train' : 'train+val'} set.`);
        const actualValset = valset || trainset;
        console.log(`Using ${actualValset.length} examples for tracking Pareto scores. ` +
    ...
    createFeedbackFunction(pred_name, predictor) {
        return (predictor_output, predictor_inputs, module_inputs, module_outputs, captured_trace) => {
            const trace_for_pred = [
                [predictor, predictor_inputs, predictor_output],
            ];
            const result = this.metric_fn(module_inputs, module_outputs, captured_trace, pred_name, trace_for_pred);
    ...
    async optimize(config) {
        ...
        const seed_score = seed_results.reduce((sum, r) => sum + r.score, 0) /
            seed_results.length;
        ...
        // Evolution loop
        while (metric_calls < config.max_metric_calls &&
            generation < max_generations) {
            generation++;
            console.log(`\n🔄 Generation ${generation}`);
            // Generate new candidates through reflection
            const new_candidates = await this.generateCandidates(candidates, config.trainset, config.valset, config.adapter, config.reflection_lm, config.reflection_minibatch_size);
            // Evaluate new candidates
            for (const new_candidate of new_candidates) {
                if (metric_calls >= config.max_metric_calls)
                    break;
        ...
        // Find best candidate
        const best_idx = val_aggregate_scores.reduce((maxIdx, score, idx) => score > val_aggregate_scores[maxIdx] ? idx : maxIdx, 0);
     ...
     */
    async generateCandidates(existing_candidates, trainset, valset, adapter, reflection_lm, minibatch_size) {
    ...
    isDuplicate(candidate, existing) {
        return existing.some((existing_candidate) => JSON.stringify(candidate) === JSON.stringify(existing_candidate));
    }
    /**
    ...
    createSeededRNG(seed) {
        ...
        return {
            ...
            choice: (array) => {
                return array[Math.floor(((state = (state * 1103515245 + 12345) & 0x7fffffff) / 0x7fffffff) *
                    array.length)];
            },
        };
    }
