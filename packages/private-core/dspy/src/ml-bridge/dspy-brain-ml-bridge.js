...
export class DSPyBrainMLBridge extends TypedEventBase {
    ...
    estimateDataSize(description) {
        ...
        return largeSizeKeywords.some((keyword) => description.toLowerCase().includes(keyword))
            ? 'large'
            : 'medium';
    }
    estimateAccuracyRequirement(description) {
        ...
        return highAccuracyKeywords.some((keyword) => description.toLowerCase().includes(keyword))
            ? 'high'
            : 'medium';
    }
    estimateSpeedRequirement(description) {
        const speedKeywords = ['fast', 'quick', 'real-time', 'immediate', 'urgent'];
        return speedKeywords.some((keyword) => description.toLowerCase().includes(keyword))
            ? 'high'
            : 'medium';
    }
