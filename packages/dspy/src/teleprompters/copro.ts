...
export class COPRO extends Teleprompter {
  ...
  private results_best: Record<string, {
    depth: number[];
    max: number[];
    average: number[];
    min: number[];
  ...
  private results_latest: Record<string, {
    depth: number[];
    max: number[];
    average: number[];
    min: number[];
  ...
  async compile(
    ...
    const evaluated_candidates: Record<string, Record<string, CoproCandidate>> = {};
   ...
   */
  private _createEvaluate(trainset: Example[], eval_kwargs: Record<string, any>) {
    return async (module: DSPyModule, dataset: Example[]): Promise<number> => {
      ...
      for (const example of dataset) {
        try {
          ...
          const numeric_score = typeof score === 'boolean' ? (score ? 1 : 0) : score as number;
   ...
   */
  private async _generateBasicInstructions(basic_instruction: string): Promise<InstructionCompletions> {
   ...
   */
  private async _generateInstructionsFromAttempts(attempts: string[]): Promise<InstructionCompletions> {
   ...
   */
  private _buildAttemptHistory(evaluated_candidates: Record<string, CoproCandidate>): string[] {
   ...
   */
  private _getBestCandidate(candidates: Record<string, CoproCandidate>): CoproCandidate {
   ...
   */
  private _checkCandidatesEqual(candidate1: CoproCandidate, candidate2: CoproCandidate): boolean {
   ...
   */
  private _updateLatestStats(predictorId: string, depth: number, latest_scores: number[]): void {
   ...
   */
  private _updateBestStats(predictorId: string, evaluated_candidates: Record<string, CoproCandidate>): void {
    const best_predictors = Object.values(evaluated_candidates);
    best_predictors.sort((a, b) => b.score - a.score);
  ...
  private _updateSignature(predictor: any, instruction: string, prefix: string): void {
    const signature = this._getSignature(predictor);
    const fields = signature.fields || {};
    const fieldKeys = Object.keys(fields);
