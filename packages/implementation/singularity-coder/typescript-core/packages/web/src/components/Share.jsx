...
var part_1 = require("./share/part");
var share_module_css_1 = require("./share.module.css");
...
function Share(props) {
    ...
    var _e = (0, solid_js_1.createSignal)(false), showScrollButton = _e[0], setShowScrollButton = _e[1];
    var _f = (0, solid_js_1.createSignal)(false), isButtonHovered = _f[0], setIsButtonHovered = _f[1];
    var _g = (0, solid_js_1.createSignal)(false), isNearBottom = _g[0], setIsNearBottom = _g[1];
    var _h = (0, store_1.createStore)({ info: props.info, messages: props.messages }), store = _h[0], setStore = _h[1];
    var messages = (0, solid_js_1.createMemo)(function () {
        return Object.values(store.messages).toSorted(function (a, b) { var _a; return (_a = a.id) === null || _a === void 0 ? void 0 : _a.localeCompare(b.id); });
    });
    var _j = (0, solid_js_1.createSignal)(["disconnected", "Disconnected"]), connectionStatus = _j[0], setConnectionStatus = _j[1];
    ...
    (0, solid_js_1.onMount)(function () {
        ...
        var setupWebSocket = function () {
            ...
            socket.onmessage = function (event) {
                ...
                try {
                    var d_1 = JSON.parse(event.data);
                    var _d = d_1.key.split("/"), root = _d[0], type = _d[1], splits = _d.slice(2);
                    if (root !== "session")
                        return;
                    if (type === "info") {
                    ...
                    if (type === "message") {
                        var messageID = splits[1];
                        if ("metadata" in d_1.content) {
                            d_1.content = fromV1(d_1.content);
                        }
                        d_1.content.parts =
                            (_c = (_a = d_1.content.parts) !== null && _a !== void 0 ? _a : (_b = store.messages[messageID]) === null || _b === void 0 ? void 0 : _b.parts) !== null && _c !== void 0 ? _c : [];
                        setStore("messages", messageID, (0, store_1.reconcile)(d_1.content));
                    }
                    if (type === "part") {
                        setStore("messages", d_1.content.messageID, "parts", function (arr) {
                            var index = arr.findIndex(function (x) { return x.id === d_1.content.id; });
                            if (index === -1)
                                arr.push(d_1.content);
                            if (index > -1)
                                arr[index] = d_1.content;
    ...
    function checkScrollNeed() {
        ...
        var scrolled = currentScrollY > 200; // Show after scrolling 200px
        // Only show when scrolling down, scrolled enough, and not near bottom
        var shouldShow = isScrollingDown && scrolled && !isNearBottom();
    ...
    var data = (0, solid_js_1.createMemo)(function () {
        var result = {
            rootDir: undefined,
            created: undefined,
            completed: undefined,
        ...
        for (var i = 0; i < msgs.length; i++) {
            ...
            if (msg.role === "assistant") {
                ...
                result.models["".concat(msg.providerID, " ").concat(msg.modelID)] = [
                    msg.providerID,
                    msg.modelID,
                ];
                if (msg.path.root) {
    ...
    return (<main classList={_a = {}, _a[share_module_css_1.default.root] = true, _a["not-content"] = true, _a}>
				...
				<div data-component="header-details">
					<ul data-component="header-stats">
						<li title="opencode version" data-slot="item">
							...
							<solid_js_1.Show when={(_c = store.info) === null || _c === void 0 ? void 0 : _c.version} fallback="v0.0.1">
								<span>v{(_d = store.info) === null || _d === void 0 ? void 0 : _d.version}</span>
							</solid_js_1.Show>
						</li>
						{Object.values(data().models).length > 0 ? (<solid_js_1.For each={Object.values(data().models)}>
								{function (_a) {
                var provider = _a[0], model = _a[1];
                return (<li data-slot="item">
					...
					<div data-component="header-time" title={luxon_1.DateTime.fromMillis(data().created || 0).toLocaleString(luxon_1.DateTime.DATETIME_FULL_WITH_SECONDS)}>
            ...
            var filteredParts = (0, solid_js_1.createMemo)(function () {
                return msg.parts.filter(function (x, index) {
                    if (x.type === "step-start" && index > 0)
                        return false;
                    if (x.type === "snapshot")
                        return false;
                    if (x.type === "step-finish")
                        return false;
            ...
            return (<solid_js_1.Suspense>
											<solid_js_1.For each={filteredParts()}>
												{function (part, partIndex) {
                    var last = (0, solid_js_1.createMemo)(function () {
                        return data().messages.length === msgIndex() + 1 &&
							...
							<div data-section="content">
								...
								<ul data-section="stats">
									<li>
										...
										{data().cost !== undefined ? (<span>${data().cost.toFixed(2)}</span>) : (<span data-placeholder>&mdash;</span>)}
									</li>
									<li>
									...
									<li>
										...
										{data().tokens.reasoning ? (<span>{data().tokens.reasoning}</span>) : (<span data-placeholder>&mdash;</span>)}
									</li>
								</ul>
							</div>
						</div>
			...
			<solid_js_1.Show when={showScrollButton()}>
				<button type="button" class={share_module_css_1.default["scroll-button"]} onClick={function () {
...
function fromV1(v1) {
    ...
    if (v1.role === "assistant") {
        return {
            ...
            tokens: (_e = (_d = v1.metadata.assistant) === null || _d === void 0 ? void 0 : _d.tokens) !== null && _e !== void 0 ? _e : {
                input: 0,
                output: 0,
                cache: {
                    read: 0,
            ...
            parts: v1.parts.flatMap(function (part, index) {
                var base = {
                    id: index.toString(),
                    messageID: v1.id,
                    sessionID: v1.metadata.sessionID,
                ...
                if (part.type === "text") {
                    return [
                        __assign(__assign({}, base), { type: "text", text: part.text }),
                    ];
                }
                if (part.type === "step-start") {
                    return [
                        __assign(__assign({}, base), { type: "step-start" }),
                    ];
                }
                if (part.type === "tool-invocation") {
                    return [
                        __assign(__assign({}, base), { type: "tool", callID: part.toolInvocation.toolCallId, tool: part.toolInvocation.toolName, state: (function () {
                                if (part.toolInvocation.state === "partial-call") {
                                    return {
                                        status: "pending",
                                    };
                                }
                                var _a = v1.metadata.tool[part.toolInvocation.toolCallId], title = _a.title, time = _a.time, metadata = __rest(_a, ["title", "time"]);
    ...
    if (v1.role === "user") {
        return {
            ...
            parts: v1.parts.flatMap(function (part, index) {
                ...
                if (part.type === "file") {
                    return [
                        __assign(__assign({}, base), { type: "file", mime: part.mediaType, filename: part.filename, url: part.url }),
                    ];
                }
                return [];
            }),
