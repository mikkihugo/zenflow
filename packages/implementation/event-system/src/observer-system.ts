  ...
  implements SystemObserver<SwarmEvent|MCPEvent|InterfaceEvent>
{
  ...
  removeConnection(socket: unknown): void {
  ...
  private formatEventForWebSocket(event: SystemEvent): unknown {
    return {
      id: event.id,
      type: event.type,
...
export class MetricsObserver implements SystemObserver<SystemEvent> {
  ...
  constructor(
    private metricsService?: { recordEvent(event: SystemEvent): void }
  ) {}
