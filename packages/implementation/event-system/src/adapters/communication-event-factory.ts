  ...
  implements EventManagerFactory<CommunicationEventAdapterConfig>
{
  ...
  async startAll(): Promise<void> {
    ...
    for (const [name, adapter] of this.adapters) {
      ...
      _startPromises.push(_startPromise);
    }
  ...
  async stopAll(): Promise<void> {
    ...
    for (const [name, adapter] of this.adapters) {
      ...
      _stopPromises.push(_stopPromise);
    }
  ...
  async shutdown(): Promise<void> {
    ...
    try {
      ...
      const destroyPromises = Array.from(this.adapters.entries()).map(
        async ([name, adapter]) => {
          try {
            await adapter.destroy();
          } catch (error) {
  ...
  async getCommunicationMetricsSummary(): Promise<{
    ...
    const avgLatency =
      metricsResults.size > 0 ? totalLatency / metricsResults.size : 0;
