 ...
 */
export async function createSystemEventAdapter(
  ...
  try {
    ...
    const optimizedConfig = SystemAdapterHelpers.createDefaultConfig(
      config.name,
      config
    );

    // Create system event manager configured for system events
    const adapter = new SystemEventManager(
      optimizedConfig,
      logger as any
    ) as EventManager;
    ...
    logger.error(
      `Failed to create system event adapter: ${config.name}`,
      error
    );
    throw error;
  }
