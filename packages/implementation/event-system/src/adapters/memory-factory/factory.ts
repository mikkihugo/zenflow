 ...
 */
export class MemoryEventFactory extends TypedEventBase implements EventManagerFactory<EventManagerConfig> {
  ...
  constructor(
    private readonly config: MemoryEventFactoryConfig = {},
    logger?: Logger,
  ...
  async create(config: EventManagerConfig): Promise<EventManager> {
    ...
    try {
      ...
      const optimizedConfig = MemoryFactoryHelpers.createDefaultConfig(config?.name, config);
      ...
      this.logger.error(`Failed to create memory event manager: ${config?.name}`, error);
      throw error;
    }
  }
   ...
   */
  async getMetrics(): Promise<MemoryFactoryMetrics> {
  ...
  async healthCheck(): Promise<MemoryHealthResult> {
    ...
    const memoryUsageRatio = metrics.memoryUsage.used / metrics.memoryUsage.total;
  ...
  private startMemoryMonitoring(): void {
    ...
    setInterval(() => {
      if (!MemoryFactoryHelpers.checkMemoryHealth()) {
        this.logger.warn('High memory usage detected, suggesting garbage collection');
        MemoryFactoryHelpers.suggestGarbageCollection();
      }
    }, interval);
  }
  ...
  private determineOverallHealth(
    ...
    instanceHealth: Array<{ name: string; status: string; memoryUsage: number; lastCheck: Date }>
