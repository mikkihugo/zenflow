 ...
 */
export class InterfaceEventFactory extends TypedEventBase implements EventManagerFactory<EventManagerConfig> {
  ...
  constructor(
    private readonly config: InterfaceEventFactoryConfig = {},
    logger?: Logger,
  ...
  async create(config: EventManagerConfig): Promise<EventManager> {
    ...
    try {
      ...
      const optimizedConfig = InterfaceFactoryHelpers.createDefaultConfig(config?.name, config);
      ...
      this.logger.error(`Failed to create interface event manager: ${config?.name}`, error);
      throw error;
    }
  }
   ...
   * Get factory metrics.
   */
  async getMetrics(): Promise<InterfaceFactoryMetrics> {
