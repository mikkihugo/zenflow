 ...
 */
export class SystemEventFactory extends TypedEventBase implements EventManagerFactory<EventManagerConfig> {
  ...
  constructor(
    private readonly config: SystemEventFactoryConfig = {},
    logger?: Logger,
    ...
    if (this.config.enableSystemMonitoring !== false) {
    ...
    try {
      ...
      if (successRate > 0) {
        const optimizedParams = SystemFactoryHelpers.optimizeSystemParameters(
          successRate,
          this.systemMetrics.averageSystemLoad
        );
   ...
   */
  async getMetrics(): Promise<SystemFactoryMetrics> {
    ...
    return SystemFactoryHelpers.calculateMetrics(
      this.totalCreated,
      this.totalErrors,
      this.instances.size,
      runningInstances,
  ...
  private determineOverallHealth(
    ...
    instanceHealth: Array<{ name: string; status: string; activeSystemMonitors: number; lastCheck: Date }>
