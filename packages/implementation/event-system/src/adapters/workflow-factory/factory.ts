 ...
 */
export class WorkflowEventFactory extends TypedEventBase implements EventManagerFactory<EventManagerConfig> {
  ...
  constructor(
    private readonly config: WorkflowEventFactoryConfig = {},
    logger?: Logger,
  ...
  async create(config: EventManagerConfig): Promise<EventManager> {
    ...
    try {
      ...
      const optimizedConfig = WorkflowFactoryHelpers.createDefaultConfig(config?.name, config);
      ...
      if (successRate > 0) {
        const optimizedParams = WorkflowFactoryHelpers.optimizeWorkflowParameters(
          successRate,
        ...
        optimizedConfig.processing = {
          ...optimizedConfig.processing,
          queueSize: optimizedParams.queueSize,
        };
        
        optimizedConfig.retry = {
          ...optimizedConfig.retry,
          attempts: optimizedParams.retryAttempts,
        };
        
        optimizedConfig.health = {
          ...optimizedConfig.health,
      ...
      this.logger.error(`Failed to create workflow event manager: ${config?.name}`, error);
      throw error;
    }
  }
  ...
  async remove(name: string): Promise<boolean> {
    ...
    try {
      ...
      this.logger.error(`Failed to remove workflow event manager ${name}:`, error);
      throw error;
    }
  }
   ...
   */
  async getMetrics(): Promise<WorkflowFactoryMetrics> {
    ...
    return WorkflowFactoryHelpers.calculateMetrics(
      this.totalCreated,
      this.totalErrors,
      this.instances.size,
      runningInstances,
  ...
  async healthCheck(): Promise<WorkflowHealthResult> {
    ...
    const workflowSuccessRate = WorkflowFactoryHelpers.calculateWorkflowSuccessRate(
      this.workflowMetrics.completedWorkflows,
      this.workflowMetrics.failedWorkflows
    );
  ...
  private startWorkflowMonitoring(): void {
    ...
    setInterval(() => {
      ...
      if (successRate < 0.8) {
        this.logger.warn('Workflow success rate is low, considering optimization');
      }
  ...
  private determineOverallHealth(
    ...
    instanceHealth: Array<{ name: string; status: string; activeWorkflows: number; lastCheck: Date }>
    ...
    if (metrics.workflowMetrics.averageExecutionTime > 60000) return 'unhealthy';
