...
export interface WorkflowEvent extends SystemEvent {
  type:|'javascript' | 'typescript' | 'python' | 'java' | 'csharp' | 'cpp' | 'go' | 'ruby' | 'swift' | 'kotlin'||workflow:orchestration;
  operation:|'start|complete|fail|retry|skip|branch|merge | loop'javascript' | 'typescript' | 'python' | 'java' | 'csharp' | 'cpp' | 'go' | 'ruby' | 'swift' | 'kotlin'||orchestrate;
  workflowId: string;
  taskId?: string;
  details?: {
    workflowName?: string;
