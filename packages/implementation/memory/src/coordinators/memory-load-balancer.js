...
export class MemoryLoadBalancer extends TypedEventBase {
    ...
    checkNodeOverload(node) {
        ...
        const isOverloaded = node.metrics.averageResponseTime > thresholds.maxLatency || node.status.errorRate > thresholds.maxErrorRate || node.metrics.connections > thresholds.maxConnectionsPerNode || node.metrics.memoryUsage > thresholds.maxMemoryUsage;
    ...
    updateNodeMetrics(nodeId, metrics) {
        ...
        if (node) {
            ...
            const totalLatency = Array.from(this.nodes.values()).reduce((sum, n) => sum + n.metrics.averageResponseTime, 0);
