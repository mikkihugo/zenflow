...
export class DataLifecycleManager extends TypedEventBase {
    ...
    canAccommodateInStage(stage, size) {
        if (stage === 'expired')
            return true;
        const stageConfig = this.configuration.stages[stage];
        if (!stageConfig)
            return false;
    ...
    assessMigrationNeed(entry, now) {
        ...
        const stageConfig = this.configuration.stages[entry.stage];
        if (!stageConfig)
            return false;
        ...
        try {
            await withTrace('data-lifecycle-cleanup', async () => {
                ...
                for (const [key, entry] of this.entries) {
                    const age = now - entry.createdAt;
                    const timeSinceAccess = now - entry.lastAccessed;
                    if (timeSinceAccess > this.configuration.cleanup.unusedDataThreshold || age > this.configuration.cleanup.expiredDataThreshold) {
                        this.delete(key);
                        cleaned++;
                    }
                }
