		...
		async shutdown() {
			...
			return safeAsync(async () => {
				...
				if (result.isErr()) {
					...
					logger.error(
						"Session memory store shutdown failed",
						error.toObject(),
					);
					return err(error);
				}
	...
	var MemoryManager = class {
				...
				return withRetry(
					async () => {
						return safeAsync(async () => {
							...
							if (telemetryResult.isOk()) {
								this.managerLogger.debug(
									"Memory manager telemetry initialized",
								);
							} else {
		...
		async health() {
			return withTrace("memory-manager-health", async () => {
				...
				recordHistogram(
					"memory_manager_health_duration_ms",
					timer.duration || 0,
				);
				return isHealthy;
			});
		...
		async stats() {
			return withTrace("memory-manager-stats", async () => {
				...
				recordHistogram(
					"memory_manager_stats_duration_ms",
					timer.duration || 0,
				);
