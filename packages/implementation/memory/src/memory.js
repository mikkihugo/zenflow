...
const SessionMemoryStore = (() => {
    ...
    var SessionMemoryStore = class extends _classSuper {
        ...
        constructor(options) {
            ...
            this.circuitBreaker = createCircuitBreaker(this.performStorageOperation.bind(this), {
                timeout: 5000,
            ...
            logger.debug('SessionMemoryStore constructed with comprehensive foundation integration', {
        ...
        async initialize() {
            ...
            return withTrace('memory-store-initialize', () => withRetry(async () => {
                return safeAsync(async () => {
                    logger.info('Initializing session memory store with foundation storage...', {
                        cacheEnabled: this.options.enableCache,
                        vectorEnabled: this.options.enableVectorStorage,
                    ...
                    try {
                        ...
                        const error = new MemoryConnectionError('Failed to initialize foundation storage', this.options.backendConfig.type, { originalError: storageError });
                ...
                onFailedAttempt: (error, attemptNumber) => {
                    logger.warn(`Memory initialization attempt ${attemptNumber} failed:`, error);
                    recordMetric('memory_initialization_retries', 1);
                }
            })).then(result => {
                if (result.isErr()) {
                    const error = new MemoryConnectionError('Failed to initialize session memory store', this.options.backendConfig.type, { originalError: result.error.message });
                    this.errorAggregator.add(error);
                    recordMetric('memory_initialization_failures', 1);
                    logger.error('Session memory store initialization failed', error.toObject());
                    return err(error);
                }
                return ok(undefined);
            });
        ...
        async store(sessionIdOrKey, keyOrData, dataOrOptions, options) {
            ...
            return withTrace('memory-store-operation', async () => {
                return withRetry(async () => {
                    ...
                    minTimeout: 500,
                    onFailedAttempt: (error, attemptNumber) => {
                        logger.warn(`Memory store attempt ${attemptNumber} failed for key ${storeKey}:`, error);
                ...
                if (result.isErr()) {
                    ...
                    logger.error('Memory store operation failed', { sessionId, key: storeKey, error: error.message });
                    throw error;
                }
            });
        }
        async retrieve(sessionIdOrKey, key) {
            ...
            return withTrace('memory-retrieve-operation', async () => {
                return withRetry(async () => {
                    return safeAsync(async () => {
                        ...
                        if (this.options.enableCache) {
                            ...
                            if (cached !== null) {
                                ...
                                recordHistogram('memory_retrieve_duration_ms', timer.duration || 0);
                        ...
                        const result = session?.data[actualKey] ?? null;
                        performanceTracker.endTimer('memory_retrieve');
                        ...
                        if (result !== null) {
                            ...
                            recordMetric('memory_retrieved_data_size_bytes', JSON.stringify(result).length);
                        }
                        else {
                    ...
                    minTimeout: 300,
                    onFailedAttempt: (error, attemptNumber) => {
                        logger.warn(`Memory retrieve attempt ${attemptNumber} failed for key ${retrieveKey}:`, error);
                ...
                if (result.isErr()) {
                    ...
                    logger.error('Memory retrieve operation failed', { key: retrieveKey, error: error.message });
                    throw error;
                }
                return result.value;
            });
            ...
            if (this.storage) {
                ...
                if (sessionDataStr) {
                    try {
                        ...
                        logger.error(`Failed to parse session data for ${sessionId}:`, parseError);
                    }
                }
            }
            return null;
        ...
        async delete(sessionIdOrKey, key) {
            ...
            return withTrace('memory-delete-operation', async () => {
                return withRetry(async () => {
                    ...
                    minTimeout: 300,
                    onFailedAttempt: (error, attemptNumber) => {
                        logger.warn(`Memory delete attempt ${attemptNumber} failed for key ${deleteKey}:`, error);
                ...
                if (result.isErr()) {
                    ...
                    logger.error('Memory delete operation failed', { key: deleteKey, error: error.message });
                    throw error;
                }
                return result.value;
            });
        ...
        async loadFromStorage() {
            ...
            try {
                ...
                for (const key of sessionKeys) {
                    ...
                    if (sessionDataStr) {
                        try {
                            ...
                            logger.error(`Failed to parse session data for key ${key}:`, parseError);
                        }
                    }
                }
                ...
                throw new MemoryStorageError('Failed to load sessions from storage', { originalError: error });
            }
        }
        async saveToStorage() {
            if (!this.storage)
            ...
            try {
                ...
                throw new MemoryStorageError('Failed to save sessions to storage', { originalError: error });
            }
        }
        ...
        ensureInitialized() {
            if (!(this.initialized && this.storage)) {
                const error = new MemoryError('Session memory store not initialized. Call initialize() first.', { initialized: this.initialized, hasStorage: !!this.storage });
                this.errorAggregator.add(error);
                recordMetric('memory_initialization_errors', 1);
                throw error;
            }
         ...
         */
        async performStorageOperation(params) {
            return safeAsync(async () => {
                if (!this.storage) {
                    throw new MemoryError('Storage not available for circuit breaker operation');
                }
                ...
                switch (operation) {
                    case 'store':
                        ...
                        logger.debug('Circuit breaker store operation completed', { sessionId, key });
                        return { success: true };
                    case 'retrieve':
                        ...
                        if (sessionDataStr) {
                            try {
                                ...
                                logger.debug('Circuit breaker retrieve operation completed', { sessionId });
                                return session;
                            }
                            catch (parseError) {
                                const error = new MemoryError(`Failed to parse session data for ${sessionId}`, { sessionId, parseError: ensureError(parseError).message });
                        ...
                        return null;
                    default:
                        throw new MemoryError(`Unsupported circuit breaker operation: ${operation}`);
                }
            }).then(result => {
                if (result.isErr()) {
                    const error = ensureError(result.error);
    ...
    return SessionMemoryStore = _classThis;
})();
...
const MemoryManager = (() => {
    ...
    var MemoryManager = class {
        ...
        static {
            ...
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            MemoryManager = _classThis = _classDescriptor.value;
            if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
            __runInitializers(_classThis, _classExtraInitializers);
        }
        errorAggregator = createErrorAggregator();
        managerLogger = getLogger('memory:manager');
        ...
        constructor(options) {
            ...
            this.circuitBreaker = createCircuitBreaker(this.performManagerOperation.bind(this), {
                timeout: 10000,
            ...
            this.telemetryManager = new BasicTelemetryManager({
                serviceName: 'memory-manager',
                enableTracing: true,
                enableMetrics: true
            });
            recordMetric('memory_managers_created', 1);
            this.managerLogger.debug('Memory manager initialized with comprehensive foundation integration', {
                hasCircuitBreaker: !!this.circuitBreaker,
        ...
        async initialize() {
            ...
            const timer = this.performanceTracker.startTimer('memory_manager_initialize');
                ...
                return withRetry(async () => {
                    return safeAsync(async () => {
                        ...
                        if (telemetryResult.isOk()) {
                            ...
                            this.managerLogger.warn('Failed to initialize manager telemetry:', telemetryResult.error);
                        }
                        // Initialize the store
                        const storeResult = await this.store.initialize();
                        if (storeResult.isErr()) {
                            throw new MemoryConnectionError('Failed to initialize memory store in manager', 'memory-manager', { originalError: storeResult.error });
                        }
                        ...
                        recordHistogram('memory_manager_initialization_duration_ms', initTime);
                    ...
                    minTimeout: 1500,
                    onFailedAttempt: (error, attemptNumber) => {
                        this.managerLogger.warn(`Memory manager initialization attempt ${attemptNumber} failed:`, error);
                        recordMetric('memory_manager_initialization_retries', 1);
                    }
                });
            }).then(result => {
                if (result.isErr()) {
                    const error = new MemoryConnectionError('Failed to initialize memory manager', 'memory-manager', { originalError: result.error.message });
                    this.errorAggregator.add(error);
            ...
            return withTrace('memory-manager-store', async () => {
                return withRetry(async () => {
                    return safeAsync(async () => {
                        ...
                        recordMetric('memory_manager_stored_data_size_bytes', JSON.stringify(data).length);
                    ...
                    minTimeout: 500,
                    onFailedAttempt: (error, attemptNumber) => {
                        this.managerLogger.warn(`Memory manager store attempt ${attemptNumber} failed for key ${key}:`, error);
                        recordMetric('memory_manager_store_retries', 1);
                    }
                });
            }).then(result => {
                if (result.isErr()) {
                    const error = new MemoryStorageError('Failed to store data via memory manager', { key, originalError: result.error.message });
                    this.errorAggregator.add(error);
            ...
            return withTrace('memory-manager-retrieve', async () => {
                return withRetry(async () => {
                    return safeAsync(async () => {
                        ...
                        recordHistogram('memory_manager_retrieve_duration_ms', retrieveTime);
                        if (result !== null) {
                            recordMetric('memory_manager_retrieve_successes', 1);
                            recordMetric('memory_manager_retrieved_data_size_bytes', JSON.stringify(result).length);
                        }
                        else {
                    ...
                    minTimeout: 300,
                    onFailedAttempt: (error, attemptNumber) => {
                        this.managerLogger.warn(`Memory manager retrieve attempt ${attemptNumber} failed for key ${key}:`, error);
                        recordMetric('memory_manager_retrieve_retries', 1);
                    }
                });
            }).then(result => {
                if (result.isErr()) {
                    const error = new MemoryStorageError('Failed to retrieve data via memory manager', { key, originalError: result.error.message });
                    this.errorAggregator.add(error);
        ...
        async shutdown() {
            ...
            return withTrace('memory-manager-shutdown', async () => {
                ...
                if (result.isErr()) {
                    const error = new MemoryError('Failed to shutdown memory manager', { originalError: result.error.message });
                    this.errorAggregator.add(error);
        ...
        async clear() {
            ...
            return withTrace('memory-manager-clear', async () => {
                ...
                if (result.isErr()) {
                    const error = new MemoryStorageError('Failed to clear memory store via manager', { originalError: result.error.message });
                    this.errorAggregator.add(error);
        ...
        async delete(key) {
            ...
            return withTrace('memory-manager-delete', async () => {
                ...
                if (result.isErr()) {
                    const error = new MemoryStorageError('Failed to delete data via memory manager', { key, originalError: result.error.message });
                    this.errorAggregator.add(error);
         ...
         */
        async performManagerOperation(params) {
            const { operation, key, data, options } = params;
            switch (operation) {
                case 'store':
                    if (!key)
                        throw new MemoryError('Key required for store operation');
                    await this.store.store('default', key, data, options);
                    return { success: true };
                case 'retrieve':
                    if (!key)
                        throw new MemoryError('Key required for retrieve operation');
                    return await this.store.retrieve('default', key);
                case 'delete':
                    if (!key)
                        throw new MemoryError('Key required for delete operation');
    ...
    return MemoryManager = _classThis;
})();
