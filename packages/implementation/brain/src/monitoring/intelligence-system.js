...
export class CompleteIntelligenceSystem {
    ...
    async predictTaskDurationMultiHorizon(agentId, taskType, context) {
        ...
        const longDuration = 2000 * contextComplexity * Math.pow(contextVolatility, 1.5);
        ...
        return {
            ...
            taskType,
            predictions: {
                short: { duration: shortDuration, confidence: 0.9 - (contextVolatility - 1) * 0.1 },
                medium: { duration: mediumDuration, confidence: 0.8 - (contextVolatility - 1) * 0.15 },
                long: { duration: longDuration, confidence: 0.7 - (contextVolatility - 1) * 0.2 },
            },
            timestamp: new Date(),
        };
    }
    getAgentLearningState(agentId) {
        ...
        const learningState = {
            ...
            knowledgeBase: {
                domains: agentId.type === 'researcher' ? ['research', 'analysis'] : ['coordination', 'execution'],
    ...
    getAgentHealth(agentId) {
        ...
        const typeMultiplier = agentId.type === 'coordinator' ? 0.95 : // Coordinators are more stable
            agentId.type === 'optimizer' ? 0.90 : // Optimizers work harder
                0.88; // Other types
        const instanceBonus = Math.min(0.1, agentId.instance * 0.02); // Experience bonus
        const overallHealth = Math.min(0.98, baseHealth * typeMultiplier + instanceBonus);
        // Determine status based on health score
        const status = overallHealth >= 0.85 ? 'healthy' :
            overallHealth >= 0.70 ? 'warning' :
                overallHealth >= 0.40 ? 'critical' : 'offline';
        const agentHealth = {
            agentId: agentId.id,
            status,
            ...
            issues: status !== 'healthy' ? [`${agentId.type} agent showing reduced performance`] : []
        };
