...
export class SmartPromptOptimizer {
	...
	performRegressionAnalysis(_features, similarPrompts) {
	...
	calculateOptimizationConfidence(patterns, similarPrompts) {
	...
	predictImprovementFactor(regressionInsights) {
	...
	generateOptimizationReasoning(patterns, regressionInsights) {
		const reasoning = [];
		reasoning.push(
	...
	updateOptimizationPatterns(analysisData) {
		...
		for (const pattern of this.optimizationPatterns.values()) {
			if (
				pattern.applicableContexts.some((ctx) =>
...
//# sourceMappingURL=smart-prompt-optimizer.js.map
