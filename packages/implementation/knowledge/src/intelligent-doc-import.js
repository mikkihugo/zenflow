...
export class IntelligentDocImport extends EventEmitter {
    ...
    extractFunctions(content, fileExt) {
        ...
        if (fileExt === '' || fileExt === '') {
            ...
            while ((match = functionRegex.exec(content)) !== null) {
                ...
                functions.push({ name: functionName || 'unknown', hasDocumentation: hasDocumentation || false });
            }
        }
        return functions;
    }
        ...
        if (fileExt === '' || fileExt === '') {
            ...
            while ((match = classRegex.exec(content)) !== null) {
                ...
                const hasDocumentation = lineIndex > 0 &&
                    (lines[lineIndex - 1]?.trim().includes('*/') ||
                        lines[lineIndex - 2]?.trim().includes('/**'));
                classes.push({ name: className || 'unknown', hasDocumentation: hasDocumentation || false });
            }
        }
        return classes;
    }
        ...
        if (fileExt === '') {
            ...
            while ((match = interfaceRegex.exec(content)) !== null) {
                ...
                const hasDocumentation = lineIndex > 0 &&
                    (lines[lineIndex - 1]?.trim().includes('*/') ||
                        lines[lineIndex - 2]?.trim().includes('/**'));
                interfaces.push({ name: interfaceName || 'unknown', hasDocumentation: hasDocumentation || false });
            }
        }
        return interfaces;
    }
    ...
    async generateRecommendations(analysis) {
        ...
        if (analysis.fileType === 'document' && analysis.llmAnalysis) {
            ...
            if (llmAnalysis.qualityScore >= 0.8 &&
        ...
        if (analysis.fileType === 'code' && analysis.documentationScore) {
            ...
            if (score >= 0.9) {
                ...
                confidence = 0.7;
                improvements.push(`Add documentation for ${docScore.missing.length} missing items`);
            }
            else {
                action = 'manual_review';
