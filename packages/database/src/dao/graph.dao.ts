...
export class GraphDao<T> extends BaseDao<T> implements GraphRepository<T>, DataAccessObject<T> {
  ...
  async getGraphStats(): Promise<{
    ...
    try {
      ...
      const nodeLabels = [...new Set((labelsResult.nodes as any[]).flatMap((n: any) => n.labels))];

      // Get relationship types
      const typesResult = await this.graphAdapter.queryGraph(
        'MATCH ()-[r]->() RETURN DISTINCT type(r) as relType'
      );
      const relationshipTypes = (typesResult.relationships as any[]).map((r: any) => r.type);
  ...
  protected override buildFindByIdQuery(id: string | number): { sql: string; params: unknown[] } {
    return {
