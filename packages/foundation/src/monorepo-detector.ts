...
export class WorkspaceDetector {
   ...
   */
  async detectWorkspaceRoot(startPath: string = process.cwd()): Promise<DetectedWorkspace | null> {
    try {
      ...
      logger.info(`Detected ${workspaceInfo.tool} workspace with ${workspaceInfo.totalProjects} projects`);
   ...
   */
  private async getWorkspacePackages(rootDir: string): Promise<DetectedProject[]> {
    try {
      ...
      return workspaces.map((workspace: any) => {
        ...
        return {
          name: workspace.package?.name || path.basename(workspace.location),
          path: relativePath,
          type: this.inferProjectType(relativePath, workspace.package?.name),
   ...
   */
  private detectFramework(projectDir: string, packageFile?: string): string | undefined {
   ...
   */
  private detectLanguage(_projectDir: string, packageFile?: string): string | undefined {
    if (!packageFile) return undefined;
    
    const languageMap = {
      'package.json': 'TypeScript', // Default for JS/TS projects
      'Cargo.toml': 'Rust',
      'go.mod': 'Go',
      'mix.exs': 'Elixir',
      'gleam.toml': 'Gleam',
   ...
   */
  private inferProjectType(relativePath: string, name?: string): DetectedProject['type'] {
    const pathLower = relativePath.toLowerCase();
    const nameLower = name?.toLowerCase() || '';
    
    // Path-based detection
    if (pathLower.includes('apps/') || pathLower.startsWith('apps/')) return 'app';
    if (pathLower.includes('packages/') || pathLower.startsWith('packages/')) return 'package';
    if (pathLower.includes('libs/') || pathLower.startsWith('libs/')) return 'lib';
    if (pathLower.includes('services/') || pathLower.startsWith('services/')) return 'service';
    if (pathLower.includes('tools/') || pathLower.startsWith('tools/')) return 'tool';
    if (pathLower.includes('examples/') || pathLower.startsWith('examples/')) return 'example';
    if (pathLower.includes('test') || pathLower.includes('spec')) return 'test';
    if (pathLower.includes('docs/') || pathLower.includes('documentation/')) return 'doc';
    
    // Name-based detection
    if (nameLower.includes('service')) return 'service';
    if (nameLower.includes('app')) return 'app';
    if (nameLower.includes('lib')) return 'lib';
    if (nameLower.includes('tool')) return 'tool';
    if (nameLower.includes('example')) return 'example';
    if (nameLower.includes('test')) return 'test';
    if (nameLower.includes('doc')) return 'doc';
