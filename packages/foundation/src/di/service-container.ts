 ...
 */
export interface ServiceDiscoveryOptions extends LoadModulesOptions {
...
export class ServiceContainer {
   ...
   * Resolve a service
   */
  resolve<T = any>(name: string): Result<T, ServiceContainerError> {
    try {
  ...
  async discoverServices(
    ...
    try {
      ...
      // Use Awilix's built-in module discovery
  ...
  /**
   * Integrate with existing TSyringe container for gradual migration
   */
  integrateWithTSyringe(tsyringeContainer: DependencyContainer): void {
    // This method would provide bridge functionality for gradual migration
    // Implementation would depend on specific TSyringe integration needs
    this.logger.info('TSyringe integration initialized for gradual migration');
  }
  ...
  createChild(name?: string): ServiceContainer {
    ...
    const childContainer = new ServiceContainer(childName);
    
    // Create child Awilix container
    const childAwilixContainer = this.container.createScope();
    (childContainer as any).container = childAwilixContainer;
...
// Export Awilix types and constants for external use
export { Lifetime, ResolutionMode, InjectionMode } from 'awilix';
export type { AwilixContainer, Resolver, BuildResolverOptions, LoadModulesOptions };