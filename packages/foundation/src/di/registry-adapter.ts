...
import { ServiceContainer, ServiceRegistrationOptions, ServiceInfo } from './service-container';
...
export abstract class BaseRegistryAdapter extends EventEmitter {
  ...
  constructor(options: RegistryAdapterOptions = {}) {
    ...
    this.container = new ServiceContainer(this.options.containerName);
   ...
   */
  protected emitMigrationEvent(event: string, data: any): void {
    if (this.options.enableMigrationLogging) {
...
export class AgentRegistryAdapter extends BaseRegistryAdapter {
  ...
  async queryAgents(query: {
    ...
    return agents.filter(agent => {
      ...
      if (query.namePattern) {
        ...
        if (!pattern.test(agent.name)) return false;
      }
  ...
  private extractCapabilities(capabilities: any): string[] {
    if (!capabilities) return [];
    
    const caps: string[] = [];
    if (capabilities.languages) caps.push(...capabilities.languages);
    if (capabilities.frameworks) caps.push(...capabilities.frameworks);
    if (capabilities.domains) caps.push(...capabilities.domains);
    if (capabilities.tools) caps.push(...capabilities.tools);
...
export class RegistryMigrationUtil {
  ...
  static async performanceComparison(
    ...
    testOperations: Array<() => Promise<any>>
  ): Promise<{
    oldPerformance: number;
    newPerformance: number;
    improvement: number;
  }> {
    const runTest = async (operations: Array<() => Promise<any>>): Promise<number> => {
      const start = Date.now();
      await Promise.all(operations.map(op => op()));
      return Date.now() - start;
    };

    const oldPerformance = await runTest(testOperations);
    const newPerformance = await runTest(testOperations);
