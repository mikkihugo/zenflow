 ...
 * await hybrid.executeAcrossAll(async (storageInstance) => {
 *   await storageInstance.kv.set('status', 'synced');
 * }, ['app1', 'app2']);
 * ```
 */
...
export interface KeyValueStorage {
   ...
   * const userKeys = await kvStorage.keys('user:*');
   * ```
   */
  ...
  private async getDatabaseModule(): Promise<DatabaseModule> {
    if (!this.databaseModule) {
      try {
        ...
        // Temporary comment to allow build: this.databaseModule = await import('@claude-zen/database') as DatabaseModule;
        // TODO: Restore once @claude-zen/database package is built
        logger.debug('Database module loading temporarily disabled for build');
        throw new Error('Database module loading temporarily disabled for build');
      } catch (error) {
...
/**
 * Professional storage interface for common database operations.
