...
import { getSystemStatus, getFacadeStatus, facadeStatusManager } from './facade-status-manager';
...
export async function getSystemCapabilityData(): Promise<SystemCapabilityData> {
  ...
  const facades: FacadeSummary[] = Object.entries(systemStatus.facades).map(([name, status]) => ({
    name,
    capability: status.capability,
    healthScore: status.healthScore,
    availablePackages: Object.values(status.packages)
      .filter(pkg => pkg.status === 'available' || pkg.status === 'registered').length,
 ...
 */
export async function getInstallationSuggestions(): Promise<InstallationSuggestion[]> {
  const systemStatus = getSystemStatus();
  ...
  Object.entries(systemStatus.facades).forEach(([facadeName, facade]) => {
    facade.missingPackages.forEach(packageName => {
      ...
      if (facadeName === 'infrastructure') priority = 'high';
      if (facadeName === 'intelligence' && packageName.includes('brain')) priority = 'high';
      if (facadeName === 'operations' && packageName.includes('monitoring')) priority = 'high';
 ...
 */
function getPackageFeatures(packageName: string, allFeatures: string[]): string[] {
...
export async function displaySystemStatus(): Promise<void> {
  ...
  const statusEmoji = dashboard.overall === 'full' ? '✅' : 
                     dashboard.overall === 'partial' ? '⚠️' : '❌';
  console.log(`${statusEmoji} Overall: ${dashboard.overall.toUpperCase()} (${dashboard.systemHealthScore}% health)`);
  console.log(`📦 Packages: ${dashboard.availablePackages}/${dashboard.totalPackages} available`);
  console.log(`🔧 Services: ${dashboard.registeredServices} registered in Awilix`);
  
  // Facade breakdown
  console.log('\n📊 Facade Status:');
  dashboard.facades.forEach(facade => {
    const facadeEmoji = facade.capability === 'full' ? '✅' : 
                       facade.capability === 'partial' ? '⚠️' : '❌';
    console.log(`  ${facadeEmoji} ${facade.name}: ${facade.capability} (${facade.healthScore}%)`);
...
export function startSystemMonitoring(): void {
  facadeStatusManager.on('packageStatusChanged', (packageName: string, packageInfo: any) => {
    if (packageInfo.status === 'registered') {
      logger.info(`📦 Package ${packageName} registered successfully`, {
        serviceName: packageInfo.serviceName,
        loadTime: packageInfo.loadTime
      });
    } else if (packageInfo.status === 'unavailable') {
  ...
  facadeStatusManager.on('facadeRegistered', (facadeName: string, facadeStatus: any) => {
    logger.info(`🏗️ Facade ${facadeName} registered`, {
      capability: facadeStatus.capability,
      healthScore: facadeStatus.healthScore,
      registeredServices: facadeStatus.registeredServices.length
    });
  });

  facadeStatusManager.on('systemStatusUpdated', async (systemStatus: any) => {
