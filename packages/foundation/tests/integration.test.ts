...
describe('Foundation Integration Tests', () => {
  ...
  describe('Cross-System Coordination', () => {
    itIntegration('should coordinate LLM and storage systems', async () => {
      ...
      logger.info('LLM analysis completed', { length: analysisResult.length });
    ...
    itIntegration('should handle errors across systems', async () => {
      ...
      if (result.success) {
        logger.info('Claude task succeeded', { messageCount: result.data.length });
      } else {
        logger.info('Claude task failed as expected', { error: result.error.message });
  ...
  describe('Real World Scenarios', () => {
    itIntegration('should handle swarm coordination workflow', async () => {
    ...
    itIntegration('should handle multi-role LLM workflow', async () => {
    ...
    itIntegration('should handle error recovery scenarios', async () => {
      ...
      const operations = [
        ...
        async () => {
          try {
            ...
            throw new EnhancedError('Storage not available', { originalError: error });
          }
        }
      ];
      ...
      results.forEach((result, index) => {
        if (result.status === 'fulfilled') {
          ...
          logger.warn(`Operation ${index + 1} failed`, { error: result.reason });
        }
      });
    }, 120000);
  });
  ...
  describe('Performance and Scalability', () => {
    ...
    itPerformance('should track system metrics', async () => {
