...
class DependentService {
  constructor(
    @inject('TestService') private testService: ITestService
  ) {}
...
describe('Dependency Injection System - 100% Coverage', () => {
  ...
  describe('Global Container Functions', () => {
    ...
    it('should throw on unregistered global service', () => {
      expect(() => resolveGlobal('NonExistent'))
        .toThrow(DependencyResolutionError);
    });
  });
  ...
  describe('Decorators', () => {
    ...
    it('should handle multiple decorators', () => {
      ...
      class MultiDecoratorService {
        getValue() { return 'multi'; }
      }

      const container = createContainer();
      container.register('MultiDecoratorService', MultiDecoratorService);
      
      const instance1 = container.resolve<MultiDecoratorService>('MultiDecoratorService');
      const instance2 = container.resolve<MultiDecoratorService>('MultiDecoratorService');
  ...
  describe('Error Handling', () => {
    ...
    it('should handle invalid registrations gracefully', () => {
      ...
      expect(() => container.register('', TestService))
        .toThrow();
      
      expect(() => container.register('Invalid', null as any))
        .toThrow();
    });
    ...
    it('should handle resolution with invalid tokens', () => {
      ...
      expect(() => container.resolve(null as any))
        .toThrow();
    });
  ...
  describe('Advanced Features', () => {
    it('should handle optional dependencies', () => {
      ...
      class OptionalDepService {
        constructor(@inject('optional', { optional: true }) private dep?: any) {}
        
        hasDep() { return !!this.dep; }
        getDep() { return this.dep; }
      }
      ...
      const service = container.resolve<OptionalDepService>('OptionalDepService');
  ...
  describe('Integration with Other Systems', () => {
    ...
    it('should integrate with configuration systems', () => {
      ...
      expect(dbService.getConnectionString()).toBe('postgresql://localhost:5432');
    });
  });
});