...
export function injectable<T extends Constructor>(constructor: T): T {
  ...
  const injectionTokens =
    getInjectionTokens(constructor) || new Array(paramTypes.length);
...
export function inject<T>(token: DIToken<T>) {
  return (
    ...
    const existingTokens =
      Reflect.getMetadata(INJECTION_TOKENS_KEY, target as any) || [];
    existingTokens[parameterIndex] = token;
    Reflect.defineMetadata(INJECTION_TOKENS_KEY, existingTokens, target as any);
  };
}
