...
export class FactBridge {
    ...
    async searchFacts(query) {
        if (this.useRustEngine && this.rustEngine) {
            try {
                const resultJson = await this.rustEngine.search_facts(query.query, query.factTypes || ['npm-package', 'github-repo', 'security-advisory', 'hex-package', 'rust-crate', 'go-module', 'perl-package', 'java-package', 'gitlab-repo', 'bitbucket-repo'], query.limit || 10);
    ...
    async getFact(query) {
        if (this.useRustEngine && this.rustEngine) {
            try {
                const resultJson = await this.rustEngine.get_fact(query.factType, query.identifier, query.version);
    ...
    async getAPIDocsFactsTypeScript(apiName, endpoint) {
        ...
        if (swaggerUrl) {
            try {
                ...
                if (endpoint) {
                    filteredEndpoints = endpoints.filter(ep => ep.path.includes(endpoint) || ep.description.toLowerCase().includes(endpoint.toLowerCase()));
                }
                return {
...
//# sourceMappingURL=fact-bridge.js.map