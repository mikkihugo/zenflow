...
export class LiveAPIConnector {
   ...
   */
  async fetchNPMPackage(packageName: string, version?: string): Promise<NPMFactResult> {
    const requestedVersion = version || 'latest stable';
    logger.info(`Fetching live NPM data for ${packageName}@${requestedVersion}`);
    ...
    try {
      ...
      if (!packageResponse.ok) {
        throw new Error(`NPM API error: ${packageResponse.status} ${packageResponse.statusText}`);
      }
      ...
      if (version) {
        ...
        targetVersion = packageData['dist-tags']?.latest || Object.keys(packageData.versions).pop()!;
        logger.info(`ℹ️ No version specified for ${packageName}, using latest stable: ${targetVersion}`);
      }
      ...
      return {
        ...
        repository: this.extractRepositoryUrl(versionData.repository || packageData.repository),
        ...
        maintainers: packageData.maintainers?.map((m: any) => m.name || m.email) || [],
        keywords: versionData.keywords || packageData.keywords || [],
        publishedAt: packageData.time[versionData.version] || packageData.time.created,
   ...
   */
  async fetchGitHubRepository(owner: string, repo: string): Promise<GitHubFactResult> {
    ...
    try {
      ...
      const languagesResponse = await fetch(`${this.githubApiBase}/repos/${owner}/${repo}/languages`, {
        headers: {
          'Accept': 'application/vnd.github.v3+json',
          'User-Agent': 'claude-code-zen-fact-system'
        }
      ...
      const languages = Object.entries(languagesData).reduce((acc: Record<string, number>, [lang, bytes]: [string, any]) => {
        acc[lang] = totalBytes > 0 ? Math.round((bytes / totalBytes) * 100) : 0;
        return acc;
      }, {});
      
      // Fetch recent releases
      const releasesResponse = await fetch(`${this.githubApiBase}/repos/${owner}/${repo}/releases?per_page=5`, {
        headers: {
          'Accept': 'application/vnd.github.v3+json',
          'User-Agent': 'claude-code-zen-fact-system'
        }
   ...
   */
  async fetchHexPackage(packageName: string, version?: string): Promise<HexFactResult> {
    logger.info(`Fetching live Hex data for ${packageName}${version ? `@${version}` : ''}`);
    ...
    try {
      ...
      if (!packageResponse.ok) {
        throw new Error(`Hex API error: ${packageResponse.status} ${packageResponse.statusText}`);
      }
      ...
      if (targetVersion) {
        try {
          ...
          logger.warn(`Could not fetch version data for ${packageName}@${targetVersion}:`, versionError);
        }
      }
      
      // Parse dependencies if available
      const dependencies = versionData?.requirements ? Object.entries(versionData.requirements).map(([name, req]: [string, any]) => ({
        name,
      ...
      return {
        ...
        repository: packageData.meta?.links?.github || packageData.meta?.links?.repository,
        ...
        maintainers: packageData.owners?.map((owner: any) => owner.username) || [],
  ...
  async fetchSecurityAdvisory(cveId: string): Promise<SecurityFactResult> {
    ...
    try {
      ...
      if (!cveResponse.ok) {
        throw new Error(`NVD API error: ${cveResponse.status} ${cveResponse.statusText}`);
      }
      ...
      return {
        ...
        affectedProducts: vuln.configurations?.nodes?.flatMap((node: any) => 
   ...
   */
  async fetchRustCrate(crateName: string, version?: string): Promise<RustCrateFactResult> {
    const requestedVersion = version || 'latest stable';
    logger.info(`Fetching live Rust crate data for ${crateName}@${requestedVersion}`);
    ...
    try {
      ...
      if (!crateResponse.ok) {
        throw new Error(`Crates.io API error: ${crateResponse.status} ${crateResponse.statusText}`);
      }
      ...
      if (version) {
        ...
        const versionsData = versionsResponse.ok ? await versionsResponse.json() : null;
        
        if (versionsData) {
          const stableVersions = versionsData.versions.filter((v: any) => !v.prerelease);
          targetVersion = stableVersions[0]?.num || crate.newest_version;
        } else {
          targetVersion = crate.newest_version;
        }
        logger.info(`ℹ️ No version specified for ${crateName}, using latest stable: ${targetVersion}`);
      }
      ...
      return {
        ...
        dependencies: versionData?.dependencies?.map((dep: any) => ({
          name: dep.crate_id,
          version: dep.req,
          kind: dep.kind,
        ...
        documentation: crate.documentation || `https://docs.rs/${crateName}/${targetVersion}`,
        docsSites: {
   ...
   */
  async fetchGoModule(modulePath: string, version?: string): Promise<GoModuleFactResult> {
    const requestedVersion = version || 'latest';
    logger.info(`Fetching live Go module data for ${modulePath}@${requestedVersion}`);
    ...
    try {
      ...
      if (!infoResponse.ok) {
        throw new Error(`Go proxy API error: ${infoResponse.status} ${infoResponse.statusText}`);
      }
      ...
      try {
        ...
        logger.warn(`Could not fetch pkg.go.dev data for ${modulePath}:`, error);
      }
      ...
      return {
        ...
        packages: pkgData?.Packages?.map((pkg: any) => ({
          path: pkg.Path,
          name: pkg.Name,
   ...
   */
  async fetchPerlPackage(packageName: string, version?: string): Promise<PerlPackageFactResult> {
    const requestedVersion = version || 'latest';
    logger.info(`Fetching live Perl package data for ${packageName}@${requestedVersion}`);
    ...
    try {
      ...
      if (!searchResponse.ok) {
        throw new Error(`MetaCPAN API error: ${searchResponse.status} ${searchResponse.statusText}`);
      }
      ...
      return {
        ...
        dependencies: releaseData?.dependency?.map((dep: any) => ({
          module: dep.module,
          version: dep.version,
          phase: dep.phase,
   ...
   */
  async fetchJavaPackage(groupId: string, artifactId: string, version?: string): Promise<JavaPackageFactResult> {
    const packageName = `${groupId}:${artifactId}`;
    const requestedVersion = version || 'latest';
    logger.info(`Fetching live Java package data for ${packageName}@${requestedVersion}`);
    ...
    try {
      ...
      if (!searchResponse.ok) {
        throw new Error(`Maven Central API error: ${searchResponse.status} ${searchResponse.statusText}`);
      }
   ...
   */
  async fetchGitLabRepository(projectPath: string): Promise<GitLabRepoFactResult> {
   ...
   */
  async fetchBitbucketRepository(workspace: string, repoSlug: string): Promise<BitbucketRepoFactResult> {
    ...
    try {
      ...
      return {
        ...
        cloneUrls: {
          https: repo.links.clone.find((link: any) => link.name === 'https')?.href || '',
          ssh: repo.links.clone.find((link: any) => link.name === 'ssh')?.href || ''
        },
