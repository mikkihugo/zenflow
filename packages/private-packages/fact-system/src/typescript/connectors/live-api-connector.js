...
export class LiveAPIConnector {
    ...
    hexApiBase = 'https://hex.pm/api';
    nvdApiBase = 'https://services.nvd.nist.gov/rest/json';
    cratesApiBase = 'https://crates.io/api/v1';
    goProxyBase = 'https://proxy.golang.org';
    cpanApiBase = 'https://fastapi.metacpan.org/v1';
    mavenApiBase = 'https://search.maven.org/solrsearch';
     ...
     */
    getCacheTTL(factType, hasVersion) {
        if (hasVersion) {
     ...
     */
    async fetchNPMPackage(packageName, version) {
        ...
        try {
            ...
            return {
                ...
                maintainers: packageData.maintainers?.map((m) => m.name || m.email) || [],
    ...
    async fetchGitHubRepository(owner, repo) {
        ...
        try {
            ...
            const languages = Object.entries(languagesData).reduce((acc, [lang, bytes]) => {
     ...
     */
    async fetchHexPackage(packageName, version) {
        ...
        try {
            ...
            const dependencies = versionData?.requirements ? Object.entries(versionData.requirements).map(([name, req]) => ({
                name,
            ...
            return {
                ...
                maintainers: packageData.owners?.map((owner) => owner.username) || [],
     ...
     */
    async fetchSecurityAdvisory(cveId) {
        ...
        try {
            ...
            return {
                ...
                affectedProducts: vuln.configurations?.nodes?.flatMap((node) => node.cpeMatch?.map((cpe) => cpe.criteria) || []) || [],
     ...
     */
    async fetchRustCrate(crateName, version) {
        const requestedVersion = version || 'latest stable';
        ...
        try {
            ...
            if (version) {
                ...
                if (versionsData) {
                    const stableVersions = versionsData.versions.filter((v) => !v.prerelease);
     ...
     */
    async fetchGoModule(modulePath, version) {
     ...
     */
    async fetchPerlPackage(packageName, version) {
        const requestedVersion = version || 'latest';
     ...
     */
    async fetchJavaPackage(groupId, artifactId, version) {
     ...
     */
    async fetchGitLabRepository(projectPath) {
     ...
     */
    async fetchBitbucketRepository(workspace, repoSlug) {
        ...
        try {
            ...
            return {
                ...
                cloneUrls: {
                    https: repo.links.clone.find((link) => link.name === 'https')?.href || '',
                    ssh: repo.links.clone.find((link) => link.name === 'ssh')?.href || ''
                },
...
//# sourceMappingURL=live-api-connector.js.map