...
vi.mock('../../neural-ml/src/ml-engine', () => ({
  createMLEngine: vi.fn().mockResolvedValue({
    ...
    patternLearner: {
      configure: vi.fn().mockResolvedValue(undefined),
      trainPatterns: vi.fn().mockResolvedValue([
...
describe('ML Integration Tests', () => {
  ...
  describe('Core Component Initialization', () => {
    it('should initialize ML Integration Demo successfully', async () => {
      ...
      expect(status.capabilities).toContain('Autonomous teleprompter selection using ML analysis');
    });
  ...
  describe('Autonomous Teleprompter Selection', () => {
    ...
    it('should provide different recommendations for different task types', async () => {
      ...
      const speedTask: OptimizationTask = {
        ...sampleTask,
        id: 'speed-task',
        description: 'Fast real-time optimization task requiring quick responses',
        requirements: {
      ...
      const complexTask: OptimizationTask = {
        ...sampleTask,
        id: 'complex-task',
        description: 'Complex sophisticated optimization with advanced requirements',
        complexity: {
      ...
      const speedSelection = await selector.selectOptimalTeleprompter(speedTask);
      const complexSelection = await selector.selectOptimalTeleprompter(complexTask);
  ...
  describe('DSPy-Brain ML Bridge Integration', () => {
    it('should provide intelligent teleprompter recommendations', async () => {
      ...
      const recommendation = await bridge.getIntelligentTeleprompterRecommendation(
  ...
  describe('End-to-End ML Integration Demo', () => {
    ...
    it('should provide comprehensive ecosystem status', async () => {
      ...
      expect(status.availableVariants).toHaveLength(3);
      expect(status.capabilities).toContain('Autonomous teleprompter selection using ML analysis');
      expect(status.capabilities).toContain('Battle-tested Rust crates integration (smartcore, linfa, argmin, statrs)');
      expect(status.capabilities).toContain('Multi-objective optimization (accuracy, speed, memory)');
    });
  });
  ...
  describe('Performance and Quality Validation', () => {
    it('should show improved performance with ML enhancement', async () => {
      ...
      expect(result.analysis.confidence.selectionConfidence).toBeGreaterThan(0.5);
    ...
    it('should demonstrate convergence and learning', async () => {
      ...
      const result = await teleprompter.compile(mockModule, { evaluationMethod: 'test' });
  ...
  describe('Error Handling and Robustness', () => {
    it('should handle initialization failures gracefully', async () => {
      ...
      vi.mocked(require('@claude-zen/brain').BrainCoordinator).mockImplementationOnce(() => {
        throw new Error('Initialization failed');
      });

      await expect(faultyBridge.initialize()).rejects.toThrow('Initialization failed');
    });
    ...
    it('should provide fallback recommendations when ML analysis fails', async () => {
      ...
      const minimalTask: OptimizationTask = {
        id: 'minimal-task',
        description: '',
        domain: { type: 'general', dataCharacteristics: { size: 'small', quality: 'unknown', complexity: 'simple' } },
        complexity: { computational: 'low', algorithmic: 'simple', dataVolume: 'small', timeConstraints: 'relaxed' },
        requirements: { minimumAccuracy: 0.5, maximumLatency: 5000, memoryConstraints: 256, robustness: 'basic', interpretability: 'optional' },
        constraints: { computationalBudget: 'limited', timeLimit: 10000, memoryLimit: 256, qualityThreshold: 0.5, fallbackRequired: true }
      };
...
export class MLIntegrationTestUtils {
  static createMockOptimizationTask(overrides: Partial<OptimizationTask> = {}): OptimizationTask {
    return {
      id: 'test-task',
  ...
  static async runPerformanceComparison(
    ...
    const improvement = (mlResult.analysis.selectedPerformance.accuracy - baselineResult.accuracy) / baselineResult.accuracy;
