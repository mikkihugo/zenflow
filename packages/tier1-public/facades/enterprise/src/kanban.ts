...
export function isValidWorkflowState(state: string): state is TaskFlowState {
  return DEFAULT_WORKFLOW_STATES.includes(state as TaskFlowState);
}
...
export function isValidStateTransition(from: string, to: string): boolean {
  const fromIndex = DEFAULT_WORKFLOW_STATES.indexOf(from as TaskFlowState);
  const toIndex = DEFAULT_WORKFLOW_STATES.indexOf(to as TaskFlowState);
