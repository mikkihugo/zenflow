...
/**
 * Helper to migrate from custom flow manager to WorkflowKanban
 *
 * Provides migration utilities for existing flow management systems
 * to adopt the new XState-powered workflow coordination.
 *
 * @deprecated Use this temporarily for migration from legacy systems
 */
export const createMigrationHelper = () => {
  return {
    /**
     * Convert legacy task format to WorkflowTask
     */
    convertLegacyTask: (legacyTask: Record<string, unknown>): Partial<WorkflowTask> => {
      return {
        id: legacyTask.id || `migrated-${Date.now()}`,
        title: legacyTask.title || legacyTask.name || 'Migrated Task',
        description: legacyTask.description,
        state: isValidWorkflowState(legacyTask.state) ? legacyTask.state : 'backlog',
        priority: isValidTaskPriority(legacyTask.priority) ? legacyTask.priority : 'medium',
        estimatedEffort: legacyTask.estimatedEffort || legacyTask.effort || 1,
        createdAt: legacyTask.createdAt ? new Date(legacyTask.createdAt) : new Date(),
        updatedAt: legacyTask.updatedAt ? new Date(legacyTask.updatedAt) : new Date(),
        dependencies: Array.isArray(legacyTask.dependencies) ? legacyTask.dependencies : [],
        tags: Array.isArray(legacyTask.tags) ? legacyTask.tags : [],
        metadata: legacyTask.metadata || {},
      };
    },

    /**
     * Convert legacy WIP limits to new format
     */
    convertLegacyWIPLimits: (legacyLimits: Record<string, unknown>): Partial<WIPLimits> => {
      return {
        backlog: legacyLimits.backlog || 100,
        analysis: legacyLimits.analysis || 5,
        development: legacyLimits.development || legacyLimits.inProgress || 10,
        testing: legacyLimits.testing || 8,
        review: legacyLimits.review || 5,
        deployment: legacyLimits.deployment || 3,
        done: legacyLimits.done || 1000,
        blocked: legacyLimits.blocked || 10,
        expedite: legacyLimits.expedite || 2,
        total: legacyLimits.total || 50,
      };
    },
  };
};
