...
impl RealTimeMonitor {
  ...
  fn update_baseline(&self, operation: &CompletedOperation) {
    ...
    let baseline =
      baselines.entry(operation.name.clone()).or_insert_with(|| {
        PerformanceBaseline {
    ...
    baseline.expected_throughput = alpha * operation.throughput
      + (1.0 - alpha) * baseline.expected_throughput;
      ...
      if degradation_factor > self.config.degradation_threshold {
        return Some(PerformanceDegradation {
          ...
          suggested_action: self
            .generate_optimization_suggestion(operation, baseline),
        });
      }
    }
  ...
  fn generate_optimization_suggestion(
    ...
    if operation.memory_transfer_time > operation.execution_time / 2 {
      ...
      "GPU utilization is low, consider increasing batch size or workgroup size"
        .to_string()
      ...
      "Performance degradation detected, consider profiling individual kernels"
        .to_string()
    }
  }
  ...
  pub fn get_trends(
    ...
    let recent_ops: Vec<&CompletedOperation> = history
      .operations
      .iter()
      .rev()
  ...
  pub fn get_bottleneck_analysis(
    &self,
    operation_name: &str,
    ...
    if let Some(stats) = history.aggregated_stats.get(operation_name) {
      ...
      let avg_memory_transfer =
        stats.total_time.as_secs_f64() / stats.count as f64;
...
impl PerformanceMonitorTrait for RealTimeMonitor {
  ...
  fn end_operation(
    ...
    let ongoing = operations.remove(&handle).ok_or_else(|| {
      NeuralIntegrationError::PerformanceError(
        "Invalid operation handle".to_string(),
      )
    })?;
  ...
  fn get_performance_summary(&self) -> PerformanceStats {
    ...
    let total_time: Duration =
      history.operations.iter().map(|op| op.execution_time).sum();

    let total_gpu_time: Duration =
      history.operations.iter().map(|op| op.gpu_time).sum();

    let total_throughput: f64 =
      history.operations.iter().map(|op| op.throughput).sum();

    PerformanceStats {
      total_operations: history.total_operations,
      average_execution_time: total_time.as_secs_f64()
        / history.total_operations as f64,
      gpu_utilization: (total_gpu_time.as_secs_f64() / total_time.as_secs_f64())
        as f32,
