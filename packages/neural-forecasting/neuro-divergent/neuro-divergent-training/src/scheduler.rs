...
impl<T: Float + Send + Sync> LearningRateScheduler<T>
  for ExponentialScheduler<T>
{
  fn get_learning_rate(&mut self, step: usize) -> T {
    ...
    if step < self.warmup_steps {
      // Linear warmup
      let warmup_factor =
        T::from(step).unwrap() / T::from(self.warmup_steps).unwrap();
      self.current_lr = self.initial_lr * warmup_factor;
    } else {
      // Exponential decay
      let decay_steps = step - self.warmup_steps;
      self.current_lr =
        self.initial_lr * self.decay_rate.powi(decay_steps as i32);
    }
  ...
  fn get_learning_rate(&mut self, step: usize) -> T {
    ...
    if !self.milestones.is_empty() {
      ...
      let num_reductions = self
        ...
        .count();
      self.current_lr =
        self.initial_lr * self.gamma.powi(num_reductions as i32);
    } else {
      // Use step_size
  ...
  fn state(&self) -> SchedulerState<T> {
    SchedulerState {
      ...
      additional_state: std::collections::HashMap::from([
        ...
        (
          "step_size".to_string(),
          vec![T::from(self.step_size).unwrap()],
        ),
...
impl<T: Float + Send + Sync> LearningRateScheduler<T> for CosineScheduler<T> {
  fn get_learning_rate(&mut self, step: usize) -> T {
    ...
    while t_cur >= t_max {
      ...
      t_max = (T::from(self.t_max).unwrap()
        * self.restart_factor.powi(self.restart_count as i32))
      .to_usize()
      .unwrap_or(self.t_max);
      self.current_t_max = t_max;
    }

    // Cosine annealing formula
    let pi = T::from(std::f64::consts::PI).unwrap();
    let cos_factor = (T::one()
      + (pi * T::from(t_cur).unwrap() / T::from(t_max).unwrap()).cos())
      / T::from(2.0).unwrap();
    self.current_lr =
      self.min_lr + (self.initial_lr - self.min_lr) * cos_factor;
  ...
  fn state(&self) -> SchedulerState<T> {
    SchedulerState {
      ...
      additional_state: std::collections::HashMap::from([
        ...
        (
          "current_t_max".to_string(),
          vec![T::from(self.current_t_max).unwrap()],
        ),
      ]),
    }
  }
  ...
  fn state(&self) -> SchedulerState<T> {
    SchedulerState {
      ...
      additional_state: std::collections::HashMap::from([
        ...
        (
          "patience".to_string(),
          vec![T::from(self.patience).unwrap()],
        ),
        ("threshold".to_string(), vec![self.threshold]),
        ("min_lr".to_string(), vec![self.min_lr]),
        (
          "cooldown".to_string(),
          vec![T::from(self.cooldown).unwrap()],
        ),
      ]),
    }
  }
...
impl<T: Float + Send + Sync> LearningRateScheduler<T> for WarmupScheduler<T> {
  fn get_learning_rate(&mut self, step: usize) -> T {
    ...
    if step < self.warmup_steps {
      ...
      let warmup_factor =
        T::from(step).unwrap() / T::from(self.warmup_steps).unwrap();
      self.current_lr =
        self.initial_lr + (self.target_lr - self.initial_lr) * warmup_factor;
    } else {
      // Use decay scheduler
  ...
  fn state(&self) -> SchedulerState<T> {
    ...
    state
      .additional_state
      .insert("initial_lr".to_string(), vec![self.initial_lr]);
    state
      .additional_state
      .insert("target_lr".to_string(), vec![self.target_lr]);
    state
  }

  fn restore_state(
    &mut self,
    mut state: SchedulerState<T>,
  ) -> TrainingResult<()> {
    self.current_step = state.current_step;
    self.current_lr = state.current_lr;
    self.warmup_steps = state.warmup_steps;
  ...
  fn get_learning_rate(&mut self, step: usize) -> T {
    ...
    let scale_fn = match self.mode {
      ...
      CyclicMode::Triangular2 => {
        T::one() / T::from(2_u32.pow(cycle as u32)).unwrap()
      }
    ...
    if x <= self.step_size_up {
      ...
      self.current_lr =
        self.base_lr + (self.max_lr - self.base_lr) * factor * scale_fn;
    } else {
      // Descending phase
      let factor = T::one()
        - T::from(x - self.step_size_up).unwrap()
          / T::from(self.step_size_down).unwrap();
      self.current_lr =
        self.base_lr + (self.max_lr - self.base_lr) * factor * scale_fn;
    }
  ...
  fn state(&self) -> SchedulerState<T> {
    SchedulerState {
      ...
      additional_state: std::collections::HashMap::from([
        ...
        (
          "step_size_up".to_string(),
          vec![T::from(self.step_size_up).unwrap()],
        ),
        (
          "step_size_down".to_string(),
          vec![T::from(self.step_size_down).unwrap()],
        ),
        ("gamma".to_string(), vec![self.gamma]),
      ]),
    }
...
impl<T: Float + Send + Sync> LearningRateScheduler<T> for OneCycleScheduler<T> {
  fn get_learning_rate(&mut self, step: usize) -> T {
    ...
    self.current_step = step;
    let step_ratio =
      T::from(step).unwrap() / T::from(self.total_steps).unwrap();
    ...
    if step_ratio <= self.pct_start {
      ...
      // Descending phase
      let phase_ratio =
        (step_ratio - self.pct_start) / (T::one() - self.pct_start);

      match self.anneal_strategy {
        AnnealStrategy::Cos => {
          let pi = T::from(std::f64::consts::PI).unwrap();
          let cos_factor =
            (T::one() + (pi * phase_ratio).cos()) / T::from(2.0).unwrap();
    ...
    SchedulerState {
      ...
      additional_state: std::collections::HashMap::from([
        ...
        (
          ...
          vec![T::from(self.total_steps).unwrap()],
        ),
        ...
        (
          "finished".to_string(),
          vec![if self.finished { T::one() } else { T::zero() }],
        ),
      ]),
    }
  }
  ...
  fn restore_state(&mut self, state: SchedulerState<T>) -> TrainingResult<()> {
    ...
    if let Some(final_div_factor) =
      state.additional_state.get("final_div_factor")
    {
...
impl<T: Float + Send + Sync> LearningRateScheduler<T> for SeasonalScheduler<T> {
  fn get_learning_rate(&mut self, step: usize) -> T {
    ...
    if !self.seasonal_factors.is_empty() {
      let season_idx = (step % self.season_length)
        * self.seasonal_factors.len()
        / self.season_length;
      let seasonal_factor =
        self.seasonal_factors[season_idx.min(self.seasonal_factors.len() - 1)];
  ...
  fn restore_state(
    ...
    if let Some(seasonal_factors) =
      state.additional_state.remove("seasonal_factors")
    {
...
mod tests {
  ...
  fn test_seasonal_scheduler() {
    let base_scheduler =
      SchedulerType::Exponential(ExponentialScheduler::new(0.1, 0.9));
    let seasonal_factors = vec![1.0, 1.2, 0.8, 1.1];
    let mut scheduler =
      SeasonalScheduler::new(base_scheduler, seasonal_factors, 4);
