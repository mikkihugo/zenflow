...
pub mod math {
  ...
  pub fn mae<T: Float>(y_true: &[T], y_pred: &[T]) -> NeuroDivergentResult<T> {
    if y_true.len() != y_pred.len() {
      return Err(NeuroDivergentError::math(
  ...
  pub fn quantile<T: Float + PartialOrd>(
    sorted_data: &[T],
    q: f64,
  ) -> Option<T> {
...
pub mod time {
  ...
  pub fn infer_frequency(timestamps: &[DateTime<Utc>]) -> Option<Frequency> {
    ...
    let most_common_diff = diff_counts
      .into_iter()
  ...
  pub fn is_regular_frequency(
    ...
    diffs
      .iter()
      .all(|&diff| (diff - mean_diff).abs() <= tolerance)
  }

  /// Create lag features for time series
  pub fn create_lags<T: Float>(
    data: &[T],
    lags: &[usize],
  ) -> Vec<Vec<Option<T>>> {
  ...
  pub fn seasonal_naive<T: Float + Copy>(
    ...
    for h in 1..=horizon {
      let seasonal_index =
        (data_len - seasonal_period + h - 1) % seasonal_period;
      ...
      if value.is_some() {
        if let Some(start) = start_idx {
          ...
          if let (Some(start_val), Some(end_val)) = (data[start], data[i]) {
            ...
            if steps > 0 {
              let step_size =
                (end_val - start_val) / T::from(steps + 1).unwrap();
              for j in 1..=steps {
  ...
  pub fn validate_time_series<T: Float>(
    ...
    if !columns.contains(&df.schema.unique_id_col.as_str()) {
      report.add_error(format!(
        ...
        df.schema.unique_id_col
      ));
    }
    if !columns.contains(&df.schema.ds_col.as_str()) {
      report
        .add_error(format!("Missing date/time column: {}", df.schema.ds_col));
    }
