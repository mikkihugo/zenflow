...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('approval_gates', (table) => {
    ...
    // Gate definition
    table.string('name', 200).notNullable().comment('Human-readable gate name');
    table.text('description').comment('Gate description and purpose');

    // Task association
    table.uuid('task_id').notNullable().comment('Associated task ID');
    ...
    table.integer('timeout_hours').comment('Timeout in hours for approval');
    table
      .json('auto_approval_conditions')
      .defaultTo('[]')
    ...
    table.timestamp('timeout_at').comment('When the gate will timeout');
    table.timestamp('escalated_at').comment('When the gate was escalated');
    table
      .timestamp('completed_at')
      .comment('When the gate was completed (approved/rejected)');

    // Metadata
    table.json('metadata').defaultTo('{}').comment('Additional gate metadata');
    ...
    table.check(
      'minimum_approvals > 0',
      [],
      'chk_approval_gates_min_approvals_positive'
    );
    table.check(
      'timeout_hours > 0 OR timeout_hours IS NULL',
      [],
      'chk_approval_gates_timeout_positive'
    );
    table.check(
      'created_at <= updated_at',
      [],
      'chk_approval_gates_timestamps_order'
    );
  });
  ...
  await knex.schema.createTable('approval_records', (table) => {
    ...
    table.uuid('gate_id').notNullable().comment('Associated approval gate ID');
    table.uuid('task_id').notNullable().comment('Associated task ID');
    ...
    table.text('reason').comment('Reason for the decision');
    ...
    table.index(
      ['gate_id', 'approver_id'],
      'idx_approval_records_gate_approver'
    );

    // Foreign key constraints
    table
      .foreign('gate_id')
      .references('approval_gates.id')
      .onDelete('CASCADE');
    table.foreign('task_id').references('tasks.id').onDelete('CASCADE');
