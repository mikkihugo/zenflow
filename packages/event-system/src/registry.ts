...
export class EventRegistry implements EventManagerRegistry {
  ...
  registerFactory<T extends EventManagerConfig>(
    ...
    this.factoryRegistry[type] = {
      ...
      metadata: {
        ...
        version: '1.0.0',
        capabilities: [], // getSupportedEventTypes method not available on EventManagerFactory interface
        supported: [type],
      },
      registered: new Date(),
      usage: {
  ...
  getFactory<T extends EventManagerConfig>(
    ...
    return factory as EventManagerFactory<T>;
  }
  ...
  getAllEventManagers(): Map<string, EventManager> {
    const managers = new Map<string, EventManager>();
  ...
  private async registerDefaultEventTypes(): Promise<void> {
    const defaultEventTypes = [
      ...
      {
        type: 'interface:user',
        category: EventCategories.NTERFACE,
        managerTypes: [EventManagerTypes.NTERFACE],
        priority: 2,
      },
      {
