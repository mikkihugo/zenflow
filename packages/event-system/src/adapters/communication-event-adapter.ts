...
// Generic communication system interfaces
// Using generic interfaces to maintain library design principles
interface WebSocketClientAdapter {
  healthCheck: () => Promise<{
    responseTime?: number;
    errorRate?: number;
  }>;
...
export interface CommunicationEventAdapterConfig extends EventManagerConfig {
  ...
  /** RPC protocol integration settings */
  rpcProtocol?: {
    enabled: boolean;
    ...
    servers?: string[]; // List of RPC servers to wrap
    clients?: string[]; // List of RPC clients to wrap
  };
 ...
 * - RPC protocol event management and tool execution tracking
 ...
 * @example
 */
export class CommunicationEventAdapter implements EventManager {
  ...
  constructor(config: CommunicationEventAdapterConfig) {
    ...
    this.config = {
      ...
      rpcProtocol: {
        ...
        servers: ['http-rpc-server'],
        clients: ['default-rpc-client'],
        ...config?.rpcProtocol,
      },
      protocolCommunication: {
        enabled: true,
      ...
      communication: {
        ...
        correlationPatterns: [
          'communication:websocket->communication:rpc',
          'communication:http->communication:rpc',
          'communication:protocol->communication:websocket',
          'communication:rpc->communication:http',
        ],
      ...
      switch (this.config.processing?.strategy) {
        case 'immediate':
          await this.processCommunicationBatchImmediate(
            batch as unknown as EventBatch<CommunicationEvent>,
            options
          );
          break;
        case 'queued':
  ...
  async query<T extends SystemEvent>(options: EventQueryOptions): Promise<T[]> {
    ...
    if (options?.filter) {
      events = events.filter((event) =>
        this.applyFilter(event as unknown as CommunicationEvent, options?.filter!)
      );
    }
    ...
    if (options?.sortBy) {
      events.sort((a, b) => {
        const aVal = this.getEventSortValue(
          a as unknown as CommunicationEvent,
          options?.sortBy!
        );
        const bVal = this.getEventSortValue(
          b as unknown as CommunicationEvent,
          options?.sortBy!
        );
  ...
  async healthCheck(): Promise<EventManagerStatus> {
    ...
    return {
      ...
      metadata: {
        ...
        rpcServers: this.rpcServers.size,
        rpcClients: this.rpcClients.size,
        componentHealth,
        avgCommunicationLatency:
  ...
  /**
   * Emit RPC protocol event with enhanced tracking.
   *
   * @param event
   */
  ...
  /**
   * Subscribe to RPC protocol events.
   *
   * @param listener
   */
  subscribeRPCProtocolEvents(
    listener: EventListener<CommunicationEvent>
  ): string {
    return this.subscribe(['communication:rpc'], listener);
  }
   ...
   */
  async getCommunicationHealthStatus(): Promise<
   ...
   */
  async performCommunicationHealthCheck(): Promise<
    Record<string, CommunicationHealthEntry>
  > {
    ...
    for (const [componentName, wrapped] of this.wrappedComponents.entries()) {
      try {
        ...
        } else if (wrapped.componentType === 'rpc-server') {
        ...
        const healthEntry: CommunicationHealthEntry = {
          ...
          lastCheck: new Date(),
          consecutiveFailures:
            (this.communicationHealth.get(componentName)?.consecutiveFailures ||
              0) + 1,
  ...
  private async initializeCommunicationIntegrations(): Promise<void> {
    ...
    // Wrap RPC servers if enabled
    if (this.config.rpcProtocol?.enabled) {
      await this.wrapRPCServers();
      await this.wrapRPCClients();
    }
  ...
  private async wrapRPCServers(): Promise<void> {
    ...
    for (const serverName of servers) {
      ...
      const wrappedComponent: WrappedCommunicationComponent = {
        ...
        originalMethods: new Map(),
        eventMappings: new Map([
          ['server:started', 'communication:rpc'],
          ['server:stopped', 'communication:rpc'],
          ['tool:called', 'communication:rpc'],
          ['tool:completed', 'communication:rpc'],
          ['tool:error', 'communication:rpc'],
          ['client:connected', 'communication:rpc'],
          ['client:disconnected', 'communication:rpc'],
          ['protocol:error', 'communication:rpc'],
        ]),
        isActive: true,
        healthMetrics: {
          lastSeen: new Date(),
      ...
      wrappedComponent.eventMappings.forEach((uelEvent, originalEvent) => {
        wrapper.on(originalEvent, (data) => {
          ...
          this.eventEmitter.emit(uelEvent, communicationEvent);
          this.updateComponentHealthMetrics(
            `rpc-server-${serverName}`,
            !originalEvent.includes('error')
          );
        });
      });

      this.wrappedComponents.set(`rpc-server-${serverName}`, wrappedComponent);
      this.logger.debug(`Wrapped RPC server events: ${serverName}`);
    }
  }
    ...
    for (const clientName of clients) {
      ...
      const wrappedComponent: WrappedCommunicationComponent = {
        ...
        eventMappings: new Map([
          ['client:connected', 'communication:rpc'],
          ['client:disconnected', 'communication:rpc'],
          ['tool:request', 'communication:rpc'],
          ['tool:response', 'communication:rpc'],
          ['tool:timeout', 'communication:rpc'],
          ['protocol:error', 'communication:rpc'],
        ]),
        isActive: true,
        healthMetrics: {
      ...
      wrappedComponent.eventMappings.forEach((uelEvent, originalEvent) => {
        wrapper.on(originalEvent, (data) => {
          ...
          this.eventEmitter.emit(uelEvent, communicationEvent);
          this.updateComponentHealthMetrics(
            `rpc-client-${clientName}`,
            !originalEvent.includes('error')
          );
        });
      });

      this.wrappedComponents.set(`rpc-client-${clientName}`, wrappedComponent);
      this.logger.debug(`Wrapped RPC client events: ${clientName}`);
    }
  }
        ...
        wrapper.on(originalEvent, (data) => {
          const communicationEvent: CommunicationEvent = {
            ...
            payload: {
              ...
              endpoint: data?.endpoint,
              switchingReason: data?.switchingReason,
            },
            details: {
              ...data,
              protocolType,
    ...
    for (const [componentName, wrapped] of this.wrappedComponents.entries()) {
      try {
        ...
        wrapped.originalMethods.forEach((originalMethod, methodName) => {
          if (wrapped.component && (wrapped.component as any)[methodName]) {
  ...
  private async processCommunicationEventEmission<T extends SystemEvent>(
    ...
    this.eventHistory.push(event as unknown as CommunicationEvent);
    ...
    if (this.config.communication?.enabled && event.correlationId) {
      this.updateCommunicationEventCorrelation(event as unknown as CommunicationEvent);
    }

    // Update communication-specific metrics
    this.updateCommunicationMetrics(event as unknown as CommunicationEvent);
  ...
  private startCommunicationCorrelationCleanup(): void {
    ...
    setInterval(() => {
      ...
      for (const [
        correlationId,
        correlation,
      ] of this.communicationCorrelations.entries()) {
          ...
          if (
            ...
            await this.emitWebSocketCommunicationEvent({
              ...
              payload: {
                ...
                responseTime: health.communicationLatency,
                statusCode:
                  health.throughput > thresholds.throughput ? 200 : 500,
              },
              details: {
                requestId: `optimization-${Date.now()}`,
                responseTime: health.communicationLatency,
   ...
   * @param options
   */
  private async processCommunicationBatchImmediate<
  ...
  private extractConnectionId(event: CommunicationEvent): string | undefined {
    return (event.details as any)?.connectionId || event.metadata?.['connectionId'];
  }
  ...
  private updateCommunicationMetrics(event: CommunicationEvent): void {
    ...
    if (
      messageId &&
      (event.type === 'communication:websocket' ||
        event.type === 'communication:http' ||
        event.type === 'communication:protocol' ||
        event.type === 'communication:rpc')
    ) {
...
export const CommunicationEventHelpers = {
  ...
  /**
   * Create RPC tool execution event.
   *
   * @param toolName
   * @param requestId
   * @param details
   */
  createRPCToolExecutionEvent(
    toolName: string,
    requestId: string,
    details?: unknown
  ): Omit<CommunicationEvent, 'id' | 'timestamp'> {
