...
export class ExportSystem extends EventEmitter {
  ...
  private registerBuiltInExporters():void {
    ...
    this.registerExporter('json', {
      name: 'JSON Exporter',      extension: '.json',      mimeType: 'application/json',      description: 'Export data as JSON format',      export:(data: unknown, options?:ExportOptions) => options?.pretty !== false
    ...
    this.registerExporter('yaml', {
      name: 'YAML Exporter',      extension: '.yaml',      mimeType: 'text/yaml',      description: 'Export data as YAML format',      export:(data: unknown) => this.convertToYAML(data, 0),
      validate:(data: unknown) => data !== undefined && data !== null,
});

    // CSV Exporter
    this.registerExporter('csv', {
      name: 'CSV Exporter',      extension: '.csv',      mimeType: 'text/csv',      description: 'Export array data as CSV format',      export:(data: unknown) => this.convertToCSV(data as Record<string, unknown>[]),
    ...
    this.registerExporter('xml', {
      name: 'XML Exporter',      extension: '.xml',      mimeType: 'application/xml',      description: 'Export data as XML format',      export:(data: unknown) => `<?xml version="1.0" encoding="UTF-8"?>\n<root>\n${this.convertToXML(data, 1)}\n</root>`,
      validate:(data: unknown) => data !== undefined && data !== null,
});

    // Markdown Exporter
    this.registerExporter('markdown', {
      name: 'Markdown Exporter',      extension: '.md',      mimeType: 'text/markdown',      description: 'Export document data as Markdown format',      export:(data: unknown) => this.convertToMarkdown(data),
      validate:(data: unknown) => typeof data === 'object' && data !== null,
});

    // Plain Text Exporter
    this.registerExporter('txt', {
      name: 'Text Exporter',      extension: '.txt',      mimeType: 'text/plain',      description: 'Export data as plain text format',      export:(data: unknown) => {
    ...
    this.registerExporter('html', {
      name: 'HTML Exporter',      extension: '.html',      mimeType: 'text/html',      description: 'Export data as HTML format',      export:(data: unknown) => this.convertToHTML(data),
      validate:(data: unknown) => data !== undefined && data !== null,
});
...
<body>`;
  ...
  if (key === 'title') continue;
        ...
        html += typeof value === 'string' ? `<p>${this.escapeHTML(value)}</p>` :`<p>${this.escapeHTML(String(value))}</p>`;
}
} else {
