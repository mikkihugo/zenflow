...
export const mockClaudeSDK = {
  sendMessage:vi.fn().mockResolvedValue({
    id: 'msg-123',    content: 'Mock Claude response',    role: 'assistant',    timestamp:Date.now(),
}),
...
export const mockGitHubModels = {
  sendRequest:vi.fn().mockResolvedValue({
    choices:[
      {
        message:{
          content: 'Mock GitHub Models response',          role: 'assistant',},
},
],
...
export const createMockResponse = (
  content:string = 'Mock response',  metadata:any = {}
) => ({
  content,
...
export const createMockError = (
  message:string = 'Mock error',  code?:string
) => {
