...
describe('Message Processing', () => {
  ...
  describe('Message Processor', () => {
    ...
    ')      const message = { content: 'Hello', role: ' user', timestamp:Date.now()};')
      ...
      expect(processed.content).toBe('Hello');')      expect(processed.role).toBe('user');')});

    it('should handle complex message objects', () => {
    ')      const complexMessage = {
        content: 'Analyze this code',        role: 'user',        metadata:{
          language: 'typescript',          context: 'function analysis',          priority: 'high',},
        attachments:[
    ...
    ')      const validMessage = { role: 'user', content: ' Valid message'};')      const invalidMessage = { role: 'invalid'}; // Missing content')
    ...
    ')      const unsafeMessage = {
        role: 'user',        content: 'Message with <script>alert("xss")</script> content',        metadata:{ userInput: true},
};
    ...
    ')      const emptyMessage = { role: 'user', content:'};')      const nullMessage = null;
  ...
  describe('Permission Handler', () => {
    ...
    ')        context: 'file-access',        resource: '/path/to/file.txt',});
    ...
    ')        context: 'file-modification',        resource: '/path/to/output.txt',        justification: 'Save analysis results',});

      expect(typeof granted).toBe('boolean');')      expect(mockPermissionHandler.requestPermission).toHaveBeenCalledWith(
        'write',        expect.objectContaining(
          context: 'file-modification',          justification: 'Save analysis results',)
      );
});
    ...
    ')        reason: 'Security policy change',});
      ...
      expect(typeof revoked).toBe('boolean');')      expect(mockPermissionHandler.revokePermission).toHaveBeenCalledWith(
    ...
    ')      const permissions = await mockPermissionHandler.listPermissions();
      ...
      permissions.forEach((permission) => {
        expect(permission).toHaveProperty('tool');')        expect(permission).toHaveProperty('allowed');')        expect(typeof permission.allowed).toBe('boolean');')});
});
    ...
    ')      mockPermissionHandler.checkPermission.mockRejectedValueOnce(
      ...
      await expect(
        mockPermissionHandler.checkPermission('unknown-tool')')      ).rejects.toThrow('Permission service unavailable');')});
    ...
    ')      const tools = ['read',    'write',    'bash',    'edit',    'search'];')
      ...
      checks.forEach((check) => {
        expect(check).toHaveProperty('allowed');')        expect(typeof check.allowed).toBe('boolean');')});
});
});
  ...
  describe('Message and Permission Integration', () => {
    ...
    ')      const messageWithTools = {
        role: 'user',        content: 'Please read the file and analyze it',        metadata:{
      ...
      const readPermission =
        await mockPermissionHandler.checkPermission('read');')      const analyzePermission =
    ...
    ')      const escalationMessage = {
        role: 'user',        content: 'I need to write to system files',        metadata:{
          escalation:true,
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    ')      const malformedMessages = [
        { role: 'user'}, // Missing content')        { content: 'Missing role'}, // Missing role')        {}, // Empty object
        null, // Null
        undefined, // Undefined
        'string message', // Wrong type')        123, // Number
];
    ...
    ')      const circularMessage:any = { role: 'user', content: ' Circular test'};')      circularMessage.self = circularMessage; // Create circular reference
    ...
    ')      const longContent = 'x'.repeat(1000000); // 1MB of content')      const longMessage = { role: 'user', content:longContent};')
      const startTime = Date.now();
    ...
    ')      const unicodeMessage = {
        role: 'user',        content: '测试 🚀 العربية русский 🎉 \u{1F600}',        metadata:{ encoding: 'utf-8'},
};

      const processed = mockMessageProcessor.processMessage(unicodeMessage);

      expect(processed).toBeDefined();
      expect(processed.content).toContain('测试');')      expect(processed.content).toContain('🚀');')});
});
});
