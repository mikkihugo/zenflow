...
vi.mock('../../src/llm-provider', () => ({
    ...
    ')    const instance = {
      ...
      getProviderInfo:() => ({
        id:providerId,
        type:providerId.includes('api') ? ' api' : ' cli',        capabilities:{
          fileOperations:providerId === 'claude-code',          codeCompletion:true,
          chat:true,
},
}),
      ...
      streamMessage:async function* () {
        yield { type: 'content', content: ' Mock'};')        yield type: 'done';')},
...
describe('LLMProvider', () => {
  ...
  describe('Constructor and Initialization', () => {
    ...
    ')      const githubProvider = new LLMProvider('github-models-api');')      expect(githubProvider).toBeDefined();
      expect(githubProvider.getProviderId()).toBe('github-models-api');')});

    it('should handle unknown provider IDs gracefully', () => {
    ')      const unknownProvider = new LLMProvider('unknown-provider' as any);')      expect(unknownProvider).toBeDefined();
  ...
  describe('Provider Information', () => {
    ...
    ')      const info = provider.getProviderInfo();
      expect(info).toMatchObject({
        id: 'claude-code',        type:expect.any(String),
    ...
    ')      const capabilities = provider.getCapabilities();
      expect(capabilities).toHaveProperty('fileOperations');')      expect(capabilities).toHaveProperty('codeCompletion');')      expect(capabilities).toHaveProperty('chat');')});
});

  describe('Message Handling', () => {
    ')    it('should send simple text messages', async () => {
    ')      const __mockResponse = createMockResponse('Hello from Claude!');')
      // Mock the internal sendMessage method
      vi.spyOn(provider as any, 'sendMessage').mockResolvedValue(mockResponse);')
      const __response = await provider.sendMessage('Hello');')      expect(response).toEqual(mockResponse);
});

    it('should handle complex message objects', async () => {
    ')      const complexMessage = {
        content: 'Analyze this code',        metadata:{ language: 'typescript', context: ' test'},
};

      const mockResponse = createMockResponse('Code analysis complete');')      vi.spyOn(provider as any, 'sendMessage').mockResolvedValue(mockResponse);')
      const response = await provider.sendMessage(complexMessage);
      expect(response).toEqual(mockResponse);
});

    it('should handle message sending errors gracefully', async () => {
    ')      const __error = createMockError('Network error',    'NETWORK_ERROR');')      vi.spyOn(provider as any, 'sendMessage').mockRejectedValue(error);')
  ...
  describe('Task Execution', () => {
    ...
    ')      const mockResult = {
        success:true,
        result: 'Task completed successfully',        metadata:{ duration: 500},
};

      vi.spyOn(provider as any, 'executeTask').mockResolvedValue(mockResult);')
      const __result = await provider.executeTask('simple task');')      expect(result).toEqual(mockResult);
});
    ...
    ')      const taskOptions = {
      ...
      vi.spyOn(provider as any, 'executeTask').mockResolvedValue(mockResult);')
      const __result = await provider.executeTask('complex task', taskOptions);')      expect(result).toEqual(mockResult);
});

    it('should handle task execution failures', async () => {
    ')      const __error = createMockError('Task execution failed',    'TASK_ERROR');')      vi.spyOn(provider as any, 'executeTask').mockRejectedValue(error);')
  ...
  describe('Streaming Support', () => {
    ...
    ')      const mockStream = async function* () {
        yield { type: 'content', content: ' Streaming'};')        yield type: 'content', content: ' response';')        yield type: 'done';')};

      vi.spyOn(provider as any, 'streamMessage').mockReturnValue(mockStream())();')
      const stream = provider.streamMessage('stream test');')      const chunks = [];
      ...
      expect(chunks[0]).toEqual({ type: 'content', content: ' Streaming'});')      expect(chunks[1]).toEqual(type: 'content', content:' response' );')      expect(chunks[2]).toEqual(type:'done' );')});

    it('should handle streaming errors', async () => {
    ')      const mockStream = async function* () {
        yield { type: 'content', content: ' Start'};')        throw createMockError('Stream error');')};
  ...
  describe('Health and Monitoring', () => {
    ...
    ')      const mockHealth = {
        status: 'unhealthy',        error: 'Connection failed',        lastCheck:Date.now(),
};
      vi.spyOn(provider as any, 'health').mockResolvedValue(mockHealth);')
      const health = await provider.health();
      expect(health.status).toBe('unhealthy');')      expect(health.error).toBe('Connection failed');')});
    ...
    ')      const mockStats = {
      ...
      const __stats = await provider.getStats();
...
describe('Global LLM Management', () => {
  ...
  it('should set and get global LLM provider', () => {
    ')    const provider = new LLMProvider('claude-code');')    setGlobalLLM(provider);
  ...
  it('should replace existing global LLM', () => {
    ')    const provider1 = new LLMProvider('claude-code');')    const provider2 = new LLMProvider('github-models-api');')
...
describe('Factory Functions', () => {
    ')  describe('createLLMProvider', () => ')    it('should create claude-code provider by default', () => {
  ...
  describe('getLLMProviderByCapability', () => {
    ...
    ')      const provider = getLLMProviderByCapability('file-operations');')      expect(provider).toBeInstanceOf(LLMProvider);
      expect(provider.getProviderId()).toBe('claude-code');')});
    ...
    ')      const provider = getLLMProviderByCapability('unknown-capability' as any);')      expect(provider).toBeInstanceOf(LLMProvider);
      expect(provider.getProviderId()).toBe('claude-code');')});
});
});
...
describe('Provider Type Detection', () => {
    ...
    ')    const __cliProviders = ['claude-code',    'cursor-cli',    'gemini-cli'];')
  ...
  it('should correctly identify API providers', () => {
    ')    const __apiProviders = ['github-models-api',    'anthropic-api',    'openai-api'];')
...
describe('Error Handling', () => {
  ...
  it('should handle network errors gracefully', async () => {
    ')    const __networkError = createMockError('Network timeout',    'NETWORK_TIMEOUT');')    vi.spyOn(provider as any, 'sendMessage').mockRejectedValue(networkError);')
    await expect(provider.sendMessage('test')).rejects.toThrow(')      'Network timeout')    );
});

  it('should handle authentication errors', async () => {
    ')    const __authError = createMockError('Authentication failed',    'AUTH_ERROR');')    vi.spyOn(provider as any, 'sendMessage').mockRejectedValue(authError);')
    await expect(provider.sendMessage('test')).rejects.toThrow(')      'Authentication failed')    );
});

  it('should handle rate limiting', async () => {
    ')    const __rateLimitError = createMockError('Rate limit exceeded',    'RATE_LIMIT');')    vi.spyOn(provider as any, 'sendMessage').mockRejectedValue(rateLimitError);')
  ...
  it('should handle malformed responses', async () => {
    ...
    const __response = await provider.sendMessage('test');')    expect(response).toBeNull();
});
});
