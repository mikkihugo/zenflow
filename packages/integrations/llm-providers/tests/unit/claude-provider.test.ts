...
describe('Claude Provider', () => {
  ...
  describe('executeClaudeTask', () => {
    ...
    ')      const __mockMessages = [
      ...
      mockClaudeSDK.sendMessage.mockResolvedValue({
        id: 'msg-123',        content: 'Hello! How can I help you today?',        role: 'assistant',});
    ...
    ')      const delay = (ms:number) =>
      ...
      await expect(
        executeClaudeTask('slow task', { timeoutMs:100})')      ).rejects.toThrow();
}, 1000);
  ...
  describe('executeSwarmCoordinationTask', () => {
    ...
    ')      const coordinationTask = {
        type: 'swarm-coordination',        agents:['agent1',    'agent2'],
        objective: 'Coordinate file analysis',};

      mockClaudeSDK.sendMessage.mockResolvedValue({
        content: 'Swarm coordination initiated',        metadata:{ agents: coordinationTask.agents},
});
    ...
    ')      const coordinationTask = {
        type: 'swarm-coordination',        agents:['researcher',    'analyzer',    'coordinator'],
        objective: 'Multi-agent analysis',        parallelExecution:true,
        maxConcurrency:3,
};
  ...
  describe('ClaudeTaskManager', () => {
    ...
    ')      const taskId = await taskManager.executeTask('status task');')      const status = await taskManager.getTaskStatus(taskId);

      expect(status).toBeDefined();
      expect(['pending',    'running',    'completed',    'failed']).toContain(status);')});

    it('should handle task cancellation', async () => {
    ')      const taskId = await taskManager.executeTask('long running task');')      const cancelled = await taskManager.cancelTask(taskId);
    ...
    ')      taskManager.addPermissionDenial('test-tool',    'Not allowed in test');')      const denials = taskManager.getPermissionDenials();
  ...
  describe('streamClaudeTask', () => {
    ...
    ')      const mockStream = async function* () {
        yield { type: 'start', taskId: ' stream-task-123'};')        yield type: 'content', content: ' Streaming';')        yield type: 'content', content: ' Claude';')        yield type: 'content', content: ' response';')        yield type: 'complete', result: ' Streaming Claude response';')};

      mockClaudeSDK.streamMessage.mockReturnValue(mockStream())();

      const stream = streamClaudeTask('stream test', { enableStreaming:true});')      const chunks = [];
      ...
      expect(chunks[0].type).toBe('start');')      expect(chunks[4].type).toBe('complete');')});

    it('should handle streaming errors', async () => {
    ')      const mockStream = async function* () {
        yield { type: 'start', taskId: ' error-stream'};')        throw new Error('Stream interrupted');')};
  ...
  describe('executeParallelClaudeTasks', () => {
    ...
    ')      const tasks = [
      ...
      mockClaudeSDK.sendMessage
        .mockResolvedValueOnce({ content: 'Result 1', taskId: '1'})')        .mockResolvedValueOnce(content: 'Result 2', taskId:'2' )')        .mockResolvedValueOnce(content: 'Result 3', taskId:'3' );')
    ...
    ')      const tasks = [
      ...
      mockClaudeSDK.sendMessage
        .mockResolvedValueOnce({ content: 'Success 1'})')        .mockRejectedValueOnce(new Error('Task failed'))')        .mockResolvedValueOnce(content:'Success 2' );')
      const results = await executeParallelClaudeTasks(tasks, {
        failFast:false,
});
  ...
  describe('filterMessagesForClaudeCode', () => {
    ...
    ')      const messages = [
      ...
      filtered.forEach((msg) => {
        expect(['user',    'assistant',    'system']).toContain(msg.role);')        expect(msg.content).toBeDefined();
});
});
    ...
    ')      const messages = [
      ...
      expect(filtered[0].content).toBe('First');')      expect(filtered[1].content).toBe('Second');')      expect(filtered[2].content).toBe('Third');')});
    ...
    ')      const messages = [
        ...
        { role: 'invalid'}, // Missing content')        null, // Null message
        { content: 'Missing role'}, // Missing role')        { role: 'user', content: ' Another valid message'},
];

      const filtered = filterMessagesForClaudeCode(messages as any);

      expect(filtered.length).toBe(2);
      expect(filtered[0].content).toBe('Valid message');')      expect(filtered[1].content).toBe('Another valid message');')});
});

  describe('cleanupGlobalInstances', () => {
    ')    it('should cleanup global instances', () => ')      // Create some global state
      getGlobalClaudeTaskManager();
  ...
  describe('Error Handling and Resilience', () => {
    ')    it('should handle Claude SDK initialization errors', async () => ')      vi.mocked(mockClaudeSDK.createSession).mockRejectedValue(
    ...
    ')      mockClaudeSDK.sendMessage.mockImplementation(
      ...
      await expect(
        executeClaudeTask('test', { timeoutMs:50})')      ).rejects.toThrow();
});
