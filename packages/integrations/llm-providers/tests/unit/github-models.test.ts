...
vi.mock('../../src/api/github-models', async () => {
    ')  const actual = await vi.importActual('../../src/api/github-models');')  return {
    ...actual,
...
describe('GitHub Models API', () => {
  ...
  beforeEach(() => {
    ...
    api = new GitHubModelsAPI({
      apiKey: 'test-api-key',      baseUrl: 'https://models.inference.ai.azure.com',      model:'gpt-4o')});
});
  ...
  describe('Constructor and Configuration', () => {
    ...
    ')      const customApi = new GitHubModelsAPI({
        apiKey: 'custom-key',        model: 'claude-3-sonnet',        maxTokens:8000,
        temperature:0.8
});
  ...
  describe('Request Handling', () => {
    ...
    ')      const mockStream = async function* () {
        yield { choices:[{ delta: { content: 'Hello'}}]};')        yield choices:[{ delta: { content: ' there!'}}];')        yield choices:[{ finish_reason: 'stop'}];')};

      api.streamRequest = vi.fn().mockReturnValue(mockStream())();

      const stream = api.streamRequest('stream test');')      const chunks = [];
  ...
  describe('Error Handling', () => {
    ...
    ')      const rateLimitError = createMockError('Rate limit exceeded',    'RATE_LIMITED');')      rateLimitError.status = 429;
    ...
    it('should handle authentication errors', async () => {
    ')      const authError = createMockError('Invalid API key',    'UNAUTHORIZED');')      authError.status = 401;
    ...
    ')      const notFoundError = createMockError('Model not found',    'NOT_FOUND');')      notFoundError.status = 404;
    ...
    ')      const timeoutError = createMockError('Request timeout',    'TIMEOUT');')      timeoutError.code = 'ETIMEDOUT';
  ...
  describe('Health and Monitoring', () => {
    ...
    ')      api.getHealth.mockResolvedValue(
        status: 'unhealthy',    ')        error: 'Service unavailable',        lastCheck:Date.now());

      const health = await api.getHealth();

      expect(health.status).toBe('unhealthy');')      expect(health.error).toBe('Service unavailable');')});
});
});
...
describe('createGitHubModelsProvider', () => {
  ...
  it('should create provider with custom configuration', () => {
    ')    const config = {
      apiKey: 'custom-key',      model: 'claude-3-sonnet',      temperature:0.9,
      maxTokens:8192
};
...
describe('executeGitHubModelsTask', () => {
  ...
  it('should execute simple tasks', async () => {
    ...
    expect(result.content).toBe('Task completed');')    expect(mockGitHubModels.sendRequest).toHaveBeenCalled();
});
  ...
  it('should handle task execution errors', async () => {
    ...
    await expect(executeGitHubModelsTask('failing task'))')      .rejects.toThrow('GitHub Models task failed');')});

  it('should handle different response formats', async () => {
    ')    const responses = [
      { choices:[{ message: { content: 'Response 1', role: ' assistant'}}]},
      { choices:[{ text: 'Response 2'}]}, // Alternative format')      { content: 'Response 3'} // Direct content')];
...
describe('Model-Specific Features', () => {
    ')  describe('GPT Models', () => ')    it('should handle GPT-4o specific features', async () => {
