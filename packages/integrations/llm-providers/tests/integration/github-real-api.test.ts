  ...
  describe('Real API Execution', () => {
    it('should execute real chat completion', async () => {
      ...
      const result = await copilotAPI.execute({
        messages:[{
          role: 'user',          content:'Write a simple TypeScript function that adds two numbers and returns the result. Be concise.')}]
});
  ...
  describe('Error Handling', () => {
    it('should handle invalid token gracefully', async () => {
      const invalidAPI = new GitHubCopilotAPI({
        token: 'gho_invalid_token_test',        model:'gpt-4')});
    ...
    it('should handle network errors', async () => {
      ...
      const invalidAPI = new GitHubCopilotAPI({
        token:copilotToken,
        model: 'gpt-4',        baseURL:'https://invalid-url-that-does-not-exist-12345.com')});
  ...
  describe('Provider Factory Integration', () => {
    it('should create provider via factory with real token', async () => {
      ...
      const result = await provider.execute({
        messages:[{
          role: 'user',          content:'Just respond with "factory test successful"')}]
});
