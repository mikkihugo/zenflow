...
describe('LLM Provider - Unit Tests (Vitest)', () => {
  ...
  describe('Role Management', () => {
    ...
    ')      llmProvider.setRole('researcher');')      const role = llmProvider.getRole();
      expect(role?.role).toBe('researcher');')});

    it('should have all defined roles available', () => {
    ')      const roleNames = Object.keys(SWARM_AGENT_ROLES);
      expect(roleNames).toContain('assistant');')      expect(roleNames).toContain('coder');')      expect(roleNames).toContain('analyst');')      expect(roleNames).toContain('researcher');')      expect(roleNames).toContain('coordinator');')      expect(roleNames).toContain('tester');')      expect(roleNames).toContain('architect');')});

    it('should provide correct capabilities for each role', () => {
    ')      llmProvider.setRole('coder');')      expect(llmProvider.getRole()?.capabilities).toContain('code-generation');')
      llmProvider.setRole('analyst');')      expect(llmProvider.getRole()?.capabilities).toContain('data-analysis');')
      llmProvider.setRole('researcher');')      expect(llmProvider.getRole()?.capabilities).toContain(
  ...
  describe('Global LLM Instance', () => {
    ...
    ')      const originalGlobal = getGlobalLLM();
      ...
      expect(sameGlobal.getRole()?.role).toBe('architect');')      expect(sameGlobal).toBe(originalGlobal);
});
});
});
...
describe('LLM Provider - Integration Tests (Real API)', () => {
  ...
  describe('Real Provider Calls', () => {
    ')    itIntegration(
      'should handle role-specific tasks',      async () => {
        // Test coder role
        llmProvider.setRole('coder');')        const codeResponse = await llmProvider.execute({
          messages:[
            {
              role: 'user',              content: 'Write a simple hello world function in TypeScript',},
],
});
        expect(codeResponse.success).toBe(true);
        expect(codeResponse.content).toContain('function');')
        // Test analyst role
        llmProvider.setRole('analyst');')        const analysisResponse = await llmProvider.execute({
          messages:[
    ...
    itIntegration('should handle different provider types',      async () => {
        // Test Claude Code CLI (default)
        const claudeProvider = new LLMProvider('claude-code');')        const claudeResponse = await claudeProvider.execute({
        ...
        try {
          const githubProvider = new LLMProvider('github-models-api');')          const githubResponse = await githubProvider.execute({
            messages:[{ role: 'user', content: ' Hello'}],
});
    ...
    itIntegration(
      'should respect timeout settings',      async () => {
    ...
    itIntegration(
      'should handle errors gracefully',      async () => {
...
describe('LLM Provider - Edge Cases', () => {
  ...
  it('should handle role switching during operations', () => {
    ')    llmProvider.setRole('coder');')    expect(llmProvider.getRole()?.role).toBe('coder');')
    llmProvider.setRole('researcher');')    expect(llmProvider.getRole()?.role).toBe('researcher');')
    llmProvider.setRole('analyst');')    expect(llmProvider.getRole()?.role).toBe('analyst');')});
  ...
  it('should handle rapid role switching', () => {
    ')    const roles:string[] = [
      'coder',      'analyst',      'researcher',      'architect',      'coordinator',];
