...
export class GitHubModelsAPI implements APIProvider {
  ...
  constructor(options:GitHubModelsOptions) {
    this['options'] = {
      model: 'openai/gpt-4.1',      baseURL: 'https://models.github.ai',      maxTokens:4000, // GitHub Models limit:4k output tokens
      temperature:0.7,
      ...options,
};
  ...
  async execute(request:APIRequest): Promise<APIResult> {
    try {
      ...
      const response = await fetch(
        ...
        {
          method: 'POST',          headers:{
            'Content-Type': ' application/json',            Authorization:`Bearer ${this.options.token}`,
},
          body:JSON.stringify({
      ...
      return ok({
        ...
        metadata:{
          model:this.options.model,
          provider: 'github-models',          tokens:data.usage?.total_tokens,
...
'}
    ...
    const workingModels = [
      'openai/gpt-4.1',      'openai/gpt-4o',      'openai/gpt-4o-mini',      'openai/o1',      'meta/llama-3.3-70b-instruct',      'mistral-ai/mistral-large-2411',      'deepseek/deepseek-r1',];
...
export const gitHubModelsConfig = {
  name: 'GitHub Models API',  description: 'Access to GitHub Models marketplace - various AI models',  version: '1.0.0',  capabilities:{
    conversational:true,
    multimodal:true,
    reasoning:true,
    coding:true,
  ...
  authentication:{
    type: 'bearer-token',    tokenFormat: 'ghp_xxx (Personal Access Token)',    scopes:['models'],
    requiredHeaders:{
      Accept: 'application/vnd.github+json',      'X-GitHub-Api-Version': '2022-11-28',},
},
  endpoints:{
    chat: 'https://models.github.ai/inference/chat/completions',    models: 'https://models.github.ai/models',},
  modelFormat:
