...
export class GitHubModelsAPI implements APIProvider {
  ...
  async listModels(): Promise<string[]> {
    try {
      ...
      if (response.ok) {
        ...
        if (Array.isArray(models) && models.length > 0) {
          ...
          logger.info(
            `📋 GitHub Models from catalog API:${modelIds.length} models`
          );
          logger.info(
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (modelIds.length > 0) {
              logger.info(
                `📋 GitHub Models from fallback (${endpoint}):${modelIds.length} models`
              );
              return modelIds;
            }
          }
    ...
    logger.info(
      `📋 Using known working models (final fallback):${workingModels.length} models`
    );
