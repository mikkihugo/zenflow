    ...
    try {
      ...
      for (const modelData of data['data'] || []) {
        ...
        const model:GitHubCopilotModelMetadata = {
          id:modelData.id,
          name:modelData.name,
          vendor:modelData['vendor'] || ' Unknown',          family:capabilities?.family || 'unknown',          version:modelData['version'] || modelData.id,
          ...
          category:modelData['model_picker_category'] || ' versatile',          supportsVision:!!vision,
          ...
          tokenizer:capabilities?.tokenizer || 'unknown',          type:capabilities?.type || 'chat',          lastUpdated:new Date(),
};
