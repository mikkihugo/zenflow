 ...
 *   messages:[{ 
 *     role: 'user', *     content:'Explain microservices architecture pros and cons') *}]
 *});
 * 
 * if (response.isOk()) {
 ...
 *   model: 'gpt-4o', *   temperature:0.1  // Deterministic for code generation
 *});
 *
 ...
 *   messages:[{ 
 *     role: 'user', *     content:'Write a detailed analysis of REST vs GraphQL') *}]
 *});
 *
...
export const API_ERROR_CODES = {
  MODEL_ERROR: 'MODEL_ERROR',  NETWORK_ERROR: 'NETWORK_ERROR',  AUTH_ERROR: 'AUTH_ERROR',  RATE_LIMIT: 'RATE_LIMIT',  VALIDATION_ERROR: 'VALIDATION_ERROR',} as const;
...
export class GitHubCopilotAPI implements APIProvider {
  ...
  constructor(options:GitHubCopilotOptions) {
    this['options'] = {
      ...
      baseURL: 'https://api.githubcopilot.com',      maxTokens:16384, // Updated to realistic Copilot limits
      temperature:0.7,
      stream:false,
      ...options,
};
...
export const gitHubCopilotConfig = {
  name: 'GitHub Copilot Chat API',  description: 'GitHub Copilot conversational AI for development assistance',  version: '1.0.0',  capabilities:{
    conversational:true,
    codeCompletion:true,
    agentic:true,
  ...
  authentication:{
    type: 'oauth-bearer-token',    tokenFormat: 'gho_xxx (OAuth token, NOT PAT)',    requirements:[
      'GitHub Copilot subscription',      'VSCode-style OAuth flow',      'OAuth token (gho_xxx format)',],
    requiredHeaders:{
      // eslint-disable-next-line @typescript-eslint/naming-convention
      'Copilot-Integration-Id':COPILOT_INTEGRATION_ID,
},
  ...
  endpoints:{
    chat: 'https://api.githubcopilot.com/chat/completions',    models: 'https://api.githubcopilot.com/models',},
};
