      ...
      if (response.ok) {
        ...
        if (data.data && Array.isArray(data.data)) {
          ...
          const modelIds = data.data
            .filter(
              (model) =>
                model.capabilities?.type === 'chat' || !model.capabilities?.type
            ) // Chat models only
            .map((model) => model.id)
            .filter(Boolean);

          logger.info(
            `📋 GitHub Copilot models from real API:${modelIds.length} chat models`
          );
          logger.info(
            `🎯 First few models:${modelIds.slice(0, 5).join(', ')}${modelIds.length > 5 ? '...' : ''}`
          );
        ...
        logger.warn(
          `GitHub Copilot models API returned ${response.status}:${response.statusText}`
        );
      }
