...
import { isEmail, isURL} from '@claude-zen/foundation';
...
export function findWorkspaceRoot(startPath:string): string | null {
  ...
  while (currentPath !== path.dirname(currentPath)) {
    ...
    const workspaceConfigs = [
      'pnpm-workspace.yaml',      'lerna.json',      'rush.json',      'nx.json',      'workspace.json',];
...
export function findProjectRoot(
  ...
  while (currentPath !== path.dirname(currentPath)) {
    ...
    const projectFiles = [
      'package.json',      'tsconfig.json',      'pyproject.toml',      'Cargo.toml',      'go.mod',      '.git',];
...
export function _sanitizeString(str:string): string {
  ...
  // Use foundation validation for proven sanitization
...
export function validateCommand(command:string): boolean {
  ...
  const allowedCommands = [
    'ls',    'dir',    'pwd',    'echo',    'cat',    'head',    'tail',    'grep',    'find',    'wc',    'sort',    'uniq',    'cut',    'node',    'npm',    'pnpm',    'yarn',    'git',    'tsc',    'eslint',];
