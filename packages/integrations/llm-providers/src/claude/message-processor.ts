...
import {
  _truncateForLogging as truncateForLogging,
  generateSessionId,
} from './utils';
...
export function processClaudeMessage(
  ...
  try {
    ...
    logger.debug(
      `Processed ${  type} message:${  truncateForLogging(content)}`
    );
    ...
    return {
      ...
      timestamp:Date.now(),
      metadata:{
        level: 'error',        source: 'message-processor',        messageId:`error_${  Date.now()}`,
},
};
}
}
...
function extractMessageTypeAndContent(msg:Record<string, unknown>):{
  ...
  if (msg['success'] !== undefined || msg[' exitCode'] !== undefined) {
    return {
      type: 'result',      content:extractContent(msg),
};
}
  ...
  if (msg['level'] || msg[' source'] === ' system') {
    return {
      type: 'system',      content:extractContent(msg),
};
}

  // Default to user message
  return {
    type: 'user',    content:extractContent(msg),
};
}
...
function extractAssistantMetadata(
  ...
  return {
    ...
    tokens:
      msg['tokens'] ||
      (msg['usage'] as JsonObject | undefined)?.[' total_tokens'],
...
function extractSystemMetadata(
  ...
  return {
    ...base,
    level:msg['level'] || ' info',    source:msg['source'] || ' system',    category:msg['category'],
};
}
