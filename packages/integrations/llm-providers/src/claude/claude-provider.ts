...
import type { ClaudeSDKOptions} from './claude-sdk';
import { executeClaudeTask} from './claude-sdk';
...
export const CLAUDE_SWARM_AGENT_ROLES:Record<string, SwarmAgentRole> = {
  assistant:{
    role: 'assistant',    systemPrompt:
      'You are a helpful AI assistant that provides accurate, concise responses and follows instructions carefully.',    capabilities:['general-assistance',    'question-answering',    'task-guidance'],
},

  coder:{
    role: 'coder',    systemPrompt:
      'You are an expert software engineer. Write clean, efficient, well-documented code. Follow best practices and coding standards. Request explicit permission for file operations and system commands.',    capabilities:[
      'code-generation',      'debugging',      'refactoring',      'code-review',],
},

  analyst:{
    role: 'analyst',    systemPrompt:
      'You are a data analyst and system architect. Analyze requirements, identify patterns, and provide strategic insights.',    capabilities:[
      'data-analysis',      'pattern-recognition',      'requirements-analysis',      'system-design',],
},

  researcher:{
    role: 'researcher',    systemPrompt:
      'You are a research specialist. Gather information, analyze sources, and provide comprehensive research summaries.',    capabilities:[
      'information-gathering',      'source-analysis',      'research-synthesis',      'fact-checking',],
},

  coordinator:{
    role: 'coordinator',    systemPrompt:
      'You are a project coordinator. Manage tasks, coordinate between team members, and ensure project goals are met.',    capabilities:[
      'task-management',      'team-coordination',      'project-planning',      'resource-allocation',],
},

  tester:{
    role: 'tester',    systemPrompt:
      'You are a QA engineer. Design tests, identify edge cases, validate functionality, and ensure quality standards.',    capabilities:[
      'test-design',      'quality-assurance',      'edge-case-identification',      'validation',],
},

  architect:{
    role: 'architect',    systemPrompt:
      'You are a system architect. Design scalable systems, make architectural decisions, and ensure technical excellence.',    capabilities:[
      'system-architecture',      'technical-design',      'scalability-planning',      'technology-selection',],
},
};
...
export class ClaudeProvider implements CLIProvider {
  ...
  executeAsAssistant(
    ...
    if (roleResult.isErr()) {
      return Promise.resolve(err(roleResult.error));
}
      ...
      const cliError:CLIError = {
        ...
        message:
          error instanceof Error
            ...
            : 'Task execution failed: Unknown error',        details:{
          providerId:this.id,
          currentRole:this.currentRole?.role,
},
        ...(error instanceof Error && { cause:error}),
  ...
  private async callClaudeCodeCLI(request:CLIRequest): Promise<CLIResponse> {
    ...
    try {
      ...
      const response:CLIResponse = {
        content,
        metadata:{
          provider: 'claude-code-cli',          model:`claude-3-${model}-20240229`,
          timestamp:Date.now(),
          usage:{
