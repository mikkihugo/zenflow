...
export const LLM_PROVIDER_CONFIG:Record<string, ProviderConfig> = {
  'github-models':{
    name: 'github-models',    displayName: 'GitHub Models (GPT-5)',    models:['openai/gpt-5',    'openai/gpt-4o',    'mistralai/codestral'],
    defaultModel: 'openai/gpt-5',    maxContextTokens:4000, // GPT-5 input limit
  ...
  copilot:{
    name: 'copilot',    displayName: 'GitHub Copilot (GPT-5)',    models:[
      'gpt-5',      'o3',      'o3-mini',      'o4-mini',      'gpt-4.1',      'gpt-4o',      'gpt-4',      'claude-opus-4',      'claude-sonnet-4',      'claude-3.5-sonnet',      'gemini-2.5-pro',      'gpt-3.5-turbo',],
    defaultModel: 'gpt-5',    maxContextTokens:1000000, // Maximum context window for advanced models
    ...
    api:{
      baseUrl: 'https://api.githubcopilot.com',      tokenPath: '~/.claude-zen/copilot-token.json',      headers:{
        'Content-Type': ' application/json',        'User-Agent': ' claude-code-zen/1.0',        'Copilot-Integration-Id': ' vscode-chat',},
      authType: 'bearer',},
    features:{
      structuredOutput:true,
      fileOperations:false,
  ...
  'claude-code':{
    name: 'claude-code',    displayName: 'Claude Code CLI',    models:['sonnet',    'opus'],
    defaultModel: 'sonnet',    maxContextTokens:200000, // Claude's large context
    maxOutputTokens:128000, // Claude's large output limit
    features:{
      structuredOutput:true,
  ...
  gemini:{
    name: 'gemini',    displayName: 'Gemini CLI',    models:['gemini-pro',    'gemini-flash'],
    defaultModel: 'gemini-pro',    maxContextTokens:1000000, // Very large context
  ...
  'gemini-direct':{
    name: 'gemini-direct',    displayName: 'Gemini 2.5 Flash (Main)',    models:['gemini-2.5-flash',    'gemini-2.5-pro',    'gemini-1.5-pro'],
  ...
  'gemini-pro':{
    name: 'gemini-pro',    displayName: 'Gemini 2.5 Pro (Complex)',    models:['gemini-2.5-pro',    'gemini-1.5-pro'],
...
export const ROUTING_STRATEGY:RoutingStrategy = {
  ...
  RULES:{
    ...
    largeContext:[
      'copilot',      'github-models',      'gemini-direct',      'claude-code',      'gemini',],
    ...
    codeAnalysis:[
      'copilot',      'claude-code',      'github-models',      'gemini-direct',      'gemini-pro',],
    ...
    structuredOutput:[
      'copilot',      'github-models',      'gemini-direct',      'gemini-pro',      'claude-code',      'gemini',],
},
};
...
function meetsFeatureRequirements(
  ...
  const { requiresFileOps, requiresCodebaseAware, requiresStructuredOutput} =
    context;
...
function sortProvidersByPriority(candidates:string[]): string[] {
  return candidates.sort((a, b) => {
    ...
    if (!configA || !configB) {
      return 0; // Skip comparison if configs don't exist
}
...
export function getOptimalProvider(context:ProviderRoutingContext): string[] {
  ...
  for (const [providerId, config] of Object.entries(LLM_PROVIDER_CONFIG)) {
    ...
    const meetsContextRequirements = meetsContextSizeRequirements(
      config,
      isSmallContext,
      isLargeContext
    );
