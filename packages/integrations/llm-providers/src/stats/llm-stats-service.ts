...
export class LLMStatsService {
    ...
    const record:LLMCallRecord = {
      ...
      metadata:{
        ...
        requiresCodebaseAware:
          request['task'] === ' domain-analysis' ||
          request['task'] === ' code-review',        taskComplexity:this.assessTaskComplexity(request),
  ...
  private calculateSingleProviderStats(
    ...
    return {
      ...
      successRate:
        calls['length'] > 0 ? successfulCalls[' length'] / calls[' length'] :0,
      averageResponseTime:
  ...
  private calculateRoutingStats(calls:LLMCallRecord[]): LLMRoutingStats {
    ...
    const commonRoutingPatterns = Array.from(patterns.entries())
      .map(([pattern, stats]) => ({
        ...
        successRate:
          stats['count'] > 0 ? stats[' successes'] / stats[' count'] :0,
}))
      .sort((a, b) => b['frequency'] - a.frequency);
    ...
    return {
      ...
      optimalRoutingRate:
        calls['length'] > 0 ? optimalRoutes / calls[' length'] :0,
  ...
  private calculateSystemHealth(calls:LLMCallRecord[]): LLMSystemHealth {
    ...
    if (errorRate > 0.1) {
      alerts.push({
        level: 'warning',        message:`High error rate detected: ${Math.round(errorRate * 100)}%`,
        timestamp:new Date(),
});
}
    ...
    if (averageLatency > 5000) {
      alerts.push({
        level: 'error',        message:`High latency detected: ${Math.round(averageLatency)}ms average`,
        timestamp:new Date(),
});
}
  ...
  private calculateTrends(
    ...
    for (
      ...
      const bucketEnd = new Date(
        Math.min(time + intervalMs, range.end.getTime())
      );
      const bucketCalls = calls.filter(
        (call) =>
          call['timestamp'] >= bucketStart && call[' timestamp'] < bucketEnd
      );
    ...
    const successRate = buckets.map((bucket) => ({
      ...
      rate:
        bucket.calls['length'] > 0
          ? bucket.calls.filter((call) => call.success)['length'] /
            bucket.calls.length
          :0,
}));
    ...
    const latency = buckets.map((bucket) => ({
      ...
      ms:
        bucket.calls['length'] > 0
          ? bucket.calls.reduce((sum, call) => sum + call.executionTime, 0) /
            bucket.calls.length
          :0,
}));
  ...
  private generateInsights(
    ...
    return {
      topPerformingProvider:sortedByReliability[0]?.providerId || 'none',      mostEfficientProvider:sortedByEfficiency[0]?.providerId || 'none',      bottlenecks:this.identifyBottlenecks(providerStats),
  ...
  > {
    ...
    for (const task of taskTypes) {
      ...
      result[task] = {
        preferredProvider:sorted[0]?.[0] || 'none',        alternativeProviders:sorted.slice(1).map(([provider]) => provider),
        successRate:
          taskCalls['length'] > 0
  ...
  private calculateProviderUsageTrend(
    ...
    return buckets.map((bucket) => {
      const providerCallCount = bucket.calls.reduce((count, call) => call['provider'] === provider ? count + 1:count, 0);
  ...
  private exportToCsv():string {
    const headers = [
      'timestamp',      'provider',      'task',      'success',      'executionTime',      'contextLength',      'fallbackCount',      'routingReason',].join(',    ');
