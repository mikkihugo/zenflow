...
export class GRPOML extends Teleprompter {
    ...
    policyUpdateHistory = [];
    experienceHistory = [];
    rewardShapingHistory = [];
    // Performance tracking
    startTime;
    bestPolicyReward = -Infinity;
     ...
     */
    async performPolicyGradientTraining(student, options) {
        ...
        while (this.currentEpisode < this.config.maxEpisodes &&
            ...
            for (let step = 0; step < this.config.maxStepsPerEpisode; step++) {
                ...
                const { nextState, reward, done } = await this.executeAction(student, state, action, options);
            ...
            if (this.config.useExperienceReplay &&
                ...
                if (updateResult.trustRegionViolation &&
    ...
    initializeEpisodeState(student) {
    ...
    async executeAction(student, state, action, options) {
    ...
    async computeAdvantagesForEpisode(experiences) {
    ...
    async adaptTrustRegion(updateResult) {
    ...
    async validatePolicyImprovement() {
        const tests = [];
    ...
    async analyzeExperienceData() {
        ...
        const rewardVariance = rewards.reduce((sum, r) => sum + Math.pow(r - meanReward, 2), 0) /
            rewards.length;
    ...
    calculateAverageReturn() {
        if (this.episodeRewards.length === 0)
            return 0;
        return (this.episodeRewards.reduce((sum, reward) => sum + reward, 0) /
            this.episodeRewards.length);
    }
    checkPolicyConvergence() {
    ...
    generateExplorationRateHistory() {
    ...
    async calculateRewardShapingStats() {
    ...
    async calculateExperienceReplayStats() {
    ...
    async calculateBanditPerformance() {
    ...
    generatePolicyInsights() {
        ...
        const avgEntropy = this.policyUpdateHistory.reduce((sum, update) => sum + update.entropyBonus, 0) / Math.max(this.policyUpdateHistory.length, 1);
        if (avgEntropy > 0.05) {
    ...
    async createOptimizedModule(student) {
        ...
        optimizedModule.policyParameters = this.currentPolicy?.parameters;
 ...
 */
export function createGRPOML(config) {
