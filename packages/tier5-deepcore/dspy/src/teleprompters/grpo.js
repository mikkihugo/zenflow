...
export class GRPO extends FinetuneTeleprompter {
        ...
        if (valset !== null) {
            ...
            if (this.report_train_scores) {
                ...
                const valset_scores = valset_evaluation.results
                    ...
                    .map((r) => r[r.length - 1]);
                const trainset_agg = trainset_scores.reduce((a, b) => a + b, 0) /
                    trainset_scores.length;
                const valset_agg = valset_scores.reduce((a, b) => a + b, 0) /
            ...
            if (this.report_train_scores) {
                ...
                if (!(Number.isInteger(this.num_steps_for_val) &&
    ...
    update_shuffled_trainset(original_trainset) {
        ...
        const num_to_pad = this.num_dspy_examples_per_grpo_step -
    ...
    select_training_sample_and_update_shuffled_trainset(original_trainset, train_step_idx) {
        ...
        if (this.shuffled_trainset_ids.length %
    ...
    async compile(student, config) {
        ...
        if (!teachers.includes(student)) {
            throw new Error(`Student program ${student} is not in the list of teachers ${teachers}. Please provide the student program as one of the teachers. ` + 'Alternatively, you can leave the teacher argument as None, and the student program will be used as the teacher program.');
        }
        ...
        for (let train_step_idx = 0; train_step_idx < this.num_train_steps; train_step_idx++) {
            ...
            for (let pred_id = 0; pred_id < num_student_predictors; pred_id++) {
                for (const [example_ind, example_data] of trace_data.entries()) {
                    ...
                    for (const teacher_data of example_data) {
                        for (const sample of teacher_data) {
                            ...
                            const trace_instances_for_current_pred = sample['trace']
                                .filter((t) => this.hash_signature(t[0].signature) ===
                                this.hash_signature(student.predictors()[pred_id].signature))
                    ...
                    else if (predictor_example_invocations.length !==
                    ...
                    const example_training_data = Array(final_max_len)
                        .fill(null)
                        .map(() => []);
                        ...
                        for (let rollout_idx = 0; rollout_idx < processed_invocations.length; rollout_idx++) {
                            if (group_idx < processed_invocations[rollout_idx].length) {
                                ...
                                if (this.is_failed_prediction(trace_instance[2])) {
                                    ...
                                    if (JSON.stringify(all_messages.slice(0, -1)) !==
            ...
            for (const predictor_train_batch of train_batch_per_predictor) {
                for (const grpo_train_group of predictor_train_batch) {
                    ...
                    if (new Set(grpo_train_group.map((item) => JSON.stringify(item))).size <
                        2) {
