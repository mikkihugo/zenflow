...
fn generate_comparison_data(
  ...
  let values: Vec<f64> = (0..size)
    .map(|i| {
      ...
      let seasonal =
        (i as f64 * 2.0 * std::f64::consts::PI / 168.0).sin() * 10.0; // Weekly pattern
...
fn bench_mlp_comparison(c: &mut Criterion) {
  ...
  for (size_name, size) in data_sizes {
    ...
    if let Some(python_result) = python_results
      .iter()
      .find(|r| r.model == "MLP" && r.data_size == size)
    {
      println!(
        "Python MLP {} - Time: {:.2}s, Memory: {:.1}MB",
...
fn bench_lstm_comparison(c: &mut Criterion) {
  ...
  for (config_name, num_series, seq_len) in sequence_configs {
    ...
    if let Some(python_result) =
      python_results.iter().find(|r| r.model == "LSTM")
    {
      println!(
        "Python LSTM {} - Time: {:.2}s, Memory: {:.1}MB",
...
fn bench_inference_comparison(c: &mut Criterion) {
  ...
  let batch_sizes =
    vec![("single", 1), ("small_batch", 32), ("large_batch", 256)];
  ...
  for (batch_name, batch_size) in batch_sizes {
    ...
    for (model_name, model) in &models {
      ...
      println!(
        "Python {} {} - Expected: ~{:.1}ms",
        model_name, batch_name, python_time_ms
      );
    }
  }
    ...
    group.bench_function(config_name, |b| {
      b.iter(|| {
        match model_type {
          "MLP" => {
            let config =
              MLPConfig::new(100, 10).with_hidden_layers(layers.clone());
...
fn bench_data_processing_comparison(c: &mut Criterion) {
  ...
  let data_sizes =
    vec![("small", 10000), ("medium", 100000), ("large", 1000000)];
  ...
  for (size_name, size) in data_sizes {
    ...
    println!(
      "Python preprocessing {} - Time: ~{:.1}ms",
      size_name, python_time_ms
    );
  }
...
fn convert_to_training_data(
  ...
  for i in 0..(values.len() - window_size - horizon) {
    ...
    let target_window =
      values[i + window_size..i + window_size + horizon].to_vec();
