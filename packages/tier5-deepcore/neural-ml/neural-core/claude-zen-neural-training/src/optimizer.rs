  ...
  fn initialize_buffers(&mut self, parameters: &[Vec<T>]) {
    if self.momentum.is_empty() {
      self.momentum = parameters
        ...
        .collect();
      self.variance = parameters
        .iter()
        .map(|layer| vec![T::zero(); layer.len()])
        .collect();

      if self.amsgrad {
        self.max_variance = parameters
          .iter()
          .map(|layer| vec![T::zero(); layer.len()])
          .collect();
      }
...
impl<T: Float + Send + Sync> Optimizer<T> for Adam<T> {
  fn step(
    ...
    if parameters.len() != gradients.len() {
      return Err(TrainingError::OptimizerError(
        "Parameter-gradient length mismatch".to_string(),
      ));
    }
    ...
    for (layer_idx, (params, grads)) in
      parameters.iter_mut().zip(gradients.iter()).enumerate()
    {
      for (param_idx, (&grad, param)) in
        grads.iter().zip(params.iter_mut()).enumerate()
      {
        // Update biased first moment estimate
        self.momentum[layer_idx][param_idx] = self.beta1
          * self.momentum[layer_idx][param_idx]
          + (T::one() - self.beta1) * grad;

        // Update biased second raw moment estimate
        self.variance[layer_idx][param_idx] = self.beta2
        ...
        let variance_hat = if self.amsgrad {
          ...
          self.max_variance[layer_idx][param_idx] = self.max_variance
            [layer_idx][param_idx]
            .max(self.variance[layer_idx][param_idx]);
        ...
        // Update parameters
        *param = *param
          - step_size * self.momentum[layer_idx][param_idx]
...
impl<T: Float + Send + Sync> Optimizer<T> for AdamW<T> {
    ...
    if !self.weight_decay.is_zero() {
      for params in parameters.iter_mut() {
        for param in params.iter_mut() {
          *param =
            *param * (T::one() - self.adam.learning_rate * self.weight_decay);
        }
      }
    }
  ...
  fn state(&self) -> OptimizerState<T> {
    ...
    state
      .additional_state
      .insert("weight_decay".to_string(), vec![self.weight_decay]);
    state
  }

  fn restore_state(
    &mut self,
    mut state: OptimizerState<T>,
  ) -> TrainingResult<()> {
...
impl<T: Float + Send + Sync> SGD<T> {
  ...
  fn initialize_buffers(&mut self, parameters: &[Vec<T>]) {
    if self.momentum_buffers.is_empty() && !self.momentum.is_zero() {
      self.momentum_buffers = parameters
        .iter()
        .map(|layer| vec![T::zero(); layer.len()])
        .collect();
    }
      ...
      parameters.iter_mut().zip(gradients.iter()).enumerate()
    {
        ...
        grads.iter().zip(params.iter_mut()).enumerate()
      {
        ...
        if !self.momentum.is_zero() {
          let momentum_buffer =
            &mut self.momentum_buffers[layer_idx][param_idx];
...
impl<T: Float + Send + Sync> RMSprop<T> {
  ...
  fn initialize_buffers(&mut self, parameters: &[Vec<T>]) {
    if self.square_avg.is_empty() {
      self.square_avg = parameters
        .iter()
        .map(|layer| vec![T::zero(); layer.len()])
        .collect();

      if !self.momentum.is_zero() {
        self.momentum_buffer = parameters
          .iter()
          .map(|layer| vec![T::zero(); layer.len()])
          .collect();
      }
  ...
  fn step(
      ...
      parameters.iter_mut().zip(gradients.iter()).enumerate()
    {
        ...
        grads.iter().zip(params.iter_mut()).enumerate()
      {
        ...
        self.square_avg[layer_idx][param_idx] = self.alpha
        ...
        if self.centered {
          ...
          self.grad_avg[layer_idx][param_idx] = self.alpha
        ...
        if !self.momentum.is_zero() {
          ...
          *param = *param
            - self.learning_rate * self.momentum_buffer[layer_idx][param_idx];
        } else {
...
impl<T: Float + Send + Sync> ForecastingAdam<T> {
  ...
  fn apply_seasonal_correction(&self, gradients: &mut [Vec<T>]) {
    if !self.seasonal_correction
      || self.gradient_history.len() < self.lookback_window
    {
      return;
    }
    ...
    for layer_idx in 0..gradients.len() {
      for param_idx in 0..gradients[layer_idx].len() {
        ...
        for step in (0..self.gradient_history.len()).step_by(7) {
          ...
          {
            seasonal_avg =
              seasonal_avg + self.gradient_history[step][layer_idx][param_idx];
            count += 1;
          }
        }
...
impl<T: Float + Send + Sync> Optimizer<T> for ForecastingAdam<T> {
  ...
  fn restore_state(
    ...
    if let Some(temporal_momentum) =
      state.additional_state.remove("temporal_momentum")
    {
