 ...
 */
export type QuestionType = 'approval' | 'choice' | 'input' | 'confirmation' | 'review' | 'validation' | 'checkpoint' | 'emergency' | 'custom' | 'relevance' | 'boundary' | 'relationship' | 'naming' | 'priority';
/**
 * AGUI adapter types
 */
export declare enum AGUIType {
    TERMINAL = "terminal",
    WEB = "web",
    MOCK = "mock",
    HEADLESS = "headless"
 ...
 */
export type AGUIFactory<T extends AGUIInterface = AGUIInterface> = (config?: AGUIConfig) => T | Promise<T>;
/**
 * Registry of AGUI factories
 */
export interface AGUIRegistry {
    register<T extends AGUIInterface>(type: string, factory: AGUIFactory<T>): void;
    create<T extends AGUIInterface>(type: string, config?: AGUIConfig): T | Promise<T>;
 ...
 * @extends AGUIError
 */
export declare class AGUITimeoutError extends AGUIError {
 ...
 * @extends AGUIError
 */
export declare class AGUIValidationError extends AGUIError {
    readonly input: string;
    constructor(message: string, input: string, context?: Record<string, unknown>);
}
