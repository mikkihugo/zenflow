  ...
  return {
    async processRequest(
      ...
      return {
        ...
        changes:[
          {
            type: 'analysis',            path: 'README.md',            reasoning:`File-aware AI analysis complete for task: ${request.task}`,`
},
],
        metadata:{
          filesAnalyzed:request.files?.length||0,
          provider:config.provider,
          model:config.model||'default',          executionTime:Date.now() - startTime,
},
};
},
