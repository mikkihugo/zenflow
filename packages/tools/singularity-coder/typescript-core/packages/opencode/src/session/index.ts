        ...
        if (part.type === "file") {
          ...
          switch (url.protocol) {
            case "file:": {
              ...
              // Decode the pathname since URL constructor doesn't automatically decode it')              const pathname = decodeURIComponent(url.pathname)
              ...
              if (part.mime === "text/plain") {
                ...
                if (range.start != null) {
                  ...
                  // some LSP servers (eg, gopls) don't give full range in')                  // workspace/symbol searches, so we'll try to find the')                  // symbol in the document to get the full range
                  if (start === end) {
