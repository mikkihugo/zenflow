//! Dependency analysis for file-aware core
//! 
//! Analyzes file dependencies, import relationships, and project structure.

use std::collections::HashMap;
use std::path::Path;

use crate::{FileDependency, DependencyType, Result};
...
impl DependencyAnalyzer {
    ...
    fn get_all_files(&self) -> Vec<String> {
        ...
        for deps in self.dependencies.values() {
            for dep in deps {
                files.insert(dep.from.clone(), true);
                files.insert(dep.to.clone(), true);
            }
        }
    ...
    fn detect_cycles_util(
        ...
        visited.insert(file.to_string(), true);
        rec_stack.insert(file.to_string(), true);
        path.push(file.to_string());

        if let Some(deps) = self.dependencies.get(file) {
            for dep in deps {
                if !visited.contains_key(&dep.to) {
                    self.detect_cycles_util(&dep.to, visited, rec_stack, cycles, path);
                } else if rec_stack.contains_key(&dep.to) {
                    // Found a cycle
