...
// Foundation package integration for database path utilities
// This would be imported from the actual foundation package when available
mod foundation_integration {
    use super::*;
    
    /// Foundation database path utilities
    pub struct FoundationDatabasePaths;
    
    impl FoundationDatabasePaths {
        /// Get base directory from foundation package
        pub fn get_base_directory() -> Result<PathBuf> {
            // Try to get from environment variable first
            if let Ok(base_dir) = env::var("CLAUDE_ZEN_BASE_DIR") {
            ...
            Ok(home_dir.join(".claude-zen"))
        }
        
        /// Get database configuration from foundation package
        pub fn get_database_config() -> Result<DatabaseConfig> {
            // This would integrate with foundation package configuration
            // For now, return a default configuration
            Ok(DatabaseConfig::default())
        }
        
        /// Get database paths from foundation package
        pub fn get_database_paths() -> Result<DatabasePaths> {
            let base_dir = Self::get_base_directory()?;
            
            Ok(DatabasePaths {
                base_directory: base_dir.clone(),
                data_directory: base_dir.join("data"),
                cache_directory: base_dir.join("cache"),
                models_directory: base_dir.join("models"),
                logs_directory: base_dir.join("logs"),
    ...
    impl DatabasePaths {
        ...
        /// Get database-specific path
        pub fn get_database_path(&self, db_type: &DatabaseType, db_name: &str) -> PathBuf {
            match db_type {
                DatabaseType::SQLite => self.data_directory.join(format!("{}.db", db_name)),
                DatabaseType::LanceDB => self.data_directory.join(db_name),
                DatabaseType::Kuzu => self.data_directory.join(db_name),
                DatabaseType::PostgreSQL => self.data_directory.join("postgresql"),
                DatabaseType::MySQL => self.data_directory.join("mysql"),
            }
        }
        
        /// Get cache path for database operations
        pub fn get_cache_path(&self, operation: &str) -> PathBuf {
            self.cache_directory.join(format!("{}_{}.cache", operation, chrono::Utc::now().timestamp()))
        }
        
        /// Get log path for database operations
        pub fn get_log_path(&self, operation: &str) -> PathBuf {
            self.logs_directory.join(format!("{}_{}.log", operation, chrono::Utc::now().format("%Y%m%d")))
        }
    }
}

// Re-export foundation integration for use in the module
use foundation_integration::{FoundationDatabasePaths, DatabasePaths};
