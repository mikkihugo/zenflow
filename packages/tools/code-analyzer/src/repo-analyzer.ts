...
export class RepoAnalyzer {
  ...
  private async analyzeBuildSystem(
    ...
    try {
      switch (workspace.tool) {
        case 'nx':
          analysis.dependencyGraph = await this.getNxDependencyGraph(
            workspace.root
          );
          break;
        case 'pnpm':
          analysis.dependencyGraph = await this.getPnpmWorkspaceInfo(
            workspace.root
          );
          break;
        case 'lerna':
          analysis.dependencyGraph = await this.getLernaDependencies(
            workspace.root
          );
          break;
        default:
  ...
  private async detectDomains(
    ...
    for (const project of workspace.projects) {
      ...
      if (pathParts.includes('core')) {
        domainKey = pathParts.includes('foundation')
          ? 'public-api'
          : 'deep-core';
  ...
  private async detectViolations(
    ...
    for (const project of workspace.projects) {
      try {
        ...
        const packageViolations = await this.analyzePackageImports(
          projectPath,
          domains
        );
    ...
    try {
      ...
      for (const file of files) {
        try {
          ...
          const fileViolations = await this.analyzeFileImports(
            file,
            content,
            domains
          );
  ...
  private isForbiddenImport(importPath: string): boolean {
    ...
    return (
      importPath.includes('node_modules') || importPath.startsWith('../../../')
    );
  }
