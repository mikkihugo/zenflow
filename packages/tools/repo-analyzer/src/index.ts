...
export class RepoAnalyzer extends EventEmitter {
  ...
  async analyzeDomainBoundaries() {
    this.emit('analysis:domain:started', {
    ...
    try {
      ...
      validator.on('repository:validation:started', (data:unknown) =>
        this.emit('repository:validation:started', data)
      );
      validator.on('repository:validation:completed', (data:unknown) =>
        this.emit('repository:validation:completed', data)
      );
      validator.on('repository:validation:failed', (data:unknown) =>
