...
export class DependencyAnalyzer {
  ...
  private async resolveDependencyPath(
    ...
    ')        // Relative import
        const path = await import('path');')        const resolved = path.resolve(path.dirname(fromFile), depName);

        // Try different extensions
        const fs = await import('fs/promises');')        const extensions = [
          '.ts',          '.tsx',          '.js',          '.jsx',          '.mts',          '.cts',          '.mjs',          '.cjs',          '/index.ts',          '/index.js',];
  ...
  private extractDependenciesManual(content:string): string[] {
    ...
    // Dynamic imports
    const dynamicImportRegex = /import\s*\(\s*['"]([@\w.-/]+)['"]\s*\)/g;')    while ((match = dynamicImportRegex.exec(content)) !== null) {
      dependencies.add(match[1]);
}
    ...
    if (madgeResult) {
      try {
        ...
        for (const cycle of madgeCycles) {
          cycles.push({
            cycle,
            severity:cycle.length > 3 ? 'error' : ' warning',            impactScore:this.calculateCycleImpact(cycle, dependencyMap),
  ...
  private generateCycleBreakingSuggestions(cycle:string[]): string[] {
    const suggestions = [
      'Extract common functionality into a shared module',      'Use dependency injection to invert control',      'Apply the Interface Segregation Principle',      'Consider using events or observers instead of direct dependencies',];
  ...
  private detectClusters(
    ...
    for (const node of nodes) {
      const pathParts = node.file.split('/');')      const clusterKey = pathParts.slice(0, -1).join('/'); // Directory path')
  ...
  private inferDomain(path:string): string {
    const segments = path.split('/').filter(Boolean);')    if (segments.includes('components')) return ' ui;
  ...
  private estimateAbstractness(filePath:string): number {
    ...
    if (fileName.includes('base')||fileName.includes(' core')) return 0.6;')    return 0.2;
}
