...
export class ComplexityAnalyzer {
  private logger = getLogger('ComplexityAnalyzer');')  private project?:Project;
  ...
  async initialize(
    ...
    try {
      this.project = new Project({
        ...
        skipFileDependencyResolution:options?.performanceMode === 'fast',        compilerOptions:{
          allowJs:true,
      ...
      this.logger.warn(
        'Failed to initialize TypeScript project, falling back to file-by-file analysis',        error
      );
}
    ...
    ')        const sourceFile = this.project?.getSourceFile(filePath);
      ...
      try {
        ast = babelParse(content, {
          ...
          plugins:[
            'typescript',            'jsx',            'decorators-legacy',            'dynamicImport',            'asyncGenerators',            'functionBind',            'exportDefaultFrom',            'objectRestSpread',            'classProperties',            'optionalChaining',            'nullishCoalescingOperator',],
});
} catch {
        // Fall back to Acorn
  ...
  private async detectCodeSmells(
    ...
    for (const method of methods) {
      if (method.lines > this.thresholds.linesOfCode) {
        smells.push({
          type: 'long-method',          severity:
            method.lines > this.thresholds.linesOfCode * 2 ? 'high' : ' medium',          file:filePath,
    ...
    for (const duplicate of duplicates) {
      smells.push({
        type: 'duplicate-code',        severity: 'medium',        file:filePath,
    ...
    for (const dead of deadCode) {
      smells.push({
        type: 'dead-code',        severity: 'low',        file:filePath,
        startLine:dead.line,
        endLine:dead.line,
  ...
  private async readFile(filePath:string): Promise<string|null> 
    try {
      const fs = await import('node:fs/promises');')      return await fs.readFile(filePath, 'utf-8');')} catch {
      return null;
}
