...
export class RepositoryAnalyzer {
  ...
  async analyze(_options:AnalysisOptions = {}):Promise<AnalysisResult> {
    ...
    const analysisOptions:AnalysisOptions = {
      ...
      analysisDepth: 'moderate',      enableGitAnalysis:true,
      enableComplexityAnalysis:true,
      enableDependencyAnalysis:true,
      enableDomainAnalysis:true,
      performanceMode: 'balanced',      ...options,
};
    ...
    try {
      ...
      return {
        ...
        summary,
        exportOptions:[
          'json',          'yaml',          'csv',          'html',          'markdown',          'pdf',          'graphml',          'dot',],
};
} catch (error) {
      this.logger.error('Repository analysis failed:', error);')      throw error;
}
}
  ...
  async getHealthScore(options:AnalysisOptions = {}):Promise<{
    ...
    const quickOptions:AnalysisOptions = {
      ...options,
      performanceMode: 'fast',      analysisDepth: 'shallow',};
    ...
    return {
      ...
      criticalIssues:result.recommendations
        .filter((r) => r.priority === 'urgent')')        .map((r) => r.title),
};
}
  ...
  private async analyzeWorkspace(options:AnalysisOptions): Promise<any> {
    ...
    try {
      ...
      this.logger.warn('Workspace analysis failed:', error);')      return null;
}
}
  ...
  private async analyzeComplexity(
    ...
    try {
      ...
      this.logger.warn('Complexity analysis failed:', error);')      return this.getEmptyComplexity();
}
}
  ...
  private async analyzeDependencies(options:AnalysisOptions): Promise<any> {
    ...
    try {
      ...
      this.logger.warn('Dependency analysis failed:', error);')      return this.getEmptyDependencies();
}
}
  ...
  private async analyzeGit(options:AnalysisOptions): Promise<any> {
    ...
    try {
      ...
      this.logger.warn('Git analysis failed:', error);')      return undefined;
}
}
  ...
  private async getSourceFiles(options:AnalysisOptions): Promise<string[]> {
    const patterns = [
      '**/*.{ts,tsx,js,jsx,mts,cts,mjs,cjs,py,go,rs,java,cpp,c,h,hpp}',      ...(options.includeTests
        ? []
        :[
            '!**/*.{test,spec}.{ts,tsx,js,jsx}',            '!**/test/**',            '!**/tests/**',            '!**/__tests__/**',]),
      ...(options.includeNodeModules ? [] :['!**/node_modules/**']),
      ...(options.includeDotFiles ? [] :['!**/.*']),
      ...(options.excludePatterns||[]).map((p) => `!$p`),'!**/dist/**',      '!**/build/**',      '!**/.git/**',];
    ...
    if (options.maxFileSize) {
      const fs = await import('fs/promises');')      const filteredFiles:string[] = [];
  ...
  private async countTotalLines(files:string[]): Promise<number> {
    const fs = await import('fs/promises');')    let totalLines = 0;

    await Promise.all(
      files.map(async (file) => {
        try {
          const content = await fs.readFile(file, 'utf-8');')          totalLines += content.split('\n').length;')} catch {
  ...
  private async detectLanguages(
    ...
    const fs = await import('fs/promises');')    const languages:Record<string, number> = {};

    for (const file of files) {
      try {
        const content = await fs.readFile(file, 'utf-8');')        const lines = content.split('\n').length;')        const ext = file.split('.').pop()?.toLowerCase();')
        let language = 'unknown';
        switch (ext) {
          case 'ts': ')'          case 'tsx': ')'          case 'mts': ')'          case 'cts': ')'            language = 'typescript';
            break;
          case 'js': ')'          case 'jsx': ')'          case 'mjs': ')'          case 'cjs': ')'            language = 'javascript';
            break;
          ...
          case 'java': ')'            language = 'java';
            break;
          case 'cpp': ')'          case 'cc': ')'          case 'cxx': ')'            language = 'cpp';
            break;
          case 'c': ')'            language = 'c';
            break;
          case 'h': ')'          case 'hpp': ')'            language = 'header';
            break;
}
  ...
  private generateSummary(
    ...
    if (complexityScore > 0.8) strengths.push('Low code complexity');')    else if (complexityScore < 0.5) weaknesses.push('High code complexity');')
    if (dependencyScore > 0.8) strengths.push('Well-managed dependencies');')    else if (dependencyScore < 0.5)
      weaknesses.push('Dependency management issues');')
    if (maintainabilityScore > 0.8) strengths.push('High maintainability');')    else if (maintainabilityScore < 0.5) weaknesses.push('Low maintainability');')
    if (gitScore > 0.8) strengths.push('Healthy git practices');')    else if (gitScore < 0.5) weaknesses.push('Git workflow issues');')
    return {
      overallScore,
      strengths,
      weaknesses,
      ...
      riskAssessment:{
        technicalDebtRisk:
          repository.complexity.technicalDebt > 100 ? 'high' : ' medium',        maintainabilityRisk:maintainabilityScore < 0.5 ? 'high' : ' medium',        scalabilityRisk:
          repository.dependencies.circularDependencies.length > 5
            ? 'high')            : 'low',        teamVelocityRisk:criticalIssues > 10 ? 'high' : ' medium',},
};
}
