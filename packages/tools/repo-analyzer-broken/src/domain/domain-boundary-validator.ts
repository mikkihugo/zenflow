 ...
 */
export enum Domain {
  COORDINATION = 'coordination',  WORKFLOWS = 'workflows',  NEURAL = 'neural',  DATABASE = 'database',  MEMORY = 'memory',  KNOWLEDGE = 'knowledge',  OPTIMIZATION = 'optimization',  INTERFACES = 'interfaces',  CORE = 'core',}
...
export class DomainBoundaryValidator implements DomainBoundary {
  ...
  private readonly maxCrossingLogSize:number;
  ...
  private performValidation<T>(
    ...
    if (data === null || data === undefined) {
      ...
      throw new DomainValidationError(
        ...
        'REQUIRED_VALUE_MISSING',        this.domain,
        'validation',        path,
        data,
        schema.type
      );
    ...
    if (schema.validator && !schema.validator(data)) {
      throw new DomainValidationError(
        ...
        'CUSTOM_VALIDATION_FAILED',        this.domain,
        'validation',        path,
        data,
        'custom validator')      );
}
