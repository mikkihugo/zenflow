...
export class DomainAnalyzer {
  ...
  async analyzeRepository(
    ...
    try {
      ...
      this.logger.warn('Domain analysis failed:', error);')      return [];
}
}
  ...
  private groupFilesByDirectory(files:string[]): Map<string, string[]> {
    ...
    const sortedPaths = Array.from(groups.keys()).sort((a, b) => {
      const depthA = a.split('/').length;')      const depthB = b.split('/').length;')      return depthB - depthA;
});
  ...
  private inferDomainName(path:string): string {
    const segments = path.split('/').filter(Boolean);')    const lastSegment = segments[segments.length - 1]||'root;

    // Clean up common prefixes/suffixes
    return lastSegment
      .replace(/^(src|lib|app|components|modules|packages)$/,'core')')      .replace(/[-_]/g, ' ')')      .split(' ')')      .map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
      .join(' ');')}
  ...
  private generateDomainDescription(path:string, files:string[]): string {
    const segments = path.split('/').filter(Boolean);')    const fileTypes = this.analyzeFileTypes(files);
    const primaryType = Object.keys(fileTypes).reduce((a, b) =>
      fileTypes[a] > fileTypes[b] ? a:b
    );

    const contexts = [
      'authentication',      'authorization',      'user',      'admin',      'api',      'database',      'service',      'component',      'util',      'helper',      'config',      'test',      'migration',];
  ...
  private async calculateDomainSize(files:string[]): Promise<DomainSize> {
    const fs = await import('node:fs/promises');')    const totalLines = 0;
    let functions = 0;
    let classes = 0;
    let interfaces = 0;
  ...
  private calculateDomainComplexity(files:string[], size:DomainSize): number {
    ...
    return parseFloat(
      (
        (fileCountFactor + lineFactor + functionFactor + classFactor) /
        4
      ).toFixed(3)
    );
}
  ...
  private calculateDomainCohesion(files:string[]): number {
    ...
    const fileNames = files.map((f) => f.split('/').pop()||');')    const commonPrefixes = this.findCommonPrefixes(fileNames);
    ...
    return parseFloat(
      ((prefixScore + suffixScore + extensionConsistency) / 3).toFixed(3)
    );
}
   ...
   */
  private async calculateDomainCoupling(
    files:string[],
    rootPath:string
  ):Promise<number> {
    const fs = await import('node:fs/promises');')    let totalImports = 0;
    let externalImports = 0;

    for (const file of files) {
      try {
        const content = await fs.readFile(file, 'utf-8');')        const imports = this.extractImports(content);
    ...
    return totalImports > 0
      ? parseFloat((externalImports / totalImports).toFixed(3))
      :0;
}
  ...
  private evaluateSplitRecommendation(
    ...
    return {
      ...
      estimatedEffort:{
        ...
        difficulty:score >= 5 ? 'high' : ' medium',        phases:[
          {
            name: 'Analysis',            description: 'Analyze dependencies and identify split boundaries',            estimatedHours:4,
            dependencies:[],
            risks:['Missing dependencies',    'Circular references'],
},
          {
            name: 'Implementation',            description: 'Create new domains and move files',            estimatedHours:suggestedSplits.length * 6,
            dependencies:['Analysis'],
            risks:['Breaking changes',    'Build failures'],
},
          {
            name: 'Testing',            description: 'Test split domains and fix integration issues',            estimatedHours:suggestedSplits.length * 2,
      ...
      risks:[
        {
          type: 'complexity',          severity: 'medium',          description: 'Splitting may initially increase system complexity',          mitigation: 'Use gradual migration approach',          probability:0.6,
          impact:0.4,
},
        {
          type: 'performance',          severity: 'low',          description: 'Additional boundaries may impact performance',          mitigation: 'Monitor performance metrics during migration',          probability:0.3,
          impact:0.2,
},
],
};
  ...
  private generateSplitSuggestions(
    ...
    if (suggestions.length === 0 && files.length > 10) {
      ...
      suggestions.push({
        newDomainName: 'Core',        files:files.slice(0, midpoint),
        rationale: 'Split large domain by file count',        complexity:0.5,
        dependencies:[],
});
}
  ...
  private detectFrameworks(files:string[]): string[] {
    ...
    for (const file of files) {
      ...
      if (fileName.includes('react')) frameworks.add(' React');')      if (fileName.includes('vue')) frameworks.add(' Vue');')      if (fileName.includes('angular')) frameworks.add(' Angular');')      if (fileName.includes('express')) frameworks.add(' Express');')      if (fileName.includes('next')) frameworks.add(' Next.js');')}
  ...
  private calculateExtensionConsistency(files:string[]): number {
    const extensions = files.map((f) => f.split('.').pop()||');')    const uniqueExtensions = new Set(extensions);
  ...
  private extractImports(content:string): string[] {
    ...
    // ES6 imports
    const importRegex =
      /import\s+(?:[\w*{}\s]+\s+from\s+)?['"]([@\w.-/]+)['"]/g;')    let match;
    while ((match = importRegex.exec(content)) !== null) {
      imports.push(match[1]);
}

    // CommonJS requires
    const requireRegex = /require\s*\(\s*['"]([@\w.-/]+)['"]\s*\)/g;')    while ((match = requireRegex.exec(content)) !== null) {
      imports.push(match[1]);
}
  ...
  private isExternalImport(
    ...
    ')      // Relative import - check if target is in domain
      const currentDir = currentFile.split('/').slice(0, -1).join('/');')      const path = require('node:path');')      const resolvedPath = path.resolve(currentDir, importPath);
  ...
  private groupFilesByPurpose(files:string[]): Map<string, string[]> {
    ...
    const purposes = [
      'component',      'service',      'util',      'helper',      'model',      'type',      'interface',      'config',      'test',      'spec',];
