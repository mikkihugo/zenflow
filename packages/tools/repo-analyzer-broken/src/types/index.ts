...
export type DomainType =
  | 'core' // Central business logic')  | 'feature' // Feature-specific code')  | 'infrastructure' // Infrastructure/framework code')  | 'utility' // Shared utilities')  | 'api' // API/interface layer')  | 'ui' // User interface')  | 'data' // Data access layer')  | 'test'; // Test code')
...
export interface SplitRisk {
  type:
    | 'data-consistency')    | 'performance')    | 'complexity')    | 'team-coordination')    | 'technical;
  severity:'low' | ' medium' | ' high' | ' critical;
  description:string;
  mitigation:string;
...
export interface AnalysisRecommendation {
  type:
    | 'split-domain')    | 'merge-domains')    | 'refactor-hotspot')    | 'reduce-coupling')    | 'improve-cohesion;
  priority:'low' | ' medium' | ' high' | ' urgent;
  title:string;
  description:string;
  rationale:string;
...
export interface ActionItem {
  description:string;
  type:
    | 'code-change')    | 'architecture-change')    | 'process-change')    | 'tooling-change;
  estimatedHours:number;
  dependencies:string[];
}
