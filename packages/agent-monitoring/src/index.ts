...
import type { MLModelRegistry } from '@claude-zen/adaptive-learning';
...
export class CompleteIntelligenceSystem implements IntelligenceSystem {
  ...
  private initializeSystems(
    ...
    if (config.healthMonitoring.enabled) {
      this.healthMonitor = createAgentHealthMonitor(
        {
          healthCheckInterval:
            config.healthMonitoring.healthCheckInterval || 30000,
          alertThresholds: {
            cpu: 0.8,
            memory: 0.9,
            taskFailureRate: 0.3,
      ...
      this.predictiveAnalytics = createPredictiveAnalyticsEngine(
        {
          ...
          enableEmergentBehaviorPrediction:
            config.predictiveAnalytics.enableEmergentBehavior || true,
          forecastHorizons: (config.predictiveAnalytics
            .forecastHorizons as ForecastHorizon[]) || [
            'short',
            'medium',
            'long',
          ],
          confidenceThreshold:
            config.predictiveAnalytics.confidenceThreshold || 0.75,
          enableEnsemblePrediction:
            config.predictiveAnalytics.ensemblePrediction || true,
          caching: {
  ...
  async predictTaskDurationMultiHorizon(
    ...
    return this.predictiveAnalytics.predictTaskDurationMultiHorizon(
      agentId,
      taskType,
      context
    );
  ...
  async forecastPerformanceOptimization(
    ...
    return this.predictiveAnalytics.forecastPerformanceOptimization(
      swarmId,
      horizon
    );
  }
  ...
  async predictKnowledgeTransferSuccess(
    ...
    return this.predictiveAnalytics.predictKnowledgeTransferSuccess(
      sourceSwarm,
      targetSwarm,
      patterns
    );
 ...
 */
export async function createProductionIntelligenceSystem(dependencies?: {
  neuralLearning?: Tier3NeuralLearning;
  mlRegistry?: MLModelRegistry;
  databaseManager?: SwarmDatabaseManager;
}): Promise<IntelligenceSystem> {
...
export const INTELLIGENCE_SYSTEM_INFO = {
  ...
  components: {
    taskPredictor:
      'TIER 1 - Task duration prediction with confidence intervals',
