...
async fn test_multi_threaded_training() -> Result<()> {
  ...
  let models: Vec<_> = (0..4)
    .map(|i| {
      ...
      registry
        .create_model(model_type, &ModelConfig::default())
        .unwrap()
    })
    .collect();

  // Parallel training using tokio tasks
  let training_tasks: Vec<_> = models
    .into_iter()
    .zip(datasets.iter())
    .enumerate()
    .map(|(i, (model, data))| {
  ...
  assert!(
    ...
    "At least 2 trainings should succeed"
  );

  // Performance assertions
  assert!(
    elapsed < Duration::from_secs(60),
    "Parallel training should complete within 60s"
  );

  let memory_stats = memory_tracker.get_statistics();
  assert!(
    memory_stats.peak_usage < 1024 * 1024 * 1024,
    "Peak memory should be under 1GB"
  );

  let training_times = performance_monitor.get_training_times();
  let avg_time =
    training_times.iter().sum::<Duration>() / training_times.len() as u32;
...
async fn test_parallel_model_execution() -> Result<()> {
  ...
  // Pre-train models
  let training_data =
    TimeSeriesData::synthetic(500, "daily")?.to_training_data()?;
  let trainer = ModelTrainer::new();
  ...
  assert!(
    avg_inference_time < Duration::from_millis(1000),
    "Inference should be fast"
  );
  assert!(
    total_time < Duration::from_secs(10),
    "Total parallel inference should be quick"
  );
...
async fn test_memory_usage_across_pipeline() -> Result<()> {
  ...
  for _ in 0..3 {
    let config =
      ModelConfig::from_params([("hidden_size", "128"), ("num_layers", "2")])?;
  ...
  assert!(
    model_stage_memory > data_stage_memory,
    "Model creation should increase memory"
  );
  assert!(
    training_stage_memory > model_stage_memory,
    "Training should increase memory"
  );

  // Verify cleanup effectiveness
  let cleanup_ratio =
    cleanup_stage_memory as f64 / memory_stats.peak_usage as f64;
  assert!(
    cleanup_ratio < 0.8,
    "Cleanup should release significant memory"
  );

  // Performance assertions
  assert!(
    memory_stats.peak_usage < 2 * 1024 * 1024 * 1024,
    "Peak usage should be under 2GB"
  );
...
async fn test_performance_benchmarks() -> Result<()> {
  ...
  let creation_benchmark =
    performance_monitor.benchmark("model_creation", || async {
      let registry = ModelRegistry::new();
      registry
        .register_model("lstm", Box::new(|| Box::new(LSTM::default())))?;
  ...
  let creation_results =
    creation_benchmark.run(benchmark_config.clone()).await?;
  assert!(
    creation_results.average < Duration::from_millis(100),
    "Model creation should be under 100ms"
  );

  // Benchmark 2: Training speed (small dataset)
  let training_benchmark =
    performance_monitor.benchmark("training_speed", || async {
      let registry = ModelRegistry::new();
      registry
        .register_model("lstm", Box::new(|| Box::new(LSTM::default())))?;
  ...
  let training_results =
    training_benchmark.run(benchmark_config.clone()).await?;
  assert!(
    training_results.average < Duration::from_secs(30),
  ...
  let training_data =
    TimeSeriesData::synthetic(100, "daily")?.to_training_data()?;
  let trainer = ModelTrainer::new();
  let trained_model = trainer.train(model, &training_data).await?;

  let inference_benchmark =
    performance_monitor.benchmark("inference_speed", || async {
      let data =
        TimeSeriesData::synthetic(50, "daily")?.to_inference_data(12)?;
  ...
  assert!(
    inference_results.average < Duration::from_millis(500),
    "Inference should be under 500ms"
  );

  // Benchmark 4: Memory efficiency
  let memory_benchmark =
    performance_monitor.benchmark_memory("memory_efficiency", || async {
  ...
  assert!(
    memory_results.peak_usage < 512 * 1024 * 1024,
...
async fn test_scalability_under_load() -> Result<()> {
  ...
  assert!(
    creation_load_results.success_rate > 0.9,
    "Success rate should be >90%"
  );
  assert!(
    creation_load_results.average_latency < Duration::from_secs(1),
  ...
  let inference_throughput_test = load_tester.test_throughput(
    ...
    Duration::from_secs(10), // for 10 seconds
    {
      ...
      move || {
        ...
        async move {
          let data =
            TimeSeriesData::synthetic(24, "daily")?.to_inference_data(1)?;
  ...
  assert!(
    throughput_results.requests_per_second > 50.0,
...
impl LoadTester {
  ...
  async fn test_concurrent_operations<F, Fut, T>(
    &self,
    _name: &str,
    _count: usize,
    _op: F,
  ...
  async fn test_throughput<F, Fut, T>(
    &self,
    _name: &str,
    _rps: usize,
    _duration: Duration,
  ...
  async fn test_memory_pressure<F, Fut, T>(
    &self,
    _name: &str,
    _limit: usize,
    _op: F,
  ) -> Result<MemoryPressureResults>
  where
    F: Fn() -> Fut + Send + Sync + 'static,
    Fut: std::future::Future<Output = Result<T>> + Send,
    T: Send + 'static,
