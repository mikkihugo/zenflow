...
impl<T: Float> TimeSeriesDataFrame<T> {
  ...
  pub fn time_range(
    ...
    let min_ts = ds_col
      ...
      .ok_or_else(|| {
        NeuroDivergentError::data(
          "Could not calculate min timestamp".to_string(),
        )
      })?;
    ...
    let max_ts = ds_col
      ...
      .ok_or_else(|| {
        NeuroDivergentError::data(
          "Could not calculate max timestamp".to_string(),
        )
      })?;
...
impl<T: Float> CrossValidationDataFrame<T> {
  ...
  pub fn get_model_results(
    ...
    let model_cols: Vec<String> = self
      ...
      .iter()
      .filter(|col| {
        col.as_str().contains(model_name)
          || col.as_str() == self.schema.unique_id_col.as_str()
          || col.as_str() == self.schema.ds_col.as_str()
          || col.as_str() == "cutoff"
      })
      .map(|s| s.to_string())
      .collect();
