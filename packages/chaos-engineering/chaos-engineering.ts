...
export class ChaosEngineering extends EventEmitter {
  ...
  constructor(options: ChaosEngineeringOptions = {}) {
    ...
    const configValidation = validateSharedConfig();
    if (!configValidation.isValid) {
      this.logger.warn('Shared config validation issues:', configValidation.issues);
    }
    ...
    this.options = {
      ...
      experimentTimeout: options?.experimentTimeout || 
        configHelpers.getTimeout('general') || 
    ...
    this.configWatcher = watchSharedConfig(() => {
  ...
  private reloadConfiguration() {
    ...
    this.options.maxConcurrentExperiments = sharedConfig.performance?.maxConcurrent || 
      sharedConfig.get('chaos.maxConcurrentExperiments', 3);
    this.options.experimentTimeout = configHelpers.getTimeout('general') || 
      sharedConfig.get('chaos.experimentTimeout', 300000);
