...
export class TelemetryManager {
  ...
  recordMetric(name: string, value: number = 1, attributes?: Record<string, any>): void {
  ...
  async withAsyncTrace<T>(nameOrFn: string | (() => Promise<T>), fn?: () => Promise<T>): Promise<T> {
...
export function recordMetric(name: string, value: number = 1, attributes?: Record<string, any>): void {
...
export async function withAsyncTrace<T>(name: string, fn: () => Promise<T>): Promise<T>;
...
export function metered(name?: string) {
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    const originalMethod = descriptor.value;
    descriptor.value = function (...args: any[]) {
      const metricName = name || `${target.constructor.name}.${propertyKey}.calls`;
