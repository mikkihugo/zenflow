...
export class TaskComplexityEstimator {
     ...
     */
    async estimateComplexity(task, prompt, context = {}, agentRole) {
        if (!this.initialized) {
     ...
     */
    async learnFromOutcome(task, prompt, context, actualComplexity, actualDuration, actualSuccess, agentRole) {
        if (!this.initialized) {
            await this.initialize();
        }
        try {
            ...
            logger.debug(`📚 Learned from task outcome: ${task} (complexity: ${actualComplexity.toFixed(2)})`);
        }
        catch (error) {
    ...
    analyzePromptComplexity(prompt) {
        ...
        const technicalMatches = technicalKeywords.filter(keyword => prompt.toLowerCase().includes(keyword)).length;
        ...
        const stepMatches = stepIndicators.filter(indicator => prompt.toLowerCase().includes(indicator)).length;
        if (stepMatches > 2) {
            complexity += 0.15;
    ...
    analyzeContextComplexity(context) {
        ...
        const complexityHints = complexityKeywords.filter(keyword => contextString.includes(keyword)).length;
    ...
    async matchComplexityPatterns(task, prompt, context) {
        let bestMatch = 0;
        let bestPattern = '';
        for (const pattern of this.complexityPatterns) {
            let matchScore = 0;
            // Check keyword matches
            const keywordMatches = pattern.keywords.filter(keyword => prompt.toLowerCase().includes(keyword) || task.toLowerCase().includes(keyword)).length;
            if (keywordMatches > 0) {
                matchScore += (keywordMatches / pattern.keywords.length) * 0.7;
            }
            // Check context key matches
            const contextMatches = pattern.contextKeys.filter(key => Object.prototype.hasOwnProperty.call(context, key)).length;
            if (contextMatches > 0) {
    ...
    analyzeRoleComplexity(agentRole) {
        if (!agentRole) {
        ...
        const complexity = roleComplexities[agentRole.toLowerCase()] || 0.5;
        return {
            score: complexity * 0.3, // Scale down role impact
            reasoning: `${agentRole} role (${(complexity * 100).toFixed(0)}% complexity)`
        };
    }
    suggestOptimizationMethod(complexity, context) {
    ...
    estimateDurationFromComplexity(complexity, agentRole) {
    ...
    extractKeyFactors(prompt, context, complexity) {
        const factors = [];
    ...
    updateKeywordWeights(prompt, actualComplexity) {
    ...
    extractNumericalFeatures(prompt, context) {
        return [
    ...
    getSimpleComplexityEstimate(prompt, context) {
...
//# sourceMappingURL=task-complexity-estimator.js.map