...
export class RetrainingMonitor {
    ...
    async startMonitoring() {
        ...
        try {
            ...
            this.logger.info('Operations facade not available, retraining monitor will not start');
            return;
        }
        try {
            ...
            this.intervalId = setInterval(() => this.checkAndTriggerRetraining().catch(error => this.logger.error('Error in retraining check cycle:', error)), retrainingConfig.checkIntervalMs);
            this.isMonitoring = true;
            this.logger.info(`Retraining monitor started, checking every ${retrainingConfig.checkIntervalMs / 1000 / 60} minutes with threshold ${retrainingConfig.minCoordinationSuccessRateThreshold}`);
        }
        catch (error) {
     ...
     */
    async manualRetrain(reason, additionalMetrics = {}) {
    ...
    async checkAndTriggerRetraining() {
        try {
            ...
            if (coordinationSuccessRate < config.minCoordinationSuccessRateThreshold) {
                this.logger.warn(`Coordination success rate (${coordinationSuccessRate}) below threshold (${config.minCoordinationSuccessRateThreshold}). Triggering retraining.`);
                const trigger = {
                    timestamp: new Date(),
     ...
     */
    async executeRetrainingWorkflow(trigger) {
        ...
        try {
            ...
            if (this.dbAccess) {
                ...
                await kv.set(`retraining:trigger:${trigger.timestamp.getTime()}`, JSON.stringify(trigger));
            }
...
Format as JSON with keys: approach, epochs, batchSize, successCriteria, risks`;
            ...
            try {
                ...
                this.logger.warn('LLM provider not available via operations facade, using default plan');
            }
            ...
            if (this.dbAccess) {
                const kv = await this.dbAccess.getKV('brain');
                await kv.set(`retraining:result:${trigger.timestamp.getTime()}`, JSON.stringify(result));
            }
     ...
     */
    async isInCooldownPeriod(cooldownHours) {
        if (!this.dbAccess)
            return false;
        try {
            const kv = await this.dbAccess.getKV('brain');
            const lastRetrainingData = await kv.get('retraining:last');
            if (!lastRetrainingData)
                return false;
     ...
     */
    async hasExceededDailyLimit(maxAttempts) {
        if (!this.dbAccess)
            return false;
        try {
    ...
    async initialize(config) {
        try {
            ...
            this.logger.info('RetrainingMonitor initialization completed');
        }
        catch (error) {
            this.logger.error('Failed to initialize RetrainingMonitor:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async recordPromptFeedback(promptId, feedback) {
        try {
            this.logger.debug(`Recording prompt feedback for ${promptId}`, feedback);
            if (!this.dbAccess) {
                this.logger.warn('Database access not available, feedback not recorded');
                return;
            }
            ...
            if (!feedback.success && feedback.accuracy && feedback.accuracy < 0.7) {
                this.logger.warn(`Poor accuracy (${feedback.accuracy}) detected for prompt ${promptId}, may trigger retraining`);
            }
        }
        catch (error) {
            this.logger.error(`Failed to record prompt feedback for ${promptId}:`, error);
        }
    }
}
export default RetrainingMonitor;
//# sourceMappingURL=retraining-monitor.js.map