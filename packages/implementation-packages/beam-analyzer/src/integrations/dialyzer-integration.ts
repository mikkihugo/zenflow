...
export class DialyzerIntegration {
  ...
  async analyze(
    ...
    try {
      this.logger.info(`Running Dialyzer analysis for ${project.language} project`);
      ...
      const analysisResult = await this.runDialyzerAnalysis(project, context, pltPath, options);
      ...
      this.logger.info(`Dialyzer analysis completed: ${warnings.length} warnings found`);
      return ok(result);
  ...
  private async buildPlt(
    ...
    return new Promise((resolve) => {
      ...
      args.push('--apps', 'erts', 'kernel', 'stdlib', 'crypto', 'public_key', 'ssl');
      ...
      this.logger.debug(`Building PLT with command: dialyzer ${args.join(' ')}`);
  ...
  private async runDialyzerAnalysis(
    ...
    return new Promise((resolve) => {
      ...
      if (options.warnings && options.warnings.length > 0) {
        ...
        // Default warnings
        args.push('-Wunmatched_returns', '-Werror_handling', '-Wrace_conditions');
      }
      ...
      this.logger.debug(`Running Dialyzer with command: dialyzer ${args.join(' ')}`);
  ...
  private parseDialyzerWarningLine(line: string): DialyzerWarning | null {
    ...
    const match = line.match(/^([^:]+):(\d+):\s*(?:([^:]+):\s*)?Warning:\s*(.+)$/);
    if (!match) {
      return null;
    }
  ...
  private extractWarningType(message: string): DialyzerWarningType {
    ...
    if (lowerMessage.includes('no local return') || lowerMessage.includes('no return')) {
      return 'no_return';
    }
    if (lowerMessage.includes('unused') || lowerMessage.includes('will never be called')) {
      return 'unused_fun';
    }
    ...
    if (lowerMessage.includes('race condition') || lowerMessage.includes('race')) {
   ...
   * Get PLT information
   */
  async getPltInfo(pltPath: string): Promise<Result<{ modules: string[]; size: number }, BeamAnalysisError>> {
    return new Promise((resolve) => {
