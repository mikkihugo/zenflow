...
export class BeamPatternAnalyzer {
  ...
  async analyzePatterns(
    ...
    try {
      this.logger.info(`Analyzing BEAM patterns for ${project.language} project`);

      const findings: BeamFinding[] = [];

      // Get default rules for the project's languages
      const defaultRules = this.getDefaultRules([project.language, ...(project.additionalLanguages || [])]);
      const allRules = [...defaultRules, ...customRules];

      // Analyze each source file
      for (const app of project.applications) {
        for (const sourceDir of app.sourceDirs) {
          const files = await this.getSourceFiles(path.join(project.root, sourceDir), project.language);
      ...
      this.logger.info(`Pattern analysis completed: ${findings.length} findings`);
      return ok(findings);
  ...
  private getErlangRules(): BeamAnalysisRule[] {
    return [
      ...
      },
      {
        ...
        description: 'Potential process leak - spawned process not linked or monitored',
        severity: 'high',
        languages: ['erlang'],
        pattern: {
          type: 'regex',
      ...
      },
      {
        ...
        messageTemplate: 'gen_server:call without timeout may block indefinitely',
        fixSuggestion: 'Add timeout parameter: gen_server:call(Server, Request, Timeout)'
      },
      {
    ...
    return [
      {
        ...
        fixSuggestion: 'Add Task.await/1,2 or use Task.start/1 for fire-and-forget'
      },
      {
        name: 'elixir-genserver-cast-sync',
        description: 'Using GenServer.cast for operations that should be synchronous',
        severity: 'medium',
        languages: ['elixir'],
        pattern: {
          type: 'regex',
        ...
        messageTemplate: 'Update operations should use GenServer.call for confirmation',
        fixSuggestion: 'Use GenServer.call/2,3 for operations requiring confirmation'
      },
      {
        ...
        messageTemplate: 'Consider if :permanent restart is appropriate for this worker',
        fixSuggestion: 'Use :temporary for one-off tasks, :transient for tasks that may fail normally'
      },
      {
        ...
        messageTemplate: 'Chaining Enum operations with Enum.into may be inefficient',
        fixSuggestion: 'Consider using Stream for lazy evaluation or direct collection functions'
      },
      {
        ...
        messageTemplate: 'Consider extracting private controller logic to separate modules',
    ...
    return [
      {
        ...
        messageTemplate: 'Result type should be handled with case expression or try syntax',
        fixSuggestion: 'Use case expression or try syntax to handle Ok/Error variants'
      },
      {
    ...
    return [
      {
        ...
        description: 'Complex lambda expressions that should be named functions',
        severity: 'medium',
        languages: ['lfe'],
        pattern: {
          type: 'regex',
        ...
        fixSuggestion: 'Extract lambda to defun for better readability and reusability'
      },
      {
   ...
   */
  private async analyzeFile(filePath: string, rules: BeamAnalysisRule[]): Promise<BeamFinding[]> {
    const findings: BeamFinding[] = [];
    ...
    try {
      ...
      for (const rule of rules) {
        ...
        const ruleFindings = await this.applyRule(filePath, content, lines, rule);
  ...
  private async applyRule(
    ...
    if (rule.pattern.type === 'regex') {
      ...
      while ((match = regex.exec(content)) !== null) {
        ...
        if (rule.pattern.constraints && !this.checkConstraints(match, content, rule.pattern.constraints)) {
          continue;
        }
   ...
   */
  private getLocationFromMatch(match: RegExpExecArray, lines: string[]): BeamLocation {
   ...
   */
  private checkConstraints(match: RegExpExecArray, content: string, constraints: Record<string, unknown>): boolean {
    ...
    if (constraints.not_followed_by) {
      const followingText = content.substring(match.index! + match[0].length, match.index! + match[0].length + 100);
   ...
   */
  private async getSourceFiles(sourceDir: string, language: BeamLanguage): Promise<string[]> {
    const files: string[] = [];
   ...
   */
  private async scanDirectory(dir: string, extensions: string[], files: string[]): Promise<void> {
    try {
