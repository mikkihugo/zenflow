...
export class TeamworkStorage {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      logger.info('Initializing teamwork storage with foundation infrastructure...');
    ...
    try {
      ...
      if (sessionData && typeof sessionData === 'object') {
        ...
        return {
          ...
          messages: (data['messages'] as any[])?.map(msg => ({
            ...msg,
            timestamp: new Date(msg.timestamp)
          })) || [],
          outcomes: (data['outcomes'] as any[])?.map(outcome => ({
            ...outcome,
  ...
  async storeMessage(message: ConversationMessage): Promise<void> {
    ...
    try {
      ...
      await sql.query('INSERT OR REPLACE INTO messages VALUES (?, ?, ?, ?, ?, ?)', [
        message.id,
   ...
   */
  async getMessages(sessionId: string, limit = 100): Promise<ConversationMessage[]> {
    await this.ensureInitialized();
    ...
    try {
      ...
      return results.map((row: any) => ({
        ...
        fromAgent: { id: row.agentId, type: 'coordinator', swarmId: 'default', instance: 0 }, // Reconstruct AgentId
      ...
      logger.error(`Failed to retrieve messages for session ${sessionId}:`, error);
      return [];
    }
  }
  ...
  async addMessage(conversationId: string, message: ConversationMessage): Promise<void> {
    ...
    try {
      ...
      logger.debug(`Added message ${message.id} to conversation ${conversationId}`);
    } catch (error) {
