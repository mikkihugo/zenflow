...
async function testSafe6CrossPackageIntegration(): Promise<{
  ...
  try {
    ...
    try {
      ...
      if (coordinator && typeof coordinator.coordinateWithSafe6 === 'function') {
        ...
        logger.warn('⚠️ Brain coordinator available but SAFe 6.0 method not found');
      }
    ...
    try {
      const { Safe6DevelopmentManager } = await import('./safe6-development-manager');
    ...
    try {
      ...
      results.taskMasterIntegration = { 
        status: 'success', 
        integration: !!taskMasterIntegration.default || !!taskMasterIntegration.SafeFrameworkIntegration
      };
      ...
      errors.push(errorMsg);
      logger.error(errorMsg);
      results.taskMasterIntegration = { status: 'failed', error: String(error) };
    }
    ...
    logger.info(`🎯 Integration test complete: ${successCount}/${totalTests} tests passed`);
    
    if (success) {
      logger.info('🎉 All SAFe 6.0 cross-package integrations working correctly!');
    } else {
