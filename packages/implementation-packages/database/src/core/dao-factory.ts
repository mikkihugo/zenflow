...
class ConcreteDao<T> extends BaseDao<T> {
  constructor(adapter: DatabaseAdapter, logger: Logger, tableName: string) {
...
function createMemoryDao<T>(
  ...
  return new MemoryDaoImpl(adapter, logger, tableName) as Dao<T>;
}
...
function createCoordinationDao<T>(
  ...
  return new CoordinationDaoImpl(adapter, logger, tableName) as Dao<T>;
}
...
function createGraphDao<T>(
  ...
  return new GraphDaoImpl(adapter, logger, tableName) as Dao<T>;
}
