...
export class DALFactory {
   ...
   * @returns {Promise<DataAccessObject<T>>} A promise that resolves to a configured DAO instance.
   ...
   * @returns {Promise<GraphRepository<T>>} A promise that resolves to a graph repository instance.
  ...
  async createKuzuGraphRepository<T>(
    ...
    return (await this.createRepository<T>(config)) as GraphRepository<T>;
  }
   ...
   * @returns {Promise<VectorRepository<T>>} A promise that resolves to a vector repository instance.
  ...
  async createLanceDBVectorRepository<T>(
    ...
    return (await this.createRepository<T>(config)) as VectorRepository<T>;
  }
   ...
   * @returns {Promise<CoordinationRepository<T>>} A promise that resolves to a coordination repository.
   ...
   * @returns {Promise<MemoryRepository<T>>} A promise that resolves to a memory repository.
  ...
  async createMemoryRepository<T>(
    ...
    return (await this.createRepository<T>(config)) as MemoryRepository<T>;
  }
 ...
 * @example
 */
export class MultiDatabaseDAO<T> implements DataAccessObject<T> {
  constructor(
    private primaryDAO: DataAccessObject<T>,
    private secondaryDAOs: DataAccessObject<T>[],
  ...
  async getMetrics(): Promise<PerformanceMetrics> {
    ...
    return {
      ...
      queriesPerSecond: primary.queriesPerSecond,
      connectionPool: primary.connectionPool,
      memoryUsage: primary.memoryUsage,
      custom: {
        primary,
        secondaries: secondaries.map((result) =>
          result?.status === 'fulfilled'
