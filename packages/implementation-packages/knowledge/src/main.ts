...
export interface KnowledgeStore {
  add(item: Omit<KnowledgeItem, 'id' | 'timestamp' | 'version' | 'isActive'>, options?: KnowledgeItemOptions): Promise<Result<UUID, KnowledgeError>>;
  get(id: UUID): Promise<Result<KnowledgeItem | null, KnowledgeError>>;
  update(id: UUID, updates: Partial<KnowledgeItem>): Promise<Result<KnowledgeItem, KnowledgeError>>;
  delete(id: UUID): Promise<Result<boolean, KnowledgeError>>;
  query(query?: KnowledgeQuery): Promise<Result<KnowledgeItem[], KnowledgeError>>;
  search(text: string, options?: { limit?: number; type?: KnowledgeItem['type'] }): Promise<Result<KnowledgeItem[], KnowledgeError>>;
...
export class FoundationKnowledgeStore extends TypedEventBase implements KnowledgeStore {
  ...
  async initialize(): Promise<Result<void, KnowledgeError>> {
    ...
    const timer = this.performanceTracker.startTimer('knowledge_store_initialize');
   ...
   */
  async add(item: Omit<KnowledgeItem, 'id' | 'timestamp' | 'version' | 'isActive'>, options?: KnowledgeItemOptions): Promise<Result<UUID, KnowledgeError>> {
    if (!this.initialized) {
    ...
    return withTrace('knowledge_store_add', async () => {
      ...
      try {
        ...
        const validation = validateObject(item, {
          ...
          type: { type: 'string', required: true, enum: ['fact', 'rule', 'pattern', 'insight', 'procedure', 'concept'] },
          confidence: { type: 'number', required: true, min: 0, max: 1 }
        });
        
        if (!validation.success) {
          const error = new KnowledgeError('Invalid knowledge item data', { validation: validation.errors });
          this.errorAggregator.addError(error);
          return err(error);
        }
        ...
        const storageResult = await withRetry(
          () => this.circuitBreaker.execute('set', `knowledge:${id}`, JSON.stringify(knowledgeItem)),
  ...
  async get(id: UUID): Promise<Result<KnowledgeItem | null, KnowledgeError>> {
    ...
    return withTrace('knowledge_store_get', async () => {
      ...
      try {
        if (!isUUID(id)) {
          const error = new KnowledgeError('Invalid UUID format', { id, operation: 'get' });
          this.errorAggregator.addError(error);
          return err(error);
        }
  ...
  async update(id: UUID, updates: Partial<KnowledgeItem>): Promise<Result<KnowledgeItem, KnowledgeError>> {
  ...
  async query(query?: KnowledgeQuery): Promise<Result<KnowledgeItem[], KnowledgeError>> {
    if (!this.initialized) {
      const initResult = await this.initialize();
      if (!initResult.success) return err(initResult.error);
    }
  ...
  async search(text: string, options?: { limit?: number; type?: KnowledgeItem['type'] }): Promise<Result<KnowledgeItem[], KnowledgeError>> {
    return this.query({
      contentSearch: text,
      type: options?.type,
  ...
  async clear(): Promise<Result<void, KnowledgeError>> {
    try {
      ...
      return err(new KnowledgeError('Failed to clear knowledge store', { operation: 'clear' }, ensureError(error)));
    }
  }

  async getStats(): Promise<Result<KnowledgeStats, KnowledgeError>> {
    try {
      const activeItems = Array.from(this.items.values()).filter(item => item.isActive);
      const totalItems = activeItems.length;
      
      const itemsByType: Record<KnowledgeItem['type'], number> = {
        fact: 0, rule: 0, pattern: 0, insight: 0, procedure: 0, concept: 0
      };
      ...
      return err(new KnowledgeError('Failed to get stats', { operation: 'stats' }, ensureError(error)));
    }
  }
  ...
  async shutdown(): Promise<Result<void, KnowledgeError>> {
    try {
      ...
      return err(new KnowledgeError('Failed to shutdown', { operation: 'shutdown' }, ensureError(error)));
    }
  }
  ...
  private async performStorageOperation(operation: string, ...args: any[]): Promise<any> {
    if (!this.storage) {
      throw new StorageError('Storage not initialized');
    }
    
    switch (operation) {
      case 'get': return this.storage.get(args[0]);
      case 'set': return this.storage.set(args[0], args[1]);
      case 'delete': return this.storage.delete(args[0]);
  ...
  private async loadFromStorage(): Promise<Result<void, KnowledgeError>> {
    ...
    try {
      ...
      this.logger.debug('Loaded knowledge items from storage', { count: knowledgeKeys.length });
      return ok(undefined);
    } catch (error) {
      return err(new KnowledgeError('Failed to load from storage', { operation: 'loadFromStorage' }, ensureError(error)));
    }
  }
}
...
export interface KnowledgeManager {
  ...
  addKnowledge(content: string, type: KnowledgeItem['type'], options?: { confidence?: number; source?: string; tags?: string[] }): Promise<Result<UUID, KnowledgeError>>;
  getKnowledge(id: UUID): Promise<Result<KnowledgeItem | null, KnowledgeError>>;
  queryKnowledge(query?: KnowledgeQuery): Promise<Result<KnowledgeItem[], KnowledgeError>>;
  searchKnowledge(text: string, options?: { limit?: number; type?: KnowledgeItem['type'] }): Promise<Result<KnowledgeItem[], KnowledgeError>>;
  getStats(): Promise<Result<KnowledgeStats, KnowledgeError>>;
  shutdown(): Promise<Result<void, KnowledgeError>>;
}
...
export class EnterpriseKnowledgeManager implements KnowledgeManager {
  ...
  async addKnowledge(
    ...
    return withContext(
      { operation: 'addKnowledge', type, confidence },
      () => this.store.add(
  ...
  async getKnowledge(id: UUID): Promise<Result<KnowledgeItem | null, KnowledgeError>> {
    return withContext(
      { operation: 'getKnowledge', id },
      () => this.store.get(id)
    );
  }

  async queryKnowledge(query?: KnowledgeQuery): Promise<Result<KnowledgeItem[], KnowledgeError>> {
    return withContext(
      { operation: 'queryKnowledge', query },
  ...
  async searchKnowledge(
    ...
    return withContext(
      { operation: 'searchKnowledge', text, options },
...
export async function getKnowledgeSystemAccess(storeConfig?: any): Promise<any> {
  ...
  try {
    ...
    console.warn('Fact system not available, knowledge will work in standalone mode');
  }
  
  return {
    createStore: () => new FoundationKnowledgeStore(),
    createManager: (existingStore?: KnowledgeStore) => new EnterpriseKnowledgeManager(existingStore),
    addKnowledge: (content: string, type: KnowledgeItem['type'], options?: any) => 
    ...
    searchKnowledge: (text: string, options?: any) => manager.searchKnowledge(text, options),
    ...
    searchFromFacts: async (query: string, options?: any) => {
      ...
      return {
        local: localResults,
        facts: factResults,
        combined: [...(localResults.success ? localResults.data : []), ...(factResults || [])]
      };
    },
...
export async function getKnowledgeManager(store?: KnowledgeStore): Promise<EnterpriseKnowledgeManager> {
  const manager = new EnterpriseKnowledgeManager(store);
...
export async function getKnowledgeStorage(config?: any): Promise<any> {
  ...
  await store.initialize();
  return {
    add: (item: any, options?: KnowledgeItemOptions) => store.add(item, options),
    get: (id: UUID) => store.get(id),
    update: (id: UUID, updates: Partial<KnowledgeItem>) => store.update(id, updates),
...
export const knowledgeSystem = {
  ...
  createManager: (store?: KnowledgeStore) => new EnterpriseKnowledgeManager(store),
  createStore: () => new FoundationKnowledgeStore(),
  // Enhanced with fact-system integration
