...
export class CapacityPlanningService extends TypedEventBase {
  ...
  async calculateTeamCapacities(teams: any[]): Promise<TeamCapacity[]> {
    ...
    try {
      ...
      this.emit('team-capacities-calculated', {
        ...
        averageVelocity: capacities.reduce((sum, c) => sum + c.velocity, 0) / capacities.length
      });
  ...
  async implementCapacityPlanning(
    ...
    try {
      ...
      planningResult.utilizationRate = planningResult.totalCapacity > 0 
        ? planningResult.allocatedCapacity / (planningResult.totalCapacity - planningResult.bufferCapacity - planningResult.innovationCapacity)
        : 0;
  ...
  private calculateExperienceMultiplier(members: any[]): number {
    ...
    const avgExperience = members.reduce((sum, member) => {
      return sum + (experienceWeights[member.experience as keyof typeof experienceWeights] || 1.0);
    }, 0) / members.length;
    ...
    for (const member of members) {
      for (const skill of member.skills || []) {
        if (!skillMap.has(skill.name)) {
          skillMap.set(skill.name, { count: 0, capacity: 0, proficiencies: [] });
        }
   ...
   */
  private calculateAverageProficiency(proficiencies: string[]): 'basic' | 'intermediate' | 'advanced' | 'expert' {
    ...
    const avgValue = proficiencies.reduce((sum, prof) => {
      return sum + (proficiencyValues[prof as keyof typeof proficiencyValues] || 2);
  ...
  private async allocateFeatureToTeam(
    ...
    const allocation: TeamAllocation = {
      ...
      endIteration: teamMatch.endIteration || (1 + Math.ceil(feature.complexity / team.velocity)),
  ...
  private calculateSkillMatches(
    ...
    for (const requiredSkill of requiredSkills) {
      const teamSkill = teamSkills.find(s => s.skillName.toLowerCase() === requiredSkill.toLowerCase());
  ...
  private evaluateSkillMatchQuality(
    ...
    const proficiencyLevels = { basic: 1, intermediate: 2, advanced: 3, expert: 4 };
    const requiredLevel = proficiencyLevels[required as keyof typeof proficiencyLevels] || 2;
    const availableLevel = proficiencyLevels[available as keyof typeof proficiencyLevels] || 1;

    if (memberCount === 0) return 'poor';
    if (availableLevel >= requiredLevel + 1) return 'perfect';
    if (availableLevel >= requiredLevel) return memberCount >= 2 ? 'good' : 'adequate';
  ...
  private assessAllocationRisks(
    ...
    // Skill gap risks
    const poorSkillMatches = skillMatches.filter(sm => sm.matchQuality === 'poor');
  ...
  private identifyCapacityRisks(
    ...
    if (underutilizedTeams.length > 0) {
      risks.push({
        ...
        impact: 'Inefficient resource utilization and reduced delivery capacity',
    ...
    if (unallocatedFeatures.length > 0) {
      ...
      if (criticalUnallocated.length > 0) {
        risks.push({
          ...
          mitigation: 'Increase capacity, reduce scope, or defer lower priority work',
          severity: 'critical',
          affectedTeams: [],
  ...
  private async generateCapacityRecommendations(
    ...
    const intelligentRecommendations = await this.brainCoordinator.generateRecommendations({
      planningResult,
      teamCapacities,
      allocationStrategy,
      context: {
  ...
  private analyzeSkillCoverage(
    ...
    const skillGaps = Array.from(allSkills).filter(skill => 
      skillTeamCount.get(skill) === 1
    );
  ...
  private analyzeTeamDistribution(
    ...
    const averageUtilization = teamWorkloads.reduce((sum, t) => sum + t.utilization, 0) / teamWorkloads.length;
    const utilizationVariance = teamWorkloads.reduce((sum, t) => sum + Math.pow(t.utilization - averageUtilization, 2), 0) / teamWorkloads.length;
   ...
   */
  private analyzeRiskDistribution(risks: CapacityRisk[]): RiskDistributionAnalysis {
    ...
    return {
      ...
      mitigation: ['Implement proactive capacity monitoring', 'Establish cross-training programs']
    };
  }
