...
export async function getAGUISystemAccess(config?: AGUIConfig): Promise<any> {
  ...
  return {
    ...
    createTaskApproval: (approvalConfig?: TaskApprovalConfig) => createTaskApprovalSystem(approvalConfig),
    createBasicAGUI: (aguiConfig?: any) => createBasicAGUI(aguiConfig),
    askQuestion: (question: string, options?: any) => agui.askQuestion(question, options),
    requestApproval: (request: ApprovalRequest) => taskApproval.requestApproval(request),
...
export async function getTaskApprovalAccess(config?: TaskApprovalConfig): Promise<any> {
  const taskApproval = await createTaskApprovalSystem(config);
  return {
    request: (request: ApprovalRequest) => taskApproval.requestApproval(request),
    batch: (requests: ApprovalRequest[]) => taskApproval.batchApproval?.(requests),
    getStats: () => taskApproval.getStatistics?.(),
    configure: (newConfig: Partial<TaskApprovalConfig>) => taskApproval.updateConfig?.(newConfig)
  };
}
...
export async function getHumanInteraction(config?: AGUIConfig): Promise<any> {
  const { agui, taskApproval } = await createAGUI(config);
  return {
    ask: (question: string, options?: any) => agui.askQuestion(question, options),
    confirm: (message: string) => agui.askQuestion(message, { type: 'boolean' }),
    approve: (request: ApprovalRequest) => taskApproval.requestApproval(request),
    notify: (message: string, type?: MessageType) => agui.showMessage?.(message, type || 'info'),
    progress: (info: ProgressInfo) => agui.showProgress?.(info)
  };
}

export async function getWorkflowGates(config?: TaskApprovalConfig): Promise<any> {
  const taskApproval = await createTaskApprovalSystem(config);
  return {
    gate: (request: ApprovalRequest) => taskApproval.requestApproval(request),
    checkpoint: (description: string, context?: any) => taskApproval.requestApproval({
      taskType: 'checkpoint',
      description,
      context
    }),
