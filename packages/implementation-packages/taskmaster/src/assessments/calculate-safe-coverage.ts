...
import { ESSENTIAL_SAFE_COMPONENTS, EssentialSafeComponent } from './essential-safe-readiness.js';
...
export function calculateActualSafeCoverage(): {
  ...
  const completeComponents = ESSENTIAL_SAFE_COMPONENTS.filter(c => c.taskMasterSupport === 'complete');
  const partialComponents = ESSENTIAL_SAFE_COMPONENTS.filter(c => c.taskMasterSupport === 'partial');
  const missingComponents = ESSENTIAL_SAFE_COMPONENTS.filter(c => c.taskMasterSupport === 'missing');
  ...
  return {
    ...
    recommendation: generateRecommendation(essentialSafeCoverage, overallSafeCoverage)
  };
}

function generateRecommendation(essentialCoverage: number, overallCoverage: number): string {
