...
export declare class PromptManagementService {
     ...
     */
    createPromptVersion(promptId: string, data: {
        content: string;
        description: string;
        config?: Partial<PromptVersion['config']>;
        tags?: string[];
    }, createdBy: string, auditContext: any): Promise<PromptVersion>;
    /**
     * Create prompt variant for A/B testing
     */
    createPromptVariant(promptId: string, versionId: string, data: {
        name: string;
        trafficAllocation: number;
    }, createdBy: string, auditContext: any): Promise<PromptVariant>;
    /**
     * Create collaborative draft with teamwork integration
     */
    createDraft(promptId: string, data: {
        title: string;
        content: string;
        config?: Partial<PromptVersion['config']>;
        collaborators?: string[];
    }, authorId: string): Promise<PromptDraft>;
    /**
     * Approve prompt version with SOC2 audit trail
     */
    approvePromptVersion(promptId: string, versionId: string, approvedBy: string, auditContext: any, approvalData: {
        reason: string;
        riskAssessment: 'low' | 'medium' | 'high';
        approvalReference?: string;
    }): Promise<void>;
     ...
     */
    getPromptForGate(gateType: string, context: any): Promise<{
     ...
     */
    trackPromptPerformance(versionId: string, result: {
        success: boolean;
        confidence: number;
