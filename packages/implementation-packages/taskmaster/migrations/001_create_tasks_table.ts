...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable('tasks', (table) => {
    ...
    table.string('state', 50).notNullable().defaultTo('backlog')
      .comment('Current workflow state');
    table.enum('complexity', ['trivial', 'simple', 'moderate', 'complex', 'epic']).notNullable()
      .comment('Task complexity classification');
    
    // Effort tracking
    table.decimal('estimated_hours', 8, 2)
      .comment('Estimated effort in hours');
    table.decimal('actual_hours', 8, 2).defaultTo(0)
      .comment('Actual time logged in hours');
    
    // Assignment and ownership
    table.uuid('assignee_id')
      .comment('Currently assigned user ID');
