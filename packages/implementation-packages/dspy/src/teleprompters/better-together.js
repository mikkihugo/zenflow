...
export class BetterTogether extends Teleprompter {
    ...
    constructor(config) {
        ...
        if (!globalThis.dspy?.settings?.experimental) {
        ...
        this.prompt_optimizer = prompt_optimizer || new BootstrapFewShotWithRandomSearch({ metric });
        this.weight_optimizer = weight_optimizer || new BootstrapFinetune({ metric });
        // Validate supported optimizers exactly matching Stanford implementation
        const is_supported_prompt = this.prompt_optimizer instanceof BootstrapFewShotWithRandomSearch;
        const is_supported_weight = this.weight_optimizer instanceof BootstrapFinetune;
    ...
    async compile(student, config) {
        ...
        prepared_student = await this._run_strategies(parsed_strategy, prepared_student, working_trainset, valset_ratio);
    ...
    _parse_strategy(strategy) {
        ...
        if (!parsed_strategy.every(s => s === "p" || s === "w")) {
            throw new Error(`The strategy should be a sequence of 'p' and 'w' separated by '${STRAT_SEP}', but found: ${strategy}`);
        }
     ...
     */
    async _run_strategies(parsed_strategy, student, trainset, valset_ratio) {
        ...
        for (let ind = 0; ind < parsed_strategy.length; ind++) {
            ...
            const current_strategy = parsed_strategy.slice(0, ind + 1).join(STRAT_SEP);
            ...
            if (step_code === "p") {
                student = await this._compile_prompt_optimizer(student, trainset, valset_ratio);
            }
            else if (step_code === "w") {
     ...
     */
    async _compile_prompt_optimizer(student, trainset, valset_ratio) {
     ...
     */
    async _compile_weight_optimizer(student, trainset) {
        ...
        if (this.weight_optimizer.train_kwargs) {
            for (let i = 0; i < original_lms.length && i < new_lms.length; i++) {
                ...
                if (original_lm && new_lm) {
                    const original_params = this.weight_optimizer.train_kwargs[original_lm];
                    if (original_params) {
                        this.weight_optimizer.train_kwargs[new_lm] = original_params;
                    }
                }
            }
        }
     ...
     */
    _get_predictors(student) {
        const predictors = [];
        // In Stanford DSPy, student.predictors() returns a list of predictors
        if (typeof student.predictors === 'function') {
            return student.predictors();
        }
