...
export class GEPA extends Teleprompter {
    ...
    async compile(student, config) {
        ...
        if (this.auto !== null && this.auto !== undefined) {
            ...
            this.max_metric_calls = this.max_full_evals * (trainset.length + (valset ? valset.length : 0));
        }
        ...
        console.log(`Running GEPA for approx ${this.max_metric_calls} metric calls of the program. ` +
            `This amounts to ${(this.max_metric_calls / (valset === null ? trainset.length : trainset.length + valset.length)).toFixed(2)} full evals on the ${valset === null ? 'train' : 'train+val'} set.`);
    ...
    async optimize(config) {
        ...
        const seed_score = seed_results.reduce((sum, r) => sum + r.score, 0) / seed_results.length;
