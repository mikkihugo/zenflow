...
export class BootstrapFewShot extends Teleprompter {
   ...
   */
  private async _prepare_student_and_teacher(student: DSPyModule, teacher?: DSPyModule | null): Promise<void> {
    ...
    this.teacher = teacher !== null && teacher !== undefined ? teacher.deepcopy() : student.deepcopy();
  ...
  private async _bootstrap_one_example(example: Example, round_idx: number = 0): Promise<boolean> {
    ...
    try {
      ...
      if (this.metric) {
        ...
        if (this.metric_threshold !== null && this.metric_threshold !== undefined) {
