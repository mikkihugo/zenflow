...
import { getOptimalProvider, LLM_PROVIDER_CONFIG } from '@claude-zen/llm-routing';
...
export class FileAwareAIEngine {
  ...
  async initialize(): Promise<void> {
    logger.info('Initializing file-aware AI engine', { rootPath: this.rootPath });
  ...
  private selectProvider(request: FileAwareRequest, context: AnalyzedContext) {
    const contextLength = context.summary.length + 
   ...
   */
  private async prepareAIContext(context: AnalyzedContext, request: FileAwareRequest): Promise<string> {
    let aiContext = `# Task\n${request.task}\n\n`;
  ...
  private async createBackup(changes: FileChange[]): Promise<void> {
    const backupDir = join(this.rootPath, '.file-aware-ai-backups', Date.now().toString());
    ...
    for (const change of changes) {
      if (change.type === 'modify' || change.type === 'delete') {
        try {
          ...
          logger.debug('Created backup', { file: change.path, backup: backupPath });
        } catch (error) {
   ...
   */
  private async autoCommit(changes: FileChange[], branchName?: string): Promise<void> {
   ...
   */
  private mergeWithDefaults(config?: Partial<FileAwareConfig>): FileAwareConfig {
    const defaults: FileAwareConfig = {
      indexing: {
        ...
        supportedLanguages: ['ts', 'tsx', 'js', 'jsx', 'py', 'rs', 'go', 'java'],
