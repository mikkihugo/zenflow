...
export interface BaseExporter {
  ...
  getQueueSize(): number;
  getHealthStatus(): Promise<{ status: 'healthy' | 'degraded' | 'unhealthy'; lastSuccess?: number; lastError?: string; }>;
}
...
export class ExporterManager {
  ...
  async initialize(): Promise<void> {
    const initPromises = Array.from(this.exporters.entries()).map(async ([name, exporter]) => {
      try {
  ...
  async exportBatch(dataItems: TelemetryData[]): Promise<ExportResult[]> {
    const exportPromises = Array.from(this.exporters.entries()).map(async ([name, exporter]) => {
      ...
      if (config.signals) {
        filteredItems = dataItems.filter(item => config.signals!.includes(item.type));
      }
  ...
  async shutdown(): Promise<void> {
    const shutdownPromises = Array.from(this.exporters.entries()).map(async ([name, exporter]) => {
      try {
   ...
   */
  async getHealthStatus(): Promise<Record<string, { status: 'healthy' | 'degraded' | 'unhealthy'; lastSuccess?: number; lastError?: string; }>> {
...
export { JaegerExporter, OTLPExporter, PrometheusExporter, ConsoleExporter, FileExporter };