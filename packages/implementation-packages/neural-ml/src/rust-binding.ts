...
export class RustNeuralML {
  ...
  private async executeRustOptimization(task: RustOptimizationTask): Promise<any> {
  ...
  private async executeRustCommand(args: string[]): Promise<string> {
    if (this.rustPath === 'cargo') {
      ...
      return this.executeCommand('cargo', [
        'run', 
        '--release',
        ...features,
        '--'
 ...
 */
export function createRustNeuralML(config: RustMLConfig, logger: Logger): RustNeuralML {
