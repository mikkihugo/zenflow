...
import { mockLogger, mockConfig, createMockMemoryConfig, createMockMemoryBackend, createMockResult } from '../mocks/foundation-mocks';
...
describe('MemoryManager', () => {
  ...
  describe('Memory Operations', () => {
    ...
    it('should handle operations with specific backend', async () => {
      const backend1Config = { ...createMockMemoryConfig(), type: 'memory' as const };
      const backend2Config = { ...createMockMemoryConfig(), type: 'json' as const };
  ...
  describe('Error Handling and Resilience', () => {
    ...
    it('should validate input parameters', async () => {
      ...
      const invalidValueResult = await manager.store('key', Symbol('invalid') as any);
      expect(invalidValueResult.isOk() || invalidValueResult.isErr()).toBe(true);
    });
  });
