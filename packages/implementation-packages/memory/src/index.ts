...
export async function getMemorySystemAccess(config?: MemoryConfig): Promise<any> {
  ...
  return {
    createManager: (managerConfig?: MemoryConfig) => SimpleMemoryFactory.createBasicMemory({
      type: managerConfig?.backendConfig?.type as 'sqlite' | 'memory',
      path: managerConfig?.backendConfig?.path
    }),
    createStore: (storeId: string, options?: SessionMemoryStoreOptions) => manager.createStore(storeId, options),
...
export async function getMemoryManager(config?: MemoryConfig): Promise<MemoryManager> {
  const { MemoryManager } = await import('./memory');
...
export async function getMemoryStorage(storeId: string, config?: MemoryConfig): Promise<any> {
  const system = await getMemorySystemAccess(config);
  const store = await system.createStore(storeId);
  return {
    store: (key: string, value: any, options?: StoreOptions) => store.store(key, value, options),
...
export async function getSessionMemory(sessionId: string, config?: MemoryConfig): Promise<any> {
  const system = await getMemorySystemAccess(config);
