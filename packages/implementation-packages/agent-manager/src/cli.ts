...
program
  .name('agent-manager')
  .description('🐝 Ephemeral swarm orchestrator for cognitive diversity and task coordination')
  .version('1.0.0');
...
program
  ...
  .option('-c, --cognitive <types>', '🧠 Override cognitive types (researcher,coder,analyst,architect)')
  .option('--topology <topology>', '🔗 Override coordination topology (mesh,hierarchical,ring,star)')
  ...
  .option('--manual', '⚙️ Skip intelligent configuration and use manual settings')
  .option('--analyze-repo', '🔍 Force repository analysis (default: auto-detect)')
    ...
    try {
      ...
      if (options.manual) {
        ...
        cognitiveTypes = options.cognitive 
          ? options.cognitive.split(',').map((type: string) => type.trim()) as CognitiveArchetype[]
          : ['researcher', 'coder'];
        ...
        for (const type of cognitiveTypes) {
          if (!validTypes.includes(type)) {
            console.error(`❌ Invalid cognitive type: ${type}. Valid types: ${validTypes.join(', ')}`);
            process.exit(1);
          }
        }
        ...
        if (!validTopologies.includes(topology)) {
          console.error(`❌ Invalid topology: ${topology}. Valid topologies: ${validTopologies.join(', ')}`);
          process.exit(1);
        }
      } else {
        // Intelligent configuration mode (default)
        console.log('🧠 Analyzing task and repository for optimal swarm configuration...');
        
        try {
          const { generateIntelligentConfig, analyzeRepository } = await import('./intelligent-config');
          ...
          if (options.analyzeRepo) {
            ...
            console.log(`📊 Repository: ${repoAnalysis.projectType} (${repoAnalysis.complexity} complexity)`);
          }
          
          // Generate intelligent configuration
          recommendation = await generateIntelligentConfig(options.task, repoAnalysis);
          ...
          console.log(`   🧠 Cognitive Types: ${recommendation.cognitiveTypes.join(', ')}`);
          console.log(`   🔗 Topology: ${recommendation.topology}`);
          console.log(`   👥 Agent Count: ${recommendation.agentCount}`);
          console.log(`   📈 Confidence: ${recommendation.confidence}%`);
          
          console.log('\n💭 Reasoning:');
          console.log(`   📝 Task Analysis: ${recommendation.reasoning.taskAnalysis}`);
          console.log(`   🧩 Cognitive Choice: ${recommendation.reasoning.cognitiveRationale}`);
          console.log(`   🔗 Topology Choice: ${recommendation.reasoning.topologyRationale}`);
          if (repoAnalysis) {
            console.log(`   📁 Repository Influence: ${recommendation.reasoning.repositoryInfluence}`);
          }
          ...
          console.warn(`⚠️ Intelligent configuration failed, using fallback: ${error instanceof Error ? error.message : error}`);
      ...
      console.log(`   🧠 Cognitive Types: ${swarm.agents.map(a => a.archetype).join(', ')}`);
      ...
      for (const agent of swarm.agents) {
        ...
        console.log(`      ⚡ Decision Speed: ${agent.cognition.decisionSpeed}ms`);
        console.log(`      🧩 Patterns: ${agent.cognition.patterns.join(', ')}`);
        console.log(`      💪 Strengths: ${agent.cognition.strengths.join(', ')}`);
        console.log('');
      }
      
      console.log('💡 Next steps:');
      console.log(`   agent-manager exec ${swarm.id}     # Execute swarm coordination`);
      console.log(`   agent-manager list                  # View all active swarms`);
      console.log(`   agent-manager dissolve ${swarm.id}  # Clean up when done`);
      
    } catch (error) {
      console.error('❌ Failed to create swarm:', error instanceof Error ? error.message : error);
      process.exit(1);
    }
  });
...
program
  ...
  .option('--max-turns <turns>', '🔄 Maximum Claude SDK interactions for this execution', '50')
  .action(async (swarmId: string, options) => {
    try {
      ...
      console.log(`   🤝 Consensus: ${result.coordination.consensusReached ? '✅ Reached' : '❌ Not reached'}`);
      console.log(`   🔢 Total Decisions: ${result.coordination.totalDecisions}`);
      
      if (result.neuralMetrics) {
        console.log(`\n🧠 Neural Acceleration:`);
        console.log(`   🔧 WASM calls: ${result.neuralMetrics.wasmCallsExecuted}`);
        console.log(`   🚀 Acceleration gain: ${result.neuralMetrics.accelerationGain}x faster`);
      }
      ...
      for (const agentResult of result.agentResults) {
        ...
        console.log(`      ⚡ Avg decision time: ${agentResult.averageDecisionTime}ms`);
      ...
      console.error('❌ Failed to execute swarm:', error instanceof Error ? error.message : error);
      process.exit(1);
    }
  });
...
program
  ...
  .action(async (options) => {
    try {
      logger.debug('CLI list command executed', { workingDirectory: process.cwd() });
      const manager = await getManager();
      logger.debug('About to call getActiveSwarms()');
      const swarms = manager.getActiveSwarms();
      logger.debug('getActiveSwarms() completed', { swarmCount: swarms.length });
      ...
      for (const swarm of swarms) {
        ...
        console.log(`  Agents: ${swarm.agents.length} (${swarm.agents.map(a => a.archetype).join(', ')})`);
        console.log(`  Topology: ${swarm.topology}`);
        ...
        if (options.detailed) {
          ...
          console.log(`    Avg Decision Time: ${swarm.performance.averageDecisionTime}ms`);
          console.log(`    Coordination Events: ${swarm.performance.coordinationEvents}`);
          console.log(`    Claude Interactions: ${swarm.performance.claudeInteractions}`);
          console.log(`    Last Activity: ${swarm.performance.lastActivity.toLocaleString()}`);
        }
      ...
      console.error('❌ Failed to list swarms:', error instanceof Error ? error.message : error);
      process.exit(1);
    }
  });
...
program
  ...
  .action(async (swarmId: string) => {
    try {
      ...
      console.error('❌ Failed to dissolve swarm:', error instanceof Error ? error.message : error);
      process.exit(1);
    }
  });
...
program
  ...
  .action(async (swarmId: string) => {
    try {
      ...
      console.error('❌ Failed to pause swarm:', error instanceof Error ? error.message : error);
      process.exit(1);
    }
  });
...
program
  ...
  .action(async (swarmId: string) => {
    try {
      ...
      console.error('❌ Failed to resume swarm:', error instanceof Error ? error.message : error);
      process.exit(1);
    }
  });
...
program
  ...
  .action(async () => {
    try {
      ...
      console.log(`✅ Successful Coordinations: ${metrics.successfulCoordinations}`);
  ...
  .action(async (task: string, options) => {
    try {
      ...
      if (options.analyzeRepo) {
        ...
        repoAnalysis = await analyzeRepository();
        console.log(`📊 Repository: ${repoAnalysis.projectType} (${repoAnalysis.complexity} complexity)`);
        console.log(`🔧 Technologies: ${repoAnalysis.technologies.slice(0, 5).join(', ')}`);
        console.log(`📝 Has Tests: ${repoAnalysis.hasTests ? 'Yes' : 'No'}`);
        console.log(`📚 Has Docs: ${repoAnalysis.hasDocs ? 'Yes' : 'No'}\n`);
      }
      
      // Generate recommendation
      const recommendation = await generateIntelligentConfig(task, repoAnalysis);
      ...
      if (options.detailed) {
        ...
        console.log(`   📝 Task Analysis: ${recommendation.reasoning.taskAnalysis}`);
        console.log(`   🧩 Cognitive Rationale: ${recommendation.reasoning.cognitiveRationale}`);
        console.log(`   🔗 Topology Rationale: ${recommendation.reasoning.topologyRationale}`);
        console.log(`   👥 Agent Count Rationale: ${recommendation.reasoning.agentCountRationale}`);
        if (repoAnalysis) {
          console.log(`   📁 Repository Influence: ${recommendation.reasoning.repositoryInfluence}`);
        }
      }
      ...
      console.log(`   agent-manager create --task "${task}" ${recommendation.cognitiveTypes.length > 2 ? '--analyze-repo' : ''}`);
      
    } catch (error) {
      console.error('❌ Failed to analyze task:', error instanceof Error ? error.message : error);
      process.exit(1);
    }
  });
...
program
  ...
  .action(async () => {
    try {
      ...
      console.log(`✅ Foundation Logger: ${logger ? 'Available' : 'Unavailable'}`);
      ...
      try {
        ...
        console.log(wasmAvailable ? '✅ WASM Neural: Available' : '⚠️  WASM Neural: Not Available (optional)');
      } catch {
...
mcpCommand
  ...
  .action(() => {
    ...
    console.log('  claude mcp add agent-manager npx @claude-zen/agent-manager mcpserver\n');
    console.log('Available MCP tools:');
    console.log('  • create_swarm       - Create ephemeral swarms with cognitive diversity');
    console.log('  • execute_swarm      - Execute swarm coordination with <100ms decisions');
    console.log('  • list_swarms        - List active swarms with performance metrics');
    console.log('  • dissolve_swarm     - Dissolve swarms and clean up resources');
    console.log('  • pause_swarm        - Pause for Claude CLI session restart');
    console.log('  • resume_swarm       - Resume after session restart');
    console.log('  • get_metrics        - Performance metrics and cognitive distribution');
    console.log('  • health_check       - System health status and WASM availability');
    console.log('\nExample usage in Claude Code:');
    console.log('  "Create a swarm with researcher and coder agents for security analysis"');
