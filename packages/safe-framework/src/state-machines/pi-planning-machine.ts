...
export type PIPlanningEvent = 
  | { type: 'START_PLANNING'; startTime: Date }
  | { type: 'BUSINESS_CONTEXT_COMPLETE' }
  | { type: 'ARCHITECTURE_VISION_COMPLETE' }
  | { type: 'PLANNING_CONTEXT_COMPLETE' }
  | { type: 'BEGIN_DAY1_PLANNING' }
  | { type: 'TEAM_PLANNING_COMPLETE'; artId: string; capacity: TeamCapacity; features: Feature[] }
  | { type: 'DAY1_COMPLETE'; dependencies: Dependency[]; risks: Risk[] }
  | { type: 'BEGIN_DAY2_PLANNING' }
  | { type: 'DEPENDENCY_RESOLVED'; dependencyId: string }
  | { type: 'RISK_ADDRESSED'; riskId: string }
  | { type: 'OBJECTIVE_DEFINED'; objective: PIObjective }
  | { type: 'DAY2_COMPLETE'; objectives: PIObjective[] }
  | { type: 'CONFIDENCE_VOTE'; vote: number }
  | { type: 'PLANNING_ADJUSTMENTS_NEEDED'; adjustments: string[] }
  | { type: 'ADJUSTMENTS_COMPLETE' }
  | { type: 'COMMIT_TO_PI' }
  | { type: 'ABORT_PLANNING'; reason: string }
...
const piPlanningActions = {
  ...
  initializePlanning: assign({
    planningStartTime: ({ event }: { event: Extract<PIPlanningEvent, { type: 'START_PLANNING' }> }) => event.startTime,
  ...
  recordTeamPlanning: assign({
    totalCapacity: ({ context, event }) => {
      const teamEvent = event as Extract<PIPlanningEvent, { type: 'TEAM_PLANNING_COMPLETE' }>;
  ...
  captureDay1Artifacts: assign({
    ...
    dependencies: ({ event }: { event: Extract<PIPlanningEvent, { type: 'DAY1_COMPLETE' }> }) => event.dependencies,
    risks: ({ event }: { event: Extract<PIPlanningEvent, { type: 'DAY1_COMPLETE' }> }) => event.risks,
  }),
  ...
  storeObjectives: assign({
    piObjectives: ({ event }: { event: Extract<PIPlanningEvent, { type: 'DAY2_COMPLETE' }> }) => event.objectives,
  ...
  recordConfidenceVote: assign({
    confidenceVote: ({ event }: { event: Extract<PIPlanningEvent, { type: 'CONFIDENCE_VOTE' }> }) => event.vote,
  }),
  ...
  handleAdjustments: assign({
    blockers: ({ event }: { event: Extract<PIPlanningEvent, { type: 'PLANNING_ADJUSTMENTS_NEEDED' }> }) => event.adjustments,
  }),
   ...
   */
  logMilestone: ({ context, event }: { context: PIPlanningContext; event: PIPlanningEvent }) => {
