...
export type EpicKanbanEvent = 
  | { type: 'SUBMIT_FOR_ANALYSIS'; businessCase: BusinessCase }
  | { type: 'ANALYSIS_COMPLETE'; wsjfPriority: WSJFPriority }
  | { type: 'ANALYSIS_REJECTED'; reason: string }
  | { type: 'APPROVE_FOR_IMPLEMENTATION' }
  | { type: 'CAPACITY_AVAILABLE' }
  | { type: 'CAPACITY_FULL' }
  | { type: 'IMPLEMENTATION_STARTED'; startTime: Date }
  | { type: 'IMPLEMENTATION_BLOCKED'; blockers: string[] }
  | { type: 'UNBLOCK_IMPLEMENTATION' }
  | { type: 'IMPLEMENTATION_COMPLETE' }
  | { type: 'SPLIT_EPIC'; splitEpics: PortfolioEpic[] }
  | { type: 'CANCEL_EPIC'; reason: string }
  | { type: 'RETRY' };
...
const epicKanbanGuards = {
  ...
  withinTimebox: ({ context }: { context: EpicKanbanContext }) => {
    ...
    const weeksSinceStart = Math.floor(
      (Date.now() - context.implementationStartTime.getTime()) / (7 * 24 * 60 * 60 * 1000)
    );
...
const epicKanbanActions = {
  ...
  startAnalysis: assign({
    businessCase: ({ event }: { event: Extract<EpicKanbanEvent, { type: 'SUBMIT_FOR_ANALYSIS' }> }) => event.businessCase,
  ...
  storeWSJFPriority: assign({
    wsjfPriority: ({ event }: { event: Extract<EpicKanbanEvent, { type: 'ANALYSIS_COMPLETE' }> }) => event.wsjfPriority,
  }),
  ...
  handleAnalysisRejection: assign({
    errorMessage: ({ event }: { event: Extract<EpicKanbanEvent, { type: 'ANALYSIS_REJECTED' }> }) => event.reason,
  }),
  ...
  startImplementation: assign({
    implementationStartTime: ({ event }: { event: Extract<EpicKanbanEvent, { type: 'IMPLEMENTATION_STARTED' }> }) => event.startTime,
   ...
   */
  addBlockers: assign({
    blockers: ({ event }: { event: Extract<EpicKanbanEvent, { type: 'IMPLEMENTATION_BLOCKED' }> }) => event.blockers,
  }),
   ...
   */
  logTransition: ({ context, event }: { context: EpicKanbanContext; event: EpicKanbanEvent }) => {
