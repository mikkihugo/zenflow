...
import { executeSwarmCoordinationTask, getLogger, EnhancedError, getConfig } from '@claude-zen/foundation';
import { registerSwarm, getSwarm, getAllSwarms, updateSwarm, removeSwarm, getSwarmCount as getRegistrySwarmCount } from './swarm-registry';
/**
...
export class AgentManager extends EventEmitter {
    ...
    async executeSwarm(swarmId, options) {
        ...
        try {
            ...
            const coordinationResult = await executeSwarmCoordinationTask(this.buildSwarmExecutionPrompt(swarm), swarm.agents.map(a => a.id), {
            ...
            const enhancedError = new EnhancedError('Swarm execution failed', { originalError: error, swarmId });
    ...
    async resumeSwarm(swarmId) {
        ...
        this.logger.info('▶️ Swarm resumed after session restart', {
            swarmId,
            pausedDuration: Date.now() - swarm.resumption.checkpoint
        });
    ...
    async initialize(config) {
        try {
            ...
            const enhancedError = new EnhancedError('Failed to initialize AgentManager', { originalError: error });
    ...
    async shutdown() {
        try {
            ...
            const enhancedError = new EnhancedError('Error during AgentManager shutdown', { originalError: error });
     ...
     */
    instantiateCognitiveAgent(archetype, swarmId) {
     ...
     */
    getCapabilitiesForArchetype(archetype) {
        const capabilityMap = {
     ...
     */
    buildSwarmExecutionPrompt(swarm) {
