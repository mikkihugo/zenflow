...
import { createIntelligenceLinter } from './intelligence-linter';
...
export async function testSingularityLinting(): Promise<void> {
  ...
  try {
    ...
    const singularity = createSingularityIntegration({
      ...
      maxDepth: 3,
      focusAreas: ['complexity', 'performance', 'maintainability', 'type-safety'],
    ...
    logger.info(`   Confidence: ${(analysisResult.confidence * 100).toFixed(1)}%`);
    logger.info(`   Analysis time: ${analysisResult.performance.totalTimeMs}ms`);
    ...
    if (insights.agentAnalysis.length > 0) {
      logger.info(`   🤖 Agent Analysis (${insights.agentAnalysis.length} agents):`);
      insights.agentAnalysis.forEach((agent, index) => {
        ...
        logger.info(`        Execution: ${agent.performanceMetrics.executionTimeMs}ms`);
      });
    }
    ...
    logger.info(`     Complexity Score: ${codeMesh.complexityScore.toFixed(1)}`);
    logger.info(`     Architecture Quality: ${(codeMesh.architectureQuality * 100).toFixed(1)}%`);
    logger.info(`     Maintainability Index: ${(codeMesh.maintainabilityIndex * 100).toFixed(1)}%`);
    logger.info(`     Technical Debt Ratio: ${(codeMesh.technicalDebtRatio * 100).toFixed(1)}%`);
    logger.info(`     Health Indicators:`);
    logger.info(`       Type Safety: ${(codeMesh.healthIndicators.typesSafety * 100).toFixed(1)}%`);
    logger.info(`       Test Coverage: ${(codeMesh.healthIndicators.testCoverage * 100).toFixed(1)}%`);
    logger.info(`       Documentation: ${(codeMesh.healthIndicators.documentation * 100).toFixed(1)}%`);
    logger.info(`       Consistency: ${(codeMesh.healthIndicators.consistency * 100).toFixed(1)}%`);
    
    // Performance predictions
    if (insights.performancePredictions.length > 0) {
      logger.info(`   ⚡ Performance Predictions (${insights.performancePredictions.length} metrics):`);
      insights.performancePredictions.forEach((prediction, index) => {
        logger.info(`     ${index + 1}. ${prediction.metric}: ${prediction.predictedValue.toFixed(2)}`);
        logger.info(`        Confidence: ${(prediction.confidence * 100).toFixed(1)}%`);
        logger.info(`        Optimizations: ${prediction.optimizations.join(', ')}`);
      });
    }
    ...
    if (insights.fileAwareContext) {
      ...
      logger.info(`     Cross-file Patterns: ${fileContext.crossFilePatterns.length}`);
      logger.info(`     Project Context Insights: ${fileContext.projectContext.length}`);
    }
    ...
    if (systemInfo.hardware) {
      logger.info(`   Hardware Optimization: ${systemInfo.hardware.optimizationStrategy}`);
...
export async function testRealFileLinting(filePath: string): Promise<void> {
  ...
  try {
    ...
    logger.info(`📊 File info: ${path.basename(filePath)} (${language}, ${content.length} chars)`);
    ...
    logger.info(`   Overall Confidence: ${(analysisResult.confidence * 100).toFixed(1)}%`);
    logger.info(`   Analysis Time: ${analysisResult.performance.totalTimeMs}ms`);
    logger.info(`   Complexity Score: ${analysisResult.singularityInsights.codeMeshAnalysis.complexityScore.toFixed(1)}`);
    logger.info(`   Maintainability: ${(analysisResult.singularityInsights.codeMeshAnalysis.maintainabilityIndex * 100).toFixed(1)}%`);
    logger.info(`   Active Agents: ${analysisResult.singularityInsights.agentAnalysis.length}`);
    logger.info(`   Performance Predictions: ${analysisResult.singularityInsights.performancePredictions.length}`);
