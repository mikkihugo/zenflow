...
export class RecommendationEngine {
  ...
  async generateRecommendations(
    ...
    recommendations.push(...this.generateArchitectureRecommendations(repository));

    // Performance recommendations
    recommendations.push(...this.generatePerformanceRecommendations(repository));
   ...
   */
  private generateComplexityRecommendations(repository: RepositoryMetrics): AnalysisRecommendation[] {
    const recommendations: AnalysisRecommendation[] = [];
    ...
    if (complexity.cyclomatic > 50) {
      recommendations.push({
        ...
        rationale: 'High cyclomatic complexity indicates complex decision logic that should be simplified through refactoring.',
        effort: this.estimateEffort(16, 'medium', 'Refactoring complex functions requires careful analysis and testing'),
        benefits: [
    ...
    if (complexity.maintainabilityIndex < 30) {
      recommendations.push({
        ...
        rationale: 'Low maintainability index indicates code that is difficult to understand, modify, and extend.',
        effort: this.estimateEffort(24, 'high', 'Improving maintainability requires systematic refactoring'),
        benefits: [
    ...
    if (complexity.technicalDebt > 100) {
      recommendations.push({
        ...
        rationale: 'High technical debt slows down development and increases maintenance costs over time.',
        effort: this.estimateEffort(complexity.technicalDebt * 0.5, 'medium', 'Technical debt requires gradual, systematic cleanup'),
        benefits: [
    ...
    if (complexity.codeSmells.length > 10) {
      const criticalSmells = complexity.codeSmells.filter(s => s.severity === 'high' || s.severity === 'critical');
      ...
      if (criticalSmells.length > 0) {
        recommendations.push({
          ...
          rationale: 'Critical code smells indicate fundamental design or implementation issues that impact code quality.',
          effort: this.estimateEffort(criticalSmells.length * 2, 'medium', 'Each code smell requires analysis and targeted refactoring'),
          benefits: [
   ...
   */
  private generateDependencyRecommendations(repository: RepositoryMetrics): AnalysisRecommendation[] {
    const recommendations: AnalysisRecommendation[] = [];
    const dependencies = repository.dependencies;

    // Circular dependencies
    if (dependencies.circularDependencies.length > 0) {
      const criticalCycles = dependencies.circularDependencies.filter(c => c.severity === 'error');
      ...
      recommendations.push({
        ...
        rationale: 'Circular dependencies prevent proper modularization and can cause build issues, infinite loops, and testing difficulties.',
        effort: this.estimateEffort(dependencies.circularDependencies.length * 4, 'high', 'Breaking circular dependencies requires architectural changes'),
        benefits: [
        ...
        actionItems: dependencies.circularDependencies.slice(0, 5).map((cycle, index) => ({
          description: `Break circular dependency: ${cycle.cycle.join(' → ')}`,
          type: 'architecture-change' as const,
          estimatedHours: 4,
          dependencies: index > 0 ? [`Break circular dependency: ${dependencies.circularDependencies[index-1].cycle.join(' → ')}`] : []
        }))
      });
    }
    ...
    if (dependencies.coupling.instability > 0.8) {
      recommendations.push({
        ...
        rationale: 'High instability indicates that many modules depend on external dependencies, making the system fragile to changes.',
        effort: this.estimateEffort(20, 'high', 'Reducing instability requires architectural restructuring'),
        benefits: [
    ...
    if (dependencies.cohesion.lcom > 0.7) {
      recommendations.push({
        ...
        rationale: 'Low cohesion indicates that modules are doing too many unrelated things, making them harder to understand and maintain.',
        effort: this.estimateEffort(16, 'medium', 'Improving cohesion requires careful module reorganization'),
        benefits: [
   ...
   */
  private generateDomainRecommendations(repository: RepositoryMetrics): AnalysisRecommendation[] {
    const recommendations: AnalysisRecommendation[] = [];
    ...
    for (const domain of repository.domains) {
      if (domain.splitRecommendation?.shouldSplit) {
        ...
        recommendations.push({
          ...
          benefits: Object.entries(split.benefits).map(([key, value]) => 
   ...
   */
  private generateGitRecommendations(repository: RepositoryMetrics): AnalysisRecommendation[] {
    const recommendations: AnalysisRecommendation[] = [];
    ...
    if (git.hotFiles.length > 20) {
      ...
      recommendations.push({
        ...
        rationale: 'Hot files indicate unstable code that changes frequently, often due to poor design or multiple responsibilities.',
        effort: this.estimateEffort(criticalHotFiles.length * 6, 'medium', 'Each hot file needs analysis and potential refactoring'),
        benefits: [
    ...
    if (git.contributors < 3) {
      recommendations.push({
        ...
        rationale: 'Having few contributors creates bus factor risk and may indicate that knowledge is concentrated in too few people.',
        effort: this.estimateEffort(8, 'low', 'Improving collaboration requires process and cultural changes'),
        benefits: [
   ...
   */
  private generateArchitectureRecommendations(repository: RepositoryMetrics): AnalysisRecommendation[] {
    const recommendations: AnalysisRecommendation[] = [];
    ...
    if (repository.totalFiles > 500) {
      recommendations.push({
        ...
        rationale: 'Large repositories become harder to navigate, build times increase, and team coordination becomes more complex.',
        effort: this.estimateEffort(40, 'very-high', 'Repository splitting is a major architectural change'),
        benefits: [
   ...
   */
  private generatePerformanceRecommendations(repository: RepositoryMetrics): AnalysisRecommendation[] {
    const recommendations: AnalysisRecommendation[] = [];
    ...
    if (avgFileSize > 200) {
      recommendations.push({
        ...
        rationale: 'Large files often violate the Single Responsibility Principle and are harder to review, test, and maintain.',
        effort: this.estimateEffort(12, 'medium', 'File splitting requires careful analysis of responsibilities'),
        benefits: [
   ...
   */
  private generateSecurityRecommendations(repository: RepositoryMetrics): AnalysisRecommendation[] {
    const recommendations: AnalysisRecommendation[] = [];
    ...
    if (repository.dependencies.totalDependencies > 100) {
      recommendations.push({
        ...
        rationale: 'Many external dependencies increase security risks, maintenance burden, and potential for supply chain attacks.',
        effort: this.estimateEffort(8, 'low', 'Dependency audit is primarily analysis work'),
        benefits: [
   ...
   */
  private generateWorkflowRecommendations(repository: RepositoryMetrics): AnalysisRecommendation[] {
    const recommendations: AnalysisRecommendation[] = [];
    ...
    if (repository.complexity.cyclomatic > 30) {
      recommendations.push({
        ...
        description: 'High complexity detected. Implement automated code quality checks to prevent regression.',
        rationale: 'Automated quality gates help maintain code quality standards and catch issues early in development.',
        effort: this.estimateEffort(6, 'low', 'Setting up quality gates is primarily configuration work'),
        benefits: [
        ...
        actionItems: [
          ...
          },
          {
            ...
            type: 'tooling-change',
            estimatedHours: 2,
            dependencies: ['Set up ESLint/TSLint with complexity rules', 'Configure code coverage thresholds']
          }
        ]
      });
    }
  ...
  // Helper methods
  private estimateEffort(hours: number, difficulty: EffortEstimate['difficulty'], description: string): EffortEstimate {
    return {
  ...
  private getPriorityWeight(priority: AnalysisRecommendation['priority']): number {
    const weights = {
      urgent: 4,
      high: 3,
      medium: 2,
