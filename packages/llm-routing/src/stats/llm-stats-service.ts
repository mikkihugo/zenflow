...
export class LLMStatsService {
      ...
      summary: {
        ...
        successRate:
          filteredCalls.length > 0
            ? filteredCalls.filter((c) => c.success).length / filteredCalls.length
            : 0,
        averageResponseTime:
          filteredCalls.length > 0
            ? filteredCalls.reduce((sum, c) => sum + c.executionTime, 0) / filteredCalls.length
            : 0,
  ...
  private calculateSingleProviderStats(
    ...
    return {
      ...
      costEfficiency:
        calls.length > 0
          ? calls.reduce((sum, c) => sum + c.performance.costEfficiency, 0) / calls.length
          : 0,
  ...
  private calculateSystemHealth(calls: LLMCallRecord[]): LLMSystemHealth {
    ...
    const averageLatency =
      recentCalls.length > 0
        ? recentCalls.reduce((sum, c) => sum + c.executionTime, 0) / recentCalls.length
        : 0;
  ...
  private calculateTaskTypeRouting(calls: LLMCallRecord[]): Record<string, {
