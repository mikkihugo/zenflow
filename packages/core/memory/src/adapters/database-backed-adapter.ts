...
export class DatabaseBackedAdapter extends BaseMemoryBackend {
  ...
  async initialize():Promise<Result<void, Error>> {
    try {
      ...
      logger.error('Failed to initialize database-backed memory adapter', { error});
  ...
  async retrieve(key:string): Promise<Result<unknown, Error>> {
    ...
    try {
      ...
      logger.debug('Retrieved memory entry', { key, age:Date.now() - entry.timestamp});
  ...
  getCapabilities():BackendCapabilities {
    return {
      persistent:this.config.type === 'sqlite',      transactional:true,
      queryable:false,
      scalable:false,
      distributed:false,
