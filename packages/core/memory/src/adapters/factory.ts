...
export class MemoryBackendFactory {
  ...
  public async createBackend(
    ...
    const backend = new (backendClass as new (
      config: MemoryConfig
    ) => BaseMemoryBackend & BackendInterface)(fullConfig);
  ...
  public async getBackendCapabilities(
    ...
    const backendClass = await this.getBackendClass(type);
    const tempBackend = new (backendClass as new (
      config: MemoryConfig
    ) => BackendInterface)(this.defaultConfig);
