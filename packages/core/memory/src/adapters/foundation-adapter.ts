...
export class FoundationMemoryBackend extends BaseMemoryBackend {
  ...
  override async store(
    ...
    try {
      if (this.databaseSystem && typeof this.databaseSystem.store === 'function') {
  ...
  override async retrieve<T = JSONValue>(
    ...
    try {
      ...
      if (this.databaseSystem && typeof this.databaseSystem.retrieve === 'function') {
  ...
  override async delete(key:string, namespace = 'default'):Promise<boolean> {
    ...
    try {
      // Try database system first
      if (this.databaseSystem && typeof this.databaseSystem.delete === 'function') {
  ...
  override getCapabilities():BackendCapabilities {
    ...
    return {
      ...
      searchable:true,
      transactional:
        config.storageType === 'database' || config.storageType === ' hybrid',      vectorized:config.databaseType === 'lancedb',      distributed:false,
      concurrent:true,
      compression:false,
      encryption:false,
};
  ...
  private override updateStats(operation:string, size?:number): void {
    ...
    this.logger.debug(`Memory operation:${operation} at ${timestamp}`, {
      operation,
      size:size || 0,
      backend: 'foundation-adapter',      timestamp,
});
