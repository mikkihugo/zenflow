...
'      throw error;
  ...
  private checkHealthStatusChange(
    ...
    if (previous.healthy && !current.healthy) {
      ...
      if (consecutiveSuccesses >= this.config.healthyThreshold) {
        ...
        recordMetric('memory_node_status_change', 1, {
          nodeId:node.id,
          status: 'recovered',          consecutiveSuccesses,
});
}
}
  ...
  private updateHealthStats():void {
    ...
    recordMetric(
      'memory_health_stats_unhealthy_nodes',      this.stats.unhealthyNodes
    );
    recordMetric(
      'memory_health_stats_recovering_nodes',      this.stats.recoveringNodes
    );
}
  ...
  getOverallHealth():{
    ...
    if (totalNodes === 0) {
      return {
        ...
        summary: 'No nodes available',        details:this.stats,
};
}
