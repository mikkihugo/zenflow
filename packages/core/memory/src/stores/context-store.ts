...
export const DEFAULT_CONTEXT_CONFIG:ContextManagerConfig = {
  ...
  namespaces:{
    agents: 'context:agents',    swarms: 'context:swarms',    sessions: 'context:sessions',    global: 'context:global',},
};
...
export class ContextManager extends EventEmitter {
   ...
   */
  async saveContext(
    type: 'agent|swarm|session|global',    id:string,
  ...
  async loadAgentContext(agentId:string): Promise<AgentContext | null> {
    ...
    return {
      agentId,
      swarmId: 'unknown',      type: 'unknown',      instance:1,
      environment:{},
      preferences:{},
      history:[],
      metrics:{
...
export async function withContextLoading<T>(
  ...
  contextType: 'agent|swarm|session',  contextId:string,
