...
export class MemoryMonitor extends EventEmitter {
  ...
  private checkAlerts(metrics:MemoryMetrics): void {
    ...
    if (metrics.errorRate > thresholds.errorRate) {
      this.createAlert({
        type: 'error',        severity:
          metrics.errorRate > thresholds.errorRate * 2 ? 'critical' : ' warning',        message:`Error rate (${(metrics.errorRate * 100).toFixed(2)}%) exceeds threshold (${(thresholds.errorRate * 100).toFixed(2)}%)`,
        source: 'error_monitor',        metadata:{
  ...
  generateHealthReport():{
    ...
    if (!currentMetrics) {
      return {
        overall: 'critical',        score:0,
        details:{ error: 'No metrics available'},
        recommendations:[
          'Start metric collection',          'Register system components',],
};
}
