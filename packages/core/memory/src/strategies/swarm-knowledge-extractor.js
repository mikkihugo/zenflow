 ...
 */
import { TypedEventBase, getLogger, recordMetric, withTrace, TelemetryManager, } from '@claude-zen/foundation';
...
export class SwarmKnowledgeExtractor extends TypedEventBase {
    ...
    telemetry;
    lifecycleManager;
    brainCoordinator; // Will be dynamically imported
    mlCoordinator; // Will be dynamically imported
    patternRecognizer; // Will be dynamically imported
    sparcEngine; // Will be dynamically imported
    initialized = false;
    constructor(config) {
        super();
        this.config = config;
     ...
     */
    async extractKnowledge(sessionData) {
        if (!this.initialized) {
        ...
        return withTrace('extract-swarm-knowledge', async (span) => {
            ...
            try {
                ...
                const [successPatterns, performanceMetrics, learningOutcomes, failurePatterns, sparcInsights,] = await Promise.all([
     ...
     */
    async extractBeforeDeletion(entryId, entryData) {
        ...
        try {
            ...
            if (this.patternRecognizer) {
                ...
                if (successfulDecisions.length > 0) {
                    ...
                    patterns.push(...mlPatterns.map((p) => ({
    ...
    async extractPerformanceMetrics(sessionData) {
    ...
    async extractLearningOutcomes(sessionData) {
        const outcomes = [];
    ...
    async extractFailurePatterns(sessionData) {
        const patterns = [];
    ...
    async extractSPARCInsights(sessionData) {
        ...
        try {
            ...
            const bottlenecks = [];
            const optimizations = [];
            for (const [phase, metrics] of Object.entries(sessionData.sparcPhases)) {
    ...
    async calculateImportance(sessionData) {
    ...
    async calculateConfidence(sessionData) {
        ...
        const dataPoints = sessionData.decisions.length +
