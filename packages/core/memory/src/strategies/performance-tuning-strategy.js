 ...
 */
import { TypedEventBase, getLogger, recordMetric, TelemetryManager, withTrace, } from '@claude-zen/foundation';
export class PerformanceTuningStrategy extends TypedEventBase {
    ...
    getTrend(recent, older, higherIsBetter) {
        if (older === 0)
            return 'stable';
    ...
    async generateTuningRecommendations(analysis, snapshot) {
        const recommendations = [];
        // Response time optimization
        if (analysis.responseTimeTrend === 'degrading' ||
            ...
            if (this.config.actions.adjustCacheSize) {
                recommendations.push({
                    ...
                    parameters: {
                        ...
                        newSize: Math.min((this.tuningParameters.get('cacheSize')?.currentValue || 1000) *
                            1.2, this.tuningParameters.get('cacheSize')?.maxValue || 10000),
                    },
        ...
        if (analysis.throughputTrend === 'degrading' ||
        ...
        // Memory optimization
        if (analysis.memoryTrend === 'degrading' ||
    ...
    async applyTuningAction(recommendation) {
        ...
        switch (recommendation.action) {
            ...
            case 'increase_cleanup_frequency':
                ...
                break;
            case 'optimize_ttl': {
                const newTtl = recommendation.parameters.ttl;
    ...
    updateTuningParameter(name, value) {
    ...
    getPerformanceSnapshots(limit = 100) {
