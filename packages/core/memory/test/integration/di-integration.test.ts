...
describe('Memory System DI Integration', () => {
  ...
  describe('Configuration Integration', () => {
    it('should use configuration from DI container', async () => {
      const testConfig = {
        memory:{
          defaultBackend: 'sqlite',          maxSize:10000,
          ttl:3600000,
          compression:true,
},
        backends:{
          sqlite:{
            path: './data/memory.db',            maxConnections:10,
},
          lancedb:{
            path: './data/vectors',            dimensions:1536,
},
},
};
  ...
  describe('Event System Integration', () => {
    it('should integrate with event system through DI', async () => {
      ...
      eventBus.publish('memory.operation.store', {
        key: 'test-key',        backend: 'sqlite',        timestamp:Date.now(),
});

      expect(mockEventBus.publish).toHaveBeenCalledWith(
        'memory.operation.store',        expect.objectContaining({
          key: 'test-key',          backend: 'sqlite',})
      );
});
