...
export class VectorStorageImpl {
    connection;
    config;
    ...
    async search(vector, options) {
        try {
            ...
            const results = result.rows.map((row) => {
                ...
                const score = this.calculateCosineSimilarity(Array.from(vector), Array.from(storedVector));
                const vectorResult = {
                    id: row.id,
    ...
    async delete(id) {
        try {
            ...
            await this.ensureCollectionExists();
            const result = await this.connection.execute(`DELETE FROM "${this.collectionName}" WHERE id = ?`, [id]);
    ...
    async count() {
        try {
            await this.ensureCollectionExists();
            const result = await this.connection.query(`SELECT COUNT(*) as count FROM "${this.collectionName}"`);
    ...
    async getStats() {
        try {
            await this.ensureCollectionExists();
            const countResult = await this.connection.query(`SELECT COUNT(*) as count FROM "${this.collectionName}"`);
            const totalVectors = countResult.rows[0]?.count || 0;
            // Get sample vector to determine dimensions
            let dimensions = 0;
            if (totalVectors > 0) {
                const sampleResult = await this.connection.query(`SELECT vector FROM "${this.collectionName}" LIMIT 1`);
                if (sampleResult.rows.length > 0) {
                    const sampleVector = new Float32Array(sampleResult.rows[0].vector.buffer);
            ...
            return {
                ...
                memoryUsageMB: Math.round((totalVectors * dimensions * 4) / (1024 * 1024)), // Rough estimate
            };
        }
        catch (error) {
    ...
    async isHealthy() {
        try {
            await this.ensureCollectionExists();
            await this.connection.query(`SELECT COUNT(*) FROM "${this.collectionName}" LIMIT 1`);
            return true;
        }
        catch {
            return false;
