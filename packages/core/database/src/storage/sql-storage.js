...
class TransactionSQLStorageImpl {
    txConnection;
    config;
    constructor(txConnection, config) {
        this.txConnection = txConnection;
        this.config = config;
    }
    async query(sql, params) {
        return await this.txConnection.query(sql, params);
    }
    ...
    async getTableSchema(name) {
        ...
        const result = await this.query(`PRAGMA table_info(${name})`);
