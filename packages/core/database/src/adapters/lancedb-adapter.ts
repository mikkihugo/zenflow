...
export class LanceDBAdapter implements DatabaseConnection {
    ...
    try {
      ...
      try {
        ...
        throw new ConnectionError(
          'LanceDB package not found. Please install with:npm install @lancedb/lancedb',          correlationId,
  ...
  async createTableWithEmbedding(
    ...
    try {
      ...
      const sampleData = [
        {
          id: 'sample',          [schema.vectorColumn || 'vector']:new Array(
    ...
    try {
      ...
      if (table && 'createIndex' in table) {
        ...
        await indexableTable.createIndex(
          options.column || 'vector',          indexConfig
        );
}
