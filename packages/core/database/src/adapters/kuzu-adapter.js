...
export class KuzuAdapter {
    ...
    async connect() {
        ...
        try {
            ...
            try {
                ...
                this.kuzuModule = {
                    Database: kuzuImport.Database,
                    Connection: kuzuImport.Connection,
                };
                logger.debug('Successfully imported Kuzu module', { correlationId });
            }
            catch (importError) {
                logger.error('Failed to import Kuzu package - package may not be installed', {
                    correlationId,
                    error: importError instanceof Error
                        ? importError.message
                        : String(importError),
                });
                throw new ConnectionError('Kuzu package not found. Please install with:npm install kuzu', correlationId, importError instanceof Error ? importError : undefined);
            }
            ...
            try {
                ...
                this.database = new this.kuzuModule.Database(this.config.database, // databasePath
                ...
                logger.error('Failed to create Kuzu database or connection', {
                    correlationId,
                    database: this.config.database,
                    error: kuzuError instanceof Error ? kuzuError.message : String(kuzuError),
                });
                throw new ConnectionError(`Failed to create Kuzu database:${kuzuError instanceof Error ? kuzuError.message : String(kuzuError)}`, correlationId, kuzuError instanceof Error ? kuzuError : undefined);
            }
        }
        catch (error) {
            ...
            throw new ConnectionError(`Failed to connect to Kuzu:${error instanceof Error ? error.message : String(error)}`, correlationId, error instanceof Error ? error : undefined);
        }
    }
    async disconnect() {
        if (!this.isConnectedState) {
        ...
        try {
            ...
            throw new ConnectionError(`Failed to disconnect from Kuzu:${error instanceof Error ? error.message : String(error)}`, correlationId, error instanceof Error ? error : undefined);
        }
    }
    isConnected() {
    ...
    async getCurrentMigrationVersion() {
        try {
            const result = await this.query('MATCH (m:_Migration) RETURN m.version as version ORDER BY m.version DESC LIMIT 1');
     ...
     */
    async createNodeTable(tableName, properties, primaryKey) {
        if (!this.isConnected()) {
            await this.connect();
        }
        const correlationId = this.generateCorrelationId();
 ...
 */
async;
insertNodes(tableName, string, nodes, (Array));
Promise < void  > {
    : .isConnected()
};
{
...
try {
    for (const rel of relationships) {
        ...
        if (rel.properties && Object.keys(rel.properties).length > 0) {
            ...
            .join(',    ');
          cypher += `, CREATE;
            (from) - [$, { tableName }, { $ }, { relProps }];
        }
        - > (to) `;
} else {
          cypher += `;
        CREATE(from) - [$, { tableName }] -  > (to) `;
}
  ...
  private async createMigrationsTable():Promise<void> {
    try {
      await this.query(`;
        ...
        NOT;
        EXISTS;
        _Migration(version, STRING, name, STRING, applied_at, TIMESTAMP, PRIMARY, KEY(version)) `);
} catch (error) {
