...
export function quickStart(
  type:'sqlite' | ' lancedb' | ' kuzu',  database:string,
...
export const defaultConfigs = {
  development:{
    sqlite:{
      ...
      database: './dev.db',      pool:{ min: 1, max:5},
      retryPolicy:{
        maxRetries:2,
        initialDelayMs:100,
        maxDelayMs:1000,
    ...
    lancedb:{
      ...
      database: './dev_vectors',      retryPolicy:{
        maxRetries:2,
        initialDelayMs:100,
        maxDelayMs:1000,
        backoffFactor:2,
    ...
    kuzu:{
      type:'kuzu' as const,
      database: './dev_graph',      retryPolicy:{
        maxRetries:2,
        initialDelayMs:100,
        maxDelayMs:1000,
        backoffFactor:2,
  ...
  production:{
    sqlite:{
      type:'sqlite' as const,
      database: './prod.db',      pool:{ min: 5, max:50},
      retryPolicy:{
        maxRetries:5,
        initialDelayMs:200,
        maxDelayMs:10000,
    ...
    lancedb:{
      type:'lancedb' as const,
      database: './prod_vectors',      retryPolicy:{
        maxRetries:5,
        initialDelayMs:200,
        maxDelayMs:10000,
        backoffFactor:2,
    ...
    kuzu:{
      type:'kuzu' as const,
      database: './prod_graph',      options:{ bufferPoolSize: 4 * 1024 * 1024 * 1024, maxNumThreads:16},
      retryPolicy:{
        maxRetries:5,
        initialDelayMs:200,
        maxDelayMs:10000,
