...
fn generate_csv_data(
  ...
  for series in 0..num_series {
    for t in 0..length {
      ...
      for f in 0..features {
        let value =
          (t as f64 * 0.1 + f as f64).sin() * 10.0 + 50.0 + series as f64 * 5.0;
...
fn bench_csv_loading(c: &mut Criterion) {
  ...
  for (size_name, num_series, length, features) in data_sizes {
    ...
    group.bench_with_input(
      ...
      |b, path| {
        b.iter(|| {
          let df = LazyFrame::scan_csv(black_box(path), Default::default())
            .collect()
            .unwrap();
          black_box(df);
        });
        ...
        b.iter(|| {
          let loader = CSVDataLoader::new(path.clone())
            ...
            .with_feature_columns(
              (0..features).map(|i| format!("feature_{}", i)).collect(),
            );
...
fn bench_preprocessing(c: &mut Criterion) {
  ...
  let data_sizes = vec![("small", 1000), ("medium", 10000), ("large", 100000)];
...
fn bench_feature_engineering(c: &mut Criterion) {
  ...
  let sequence_lengths =
    vec![("short", 100), ("medium", 1000), ("long", 10000)];
  ...
  for (seq_name, seq_len) in sequence_lengths {
    ...
    let values: Vec<f64> = (0..seq_len)
      .map(|i| {
        ...
        let seasonal =
          (i as f64 * 2.0 * std::f64::consts::PI / 24.0).sin() * 10.0;
      ...
      |b, series| {
        b.iter(|| {
          let temporal_features =
            extract_temporal_features(&series.timestamps());
      ...
      |b, series| {
        b.iter(|| {
          ...
          let fourier_features =
            generate_fourier_features(series.len(), &periods, 3);
    ...
    group.bench_with_input(
      ...
      |b, series| {
        b.iter(|| {
          ...
          let features =
            feature_engine.generate_features(black_box(series)).unwrap();
          black_box(features);
        });
      },
    );
...
fn bench_data_validation(c: &mut Criterion) {
  ...
  for (size_name, num_series, length) in dataset_sizes {
    ...
    group.bench_with_input(
      ...
      |b, dataset| {
        b.iter(|| {
          let validator = DataValidator::new().with_missing_value_check(true);
    ...
    group.bench_with_input(
      ...
      |b, dataset| {
        b.iter(|| {
          let validator =
            DataValidator::new().with_temporal_consistency_check(true);
...
fn bench_augmentation(c: &mut Criterion) {
  ...
  let data_sizes = vec![("small", 1000), ("medium", 5000), ("large", 10000)];
...
fn bench_array_operations(c: &mut Criterion) {
  ...
  for (size_name, rows, cols) in array_sizes {
    let data: Vec<f64> =
      (0..rows * cols).map(|i| (i as f64 * 0.01).sin()).collect();
