...
fn bench_basic_models(c: &mut Criterion) {
  ...
  for (size_name, size) in data_sizes {
    ...
    group.bench_with_input(
      ...
      |b, data| {
        b.iter(|| {
          let config = NLinearConfig::new(24, 12).with_individual(false);
...
fn bench_recurrent_models(c: &mut Criterion) {
  ...
  let sequence_lengths = vec![("short", 50), ("medium", 200), ("long", 1000)];
...
fn bench_transformer_models(c: &mut Criterion) {
  ...
  let sequence_lengths = vec![("short", 96), ("medium", 192), ("long", 336)];
...
fn bench_specialized_models(c: &mut Criterion) {
  ...
  let input_sizes = vec![("small", 48), ("medium", 96), ("large", 192)];
  ...
  for (size_name, layers) in layer_configs {
    group.bench_with_input(
      ...
      |b, layers| {
        b.iter(|| {
          let config =
            MLPConfig::new(24, 12).with_hidden_layers(layers.clone());
          let model = MLP::new(config).unwrap();
          black_box(model);
        });
      },
