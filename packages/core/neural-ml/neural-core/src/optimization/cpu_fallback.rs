...
use super::{OptimizedOps, ActivationType as SuperActivationType};

/// CPU-optimized matrix operations implementation
pub struct CpuOptimizedOps;

impl CpuOptimizedOps {
    pub fn new() -> Self {
        log::info!("Initializing CPU-optimized matrix operations");
        Self
    }
}

impl OptimizedOps<f32> for CpuOptimizedOps {
    fn matrix_multiply(&self, a: &[f32], b: &[f32], m: usize, n: usize, k: usize) -> Vec<f32> {
        matrix_multiply_parallel(a, b, m, n, k)
    }

    fn vector_add(&self, a: &[f32], b: &[f32]) -> Vec<f32> {
        vector_add_optimized(a, b)
    }

    fn vector_multiply(&self, a: &[f32], b: &[f32]) -> Vec<f32> {
        vector_multiply_optimized(a, b)
    }

    fn reduce_sum(&self, values: &[f32]) -> f32 {
        reduce_sum_parallel(values)
    }

    fn neural_activation(&self, values: &[f32], activation: SuperActivationType) -> Vec<f32> {
        let local_activation = match activation {
            SuperActivationType::ReLU => ActivationType::ReLU,
            SuperActivationType::Sigmoid => ActivationType::Sigmoid,
            SuperActivationType::Tanh => ActivationType::Tanh,
            SuperActivationType::Gelu => ActivationType::Gelu,
        };
        neural_activation(values, local_activation)
    }
}