...
export declare class BayesianOptimizer {
  ...
  predict(parameters: number[]): Promise<
    MLResult<{
      mean: number;
      variance: number;
    }>
...
export declare class PatternRecognizer {
  ...
  detectTrends(data: number[]): Promise<
    MLResult<{
      trend: string;
      strength: number;
      significance: number;
  ...
  >;
  detectPeriodicity(data: number[]): Promise<
    MLResult<{
      periods: number[];
      strengths: number[];
    }>
 ...
 */
export declare function getAvailableAlgorithms(): Promise<
  MLResult<Record<string, string[]>>
>;
/**
 * Validate configuration objects before passing to WASM
 */
export declare function validateConfig<T extends Record<string, any>>(
  config: T,
