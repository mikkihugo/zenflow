...
export class EventLogger {
  ...
  private static foundationConfigEnabled = process.env['CLAUDE_EVENT_LOGGING'] === ' true';
  ...
  static logWithContext(eventName:string, payload:unknown, context:{
    ...
    const ctx = {
      component:context.component || 'unknown',      phase:context.phase,
      timestamp:context.timestamp || new Date()
};

    logger.info(`📡 Event: ${eventName} [${ctx.component}${ctx.phase ? `:${ctx.phase}` : ''}]`);
 ...
 * Quick error logging
 */
export function logError(eventName:string, error:Error | string, component?:string): void {
