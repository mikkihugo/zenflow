...
export class DynamicEventRegistry extends EventBus {
  ...
  constructor() {
    ...
    EventLogger.log('dynamic-registry:initialized', { registryId:this.constructor.name});
}
  ...
  private setupEventDiscovery():void {
    ...
    this.on('registry:module-register', this.handleModuleRegistration.bind(this));
    this.on('registry:module-unregister', this.handleModuleUnregistration.bind(this));
  ...
  private updateMetrics():void {
    ...
    this.metricsHistory.push({ timestamp:now, metrics:{ ...this.eventMetrics}});
  ...
  private processEventBuffer():void {
    ...
    this.eventBuffer = this.eventBuffer.filter(flow => flow.timestamp.getTime() > cutoff);
}
  ...
  private checkModuleHealth():void {
    ...
    for (const [moduleId, module] of this.activeModules) {
      ...
      if (timeSinceLastSeen > staleThreshold && module.status === 'active') {
          module.status = 'idle';
          EventLogger.logError('registry:module-idle', `Module ${moduleId} is idle`, { component: ' DynamicEventRegistry'});
}
      
      if (timeSinceLastSeen > staleThreshold * 3) {
        module.status = 'disconnected';
        EventLogger.logError('registry:module-disconnected', `Module ${moduleId} disconnected`, { component: ' DynamicEventRegistry'});
}
}
}
  ...
  getEventFlows(eventName?:EventName): EventFlow[] {
    ...
    return allFlows.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime());
}
   ...
   */
  getDynamicEventCatalog():Record<string, {
    type:string;
    flows:number;
    lastSeen?:Date;
  ...
  private inferModuleType(moduleType:string): ActiveModule['type'] {
    const typeMap:Record<string, ActiveModule['type']> = {
      'sparc': 'sparc',      'brain': 'brain',      'dspy': 'dspy',      'teamwork': 'teamwork',      'llm': 'llm',      'git': 'git',      'system': 'system',      'safe': 'safe',      'claude-code': 'claude-code'};
  ...
  private findModuleByName(name:string): ActiveModule | undefined {
    for (const module of this.activeModules.values()) {
      if (module.name.toLowerCase().includes(name.toLowerCase()) || 
          module.type === name) {
        return module;
}
}
  ...
  private getModulesForEvent(eventName:EventName): ActiveModule[] {
    return Array.from(this.activeModules.values()).filter(
      module => module.events.includes(eventName)
    );
}
 ...
 */
export function sendModuleHeartbeat(moduleId:string, metadata?:Record<string, unknown>):void {
