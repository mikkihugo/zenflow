...
class LoggingConfigurationManager {
  ...
  private async setupInternalCollector() {
    ...
    try {
      ...
      if (otelConfig.useInternalOtelCollector && otelConfig.zenOtelEnabled) {
        const isCollectorReachable = await this.checkCollectorAvailability(
          otelConfig.internalCollectorEndpoint
        );

        if (isCollectorReachable) {
          config.internalCollectorSink = this.createInternalCollectorSink(
            otelConfig.internalCollectorEndpoint
          );
  ...
  private createWrappedLogger(
    name: string,
    logTapeLogger: LogTapeLogger,
    componentLevel: string
  ): Logger {
    return {
      trace: this.createLoggerMethod(
        name,
        LoggingLevel.TRACE,
        logTapeLogger.debug.bind(logTapeLogger),
        componentLevel
      ),
      debug: this.createLoggerMethod(
        name,
        LoggingLevel.DEBUG,
        logTapeLogger.debug.bind(logTapeLogger),
        componentLevel
      ),
      info: this.createLoggerMethod(
        name,
        LoggingLevel.INFO,
        logTapeLogger.info.bind(logTapeLogger),
        componentLevel
      ),
      warn: this.createLoggerMethod(
        name,
        LoggingLevel.WARN,
        logTapeLogger.warn.bind(logTapeLogger),
        componentLevel
      ),
      error: this.createLoggerMethod(
        name,
        LoggingLevel.ERROR,
        logTapeLogger.error.bind(logTapeLogger),
        componentLevel
      ),
      fatal: this.createLoggerMethod(
        name,
        LoggingLevel.FATAL,
        logTapeLogger.error.bind(logTapeLogger),
        componentLevel
  ...
  getLogger(name: string): Logger {
    ...
    const logger = this.createWrappedLogger(
      name,
      logTapeLogger,
      componentLevel
    );
...
// =============================================================================
// REAL-TIME LOGGING COORDINATION - For WebSocket broadcasting
// =============================================================================

// Internal log entry storage for real-time broadcasting
interface LogEntry {
  timestamp: string;
  level: LoggingLevel;
  category: string;
...
let logBroadcaster: ((event: string, data: unknown) => void) | null = null;
 ...
 */
export function getLogEntries(limit: number = 100): LogEntry[] {
