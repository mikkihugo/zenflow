...
export class LogTapeSyslogBridge {
    ...
    sendToSystemLog(level, message) {
        try {
            ...
            process.stderr.write(`SYSLOG_ERROR: Failed to write to syslog: ${errorMsg}\n`);
        }
    }
    /**
...
export const logToSyslog = {
    ...
    warn: (component, message, metadata) => syslogBridge.warn(component, message, metadata),
    error: (component, message, metadata) => syslogBridge.error(component, message, metadata),
    debug: (component, message, metadata) => syslogBridge.debug(component, message, metadata),
};
