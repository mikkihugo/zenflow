...
export function createRetryStrategy(options: {
 ...
 ((errorInfo.metadata?.['retryCount'] as number) ?? 0) < maxRetries
 ),
 recover: async (errorInfo) => {
 const retryCount =
 ((errorInfo.metadata?.['retryCount'] as number) ?? 0) + 1;
...
export function createRateLimitStrategy(options: {
 ...
 return (
 errorInfo.errorType === 'RateLimitError' ||
 ...
 */
function convertToRecoveryStrategy(
 simple: SimpleRecoveryStrategy
): RecoveryStrategy {
 return {
 id: simple.id,
 name: simple.name,
