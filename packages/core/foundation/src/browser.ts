...
export const createContainer = (): Container => {
  ...
  return {
    ...
    resolve: <T>(key: string): T | undefined =>
      services.get(key) as T | undefined,
...
export const concurrent = <T>(promises: Promise<T>[]): Promise<T[]> =>
  Promise.all(promises);
export const pTimeout = <T>(promise: Promise<T>, ms: number): Promise<T> =>
...
export const isoStringFromTimestamp = (timestamp: number) =>
  new Date(timestamp).toISOString();
