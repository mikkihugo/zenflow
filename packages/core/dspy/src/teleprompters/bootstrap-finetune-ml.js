...
export class BootstrapFinetuneML extends Teleprompter {
	...
	async shouldStopEarly(_iteration) {
				...
				return (
					...
					(config.initialLearningRate - config.minLearningRate) *
						...
						(1 +
							Math.cos(
								(Math.PI * iteration) /
									this.config.bayesianConfig.maxIterations,
							))
				);
			case "adaptive": {
	...
	async selectBestFromParetoFront(paretoFront, _candidates) {
	...
	async performFinetuneStep(module, _trainset, _config, _teacher) {
		// Perform one fine-tuning step
		return module;
	}
	async performArchitectureSearch(_student, _trainset, _valset) {
	...
	async saveCheckpoint(_module, step) {
