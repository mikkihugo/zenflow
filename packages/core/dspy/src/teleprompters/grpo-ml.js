...
export class GRPOML extends Teleprompter {
	...
	initializeEpisodeState(_student) {
	...
	async executeAction(_student, state, action, _options) {
		...
		for (let i = 0; i < state.length; i++) {
			nextStateArray[i] = Math.max(
				0,
				Math.min(1, state[i] + (Math.random() - 0.5) * 0.1),
			);
		}
	...
	generateExplorationRateHistory() {
		...
		return Array.from({ length: this.currentEpisode }, (_, i) => {
			return baseRate * this.config.decayRate ** i;
		});
	}
