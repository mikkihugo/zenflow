...
export class BootstrapFewShot extends Teleprompter {
	...
	private _prepare_predictor_mappings():void {
		...
		for (const [i, [name1, predictor1]] of studentNamedPredictors.entries()) {
			...
			if (
				...
				throw new Error(
					`Student and teacher must have the same signatures. ${JSON.stringify(predictor1.signature)} != ${JSON.stringify(predictor2.signature)}`,
				);
}
	...
	private async _bootstrap_one_example(
		...
		try {
			...
			if (this.metric) {
				...
				success = this.metric_threshold !== null &&
					this.metric_threshold !== undefined ? (metric_val as number) >= this.metric_threshold:Boolean(metric_val);
}
