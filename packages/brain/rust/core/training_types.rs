...
pub trait TrainingAlgorithm<T> {
    fn train_step(&mut self, data: &TrainingData<T>) -> Result<TrainingState, TrainingError>;
}
...
where 
    T: Clone + Into<f64>,
{
    fn train_step(&mut self, _data: &TrainingData<T>) -> Result<TrainingState, TrainingError> {
