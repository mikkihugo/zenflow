...
export class BehavioralIntelligence {
  ...
  private agentProfiles: Map<string, AgentBehavioralProfile> = new Map();
  private trainingBuffer: AgentExecutionData[] = [];
  private readonly bufferSize = 100;
  
  // 🧠 Enhanced ML Models - Using validated packages
  private behaviorClusterer?: any; // DBSCAN instance
  private kmeansClusterer?: any; // kmeans function
  private performanceTimeSeries: Map<string, any> = new Map(); // Will use SMA from moving-averages
   ...
   */
  private async updateAgentPerformanceTimeSeries(executionData: AgentExecutionData): Promise<void> {
   ...
   */
  private async updateAgentFeatureVector(executionData: AgentExecutionData): Promise<void> {
    const features = [
  ...
  async getAgentBehavioralClusters(): Promise<Map<number, string[]>> {
    ...
    clusters.forEach((cluster: number[], clusterId: number) => {
      if (cluster.length > 0) {
        clusterMap.set(clusterId, cluster.map((pointIndex: number) => agentIds[pointIndex]));
      }
    });
  ...
  async predictPerformanceTrend(agentId: string): Promise<{
    ...
    const regressionData: Array<[number, number]> = history.map((value, idx) => [idx, value]);
  ...
  getEnhancedStats(): {
    ...
    if (this.behaviorClusterer) mlModelsActive.push('DBSCAN');
    if (this.kmeansClusterer) mlModelsActive.push('K-Means');
    if (this.performanceTimeSeries.size > 0) mlModelsActive.push('Time Series');
  ...
  private async updateAgentProfile(executionData: AgentExecutionData): Promise<void> {
  ...
  private prepareComplexityInput(taskType: string, context: Record<string, unknown>): number[] {
    return [
      this.encodeTaskType(taskType),
  ...
  private mapComplexityToDifficulty(complexity: number): 'easy' | 'medium' | 'hard' | 'expert' {
 ...
 */
export async function demoBehavioralIntelligence(brainJsBridge: BrainJsBridge): Promise<void> {
