...
// Try to re-export everything from real package
try {
  const sparcPackage = require('@claude-zen/sparc');
  
  // Re-export specific items we know exist
  if (sparcPackage.SPARC) {
    export const SPARC = sparcPackage.SPARC;
  }
  if (sparcPackage.SPARCEngineCore) {
    export const SPARCEngineCore = sparcPackage.SPARCEngineCore;
  }
  if (sparcPackage.SPARCCommander) {
    export const SPARCCommander = sparcPackage.SPARCCommander;
  }
  if (sparcPackage.SafeSparcWorkflow) {
    export const SafeSparcWorkflow = sparcPackage.SafeSparcWorkflow;
  }
  
  logger.info('Successfully exported real SPARC package classes');
} catch (error) {
  logger.warn('Could not load real SPARC package for direct exports:', error);
}
