...
async function testCorrectEndpoint() {
  ...
  try {
      ...
      body: {
        messages: [
          ...
          },
          {
            role: 'user',
            content:
              "Analyze the benefits of GitHub Models. Provide response as JSON with 'analysis', 'benefits' array, and 'score' (0-1).",
          },
        ],
        model: model,
        temperature: 0.1,
    ...
    if (isUnexpected(jsonResponse)) {
      throw new Error(
        `JSON API Error: ${JSON.stringify(jsonResponse.body?.error || jsonResponse.body)}`,
      );
    }
    ...
    const response = await client.path('/chat/completions').post({
      body: {
        messages: [
          ...
          {
            role: 'user',
            content: 'Analyze GitHub Models benefits for developers.',
          },
        ],
        model: model,
        response_format: {
