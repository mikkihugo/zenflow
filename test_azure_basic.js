...
async function testBasicAzure() {
  ...
  try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a helpful assistant. Respond in valid JSON format.',
          },
          {
            role: 'user',
            content:
              "Explain the benefits of Azure AI inference for GitHub Models in exactly 3 points. Format as JSON with 'benefits' array.",
          },
        ],
        model: 'gpt-4o',
        temperature: 0.1,
    ...
    try {
      ...
      console.log(
        'This shows the importance of structured output for guaranteed JSON responses',
      );
    }
  } catch (error) {
