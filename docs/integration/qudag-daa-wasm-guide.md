...
claude-zen-neural.wasm: $(SOURCES)
	$(CC) $(CFLAGS) $(TARGET) -o $@ $^
	wasm-opt -O3 --enable-simd --enable-bulk-memory $@ -o $@

# Verify WASM module
verify: claude-zen-neural.wasm
	wasm-validate $<
...
npx claude-zen@2.0.0 neural profile --memory-trace
```
...
npx claude-zen@2.0.0 neural validate --all-models
...
npx claude-zen@2.0.0 neural profile --wasm-trace

# Check SIMD optimization
npx claude-zen@2.0.0 neural status | grep -i simd

# Analyze bottlenecks
npx claude-zen@2.0.0 bottleneck analyze --component neural
```
