...
- **Confusing tool naming**: Both `swarm/*` and `mcp__claude-zen__*` tools
...
mcp__claude-zen__task_orchestrate({ task: "Research AI patterns" })
```

#### 3.2 CLI Command Usage
```bash
# Current pattern - requires shell execution
claude-zen ruv-swarm init --topology mesh --max-agents 8
claude-zen ruv-swarm spawn researcher --name "AI Researcher"
```
...
import { RuvSwarm } from '@claude-zen/ruv-swarm-sdk';
...
class MCPBridge {
  ...
  private mapToMCPTool(sdkMethod: string): string {
    const mapping: Record<string, string> = {
      'swarm.init': 'mcp__claude-zen__swarm_init',
      'agent.spawn': 'mcp__claude-zen__agent_spawn',
      'task.orchestrate': 'mcp__claude-zen__task_orchestrate',
      // ... more mappings
    };
...
1. Create `@claude-zen/ruv-swarm-sdk` package
...
```typescript
import { MockSwarm } from '@claude-zen/ruv-swarm-sdk/testing';
