...
async function testMeshTopology(wasm, agentCount) {
  ...
  try {
    ...
    results.connectivity.connectivityRatio =
      status.topology.connections / expectedConnections;
    ...
    results.scalability.performanceRatio =
      1000 / (orchestrationTime * Math.log(agentCount));
...
async function testStarTopology(wasm, agentCount) {
  ...
  try {
    ...
    results.avgPeripheralSpawnTime =
      totalPeripheralSpawnTime / (agentCount - 1);
    ...
    const avgTaskTime =
      tasks.reduce((sum, t) => sum + t.orchestrationTime, 0) / tasks.length;
    ...
    // Scalability metrics
    results.scalability.agentCount = agentCount;
    results.scalability.hubMemoryUsage =
      status.performance.total_memory_usage_mb / agentCount;
    results.scalability.communicationOverhead =
      avgTaskTime * Math.log(agentCount);
  } catch (error) {
    testResults.failures.push({
      test: 'star_topology',
      agentCount,
...
async function testHierarchicalTopology(wasm, agentCount) {
  ...
  try {
    ...
    results.levels.distribution = Object.entries(agentsByLevel).map(
      ([level, agents]) => ({
    ...
    results.propagation.levelsInvolved = Math.ceil(
      Math.log2(rootTask.assigned_agents.length),
    );
    ...
    results.coordination.avgConnectionsPerAgent =
      status.topology.connections / agentCount;
    results.coordination.hierarchyEfficiency =
      1 - status.topology.connections / (agentCount * (agentCount - 1));

    // Scalability analysis
    results.scalability.agentCount = agentCount;
    results.scalability.communicationComplexity = Math.log2(agentCount);
    results.scalability.memoryEfficiency =
      status.performance.total_memory_usage_mb /
      (agentCount * Math.log2(agentCount));
  } catch (error) {
    testResults.failures.push({
...
async function testRingTopology(wasm, agentCount) {
  ...
  try {
    ...
    results.connectivity.actualConnections = status.topology.connections;
    results.connectivity.isProperRing =
      status.topology.connections === agentCount;
    ...
    results.propagation.avgLatencyPerHop =
      propagationTasks.reduce(
        (sum, t) => sum + t.orchestrationTime / t.distance,
        0,
      ) / propagationTasks.length;

    // Calculate maximum propagation distance
    results.latency.maxDistance = Math.floor(agentCount / 2);
    results.latency.worstCaseLatency =
      results.latency.maxDistance * results.propagation.avgLatencyPerHop;
    ...
    results.scalability.scalabilityScore =
      1000 / (results.latency.worstCaseLatency * Math.log(agentCount));
  } catch (error) {
    testResults.failures.push({
      test: 'ring_topology',
      agentCount,
...
async function runTopologyValidation() {
  ...
  for (const topology of TEST_CONFIG.topologies) {
    ...
    for (const agentCount of TEST_CONFIG.scalabilityTests) {
      ...
      if (
        results &&
        results.scalability &&
        results.scalability.performanceRatio !== undefined
      ) {
        const perfRatio =
          results.scalability.performanceRatio ||
          results.scalability.scalabilityScore ||
  ...
  testResults.coordination_efficiency = calculateCoordinationEfficiency(
    testResults.topologies_tested,
  );

  // Save results to file
  const outputPath = path.join(
    __dirname,
    'swarm-topology-validation-results.json',
  );
  ...
  console.log('\nTest Summary:');
  console.log(
    `- Topologies tested: ${Object.keys(testResults.topologies_tested).length}`,
  );
...
function calculateCoordinationEfficiency(topologyResults) {
  ...
  for (const [topology, agentTests] of Object.entries(topologyResults)) {
    ...
    for (const [agentCount, results] of Object.entries(agentTests)) {
      if (results && results.orchestrationTime) {
        ...
        const memoryPerAgent =
          (results.scalability?.memoryUsage || 0) / parseInt(agentCount);
    ...
    if (testCount > 0) {
      efficiency[topology].avgOrchestrationTime =
        totalOrchestrationTime / testCount;
