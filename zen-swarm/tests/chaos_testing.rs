...
async fn create_chaos_swarm(
  chaos_config: ChaosConfig,
) -> Result<(Swarm, ChaosEngine), Box<dyn std::error::Error>> {
...
mod tests {
  ...
  async fn test_random_agent_failures() {
    ...
    let (mut swarm, chaos_engine) =
      create_chaos_swarm(chaos_config).await.unwrap();
    ...
    // Start chaos engine
    let chaos_handle = tokio::spawn(async move { chaos_engine.run().await });
  ...
  async fn test_load_spikes() {
    let (mut swarm, _) =
      create_chaos_swarm(ChaosConfig::default()).await.unwrap();
    ...
    println!(
      "Load spike handled in {:?} with {} agents",
      spike_duration, final_agent_count
    );
  }
  ...
  async fn test_resource_constraints() {
    ...
    assert!(
      resource_errors > 0,
      "Should have encountered resource limits"
    );
  ...
  async fn test_cascading_chaos() {
    ...
    let chaos_handle = tokio::spawn(async move {
      chaos_engine
        .run_scenario(ChaosScenario::CascadingFailure)
        .await
    });
    ...
    println!(
      "Cascading chaos: {} successful, {} failed",
      successful, failed
    );
  ...
  async fn test_adaptive_chaos_response() {
    ...
    for scenario in scenarios {
      ...
      let scenario_handle = tokio::spawn({
        ...
        async move { engine.run_scenario(scenario).await }
      });
  ...
  async fn test_chaos_patterns() {
    ...
    for pattern in patterns {
      ...
      let pattern_handle = tokio::spawn({
        ...
        async move { engine.apply_pattern(pattern, Duration::from_secs(20)).await }
      });
      ...
      let avg_latency =
        pattern_metrics.total_latency / pattern_metrics.successes as u32;

      println!(
        "Pattern impact - Success rate: {:.2}%, Avg latency: {:?}",
        success_rate * 100.0,
        avg_latency
      );
  ...
  async fn apply_pattern(&self, pattern: ChaosPattern, duration: Duration) {
    ...
    match pattern {
      ...
      ChaosPattern::Targeted { target_type, event } => {
        while start.elapsed() < duration {
          self
            .inject_targeted_event(target_type.clone(), event.clone())
            .await;
