...
use ruv_swarm_agents::{
  AnalyticalProcessor, ConvergentThinker, CreativeGenerator, DivergentThinker,
  PatternRecognizer, SystemIntegrator,
};
use ruv_swarm_core::{
  agent::{Agent, AgentType, CognitivePattern},
  metrics::{DiversityMetrics, PerformanceMetrics},
...
mod tests {
  ...
  async fn test_convergent_vs_divergent_strategies() {
    ...
    let convergent_result =
      convergent_swarm.orchestrate(problem.clone()).await.unwrap();
  ...
  async fn test_mixed_cognitive_teams() {
    ...
    let balanced_analytical = balanced_swarm
      .orchestrate(analytical_problem.clone())
      .await
      .unwrap();
    let balanced_creative = balanced_swarm
      .orchestrate(creative_problem.clone())
      .await
      .unwrap();

    // Specialized teams should excel in their domains
    let specialized_analytical = analytical_team
      .orchestrate(analytical_problem)
      .await
      .unwrap();
    let specialized_creative =
      creative_team.orchestrate(creative_problem).await.unwrap();

    // Verify specialized teams outperform in their domains
    assert!(
      get_task_quality(&specialized_analytical)
        > get_task_quality(&balanced_analytical) * 0.9,
      "Analytical team should excel at analytical tasks"
    );

    assert!(
      get_task_quality(&specialized_creative)
        > get_task_quality(&balanced_creative) * 0.9,
  ...
  async fn test_cognitive_pattern_adaptation() {
    ...
    let adaptation_score =
      calculate_adaptation_score(&initial_patterns, &adapted_patterns);
    assert!(
      adaptation_score > 0.3,
      "Agents should show pattern adaptation"
    );
  }
  ...
  async fn test_diversity_impact_on_performance() {
    ...
    assert!(
      optimal_diversity > 0.4 && optimal_diversity < 0.8,
  ...
  async fn test_cognitive_load_distribution() {
    ...
    assert!(
      min_load / avg_load > 0.5,
      "No agent should be underutilized"
    );
  }
  ...
  async fn test_cultural_evolution() {
    ...
    let generational_performance =
      swarm.get_generational_metrics().await.unwrap();
    ...
    assert!(
      ...
      "Performance should improve over generations"
    );
  }
}
...
fn generate_patterns_with_diversity(
  count: usize,
  diversity: f64,
) -> Vec<CognitivePattern> {
  let patterns = vec![
    CognitivePattern::Convergent,
    CognitivePattern::Divergent,
    CognitivePattern::Analytical,
