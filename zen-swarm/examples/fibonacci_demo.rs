...
use ruv_swarm_core::{
  ...
  task::{Task, TaskOutput, TaskPayload, TaskPriority, TaskResult, TaskStatus},
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  for n in &numbers_to_calculate {
    let task = Task::new(format!("fib_{}", n), "fibonacci_calculation")
...
mod tests {
  ...
  fn test_fibonacci_implementations() {
    ...
    for (n, expected) in test_cases {
      ...
      assert_eq!(
        ...
        expected,
        "Iterative implementation failed for n={}",
        n
      );
      ...
      assert_eq!(
        fibonacci_matrix(n),
        expected,
        "Matrix implementation failed for n={}",
        n
      );
      ...
      if n <= 30 {
        assert_eq!(
          ...
          expected,
          "Recursive implementation failed for n={}",
          n
        );
      }
    }
  ...
  async fn test_parallel_fibonacci_swarm() {
    ...
    let task = Task::new("test_fib", "fibonacci_calculation").with_payload(
      TaskPayload::Json(
        json!({
            "n": 20,
            "method": "iterative"
