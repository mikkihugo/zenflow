...
build_wasm() {
    ...
    # Build using the working zen-swarm-wasm crate
    print_status "Building with zen-swarm-wasm crate (unified crate has compilation errors)..."
    cd ../zen-swarm-wasm
    ...
    cd ../zen-swarm-wasm-unified
    if [ -d "../zen-swarm-wasm/pkg" ]; then
        cp -r ../zen-swarm-wasm/pkg ./
...
// Additional TypeScript utilities for zen-swarm WASM
