...
// Mock types for external dependencies that aren't in shared yet
interface HealthMonitor {
	...
	getCurrentHealth(): {
		...
		activeRecoveries?: { length: number };
...
interface ConnectionManager {
	getAllConnections(): Array<{ id: string; status: string }>;
...
interface RecoveryWorkflows {
	executeRecovery(scenario: string): Promise<boolean>;
	validateRecovery(scenario: string): Promise<boolean>;
	getRecoveryStatus(): {
		activeRecoveries?: { length: number };
		[key: string]: unknown;
	};
	triggerRecovery(scenario: string): Promise<boolean>;
}
...
export interface ChaosEngineeringOptions {
	enableChaos?: boolean;
...
export class ChaosEngineering extends TypedEventBase {
	...
	private mcpTools: unknown | null = null;
	...
	async performSafetyChecks(experiment: ChaosExperiment) {
		...
		if (this.healthMonitor) {
			...
			if (
				...
				throw new Error(
					`System health is degraded - experiment blocked: ${healthStatus.status}`,
				);
			}
		}
	...
	async cleanupExperiment(execution: ExperimentExecution) {
		...
		const experiment = this.experiments.get(execution.experimentName);
		if (experiment?.cooldown && experiment.cooldown > 0) {
			this.logger.debug(
	...
	async checkSystemRecovery(_execution: ExperimentExecution) {
		...
		if (this.healthMonitor) {
			// Perform async health check with monitoring
			const healthStatus = await new Promise((resolve) => {
				setTimeout(() => {
					resolve(this.healthMonitor?.getCurrentHealth());
				}, 100);
			});
			if ((healthStatus as any)?.status !== "healthy") {
				return false;
			}
		}
		...
		if (this.connectionManager) {
			...
			if (!connectionStatus?.connections) return false;
	...
	registerBuiltInInjectors() {
		// Import and register advanced injectors
		this.registerAdvancedInjectors();

		// Register basic built-in injectors
		...
		this.registerFailureInjector("cpu_stress", {
			...
			cleanup: async (injectionResult: InjectionResult) => {
				...
				if (injectionResult.workers) {
					for (const worker of injectionResult.workers) {
						try {
							worker.terminate();
						} catch (error) {
							// Worker may already be terminated
							this.logger.debug("Worker termination:", error);
						}
					}
				}
			},
	...
	/**
	 * Register advanced failure injectors from separate module
	 */
	private async registerAdvancedInjectors() {
		try {
			// Dynamic import to avoid circular dependencies
			const { getAllAdvancedInjectors } = await import("./injectors.js");
			const advancedInjectors = getAllAdvancedInjectors();

			for (const [name, injector] of Object.entries(advancedInjectors)) {
				this.registerFailureInjector(name, injector);
			}

			this.logger.info("Advanced failure injectors registered", {
				count: Object.keys(advancedInjectors).length,
			});
		} catch (error) {
			this.logger.warn("Failed to load advanced injectors", { error });
		}
	}
	...
	registerBuiltInExperiments() {
		// Register basic built-in experiments first
		...
		// Register extended experiments from separate module
		this.registerExtendedExperiments();
	}

	/**
	 * Register extended experiments from separate module
	 */
	private async registerExtendedExperiments() {
		try {
			// Dynamic import to avoid circular dependencies
			const { getAllExtendedExperiments } = await import("./experiments.js");
			const extendedExperiments = getAllExtendedExperiments();

			for (const experimentDef of extendedExperiments) {
				if (experimentDef.name) {
					this.registerExperiment(experimentDef.name, experimentDef);
				}
			}

			this.logger.info("Extended experiments registered", {
				count: extendedExperiments.length,
				totalExperiments: this.experiments.size,
			});
		} catch (error) {
			this.logger.warn("Failed to load extended experiments", { error });
		}
	}
	...
	async checkResourceUsage() {
		// Perform async resource monitoring with system checks
		const [memUsage, totalMem] = await Promise.all([
			new Promise((resolve) =>
				setTimeout(() => resolve(process.memoryUsage()), 10),
			),
			new Promise((resolve) =>
				setTimeout(() => resolve(require("node:os").totalmem()), 10),
			),
		]);
	...
	async collectImpactMetrics() {
		// Perform async metrics collection with comprehensive monitoring
		const [memoryMetrics, cpuMetrics, connectionStats] = await Promise.all([
			new Promise((resolve) =>
				setTimeout(() => resolve(process.memoryUsage()), 10),
			),
			new Promise((resolve) =>
				setTimeout(() => resolve(process.cpuUsage()), 10),
			),
			new Promise((resolve) =>
				setTimeout(
					() =>
		...
		return {
			...
			health: this.healthMonitor
				? (this.healthMonitor as any).currentHealth
				: null,
		};
	}
	...
	setMCPTools(mcpTools: unknown) {
		...
		this.logger.info("MCP Tools integration configured", {
			mcpToolsAvailable: mcpTools !== null && mcpTools !== undefined,
		});
	}

	/**
	 * Get configured MCP tools for chaos experiments
	 */
	getMCPTools() {
	 ...
	 */
	/**
	 * Reload configuration from shared config system
	 */
...
// Professional chaos engineering system object with proper naming (matches brainSystem pattern)
export const chaosEngineeringSystem = {
	getAccess: getChaosEngineeringSystemAccess,
	getInstance: getChaosEngineeringInstance,
	getFailureInjection,
	getResilienceTesting,
