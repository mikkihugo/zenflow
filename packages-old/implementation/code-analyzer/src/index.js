...
export function createLiveCodeAnalyzer(repositoryPath) {
    return new CodeAnalyzer(repositoryPath);
}
export function createAICodeAnalyzer(repositoryPath, aiConfig) {
    const analyzer = new CodeAnalyzer(repositoryPath);
    // Configure AI features when available
    if (aiConfig) {
        // Future AI configuration implementation
        console.debug('AI configuration provided for code analyzer', { ': config, aiConfig,
        });
    }
    return analyzer;
}
