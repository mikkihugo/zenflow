...
export class ReportGenerator {
    logger = getLogger('ReportGenerator');
    ';
    /**
     * Generate report in specified format
     */
    async generateReport(result, format, outputPath) {
...
<body>
    ...
    < /ul>
    < h3 > Areas);
for (Improvement < (/h3>); $; { result, : .summary.weaknesses.map((weakness) => `<li>${weakness}</li>`).join(')}'
        < /ul>
        < h3 > Risk, Assessment < /h3>
        < div, class {
    } = "risk-assessment" >
        class {
        }, "risk-item risk-${result.summary.riskAssessment.technicalDebtRisk}" >
        Technical, Debt, Risk, /strong> ${result.summary.riskAssessment.technicalDebtRisk.toUpperCase()}
        < /div>
        < div, class {
    } = "risk-item risk-${result.summary.riskAssessment.maintainabilityRisk}" >
        Maintainability, Risk, /_strong> ${result.summary.riskAssessment.maintainabilityRisk.toUpperCase()}
        < /div>
        < div, class {
    } = "risk-item risk-${result.summary.riskAssessment.scalabilityRisk}" >
        Scalability, Risk, /_strong> ${result.summary.riskAssessment.scalabilityRisk.toUpperCase()}
        < /div>
        < div, class {
    } = "risk-item risk-${result.summary.riskAssessment.teamVelocityRisk}" >
        Team, Velocity, Risk, /_strong> ${result.summary.riskAssessment.teamVelocityRisk.toUpperCase()}
        < /div>
        < /div>
        < /div>
        < div, class {
    } = "section" >
        Recommendations(_$, { result, : .recommendations.length }) < /_h2>, $, { result, : .recommendations
            .slice(0, 20)
            .map((_rec) => ``
            < div, class {
        } = "recommendation priority-${rec.priority}" >
            $, { rec, : .title } < span, style = "color: #666; font-size: 14px;" > [$, { rec, : .priority.toUpperCase() }] < /span></h3 >
            $, { rec, : .description } < /p>
            < p > Effort, /strong> ${rec.effort.hours} hours (${rec.effort.difficulty})</p >
            Benefits, /strong></p >
            $rec.benefits.map((benefit) => `<li>${benefit}</li>`).join(')}'
                ...
                rec, : .risks.length > 0
                    ...
                    Domains(_$, { result, : .domains.length }) < /_h2>, $, { result, : .domains
                        .map((_domain) => ``
                        < div, class {
                    } = "domain" >
                        $, { domain, : .name } < span, style = "color: #666; font-size: 14px;" > [$, { domain, : .type }] < /span></h3 >
                        $domain.description < /p>
                        < div, class {
                    } = "stats-grid" >
                        Files, /_strong> ${domain.files.length}
                        < /div>
                        < div >
                        Complexity, /strong> $(domain.complexity * 100).toFixed(1)%
                        < /div>
                        < div >
                        Cohesion, /strong> $(domain.cohesion * 100).toFixed(1)%
                        < /div>
                        < div >
                        Coupling, /strong> $(domain.coupling * 100).toFixed(1)%
                        < /div>
                        < /div>, $, domain.splitRecommendation
                        ? ``
                            < p > Split : , Recommendation, /strong> $domain.splitRecommendation.shouldSplit ? 'Yes' : 'No''(Confidence, $(domain.splitRecommendation.confidence * 100).toFixed(1) % ) < /p> ``, ''
                        < /div> ``)
                        .join(')}'
                        < /div>
                        < div, class {
                    } = "section" >
                        Language, Distribution < /_h2>
                        < div, class {
                    } = "stats-grid" >
                        $, { Object, : .entries(result.repository.languages)
                            ...
                            class {
                            }, "metric-value" > $, { lines, : .toLocaleString() } < /div>
                            < div, class {
                        } = "metric-label" > $, { lang, : .toUpperCase() } < /div>
                            < /div> ``)
                            .join(')}'
                            < /div>
                            < /div>, $, {
                            ...
                            class {
                            }, "metric-value" > $, { result, : .repository.gitMetrics.totalCommits } < /div>
                            < div, class {
                        } = "metric-label" > Total, Commits < /_div>
                            < /div>
                            < div, class {
                        } = "stat-card" >
                            class {
                            }, "metric-value" > $, { result, : .repository.gitMetrics.contributors } < /div>
                            < div, class {
                        } = "metric-label" > Contributors < /_div>
                            < /div>
                            < div, class {
                        } = "stat-card" >
                            class {
                            }, "metric-value" > $, { result, : .repository.gitMetrics.averageCommitsPerDay.toFixed(1) } < /div>
                            < div, class {
                        } = "metric-label" > Commits / Day < /_div>
                            < /div>
                            < div, class {
                        } = "stat-card" >
                            class {
                            }, "metric-value" > $, { result, : .repository.gitMetrics.hotFiles.length } < /div>
                            < div, class {
                        } = "metric-label" > Hot, Files < /_div>
                            < /div>
                            < /div>
                            < /div> ``, '')
                    }
                        ...
                        Technical, Details < /_h2>
                        < _h3 > Complexity, Analysis < /_h3>
                        < _ul >
                        Cyclomatic, Complexity, /_strong> ${result.repository.complexity.cyclomatic}</li >
                        Halstead, Volume, /strong> ${result.repository.complexity.halstead.volume.toFixed(2)}</li >
                        Technical, Debt, /strong> ${result.repository.complexity.technicalDebt.toFixed(1)} hours</li >
                        Code, Smells, /strong> ${result.repository.complexity.codeSmells.length}</li >
                        Complexity, Hotspots, /strong> ${result.repository.complexity.hotspots.length}</li >
                        /ul>
                        < h3 > Dependency, Analysis < /h3>
                        < ul >
                        Total, Dependencies, /strong> ${result.repository.dependencies.totalDependencies}</li >
                        Direct, Dependencies, /strong> ${result.repository.dependencies.directDependencies}</li >
                        Circular, Dependencies, /strong> ${result.repository.dependencies.circularDependencies.length}</li >
                        Instability, /strong> $(result.repository.dependencies.coupling.instability * 100).toFixed(1)%</li >
                        Stability, Index, /strong> $(result.repository.dependencies.stability.stabilityIndex * 100).toFixed(1)%</li >
                        /ul>
                        < /div>
                        < /div>
                        < /body>
                        < /html>`;`, await fs.writeFile(outputPath, html, 'utf-8')), ': ,
                    return: outputPath }
                /**
                ...
                private, async, generateMarkdownReport(result, AnalysisResult, outputPath, string), Promise < string > {
                    ...
                    ** Path,  ** $, { result, : .repository.path }, #, #, Overall, Health, Score, $, {}(result.summary.overallScore * 100).toFixed(1)) } %
                #, #, Summary | Metric | Value || --, --, --, -- | --, --, -- -  || Total, Files | $, { result, : .repository.totalFiles } || Total, Lines | $, { result, : .repository.totalLines.toLocaleString() } || Cyclomatic, Complexity | $, { result, : .repository.complexity.cyclomatic } || Maintainability, Index | $, { result, : .repository.complexity.maintainabilityIndex.toFixed(1) } || Total, Dependencies | $, { result, : .repository.dependencies.totalDependencies } || Domains | $, { result, : .domains.length } | #, #, #, Strengths, $, { result, : .summary.strengths.map((s) => `- ${s}`).join('\n') }, ', #, #, #, Areas)), for: Improvement,
        ...
        -  ** Team, Velocity, Risk,  ** $, { result, : .summary.riskAssessment.teamVelocityRisk.toUpperCase() }, #, #, Top, Recommendations, $, { result, : .recommendations
            .slice(0, 10)
            .map((_rec, _index) => ``, #, #, #, $, { index } + 1) }.$, { rec, : .title }[$], { rec, : .priority.toUpperCase() }, $, { rec, : .description }
...
{
    result.domains
        .map((_domain) => ``, #, #, #, $, { domain, : .name }[$], { domain, : .type }, $, { domain, : .description }
 ...
 ** $;
{
    (domain.cohesion * 100).toFixed(1);
}
 %
    - ** Coupling;
 ** $;
{
    (domain.coupling * 100).toFixed(1);
}
 %
    $;
{
...
{
    Object.entries(result.repository.languages)
        .map(([lang, lines]) => `- **${lang.toUpperCase()}:** ${lines.toLocaleString()} lines` `
  )
  .join('\n')}'
...
Complexity:  ** $;
{
    result.repository.complexity.cyclomatic;
}
- ** Halstead;
Volume:  ** $;
{
    result.repository.complexity.halstead.volume.toFixed(2);
}
- ** Technical;
Debt:  ** $;
{
    result.repository.complexity.technicalDebt.toFixed(1);
}
hours
    -  ** Code;
Smells:  ** $;
{
    result.repository.complexity.codeSmells.length;
}
- ** Complexity;
Hotspots:  ** $;
{
    result.repository.complexity.hotspots.length;
}
#;
#;
#;
...
Dependencies:  ** $;
{
    result.repository.dependencies.totalDependencies;
}
- ** Direct;
Dependencies:  ** $;
{
    result.repository.dependencies.directDependencies;
}
- ** Circular;
Dependencies:  ** $;
{
    result.repository.dependencies.circularDependencies.length;
}
- ** Instability;
 ** $;
{
    (result.repository.dependencies.coupling.instability * 100).toFixed(1);
}
 %
    - ** Stability;
Index:  ** $;
{
    (result.repository.dependencies.stability.stabilityIndex * 100).toFixed(1);
}
 %
    -- -
        * Generated;
...
return outputPath;
async;
generatePdfReport(result, AnalysisResult, outputPath, string);
Promise < string > {
...
<graphml xmlns="http://graphml.graphdrawing.org/xmlns"
  ...
  <graph id="DependencyGraph" edgedefault="directed">
    ${result.repository.dependencies.dependencyGraph.nodes
        .map((node) => ``
        < node, id = "${node.id}" >
        key, "name" > $, { node, : .file } < /data>
        < data, key = "type" > $, { node, : .type } < /data>
        < data, key = "complexity" > $, { node, : .complexity } < /data>
        < /node>``)
        .join(')}', $, { result, : .repository.dependencies.dependencyGraph.edges
            .map((edge, _index) => ``
            < edge, id = "e${index}", source = "${edge.from}", target = "${edge.to}" >
            key, "weight" > $, { edge, : .weight } < /data>
            < /edge>``)
            .join(')}'
            < /graph>
            < /graphml>`;`, await fs.writeFile(outputPath, graphml, 'utf-8')), ': ,
        return: outputPath }
    /**
...
${result.repository.dependencies.dependencyGraph.edges
  ...
  private async buildReportMetadata(
    ...
    return {
      ...
      analysisScope: `, $, { result, : .repository.totalFiles }, files, $, { result, : .repository.totalLines }, lines `,`, totalMetrics, {
