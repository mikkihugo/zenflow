...
export class DomainAnalyzer {
    logger = getLogger('DomainAnalyzer');
    ';
    /**
     * Analyze repository domains
     */
    async analyzeRepository(rootPath, files, options) {
...
async;
analyzeDomain(id, string, path, string, files, string[], rootPath, string, options ?  : AnalysisOptions);
Promise < Domain | null > {
    ...
    const: splitRecommendation = this.evaluateSplitRecommendation(complexity, cohesion, coupling, size, files),
    return: {
        id,
        name,
        description,
...
groupFilesByDirectory(files, string[]);
Map < string, string[] > {
...
generateDomainDescription(path, string, files, string[]);
string;
{
    const segments = path.split('/').filter(Boolean);
    ';
    const fileTypes = this.analyzeFileTypes(files);
    const primaryType = Object.keys(fileTypes).reduce((a, b) => fileTypes[a] > fileTypes[b] ? a : b);
    const contexts = [
        'authentication',
        'authorization',
        'user',
    ...
    const detectedContext = segments.find((segment) => contexts.some((context) => segment.toLowerCase().includes(context)));
    if (detectedContext) {
    ...
    return `;
        Domain;
        containing;
        $files.length$primaryTypefiles `;`;
    }
    classifyDomainType(path, string, files, string[]);
    DomainType;
    {
        const pathLower = path.toLowerCase();
        const fileTypes = this.analyzeFileTypes(files);
        // Infrastructure patterns
        if (pathLower.includes('config') || pathLower.includes('infrastructure') || pathLower.includes('database') || pathLower.includes('migration'))
            ';
        return 'infrastructure;;
        // API patterns
        if (pathLower.includes('api') || pathLower.includes('route') || pathLower.includes('endpoint') || pathLower.includes('controller'))
            ';
        return 'api;;
        // UI patterns
        if (pathLower.includes('component') || pathLower.includes('view') || pathLower.includes('ui') || pathLower.includes('frontend'))
            ';
        return 'ui;;
        // Utility patterns
        if (pathLower.includes('util') || pathLower.includes('helper') || pathLower.includes('tool') || pathLower.includes('lib'))
            ';
        return 'utility;;
        // Test patterns
        if (pathLower.includes('test') || pathLower.includes('spec') || Object.keys(fileTypes).some((type) => type.includes('test')))
            ';
        return 'test;;
        // Data patterns
        if (pathLower.includes('model') || pathLower.includes('entity') || pathLower.includes('data') || pathLower.includes('repository'))
            ';
        return 'data;;
        // Core business logic
        if (pathLower.includes('core') || pathLower.includes('business') || pathLower.includes('domain') || pathLower.includes('service'))
            ';
        return 'core;;
        // Feature-specific
    ...
    async;
    calculateDomainSize(files, string[]);
    Promise < DomainSize > {
    ...
    calculateDomainComplexity(files, string[], size, DomainSize);
    number;
    {
        // Complexity factors
        ...
        return parseFloat(((fileCountFactor + lineFactor + functionFactor + classFactor) /
            4).toFixed(3));
    }
    calculateDomainCohesion(files, string[]);
    number;
    {
    ...
    async;
    calculateDomainCoupling(files, string[], rootPath, string);
    Promise < number > {
    ...
    evaluateSplitRecommendation(complexity, number, cohesion, number, coupling, number, size, DomainSize, files, string[]);
    ...
    generateSplitSuggestions(files, string[], size, DomainSize);
    SuggestedSplit[];
    {
        const suggestions = [];
