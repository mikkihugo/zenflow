...
export class DependencyAnalyzer {
    logger = getLogger('DependencyAnalyzer');
    ';
    /**
     * Analyze dependencies for the entire repository
     */
    async analyzeRepository(rootPath, options) {
...
async;
extractDependencies(content, string, filePath, string);
...
try { }
  ...
  private classifyDependency(
    ...
    if (content.includes(`;
    require(`) && content.includes(depName))`);
    return 'require;;
    return 'import;;
}
findDependencyLine(depName, string, content, string);
number;
{
...
extractDependenciesManual(content, string);
string[];
{
...
async;
detectCircularDependencies(madgeResult, any, dependencyMap, (Map));
...
return cycles;
calculateCycleImpact(cycle, string[], dependencyMap, (Map));
number;
{
...
generateCycleBreakingSuggestions(cycle, string[]);
string[];
{
    const suggestions = [
...
buildDependencyGraph(dependencyMap, (Map), files, string[]);
DependencyGraph;
{
    const nodes = [];
    const edges = [];
...
calculateCouplingMetrics(graph, DependencyGraph);
CouplingMetrics;
{
...
calculateCohesionMetrics(graph, DependencyGraph);
CohesionMetrics;
{
...
calculateStabilityMetrics(graph, DependencyGraph, circularDeps, CircularDependency[]);
StabilityMetrics;
{
...
return 'javascript;;
return 'javascript;;
classifyFileType(filePath, string);
...
calculateNodeStability(filePath, string, dependencyMap, (Map));
number;
{
...
detectClusters(nodes, DependencyNode[], edges, DependencyEdge[]);
DependencyCluster[];
{
