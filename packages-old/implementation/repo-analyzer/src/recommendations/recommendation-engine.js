...
export class RecommendationEngine {
    logger = getLogger('RecommendationEngine');
    ';
    /**
     * Generate comprehensive recommendations
     */
    async generateRecommendations(repository, options) {
        ...
        const _analysisContext = await this.buildAnalysisContext(repository, options);
     ...
     */
    generateComplexityRecommendations(repository) {
        const recommendations = [];
    ...
    if (complexity.maintainabilityIndex < 30) {
      recommendations.push({
        ...
        description: `, Maintainability, index, is, $, { complexity, : .maintainabilityIndex.toFixed(1) }(threshold, 30).Code, requires, significant, effort, to, maintain. `,`, rationale, 'Low maintainability index indicates code that is difficult to understand, modify, and extend.', effort, this.estimateEffort(24, 'high', 'Improving maintainability requires systematic refactoring', '), benefits, [
    ...
    if (complexity.codeSmells.length > 10) {
      ...
      if (criticalSmells.length > 0) {
        recommendations.push({
          ...
          description: `, Found, $, { criticalSmells, : .length }, critical, code, smells, that, need, immediate, attention. `,`, rationale, 'Critical code smells indicate fundamental design or implementation issues that impact code quality.', effort, this.estimateEffort(criticalSmells.length * 2, 'medium', 'Each code smell requires analysis and targeted refactoring', '), benefits, [
  ...
  private generateDependencyRecommendations(
    ...
    if (dependencies.circularDependencies.length > 0) {
      ...
      recommendations.push({
        ...
        description: `), Found, $, { dependencies, : .circularDependencies.length }, circular, dependencies($, { criticalCycles, : .length }, critical).These, create, tight, coupling, and, make, code, harder, to, test, and, maintain. `,`, rationale, 'Circular dependencies prevent proper modularization and can cause build issues, infinite loops, and testing difficulties.', effort, this.estimateEffort(dependencies.circularDependencies.length * 4, 'high', 'Breaking circular dependencies requires architectural changes', '), benefits, [
...
if (dependencies.coupling.instability > 0.8) {
    ...
    if (dependencies.cohesion.lcom > 0.7) {
      recommendations.push({
        ...
        description: `, Module, cohesion, is, low(LCOM, $, {}(dependencies.cohesion.lcom * 100).toFixed(1)));
}
 % ;
Modules;
contain;
...
rationale: 'Low cohesion indicates that modules are doing too many unrelated things, making them harder to understand and maintain.',
    effort;
this.estimateEffort(16, 'medium', 'Improving cohesion requires careful module reorganization', '),
    benefits;
[
...
;
return recommendations;
generateDomainRecommendations(repository, RepositoryMetrics);
AnalysisRecommendation[];
{
    const recommendations = [];
    for (const domain of repository.domains) {
        if (domain.splitRecommendation?.shouldSplit) {
          ...
          description: `, Domain, "${domain.name}", should, be, split(confidence, $, {}(split.confidence * 100).toFixed(1)));
        }
         % ;
        $;
        {
  ...
  private generateArchitectureRecommendations(
    ...
    if (repository.totalFiles > 500) {
      recommendations.push({
        ...
        description: `, Repository, has, $, { repository, : .totalFiles }, files.Large, repositories, can, impact, development, velocity, and, team, coordination. `,`, rationale, 'Large repositories become harder to navigate, build times increase, and team coordination becomes more complex.', effort, this.estimateEffort(40, 'very-high', 'Repository splitting is a major architectural change', '), benefits, [
...
;
return recommendations;
generatePerformanceRecommendations(repository, RepositoryMetrics);
AnalysisRecommendation[];
{
    const recommendations = [];
    // Many large files
  ...
  private generateSecurityRecommendations(
    ...
    if (repository.dependencies.totalDependencies > 100) {
      recommendations.push({
        ...
        description: `, Repository, has, $, { repository, : .dependencies.totalDependencies }, external, dependencies.This, increases, security, attack, surface. `,`, rationale, 'Many external dependencies increase security risks, maintenance burden, and potential for supply chain attacks.', effort, this.estimateEffort(8, 'low', 'Dependency audit is primarily analysis work', '), benefits, [
...
return recommendations;
generateWorkflowRecommendations(repository, RepositoryMetrics);
AnalysisRecommendation[];
{
    const recommendations = [];
...
estimateEffort(hours, number, difficulty, EffortEstimate['difficulty'], description, string);
EffortEstimate;
return {
...
getPriorityWeight(priority, AnalysisRecommendation['priority'], ');
number;
{
    const weights = {
        urgent: 4,
...
async;
buildAnalysisContext(repository, RepositoryMetrics, options ?  : AnalysisOptions);
Promise <
    focusAreas;
string[];
