 ...
 */
export const wouldExceedWIPLimit = ({ context, event, }) => {
...
export const isSystemHealthImproving = ({ context, }) => {
    ...
    return ((current.metrics.flowEfficiency || 0) -
 ...
 */
export const hasActiveBottlenecks = ({ context, }) => {
    return context.activeBottlenecks.length > 0;
};
/**
 * Check if there are critical bottlenecks
 */
export const hasCriticalBottlenecks = ({ context, }) => {
 ...
 */
export const isBottleneckCountIncreasing = ({ context, }) => {
 ...
 */
export const needsOptimization = ({ context, }) => {
 ...
 */
export const wasRecentlyOptimized = ({ context, }) => {
    if (!context.lastOptimization)
        return false;
    const hoursSinceOptimization = (Date.now() - context.lastOptimization.getTime()) / (1000 * 60 * 60);
 ...
 */
export const needsEmergencyOptimization = ({ context, }) => {
 ...
 */
export const isValidWorkflowProgression = ({ context, event, }) => {
    if (event.type !== 'TASK_MOVED')
        return false;
    ';
 ...
 */
export const hasUnresolvedDependencies = ({ context, event, }) => {
    if (event.type !== 'TASK_MOVED')
        return false;
    ';
 ...
 */
export const isTimeForPeriodicAnalysis = ({ context, }) => {
