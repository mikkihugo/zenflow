...
export const moveTask = assign({
    ...
    tasksByState: ({ context, event }) => {
        ...
        return {
            ...context.tasksByState,
            [event.fromState]: context.tasksByState[event.fromState].filter((id) => id !== event.taskId),
...
export const recordWIPViolation = assign({
    wipViolations: ({ context, event }) => {
        if (event.type !== 'WIP_LIMIT_EXCEEDED')
            return context.wipViolations;
        ';
        const violation = {
            state: event.state,
            count: event.count,
...
export const updateWIPLimits = assign({
    wipLimits: ({ context, event }) => {
        if (event.type !== 'WIP_LIMITS_UPDATED')
            return context.wipLimits;
        ';
        return {
            ...context.wipLimits,
            ...event.wipLimits,
...
export const addBottleneck = assign({
    activeBottlenecks: ({ context, event }) => {
        if (event.type !== 'BOTTLENECK_DETECTED')
            return context.activeBottlenecks;
        ';
        // Check if bottleneck already exists
        const existingIndex = context.activeBottlenecks.findIndex((b) => b.id === event.bottleneck.id);
        if (existingIndex >= 0) {
...
export const resolveBottleneck = assign({
    activeBottlenecks: ({ context, event }) => {
        if (event.type !== 'BOTTLENECK_RESOLVED')
            return context.activeBottlenecks;
        ';
        return context.activeBottlenecks.filter((b) => b.id !== event.bottleneckId);
    },
    bottleneckHistory: ({ context, event }) => {
        if (event.type !== 'BOTTLENECK_RESOLVED')
            return context.bottleneckHistory;
        ';
        const resolvedBottleneck = context.activeBottlenecks.find((b) => b.id === event.bottleneckId);
...
export const updateFlowMetrics = assign({
    currentMetrics: ({ context, event }) => {
        if (event.type !== 'FLOW_ANALYSIS_COMPLETE')
            return context.currentMetrics;
        ';
...
export const recordOptimization = assign({
    lastOptimization: ({ event }) => {
        if (event.type !== 'OPTIMIZATION_TRIGGERED')
            return null;
        ';
        return new Date();
    },
...
export const updateSystemHealth = assign({
    systemHealth: ({ context, event }) => {
        if (event.type !== 'SYSTEM_HEALTH_UPDATED')
            return context.systemHealth;
        ';
