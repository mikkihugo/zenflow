...
export class UEL {
    ...
    async createEventManager(config) {
        if (!this.initialized) {
    ...
    async createSimpleManager(config) {
        return {
            name: config.name,
            type: config.type,
            config,
    ...
    async destroyEventManager(name) {
    ...
    async enableMigrationSupport() {
        try {
            ...
            this.migrationHelper = new EventEmitterMigrationHelper(firstManager, this.logger);
    ...
    async createCompatibleEventEmitter(name) {
        ...
        for (const type of Object.values(EventManagerTypes)) {
            if (!this.factories.has(type)) {
                this.registerFactory(type, {
                    ...
                    get: (name) => this.managers.get(name),
                    list: () => Array.from(this.managers.values()),
                    has: (name) => this.managers.has(name),
                    remove: async (name) => this.destroyEventManager(name),
                });
            }
        }
    }
    async enableValidationFramework() {
        this.logger.debug('Enabling UEL validation framework');
        this.status.validationEnabled = true;
    }
    async enableCompatibilityLayer() {
        try {
            const { CompatibilityFactory } = await import('../compatibility');
            this.compatibilityFactory = CompatibilityFactory.getInstance();
            await this.compatibilityFactory.initialize(this.managers.values().next().value, // Use first manager as default
            this.logger);
 ...
 */
export function getUEL() {
    return UEL.getInstance();
}
/**
 * Initialize UEL with configuration.
 */
export async function initializeUEL(config) {
    const uel = getUEL();
