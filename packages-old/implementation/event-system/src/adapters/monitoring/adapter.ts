...
import type { EventManager } from '../../core/interfaces';
 ...
 */
export async function createMonitoringEventAdapter(
  ...
  try {
    ...
    const optimizedConfig = MonitoringAdapterHelpers.createDefaultConfig(
      config.name,
      config
    );

    // Create monitoring event manager configured for monitoring events
    const adapter = new MonitoringEventManager(
      optimizedConfig,
      logger as any
    ) as EventManager;

    logger.info(
      `Monitoring event adapter created successfully: ${config.name}`
    );
    return adapter;
  } catch (error) {
    logger.error(
      `Failed to create monitoring event adapter: ${config.name}`,
      error
    );
    throw error;
  }
