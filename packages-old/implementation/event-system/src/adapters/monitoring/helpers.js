 ...
 */
export function validateMonitoringAdapterConfig(config) {
    return !!(config && config.name && config.type === 'monitoring');
}
/**
 * Helper function to create default monitoring adapter config.
 */
export function createDefaultMonitoringAdapterConfig(name, overrides) {
    return {
        name,
        type: 'monitoring',
        enabled: true,
 ...
 */
export function logMonitoringAdapterEvent(logger, event, data) {
    logger.debug(`Monitoring adapter event: ${event}`, data);
}
/**
 * Helper function to generate monitoring adapter event IDs.
 */
export function generateMonitoringAdapterEventId() {
