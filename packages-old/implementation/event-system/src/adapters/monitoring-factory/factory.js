 ...
 */
export class MonitoringEventFactory extends EventEmitter {
    factoryConfig;
    systemConfig;
    logger;
    instances = new Map();
    ...
    async create(config) {
        ...
        try {
            ...
            const optimizedConfig = MonitoringFactoryHelpers.createDefaultConfig(config?.name, config);
            // Create manager instance
            const manager = await createMonitoringEventAdapter(optimizedConfig);
            // Store in registry
            ...
            this.logger.info(`Monitoring event manager created successfully: ${config?.name}`);
            return manager;
        }
        catch (error) {
            this.totalErrors++;
            ...
            this.logger.error(`Failed to create monitoring event manager: ${config?.name}`, error);
            throw error;
        }
    }
    /**
    ...
    async remove(name) {
        ...
        try {
            ...
            this.logger.error(`Failed to remove monitoring event manager ${name}:`, error);
            throw error;
        }
    }
    /**
    ...
    async getFactoryMetrics() {
        ...
        return MonitoringFactoryHelpers.calculateMetrics(this.totalCreated, this.totalErrors, this.instances.size, runningInstances, this.startTime);
    }
    /**
    ...
    async healthCheck() {
        ...
        const monitoringSuccessRate = MonitoringFactoryHelpers.calculateMonitoringSuccessRate(this.monitoringMetrics.activeMonitors, this.monitoringMetrics.failedMonitors);
        return {
            status,
    ...
    startMonitoringSystem() {
        ...
        setInterval(() => {
            const successRate = MonitoringFactoryHelpers.calculateMonitoringSuccessRate(this.monitoringMetrics.activeMonitors, this.monitoringMetrics.failedMonitors);
            if (successRate < 0.8) {
                this.logger.warn('Monitoring success rate is low, considering optimization');
            }
    ...
    determineOverallHealth(metrics, instanceHealth) {
        ...
        const successRate = MonitoringFactoryHelpers.calculateMonitoringSuccessRate(metrics.monitoringMetrics.activeMonitors, metrics.monitoringMetrics.failedMonitors);
        if (successRate < 0.5)
            return 'unhealthy';
        if (successRate < 0.8)
            return 'degraded';
