...
export function createMonitoringEventFactory(
  config?: MonitoringEventFactoryConfig
...
export async function createMonitoringManager(
...
export async function createBasicMonitoringManager(
  name: string,
...
export async function createAdvancedMonitoringManager(
  name: string,
  overrides?: Partial<EventManagerConfig>
): Promise<import('../../core/interfaces').EventManager> {
  const config = MonitoringFactoryHelpers.createDefaultConfig(name, {
