...
;
  ...
  const _result = await this.executeTaskWithAgent(agent, taskDefinition, options);
;
...
;
    if (availableAgents.length === 0) {
      ...
      if (this.getTotalActiveAgents() < this.config.maxConcurrentAgents) {
        const __agentId = await this.spawnAgent({type = availableAgents.map(agent => ({
