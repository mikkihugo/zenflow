...
;
      ...
      const _tasks = await this.decomposeObjective(context.objective);
...
;
      ...
      const _agents = await this.spawnRequiredAgents(context);
...
;
    // Collect results
    const _results = await this.collectSwarmResults(context);
