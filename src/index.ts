  ...
  try {
    ...
    try {
      ...
      if ((global as any).stdioMcpServer) {
        ...
        shutdownResults.push({ component: 'StdioMCPServer', status: 'success' });
    ...
    try {
      const { MemorySystemFactory } = await import('./memory/index.ts');
      if ((global as any).memorySystem && typeof (global as any).memorySystem.shutdown === 'function') {
    ...
    if (errorCount > 0) {
      console.warn('⚠️ Some components failed to shutdown gracefully:', 
...
export async function healthCheck() {
  ...
  try {
    ...
    if (memorySystem && typeof memorySystem.getHealthReport === 'function') {
      ...
      healthStatus.components.memory = {
        status: healthReport.overall === 'healthy' ? 'healthy' : 
               healthReport.overall === 'warning' ? 'degraded' : 'unhealthy',
  ...
  try {
    ...
    if (httpMcpServer && typeof httpMcpServer.isRunning === 'function') {
      interfaceDetails.httpMcp = httpMcpServer.isRunning() ? 'running' : 'stopped';
    ...
    if (stdioMcpServer && typeof stdioMcpServer.isRunning === 'function') {
      interfaceDetails.stdioMcp = stdioMcpServer.isRunning() ? 'running' : 'stopped';
    } else {
