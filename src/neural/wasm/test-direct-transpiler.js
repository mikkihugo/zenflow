...
async function testDirectTranspiler() {
  ...
  try {
    ...
    console.log(
      '✅ Loader initialized with status:',
      loader.getModuleStatus().status
    );
...
testDirectTranspiler().then((success) => {
  console.log(
    '\n' +
      (success
        ? '🎉 SUCCESS: CUDA transpiler working through zen-swarm!'
        : '❌ FAILED: CUDA transpiler not functional')
  );
