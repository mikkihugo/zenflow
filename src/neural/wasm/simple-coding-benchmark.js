...
async function benchmarkRuvSwarm() {
  ...
  try {
    ...
    const { mcp__ruv_swarm__swarm_init } = await import('../../../mcp-test-functions.js').catch(() => ({}));
    ...
    // Spawn coding agents
    const { mcp__ruv_swarm__agent_spawn } = await import('../../../mcp-test-functions.js');
    ...
    const { mcp__ruv_swarm__task_orchestrate } = await import('../../../mcp-test-functions.js');
...
async function benchmarkZenSwarm() {
  ...
  try {
    ...
    console.log('🧠 Neural networks enabled:', swarmResult.features.neural_networks);
    console.log('⚡ WASM acceleration:', swarmResult.features.wasm_acceleration);
...
async function runBenchmarkComparison() {
  ...
  const statusWinner = (ruvResult.success && zenResult.success) ? 'Tie' :
                      ruvResult.success ? 'ruv-swarm' :
                      zenResult.success ? 'zen-swarm' : 'Both failed';
  
  console.log(`│ Status          │ ${ruvStatus.padEnd(11)} │ ${zenStatus.padEnd(11)} │ ${statusWinner.padEnd(11)} │`);
  
  if (ruvResult.success && zenResult.success) {
    // Execution time
    const timeWinner = ruvResult.time < zenResult.time ? 'ruv-swarm' : 'zen-swarm';
    console.log(`│ Execution Time  │ ${ruvResult.time.toFixed(1).padStart(7)}ms  │ ${zenResult.time.toFixed(1).padStart(7)}ms  │ ${timeWinner.padEnd(11)} │`);
    ...
    console.log(`│ Speed Factor    │ 1.0x        │ ${speedup.toFixed(1)}x        │ ${faster.padEnd(11)} │`);
    
    // Features comparison
    const ruvFeatures = ruvResult.memory_efficient ? '💾 Memory' : '⚡ Basic';
    const zenFeatures = zenResult.wasm_acceleration ? '🚀 WASM' : '⚡ Basic';
    const featureWinner = zenResult.wasm_acceleration ? 'zen-swarm' : ruvResult.memory_efficient ? 'ruv-swarm' : 'Tie';
    
    console.log(`│ Features        │ ${ruvFeatures.padEnd(11)} │ ${zenFeatures.padEnd(11)} │ ${featureWinner.padEnd(11)} │`);
  }
  ...
  if (ruvResult.success && zenResult.success) {
    const speedDiff = ((ruvResult.time - zenResult.time) / ruvResult.time * 100);
    
    if (Math.abs(speedDiff) > 5) {
      const faster = speedDiff > 0 ? 'zen-swarm' : 'ruv-swarm';
      console.log(`• ${faster} is ${Math.abs(speedDiff).toFixed(1)}% faster at task execution`);
    } else {
    ...
    if (zenResult.neural_optimization) {
      console.log('• zen-swarm uses neural optimization for better code quality');
    }
  ...
  if (zenResult.success && zenResult.wasm_acceleration) {
    console.log('• Choose zen-swarm for CPU-intensive tasks needing WASM acceleration');
  }
  ...
  if (ruvResult.success && zenResult.success) {
    console.log('• Both swarms are viable - choice depends on specific requirements');
  }
