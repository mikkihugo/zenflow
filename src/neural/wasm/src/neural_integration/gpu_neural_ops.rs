...
use super::{
  ActivationFunction, BufferHandle, GpuBackendTrait, MemoryManagerTrait,
  NeuralIntegrationError, NeuralOperation, NeuralResult,
};
use std::sync::Arc;
  ...
  T: Clone + Send + Sync + 'static + bytemuck::Pod,
{
  match operation {
    ...
    NeuralOperation::ForwardPropagation {
      layer_sizes,
      _phantom,
    } => {
    ...
    NeuralOperation::BackwardPropagation {
      layer_sizes,
      _phantom,
    } => execute_backward_propagation(
      &layer_sizes,
      inputs,
      backend,
      memory_manager,
  ...
  T: Clone + Send + Sync + 'static + bytemuck::Pod,
{
  ...
  if inputs.len() < expected_size {
    return Err(NeuralIntegrationError::OperationError(format!(
  ...
  T: Clone + Send + Sync + 'static + bytemuck::Pod,
{
  ...
  let kernel_source =
    generate_convolution_kernel(channels, kernel_size, stride);
  let kernel = super::bridge::extract_wgsl_from_rust(&kernel_source)?;
  ...
  T: Clone + Send + Sync + 'static + bytemuck::Pod,
{
  ...
  for (operation, input) in operations.iter().zip(inputs.iter()) {
    let result =
      execute_operation(operation.clone(), input, backend, memory_manager)?;
    results.push(result);
  }
...
fn generate_matrix_multiply_kernel(
  a_rows: usize,
  a_cols: usize,
  b_cols: usize,
) -> String {
...
fn generate_convolution_kernel(
  channels: usize,
  kernel_size: usize,
  stride: usize,
) -> String {
