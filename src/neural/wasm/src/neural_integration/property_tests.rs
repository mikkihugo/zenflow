...
fn compute_gradients(weights: &[f32], inputs: &[f32]) -> Vec<f32> {
    ...
    weights.iter().zip(inputs.iter().cycle()).map(|(w, i)| w * i * 0.1).collect()
}

fn apply_gradient_update(weights: &[f32], gradients: &[f32], learning_rate: f32) -> Vec<f32> {
    weights.iter().zip(gradients.iter())
...
fn forward_pass(weights: &[f32], inputs: &[f32]) -> Vec<f32> {
    ...
    inputs.iter().zip(weights.iter().cycle())
