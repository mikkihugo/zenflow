...
impl KernelStats {
  ...
  pub fn record_launch(
    ...
    let threads_per_block =
      config.block.dim.x * config.block.dim.y * config.block.dim.z;
    ...
    if duration.as_secs_f64() > 0.0 {
      self.throughput_gbps =
        ((bytes_processed as f64 / 1e9) / duration.as_secs_f64()) as f32;
    ...
    self.occupancy =
      (threads_per_sm as f32 / max_threads_per_sm as f32) * 100.0;
  }
...
impl KernelProfiler {
  ...
  pub fn compare_kernels(
    ...
    Some(KernelComparison {
      kernel1: stat1.clone(),
      kernel2: stat2.clone(),
      speedup: stat1.average_time.as_secs_f64()
        / stat2.average_time.as_secs_f64(),
...
impl RooflineModel {
  ...
  pub fn analyze_kernel(
    ...
    let memory_bound_gflops =
      self.peak_memory_bandwidth_gbps * arithmetic_intensity;
    ...
    RooflineAnalysis {
      ...
      efficiency,
      is_memory_bound,
      bottleneck: if is_memory_bound { "Memory" } else { "Compute" }
        .to_string(),
    }
  }
}
...
impl RooflineAnalysis {
  pub fn print_analysis(&self) {
    ...
    println!(
      "Achievable performance: {:.2} GFLOPS",
      self.achievable_gflops
    );
