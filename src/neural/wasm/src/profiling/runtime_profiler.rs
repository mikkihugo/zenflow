...
impl RuntimeProfiler {
  ...
  pub fn print_summary(&self) {
    ...
    for (op_type, stat) in sorted_ops {
      let percentage =
        (stat.total_time.as_secs_f64() / total_runtime.as_secs_f64()) * 100.0;
  ...
  fn print_timeline_analysis(&self) {
    ...
    println!(
      ...
      (critical_path_time.as_secs_f64()
  ...
  pub fn export_trace(&self, path: &str) -> Result<(), CudaRustError> {
    ...
    writeln!(file, "[").map_err(|e| {
      CudaRustError::RuntimeError(format!("Failed to write header: {e}"))
    })?;

    for (i, event) in events.iter().enumerate() {
      let start_us =
        event.start_time.duration_since(self.start_time).as_micros();
    ...
    "args": {{}}
        ...
        event.name, event.operation_type, start_us, duration_us
      );

      if i < events.len() - 1 {
        writeln!(file, "{trace_event},").map_err(|e| {
          CudaRustError::RuntimeError(format!("Failed to write event: {e}"))
        })?;
      } else {
        writeln!(file, "{trace_event}").map_err(|e| {
          CudaRustError::RuntimeError(format!("Failed to write event: {e}"))
    ...
    writeln!(file, "]").map_err(|e| {
      CudaRustError::RuntimeError(format!("Failed to write footer: {e}"))
    })?;
  ...
  pub fn analyze_bottlenecks(&self) -> BottleneckAnalysis {
    ...
    let mut time_by_operation: Vec<_> = stats
      .iter()
      .map(|(op, stat)| (*op, stat.total_time))
      .collect();
    time_by_operation.sort_by(|a, b| b.1.cmp(&a.1));

    let primary_bottleneck = time_by_operation
      .first()
      .map(|(op, _)| *op)
...
impl OptimizationSuggestions {
  pub fn analyze(profiler: &RuntimeProfiler) -> Self {
    ...
    if let Some(percentage) =
      analysis.time_distribution.get(&OperationType::ModuleLoad)
    {
      if *percentage > 20.0 {
    ...
    if let Some(percentage) = analysis
      .time_distribution
      .get(&OperationType::ModuleCompile)
    {
      if *percentage > 30.0 {
        suggestions.push(Suggestion {
          severity: SuggestionSeverity::High,
          category: SuggestionCategory::RuntimeOptimization,
          message:
            "Compilation takes >30% of runtime. Use pre-compiled WASM modules."
              .to_string(),
    ...
    if let Some(percentage) = analysis
      .time_distribution
      .get(&OperationType::MemoryTransfer)
    {
      if *percentage > 40.0 {
        suggestions.push(Suggestion {
                    severity: SuggestionSeverity::High,
...
mod tests {
  ...
  fn test_runtime_profiler() {
    ...
    // Simulate operations
    let timer1 =
      profiler.start_operation(OperationType::ModuleLoad, "test_module");
    std::thread::sleep(Duration::from_millis(10));
    profiler.end_operation(timer1, HashMap::new());

    let timer2 =
      profiler.start_operation(OperationType::KernelLaunch, "test_kernel");
  ...
  fn test_bottleneck_analysis() {
    ...
    for _ in 0..10 {
      let timer =
        profiler.start_operation(OperationType::MemoryTransfer, "transfer");
      std::thread::sleep(Duration::from_millis(10));
