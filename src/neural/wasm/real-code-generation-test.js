    ...
    if (wasmModule && wasmModule.transpile_cuda) {
      ...
      for (let i = 0; i < testCudaKernels.length; i++) {
        ...
        transpiledResults.push({
          ...
          time_ms: kernelTime,
          valid_rust:
            rustCode.includes('use cuda_rust_wasm') &&
            rustCode.includes('kernel'),
          sample_output: rustCode.substring(0, 150) + '...',
        });

        console.log(
          `  ✅ Kernel ${i + 1}: ${kernel.length} chars → ${rustCode.length} chars (${kernelTime.toFixed(1)}ms)`
        );
      }
      ...
      results.wasm_transpiler = {
        ...
        total_input_chars: transpiledResults.reduce(
          (sum, r) => sum + r.input_chars,
          0
        ),
        total_output_chars: transpiledResults.reduce(
          (sum, r) => sum + r.output_chars,
          0
        ),
      ...
      console.log(
        `✅ WASM Transpiler: ${transpiledResults.length} kernels in ${wasmTime.toFixed(1)}ms\n`
      );
    }
  } catch (error) {
...
`;
    ...
    console.log(
      `✅ Generated ${writtenFiles.length} files in ${codeGenTime.toFixed(1)}ms`
    );
    console.log(`📁 Output directory: ${testDir}`);
    console.log(
      `📊 Total: ${results.file_system_verification.total_lines} lines, ${(results.file_system_verification.total_size_bytes / 1024).toFixed(1)}KB\n`
    );
  } catch (error) {
  ...
  if (results.wasm_transpiler?.success) {
    ...
    console.log(
      `  📊 Avg per kernel: ${wasm.avg_time_per_kernel.toFixed(1)}ms`
    );
    ...
    console.log(
      `  🎯 Throughput: ${(wasm.total_input_chars / (wasm.total_time_ms / 1000)).toFixed(0)} chars/sec`
    );
  }
  ...
  if (results.file_system_verification?.success) {
    ...
    console.log(
      `  💾 Total size: ${(files.total_size_bytes / 1024).toFixed(1)}KB`
    );
    console.log(
      `  🎯 Throughput: ${(files.total_lines / (files.total_time_ms / 1000)).toFixed(0)} lines/sec`
    );
...
testActualCodeGeneration()
  .then((results) => {
    ...
    if (
      results.wasm_transpiler?.success ||
      results.file_system_verification?.success
    ) {
