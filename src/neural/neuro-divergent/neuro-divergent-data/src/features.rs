...
impl<T: Float> FeatureEngine<T> {
  ...
  pub fn with_fourier_features(
    mut self,
    seasonal_periods: Vec<usize>,
    fourier_terms: Vec<usize>,
  ) -> Self {
    self.config.fourier_features =
      Some(FourierConfig::new(seasonal_periods, fourier_terms));
    self
  }
  ...
  pub fn generate_features(
    ...
    if let Some(ref rolling_config) = self.config.rolling_features {
      let (rolling_features, rolling_names) =
        self.generate_rolling_features(data, rolling_config)?;
    ...
    if let Some(ref temporal_config) = self.config.temporal_features {
      let (temporal_features, temporal_names) =
        self.generate_temporal_features(data, temporal_config)?;
    ...
    if let Some(ref fourier_config) = self.config.fourier_features {
      let (fourier_features, fourier_names) =
        self.generate_fourier_features(data, fourier_config)?;
    ...
    if let Some(ref exog_config) = self.config.exogenous_features {
      let (exog_features, exog_names) =
        self.generate_exogenous_features(data, exog_config)?;
  ...
  fn generate_lag_features(
    ...
    if n_samples <= max_lag {
      return Err(DataPipelineError::InvalidFormat {
        message: format!(
          "Insufficient data for max lag {}: need > {}, got {}",
  ...
  fn generate_rolling_features(
    ...
    if n_samples <= max_window {
      return Err(DataPipelineError::InvalidFormat {
        message: format!(
          "Insufficient data for max window {}: need > {}, got {}",
  ...
  fn generate_temporal_features(
    ...
    if config.day_of_week {
      if config.cyclic_encoding {
        feature_names
          .extend_from_slice(&["dow_sin".to_string(), "dow_cos".to_string()]);
        ...
        for day in 0..7 {
          ...
          for timestamp in &timestamps {
            let dow =
              (timestamp.weekday().number_from_monday() as usize - 1) % 7;
    ...
    if config.month {
      if config.cyclic_encoding {
        ...
        for month in 1..=12 {
          ...
          for timestamp in &timestamps {
            month_feature.push(if timestamp.month() == month {
              T::one()
            } else {
              T::zero()
            });
    ...
    if config.quarter {
      ...
      for quarter in 1..=4 {
        ...
        for timestamp in &timestamps {
          ...
          quarter_feature.push(if ts_quarter == quarter {
            T::one()
          } else {
            T::zero()
          });
    ...
    if config.hour_of_day {
      if config.cyclic_encoding {
        feature_names
          .extend_from_slice(&["hour_sin".to_string(), "hour_cos".to_string()]);
        ...
        for hour in 0..24 {
          ...
          for timestamp in &timestamps {
            hour_feature.push(if timestamp.hour() as usize == hour {
              T::one()
            } else {
              T::zero()
            });
    ...
    for (i, &period) in config.seasonal_periods.iter().enumerate() {
      ...
      for k in 1..=n_terms {
        ...
        for t in 0..n_samples {
          let angle =
            2.0 * std::f64::consts::PI * k as f64 * t as f64 / period as f64;
  ...
  fn compute_rolling_statistic(
    ...
    match stat {
      ...
      RollingStatistic::Std => {
        let mean = self
          .compute_rolling_statistic(window_data, RollingStatistic::Mean)?;
        let variance: T = window_data
          .iter()
          .map(|&x| (x - mean) * (x - mean))
          .fold(T::zero(), |acc, x| acc + x)
          / T::from(window_data.len()).unwrap();
        Ok(variance.sqrt())
      }
      ...
      RollingStatistic::Min => Ok(
        *window_data
          .iter()
          .min_by(|a, b| a.partial_cmp(b).unwrap())
          .unwrap(),
      ),
      RollingStatistic::Max => Ok(
        *window_data
          .iter()
      ...
      RollingStatistic::Skewness => {
        let mean = self
          .compute_rolling_statistic(window_data, RollingStatistic::Mean)?;
        let std =
          self.compute_rolling_statistic(window_data, RollingStatistic::Std)?;
  ...
  fn combine_feature_matrices(
    &self,
    matrices: Vec<Array2<T>>,
  ) -> Result<Array2<T>> {
...
mod tests {
  ...
  fn test_rolling_features() {
    ...
    assert!(features
      .feature_names
      .iter()
      .any(|name| name.contains("rolling_mean_7")));
    assert!(features
      .feature_names
      .iter()
      .any(|name| name.contains("rolling_std_7")));
    assert!(features
      .feature_names
      .iter()
      .any(|name| name.contains("rolling_min_7")));
    assert!(features
      .feature_names
      .iter()
      .any(|name| name.contains("rolling_max_7")));
  }
  ...
  fn test_temporal_features() {
    ...
    assert!(features
      .feature_names
      .iter()
      .any(|name| name.contains("quarter")));
    assert!(features
      .feature_names
      .iter()
      .any(|name| name.contains("is_business_day")));
  }
  ...
  fn test_fourier_features() {
    ...
    let fourier_features: Vec<_> = features
      .feature_names
      .iter()
    ...
    assert!(features
      .feature_names
      .iter()
      .any(|name| name.contains("fourier_sin_7_1")));
    assert!(features
      .feature_names
      .iter()
      .any(|name| name.contains("fourier_cos_7_1")));
    assert!(features
      .feature_names
      .iter()
      .any(|name| name.contains("fourier_sin_7_2")));
    assert!(features
      .feature_names
      .iter()
      .any(|name| name.contains("fourier_cos_7_2")));
  }
  ...
  fn test_rolling_statistics_computation() {
    ...
    // Test mean
    let mean = feature_engine
      .compute_rolling_statistic(&data, RollingStatistic::Mean)
      .unwrap();
    assert_relative_eq!(mean, 3.0, epsilon = 1e-10);

    // Test min/max
    let min = feature_engine
      .compute_rolling_statistic(&data, RollingStatistic::Min)
      .unwrap();
    let max = feature_engine
      .compute_rolling_statistic(&data, RollingStatistic::Max)
      .unwrap();
    assert_relative_eq!(min, 1.0, epsilon = 1e-10);
    assert_relative_eq!(max, 5.0, epsilon = 1e-10);

    // Test median
    let median = feature_engine
      .compute_rolling_statistic(&data, RollingStatistic::Median)
      .unwrap();
