...
impl<T: Float + Send + Sync> MetricCalculator<T> {
  ...
  pub fn calculate_all(
    ...
    y_true: &[T],
    y_pred: &[T],
  ) -> TrainingResult<HashMap<String, T>> {
...
impl<T: Float + Send + Sync> IntervalScore<T> {
  ...
  pub fn calculate_interval(
    ...
    let sum = y_true
      ...
      .map(|((&t, &l), &u)| {
        let width = u - l;
        let lower_penalty = if t < l {
          two / self.alpha * (l - t)
        } else {
          T::zero()
        };
        let upper_penalty = if t > u {
          two / self.alpha * (t - u)
        } else {
          T::zero()
        };
