...
async fn test_multiple_time_series_forecasting() -> Result<()> {
    ...
    // Sales data (seasonal)
    series_data.insert("sales", TimeSeriesData::seasonal(365, 30.0, 12.0, "daily")?);
    
    // Temperature data (seasonal with trend)
    series_data.insert("temperature", TimeSeriesData::seasonal_with_trend(365, 20.0, 10.0, 0.01, "daily")?);
    
    // Stock prices (random walk with volatility)
    series_data.insert("stock_price", TimeSeriesData::random_walk(365, 100.0, 0.02, "daily")?);
    
    // Energy consumption (multiple seasonality)
    series_data.insert("energy", TimeSeriesData::multi_seasonal(365, vec![7, 365], vec![50.0, 200.0], "daily")?);
    ...
    for (name, model) in models {
        ...
        let task = tokio::spawn(async move {
            trainer.train(model, &data).await.map(|trained| (name_clone, trained))
        });
    ...
    for (name, model) in &trained_models {
        let inference_data = processed_series[name].to_inference_data(
            match name.as_str() {
                "sales" | "temperature" => 7,
                "stock_price" => 1,
                "energy" => 24,
    ...
    for (name, predictions) in &forecasts {
        assert!(!predictions.is_empty(), "Predictions should not be empty for {}", name);
        ...
        assert_eq!(predictions.len(), expected_horizon, 
                  "Forecast horizon should match expected for {}", name);
    }
    
    // Test cross-series correlation analysis
    let correlation_analysis = analyze_cross_series_correlations(&forecasts)?;
    assert!(!correlation_analysis.is_empty(), "Should find some correlations");
    
    println!("   ✅ Forecasted {} time series successfully", forecasts.len());
...
async fn test_model_ensemble_creation() -> Result<()> {
    ...
    let training_data = TimeSeriesData::seasonal(1000, 100.0, 20.0, "daily")?.to_training_data()?;
    let validation_data = TimeSeriesData::seasonal(200, 100.0, 20.0, "daily")?.to_training_data()?;
    ...
    let weights = calculate_model_weights(&trained_base_models, &validation_data).await?;
    let weighted_ensemble = EnsembleModel::new_weighted(trained_base_models.clone(), weights)?;
    ...
    let trained_stacking = trainer.train_ensemble(&stacking_ensemble, &training_data).await?;
    ensembles.push(("stacking", trained_stacking));
    
    // 4. Dynamic ensemble (selects best model per prediction)
    let dynamic_ensemble = EnsembleModel::new_dynamic(trained_base_models.clone())?;
    ensembles.push(("dynamic", dynamic_ensemble));
    
    // Evaluate ensemble performance
    let test_data = TimeSeriesData::seasonal(100, 100.0, 20.0, "daily")?.to_inference_data(12)?;
    ...
    for (name, ensemble) in &ensembles {
        ...
        println!("   ✅ {} ensemble: MAE={:.2}, RMSE={:.2}", 
                name, metrics.mae, metrics.rmse);
    }
    
    // Verify ensemble diversity
    let diversity_score = calculate_ensemble_diversity(&ensembles, &test_data).await?;
    assert!(diversity_score > 0.1, "Ensemble should have meaningful diversity");
    ...
    println!("   ✅ Created and evaluated {} ensemble strategies", ensembles.len());
...
async fn test_online_learning_updates() -> Result<()> {
    ...
    let mut current_model = trainer.train(initial_model, &initial_training_data).await?;
    ...
    for batch in 0..10 {
        ...
        let new_data = TimeSeriesData::seasonal(50, 100.0 * shift_factor, 20.0, "daily")?;
        let new_training_data = new_data.to_training_data()?;
        
        // Test model performance before update
        let test_data = TimeSeriesData::seasonal(20, 100.0 * shift_factor, 20.0, "daily")?
            .to_inference_data(5)?;
        let pre_update_predictions = current_model.predict(&test_data).await?;
        let pre_update_metrics = calculate_forecast_metrics(&pre_update_predictions, &test_data)?;
        
        // Update model with new data
        current_model = trainer.update_online(&current_model, &new_training_data).await?;
        
        // Test model performance after update
        let post_update_predictions = current_model.predict(&test_data).await?;
        let post_update_metrics = calculate_forecast_metrics(&post_update_predictions, &test_data)?;
        ...
        performance_history.push(OnlineLearningMetrics {
            ...
            adaptation_score: calculate_adaptation_score(&pre_update_metrics, &post_update_metrics),
        });
        
        // Verify model adaptation
        if batch > 2 { // Allow some initial adaptation time
            let recent_performance = &performance_history[performance_history.len().saturating_sub(3)..];
            let avg_adaptation = recent_performance.iter()
                .map(|m| m.adaptation_score)
                .sum::<f64>() / recent_performance.len() as f64;
            
            assert!(avg_adaptation > -0.5, "Model should not degrade significantly");
        }
    }
    ...
    let final_predictions_on_original = current_model.predict(&original_test_data).await?;
    let final_metrics_on_original = calculate_forecast_metrics(&final_predictions_on_original, &original_test_data)?;
    
    // Model should still perform reasonably on original data
    assert!(final_metrics_on_original.mae < initial_metrics.pre_update_mae * 2.0,
    ...
    for (i, metrics) in performance_history.iter().enumerate().take(3) {
        ...
        registry.save_model_version(&*current_model, &version_path, i).await?;
    ...
    let rollback_predictions = rolled_back_model.predict(&original_test_data).await?;
    assert!(!rollback_predictions.is_empty());
    
    println!("   ✅ Processed {} online learning batches", performance_history.len());
...
async fn test_real_world_production_scenario() -> Result<()> {
    ...
    for (tenant_id, use_case, history_days) in tenants {
        ...
        let raw_data = match use_case {
            "sales forecasting" => TimeSeriesData::seasonal_with_noise(history_days, 1000.0, 100.0, 0.1, "daily")?,
            "demand forecasting" => TimeSeriesData::multi_seasonal(history_days, vec![24, 168], vec![50.0, 200.0], "hourly")?,
            "capacity planning" => TimeSeriesData::trending(history_days, 100.0, 0.02, "daily")?,
        ...
        let (model_type, config) = match use_case {
            "sales forecasting" => ("lstm", ModelConfig::from_params([
                ("hidden_size", "128"),
                ("num_layers", "2"),
                ("seasonal_features", "true"),
                ("input_size", "30"),
            ...
            "demand forecasting" => ("lstm", ModelConfig::from_params([
                ("hidden_size", "96"),
                ("num_layers", "3"),
                ("multi_seasonal", "true"),
                ("input_size", "168"),
            ...
            "capacity planning" => ("mlp", ModelConfig::from_params([
                ("hidden_size", "64"),
                ("num_layers", "2"),
                ("trend_modeling", "true"),
                ("input_size", "14"),
        ...
        let trained_model = trainer.train_with_callbacks(model, &training_data).await?;
        
        // 4. Model validation and testing
        let validation_split = processed_data.split_for_validation(0.2)?;
        let validation_metrics = trainer.validate(&trained_model, &validation_split).await?;
        
        assert!(validation_metrics.mae > 0.0, "Validation should produce meaningful metrics");
        ...
        let quality_score = processor.assess_prediction_quality(&final_predictions)?;
        
        assert!(quality_score > 0.5, "Predictions should meet quality threshold");
        ...
        tenant_results.insert(tenant_id.to_string(), TenantResult {
        ...
        println!("   ✅ {} completed: MAE={:.3}, Quality={:.3}", 
                tenant_id, validation_metrics.mae, quality_score);
    }
    
    // 8. System-wide analysis
    let total_predictions: usize = tenant_results.values()
        .map(|r| r.predictions_count)
        .sum();
    
    let avg_quality: f64 = tenant_results.values()
        .map(|r| r.quality_score)
        .sum::<f64>() / tenant_results.len() as f64;
    
    let total_model_size: f64 = tenant_results.values()
        .map(|r| r.model_size_mb)
        .sum();
    
    assert_eq!(tenant_results.len(), 3, "Should process all tenants");
    assert!(total_predictions > 50, "Should generate substantial predictions");
...
fn analyze_cross_series_correlations(forecasts: &HashMap<String, Vec<f64>>) -> Result<HashMap<String, f64>> {
...
async fn calculate_model_weights(models: &[Box<dyn neuro_divergent_core::BaseModel<f64> + Send + Sync>], validation_data: &neuro_divergent_core::TrainingData) -> Result<Vec<f64>> {
...
async fn calculate_ensemble_diversity(ensembles: &[(&str, EnsembleModel)], test_data: &neuro_divergent_core::InferenceData) -> Result<f64> {
...
fn calculate_forecast_metrics(predictions: &[f64], test_data: &neuro_divergent_core::InferenceData) -> Result<ForecastMetrics> {
...
fn calculate_adaptation_score(pre_metrics: &ForecastMetrics, post_metrics: &ForecastMetrics) -> f64 {
