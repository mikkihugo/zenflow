...
async fn test_error_propagation_across_crates() -> Result<()> {
    ...
    if let Err(core_error) = invalid_data {
        ...
        assert!(training_result.is_err(), "Should propagate core error to models");
        ...
        if let Err(model_error) = training_result {
            ...
            assert!(registry_result.is_err(), "Should propagate model error to registry");
...
async fn test_graceful_degradation() -> Result<()> {
    ...
    let fallback_result = registry.create_model_with_fallback("lstm", &invalid_config).await;
    match fallback_result {
        ...
        Err(e) => {
            ...
            assert!(e.to_string().contains("fallback") || e.to_string().contains("default"));
    ...
    let simple_fallback = registry.create_model_with_fallback("lstm", &complex_config).await;
...
async fn test_recovery_scenarios() -> Result<()> {
    ...
    // Scenario 1: Network interruption during model download
    {
        ...
        let download_result = reg.download_model_with_retry(
            "nonexistent_model",
            3, // max retries
        ).await;
        
        match download_result {
            Err(RegistryError::ModelNotFound { model_name, alternatives }) => {
    ...
    let training_result = memory_stressed_trainer.train_with_recovery(
    ...
    match training_result {
        ...
        Err(ModelError::OutOfMemory { recovery_suggestions }) => {
    ...
    
    {
        ...
        match load_result {
            Err(RegistryError::CorruptedModel { path, checksum_mismatch }) => {
...
async fn test_error_reporting_and_diagnostics() -> Result<()> {
    ...
    if let Err(error) = creation_result {
        ...
        assert!(error_details.contains("hidden_size"), "Should report hidden_size issue");
        assert!(error_details.contains("num_layers"), "Should report num_layers issue");
        assert!(error_details.contains("dropout"), "Should report dropout issue");
        assert!(error_details.contains("unknown_param"), "Should report unknown parameter");
    ...
    let context_test = async {
        ...
        let model = registry.create_model("invalid_model", &ModelConfig::default())
            .context("Creating model from registry")?;
        
        trainer.train(model, &data.to_training_data()?)
            .await
    ...
    if let Err(error) = context_test.await {
        ...
        assert!(error_chain.contains("Creating model from registry") || 
...
async fn test_error_boundaries_and_isolation() -> Result<()> {
    ...
    println!("   ✅ Error isolation verified: {} successes, {} failures", 
