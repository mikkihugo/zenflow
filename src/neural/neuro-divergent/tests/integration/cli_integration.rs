...
"#;
  ...
  match output {
    ...
    Err(_) => {
      ...
      test_programmatic_training(&config_path, &data_path, &output_path)
        .await?;
    }
  }
...
async fn test_cli_results_visualization() -> Result<()> {
  ...
  match output {
    ...
    Err(_) => {
      ...
      test_programmatic_visualization(&model_path, &data_path, &output_path)
        .await?;
    }
  }
  ...
  match invalid_file {
    Ok(result) => {
      ...
      if !result.status.success() {
        let error_text = format!("{}{}", stdout, stderr);
        assert!(
          error_text.contains("file")
            || error_text.contains("path")
            || error_text.contains("not found")
            || error_text.contains("exist")
        );
...
async fn test_programmatic_training(
  config_path: &PathBuf,
  data_path: &PathBuf,
  output_path: &PathBuf,
) -> Result<()> {
  ...
  let config: ModelConfig =
    toml::from_str(&config_str).unwrap_or_else(|_| ModelConfig::default());
...
async fn test_programmatic_visualization(
  model_path: &PathBuf,
  data_path: &PathBuf,
  output_path: &PathBuf,
) -> Result<()> {
