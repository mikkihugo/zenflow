...
))
{
  ...
  if (this.bindings) {
    ...
    for (const modelName of modelList) {
      ...
      ('analysis')
      : await this.createAnalysisModel(),
      ('classification')
      : await this.createClassificationModel(),
      ('generation')
      : await this.createGenerationModel(),
      ('code-completion-base')
      : await this.createCodeCompletionModel(),
      ('bug-detector-v2')
      : await this.createBugDetectionModel(),
      ('refactor-assistant')
      : await this.createRefactoringModel(),
      ('test-generator-pro')
      : await this.createTestGenerationModel(),
      ('docs-writer')
      : await this.createDocumentationModel()
    }
