...
export abstract class BaseDao<T> implements IDao<T> {
  ...
  async findBy(criteria: Partial<T>, options?: QueryOptions): Promise<T[]> {
    this.logger.debug(`Finding entities by criteria in table: ${this.tableName}`, { criteria, options });
  ...
  async executeCustomQuery<R = any>(customQuery: CustomQuery): Promise<R> {
    ...
    try {
      if (typeof customQuery.query === 'string') {
        const result = await this.adapter.query(customQuery.query, customQuery.parameters ? Object.values(customQuery.parameters) : undefined);
        return result as R;
      } else {
  ...
  protected buildFindByQuery(criteria: Partial<T>, options?: QueryOptions): { sql: string; params: any[] } {
  ...
  protected buildUpdateQuery(id: string | number, updates: Partial<T>): { sql: string; params: any[] } {
  ...
  protected buildWhereClause(criteria: Record<string, any>): string {
    ...
    const conditions = Object.keys(criteria).map(column => `${column} = ?`).join(' AND ');
...
export abstract class BaseManager<T> implements IManager<T> {
   ...
   */
  protected abstract getDatabaseType(): 'relational' | 'graph' | 'vector' | 'memory' | 'coordination';
   ...
   * Helper methods
   */
  protected calculateHealthScore(healthy: boolean, responseTime: number, connectionStats: any): number {
    if (!healthy) return 0;
