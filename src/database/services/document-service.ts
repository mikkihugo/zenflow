/**
 * Document Service - Pure Database-Driven Operations
 *
 ...
 * Integrates with existing DatabaseCoordinator for multi-engine support
 */

import { nanoid } from 'nanoid';
import type { DocumentType } from '../../types/workflow-types';
import type { DatabaseCoordinator, DatabaseQuery } from '../core/database-coordinator';
import type {
  ADRDocumentEntity,
  BaseDocumentEntity,
...
/**
 * Pure Database-Driven Document Service
 * Replaces file-based operations with database entities
 */
export class DocumentService {
  private coordinator: DatabaseCoordinator;
  ...
  /**
   * Initialize document service and database schema
   */
  async initialize(): Promise<void> {
    // Create database tables using existing coordinator
    const schemaQueries = [
      this.createSchemaQuery('documents'),
      this.createSchemaQuery('document_relationships'),
      this.createSchemaQuery('document_workflow_states'),
      this.createSchemaQuery('projects'),
      this.createSchemaQuery('document_search_index'),
    ];

    for (const query of schemaQueries) {
      await this.coordinator.executeQuery(query);
    }
  }
  ...
  /**
   * Create a new document with automatic relationship generation
   */
  async createDocument<T extends BaseDocumentEntity>(
    ...
    const insertQuery: DatabaseQuery = {
      id: `insert_document_${id}`,
      type: 'write',
      operation: 'document_insert',
      parameters: {
        collection: 'documents',
        document: this.serializeDocument(fullDocument),
      },
      requirements: {
        consistency: 'strong',
        timeout: 30000,
    ...
    if (options.autoGenerateRelationships) {
      await this.generateDocumentRelationships(fullDocument);
    }
    ...
    if (options.generateSearchIndex !== false) {
      await this.generateSearchIndex(fullDocument);
    }
