...
console.warn(`📁 Adding ${config.type} database = {id = new SQLiteConnectionPool(
        ...
        case 'kuzu':
          const kuzuDB = new KuzuAdvancedInterface({dbPath = kuzuDB;
          break;
...
: Promise<DatabaseMetrics[]>
{
  ...
  async query<T = any>(sql, params?: any[], options?: QueryOptions): Promise<QueryResult<T>> {
    const startTime = Date.now();
    
    try {
      const result = await this.pool.execute(sql, params, {timeout = await this.pool.execute(sql, params);
