...
;
        const _graphResult = await instance.connection.executeGraphQuery(query.graphQuery);
        result = graphResult;
;
    ...
    for (const query of queries) {
      try {
        const _result = await this.executeQuery(databaseId, query, options);
...
;
  ...
  for (const [id, instance] of this.databases) {
    totalCount++;
    const _dbHealth = await this.checkDatabaseHealth(instance);
...
;
    try {
      const _result = await this.pool.execute(sql, params, {timeout = await this.pool.execute(sql, params);
    ...
    // return { nodes: result, totalCost: 0, estimatedRows: 0  // LINT: unreachable code removed};
  }
}
;
