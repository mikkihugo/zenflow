...
export class KuzuAdapter {
  ...
  async createNode(label: string, properties: Record<string, any>): Promise<string> {
    ...
    try {
      const id = properties.id || `${label}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  async createRelationship(
    ...
    try {
      ...
      if (propKeys.length > 0) {
        const propAssignments = propKeys.map(key => `r.${key} = $`).join(', ');
  ...
  async findNodes(label: string, properties: Record<string, any> = {}, options: GraphQueryOptions = {}): Promise<GraphNode[]> {
    if (!this.connected) await this.connect();
    if (!this.connection) throw new Error('Database not connected');
  ...
  async getNeighbors(nodeId: string, depth: number = 1, relationshipType?: string): Promise<{
    nodes: GraphNode[];
  ...
  async updateNode(id: string, properties: Record<string, any>): Promise<void> {
    ...
    try {
      ...
      const queryWithValues = this.embedParameters(query, [id, ...Object.values(propsWithUpdate)]);
  ...
  async execute(cypher: string, params: any[] = []): Promise<any> {
    if (!this.connected) await this.connect();
    if (!this.connection) throw new Error('Database not connected');
  ...
  async getSchema(): Promise<any> {
    ...
    try {
      ...
      return { 
        tables: ['Document', 'Project', 'User', 'Tag'].map(name => ({ name, type: 'node_table' })),
        views: [] 
      };
    } catch (error) {
  ...
  private embedParameters(query: string, params: any[]): string {
    let result = query;
    let paramIndex = 0;
  ...
  async queryGraph(cypher: string, params: any = {}): Promise<any> {
    try {
  ...
  async getNodeCount(label?: string): Promise<number> {
    try {
      const query = label ? `MATCH (n:${label}) RETURN count(n)` : 'MATCH (n) RETURN count(n)';
  ...
  async getGraphStats(): Promise<{
    ...
    try {
      ...
      return {
        ...
        relationshipTypes: ['RELATES_TO', 'BELONGS_TO', 'AUTHORED_BY', 'TAGGED_WITH', 'DEPENDS_ON', 'IMPLEMENTS', 'SUPERSEDES']
      };
    } catch (error) {
