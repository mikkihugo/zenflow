...
;
    try {
      const __centralityScores = await this.computeDegreeCentrality(nodeType, relationshipType);
          break;
        case 'betweenness':;
...
;
    ...
    for (let i = 0; i < Math.min(nodes.length, 20); i++) { // Limit for performance
      for (let j = i + 1; j < Math.min(nodes.length, 20); j++) {
        try {
          const _pathResult = await this.advancedTraversal({startNode = pathResult.data[0].path_nodes  ?? [];
...
;
    try {
      const _communities = await this.louvainCommunityDetection(nodeType, relationshipType, resolution);
          break;
