...
import { createRepository, createDAO, DatabaseTypes, EntityTypes } from '../index';
import type {
  ADRDocumentEntity,
  BaseDocumentEntity,
...
export class DocumentManager {
  ...
  async initialize(): Promise<void> {
    ...
    this.documentRepository = await createRepository<BaseDocumentEntity>(
      EntityTypes.Document, 
      this.databaseType
    );
    
    this.projectRepository = await createRepository<ProjectEntity>(
      'Project', 
      this.databaseType
    );
    
    this.relationshipRepository = await createRepository<DocumentRelationshipEntity>(
      'DocumentRelationship', 
      this.databaseType
    );
    
    this.workflowRepository = await createRepository<DocumentWorkflowStateEntity>(
    ...
    this.projectDAO = await createDAO<ProjectEntity>(
      'Project', 
      this.databaseType
    );
  }
   ...
   */
  private async findParentDocuments(document: BaseDocumentEntity): Promise<Omit<DocumentRelationshipEntity, 'id' | 'created_at'>[]> {
    const relationships: Omit<DocumentRelationshipEntity, 'id' | 'created_at'>[] = [];
    ...
    if (parentTypes.length > 0) {
      ...
      for (const parent of potentialParents.slice(0, 3)) { // Limit to 3 most relevant
        const strength = this.calculateRelationshipStrength(document, parent);
        if (strength > 0.3) { // Only create relationships above threshold
          relationships.push({
   ...
   */
  private async findSemanticRelationships(document: BaseDocumentEntity): Promise<Omit<DocumentRelationshipEntity, 'id' | 'created_at'>[]> {
    const relationships: Omit<DocumentRelationshipEntity, 'id' | 'created_at'>[] = [];
   ...
   */
  private async findWorkflowRelationships(document: BaseDocumentEntity): Promise<Omit<DocumentRelationshipEntity, 'id' | 'created_at'>[]> {
    const relationships: Omit<DocumentRelationshipEntity, 'id' | 'created_at'>[] = [];
   ...
   */
  private calculateRelationshipStrength(doc1: BaseDocumentEntity, doc2: BaseDocumentEntity): number {
    let strength = 0;
  ...
  private async getDocumentRelationships(
    ...
    return allRelationships.filter(r => 
      r.source_document_id === documentId || r.target_document_id === documentId
    );
  }
...
interface WorkflowAutomationRule {
  name: string;
  condition: {
    type: 'status_change' | 'stage_duration' | 'document_type' | 'priority_level' | 'completion_percentage' | 'has_relationships' | 'custom_field';
    value: any;
  };
  action: {
    type: 'advance_stage' | 'create_document' | 'update_status' | 'assign_reviewer' | 'generate_artifacts' | 'send_notification' | 'update_relationships';
    value: any;
  };
}
