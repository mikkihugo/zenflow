...
export interface ADRStats {
  ...
  semantic_clusters?: Array<{ // New: semantic clustering
    theme: string;
    count: number;
    adrs: string[];
  }>;
...
export class ADRManagerHybrid {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (existingProjects.length > 0) {
        ...
        this.architectureProject = await this.hybridManager.createDocument<ProjectEntity>({
          type: 'project' as any,
          ...
          content: 'Central project for managing all Architecture Decision Records',
          status: 'active',
          priority: 'high',
  ...
  async createADR(options: ADRCreateOptions): Promise<ADRDocumentEntity> {
    ...
    const adr = await this.hybridManager.createDocument<ADRDocumentEntity>({
      ...
      alternatives_considered: options.alternatives?.map(alt => alt.name) || [],
  ...
  async queryADRs(options: ADRQueryOptions = {}): Promise<{
    ...
    if (semantic_query) {
      ...
      const relationships = await this.hybridManager.getDocumentRelationships(related_to, 2);
      const relatedIds = relationships.map(r => 
        r.source_document_id === related_to ? r.target_document_id : r.source_document_id
      );
      ...
      searchResults = hybridResults.slice(offset, offset + limit).map(r => r.document as ADRDocumentEntity);
    }

    // Generate semantic insights
    const semantic_insights = await this.generateSemanticInsights(searchResults);
  ...
  private async analyzeDecisionImpact(adrId: string): Promise<{
    ...
    const relationships = await this.hybridManager.getDocumentRelationships(adrId, 2);
  ...
  private async generateSemanticInsights(adrs: ADRDocumentEntity[]): Promise<{
    ...
    const allContent = adrs.map(adr => `${adr.title} ${adr.context} ${adr.decision}`).join(' ');
    ...
    for (const adr of adrs) {
      const relationships = await this.hybridManager.getDocumentRelationships(adr.id, 1);
  ...
  private async generateSemanticClusters(adrs: ADRDocumentEntity[]): Promise<Array<{
    theme: string;
    count: number;
    adrs: string[];
  }>> {
