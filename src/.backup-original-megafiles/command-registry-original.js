...
import { importMonorepoCommand } from './command-handlers/monorepo-import-command.js';
...
export const createMeowCLI = () => {
  return meow(`
	...
	Commands
	  ...
	  import         Import and analyze existing monorepo (15 microservices pilot)
	...
	Examples
	  ...
	  $ claude-zen import /path/to/monorepo --auto-create-hives --max-services 15
`, {
...
Examples:
  ...
  commandRegistry.set('hive-mind', {
    ...
    description: '🏗️ Persistent service-level coordination with database memory',
    usage: 'hive-mind <command> [options]',
    examples: [
      'hive-mind create my-service',
      'hive-mind spawn "Build user service" --service users',
      'hive-mind assign "Optimize database" --name api-service',
      'hive-mind status --verbose',
    ],
    details: `
Hive Mind - Persistent Service Coordination:
  • Database-backed memory across sessions
  • Service-specific coordination contexts  
  • Learning from previous decisions
  • Cross-session task continuity

Commands:
  create <name>       Create new persistent service hive
  spawn <objective>   Launch persistent coordination 
  assign <objective>  Assign task to existing hive
  status             Show all hive statuses
  list               List all created hives
  scan               Scan project for optimization

Use for: Long-running services, complex projects, learning systems
vs. 'swarm' for: Quick tasks, prototyping, one-off coordination`
  });

  // Direct swarm command - temporary session-based coordination
  commandRegistry.set('swarm', {
    handler: async (args, flags) => {
      const { swarmCommand } = await import('./command-handlers/swarm-command.js');
      return swarmCommand(args, flags);
    },
    description: '🐝 Launch temporary swarm coordination (session-based)',
    usage: 'swarm <objective> [options]',
    examples: [
      'swarm "Build a REST API" --strategy development',
      'swarm "Create user interface" --topology mesh',
      'swarm "Research architecture patterns" --max-agents 5 --analysis',
    ],
  });
...
plugins, settings, and development workflows.`
  ...
  commandRegistry.set('import', {
    handler: importMonorepoCommand,
    description: 'Import and analyze existing monorepo (15 microservices pilot)',
    usage: 'import [monorepo-path] [options]',
    examples: [
      'import /path/to/monorepo',
      'import . --auto-create-hives --max-services 15',
      'import ../my-monorepo --analyze-code --setup-graph',
      'import --verbose --max-services 10'
    ],
    details: `
Monorepo Import & Analysis (15 Microservices Pilot):
  • Multi-strategy service discovery (directory, package.json, Docker, Bazel, NX)
  • Code analysis and technology detection
  • Kuzu graph database integration for service relationships
  • Automatic hive creation for discovered services
  • Dependency analysis and architectural recommendations
  • Service pattern detection and standardization suggestions

Discovery Strategies:
  📁 Directory structure (services/, apps/, packages/)
  📦 package.json analysis with dependency mapping
  🐳 Dockerfile detection for containerized services  
  🏗️ BUILD file analysis (Bazel/Buck support)
  🎯 NX workspace integration with project.json
...
Graph Database (Kuzu):
  🕸️ Service relationship graph storage
...
Options:
  ...
  --auto-create-hives           Automatically create hives for services
  --analyze-code                Enable deep code analysis (default: true)
  --setup-graph                 Set up Kuzu graph database (default: true)
  --verbose                     Show detailed analysis progress

Perfect for: Migrating existing monorepos to Claude Zen, understanding
service architectures, and establishing coordination patterns.`
  });
