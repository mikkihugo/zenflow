...
import { getEnvironmentType, isInteractive, isRawModeSupported } from './interactive-detector.js';
...
export function nonInteractivePrompt<T>(message => {
    const choiceValue = typeof choice === 'string' ?choice = typeof choice === 'string' ? choice : (choice.name || choice.value);
 ...
 * @returns The default value
 */
export function nonInteractiveConfirm(message = false): boolean {
