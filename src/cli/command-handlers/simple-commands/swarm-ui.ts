...
updateSwarmData();
{
  ...
  async createObjective(description): any 
    try {
      this.log(`Creatingobjective = ['swarm', description, '--ui', '--monitor'];
      const process = spawn('claude-zen', args, {detached = `swarm-${Date.now()}`;
      this.activeProcesses.set(processId, process);

      this.log(`Launched swarm with PID => {
        this.updateSwarmData();
      }, 2000);catch(error) 
      ...
      // Windows => {
        ...
        if(error) {
          this.log(`Commanderror = 'info'): any {
