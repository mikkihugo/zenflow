...
export class ValidationTestRunner {
  ...
  async createTestFiles() {
    try {
      await node.mkdir(`${this.workingDir}/test-temp`, { recursive: true });
      await node.writeTextFile(`${this.workingDir}/test-temp/CLAUDE.md`, '# Test CLAUDE.md');
      await node.writeTextFile(`${this.workingDir}/test-temp/memory-bank.md`, '# Test Memory Bank');
    } catch {
  ...
  async cleanupTestFiles() {
    try {
      await node.remove(`${this.workingDir}/test-temp`, { recursive: true });
    } catch {
  ...
  async cleanupTestConfigs() {
    try {
      await node.remove(`${this.workingDir}/test-roomodes`);
    } catch {
      // Cleanup failed - not critical
    }
  }
  ...
  async createTestSparcConfig() {
    try {
      await this.createTestConfigs();
      await node.mkdir(`${this.workingDir}/test-roo`, { recursive: true });
    } catch {
  ...
  async cleanupTestSparcConfig() {
    try {
      await this.cleanupTestConfigs();
      await node.remove(`${this.workingDir}/test-roo`, { recursive: true });
    } catch {
      // Cleanup failed - not critical
    }
  }
