...
async function initializeProject(projectPath, options = {}) {
  ...
  try {
    ...
    await node.mkdir(absoluteProjectPath, { recursive: true });
    ...
    await Promise.all(directories.map(dir => 
      node.mkdir(dir, { recursive: true }).catch(() => {})
    ));
    await Promise.all(
      directories.map((dir) => node.mkdir(dir, { recursive: true }).catch(() => {})),
    );
    ...
    fileCreationTasks.push(node.writeTextFile('CLAUDE.md', claudeMd));

    // memory-bank.md
    const memoryBankMd = minimal ? createMinimalMemoryBankMd() : createFullMemoryBankMd();
    fileCreationTasks.push(node.writeTextFile('memory-bank.md', memoryBankMd));

    // coordination.md
    const coordinationMd = minimal ? createMinimalCoordinationMd() : createFullCoordinationMd();
    fileCreationTasks.push(node.writeTextFile('coordination.md', coordinationMd));

    // README files
    fileCreationTasks.push(
      node.writeTextFile('memory/agents/README.md', createAgentsReadme()),
      node.writeTextFile('memory/sessions/README.md', createSessionsReadme())
    );
    ...
    fileCreationTasks.push(
      node.writeTextFile('memory/claude-flow-data.json', JSON.stringify(initialData, null, 2))
    );
    ...
    if (ENVIRONMENT_CONFIGS[environment]) {
      ...
      const envContent = Object.entries(envConfig.config)
        ...
        .join('\n');
      fileCreationTasks.push(node.writeTextFile('.env', envContent));
    }
    ...
    if (template && PROJECT_TEMPLATES[template]) {
      ...
      if (templateConfig.extraFiles) {
        for (const [filePath, content] of Object.entries(templateConfig.extraFiles)) {
          ...
          fileCreationTasks.push(node.writeTextFile(filePath, fileContent));
