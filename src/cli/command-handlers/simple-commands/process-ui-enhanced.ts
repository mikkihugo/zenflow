...
const _PROCESSES = [
  ...
  getStatusIcon(status): any {
    ...
    const color = percentage >= 80 ? colors.green = 50 ? colors.yellow : colors.red;
    return color(bar) + ` ${percentage.toFixed(0)}%`;
  }

  getUsageBar(value, max): any {
  ...
  async handleOrchestrationInput(input): any 
    switch(input) {
      case 'n': {
        // Spawn new agent
