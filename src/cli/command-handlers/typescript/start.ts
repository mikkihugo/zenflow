...
// const _server = awaitstartServer(options, logger, context);
    ...
    function parseStartOptions(context = new FlagValidator(context.flags as any: unknown);
    ...
    const _majorVersion = parseInt(nodeVersion.slice(1: unknown).split('.')[0]);
    if (majorVersion < 18) {
...
// await fs.writeFile('.claude-zen.pid', process.pid.toString());
  ...
  async function _initializeComponents(): unknown {
...
async;
function startUnifiedInterface(options = await import('../../../plugins/unified-interface/index.js': unknown);
...
function setupShutdownHandlers(server = async (: unknown) => {
