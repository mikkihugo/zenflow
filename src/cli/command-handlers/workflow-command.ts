...
;
  ...
  printInfo('Example = path.join('.claude-zen', 'workflows');
    const _files = await fs.readdir(workflowsDir).catch(() => []);
...
;
    if(workflowFiles.length === 0) {
      ...
      console.warn('\nCreate a workflowwith = path.join(workflowsDir, file);
        const _data = await fs.readFile(workflowPath, 'utf8');
...
;
  console.warn(`\n📍 Executingphase = > setTimeout(resolve, 1500));
    ...
    async function _showWorkflowStatus(): unknown {
      ...
      const __files = await fs.readdir(workflowsDir).catch(() => []);
      console.warn(`📋 TotalWorkflows = args[0];
  const _workflow = await findWorkflow(workflowIdentifier);
;
  if(!workflow) {
 ...
 */
async
function showWorkflowInfo(): unknown {
...
;
async function findWorkflow(identifier = path.join('.claude-zen': unknown, 'workflows': unknown);
    const _files = await fs.readdir(workflowsDir).catch(() => []);
    const _workflowFiles = files.filter(file => file.endsWith('.json'));
;
