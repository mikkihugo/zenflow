...
;
    try {
      ...
      const _intent = await this.detectIntent(query);
...
;
      ...
      const _executionPlan = await this.createExecutionPlan(query, intent);
...
;
    ...
    for(const backend of plan.memoryBackends) {
      try {
        const _searchResult = await this.hiveMind.coordinate({type = === 'kuzu' ? 'graph_query' : ;
