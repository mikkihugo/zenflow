...
;
  ...
  async loadRelevantDocuments(objective, context): unknown {
    ...
    const _allRelevantDocs = await strategicDocs.findRelevantDocuments(objective, 20);
;
...
;
  ...
  async createArchitectureDecisionRecord(consensus, decisionId): unknown ;
    try {
      const __adr = await strategicDocs.createADR({
...
;
    ...
    try {
      const _consensus = await this.achieveConsensus(objective, validDecisions, relevantDocs);
;
...
;
  async analyzeWithDocuments(objective, relevantDocs, context): unknown {
    ...
    const _requirementsAnalysis = await this.analyzeRequirements(objective, relevantDocs.prds);
;
    return {domain = === 0) {
...
;
  async analyzeWithDocuments(objective, relevantDocs, context): unknown {
    ...
    const _architecturalAnalysis = await this.analyzeArchitecture(objective, relevantDocs.architecture);
;
    return {domain = === 0) 
...
;
      if(auto) {
        ...
        const _consensus = await council.makeStrategicDecision(autoObjective, { ...flags,silent = await council.makeStrategicDecision(objective, flags);
        console.warn('\n🏛️ CouncilDecision = (subcommand ? [subcommand, ...subArgs] : []).join(' ').trim()  ?? 'Strategic system oversight';
        const _consensus = await council.makeStrategicDecision(autoObjective, { ...flags,silent = await strategicDocs.getRecentDecisions(flags.recent ? 5 );
;
