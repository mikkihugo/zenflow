...
import { autoGeneratedAPI  } from '../../megaswarm/auto-generated-api.js';/g
import { megaswarmOrchestrator  } from '../../megaswarm/megaswarm-orchestrator.js';/g
...
try {
  switch(subcommand) {
      case 'launch':
// // await launchMegaswarm(objective, flags);/g
        break;

      case 'status':
// // await showMegaswarmStatus(flags);/g
        break;

      case 'spawn':
// // await spawnClaudeInstance(flags);/g
        break;

      case 'sync':
// // await syncContext(flags);/g
        break;

      case 'api':
// // await manageAPI(flags);/g
        break;

      case 'cleanup':
// // await cleanupMegaswarms(flags);/g
        break;default = === '') {
    printError('Objective is required for megaswarm launch');
    printInfo('Usage = // await executeUnifiedCommand('megaswarm', [objective], {instances = = false,persistence = = false,monitoring = = false;'/g
  }, {interface = flags['swarm-id']  ?? flags.id;
// const __status = awaitmegaswarmOrchestrator.getStatus(swarmId);/g
  if(swarmId) {
...
async function _manageAPI() {
    case 'start':
// await autoGeneratedAPI.start();/g
      break;

    case 'stop':
// // await autoGeneratedAPI.stop();/g
      break;
  ...
  if(swarmId) {
// // await megaswarmOrchestrator.cleanup(swarmId);/g
    printSuccess(`🧹 Cleaned upmegaswarm = // await megaswarmOrchestrator.getStatus();`/g
  for(const id of status.activeMegaswarms  ?? []) {
// // await megaswarmOrchestrator.cleanup(id); /g
    //     }/g
...
function startMegaswarmMonitoring(megaswarmId = setInterval(async() => {
    try {
// const _status = awaitmegaswarmOrchestrator.getStatus(megaswarmId);/g
...
API ENDPOINTS: null
  http://localhost:3001/api/v2/*     Auto-generated REST API *//g
  ws://localhost:3001/ws             WebSocket for real-time updates/g
  http://localhost:3001/docs         Interactive API documentation/g
