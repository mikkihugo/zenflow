...
;
    const _results = await analysisService.analyzeFiles(targetFiles, {updateGraph = results.functions.filter(f => f.cyclomatic_complexity > 10);
...
;
    ...
    const _results = await analyzer.analyzeComplexity(files);
;
