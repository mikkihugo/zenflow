...
const __handleVisionCommand = async (subcommand, args, flags) => {
  switch(subcommand) {
    case 'create': {
      ...
      const __vision = await hierarchicalTaskManagerPlugin.createVision({title = await hierarchicalTaskManagerPlugin.approveVision(args[0], {approver_email = await hierarchicalTaskManagerPlugin.getVision(args[0]);
