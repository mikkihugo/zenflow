...
;
async function preTaskCommand(subArgs: unknown, flags = flags;
...
;
async function postTaskCommand(_subArgs: unknown, flags = flags;
...
;
  ...
  if (generateSummary) console.warn(`📊 Summarygeneration = await getMemoryStore();
    const _tasks = await store.list({namespace = await store.list({ namespace: 'file-history',limit = await store.list({ namespace: 'command-history',limit = await store.list({ namespace: 'agent-roster',limit = null;
    if(exportMetrics) {
      const _now = new Date();
...
;
    ...
    let _sessionData;
    if(sessionId === 'latest') {
      const _sessions = await store.list({namespace = sessions[0]?.value;
    } else {
