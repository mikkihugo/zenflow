...
export async function manageMemoryWizard() {
  ...
  try {
    const _action = await inquirer.prompt([
      {
        type = {list = {}): any {
...
export async function searchMemories() {
  ...
  try {
    ...
    for(const result of results) {
      ...
      if(answers.searchType !== 'keys') {
        const _preview = result.value.substring(0, 200);
...
export async function storeMemoryWizard() {
  ...
  const answers = await inquirer.prompt([
    ...
    {type = 'general', importance = 0.5, sessionId = null): any {
  const sessionDir = './.claude/hive-mind';
  const dbPath = path.join(sessionDir, 'hive-mind.db');
  const db = new Database(dbPath);
 ...
 */
async function identifyMemoriesToDelete(db = 'SELECT id, key, type, importance, created_at FROM collective_memory WHERE ';
  const conditions = [];
  const params = [];
