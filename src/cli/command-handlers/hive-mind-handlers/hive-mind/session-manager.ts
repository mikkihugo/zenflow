...
import { createDatabase, isSQLiteAvailable } from '../../../../memory/sqlite-wrapper.js';

export class HiveMindSessionManager {
  constructor(hiveMindDir = null): any {
  ...
  initializeInMemoryFallback() {
    ...
    this.memoryStore = {sessions = this.db.prepare('PRAGMA table_info(sessions)').all();
      ...
      console.error('Migration error = {}): any {
   ...
   * Pause a session
   */
  async pauseSession(sessionId): any 
    await this.ensureInitialized();
   ...
   */
  async resumeSession(sessionId): any {
   ...
   */
  async archiveSessions(daysOld = 30): any {
   ...
   * Log session event
   */
  async logSessionEvent(sessionId, logLevel, message, agentId = null, data = null): any 
    await this.ensureInitialized();
   ...
   * Get session logs
   */
  getSessionLogs(sessionId, limit = 100, offset = 0): any {
    const stmt = this.db.prepare(`
      SELECT * FROM session_logs 
      WHERE session_id = ? 
      ORDER BY timestamp DESC 
   ...
   * Import session data
   */
  async importSession(importPath): any {
   ...
   */
  removeChildPid(sessionId, pid): any {
    const session = this.db.prepare('SELECT child_pids FROM sessions WHERE id = ?').get(sessionId);
    if (!session) return false;
   ...
   */
  async getChildPids(sessionId): any {
    await this.ensureInitialized();
   ...
   */
  async stopSession(sessionId): any {
