...
export async function startCommand(subArgs, flags) {
  ...
  const apiPort = flags['api-port'] || getArgValue(subArgs, '--api-port') || 3001;
  ...
  try {
    ...
    // Start Auto-Generated API server (always enabled now)
    try {
      const { AutoGeneratedAPI } = await import('../../api/auto-generated-api.js');
      const apiServer = new AutoGeneratedAPI({ port: apiPort });
      const apiResult = await apiServer.start();
      printSuccess(`🚀 Auto-Generated API started on port ${apiPort}`);
      printInfo(`📖 API Documentation: http://localhost:${apiPort}/docs`);
    } catch (error) {
      printWarning(`API server failed to start: ${error.message}`);
    }
