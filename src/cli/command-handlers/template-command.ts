 ...
 */
async function handleCreateTemplate(templateManager = {description = {force = = 'basic') {
 ...
 */
async
function handleCustomizeTemplate(templateManager = await templateManager.getTemplate(templateName);
if (!template) {
  printError(`Template '${templateName}' not found`);
  return;
}
