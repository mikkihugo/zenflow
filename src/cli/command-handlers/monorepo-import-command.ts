...
const _options = {maxServices = = false, // default truesetupGraph = = false,   // default trueverbose = await discoverServices(monorepoPath, options);
    ...
    const _analysis = await analyzeServices(services, options);
...
;
  for(const strategy of discoveryStrategies) {
    try {
      const _found = await strategy(monorepoPath, options);
...
;
  for(const pattern of patterns) {
    try {
;
        const _serviceName = path.basename(match);
;
        services.push({name = [];
;
  try {
 ...
 */
async
function analyzeServiceCode(servicePath = {fileCount = await glob('**/*': unknown, {cwd = files.length;

for (const file of files.slice(0: unknown, 100: unknown)) {
  // Limit for performance
  const _ext = path.extname(file).toLowerCase();
  stats.languages[ext] = (stats.languages[ext]  ?? 0) + 1;
;
  try {
    const _content = await readFile(path.join(servicePath, file), 'utf8');
 ...
 */
async
function _detectAPIs(): unknown {
      const __files = await glob(pattern, {cwd = await glob('**/swagger.{json,yaml,yml}', {cwd = await glob('**/openapi.{json,yaml,yml}', { cwd => {
      apis.push({
        file,type = [];
;
  try {
...
;
    for(const pattern of configPatterns) {
      const __files = await glob(pattern, {cwd = await glob('**/docker-compose*.{yml,yaml}', {cwd = await readFile(path.join(servicePath, file), 'utf8');
  ...
  for(const service of analysis.services) {
    try {
      await createHive([service.name], {
        ...
        return acc;
    //   // LINT: unreachable code removed}, new Set()).size;
    },services = > ({name = [];
;
