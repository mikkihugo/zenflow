 ...
 * Strategy1 = [];
  ...
  try {
    ...
    // Look for project.json files
      ...
      printWarning(`⚠️ Failed to analyzeservice = detectServicePatterns(analysis.services);
 ...
 */
async
function analyzeServiceCode(servicePath = {fileCount = await glob('**/*', {cwd = files.length;
 ...
 */
async
function detectAPIs(servicePath = [];
...
try {
    ...
    for(const pattern of patterns) {
      const _files = await glob(pattern, {cwd = await glob('**/swagger.{json,yaml,yml}', {cwd = await glob('**/openapi.{json,yaml,yml}', { cwd => {
      apis.push({
        file,type = [];
  ...
  try {
    ...
    for(const pattern of configPatterns) {
      const _files = await glob(pattern, {cwd = await glob('**/docker-compose*.{yml,yaml}', {cwd = await readFile(path.join(servicePath, file), 'utf8');
