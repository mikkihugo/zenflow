...
const _result = await TimeoutProtection.execRuvSwarmHookWithTimeout('pre-task', params);
;
...
const _result = await GitHubCliSafe.createPullRequestSafe({title = await GitHubCliSafe.execGhSafe(['repo', 'view'], {timeout = await agentLoader.getAgentType('analyst'); // Maps to 'code-analyzer'
...
const _result = await execRuvSwarmHook('pre-task', params);
;
...
;
// NEW (safe)
const _result = await GitHubCliSafe.createPullRequestSafe({ title, body });
\`\`\`
;
...
const _agentTypeInfo = await agentLoader.getAgentType('analyst'); // Returns code-analyzer
