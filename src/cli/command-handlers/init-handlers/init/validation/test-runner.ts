...
;
      ...
      const _forceValidation = await this.validationSystem.validatePreInit({force = forceValidation;
;
...
;
      ...
      const __rollbackPoints = await this.rollbackSystem.listRollbackPoints();
...
;
  ...
  const _checkpoint = await stateTracker.createCheckpoint('test-phase', {test = checkpoint;
;
...
;
  ...
  const _rollbackPoint = await stateTracker.recordRollbackPoint('test', {testData = rollbackPoint;
;
...
;
      ...
      const _genericRecovery = await recoveryManager.performRecovery('test-failure', {test = genericRecovery;
    } catch (/* error */) {
      result.success = false;
