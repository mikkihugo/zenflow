...
;
    for(const templatePath of this.templatePaths) {
      try {
        const _exists = await fs.access(templatePath).then(() => true).catch(() => false);
        if (!exists) continue;
;
        const __entries = await fs.readdir(templatePath, {withFileTypes = path.join(templatePath, entry.name);
            const _manifestPath = path.join(templateDir, 'template.json');
;
            try {
              const _manifestContent = await fs.readFile(manifestPath, 'utf8');
...
;
   ...
   */;
  async listTemplates() {
    const _templates = await this.discoverTemplates();
...
;
  ...
  async installTemplate(templateName, targetPath, options = {}): unknown {
    const _template = await this.getTemplate(templateName);
;
    if(!template) {
...
;
    ...
    try {
      const _targetStats = await fs.stat(absoluteTargetPath);
      if (targetStats.isDirectory()) {
        const _entries = await fs.readdir(absoluteTargetPath);
...
;
    try {
      const _entries = await fs.readdir(sourcePath, {withFileTypes = path.join(sourcePath, entry.name);
