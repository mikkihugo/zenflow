...
export class SwarmStopCommand extends BaseCommand {
  ...
  protected async validate(context: CommandContext): Promise<CommandValidationResult | null> {
    ...
    if (!swarmId && !stopAll) {
      warnings.push(
        'No swarm ID provided. Will stop the most recent swarm. Use --all to stop all swarms.'
      );
    }
  ...
  private async stopAllSwarms(
    ...
    for (const swarm of runningSwarms) {
      try {
        ...
        console.log(
          `❌ Failed to stop swarm '${swarm.id}': ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
  ...
  private async getRunningSwarms(): Promise<
    Array<{ id: string; createdAt: Date; agents: number }>
  > {
    try {
  ...
  private async gracefulStopSwarm(
    swarmId: string,
    timeout: number,
    killAgents: boolean
  ): Promise<void> {
