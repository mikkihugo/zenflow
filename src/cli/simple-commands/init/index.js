...
import { node, cwd, exit, existsSync } from '../../node-compat.js';
import process from 'process';
import { createLocalExecutable } from './executable-wrapper.js';
// SPARC structure removed - using Hive Mind system
...
export async function initCommand(subArgs, flags) {
  ...
  try {
    ...
    for (const file of files) {
      try {
        await node.stat(`${workingDir}/${file}`);
...
<<<<<<< HEAD
    ...
    if (!initDryRun) {
      await node.writeTextFile('CLAUDE.md', claudeMd);
    ...
    if (!initDryRun) {
      await node.writeTextFile('memory-bank.md', memoryBankMd);
    ...
    if (!initDryRun) {
      await node.writeTextFile('coordination.md', coordinationMd);
    ...
    if (initSparc && !initDryRun) {
      try {
        ...
        for (const [filename, content] of Object.entries(sparcTemplates)) {
          try {
            await node.writeTextFile(`${sparcTargetDir}/${filename}`, content);
        ...
        await node.writeTextFile(`${sparcTargetDir}/sparc-modes.md`, sparcModesOverview);
      ...
      try {
        ...
        for (const [filename, content] of Object.entries(swarmTemplates)) {
          try {
            await node.writeTextFile(`${swarmTargetDir}/${filename}`, content);
            console.log(`    ✓ Created ${filename}`);
          } catch (err) {
            console.log(`    ⚠️  Could not create ${filename}: ${err.message}`);
          }
      ...
      try {
        ...
        await node.writeTextFile(`${workingDir}/.claude/config.json`, JSON.stringify(configContent, null, 2));
    ...
    if (!initDryRun) {
      await node.writeTextFile('memory/agents/README.md', agentsReadme);
    ...
    if (!initDryRun) {
      await node.writeTextFile('memory/sessions/README.md', sessionsReadme);
...
<<<<<<< HEAD
    if (!initDryRun) {
      await node.writeTextFile('memory/claude-flow-data.json', JSON.stringify(initialData, null, 2));
    ...
    if (initSparc) {
      ...
      if (initDryRun) {
        ...
        try {
          ...
          const createSparcCommand = new node.Command('npx', {
...
<<<<<<< HEAD
  const workingDir = node.env.get('PWD') || node.cwd();
...
<<<<<<< HEAD
    ...
    await node.writeTextFile(`${workingDir}/CLAUDE.md`, claudeMd);
||||||| 47d5ef4
    const claudeMd = options.sparc ? createSparcClaudeMd() : 
                     options.minimal ? createMinimalClaudeMd() : createFullClaudeMd();
...
>>>>>>> origin/main
    ...
    await node.writeTextFile(`${workingDir}/memory-bank.md`, memoryBankMd);

<<<<<<< HEAD
    const coordinationMd = options.minimal ? createMinimalCoordinationMd() : createFullCoordinationMd();
    await node.writeTextFile(`${workingDir}/coordination.md`, coordinationMd);
||||||| 47d5ef4
    const coordinationMd = options.minimal ? createMinimalCoordinationMd() : createFullCoordinationMd();
...
async function createDirectoryStructure(workingDir, dryRun = false) {
  ...
  if (!dryRun) {
    for (const dir of directories) {
      await node.mkdir(`${workingDir}/${dir}`, { recursive: true });
    }
  }
}
...
<<<<<<< HEAD
    await node.writeTextFile(`${workingDir}/memory/claude-flow-data.json`, JSON.stringify(initialData, null, 2));
    
    await node.writeTextFile(`${workingDir}/memory/agents/README.md`, createAgentsReadme());
    await node.writeTextFile(`${workingDir}/memory/sessions/README.md`, createSessionsReadme());
||||||| 47d5ef4
...
async function enhancedClaudeFlowInit(flags, subArgs = []) {
  ...
  try {
    ...
    // Create CLAUDE.md
    if (!dryRun) {
      await node.writeTextFile(`${workingDir}/CLAUDE.md`, createEnhancedClaudeMd());
    ...
    if (!dryRun) {
      await node.mkdir(claudeDir, { recursive: true });
      await node.mkdir(`${claudeDir}/commands`, { recursive: true });
      await node.mkdir(`${claudeDir}/helpers`, { recursive: true });
    ...
    if (!dryRun) {
      await node.writeTextFile(`${claudeDir}/settings.json`, createEnhancedSettingsJson());
    ...
    if (!dryRun) {
<<<<<<< HEAD
      await node.writeTextFile(`${claudeDir}/settings.local.json`, JSON.stringify(settingsLocal, null, 2));
||||||| 47d5ef4
...
>>>>>>> origin/main
    ...
    if (!dryRun) {
      await node.writeTextFile(`${workingDir}/.mcp.json`, JSON.stringify(mcpConfig, null, 2));
    ...
    if (!dryRun) {
<<<<<<< HEAD
      await node.writeTextFile(`${workingDir}/claude-flow.config.json`, JSON.stringify(claudeFlowConfig, null, 2));
||||||| 47d5ef4
...
>>>>>>> origin/main
    ...
    for (const [category, commands] of Object.entries(COMMAND_STRUCTURE)) {
      ...
      if (!dryRun) {
<<<<<<< HEAD
        await node.mkdir(categoryDir, { recursive: true });
...
`;
<<<<<<< HEAD
        await node.writeTextFile(`${categoryDir}/README.md`, categoryReadme);
...
>>>>>>> origin/main
        ...
        for (const command of commands) {
          ...
          if (doc) {
            await node.writeTextFile(`${categoryDir}/${command}.md`, doc);
          }
        }
    ...
    if (!dryRun) {
      ...
      await node.writeTextFile(`${workingDir}/claude-flow`, unixWrapper);
      await fs.chmod(`${workingDir}/claude-flow`, 0o755);

      // Windows wrapper
<<<<<<< HEAD
      await node.writeTextFile(`${workingDir}/claude-flow.bat`, createWrapperScript('windows'));
...
>>>>>>> origin/main
      // PowerShell wrapper
<<<<<<< HEAD
      await node.writeTextFile(`${workingDir}/claude-flow.ps1`, createWrapperScript('powershell'));
...
>>>>>>> origin/main
    ...
    for (const helper of helpers) {
      if (!dryRun) {
        ...
        if (content) {
          await node.writeTextFile(`${claudeDir}/helpers/${helper}`, content);
