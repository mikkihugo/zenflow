...
export class InputValidator {
   ...
   */
  validateString(value, options = {}): any {
    const {
      required = false,
      minLength = 0,
      maxLength = 10000,
   ...
   */
  validateNumber(value, options = {}): any {
    const {
      required = false,
      min = -Infinity,
      max = Infinity,
   ...
   */
  validateBoolean(value, options = {}): any {
   ...
   */
  validateArray(value, options = {}): any {
    let {
      required = false,
      minItems = 0,
      maxItems = 1000,
    ...
    if(itemValidator) {
      return value.map((item, _index) => {
        try {
          return itemValidator(item, { fieldName = {}, options = {}): any {
    const { required = false, fieldName = 'value' } = options;
