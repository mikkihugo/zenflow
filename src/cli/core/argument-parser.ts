...
for (const i = 0; i < args._length; i++) {
  const _arg = args[i];
...
return {
    flags,
    positionalArgs,hasFlag = > name in flags,getFlag = null): _any => flags[name] ?? defaultValue,getBooleanFlag = > Boolean(flags[name]),requireFlag = null) => {
      if (!(_name in _flags)) {
...
if (args.length === 0) {
  ...
  const [command, _subcommand, ..._remainingArgs] = parsed.positionalArgs;
...
: T
{
  ...
  if (!validValues.includes(value)) {
    throw new ValidationError(
        errorMessage || `Flag --${name} must be oneof = null): string | null {
  ...
  public validateFlag(name = this.flags[name];
    ...
    if (options.required && (value === undefined || value === null)) {
      throw new ValidationError(options.errorMessage || `Flag --${name} is required`);
    }
      ...
      switch (options.type) {
        case 'string':
          if (typeof value !== 'string') {
            throw new ValidationError(options.errorMessage || `Flag --${name} must be a string`);
          }
          break;
        case 'number':
          if (isNaN(Number(value))) {
            throw new ValidationError(options.errorMessage || `Flag --${name} must be a number`);
          }
          break;
        case 'boolean':
          if (typeof value !== 'boolean') {
            throw new ValidationError(options.errorMessage || `Flag --${name} must be a boolean`);
          }
          break;
        case 'array':
          if (!Array.isArray(value)) {
            throw new ValidationError(options.errorMessage || `Flag --${name} must be an array`);
          }
          break;
      }
    }
    ...
    if (options.validValues && !options.validValues.includes(value)) {
      throw new ValidationError(
        options.errorMessage || `Flag --${name} must be oneof = === 'number')
    {
   ...
   */
  export function convertToStandardFormat(argv = parseCommandStructure(argv);
  const _normalizedFlags = normalizeFlags(structure.flags);
...
for (const [flagName, definition] of Object.entries(flagDefinitions)) {
  const typeInfo = definition.type ? ` (${definition.type})` : '';
  const defaultInfo = definition.default !== undefined ? ` [default: ${definition.default}]` : '';
