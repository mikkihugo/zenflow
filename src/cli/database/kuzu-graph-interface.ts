...
;
        const _queryResult = await this.executeQuery(query);
...
;
   ...
   */;
  async insertNodes(nodeType, nodes): unknown ;
...
;
    // Dependency patterns
    for(const service of services) {
      const _deps = await this.findServiceDependencies(service.properties.name);
...
;
    for(const rule of rules) {
      if(rule.type === 'circular') {
        ...
        const _circular = await this.findCircularDependencies();
...
;
        try {
          ...
          printWarning(`⚠️ Error getting query results = {}): unknown ;
    try {
