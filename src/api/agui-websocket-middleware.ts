...
export class AGUIWebSocketMiddleware {
  private options: AGUIWebSocketOptions;
  private clientAdapters = new Map<WebSocket, AGUIAdapter>();
  private sessions = new Map<string, ClientSession>();
  private globalAdapter: AGUIAdapter;
  private stats: AGUIStats;

  constructor(webSocketService: any, options: AGUIWebSocketOptions = {}) {
   ...
   * Setup global event forwarding
   */
  private setupGlobalEventForwarding(): void {
   ...
   */
  setupClient(ws: WebSocket, sessionId?: string): string {
   ...
   * Setup client message handling
   */
  private setupClientMessageHandling(ws: WebSocket, adapter: AGUIAdapter): void {
   ...
   */
  private handleClientMessage(ws: WebSocket, adapter: AGUIAdapter, message: any): void {
    switch (message.type) {
   ...
   */
  private sendToClient(ws: WebSocket, event: any): void {
