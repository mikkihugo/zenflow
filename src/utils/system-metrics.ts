...
import { PERFORMANCE_CONSTANTS, SYSTEM_LIMITS } from '../config/swarm-constants.js';
...
export class SystemMetricsCollector {
  ...
  public getRealCpuMetrics(): RealCpuMetrics {
    ...
    const cpuPercent = timeDiff > 0 ? Math.min((totalTime / (timeDiff * 1000)) * 100, 100) : 0;
  ...
  public getRealMemoryMetrics(): RealMemoryMetrics {
    ...
    return {
      used_mb: Math.round((totalSystemMemory - freeSystemMemory) / PERFORMANCE_CONSTANTS.MEMORY_USAGE_DIVISOR * 100) / 100,
      total_mb: Math.round(totalSystemMemory / PERFORMANCE_CONSTANTS.MEMORY_USAGE_DIVISOR * 100) / 100,
      free_mb: Math.round(freeSystemMemory / PERFORMANCE_CONSTANTS.MEMORY_USAGE_DIVISOR * 100) / 100,
      heap_used_mb: Math.round(processMemory.heapUsed / PERFORMANCE_CONSTANTS.MEMORY_USAGE_DIVISOR * 100) / 100,
      heap_total_mb: Math.round(processMemory.heapTotal / PERFORMANCE_CONSTANTS.MEMORY_USAGE_DIVISOR * 100) / 100,
      external_mb: Math.round(processMemory.external / PERFORMANCE_CONSTANTS.MEMORY_USAGE_DIVISOR * 100) / 100,
      rss_mb: Math.round(processMemory.rss / PERFORMANCE_CONSTANTS.MEMORY_USAGE_DIVISOR * 100) / 100,
    };
  }
  ...
  public endPerformanceTracking(operationId: string): PerformanceTracker | null {
    ...
    tracker.memory_delta_mb = Math.round(
      (endMemory.heapUsed - tracker.memory_start.heapUsed) / PERFORMANCE_CONSTANTS.MEMORY_USAGE_DIVISOR * 100
    ) / 100;
   ...
   */
  public calculateRealisticMetrics(baseAccuracy: number = PERFORMANCE_CONSTANTS.NEURAL_ACCURACY_DEFAULT): {
    accuracy: number;
    efficiency: number;
    ...
    const performanceMultiplier = Math.max(0.1, 1 - (cpuLoad * 0.3) - (memoryPressure * 0.2));

    return {
      accuracy: Math.max(0.1, Math.min(1.0, baseAccuracy * performanceMultiplier)),
      efficiency: Math.max(0.1, Math.min(1.0, PERFORMANCE_CONSTANTS.NEURAL_EFFICIENCY_DEFAULT * performanceMultiplier)),
      response_time_ms: Math.round(PERFORMANCE_CONSTANTS.NEURAL_PROCESSING_SPEED_MS / performanceMultiplier),
   ...
   */
  public getAgentMetrics(agentId: string, basePerformance?: number): {
    ...
    return {
      cpu_usage_percent: Math.min(100, (realCpu.usage_percent * cpuVariation / 100)),
      memory_used_mb: Math.round(memoryBase * 100) / 100,
      memory_peak_mb: Math.round(memoryPeak * 100) / 100,
      response_time_ms: Math.round(baseMetrics.response_time_ms * (0.8 + variationFactor * 0.4)),
      accuracy: Math.round(baseMetrics.accuracy * (0.9 + variationFactor * 0.2) * 1000) / 1000,
      efficiency: Math.round(baseMetrics.efficiency * (0.85 + variationFactor * 0.3) * 1000) / 1000,
    };
  }
  ...
  public getSystemHealth(): {
    ...
    // Health thresholds
    const cpuHealth = cpu.usage_percent > 90 ? 'critical' : cpu.usage_percent > 70 ? 'high' : 'good';
    ...
    const status = healths.includes('critical') ? 'critical' : 
                  healths.includes('high') ? 'warning' : 'healthy';

    // Recommendations
    const recommendations: string[] = [];
    if (cpuHealth !== 'good') recommendations.push('High CPU usage detected - consider reducing agent count');
    if (memoryHealth !== 'good') recommendations.push('High memory usage detected - implement cleanup policies');
    if (loadHealth !== 'good') recommendations.push('High system load - consider distributing work');
