  ...
  public async executeWithErrorHandling<T>(
    ...
    try {
      ...
      if (options.resilience) {
        ...
        if (options.resilience.errorBoundary) resilienceOptions.errorBoundary = options.resilience.errorBoundary;
        if (options.resilience.timeoutMs) resilienceOptions.timeoutMs = options.resilience.timeoutMs;
