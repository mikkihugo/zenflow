...
import { performGapAnalysis, generateComparisonReport, auditAutoAssignmentCapabilities } from '../coordination/agents/gap-analysis.js';
  ...
  try {
    switch (command) {
      ...
      case 'summary': {
        ...
        console.log(`🚀 Our Advantage: ${analysis.ourAdvantage} additional agents (${analysis.advantageRatio}x more)`);
        ...
        for (const [category, data] of topAdvantages) {
          console.log(`  • ${category}: +${data.advantage} agents (${data.ours} vs ${data.theirs})`);
        }
        console.log('');
        break;
      }
