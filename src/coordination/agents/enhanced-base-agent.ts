...
import { withUniversalFACT, type UniversalFACTCapabilities } from '../universal-fact-mixin.ts';
 ...
 */
export class EnhancedBaseAgent extends withUniversalFACT('Agent')(BaseAgent) 
  implements UniversalFACTCapabilities {
  
  protected hierarchyLevel: 'Agent' = 'Agent';

  constructor(config: AgentConfig) {
    super(config);
    logger.debug(`Enhanced Agent ${this.id} initialized with universal FACT access`);
  }
  ...
  public override async execute(task: any): Promise<any> {
    ...
    try {
      logger.debug(`Agent ${this.id} executing task with FACT integration: ${task.id}`);
      ...
      logger.debug(`✅ Agent ${this.id} completed task with FACT integration: ${task.id}`);
      return result;
    } catch (error) {
      logger.error(`❌ Agent ${this.id} task execution with FACT failed:`, error);
      throw error;
    }
  }
  ...
  private async gatherTaskKnowledge(task: any): Promise<void> {
    try {
      logger.debug(`Agent ${this.id} gathering FACT knowledge for task: ${task.id}`);

      const knowledgeQueries: string[] = [];

      // Extract knowledge requirements from task
      if (task.packages?.length > 0) {
        knowledgeQueries.push(...task.packages.map((pkg: string) => `npm-package:${pkg}`));
      }

      if (task.repositories?.length > 0) {
        knowledgeQueries.push(...task.repositories.map((repo: string) => `github-repo:${repo}`));
      }

      if (task.apis?.length > 0) {
        knowledgeQueries.push(...task.apis.map((api: string) => `api-docs:${api}`));
      }

      if (task.security?.length > 0) {
        knowledgeQueries.push(...task.security.map((cve: string) => `security-advisory:${cve}`));
      }
        ...
        try {
          ...
          switch (type) {
            ...
            case 'security-advisory':
              ...
              break;
            default:
              factResult = await this.searchSharedFacts({ query: subject, limit: 5 });
          }
      ...
      logger.debug(`✅ Agent ${this.id} gathered ${knowledgeResults.length} knowledge items`);
    } catch (error) {
  ...
  private async storeTaskInsights(task: any, result: any): Promise<void> {
    try {
      ...
      logger.debug(`Agent ${this.id} storing task insights to shared FACT system`);
      ...
      logger.debug(`✅ Agent ${this.id} stored ${insights.length} insights to shared FACT system`);
    } catch (error) {
   ...
   */
  private extractTaskInsights(task: any, result: any): Array<{
    subject: string;
    content: unknown;
    confidence?: number;
  }> {
    const insights: Array<{ subject: string; content: unknown; confidence?: number }> = [];
    ...
    try {
      ...
      if (this.config.memory && typeof this.config.memory === 'object' && 'shortTerm' in this.config.memory) {
        ...
        const accumulatedKnowledge = Array.from(memory.shortTerm.entries())
          .filter(([key]) => key.startsWith('knowledge_') || key.startsWith('task_knowledge_'))
        ...
        if (accumulatedKnowledge.length > 0) {
          ...
          logger.debug(`✅ Agent ${this.id} stored session knowledge before shutdown`);
        }
      }
    } catch (error) {
...
export class EnhancedResearcherAgent extends EnhancedBaseAgent {
  ...
  protected override async executeTaskByType(task: any): Promise<unknown> {
...
export function createEnhancedAgent(config: AgentConfig): EnhancedBaseAgent {
  switch (config?.type) {
    ...
    case 'coder':
      return new EnhancedCoderAgent(config);
    case 'analyst':
