...
export class DefaultEnhancedHookManager implements EnhancedHookManager {
  ...
  async executeHooks(trigger: HookTrigger, context: HookContext): Promise<HookResult[]> {
    ...
    this.logger.debug('Hook execution completed', {
      ...
      averageHookTime: totalDuration / Math.max(results.length, 1)
    });
...
class ContextLoader {
  ...
  private async loadSessionHistory(_sessionId: string): Promise<any[]> {
    // Mock implementation
    return [];
  }
  ...
  private async loadUserPreferences(_userId?: string): Promise<Record<string, any>> {
    // Mock implementation
    return {
      theme: 'dark',
      formatter: 'prettier',
  ...
  private async loadProjectContext(_workingDirectory: string): Promise<Record<string, any>> {
    // Mock implementation
    return {
