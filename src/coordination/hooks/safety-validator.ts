...
export class BashSafetyValidator implements SafetyValidator {
  private readonly DANGEROUS_PATTERNS: Array<{ pattern: RegExp; type: string; severity: RiskLevel; description: string; mitigation?: string }> = [
    {
      pattern: /rm\s+-rf\s+\//,
  ...
  private readonly HIGH_RISK_COMMANDS: Array<{ command: string; description: string; alternatives: string[] }> = [
    {
      command: 'dd',
...
export class FileOperationValidator {
  ...
  async validateFileAccess(path: string, operation: 'read' | 'write' | 'execute'): Promise<ValidationResult> {
    const risks: SecurityRisk[] = [];
    ...
    return {
      ...
      riskLevel,
      risks,
      reason: riskLevel === 'CRITICAL' ? 'Access denied due to critical security risks' : 'File access evaluated',
...
export class SecurityRiskAssessment {
  ...
  static categorizeRisks(risks: SecurityRisk[]): { critical: SecurityRisk[]; high: SecurityRisk[]; medium: SecurityRisk[]; low: SecurityRisk[] } {
    return {
  ...
  static generateSecurityReport(risks: SecurityRisk[]): string {
    ...
    const categorized = this.categorizeRisks(risks);
