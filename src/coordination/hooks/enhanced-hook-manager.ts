...
import { BashSafetyValidator, FileOperationValidator } from './safety-validator.js';
import { IntelligentAgentAssignor } from './auto-agent-assignment.js';
import { HookPerformanceTracker, OperationPerformanceOptimizer } from './performance-tracker.js';
...
export class DefaultEnhancedHookManager implements EnhancedHookManager {
  ...
  async executeHooks(trigger: HookTrigger, context: HookContext): Promise<HookResult[]> {
    ...
    for (const hook of enabledHooks) {
      try {
        ...
        if (!result.allowed) {
          console.warn(`Hook ${hook.id} blocked operation: ${result.errors.map(e => e.message).join(', ')}`);
          break;
        }
  ...
  private async executeHookWithTimeout(hook: EnhancedHook, context: HookContext, timeout: number): Promise<HookResult> {
    return Promise.race([
