...
import { getUniversalFACTAccess, type UniversalFACTAccess } from './shared-fact-access.ts';
 ...
 */
export function withUniversalFACT<T extends new (...args: any[]) => object>(
    ...
    return class extends BaseClass implements UniversalFACTCapabilities {
      ...
      public async getSharedFACTSystem(): Promise<CollectiveFACTSystem> {
        if (!this._sharedFactSystem) {
          logger.debug(`${hierarchyLevel} initializing shared FACT system access`);
      ...
      public async searchSharedFacts(query: {
        ...
        try {
          ...
          logger.debug(`${hierarchyLevel} searched shared facts: ${results.length} results`);
          return results;
        } catch (error) {
      ...
      public async storeSharedFact(fact: {
        ...
        try {
          ...
          logger.debug(`✅ ${hierarchyLevel} stored shared fact: ${fact.subject}`);
        } catch (error) {
       ...
       */
      public async getSharedNPMFacts(packageName: string, version?: string): Promise<unknown> {
        try {
          const factSystem = await this.getSharedFACTSystem();
          const result = await factSystem.getNPMPackageFacts(packageName, version);
          
          logger.debug(`✅ ${hierarchyLevel} retrieved NPM facts for: ${packageName}`);
       ...
       */
      public async getSharedGitHubFacts(owner: string, repo: string): Promise<unknown> {
        try {
          const factSystem = await this.getSharedFACTSystem();
          const result = await factSystem.getGitHubRepoFacts(owner, repo);
          
          logger.debug(`✅ ${hierarchyLevel} retrieved GitHub facts for: ${owner}/${repo}`);
          return result.content;
        } catch (error) {
          logger.error(`${hierarchyLevel} GitHub fact retrieval failed:`, error);
          throw error;
        }
      }
       ...
       */
      public async getSharedAPIFacts(api: string, endpoint?: string): Promise<unknown> {
        try {
          const factSystem = await this.getSharedFACTSystem();
      ...
      public async getSharedSecurityFacts(cve: string): Promise<unknown> {
        try {
          ...
          logger.debug(`✅ ${hierarchyLevel} retrieved security facts for: ${cve}`);
          return result.content;
        } catch (error) {
          logger.error(`${hierarchyLevel} security fact retrieval failed:`, error);
          throw error;
        }
      }
    };
...
export abstract class UniversalFACTBase implements UniversalFACTCapabilities {
  private _sharedFactSystem: CollectiveFACTSystem | null = null;
  protected abstract hierarchyLevel: 'Cube' | 'Matron' | 'Queen' | 'SwarmCommander' | 'Agent';
  
  public async getSharedFACTSystem(): Promise<CollectiveFACTSystem> {
    if (!this._sharedFactSystem) {
      logger.debug(`${this.hierarchyLevel} initializing shared FACT system access`);
  ...
  public async searchSharedFacts(query: unknown): Promise<unknown[]> {
    try {
      ...
      logger.debug(`${this.hierarchyLevel} searched shared facts: ${results.length} results`);
      return results;
    } catch (error) {
  ...
  public async getSharedNPMFacts(packageName: string, version?: string): Promise<unknown> {
    try {
      ...
      logger.debug(`✅ ${this.hierarchyLevel} retrieved NPM facts for: ${packageName}`);
      return result.content;
    } catch (error) {
  ...
  public async getSharedGitHubFacts(owner: string, repo: string): Promise<unknown> {
    try {
      ...
      logger.debug(`✅ ${this.hierarchyLevel} retrieved GitHub facts for: ${owner}/${repo}`);
      return result.content;
    } catch (error) {
      logger.error(`${this.hierarchyLevel} GitHub fact retrieval failed:`, error);
      throw error;
    }
  }
  ...
  public async getSharedSecurityFacts(cve: string): Promise<unknown> {
    try {
      ...
      logger.debug(`✅ ${this.hierarchyLevel} retrieved security facts for: ${cve}`);
      return result.content;
    } catch (error) {
      logger.error(`${this.hierarchyLevel} security fact retrieval failed:`, error);
      throw error;
    }
  }
}
