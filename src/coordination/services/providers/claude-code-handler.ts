...
import { runClaudeCode, filterMessagesForClaudeCode } from "../../../integrations/claude-code/index.js";
...
export function filterMessagesForClaudeCode(messages: Anthropic.Messages.MessageParam[]): Anthropic.Messages.MessageParam[] {
    return messages.map(message => {
        if (Array.isArray(message.content)) {
            ...
            return {
                ...message,
                content: textContent.length > 0 ? textContent : [{ type: 'text' as const, text: 'Empty message' }]
            };
        }
        return message;
    });
