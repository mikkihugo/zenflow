...
class ClaudeFlowEnhanced {
  ...
  async executeWorkflow(workflowId, context = {}) {
    ...
    try {
      ...
      for (const [_batchIndex, batch] of batches.entries()) {
        if (batch.length === 1) {
  ...
  async executeStepsBatch(steps, context, swarm) {
    ...
    // Log execution summary for claude-flow monitoring
    this.logger.info('Claude Flow workflow steps completed', {
    ...
    this.ruvSwarm.emit('claude-flow:workflow-completed', {
      stepCount: results.length,
      completed,
      failed,
      performance: {
        totalSteps: results.length,
        successRate: completed / results.length,
        failures: failed,
      },
    });
  ...
  async executeStep(step, context, swarm) {
    ...
    try {
      ...
      // Log execution failure with timing for performance analysis
      this.logger.error('Claude Flow step execution failed', {
      ...
      throw new ClaudeFlowError(
        ...
        { executionTime, stepType: step.type }
      );
    }
  }
