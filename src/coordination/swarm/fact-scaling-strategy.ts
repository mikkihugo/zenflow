...
export class FACTScalingStrategy {
   ...
   */
  async createDownloadStrategy(packageJson: unknown): Promise<{
    immediate: string[];
    queued: string[];
    estimated: {
    ...
    devDeps.forEach((pkg) => {
      if (!packagePriorities.has(pkg)) {
        const level = popularDevPackages.some((popular) =>
          pkg.includes(popular)
        )
          ? 'medium'
          : 'low';
        packagePriorities.set(pkg, {
    ...
    const result = {
      ...
      estimated: {
        immediateTime: this.estimateDownloadTime(
          this.downloadQueue.immediate.length
        ),
        totalTime: this.estimateDownloadTime(
          this.downloadQueue.immediate.length + estimatedTransitive
        ),
        totalPackages:
          this.downloadQueue.immediate.length + estimatedTransitive,
      },
    ...
    console.log(
      `   Immediate: ${result.immediate.length} packages (${result.estimated.immediateTime})`
    );
    console.log(`   Queued: ${result.queued.length} packages`);
    console.log(
      `   Estimated total: ${result.estimated.totalPackages} packages (${result.estimated.totalTime})`
    );
  ...
  async executeImmediateDownloads(packages: string[]): Promise<{
    ...
    console.log(
      `🚀 Starting immediate download of ${packages.length} critical packages...`
    );
    ...
    const limitedPackages = packages.slice(
      0,
      this.config.maxImmediateDownloads
    );
    if (packages.length > this.config.maxImmediateDownloads) {
      console.log(
        `⚠️  Limited to ${this.config.maxImmediateDownloads} immediate downloads`
      );
      // Move remainder to high priority queue
      this.downloadQueue.high.unshift(
        ...packages.slice(this.config.maxImmediateDownloads)
      );
    }
    ...
    for (const pkg of limitedPackages) {
      ...
      try {
        ...
        if (this.circuitBreaker.failures >= 3) {
          ...
          console.log(
            `🔴 Circuit breaker activated after ${this.circuitBreaker.failures} failures`
          );
        }
      }
    }
  ...
  getSystemStatus(): {
    ...
    const totalQueued = Object.values(this.downloadQueue).reduce(
  ...
  private async analyzeSecurityRisks(
    priorities: Map<string, PackagePriority>
  ): Promise<void> {
  ...
  private async analyzePopularity(
    priorities: Map<string, PackagePriority>
  ): Promise<void> {
  ...
  private getFromCache(packageName: string): Record<string, unknown> | null {
  ...
  private storeInCache(packageName: string, data: unknown): void {
  ...
  private async downloadSinglePackage(packageName: string): Promise<unknown> {
    try {
      // Real NPM API call
      ...
      console.log(
        `✅ Processed ${packageName} into FACT knowledge (${factKnowledge.snippets?.length || 0} snippets, ${factKnowledge.examples?.length || 0} examples)`
      );
   ...
   */
  private async transformNpmToFACT(packageData: unknown): Promise<unknown> {
    ...
    return {
      ...
      bestPractices,
      dependencies,
      tags: ['npm', 'nodejs', ...(packageData.keywords || []).slice(0, 5)],
      githubSources,
      source: 'npm-registry',
      ...
      metadata: {
        ...
        devDependencyCount: Object.keys(versionInfo.devDependencies || {})
          .length,
        hasTypescript: Boolean(versionInfo.types || versionInfo.typings),
        isPopular:
          packageData.time &&
          Object.keys(packageData.versions || {}).length > 10,
  ...
  private extractUsageExamples(
    ...
    if (packageData.readme) {
      const readmeSnippets = this.parseReadmeForExamples(
        packageData.readme,
        packageData.name
      );
  ...
  private extractBestPractices(
    ...
    practices.push({
      ...
      rationale:
        'Prevents unexpected breaking changes in production deployments',
    ...
    if (
      ...
      practices.push({
        ...
        rationale:
          'Keeps security vulnerabilities patched and dependencies current',
  ...
  private extractGitHubSources(
    ...
    if (versionInfo.repository?.url) {
      const repoUrl = versionInfo.repository.url
        .replace(/^git\+/, '')
        .replace(/\.git$/, '');
  ...
  private parseReadmeForExamples(
    readme: string,
    packageName: string
  ): unknown[] {
    const examples = [];
   ...
   */
  private async storeInFACTDatabase(factKnowledge: unknown): Promise<void> {
    console.log(
      `🔧 STORAGE DEBUG: Attempting to store FACT for ${factKnowledge.tool}`
    );
    console.log(
      `🔧 STORAGE DEBUG: Current working directory: ${process.cwd()}`
    );
    ...
    try {
      ...
      console.log(
        `💾 ✅ STORAGE SUCCESS: Stored FACT knowledge for ${factKnowledge.tool}@${factKnowledge.version} in ${filename} (${stats.size} bytes)`
      );
    } catch (error) {
      // Don't fail the entire operation if storage fails
      console.error(
        `❌ STORAGE ERROR: Failed to store FACT knowledge for ${factKnowledge.tool}: ${error.message}`
      );
   ...
   */
  private async saveFACTFile(factKnowledge: unknown): Promise<void> {
    try {
      const fs = await import('fs/promises');
      const path = await import('path');
  ...
  private async processQueue(queueName: keyof DownloadQueue): Promise<void> {
    ...
    console.log(
      `🔄 Processing ${queueName} priority queue (${queue.length} packages)...`
    );
