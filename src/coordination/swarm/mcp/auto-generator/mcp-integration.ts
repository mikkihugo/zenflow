...
export class AutoGeneratedToolsManager {
   ...
   */
  private async validateInput(toolName: string, args: unknown): Promise<void> {
   ...
   */
  private validateType(value: unknown, schema: unknown): boolean {
    switch (schema.type) {
      case 'string':
  ...
  for (const tool of tools) {
    try {
      ...
      if (typeof mcpServer.registerTool === 'function') {
        mcpServer.registerTool(tool, async (args: unknown) => {
