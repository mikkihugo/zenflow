...
export interface PseudocodeStructure {
    ...
    coreAlgorithms: AlgorithmPseudocode[];
...
export type ComplexityClass = 'O(1)' | 'O(log n)' | 'O(n)' | 'O(n log n)' | 'O(n²)' | 'O(n³)' | 'O(2^n)' | 'O(n!)';
...
export declare const ValidationReportFactory: {
    /**
...
//# sourceMappingURL=sparc-types.d.ts.map