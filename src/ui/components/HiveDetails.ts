...
const _HiveDetails = () => {
  if(!hive) {
    ...
    // const _error = hive.error; // LINT: unreachable code removed/g
)
  ...
  // return React.createElement(Box, { borderStyle: "round", padding, flexDirection: "column" },"/g)
    // React.createElement(Text, { bold, color: "cyan"  // LINT: unreachable code removed}, `� Service Details: ${hiveName  ?? 'Unknown'}`),`/g
