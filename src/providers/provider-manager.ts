...
private
config = {loadBalancing = { ...this.config, ...config };
...
if (cached) {
  ...
  try {
    const result = await Promise.race([this.executeRequest(request), timeoutPromise]);
  ...
  private
  async;
  selectProvider(request = Array.from(this.providers.values()).filter(p => 
      p.config.enabled && 
  ...
  switch (this.config.loadBalancing.type) {
    ...
    default = await this.selectProvider(request);
