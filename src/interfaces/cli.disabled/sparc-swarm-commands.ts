...
class SPARCSwarmCoordinator {
  ...
  getActiveSPARCTasks(): Array<{
    phaseProgress: Record<string, { status: string }>;
  }> {
    return [];
  }
}
      ...
      try {
        ...
        if (activeTasks.length === 0) {
          ...
          activeTasks.forEach((task) => {
            if (options?.detailed) {
              Object.entries(task.phaseProgress).forEach(
                ([_phase, progress]) => {
      ...
      try {
        ...
        const complexityExamples = {
          simple: {
            ...
            description:
              'Build a React component to display user profile information',
            acceptance_criteria: [
              'Display user name',
        ...
        const example =
          complexityExamples[
            options?.complexity as keyof typeof complexityExamples
          ];
    ...
    .action(async (options) => {
      try {
        ...
        const validationResults =
          await engine.validateArchitecture(architecture);
      ...
      try {
        ...
        if (options.format === 'markdown') {
          const markdownContent =
            generateImplementationPlanMarkdown(implementationPlan);
          const mdPath = outputPath.replace(/\.json$/, '.md');
          await fs.writeFile(mdPath, markdownContent, 'utf8');
        } else {
          await fs.writeFile(
