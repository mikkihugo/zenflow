...
import type { TerminalMode } from '../shared/command-interfaces.js';
import {
  CommandExecutionRendererAdapter,
  InteractiveTerminalApplicationAdapter,
  ModeDetectorAdapter,
} from './adapters/terminal-adapters.js';
...
export const TerminalApp: React.FC<TerminalAppProps> = ({
  ...
  const executeAdvancedCommand = async (
    ...
    try {
      ...
      const command = state.cliEngine
        .getCommandRegistry()
        .getCommand(commandName as string);
  ...
  const generateHelpContent = () => {
    ...
    return {
      ...
      examples: [
        {
          description:
            'Create AI-optimized project with neural network capabilities',
          command:
