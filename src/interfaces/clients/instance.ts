...
import { ClientManager, ClientManagerConfig } from './manager.ts';
...
export const UACLHelpers = {
   ...
   */
  getQuickStatus(): { status: string; initialized: boolean; clientCount: number } {
   ...
   */
  async performHealthCheck(): Promise<Array<{ component: string; status: string; details?: any }>> {
    const results: Array<{ component: string; status: string; details?: any }> = [];
