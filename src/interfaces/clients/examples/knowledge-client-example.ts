...
export async function example3_PerformQueries(): Promise<void> {
  ...
  try {
    const knowledgeClient = await createFACTClient('./FACT', process.env.ANTHROPIC_API_KEY || 'test');
    await knowledgeClient.connect();
...
export async function example6_ErrorHandling(): Promise<void> {
  ...
  try {
    ...
    const isHealthy = await knowledgeClient.health();
    console.log(`🏥 Health check with invalid config: ${isHealthy ? 'Healthy' : 'Unhealthy (Expected)'}`);
    ...
    console.log('✅ Expected error during client creation:', error instanceof Error ? error.message : 'Unknown error');
    console.log('✅ Example 6 completed successfully\n');
  }
}
