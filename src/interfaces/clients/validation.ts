...
import { uacl, ClientType, UACLHelpers, type ClientInstance } from './index';
import { createCompatibleAPIClient, createCompatibleWebSocketClient, createCompatibleKnowledgeClient, createCompatibleMCPClient } from './compatibility';
...
export class UACLValidator {
  ...
  private async validateCore(): Promise<ValidationResult[]> {
    ...
    try {
      ...
      results.push({
        ...
        status: health.overall === 'critical' ? 'fail' : health.overall === 'warning' ? 'warning' : 'pass',
  ...
  private async validateFactories(): Promise<ValidationResult[]> {
    ...
    const clientTypes = [ClientType.HTTP, ClientType.WEBSOCKET, ClientType.KNOWLEDGE, ClientType.MCP];
