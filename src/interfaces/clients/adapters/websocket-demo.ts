...
export async function enhancedWebSocketExample() {
  ...
  for (const [name, client] of [['Legacy', legacyClient], ['UACL', uaclClient]] as const) {
    console.log(`\n--- ${name} Client ---`);
...
export async function loadBalancedWebSocketExample() {
  ...
  const configs = [
    ...
    { ...WebSocketClientPresets.HighPerformance('wss://ws.postman-echo.com/raw'), name: 'postman-1' }
  ];
