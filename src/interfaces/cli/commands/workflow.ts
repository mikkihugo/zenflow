...
import { logSystemInfo, getSystemInfo } from '../../../config/system-info.ts';
...
export const testWorkflowCommand: WorkflowCommand = {
  ...
  async handler(input: string[], flags: any) {
    ...
    const validation = await import('../../../config/system-info.ts')
      .then(module => module.validateConfigForSystem(testConfig));
