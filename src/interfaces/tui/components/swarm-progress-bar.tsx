  ...
  return (
    <Box flexDirection="column">
      ...
      <Box>
        ...
        {showPercentage && <Text color={color}> {percentage.toFixed(1)}%</Text>}
        ...
        <Box flexDirection="column" marginTop={1}>
          ...
          {agentContributions.slice(0, 5).map((agent) => {
            ...
            return (
              <Box key={agent.id}>
                <Text dimColor>
                  {agentStatusIcon} {agent.id}: {agent.progress} tasks (
          ...
          {agentContributions.length > 5 && (
            <Text dimColor>... and {agentContributions.length - 5} more agents</Text>
          )}
        </Box>
      )}
    </Box>
