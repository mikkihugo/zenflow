...
import { type SwarmAgent, SwarmMetrics, type SwarmStatus, type SwarmTask } from '../types';
...
export class SwarmCoordinationUtils {
  static getOptimalTopology(
    agentCount: number,
    taskComplexity: 'low' | 'medium' | 'high'
  ): SwarmStatus['topology'] {
  ...
  static calculateCoordinationEfficiency(
    agents: SwarmAgent[],
    tasks: SwarmTask[]
  ): {
    efficiency: number;
    ...
    const efficiency =
      taskMetrics.successRate * 0.5 +
      utilization * 0.3 +
      Math.min(1, taskMetrics.completed / Math.max(1, taskMetrics.completed + taskMetrics.failed)) *
        0.2;
