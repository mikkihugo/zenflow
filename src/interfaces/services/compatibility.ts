...
import { USL, usl, initializeUSL } from './index';
...
export class USLCompatibilityLayer {
  ...
  async createSessionRecoveryService(name: string, options?: any): Promise<IService> {
    ...
    // Legacy fallback
    return await usl.createCoordinationService(name, { type: ServiceType.SESSION_RECOVERY, ...options });
  }

  /**
   * @deprecated Use serviceManager.createArchitectureStorageService() instead
   */
  async createArchitectureStorageService(name: string, dbType?: string, options?: any): Promise<IService> {
   ...
   */
  async migrateServiceDiscovery(existingServiceReferences: Array<{
    name: string;
  ...
  private detectServiceType(serviceInstance: any): ServiceType | null {
    ...
    if (serviceInstance.isCoordinationService || serviceInstance.constructor?.name?.includes('Coordination')) {
      return ServiceType.COORDINATION;
    }
    
    if (serviceInstance.isIntegrationService || serviceInstance.constructor?.name?.includes('Integration')) {
      return ServiceType.API;
    }
    
    if (serviceInstance.isInfrastructureService || serviceInstance.constructor?.name?.includes('Infrastructure')) {
  ...
  private canAutoMigratePattern(pattern: string): boolean {
    return this.config.legacyPatterns.some(lp => 
      pattern.includes(lp.pattern) && lp.autoMigration
    );
  }
 ...
 */
export const initializeCompatibility = async (config?: Partial<CompatibilityConfig>): Promise<void> => {
