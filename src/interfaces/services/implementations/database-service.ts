...
export class DatabaseService extends BaseService implements IService {
  ...
  private async executeTransaction(queries: Array<{ sql: string; parameters?: any[] }>): Promise<any> {
  ...
  private async performBackup(): Promise<any> {
    ...
    const backup = {
      ...
      path: config.backup.path ? `${config.backup.path}/${backupId}.sql` : `./backups/${backupId}.sql`,
      status: 'completed'
    };
