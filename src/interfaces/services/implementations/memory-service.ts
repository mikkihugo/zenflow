...
export class MemoryService extends BaseService implements IService {
  ...
  protected async doHealthCheck(): Promise<boolean> {
    try {
      ...
      if (config.storage?.maxMemory) {
        ...
        if (currentUsage > config.storage.maxMemory * 1.1) { // Allow 10% overage
      ...
      if (config.eviction?.maxSize) {
        if (this.store.size > config.eviction.maxSize * 1.1) { // Allow 10% overage
  ...
  private countExpiredKeys(): number {
    const now = Date.now();
    return Array.from(this.metadata.values())
      .filter(meta => meta.ttl && now > meta.ttl).length;
  }
