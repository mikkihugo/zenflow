...
export class SparcHybridTools {
  ...
  private async initializeAsync(): Promise<void> {
    try {
      ...
      logger.info('✅ SPARC Hybrid Tools initialized with production databases');
    } catch (error) {
  ...
  getToolDefinitions(): MCPToolDefinition[] {
    return [
      ...
      {
        name: 'adr_create',
        description: 'Create a new Architecture Decision Record with semantic indexing and relationship detection',
        inputSchema: {
          type: 'object',
          properties: {
            title: {
            ...
            stakeholders: {
              type: 'array',
              items: { type: 'string' },
              description: 'Stakeholders involved in or affected by this decision'
            },
...

      {
        ...
        description: 'Search ADRs using semantic similarity and relationship analysis',
        inputSchema: {
          type: 'object',
          properties: {
            query: {
...

      {
        ...
        description: 'Generate a relationship map showing how ADRs influence each other',
        inputSchema: {
          type: 'object',
          properties: {
            adr_id: {
              type: 'string',
              description: 'ADR ID to analyze relationships for (optional - if not provided, analyzes all)'
            },
            max_depth: {
              type: 'number',
              minimum: 1,
            ...
            relationship_types: {
              ...
              items: {
                type: 'string',
                enum: ['generates', 'implements', 'depends_on', 'relates_to', 'supersedes']
              },
      ...
      {
        name: 'adr_stats',
        description: 'Get comprehensive ADR statistics and analytics including semantic clusters',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      // Hybrid Document Search Tools
      {
        ...
        description: 'Search all documents (Vision, ADR, PRD, Epic, Feature, Task) using hybrid semantic + graph search',
        inputSchema: {
          type: 'object',
          properties: {
            query: {
            ...
            semantic_weight: {
              ...
              maximum: 1.0,
              default: 0.7,
              description: 'Weight for semantic vs graph search (0=graph only, 1=semantic only)'
            },
            max_results: {
              type: 'number',
              minimum: 1,
      ...
      // SPARC Project Management Tools
      {
        ...
        description: 'Initialize a new SPARC methodology project with Vision → ADR → PRD → Epic → Feature → Task workflow',
        inputSchema: {
          type: 'object',
          properties: {
            project_name: {
...

      {
        ...
        description: 'Get status of SPARC workflow progression for a project or document',
        inputSchema: {
          type: 'object',
          properties: {
            project_id: {
      ...
      // Decision Impact Analysis Tools
      {
        ...
        description: 'Analyze the impact of architectural decisions across the entire project ecosystem',
        inputSchema: {
          type: 'object',
          properties: {
            decision_id: {
      ...
      // Document Generation Tools
      {
        ...
        description: 'Generate semantic relationships between documents based on content similarity',
        inputSchema: {
          type: 'object',
          properties: {
            document_id: {
  ...
  private async getADRRelationshipMap(args: any): Promise<any> {
    const relationships = args.adr_id 
      ? await this.hybridManager.getDocumentRelationships(args.adr_id, args.max_depth || 2)
  ...
  private async analyzeDecisionImpact(args: any): Promise<any> {
    ...
    return {
      ...
      data: {
        ...
        direct_impacts: relationships.filter(r => r.source_document_id === args.decision_id),
        influenced_by: relationships.filter(r => r.target_document_id === args.decision_id),
