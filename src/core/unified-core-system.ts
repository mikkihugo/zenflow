...
import { UnifiedDocumentationLinker } from './unified-documentation-linker';
import { UnifiedExportSystem } from './unified-export-system';
import { UnifiedInterfaceLauncher } from './unified-interface-launcher';
import { UnifiedWorkflowEngine } from './unified-workflow-engine';
...
export class UnifiedCoreSystem extends EventEmitter {
  ...
  private exportSystem: UnifiedExportSystem;
  ...
  constructor(config: UnifiedCoreConfig = {}) {
    super();
    this.config = config;
  ...
  private initializeComponents(): void {
    ...
    // Export system
    this.exportSystem = new UnifiedExportSystem();
    ...
    // Interface launcher
    this.interfaceLauncher = UnifiedInterfaceLauncher.getInstance();
   ...
   */
  static async create(config?: UnifiedCoreConfig): Promise<UnifiedCoreSystem> {
    const system = new UnifiedCoreSystem(config);
   ...
   */
  static async quickStart(config?: UnifiedCoreConfig): Promise<UnifiedCoreSystem> {
    const system = await UnifiedCoreSystem.create(config);
    await system.launch();
    return system;
  }
}
