...
export class UnifiedMemorySystem extends EventEmitter {
  ...
  async storeDocument(type: string, id: string, document: any, namespace: string = 'documents'): Promise<StorageResult> {
    const key = `${type}:${id}`;
  ...
  async searchDocuments(type: string, namespace: string = 'documents'): Promise<Record<string, any>> {
    const pattern = `${type}:*`;
