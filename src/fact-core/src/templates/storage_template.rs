...
use crate::storage::{create_storage, FactKey, FactData, StorageConfig, StorageBackend};
...
pub fn create_tool_knowledge_storage_template() -> Template {
    Template {
        ...
        description: "Store and retrieve tool knowledge data in FACT database".to_string(),
        steps: vec![
            TemplateStep {
        ...
        metadata: super::TemplateMetadata {
            ...
            tags: vec!["storage".to_string(), "knowledge".to_string(), "tool".to_string()],
...
pub async fn process_tool_knowledge_storage(context: Value) -> Result<Value> {
    ...
    let ecosystem = context.get("ecosystem")
        .unwrap_or(&Value::String("unknown".to_string()))
        .as_str()
        .unwrap_or("unknown");

    debug!("Storing knowledge for {}@{} in {} ecosystem", tool, version, ecosystem);
    ...
    let fact_key = FactKey::new(tool.to_string(), version.to_string(), ecosystem.to_string());
    
    match storage.store_fact(&fact_key, &fact_data).await {
        Ok(()) => {
            info!("✅ Successfully stored FACT knowledge for {}@{}", tool, version);
        ...
        Err(e) => {
            warn!("Failed to store FACT knowledge for {}@{}: {}", tool, version, e);
...
fn parse_snippets(context: &Value) -> Result<Vec<crate::storage::FactSnippet>> {
    ...
    if let Some(snippets_array) = context.get("snippets").and_then(|v| v.as_array()) {
...
fn parse_examples(context: &Value) -> Result<Vec<crate::storage::FactExample>> {
    ...
    if let Some(examples_array) = context.get("examples").and_then(|v| v.as_array()) {
...
fn parse_best_practices(context: &Value) -> Result<Vec<crate::storage::FactBestPractice>> {
    let mut practices = Vec::new();
    
    if let Some(practices_array) = context.get("best_practices").and_then(|v| v.as_array()) {
        for practice in practices_array {
            if let (Some(practice_text), Some(rationale)) = (
                ...
                practices.push(crate::storage::FactBestPractice {
                    ...
                    example: practice.get("example").and_then(|v| v.as_str()).map(|s| s.to_string()),
                });
            }
        }
    }
...
fn parse_troubleshooting(context: &Value) -> Result<Vec<crate::storage::FactTroubleshooting>> {
    let mut troubleshooting = Vec::new();
    
    if let Some(troubleshooting_array) = context.get("troubleshooting").and_then(|v| v.as_array()) {
...
fn parse_github_sources(context: &Value) -> Result<Vec<crate::storage::FactGitHubSource>> {
    let mut sources = Vec::new();
    
    if let Some(sources_array) = context.get("github_sources").and_then(|v| v.as_array()) {
...
mod tests {
    ...
    async fn test_process_basic_tool_knowledge() {
        ...
        assert_eq!(result.get("status").and_then(|v| v.as_str()).unwrap(), "success");
        assert!(result.get("key").and_then(|v| v.as_str()).unwrap().contains("phoenix"));
    }
    ...
    async fn test_process_complex_tool_knowledge() {
        ...
        let result = process_tool_knowledge_storage(context).await.unwrap();
        assert_eq!(result.get("status").and_then(|v| v.as_str()).unwrap(), "success");
        assert_eq!(result.get("snippets_count").and_then(|v| v.as_u64()).unwrap(), 1);
        assert_eq!(result.get("examples_count").and_then(|v| v.as_u64()).unwrap(), 1);
    }
}