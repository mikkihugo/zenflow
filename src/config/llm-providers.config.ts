...
export const ROUTING_STRATEGY = {
  ...
  RULES: {
    // Small contexts: GitHub Models (free) or Copilot (enterprise)
    smallContext: ['copilot', 'github-models', 'claude-code'],
    
    // Large contexts: Copilot (200K) > Claude Code > Gemini (1M)
    largeContext: ['copilot', 'claude-code', 'gemini'],
    
    // File operations: Use CLI providers
    fileOperations: ['claude-code', 'gemini'],
    
    // Code analysis: Copilot excellent for code understanding
    codeAnalysis: ['copilot', 'claude-code', 'gemini'],
    
    // JSON responses: All providers support structured output
    structuredOutput: ['copilot', 'github-models', 'claude-code', 'gemini']
  }
};
