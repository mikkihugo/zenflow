...
export function calculateOptimalStreams(
  ...
  if (availableMemoryGB >= 24) {
    ...
    baseConfig = {
      portfolioStreamMB: Math.round(memory8GBConfig.portfolioStreamMB + (memory32GBConfig.portfolioStreamMB - memory8GBConfig.portfolioStreamMB) * scaleFactor),
      programStreamMB: Math.round(memory8GBConfig.programStreamMB + (memory32GBConfig.programStreamMB - memory8GBConfig.programStreamMB) * scaleFactor),
      swarmStreamMB: Math.round(memory8GBConfig.swarmStreamMB + (memory32GBConfig.swarmStreamMB - memory8GBConfig.swarmStreamMB) * scaleFactor),
      systemReserveMB: Math.round(memory8GBConfig.systemReserveMB + (memory32GBConfig.systemReserveMB - memory8GBConfig.systemReserveMB) * scaleFactor),
      cacheBufferMB: Math.round(memory8GBConfig.cacheBufferMB + (memory32GBConfig.cacheBufferMB - memory8GBConfig.cacheBufferMB) * scaleFactor),
  ...
  const availableMemoryMB = (availableMemoryGB * 1024) - baseConfig.systemReserveMB - baseConfig.cacheBufferMB;
...
export class AdaptiveMemoryOptimizer {
  ...
  canAllocateStream(type: 'portfolio' | 'program' | 'swarm'): boolean {
    ...
    const memoryPerStream = this.config[`${type}StreamMB` as keyof MemoryAllocationStrategy] as number;
  ...
  private getMaxStreamsForType(type: 'portfolio' | 'program' | 'swarm'): number {
  ...
  recordPerformance(metrics: Partial<PerformanceMetrics>): void {
    ...
    if (this.autoScaleEnabled && (Date.now() - this.lastOptimization) > this.optimizationInterval) {
  ...
  private performAutoOptimization(): void {
    ...
    const avgCpuUtil = recentMetrics.reduce((sum, m) => sum + m.cpuUtilization, 0) / recentMetrics.length;
    ...
    const memoryVolatility = Math.max(...recentMetrics.map(m => m.memoryUtilization)) - Math.min(...recentMetrics.map(m => m.memoryUtilization));

    let adjustmentFactor = 1.0;
    let action = 'maintain';

    // AGGRESSIVE scale down at first sign of memory pressure
    if (avgMemoryUtil > 0.60 || maxRecentMemory > 0.65 || avgErrorRate > 0.001) {
      adjustmentFactor = 0.6; // Very aggressive scale down
      action = 'emergency-scale-down';
    }
    // Preventive scale down for any memory concerns
    else if (avgMemoryUtil > 0.45 || memoryVolatility > 0.15 || maxRecentMemory > 0.55) {
    ...
    else if (avgMemoryUtil < 0.25 && maxRecentMemory < 0.30 && avgCpuUtil < 0.35 && 
             avgErrorRate === 0 && avgThroughput > 25 && memoryVolatility < 0.03 &&
             this.performanceHistory.length >= 30) { // Require lots of perfect history
  ...
  getPerformanceTrends(): {
    ...
    const memoryTrend = avgRecentMemory > avgOlderMemory * 1.1 ? 'increasing' : 
                       avgRecentMemory < avgOlderMemory * 0.9 ? 'decreasing' : 'stable';
    
    const cpuTrend = avgRecentCpu > avgOlderCpu * 1.1 ? 'increasing' :
                     avgRecentCpu < avgOlderCpu * 0.9 ? 'decreasing' : 'stable';
    
    const throughputTrend = avgRecentThroughput > avgOlderThroughput * 1.1 ? 'increasing' :
                           avgRecentThroughput < avgOlderThroughput * 0.9 ? 'decreasing' : 'stable';
    ...
    } else if (memoryTrend === 'decreasing' && throughputTrend === 'increasing') {
 ...
 */
export function createMemoryOptimizer(config?: MemoryAllocationStrategy): AdaptiveMemoryOptimizer {
...
export function validateMemoryConfig(config: ParallelStreamConfig): {
  ...
  const totalSystemMemoryMB = totalMemoryMB + memory32GBConfig.systemReserveMB + memory32GBConfig.cacheBufferMB;
