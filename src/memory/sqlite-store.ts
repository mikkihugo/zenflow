...
class SqliteMemoryStore {
  ...
  private statements: Map<string, any>; // SQLite.Statement type would be better here
  ...
  private async _directoryExists(dir: string): Promise<boolean> {
    try {
  ...
  async store(key: string, value: string | object, options: { namespace?: string; metadata?: object; ttl?: number } = {}): Promise<{ success: boolean; id: number; size: number }> {
  ...
  async list(options: { namespace?: string; limit?: number } = {}): Promise<any[]> {
    await this.initialize();
  ...
  async search(pattern: string, options: { namespace?: string; limit?: number } = {}): Promise<any[]> {
    await this.initialize();
   ...
   */
  private _getCacheKey(operation: string, ...args: any[]): string {
    return `${operation}:${JSON.stringify(args)}`;
  }

  private _getFromCache(cacheKey: string): any | null {
  ...
  private _setCache(cacheKey: string, data: any, customTTL: number | null = null): void {
  ...
  private _invalidateCache(pattern: string | null = null): void {
    if (pattern) {
   ...
   */
  getPerformanceStats(): {
    cache: { enabled: boolean; hits: number; misses: number; hitRate: number; size: number; timeout: number };
    database: { cacheSize: number; mmapSize: number; isInitialized: boolean };
  } {
    return {
      cache: {
  ...
  async getDatabaseStats(): Promise<{
    ...
    try {
      ...
      return {
        ...
        indexNames: indexStats.map((idx: any) => idx.name)
      };
    } catch (error: any) {
