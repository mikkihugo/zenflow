...
let _glob;
try {
  const _globModule = await import('glob');
...
;
      ...
      const _duplicates = await this.processDuplicates(jscpdResults);
;
...
;
    try {
      ...
      try {
        const _reportContent = await readFile(outputFile, 'utf8');
...
;
    if(jscpdResults.duplicates) {
      for(const duplicate of jscpdResults.duplicates) {
        const _processed = await this.enhanceDuplicate(duplicate);
