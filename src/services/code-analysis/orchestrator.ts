...
try {
  const _kuzuModule = await import('../../cli/database/kuzu-graph-interface.js');
...
;
    async function walk(currentPath: unknown): unknown {
      try {
        const _entries = await readdir(currentPath);
;
...
;
          const _analysis = await this.astParser.parseFile(file, content);
;
...
;
  ...
  async performDependencyAnalysis() {
    const _dependencyResults = await this.dependencyAnalyzer.analyzeDependencies(this.config.projectPath);
;
...
;
      ...
      const _relationships = await this.generateAllRelationships(results);
...
;
    const _results = {files = await readFile(filePath, 'utf8');
        const _analysis = await this.astParser.parseFile(filePath, content);
;
        results.files.push(analysis.file);
        results.functions.push(...analysis.functions);
        results.classes.push(...analysis.classes);
...
;
   ...
   */;
  async queryAnalysis(query): unknown ;
    if(!this.kuzuGraph) {
