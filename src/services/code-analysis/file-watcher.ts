...
;
   ...
   */;
  async startWatching(orchestrator): unknown {
    if (this.isWatching) {
...
;
    try {
      ...
      const _sourceFiles = await this.discoverSourceFiles();
;
...
;
   ...
   */;
  async processFileChange(eventType, filePath): unknown ;
    try {
...
;
    ...
    discoverSourceFiles();
    {
      const _files = await this.getAllFiles(process.cwd());
;
