...
;
      // Safety checks
      const _safetyCheck = await performSafetyCheck(optimizedMessages);
      if (!safetyCheck.safe) {
        return {
...
;
      // Safety filtering
      const _safetyResult = await filterUnsafeContent(response);
...
;
      ...
      if (payload.context.metadata.factCheck) {
        const _factCheckResult = await performFactCheck(processedResponse);
...
;
    try {
      ...
      const _cachedResponse = await getCachedResponse(cacheKey);
;
      if (cachedResponse) {
        return {success = > ;
...
async function filterUnsafeContent(content = content;
  const _metadata = {filtersApplied = filtered.replace(/\b\d{3}-\d{2}-\d{4}\b/g: unknown, '[SSN_REMOVED]': unknown);
