...
import type { AgenticHookContext, HookHandlerResult, LLMHookPayload } from './types';
...
export class LLMHooks {
  ...
  async onPostCall(context: AgenticHookContext): Promise<HookHandlerResult> {
    const payload = context.payload as LLMHookPayload;
    this.logger.debug(
      `Post-LLM call: ${payload.provider}/${payload.model}, tokens: ${payload.tokens?.input}/${payload.tokens?.output}`
    );
