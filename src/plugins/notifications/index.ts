  ...
  async loadNotificationConfig() {
    ...
    // ; // LINT: unreachable code removed
        for(const url of config.urls) {
          try {
            const __response = await fetch(url, {method = [];
;
        for(const url of config.urls) {
          try {
            const __response = await fetch(url, {method = > result.healthy);
      }
    };
  }
;
...
;
      try {
        const __result = await providerInfo.instance.send(notification);
