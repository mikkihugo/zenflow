...
: Promise<
{
  ...
  const _contents = await this.getContents(owner, repo, '');
  return {languages = [];
...
const _allIssues = await this.getIssues(owner, repo, {state = allIssues.filter(issue => issue.state === 'open');
...
;
      ...
      const _allPRs = await this.makeRequest<GitHubPullRequest[]>(;
...
;
        recentActivity.push({date = new Date(Date.now() - timeRangeDays * 24 * 60 * 60 * 1000).toISOString();
      const _commits = await this.makeRequest<GitHubCommit[]>(;
...
;
      ...
      const _recentCommits = await this.makeRequest<GitHubCommit[]>(;
...
;
    try {
      const _response = await fetch(`${this.config.settings.baseUrl}${endpoint}`, {headers = await response.json();
;
