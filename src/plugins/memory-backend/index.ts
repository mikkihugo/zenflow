...
// const __searchResult = awaitthis.lanceInterface.semanticSearch(pattern, {table = JSON.parse(result.metadata  ?? '{}');
    ...
    try {
// const _searchResult = awaitthis.lanceInterface.semanticSearch(query, {table = = 'default' ? `metadata LIKE '%"namespace":"${namespace}"%'` ,
...
// await this.persist();
  ...
  async store(key, value, namespace?: string): Promise<StorageResult> ;
    throw new Error('PostgreSQL backend not yet implemented');

  async retrieve(key, namespace?: string): Promise<JSONValue | null> ;
    throw new Error('PostgreSQL backend not yet implemented');

  async search(pattern, namespace?: string): Promise<Record<string, JSONValue>> ;
    throw new Error('PostgreSQL backend not yet implemented');

  async delete(key, namespace?: string): Promise<boolean> ;
    throw new Error('PostgreSQL backend not yet implemented');
