...
constructor((config = {}))
{
  ...
  const _results = await this.storage?.search(embedding.toString(), options.namespace ?? 'default');
...
;
  const __document = {id = === 'object' && value && 'title' in value) ? (value as any).title : key,source = 'default'): Promise<JSONValue | null> {
    const _fullKey = `${namespace}:${key}`;
;
  try {
      // Use semantic search to find the document
      const __searchResult = await this.lanceInterface.semanticSearch(fullKey, {table = === 0) {
        return null;
    //   // LINT: unreachable code removed}
;
...
;
    try {
      ...
      const __searchResult = await this.lanceInterface.semanticSearch(pattern, {table = JSON.parse(result.metadata  ?? '{}');
...
;
  async listNamespaces(): Promise<string[]> ;
    ...
    try {
      const _searchResult = await this.lanceInterface.semanticSearch('*', {table = new Set<string>();
...
;
    try {
      const __result = await this.conn.query(`;
...
;
    ...
    // this.dbPath = `${config.path // LINT: unreachable code removed}/storage.db`;
;
    this.connectionPool = new SQLiteConnectionPool(this.dbPath, {minConnections = await import('node:fs/promises');
...
;
    ...
    const _result = await this.connectionPool.execute(`;
    // SELECT value FROM storage ; // LINT: unreachable code removed
...
;
    ...
    const _rows = await this.connectionPool.execute(`;
...
;
  async getStats(): Promise<BackendStats> {
    const _countResult = await this.connectionPool.execute('SELECT COUNT(*) as count FROM storage');
    const _sizeResult = await this.connectionPool.execute('SELECT SUM(LENGTH(value)) as size FROM storage');
    const _namespaceResult = await this.connectionPool.execute('SELECT COUNT(DISTINCT namespace) as namespaces FROM storage');
;
...
;
  async initialize(): Promise<void> ;
    ...
    try {
      const _fs = await import('node:fs/promises');
      const _data = await fs.readFile(this.filepath, 'utf8');
