...
;
      ...
      const _worker = await this.createSecureWorker(manifest, config, policy);
...
;
      ...
      const _threatScan = await this.scanForThreats(plugin, manifest);
      errors.push(...threatScan.errors);
      warnings.push(...threatScan.warnings);
;
      // 3. Validate permissions
      if (config.permissions && config.permissions.length > 0) {
        const _permissionValidation = await this.validatePermissions(manifest.name, config.permissions);
