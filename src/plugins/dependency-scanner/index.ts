...
;
    ...
    const _packageJsonFiles = await glob('**/package.json', {ignore = await this.analyzeDependencies(packageJsonFiles, includeTypes);
    const _conflicts = await this.detectConflicts(dependencyAnalysis);
;
    return {
      totalPackages = {};
    ...
    for(const file of packageFiles) {
      try {
        const _pkgContent = await readFile(file, 'utf8');
...
;
      if(versions.length > this.config.conflictThreshold) {
        const _conflict = await this.analyzeConflict(depName, data);
