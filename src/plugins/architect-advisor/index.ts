...
import type { PluginConfig, PluginContext, PluginManifest } from '../types';
...
export class ArchitectAdvisorPlugin extends BasePlugin {
  public declare config: ArchitectAdvisorConfig;
  ...
  private calculateCodebaseStats(codebase: any[]): CodebaseStats {
    ...
    // Find extremes
    stats.largest_file = codebase.reduce(
      (max, file) => (file.size > (max?.size || 0) ? file : max),
      codebase[0]
    );

    stats.oldest_file = codebase.reduce(
      (oldest, file) => (file.modified < (oldest?.modified || new Date()) ? file : oldest),
      codebase[0]
    );

    stats.newest_file = codebase.reduce(
      (newest, file) => (file.modified > (newest?.modified || new Date(0)) ? file : newest),
      codebase[0]
    );
  ...
  private hasDirectDatabaseAccess(file: any): boolean {
    ...
    return dbPatterns.some((pattern) => file.content.toLowerCase().includes(pattern.toLowerCase()));
  }
  ...
  private extractExports(file: any): string[] {
    ...
    const exportRegex =
      /export\s+(?:default\s+)?(?:class|function|const|let|var)\s+(\w+)|export\s*\{([^}]*)\}/g;
    let match;
  ...
  private calculateAbstractness(file: any): number {
    ...
    const abstractCount = (file.content.match(/(?:abstract\s+class|interface\s+\w+)/g) || [])
      .length;
  ...
  private countIncomingDependencies(targetFile: string, coupling: Map<string, any>): number {
    ...
    for (const [filePath, data] of coupling) {
      if (filePath !== targetFile) {
        const hasImport = data.imports.some(
          (imp: any) => this.resolvePath(filePath, imp.module) === targetFile
        );
        if (hasImport) count++;
      }
    }
  ...
  private checkSOLIDPrinciples(codebase: any[]): Finding[] {
    ...
    for (const file of codebase) {
      ...
      const longIfChains = (file.content.match(/if\s*\([^)]*\)\s*\{[^}]*\}\s*else\s*if/g) || [])
        .length;
  ...
  private async analyzeScalability(codebase: any[]): Promise<Finding[]> {
    ...
    for (const file of codebase) {
      if (
        file.content.includes('singleton') ||
      ...
      const globalUsage = globalPatterns.filter((pattern) => file.content.includes(pattern)).length;
  ...
  private async analyzeTestability(codebase: any[]): Promise<Finding[]> {
    ...
    // Find test files
    const testFiles = codebase.filter(
      (file) =>
   ...
   * ADR Generation
   */
  async generateArchitectureDecisionRecords(
    analysisType = 'all',
    options: any = {}
  ): Promise<any[]> {
  ...
  private async createADRProposals(analysis: AnalysisResult, options: any = {}): Promise<any[]> {
    ...
    const highPriorityFindings = analysis.findings.filter(
    ...
    const architecturalRecommendations = analysis.recommendations.filter(
      (r) => r.type === 'architectural_refactoring' || r.type === 'pattern_recommendation'
    );
  ...
  private async generateADRFromRecommendation(
  ...
  private async generateADRDecision(finding: Finding): Promise<string> {
    ...
    const decisionTemplates: Record<string, string> = {
      circular_dependency:
        'We will break the circular dependency by introducing abstractions and reorganizing the module structure.',
      god_class:
        'We will refactor the large class into smaller, more cohesive classes following the Single Responsibility Principle.',
      tight_coupling: 'We will reduce coupling by introducing interfaces and dependency injection.',
      security_vulnerability:
        'We will immediately address the security vulnerability by implementing secure coding practices.',
    };

    decision +=
      decisionTemplates[finding.type] ||
  ...
  private async persistAnalysis(analysis: AnalysisResult): Promise<void> {
    try {
      ...
      const analysisPath = path.join(
        this.config.recommendations_path,
  ...
  async getSystemHealth(): Promise<any> {
    ...
    const criticalIssues =
      latestAnalysis.findings?.filter((f) => f.severity === 'critical').length || 0;
