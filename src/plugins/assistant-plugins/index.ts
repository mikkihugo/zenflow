...
// const _pluginStructure = awaitthis.generatePluginStructure(;
  ...
  if(config.features?.includes('file-operations')) { imports.add("{ readFile, writeFile, mkdir  } from 'fs/promises'");
    imports.add('path');
  //   }
  ...
  if(config.features?.includes('database')) { imports.add("{ Database  } from './database'");
  //   }
