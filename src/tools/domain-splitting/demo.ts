...
export default class DomainSplittingDemo {
  ...
  private async categorizeFiles(files: string[]): Promise<{ [category: string]: string[] }> {
    ...
    for (const file of files) {
      ...
      } else if (directory.includes('core') || filename.includes('core') || filename.includes('network')) {
        categories['core-algorithms'].push(file);
      } else if (directory.includes('model') || directory.includes('preset') || filename.includes('model')) {
  ...
  private calculateComplexity(files: string[], categories: { [category: string]: string[] }): number {
  ...
  private generateRecommendations(files: string[], categories: { [category: string]: string[] }): string[] {
    ...
    for (const [category, categoryFiles] of Object.entries(categories)) {
      if (categoryFiles.length > 8) {
        recommendations.push(`${category} category has ${categoryFiles.length} files - consider sub-dividing`);
      }
    }
    ...
    return {
      ...
      targetSubDomains: [
        ...
        },
        {
          ...
          files: [...(analysis.filesByCategory['bridge'] || []), ...(analysis.filesByCategory['utilities'] || [])]
        }
      ]
    };
  }
