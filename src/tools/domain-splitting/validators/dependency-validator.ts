...
export class DependencyValidator implements DependencyMapper, SplitValidator {
  ...
  private async extractImports(filePath: string): Promise<Array<{
    module: string;
  ...
  private async extractExports(filePath: string): Promise<Array<{
    name: string;
  ...
  private determineExportType(content: string, name: string): 'function' | 'class' | 'interface' | 'type' | 'constant' {
  ...
  private isInterDomainImport(importPath: string, currentDomain: string, allDomains: string[]): boolean {
  ...
  private findHeavilyDependentFiles(graph: DependencyGraph): Array<{ current: string; dependencies: number }> {
  ...
  private suggestBetterLocation(file: { current: string; dependencies: number }, graph: DependencyGraph): string | null {
  ...
  private findProblematicCrossDependencies(plan: SubDomainPlan, graph: DependencyGraph): Array<{
    from: string;
    to: string;
  }> {
    const problematic = [];
  ...
  private async simulateBuild(): Promise<{ success: boolean; errors: string[]; warnings: string[] }> {
    try {
