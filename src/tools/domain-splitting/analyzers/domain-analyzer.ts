...
export class DomainAnalysisEngine implements DomainAnalyzer {
  ...
  async analyzeDomainComplexity(domainPath: string): Promise<DomainAnalysis> {
    ...
    console.log(`📂 Categorized into ${Object.keys(categories).filter(k => categories[k].length > 0).length} categories`);
    
    // Build dependency graph
    const dependencies = await this.buildDependencyGraph(fileAnalyses);
    console.log(`🔗 Built dependency graph with ${dependencies.nodes.length} nodes, ${dependencies.edges.length} edges`);
  ...
  async calculateSplittingBenefits(plans: SubDomainPlan[]): Promise<SplittingMetrics> {
    ...
    const averageFilesPerSubDomain = plans.length > 0 
      ? plans[0].targetSubDomains.reduce((sum, sub) => sum + sub.estimatedFiles, 0) / plans[0].targetSubDomains.length
      : 0;
  ...
  private extractExportsBasic(content: string): any[] {
    ...
    while ((match = exportRegex.exec(content)) !== null) {
      exports.push({
        name: match[3],
        type: match[2] === 'function' ? 'function' : 
              match[2] === 'class' ? 'class' :
              match[2] === 'interface' ? 'interface' :
              match[2] === 'type' ? 'type' : 'constant',
        isPublic: true
      });
    }
  ...
  private categorizeFile(filePath: string, content: string, imports: any[], exports: any[]): string {
    ...
    if (directory.includes('coordination') || filename.includes('coordination')) return 'coordination';
    if (directory.includes('model') || filename.includes('model') || directory.includes('preset')) return 'models';
  ...
  private async buildDependencyGraph(fileAnalyses: FileAnalysis[]): Promise<DependencyGraph> {
    const nodes = fileAnalyses.map(analysis => ({
      ...
      type: analysis.purpose.primary === 'test' ? 'test' as const :
            analysis.purpose.primary === 'config' ? 'config' as const :
            analysis.purpose.primary === 'utility' ? 'utility' as const : 'module' as const
    }));
  ...
  private async analyzeCoupling(fileAnalyses: FileAnalysis[], dependencies: DependencyGraph): Promise<CouplingAnalysis> {
  ...
  private calculateGroupCoupling(nodes: string[], dependencies: DependencyGraph): number {
    ...
    let possibleConnections = nodes.length * (nodes.length - 1);
  ...
  private calculateCouplingScores(dependencies: DependencyGraph): { average: number; max: number; isolated: string[] } {
  ...
  private calculateComplexity(fileAnalyses: FileAnalysis[], dependencies: DependencyGraph): number {
    ...
    const avgFileComplexity = fileAnalyses.reduce((sum, f) => sum + f.complexity, 0) / fileAnalyses.length;
    const dependencyComplexity = dependencies.edges.length / Math.pow(dependencies.nodes.length, 2);
