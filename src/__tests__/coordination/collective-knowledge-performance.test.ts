...
import { CollectiveKnowledgeBridge } from '../../coordination/collective-knowledge-bridge.ts';
...
describe('Hive Knowledge System - Performance Tests', () => {
  ...
  describe('Response Time Performance', () => {
    test(
      'should handle single knowledge queries within acceptable time',
      async () => {
    ...
    test(
      'should maintain performance with cache hits',
      async () => {
        const swarm = new SwarmKnowledgeSync(
    ...
    test.each(PERFORMANCE_CONFIG?.CONCURRENT_REQUESTS)(
      ...
      async (requestCount) => {
        const swarms = Array.from(
          { length: Math.min(requestCount, 10) },
          (_, i) =>
        ...
        const promises = queries.map(
          async ({ swarm, query, domain }, index) => {
  ...
  describe('Throughput and Scalability', () => {
    test(
      'should achieve target throughput under sustained load',
      async () => {
        const swarms = Array.from(
          { length: 5 },
          (_, i) =>
  ...
  describe('System Health Under Stress', () => {
    test(
      'should maintain system health metrics under stress',
      async () => {
        const swarm = new SwarmKnowledgeSync(
  ...
  describe('Performance Regression Detection', () => {
    test(
      'should detect performance regression in knowledge queries',
      async () => {
        const swarm = new SwarmKnowledgeSync(
        ...
        const regressionRatio =
          (afterChangesStats?.avg || 0) / (baselineStats?.avg || 1);
