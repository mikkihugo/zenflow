...
class PerformanceMetrics {
  ...
  getStats(
    name: string,
  ): { avg: number; min: number; max: number; p95: number; count: number } | null {
    ...
    test(
      ...
      async () => {
        ...
        const stressOperations = [
          ...
          // Subscriptions
          ...Array.from({ length: 20 }, (_, i) => swarm.subscribeToDomain(`stress-domain-${i}`)),
        ];
