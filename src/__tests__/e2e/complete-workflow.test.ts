...
import { handleWorkflowCommand } from '../../interfaces/cli.disabled/commands/workflow.ts';
...
describe('End-to-End Multi-Level Workflow Integration', () => {
  ...
  let memoryOptimizer: AdaptiveMemoryOptimizer;
  ...
  describe('Complete Vision→Code Flow', () => {
    test('should execute full workflow with all orchestration levels', async () => {
      ...
      const monitorData = JSON.parse(
        (monitorResult as any).content[0].text
      ) as any;
    ...
    test('should validate cross-level dependency resolution', async () => {
      ...
      const postScaleMonitor = await handleWorkflowToolCall(
        'workflow_monitor',
        {
          detailed: false,
        }
  ...
  describe('System Performance and Scalability', () => {
    test('should handle stress testing scenarios', async () => {
      ...
      const stressData = JSON.parse(
        (stressResult as any).content[0].text
      ) as any;
      ...
      expect(stressData.analysis.recommendations).toContain(
    ...
    test('should validate CLI workflow commands', async () => {
      ...
      try {
        ...
        expect(logOutput).toContain(
          'Initializing Advanced Multi-Level Workflow Architecture'
        );
  ...
  describe('Error Handling and Recovery', () => {
    test('should handle invalid configurations gracefully', async () => {
      ...
      const invalidScaleResult = await handleWorkflowToolCall(
        'workflow_scale',
        {
          direction: 'up',
      ...
      // Test unknown tool
      const unknownToolResult = await handleWorkflowToolCall(
        'unknown_tool',
        {}
      );
      ...
      const errorData = JSON.parse(
        (unknownToolResult as any).content[0].text
      ) as any;
