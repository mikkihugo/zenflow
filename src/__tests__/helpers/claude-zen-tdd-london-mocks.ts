...
export class MockQueensService {
  ...
  architectAnalyze: jest.MockedFunction<(requirements: any) => Promise<ArchitectureResult>> = jest.fn();
...
export class MockNeuralFrameworkService {
  ...
  trainModel: jest.MockedFunction<(networkId: string, data: TrainingData) => Promise<TrainingResult>> = jest.fn();
  predict: jest.MockedFunction<(networkId: string, input: number[]) => Promise<number[]>> = jest.fn();
  evaluateModel: jest.MockedFunction<(networkId: string, testData: TestData) => Promise<EvaluationResult>> = jest.fn();
