...
export class MockHiveMindService {
  ...
  spawnQueen: MockedFunction<(config: unknown) => Promise<string>> = vi.fn();
  coordinateQueens: MockedFunction<
    (task: unknown) => Promise<CoordinationResult>
  > = vi.fn();
  processTask: MockedFunction<(taskId: string) => Promise<TaskResult>> =
    vi.fn();
  getQueenStatus: MockedFunction<(queenId: string) => Promise<QueenStatus>> =
    vi.fn();
...
export class MockQueensService {
  ...
  architectAnalyze: MockedFunction<
    (requirements: unknown) => Promise<ArchitectureResult>
  > = vi.fn();
  architectDesign: MockedFunction<(spec: unknown) => Promise<DesignResult>> =
    vi.fn();
  architectCoordinate: MockedFunction<(teams: string[]) => Promise<void>> =
    vi.fn();
...
export class MockNeuralFrameworkService {
  initializeNetwork: MockedFunction<
    (config: NetworkConfig) => Promise<string>
  > = vi.fn();
  trainModel: MockedFunction<
    (networkId: string, data: TrainingData) => Promise<TrainingResult>
  > = vi.fn();
  predict: MockedFunction<
    (networkId: string, input: number[]) => Promise<number[]>
  > = vi.fn();
  evaluateModel: MockedFunction<
    (networkId: string, testData: TestData) => Promise<EvaluationResult>
  > = vi.fn();
  saveModel: MockedFunction<
    (networkId: string, path: string) => Promise<void>
  > = vi.fn();
  loadModel: MockedFunction<(path: string) => Promise<string>> = vi.fn();
...
export class MockClaudeZenApiService {
  ...
  > = vi.fn();
  stopServer: MockedFunction<() => Promise<void>> = vi.fn();
