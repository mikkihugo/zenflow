    ...
    describe('Agent Group Execution', () => {
      ...
      it('should distribute tasks across group members', async () => {
        ...
        const agentExecutions = new Set(
          results?.map((r: TaskResult) => r.agentId)
        );
      ...
      it('should handle member failures gracefully', async () => {
        ...
        expect(results).toBeDefined();
        const successfulTasks = results?.filter((r: TaskResult) => r.success);
        const failedTasks = results?.filter((r: TaskResult) => !r.success);
        ...
        failedTasks.forEach((result: TaskResult) => {
      ...
      it('should aggregate capabilities from all members', () => {
        ...
        expect(
          groupCapabilities.some((cap: unknown) => cap.name === 'specialized-0')
        ).toBe(true);
        expect(
          groupCapabilities.some((cap: unknown) => cap.name === 'specialized-1')
        ).toBe(true);
        expect(
          groupCapabilities.some((cap: unknown) => cap.name === 'specialized-2')
        ).toBe(true);
  ...
  describe('Component Management (London TDD)', () => {
    ...
    describe('Agent Component Interface', () => {
      ...
      it('should check capability compatibility correctly', () => {
        ...
        mockAgent.canHandleTask.mockImplementation((task: unknown) =>
      ...
      it('should aggregate capabilities from composite components', () => {
        ...
        expect(
          agentCaps.some((cap: unknown) => cap.name === 'agent-capability')
        ).toBe(true);
        expect(
          groupCaps.some((cap: unknown) => cap.name === 'group-capability')
        ).toBe(true);
      });
