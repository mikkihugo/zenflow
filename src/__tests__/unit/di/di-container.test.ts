...
describe('DI Container - Core Functionality', () => {
  ...
  describe('Container Disposal', () => {
    it('should dispose singleton instances', async () => {
      ...
      container.register(token, new SingletonProvider(() => new MockDisposableService(), disposeFn));
...
describe('DI Container - Integration with Core Tokens', () => {
  ...
  it('should work with swarm tokens', () => {
    class MockSwarmCoordinator {
      ...
      async addAgent(config: any): Promise<string> { return 'agent-1'; }
      async removeAgent(agentId: string): Promise<void> {}
      async assignTask(task: any): Promise<string> { return 'task-1'; }
      getMetrics(): any { return {}; }
      async shutdown(): Promise<void> {}
    }
    
    container.register(SWARM_TOKENS.SwarmCoordinator, new SingletonProvider(() => new MockSwarmCoordinator()));
