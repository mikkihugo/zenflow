...
import { TopologyOptimizer } from '../../../../coordination/swarm/core/topology-optimizer';
...
describe('Swarm Topology Management (London TDD)', () => {
  ...
  beforeEach(() => {
    ...
    mockAgents = Array.from({ length: 5 }, (_, i) => ({
      id: `agent-${i}`,
      type: 'coordinator',
      status: 'active',
  ...
  describe('Mesh Topology Creation', () => {
    ...
    it('should handle mesh topology failures gracefully', async () => {
      ...
      await expect(topologyManager.initializeTopology('mesh', mockAgents))
