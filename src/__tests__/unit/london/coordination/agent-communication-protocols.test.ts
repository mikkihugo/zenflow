...
describe('Agent Communication Protocols (London TDD)', () => {
    ...
    mockAgents = Array.from(
      ...
      (_, i) =>
        ({
          ...
          status: 'active',
          capabilities:
            i < 2 ? ['coordination', 'supervision'] : ['execution', 'analysis'],
          sendMessage: vi.fn(),
  ...
  describe('Direct Message Routing', () => {
    it('should route point-to-point messages correctly', async () => {
      ...
      mockMessageBroker.routeMessage.mockResolvedValue({
        delivered: true,
        latency: 45,
      });

      await communicationProtocol.sendDirectMessage(
        sender.id,
        receiver.id,
        message
      );
  ...
  describe('Broadcast Communication Patterns', () => {
    ...
    it('should support selective broadcast with agent filtering', async () => {
      ...
      const coordinatorAgents = mockAgents.filter(
        (a) => a.type === 'coordinator'
      );

      mockMessageBroker.broadcast.mockResolvedValue({
        success: true,
        deliveredTo: coordinatorAgents.map((a) => a.id),
        filteredOut: mockAgents
          .filter((a) => a.type !== 'coordinator')
          .map((a) => a.id),
      });

      await communicationProtocol.broadcastMessage(
        broadcaster.id,
        selectiveMessage
      );
  ...
  describe('Channel-Based Communication', () => {
    ...
    it('should handle channel subscription and message distribution', async () => {
      ...
      const channelMessage = {
        ...
        content: {
          discussion: 'task_progress',
          updates: ['step1_complete', 'step2_in_progress'],
        },
      };
      ...
      expect(subscriber.subscribeToChannel).toHaveBeenCalledWith(
        channelId,
        expect.any(Function)
      );
    });
    ...
    it('should support channel message threading and replies', async () => {
      ...
      const replyMessage = {
        ...
        type: 'reply',
        content: {
          response: 'Agreed on strategy A',
          references: ['strategy_doc_v2'],
        },
        threadId: 'thread-001',
        replyTo: 'thread-root',
      };
      ...
      await communicationProtocol.sendChannelMessage(
        channelId,
        originalMessage
      );
  ...
  describe('Message Prioritization and QoS', () => {
    it('should prioritize critical messages over regular communication', async () => {
      ...
      mockMessageBroker.sendDirectMessage.mockImplementation(
        (from, to, msg) => {
      ...
      const criticalCall = mockMessageBroker.sendDirectMessage.mock.calls.find(
        (call: unknown) => call[2]?.priority === 'critical'
      );
    ...
    it('should implement message queuing with priority ordering', async () => {
      ...
      const messages = [
        ...
        { id: 'msg-2', priority: 'high', content: { task: 'urgent_analysis' } },
        {
          id: 'msg-3',
          priority: 'normal',
          content: { task: 'regular_processing' },
        },
        {
          id: 'msg-4',
          priority: 'critical',
          content: { task: 'emergency_response' },
        },
      ];
      ...
      const enqueueCalls = (agent.getMessageQueue().enqueue as vi.Mock).mock
        .calls;
  ...
  describe('Communication Analytics and Monitoring', () => {
    it('should track message delivery metrics', async () => {
      ...
      const metrics = await communicationProtocol.getCommunicationMetrics(
        sender.id
      );
