...
import { TrainingAlgorithm } from '../../../../neural/core/training';
...
describe('Advanced Neural Training Algorithms (Classical TDD)', () => {
  ...
  // Helper method for test error calculation
  private calculateTestError(network: NeuralNetwork, testData: any[]): number {
    let totalError = 0;
    
    for (const sample of testData) {
      const prediction = network.predict(sample.input);
      const error = sample.output.reduce((sum: number, target: number, i: number) => 
        sum + Math.pow(target - prediction[i], 2), 0
      );
      totalError += error;
    }
