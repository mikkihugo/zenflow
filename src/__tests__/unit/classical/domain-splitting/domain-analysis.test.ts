...
describe('Domain Splitting - Classical TDD', () => {
  ...
  describe('Domain Analysis Engine', () => {
    ...
    it('should categorize files correctly by purpose', async () => {
      ...
      await fs.writeFile(path.join(testDomain, 'neural-network.ts'), `
        export class NeuralNetwork {
          forward() { /* neural logic */ }
        }
      `);
      
      await fs.writeFile(path.join(testDomain, 'trainer.ts'), `
      ...
      await fs.writeFile(path.join(testDomain, 'utils.ts'), `
        export function helper() { /* utility */ }
      `);
      
      await fs.writeFile(path.join(testDomain, 'config.ts'), `
    ...
    it('should build dependency graph correctly', async () => {
      ...
      await fs.writeFile(path.join(testDomain, 'moduleA.ts'), `
        import { functionB } from './moduleB';
        export function functionA() { return functionB(); }
      `);
      
      await fs.writeFile(path.join(testDomain, 'moduleB.ts'), `
        export function functionB() { return 'B'; }
      `);
      
      await fs.writeFile(path.join(testDomain, 'moduleC.ts'), `
  ...
  async function createTestDomainStructure(domainPath: string): Promise<void> {
    ...
    // Create core files
    await fs.writeFile(path.join(domainPath, 'core.ts'), `
      export class Core {
        process() { return 'core'; }
      }
    `);
    ...
    // Create model files
    await fs.writeFile(path.join(domainPath, 'model.ts'), `
    ...
    await fs.writeFile(path.join(domainPath, 'utils.ts'), `
      export function helper() { return 'help'; }
    `);
    
    // Create test file
    await fs.writeFile(path.join(domainPath, 'core.test.ts'), `
      import { Core } from './core';
      describe('Core', () => {
        it('should work', () => {
          const core = new Core();
  ...
  async function createComplexDomainStructure(domainPath: string): Promise<void> {
    ...
    await fs.writeFile(path.join(coreDir, 'algorithm.ts'), `
      export class Algorithm {
        execute() { return 'executed'; }
      }
    `);
    await fs.writeFile(path.join(coreDir, 'network.ts'), `
    ...
    await fs.writeFile(path.join(modelsDir, 'cnn.ts'), `
      import { Network } from '../core/network';
      export class CNN extends Network {
        convolve() { return 'convolved'; }
      }
    `);
    await fs.writeFile(path.join(modelsDir, 'rnn.ts'), `
      import { Network } from '../core/network';
    ...
    await fs.writeFile(path.join(agentsDir, 'neural-agent.ts'), `
    ...
    await fs.writeFile(path.join(wasmDir, 'wasm-loader.ts'), `
    ...
    // Create bridge
    await fs.writeFile(path.join(domainPath, 'bridge.ts'), `
      import { Network } from './core/network';
