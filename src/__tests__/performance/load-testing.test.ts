  ...
  describe('Memory and Resource Management Performance', () => {
    test('should optimize memory allocation under varying load patterns', async () => {
      ...
      if (optimizablePatterns.length > 0) {
        const avgGains =
          optimizablePatterns.reduce((sum, r) => sum + r.potentialGains, 0) /
          optimizablePatterns.length;
      ...
      performanceResults.push({
        ...
        optimizationResults,
        avgOptimizationTime: (
          optimizationResults.reduce((sum, r) => sum + r.optimizationTime, 0) /
    ...
    test('should handle scaling operations under concurrent load', async () => {
      ...
      performanceResults.push({
        ...
        successRate: (
          (successfulScaling.length / scalingOperations.length) *
          100
        ).toFixed(1),
      });
  ...
  describe('End-to-End Performance Validation', () => {
    test('should maintain performance SLAs under mixed workload', async () => {
      ...
      const mixedOperations = [
        () =>
          handleWorkflowToolCall('workflow_initialize', {
            repoPath: './e2e-mixed-1',
            topology: 'mesh',
          }),
        () => handleWorkflowToolCall('workflow_monitor', { detailed: true }),
        () =>
          handleWorkflowToolCall('workflow_test', {
            testType: 'performance',
            duration: 10,
          }),
        () =>
          handleWorkflowToolCall('system_info', {
            includeRecommendations: true,
          }),
        () =>
          handleWorkflowToolCall('workflow_scale', {
            direction: 'up',
            amount: 10,
          }),
      ];
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        const iterationPromises = mixedOperations.map(
          async (operation, opIndex) => {
            const opStartTime = Date.now();
            try {
...
function calculatePerformanceSummary(results: unknown[]) {
  ...
  results.forEach((r) => {
    if (typeof r.successRate === 'number') {
      ...
      const rate = Number.parseFloat(r.successRate);
  ...
  if (summary.successRates.length > 0) {
    const avgSuccessRate =
      summary.successRates.reduce((sum, rate) => sum + rate, 0) /
      summary.successRates.length;
    summary.performanceMetrics.averageSuccessRate =
