...
class PerformanceBenchmark {
  ...
  calculateStatistics(name) {
    ...
    return {
      ...
      throughput:
        samples.length / (samples[samples.length - 1].duration / 1000), // ops/sec
    };
  }
...
class LoadTester {
  ...
  analyzeResults(results) {
    ...
    return {
      ...
      throughput:
        successful.length / ((Date.now() - results[0]?.timestamp || 1) / 1000),
...
class ScalabilityTester {
  ...
  analyzeScalabilityResults(results) {
    ...
    return {
      ...
      scalabilityMetrics,
      overallEfficiency:
        scalabilityMetrics.reduce((sum, m) => sum + m.efficiency, 0) /
      ...
      recommendations:
        this.generateScalabilityRecommendations(scalabilityMetrics),
    };
  }
  ...
  describe('SessionManager Performance Tests', () => {
    ...
    test('should benchmark checkpoint operations', async () => {
      ...
      const checkpointStats =
        benchmark.calculateStatistics('single-checkpoint');
  ...
  describe('RecoveryWorkflows Performance Tests', () => {
    test('should benchmark workflow execution speed', async () => {
      ...
      for (let i = 0; i < workflowCount; i++) {
        ...
        const promise = recoveryWorkflows
          .triggerRecovery(`perf-trigger-${i}`)
          .then((execution) => {
  ...
  describe('Memory Usage and Leak Detection', () => {
    test('should detect memory leaks in long-running operations', async () => {
      ...
      const heapGrowthPattern = snapshots.map((snapshot, index) => ({
        ...
        growth:
          index > 0
            ? snapshot.memory.heapUsed - snapshots[0].memory.heapUsed
            : 0,
      }));
