...
class PerformanceAnalyzer {
  ...
  async testNeuralPerformance() {
    ...
    this.results.metrics.neuralTraining = {
      duration: trainTime,
      accuracy: accuracyMatch ? Number.parseFloat(accuracyMatch[1]) : 0,
      loss: lossMatch ? Number.parseFloat(lossMatch[1]) : 0,
  ...
  async testBenchmarks() {
    ...
    this.results.metrics.benchmarks = {
      overallScore: scoreMatch ? Number.parseInt(scoreMatch[1], 10) : 0,
      wasmLoadTime: wasmMatch ? Number.parseInt(wasmMatch[1], 10) : 0,
      swarmInitTime: swarmMatch ? Number.parseFloat(swarmMatch[1]) : 0,
      agentSpawnTime: agentMatch ? Number.parseFloat(agentMatch[1]) : 0,
      neuralOpsPerSec: neuralMatch ? Number.parseInt(neuralMatch[1], 10) : 0,
    };
