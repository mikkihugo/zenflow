...
import { MEMORY_TOKENS, DATABASE_TOKENS, CORE_TOKENS } from '../../di/tokens/core-tokens.js';
...
describe('Issue #63: Comprehensive Dependency Injection Implementation', () => {
  ...
  describe('Cross-Domain Integration & Consistency', () => {
    ...
    it('should support dependency injection across domain boundaries', () => {
      ...
      const databaseAdapter = databaseFactory.createAdapter({ type: 'sqlite', database: ':memory:' });
  ...
  describe('Performance and Resource Management', () => {
    it('should handle high-frequency operations efficiently', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        operations.push(controller.storeMemory({
          key: `test-key-${i}`,
          value: `test-value-${i}`
        }));
      }
