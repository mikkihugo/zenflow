...
describe('WebSocket Real-time Coordination Integration Tests', () => {
  ...
  describe('Real-time Swarm Status Updates', () => {
    it('should broadcast swarm initialization events to all connected clients', async () => {
      ...
      for (const clientMessages of receivedMessages) {
        const agentSpawnedEvents = clientMessages.filter(msg => 
          msg.type === 'swarm_event' && msg.event === 'agent_spawned'
        );
      ...
      for (const clientMessages of receivedMessages) {
        const taskAssignedEvents = clientMessages.filter(msg => 
          msg.type === 'swarm_event' && msg.event === 'task_assigned'
        );
    ...
    it('should provide real-time task progress tracking', async () => {
      ...
      const completionEvent = progressUpdates.find(update => 
        update.data.taskStatus === 'completed'
      );
  ...
  describe('Real-time Performance Monitoring', () => {
    ...
    it('should alert on performance threshold violations', async () => {
      ...
      const cpuAlert = alerts.find(alert => 
        alert.data.metric === 'cpu_usage' && alert.data.severity === 'warning'
      );
      ...
      const memoryAlert = alerts.find(alert => 
        alert.data.metric === 'memory_usage' && alert.data.severity === 'critical'
      );
    ...
    it('should provide real-time network latency monitoring', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        await swarmCoordinator.sendMessage(
          agents[senderIndex].id,
          agents[receiverIndex].id,
          {
            id: `network-test-${i}`,
            type: 'ping',
      ...
      expect(latestNetworkMetrics.data.latency.min).toBeLessThanOrEqual(latestNetworkMetrics.data.latency.average);
      expect(latestNetworkMetrics.data.latency.max).toBeGreaterThanOrEqual(latestNetworkMetrics.data.latency.average);
  ...
  describe('Real-time Collaboration Features', () => {
    it('should enable real-time document collaboration', async () => {
      ...
      for (const edit of edits) {
        collaborators[edit.collaborator].send(JSON.stringify({
          type: 'document_edit',
          documentId,
      ...
      for (let i = 0; i < collaborators.length; i++) {
        ...
        const receivedEdits = otherCollaboratorUpdates.filter(update => 
          update.data.documentId === documentId
        );
    ...
    it('should handle real-time cursor position sharing', async () => {
      ...
      for (let i = 0; i < collaborators.length; i++) {
        collaborators[i].send(JSON.stringify({
          type: 'join_document',
          documentId,
          userId: `user-${i}`,
      ...
      const cursorMovements = [
        ...
        { user: 1, position: { line: 2, column: 10 }, selection: { start: { line: 2, column: 8 }, end: { line: 2, column: 15 } } },
        { user: 2, position: { line: 3, column: 0 }, selection: null },
        { user: 0, position: { line: 1, column: 12 }, selection: null },
        { user: 3, position: { line: 4, column: 20 }, selection: { start: { line: 4, column: 15 }, end: { line: 4, column: 25 } } }
      ];
      
      for (const movement of cursorMovements) {
        collaborators[movement.user].send(JSON.stringify({
          type: 'cursor_move',
          documentId,
      ...
      for (let i = 0; i < collaborators.length; i++) {
        ...
        const receivedCursorUpdates = userCursorUpdates.filter(update => 
          update.data.documentId === documentId && update.data.userId !== `user-${i}`
        );
