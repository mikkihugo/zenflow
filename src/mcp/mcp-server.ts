...
try {
  ...
  MCPMessageHandler = class {
    async handleMessage(_message): any {
  ...
  MCPToolExecutor = class {
    async executeTool(_name, _args): any {
...
export class ClaudeFlowMCPServer {
    ...
    for(const item of batch) {
      ...
      try {
        ...
        this.performanceMetrics.recordRequestStart(requestId, {method = await this.errorHandler.executeWithRetry(
          ...
          {operation = this.errorHandler.createErrorResponse(
          message.id, 
          error, 
          {operation = Date.now() - batchStartTime;
   ...
   */
  async handleMessageError(error, message): any {
    ...
    return {toolExecutionStats = this.performanceMetrics.generateReport();
      console.error(`[${new Date().toISOString()}] INFO [MCP-Server] Final performancereport = this.performanceMetrics ? this.performanceMetrics.getMetrics() : {};
