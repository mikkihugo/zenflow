...
import { ConversationSession, ConversationQuery, ConversationStatus } from '../types.js';
      ...
      const conversations: ConversationSession[] = [
        {
          ...
          status: 'completed',
          context: { goal: 'Test 1', domain: 'testing', constraints: [], resources: [], expertise: [] },
          messages: [],
          outcomes: [],
          metrics: {
            messageCount: 0,
        ...
        },
        {
          ...
          startTime: new Date('2024-01-01T10:00:00Z'),
          status: 'active',
          context: { goal: 'Test 2', domain: 'testing', constraints: [], resources: [], expertise: [] },
          messages: [],
          outcomes: [],
          metrics: {
            messageCount: 0,
        ...
        },
        {
          ...
          status: 'paused',
          context: { goal: 'Test 3', domain: 'testing', constraints: [], resources: [], expertise: [] },
          messages: [],
          outcomes: [],
          metrics: {
            messageCount: 0,
    ...
    it('should handle pagination in agent conversation search', async () => {
      ...
      for (let i = 1; i <= 10; i++) {
        manyConversations.push({
          ...
          status: 'completed',
          context: { goal: `Goal ${i}`, domain: 'testing', constraints: [], resources: [], expertise: [] },
          messages: [],
          outcomes: [],
          metrics: {
            messageCount: 0,
      ...
      const conversations: ConversationSession[] = [
        {
          ...
          startTime: new Date(),
          status: 'completed',
          context: { goal: 'Review code', domain: 'code-review', constraints: [], resources: [], expertise: [] },
          messages: [],
          outcomes: [],
          metrics: {
            messageCount: 0,
        ...
        },
        {
          ...
          startTime: new Date(),
          status: 'active',
          context: { goal: 'Plan sprint', domain: 'planning', constraints: [], resources: [], expertise: [] },
          messages: [],
          outcomes: [],
          metrics: {
            messageCount: 0,
  ...
  describe('✏️ Update Conversation', () => {
    it('should update existing conversation while preserving other data', async () => {
      ...
      const originalConversation: ConversationSession = {
        ...
        startTime: new Date('2024-01-01T10:00:00Z'),
        status: 'active',
        context: { goal: 'Original goal', domain: 'testing', constraints: [], resources: [], expertise: [] },
        messages: [
          {
            id: 'msg-1',
      ...
      const updates = {
        ...
        messages: [
          ...originalConversation.messages,
          {
            ...
            metadata: { priority: 'high' as any, requiresResponse: false, context: {} as any, tags: ['update'] }
          }
        ],
        metrics: {
    ...
    it('should throw error when updating non-existent conversation', async () => {
      // Act & Assert
      await expect(memory.updateConversation('non-existent', { status: 'completed' }))
  ...
  describe('🗑️ Delete Conversation', () => {
    it('should completely remove conversation and its indexes', async () => {
      ...
      const conversation: ConversationSession = {
        ...
        startTime: new Date(),
        status: 'completed',
        context: { goal: 'Delete test', domain: 'deletion-testing', constraints: [], resources: [], expertise: [] },
        messages: [],
        outcomes: [],
        metrics: {
          messageCount: 0,
  ...
  describe('🔢 Classical TDD Patterns - State Verification', () => {
    it('should maintain data consistency across multiple operations', async () => {
      ...
      const conversation: ConversationSession = {
        ...
        startTime: new Date('2024-01-01T10:00:00Z'),
        status: 'active',
        context: { goal: 'Test consistency', domain: 'testing', constraints: [], resources: [], expertise: [] },
        messages: [],
        outcomes: [],
        metrics: {
          messageCount: 0,
