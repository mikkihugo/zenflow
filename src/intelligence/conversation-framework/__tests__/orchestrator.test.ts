...
import { ConversationMemory, ConversationConfig, ConversationSession, ConversationMessage } from '../types.js';
...
describe('ConversationOrchestratorImpl - London TDD', () => {
  ...
  describe('🎯 Create Conversation', () => {
    ...
    it('should reject unknown conversation patterns', async () => {
      ...
      // Act & Assert
      await expect(orchestrator.createConversation(config))
        .rejects.toThrow('Unknown conversation pattern: unknown-pattern');
  ...
  describe('🤝 Join Conversation', () => {
    ...
    it('should reject joining non-existent conversation', async () => {
      ...
      // Act & Assert
      await expect(orchestrator.joinConversation(conversationId, newAgent))
  ...
  describe('💬 Send Message', () => {
    ...
    it('should reject messages from non-participants', async () => {
      // Arrange
      const conversationId = 'conv-123';
      const nonParticipant: AgentId = { id: 'outsider', swarmId: 'swarm-2', type: 'hacker', instance: 0 };
      ...
      await expect(orchestrator.sendMessage(message))
        .rejects.toThrow('Agent outsider is not a participant in this conversation');
      expect(mockMemory.updateConversation).not.toHaveBeenCalled();
    });
  });
  ...
  describe('🔚 Terminate Conversation', () => {
    it('should finalize conversation and generate outcomes', async () => {
      ...
      const existingSession: ConversationSession = {
        ...
        status: 'active',
        context: { goal: 'Review code', domain: 'backend', constraints: [], resources: [], expertise: [] },
        messages: [
          {
            id: 'msg-1',
            conversationId,
