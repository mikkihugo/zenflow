...
import { ReinforcementLearningEngine, NeuralNetworkPredictor, EnsembleModels, OnlineLearningSystem } from '../ml-integration.js';
import type {
  ExecutionData,
...
describe('Adaptive Learning System - London TDD (Integration & Interactions)', () => {
  ...
  describe('PatternRecognitionEngine Integration', () => {
    ...
    test('should emit patternsAnalyzed event with correct data', async () => {
      ...
      expect(mockEmit).toHaveBeenCalledWith('patternsAnalyzed', 
    ...
    test('should emit taskPatternClassified event when classifying task completion', () => {
      ...
      const taskResult = {
        ...
        quality: 0.9,
        resourceUsage: {
          cpu: 0.7, memory: 0.6, network: 0.3, diskIO: 0.4, bandwidth: 50, latency: 120
        },
      ...
      expect(mockEmit).toHaveBeenCalledWith('taskPatternClassified',
        expect.objectContaining({
          taskId: 'task_1',
    ...
    test('should emit communicationPatternsDetected when detecting patterns', () => {
      ...
      const messages = [
        {
          id: 'msg_1', from: 'agent_1', to: 'agent_2', type: 'task_request',
          payload: {}, timestamp: Date.now(), priority: 'medium' as const,
          latency: 100, size: 256
        },
        {
          id: 'msg_2', from: 'agent_1', to: 'agent_2', type: 'task_request',
          payload: {}, timestamp: Date.now(), priority: 'medium' as const,
          latency: 120, size: 300
        }
      ];

      engine.detectCommunicationPatterns(messages);

      expect(mockEmit).toHaveBeenCalledWith('communicationPatternsDetected',
        expect.objectContaining({
          patterns: expect.any(Number),
          messages: 2,
          timestamp: expect.any(Number)
  ...
  describe('LearningCoordinator Integration', () => {
    ...
    test('should emit learningCompleted event after coordinating learning', async () => {
      ...
      const agents: Agent[] = [
        {
          id: 'agent_1', type: 'worker', capabilities: ['processing'],
          currentLoad: 0.5, performance: { efficiency: 0.8, latency: 100, errorRate: 0.1,
            resourceUtilization: { cpu: 0.7, memory: 0.6, network: 0.3, diskIO: 0.4, bandwidth: 50, latency: 120 },
            throughput: 10, quality: 0.9 },
          specializations: ['data_processing'],
          learningProgress: { totalExperience: 100, domainsLearned: ['processing'],
            currentLearningRate: 0.1, knowledgeRetention: 0.9, adaptability: 0.8 }
        }
      ];

      await coordinator.coordinateLearning(agents);

      expect(mockEmit).toHaveBeenCalledWith('learningCompleted',
        expect.objectContaining({
          agents: ['agent_1'],
    ...
    test('should emit knowledgeAdded when updating knowledge base with new patterns', async () => {
      ...
      const patterns = [
        {
          id: 'pattern_1', type: 'optimization' as const, data: { test: true },
          confidence: 0.8, frequency: 5, context: { test: true },
          metadata: { complexity: 0.5, predictability: 0.8, stability: 0.9, anomalyScore: 0.1,
            correlations: [], quality: 0.9, relevance: 0.8 },
          timestamp: Date.now()
        }
      ];
      ...
      expect(mockEmit).toHaveBeenCalledWith('knowledgeAdded',
        expect.objectContaining({
          patternId: 'pattern_1',
          type: 'optimization',
          confidence: 0.8,
    ...
    test('should emit bestPracticeIdentified when emerging best practices', () => {
      ...
      const successes: SuccessPattern[] = [
        {
          id: 'success_1', context: 'data_processing',
          actions: ['validate_input', 'process_data', 'verify_output'],
          outcomes: { efficiency: 0.9, quality: 0.95, cost: 0.7, satisfaction: 0.8, sustainability: 0.85 },
          reproducibility: 0.9, conditions: ['adequate_memory', 'low_network_load']
        }
      ];

      const practices = coordinator.emergeBestPractices(successes);

      expect(mockEmit).toHaveBeenCalledWith('bestPracticeIdentified',
        expect.objectContaining({
  ...
  describe('PerformanceOptimizer Integration', () => {
    ...
    test('should emit behaviorOptimized when optimizing agent behavior', () => {
      ...
      const patterns = [
        {
          id: 'pattern_1', type: 'task_completion' as const, data: { test: true },
          confidence: 0.8, frequency: 5, context: { agentId: 'agent_1' },
          metadata: { complexity: 0.5, predictability: 0.8, stability: 0.9, anomalyScore: 0.1,
            correlations: [], quality: 0.9, relevance: 0.8 },
          timestamp: Date.now()
        }
      ...
      expect(mockEmit).toHaveBeenCalledWith('behaviorOptimized',
        expect.objectContaining({
          agentId: 'agent_1',
    ...
    test('should emit allocationOptimized when optimizing task allocation', () => {
      ...
      const tasks: Task[] = [
        {
          id: 'task_1', type: 'processing', priority: 0.8, complexity: 0.6,
          requirements: ['cpu'], constraints: [], estimatedDuration: 1000, dependencies: []
        }
      ];

      const agents: Agent[] = [
        {
          id: 'agent_1', type: 'worker', capabilities: ['cpu', 'processing'],
          currentLoad: 0.5, performance: { efficiency: 0.8, latency: 100, errorRate: 0.1,
            resourceUtilization: { cpu: 0.7, memory: 0.6, network: 0.3, diskIO: 0.4, bandwidth: 50, latency: 120 },
            throughput: 10, quality: 0.9 },
      ...
      expect(mockEmit).toHaveBeenCalledWith('allocationOptimized',
        expect.objectContaining({
          tasks: 1,
          agents: 1,
  ...
  describe('ML Integration Interactions', () => {
    ...
    test('ReinforcementLearningEngine should emit events during training', () => {
      ...
      expect(mockEmit).toHaveBeenCalledWith('qValueUpdated',
        expect.objectContaining({
          state: 'state1',
          action: 'action1',
    ...
    test('NeuralNetworkPredictor should emit training events', async () => {
      ...
      expect(mockEmit).toHaveBeenCalledWith('trainingCompleted',
        expect.objectContaining({
    ...
    test('EnsembleModels should emit model management events', () => {
      ...
      expect(mockEmit).toHaveBeenCalledWith('modelAdded',
        expect.objectContaining({
...
describe('Adaptive Learning System - Classical TDD (Algorithms & Math)', () => {
  ...
  describe('Pattern Recognition Mathematical Operations', () => {
    ...
    test('should calculate communication efficiency correctly', () => {
      ...
      const messages = [
        {
          id: 'msg_1', from: 'agent_1', to: 'agent_2', type: 'request',
          payload: {}, timestamp: Date.now(), priority: 'medium' as const,
          latency: 100, size: 1000
        },
        {
          id: 'msg_2', from: 'agent_1', to: 'agent_2', type: 'request',
          payload: {}, timestamp: Date.now(), priority: 'medium' as const,
          latency: 200, size: 2000
        }
      ];
  ...
  describe('Online Learning Adaptation Algorithm', () => {
    ...
    test('should adapt learning rate based on performance degradation', async () => {
      ...
      const goodData = {
        ...createMockExecutionData()[0],
        success: true,
        resourceUsage: { cpu: 0.5, memory: 0.5, network: 0.3, diskIO: 0.3, bandwidth: 50, latency: 100 }
      };
      
      const badData = {
        ...createMockExecutionData()[0], 
        success: false,
        resourceUsage: { cpu: 0.9, memory: 0.9, network: 0.8, diskIO: 0.8, bandwidth: 10, latency: 1000 }
      };
...
describe('Adaptive Learning System - Integration Tests', () => {
  ...
  test('should demonstrate end-to-end learning improvement', async () => {
    ...
    const agents: Agent[] = [
      {
        id: 'agent_1', type: 'worker', capabilities: ['processing'],
        currentLoad: 0.5, performance: { efficiency: 0.6, latency: 200, errorRate: 0.3,
          resourceUtilization: { cpu: 0.7, memory: 0.6, network: 0.3, diskIO: 0.4, bandwidth: 50, latency: 120 },
          throughput: 5, quality: 0.7 },
        specializations: ['data_processing'],
        learningProgress: { totalExperience: 50, domainsLearned: ['processing'],
          currentLearningRate: 0.1, knowledgeRetention: 0.9, adaptability: 0.7 }
      }
    ];
