...
async function main() {
  ...
  try {
    ...
    console.log(`- DAL Factory: ${healthResult.components.dalFactory ? '✅' : '❌'}`);
    console.log(`- Hybrid Manager: ${healthResult.components.hybridManager ? '✅' : '❌'}`);
    console.log(`- ADR Manager: ${healthResult.components.adrManager ? '✅' : '❌'}`);
    console.log(`- Vector Search: ${healthResult.components.vectorSearch ? '✅' : '❌'}`);
    ...
    if (healthResult.healthy) {
      ...
      const testADR = await adrManager.createADR({
        ...
        context: 'Testing the cleaned hybrid database architecture without legacy dependencies.',
        decision: 'Use the new factory-based approach for all hybrid database operations.',
        consequences: 'Cleaner architecture, no circular dependencies, better maintainability.',
      ...
      const searchResults = await adrManager.semanticSearchADRs('hybrid architecture', {
        limit: 5,
