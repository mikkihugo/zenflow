...
async function analyzeViolations() {
  ...
  try {
    ...
    const simpleFiles = ['scripts/ai-eslint/test-docker-fixer.js'].filter((f) => fs.existsSync(f));
...
function processViolationsDemo(violations) {
  ...
  if (grouped.GROUP_2.length > 0) {
    ...
    grouped.GROUP_2.slice(0, 3).forEach((v) => {
      ...
      console.log(
        `  ${success ? '✅' : '❌'} ${v.rule} in ${path.basename(v.file)}:${v.line} ${success ? '(AI fixed)' : '(Context needed)'}`
      );
    });
  }
  ...
  if (grouped.GROUP_1.length > 0) {
    ...
    grouped.GROUP_1.forEach((v) => {
      console.log(
        `  📋 ${v.rule} in ${path.basename(v.file)}:${v.line} (Analysis report generated)`
      );
    });
  }
