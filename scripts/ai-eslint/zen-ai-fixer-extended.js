...
console.log('=======================================');
console.log(
  `⏱️  Extended timeouts: ${EXTENDED_TIMEOUTS.INACTIVITY_TIMEOUT / 60000}min inactivity, ${EXTENDED_TIMEOUTS.MAX_TOTAL_TIMEOUT / 60000}min max`
);
...
class ExtendedTimeoutViolationFixer {
  ...
  async applyExtendedContextAwareFix(violation) {
    ...
    console.log(
      `    ⏱️  Extended timeouts: ${EXTENDED_TIMEOUTS.INACTIVITY_TIMEOUT / 60000}min inactivity, ${EXTENDED_TIMEOUTS.MAX_TOTAL_TIMEOUT / 60000}min total`
    );
      ...
      const result = await new Promise((resolve, reject) => {
        ...
        const resetInactivityTimeout = () => {
          ...
          inactivityTimeoutHandle = setTimeout(() => {
            ...
            console.log(
              `    ⏱️  Claude CLI inactive for ${inactiveDuration}s - terminating (extended timeout)`
            );
            claude.kill('SIGTERM');
            reject(
              new Error(
                `Claude CLI extended inactivity timeout after ${EXTENDED_TIMEOUTS.INACTIVITY_TIMEOUT / 1000}s`
              )
            );
        ...
        maxTimeoutHandle = setTimeout(() => {
          ...
          reject(
            new Error(
              `Claude CLI reached extended maximum timeout of ${EXTENDED_TIMEOUTS.MAX_TOTAL_TIMEOUT / 60000} minutes`
            )
          );
...
Begin fixing this violation now - you have extended time to work carefully.`;
  ...
  showProgress() {
    ...
    const successRate =
      this.processedCount > 0 ? Math.round((this.fixedCount / this.processedCount) * 100) : 0;
  ...
  try {
    ...
    for (const file of results) {
      if (file.messages && file.messages.length > 0) {
        for (const message of file.messages.slice(0, 3)) {
          // Limit to 3 per file for testing
          violations.push({
            file: file.filePath,
    ...
    for (const [index, violation] of testViolations.entries()) {
      ...
      if (index < testViolations.length - 1) {
        console.log(
          `   ⏸️  Pausing ${EXTENDED_TIMEOUTS.BATCH_PAUSE / 1000}s before next violation...\n`
        );
