...
class DocsBuilder {
  ...
  async runJSDocValidation() {
    ...
    if (this.buildConfig.validation.failOnError && validationResults.quality.failed > 0) {
      throw new Error(`JSDoc validation failed: ${validationResults.quality.failed} files have errors`);
    }
  ...
  async generateTypeDocAPI() {
    ...
    try {
      ...
      console.log(`📖 TypeDoc generated: ${stats.modules} modules, ${stats.classes} classes, ${stats.interfaces} interfaces`);
  ...
  async generateCoverageReport() {
    ...
    console.log(`📈 Coverage report generated: ${coverageResults.overall.percentage.toFixed(1)}% overall coverage`);
...
<body>
    ...
    <div class="steps">
        ...
        <table>
            ...
            <tbody>
                ${Object.entries(this.results.steps).map(([step, result]) => `
                <tr>
                    <td>${step}</td>
...
${this.results.validation ? `## Validation Results
...
${this.results.coverage ? `## Coverage Results
