...
module.exports = function transformer(file, api, options) {
  ...
  function shouldTransformObject(objectName) {
    ...
    const builtinObjects = [
      'Object', 'Array', 'String', 'Number', 'Boolean', 'Date', 'RegExp', 'Promise', 'JSON', 'Math',
      'console', 'window', 'document', 'global', 'process', // Node.js and browser globals
      'Buffer', 'URL', 'URLSearchParams', 'fetch', 'Request', 'Response', // Web APIs
      'Error', 'TypeError', 'ReferenceError', 'SyntaxError', // Error types
      'Set', 'Map', 'WeakSet', 'WeakMap', 'Symbol', 'BigInt', // ES6+ types
      'Intl', 'Proxy', 'Reflect', 'ArrayBuffer', 'DataView' // Advanced types
    ];
  ...
  source.find(j.MemberExpression, {
    ...
    const safeProperties = [
      ...
      'forEach', 'map', 'filter', 'reduce', 'find', 'includes', 'indexOf', 'slice',
      'push', 'pop', 'shift', 'unshift', 'splice', 'concat', 'join',
      'entries', 'keys', 'values', 'hasOwnProperty',
      'id', 'name', 'type', 'className', 'innerHTML', 'innerText', 'style',
      'default', 'exports', // Common module properties
      'log', 'warn', 'error', 'info', 'debug', // Console methods
      'title', 'description', 'context' // Common config properties
    ];
    ...
    if (shouldTransform) {
      ...
      if (options.verbose) {
        console.log(`Transformed: ${objectName || 'obj'}.${propertyName} → ${objectName || 'obj'}['${propertyName}'] (${reason})`);
      }
    }
  });
  ...
  source.find(j.MemberExpression, {
    ...
    if (shouldTransformObject(objectName) || shouldTransformProperty(propertyName) ||
        propertyName.includes('-') || /^[A-Z_]+$/.test(propertyName)) {
      ...
      if (options.verbose) {
        console.log(`Transformed: ${objectName || 'obj'}?.${propertyName} → ${objectName || 'obj'}?.['${propertyName}']`);
      }
    }
  });
