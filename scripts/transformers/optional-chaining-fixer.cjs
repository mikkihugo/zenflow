...
module.exports = function transformer(file, api, options) {
  ...
  function shouldUseOptionalChaining(path) {
    ...
    if (property && ['constructor', 'toString', 'valueOf', 'hasOwnProperty'].includes(property.name)) {
      return false;
    }
    ...
    if (object.type === 'Identifier' && 
        ...
        property && 
        (property.name === 'instance' || property.name === 'getInstance' || property.name === 'create')) {
      return false;
    }
  ...
  // Transform member expressions to use optional chaining
    ...
    if (path.value.object.type === 'MemberExpression') {
      ...
      if (rootObject.type === 'Identifier' && 
        ...
        if (!rootAccess.optional) {
          ...
          if (dryRun) {
            console.log(`[DRY RUN] Would transform parameter access: ${before} → ${after} (line ${line})`);
          }
        }
      }
    }
