  ...
  if (hasJSX) {
    ...
    reactImports.forEach(path => {
      ...
      if (defaultImport && defaultImport.local.name === 'React') {
        ...
        const explicitReactUsage = source.find(j.MemberExpression).filter(memberPath => {
          return memberPath.value.object.type === 'Identifier' && 
...
function isObjectPropertyKey(path) {
  ...
  return parentValue.type === 'Property' && 
