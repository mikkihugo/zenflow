...
class TemplateValidator {
  ...
  async validateTemplates() {
    ...
    try {
      ...
      console.log(`✅ Template validation completed: ${this.results.templates.valid}/${files.length} files valid`);
   ...
   * ```
   */
  async validateExampleCode(comment, fileResult) {
    ...
    while ((match = exampleRegex.exec(comment.content)) !== null) {
      ...
      try {
        ...
        fileResult.examples.failed++;
        fileResult.issues.push(`Line ${comment.startLine}: Example validation error: ${error.message}`);
      }
    }
  }
  ...
  validateTemplateFormat(comment, template, fileResult) {
    ...
    const hasDescription = comment.content.includes('* @description') || 
                          comment.content.match(/^\s*\*\s*[A-Z]/m);
...
`;
  ...
  generateRecommendations() {
    ...
    if (this.results.templates.invalid > 0) {
      recommendations.push('📝 **Fix Template Issues**: Address invalid JSDoc templates to improve documentation consistency.');
    }
    
    if (this.results.examples.failed > 0) {
      recommendations.push('🔧 **Fix Example Code**: Update failing example code to ensure all examples are syntactically correct.');
    }
    
    if (!this.autoFix) {
      recommendations.push('⚡ **Enable Auto-fix**: Consider using --auto-fix flag to automatically resolve common template issues.');
    }
    ...
    if (layersWithIssues.length > 0) {
      recommendations.push(`🎯 **Focus on Layers**: ${layersWithIssues.map(l => l.toUpperCase()).join(', ')} have template validation issues that need attention.`);
    }
    
    if (recommendations.length === 0) {
      recommendations.push('✅ **Excellent Templates**: All JSDoc templates meet the validation standards. Maintain current quality.');
    }
...
async function main() {
  ...
  try {
    ...
    console.log(`   Example Success Rate: ${results.examples.validated > 0 ? ((results.examples.passed / results.examples.validated) * 100).toFixed(1) : 0}%`);
