...
class CodeQualityAnalyzer {
  ...
  analyzeFunctions(content, lines) {
    ...
    const functionRegex = /(async\s+)?function\s+(\w+)|(\w+)\s*[=:]\s*(async\s+)?\([^)]*\)\s*[=>{]/g;
    let match;
  ...
  checkNamingConventions(content) {
    ...
    const genericNames = [
      'data', 'info', 'item', 'obj', 'temp', 'tmp', 'value', 'val',
      'result', 'response', 'request', 'params', 'options', 'config',
      'manager', 'handler', 'service', 'util', 'helper'
    ];
    ...
    while ((match = varRegex.exec(content)) !== null) {
      ...
      if (varName.length < QUALITY_THRESHOLDS.MIN_NAME_LENGTH && !['i', 'j', 'k', 'id'].includes(varName)) {
        const lineNumber = content.substring(0, match.index).split('\n').length;
        this.addViolation(
          'short-name',
...
function analyzeBulk(pattern = 'src/**/*.ts') {
  ...
  for (const file of files) {
    ...
    if (report.violations.length > 0) {
      ...
      console.log(`   Violations: ${report.summary.totalViolations} (🔴 ${report.summary.highSeverity} 🟡 ${report.summary.mediumSeverity} ⚪ ${report.summary.lowSeverity})`);
