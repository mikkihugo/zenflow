...
class JSDocValidator {
  ...
  async validateFile(filePath, layer) {
    ...
    try {
      ...
      fileResults.coverage.percentage = fileResults.coverage.total > 0 ? 
        (fileResults.coverage.documented / fileResults.coverage.total * 100).toFixed(2) : 0;
  ...
  calculateMetrics() {
    ...
    this.results.coverage.documented = this.results.files.reduce(
      (sum, file) => sum + file.coverage.documented, 0
    );
    this.results.coverage.total = this.results.files.reduce(
      (sum, file) => sum + file.coverage.total, 0
    );
    this.results.coverage.percentage = this.results.coverage.total > 0 ? 
      (this.results.coverage.documented / this.results.coverage.total * 100).toFixed(2) : 0;
    ...
    this.results.quality.score = totalFiles > 0 ? 
      (this.results.quality.passed / totalFiles * 100).toFixed(2) : 0;
    ...
    for (const layer of this.layers) {
      ...
      layerData.coverage.percentage = layerData.coverage.total > 0 ? 
        (layerData.coverage.documented / layerData.coverage.total * 100).toFixed(2) : 0;
      ...
      layerData.quality.score = layerFiles > 0 ? 
        (layerData.quality.passed / layerFiles * 100).toFixed(2) : 0;
    }
  }
...
${file.warnings.length > 0 ? `#### Warnings  
...
`;
  ...
  generateRecommendations() {
    ...
    if (parseFloat(this.results.coverage.percentage) < 80) {
      recommendations.push('📈 **Improve Documentation Coverage**: Current coverage is below 80%. Focus on adding JSDoc to undocumented classes, interfaces, and functions.');
    }
    
    if (this.results.quality.failed > 0) {
      recommendations.push('🔧 **Fix Quality Issues**: Address JSDoc validation errors to improve overall quality score.');
    }
    ...
    for (const layer of this.layers) {
      ...
      if (parseFloat(layerData.coverage.percentage) < 70) {
        recommendations.push(`🎯 **${layer.toUpperCase()} Focus**: ${this.getLayerName(layer)} has low coverage (${layerData.coverage.percentage}%). Prioritize documentation in this layer.`);
      }
    }
    
    if (recommendations.length === 0) {
      recommendations.push('✅ **Excellent Documentation**: All validation metrics are within acceptable ranges. Continue maintaining high documentation standards.');
    }
