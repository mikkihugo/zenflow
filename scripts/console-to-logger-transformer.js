/**
 * jscodeshift transformer to convert console statements to structured logging
 */
...
module.exports = function transformer(file, api) {
  ...
  source.find(j.ImportDeclaration).forEach((path) => {
    if (
      path.value.source.value.includes('logging-config') ||
  ...
  if (hasConsoleUsage && !loggerImported) {
    // Add require statement for CommonJS or import for ES modules
    const requireStatement = j.variableDeclaration('const', [
      j.variableDeclarator(
        j.objectPattern([j.property('init', j.identifier('getLogger'), j.identifier('getLogger'))]),
        j.callExpression(j.identifier('require'), [j.literal('../config/logging-config')])
      ),
    ]);
    ...
    body.value.unshift(requireStatement);
  }
