...
class DeadCodeAnalyzer {
          ...
          if (match) {
            if (match[1] && match[2]) {
              ...
              exportItems.forEach(item => {
                ...
                if (cleanItem && !cleanItem.includes('{') && !cleanItem.includes('}')) {
      ...
      lines.forEach(line => {
        ...
        for (const pattern of importPatterns) {
          ...
          if (match && match[1]) {
            if (match[1].includes(',')) {
              // Multiple imports
              const imports = match[1].split(',').map(item => 
                item.replace(/\s+as\s+\w+/, '').trim()
              );
              imports.forEach(imp => {
    ...
    for (const [exportName, exportInfo] of this.exports) {
      if (!this.imports.has(exportName)) {
        ...
        this.potentialDeadCode.push({
          ...
          ...exportInfo,
          usageCount,
          confidence: this.calculateConfidence(exportName, exportInfo, usageCount)
        });
      }
    }
  ...
  countUsages(name) {
    try {
      ...
      return parseInt(result.trim()) - 1; // Subtract the export declaration
    } catch (error) {
      return 0;
    }
  }
  ...
  calculateConfidence(name, info, usageCount) {
    ...
    else if (usageCount === 1) confidence += 0.2; // Only the export itself
  ...
  generateReport() {
    ...
    const mediumConfidence = deadCode.filter(item => 
    ...
    if (mediumConfidence.length > 0) {
      ...
      mediumConfidence.forEach(item => {
        console.log(`📍 ${item.name} (${item.type}) - ${item.file}:${item.line} - ${(item.confidence * 100).toFixed(1)}%`);
      });
      console.log('');
    }
