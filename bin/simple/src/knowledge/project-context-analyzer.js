...
export class ProjectContextAnalyzer extends EventEmitter {
        ...
        try {
            switch (context.monorepo.type) {
                ...
                case 'rush': {
                    ...
                    context.monorepo.packages =
                        rushConfig?.projects?.map((p) => p.projectFolder) || [];
                    break;
                }
                case 'bazel': {
    ...
    async runBazelQuery(rootPath) {
        try {
            ...
            const { stdout: targetsOutput } = await execAsync('bazel query "//..." --output=build 2>/dev/null', { cwd: rootPath, timeout: 30000 });
            const { stdout: depsOutput } = await execAsync('bazel query "deps(//...)" --output=graph 2>/dev/null', { cwd: rootPath, timeout: 30000 });
        ...
        for (const block of ruleBlocks) {
            try {
                ...
                if (target.name) {
                    ...
                    const packagePath = packageMatch && packageMatch[1]
    ...
    async detectAPIs(context) {
        const apiDeps = context.dependencies.filter((dep) => dep.name.includes('api') ||
