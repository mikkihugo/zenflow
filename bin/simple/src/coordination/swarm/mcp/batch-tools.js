export const batchExecuteTool = {
    ...
    async handler(params) {
        try {
            ...
            const sequentialTime = operationCount * 200;
            const speedImprovement = sequentialTime / executionTime;
            const tokenReduction = Math.min(85, Math.max(15, (operationCount - 1) * 12));
...
export const projectInitBatchTool = {
    ...
    async handler(params) {
        try {
            ...
            const executionTime = Date.now() - startTime;
            const estimatedSequentialTime = operations.length * 300;
