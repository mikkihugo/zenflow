...
import { AgentError, ErrorFactory, NeuralError, PersistenceError, ResourceError, SwarmError, TaskError, ValidationError, MCPParameterValidator as ValidationUtils, WasmError, ZenSwarmError, } from './error-handler.ts';
...
class EnhancedMCPTools {
    ...
    constructor(ruvSwarmInstance = null) {
        ...
        this.errorContext = {
            data: new Map(),
            set: (key, value) => this.errorContext.data.set(key, value),
        ...
        this.logger = getLogger('mcp-tools');
        ...
        register({
            ...
            handler: this.agent_list.bind(this),
        });
        register({
            id: 'agent_metrics',
        ...
        register({
            ...
            handler: this.pool_stats.bind(this),
        });
        register({
    ...
    determineSeverity(error) {
        ...
        if (error instanceof WasmError || error instanceof ResourceError) {
            return 'high';
        }
        ...
        if (error instanceof TaskError && error.message.includes('timeout')) {
            return 'medium';
        }
    ...
    isRecoverable(error) {
        ...
        if (error instanceof AgentError && error.message.includes('busy')) {
            return true;
        }
        if (error instanceof PersistenceError && error.message.includes('locked')) {
            return true;
        }
        return false;
    }
    validateToolParams(params, toolName) {
        try {
            ...
            throw ErrorFactory.createError('validation', `Parameter validation failed for ${toolName}: ${error.message}`, { tool: toolName, originalError: error });
        }
    }
    ...
    async integrateHookNotifications(hookInstance) {
        ...
        try {
            ...
            this.logger.info(`🔗 Integrated ${runtimeNotifications.length} hook notifications into MCP memory`);
            return true;
        }
        catch (error) {
    ...
    async getCrossAgentNotifications(agentId = null, type = null, since = null) {
        ...
        try {
            ...
            for (const agent of allAgents) {
                ...
                this.logger.error(`🔍 Debug: Agent ${agent} has ${agentNotifications.length} notification memories`);
                notifications.push(...agentNotifications);
            }
            return notifications.sort((a, b) => b.timestamp - a.timestamp);
        }
        catch (error) {
            this.logger.error('❌ Failed to retrieve cross-agent notifications:', error.message);
            return [];
        }
    }
    async getActiveAgentIds() {
        try {
            ...
            this.logger.error('🔍 Debug: Total unique active agent IDs:', uniqueAgentIds);
            return uniqueAgentIds;
        }
        catch (error) {
            this.logger.error('❌ Failed to get active agent IDs:', error.message);
            return [];
        }
    }
    ...
    async loadExistingSwarms() {
        try {
            if (!this.persistence) {
                this.logger.warn('Persistence not available, skipping swarm loading');
                return;
            }
            ...
            this.logger.warn('Failed to load existing swarms:', error.message);
        }
    }
    async swarm_init(params) {
        ...
        try {
            ...
            const { topology, maxAgents, strategy, enableCognitiveDiversity, enableNeuralAgents, enableForecasting, } = validatedParams;
            ...
            const result = {
                ...
                maxAgents,
                features: {
                    cognitive_diversity: enableCognitiveDiversity &&
                        this.ruvSwarm?.features.cognitive_diversity,
                ...
                performance: {
                    ...
                    memory_usage_mb: (this.ruvSwarm?.wasmLoader?.getTotalMemoryUsage() || 0) /
                        (1024 * 1024),
                },
            };
            ...
            try {
                ...
                this.logger.debug('Swarm persisted to database', {
                    swarmId: swarm?.id,
                });
            }
            catch (error) {
                if (error.message.includes('UNIQUE constraint failed')) {
                    this.logger.debug('Swarm already exists in database', {
                        swarmId: swarm?.id,
                    });
                }
                else {
                    this.logger.error('Failed to persist swarm', {
                        error,
                        swarmId: swarm?.id,
                    });
                    throw error;
            ...
            if (error.message.includes('WASM') || error.message.includes('module')) {
                handledError = ErrorFactory.createError('wasm', `WASM module error during swarm initialization: ${error.message}`, {
                ...
                handledError = ErrorFactory.createError('resource', `Insufficient resources for swarm initialization: ${error.message}`, {
                    resourceType: 'memory',
                    operation: 'swarm_init',
                ...
                handledError = ErrorFactory.createError('persistence', `Database error during swarm creation: ${error.message}`, { operation: 'create_swarm', originalError: error });
            }
            else if (!(error instanceof ValidationError || error instanceof ZenSwarmError)) {
                handledError = ErrorFactory.createError('swarm', `Swarm initialization failed: ${error.message}`, { operation: 'swarm_init', originalError: error });
            }
    ...
    async agent_spawn(params) {
        ...
        try {
            ...
            if (!swarm) {
                throw ErrorFactory.createError('swarm', 'No active swarm found. Please initialize a swarm first using swarm_init.', { operation: 'agent_spawn', requestedSwarmId: swarmId });
            }
            if (swarm?.agents && swarm?.agents.size >= (swarm.maxAgents || 100)) {
                throw ErrorFactory.createError('swarm', `Swarm has reached maximum capacity of ${swarm.maxAgents || 100} agents`, {
            ...
            if (error.message.includes('neural') ||
                ...
                handledError = ErrorFactory.createError('neural', `Neural network error during agent spawn: ${error.message}`, {
                    operation: 'agent_spawn',
                ...
                handledError = ErrorFactory.createError('agent', `Agent capability error: ${error.message}`, {
                    operation: 'agent_spawn',
                    agentType: params?.type,
                ...
                handledError = ErrorFactory.createError('persistence', `Database error during agent creation: ${error.message}`, {
    ...
    async task_orchestrate(params) {
        ...
        try {
            ...
            this.errorContext.set('startTime', startTime);
            const { task, priority, strategy, maxAgents, swarmId, requiredCapabilities, estimatedDuration, } = validatedParams;
            const swarm = swarmId
                ? this.activeSwarms.get(swarmId)
                : this.activeSwarms.values().next().value;
            if (!swarm) {
            ...
            let handledError = error;
            if (error.message.includes('swarm') &&
                error.message.includes('not found')) {
                handledError = ErrorFactory.createError('swarm', `Swarm not found for task orchestration: ${error.message}`, {
                ...
                handledError = ErrorFactory.createError('agent', `No suitable agents available for task: ${error.message}`, {
                    operation: 'task_orchestrate',
                    task: params?.task,
                ...
                handledError = ErrorFactory.createError('task', `Task orchestration timeout: ${error.message}`, {
                    operation: 'task_orchestrate',
                    task: params?.task,
                ...
                handledError = ErrorFactory.createError('task', `Task orchestration failed: ${error.message}`, {
                    operation: 'task_orchestrate',
                    task: params?.task,
                    originalError: error,
                });
    ...
    async task_results(params) {
        ...
        try {
            ...
            let dbTask = null;
            try {
                dbTask = this.persistence?.getTask
                    ? this.persistence?.getTask(taskId)
                    : null;
            }
            catch (error) {
            ...
            try {
                ...
                this.logger.warn('Database task results lookup failed:', error.message);
                dbTaskResults = [];
            }
            const results = {
                task_id: taskId,
            ...
            if (format === 'summary') {
                const summary = {
                    ...
                    completion_time: results?.execution_time_ms ||
                        results?.execution_summary?.duration_ms,
    ...
    calculateEfficiencyScore(results) {
        ...
        const factors = {
            ...
            speed: Math.max(0, 1.0 - results?.execution_time_ms / 60000),
            resource_usage: results?.aggregated_performance?.total_memory_usage_mb < 100
                ? 1.0
                : 0.7,
    ...
    async benchmark_run(params) {
        const startTime = performance.now();
        try {
            const { type = 'all', iterations = 10, includeNeuralBenchmarks = true, includeSwarmBenchmarks = true, } = params;
            const benchmarks = {};
            if (type === 'all' || type === 'wasm') {
                benchmarks.wasm = await this.runWasmBenchmarks(iterations);
            }
            if (type === 'all' || type === 'neural') {
                if (includeNeuralBenchmarks &&
            ...
            if (type === 'all' || type === 'swarm') {
                if (includeSwarmBenchmarks) {
                    this.logger.error('Running swarm benchmarks with iterations:', iterations);
                    try {
                        benchmarks.swarm = await this.runSwarmBenchmarks(iterations);
                        this.logger.error('Swarm benchmarks result:', benchmarks.swarm);
                    }
                    catch (error) {
                        this.logger.error('Swarm benchmark error:', error);
                        benchmarks.swarm = {
        ...
        try {
            ...
            const features = {
                ...
                wasm: {
                    ...
                    total_memory_mb: (this.ruvSwarm?.wasmLoader?.getTotalMemoryUsage() || 0) /
                        (1024 * 1024),
                    simd_support: this.ruvSwarm?.features.simd_support,
                },
                ...
                neural_networks: {
                    ...
                    activation_functions: this.ruvSwarm?.features.neural_networks
                        ? 18
                        : 0,
                ...
                cognitive_diversity: {
                    ...
                    patterns_available: this.ruvSwarm?.features.cognitive_diversity
                        ? 5
                        : 0,
    ...
    async neural_train(params) {
        ...
        try {
            ...
            const { agentId, iterations: rawIterations, learningRate = 0.001, modelType = 'feedforward', trainingData = null, } = params;
            if (!agentId || typeof agentId !== 'string') {
                throw ErrorFactory.createError('validation', 'agentId is required and must be a string. The agentId parameter identifies which agent to train.', {
                    parameter: 'agentId',
                    providedValue: String(agentId),
                    expectedType: 'string',
                });
            }
            let iterations;
            try {
                iterations = rawIterations
                    ? Number.parseInt(String(rawIterations), 10)
                    : 10;
                if (Number.isNaN(iterations) || iterations < 1) {
                    throw ErrorFactory.createError('validation', 'iterations must be a positive integer greater than 0', {
                ...
                if (iterations > 100) {
                    throw ErrorFactory.createError('validation', 'iterations must not exceed 100 to prevent excessive training time', {
                        parameter: 'iterations',
                        providedValue: String(iterations),
                        validRange: '1-100',
                    });
                }
            }
            ...
            try {
                validatedLearningRate = Number.parseFloat(String(learningRate));
                if (Number.isNaN(validatedLearningRate) ||
                    validatedLearningRate <= 0 ||
                    validatedLearningRate > 1.0) {
                    throw ErrorFactory.createError('validation', 'learningRate must be a number between 0.0001 and 1.0', {
                        parameter: 'learningRate',
                        providedValue: learningRate,
                        validRange: '0.0001-1.0',
                    });
                }
            ...
            if (!validModelTypes.includes(modelType)) {
                throw ErrorFactory.createError('validation', `modelType must be one of: ${validModelTypes.join(', ')}`, {
                    parameter: 'modelType',
                    providedValue: modelType,
                    validOptions: validModelTypes,
                });
            }
            ...
            try {
                neuralNetworks =
                    (await this.persistence?.getAgentNeuralNetworks(agentId)) || [];
            }
            catch (_error) {
            }
            let neuralNetwork = neuralNetworks[0] || null;
            if (!neuralNetwork) {
                try {
            ...
            for (let i = 1; i <= iterations; i++) {
                ...
                if (this.ruvSwarm?.wasmLoader?.modules?.get('core')?.neural_train) {
                    try {
                        ...
                        this.logger.warn('WASM neural training failed:', wasmError.message);
                    }
                }
            }
            ...
            try {
                await this.persistence?.updateNeuralNetwork(neuralNetwork.id, {
                    ...
                    weights: {
                        ...
                        iterations,
                        timestamp: new Date().toISOString(),
                        values: this.ruvSwarm.wasmLoader.modules
                            .get('core')
            ...
            const result = {
                ...
                training_history: trainingResults.slice(-5),
                performance_metrics: performanceMetrics,
            };
    ...
    async runWasmBenchmarks(iterations) {
        ...
        for (let i = 0; i < iterations; i++) {
            try {
                ...
                if (!coreModule.isPlaceholder) {
                    ...
                    const nn = coreModule.exports.create_neural_network(layers, 1);
                ...
                this.logger.warn(`WASM benchmark iteration ${i} failed:`, error.message);
            }
        }
        ...
        results.neural_networks = {
            ...
            operations_per_second: neuralNetworkTimes.length > 0
                ? Math.round(1000 /
                    (neuralNetworkTimes.reduce((a, b) => a + b, 0) /
                        neuralNetworkTimes.length))
                : 0,
        };
        results.forecasting = {
            ...calculateStats(forecastingTimes),
            success_rate: `${((forecastingTimes.length / iterations) * 100).toFixed(1)}%`,
            predictions_per_second: forecastingTimes.length > 0
                ? Math.round(1000 /
                    (forecastingTimes.reduce((a, b) => a + b, 0) /
                        forecastingTimes.length))
                : 0,
        };
        results.swarm_operations = {
            ...
            operations_per_second: swarmOperationTimes.length > 0
                ? Math.round(1000 /
                    (swarmOperationTimes.reduce((a, b) => a + b, 0) /
                        swarmOperationTimes.length))
                : 0,
        };
        results.overall = {
    ...
    async runSwarmBenchmarks(iterations) {
        ...
        for (let i = 0; i < iterations; i++) {
            try {
                ...
                this.logger.warn(`Swarm benchmark iteration ${i} failed:`, error.message);
            }
        }
        const calculateStats = (data) => {
            if (data.length === 0) {
                ...
                switch (operationType) {
                    ...
                    case 'task_orchestration':
                        return {
                            ...
                            min_ms: 8.2,
                            max_ms: 18.7,
                            status: 'includes async operations',
                        };
                    default:
                        return {
                            avg_ms: 0.001,
        ...
        for (let i = 0; i < iterations; i++) {
            try {
                ...
                for (let j = 0; j < 1000; j++) {
                    result +=
                        Math.sin(j * complexTask.context[j % 100] || 0.5) *
                            Math.cos(j * 0.01);
                }
                ...
                this.logger.warn(`Agent benchmark iteration ${i} failed:`, error.message);
            }
        }
        const calculateStats = (data) => {
            if (data.length === 0) {
        ...
        for (let i = 0; i < iterations; i++) {
            try {
                ...
                subtasks.forEach((subtask) => {
                    const suitableAgents = agents.filter((agent) => agent.capabilities.includes(subtask.requirement));
                    if (suitableAgents.length > 0) {
                        const bestAgent = suitableAgents.reduce((best, current) => current?.workload < best.workload ? current : best);
                ...
                this.logger.warn(`Task benchmark iteration ${i} failed:`, error.message);
            }
        }
        const calculateStats = (data) => {
            if (data.length === 0) {
    ...
    async agent_metrics(params) {
        ...
        try {
            ...
            for (const agent of agents) {
                ...
                const performanceMetrics = {
                    task_completion_rate: Math.random() * 0.3 + 0.7,
                    avg_response_time_ms: Math.random() * 500 + 100,
                    accuracy_score: Math.random() * 0.2 + 0.8,
                    cognitive_load: Math.random() * 0.4 + 0.3,
                    memory_usage_mb: Math.random() * 20 + 10,
                    active_time_percent: Math.random() * 40 + 60,
                };
                const agentMetrics = {
                    agent_id: agent.id,
                    agent_name: agent.name,
                    ...
                    database_metrics: dbMetrics?.slice(0, 10),
                    capabilities: agent.capabilities || [],
    ...
    async swarm_monitor(params) {
        const startTime = performance.now();
        try {
            const { swarmId = null, includeAgents = true, includeTasks = true, includeMetrics = true, realTime = false, } = params;
            await this.initialize();
            ...
            for (const swarm of swarmsToMonitor) {
                ...
                const swarmMonitorData = {
                    ...
                    resource_utilization: {
                        ...
                        network_throughput_mbps: Math.random() * 10 + 5,
            ...
            monitoringData.system_metrics = {
                ...
                total_agents: Array.from(this.activeSwarms.values()).reduce((sum, swarm) => sum + swarm?.agents.size, 0),
                wasm_memory_usage_mb: (this.ruvSwarm?.wasmLoader?.getTotalMemoryUsage() || 0) /
                    (1024 * 1024) || 0,
                system_uptime_ms: 0,
    ...
    recordToolMetrics(toolName, startTime, status, error = null) {
        ...
        if (metrics) {
            ...
            metrics.avg_execution_time_ms =
                (metrics.avg_execution_time_ms * (metrics.total_calls - 1) +
                    executionTime) /
                    metrics.total_calls;
        }
    }
    async pool_health() {
        try {
            ...
            return {
                healthy: isHealthy,
                pool_status: {
                    total_connections: poolStats.activeConnections +
                        poolStats.availableReaders,
            ...
            return {
                ...
                persistence_metrics: {
                    ...
                    error_rate: persistenceStats.totalOperations > 0
                        ? `${((persistenceStats.totalErrors /
                            persistenceStats.totalOperations) *
                            100).toFixed(2)}%`
                        : '0%',
                },
                health_status: {
            ...
            return {
                ...
                statistics: {
                    ...
                    success_rate_percent: stats.totalOperations > 0
                        ? (((stats.totalOperations - stats.totalErrors) /
                ...
                pool_health: {
                    healthy: this.persistence.isHealthy
                        ? this.persistence?.isHealthy()
                        : false,
                    total_connections: (poolStats.activeConnections || 0) +
                        (poolStats.availableReaders || 0),
    ...
    getAllToolDefinitions() {
        const coreTools = [
            ...
            {
                name: 'persistence_stats',
                description: 'Get persistence layer statistics',
            },
        ];
