...
export class ConnectionStateManager extends EventEmitter {
    ...
    async shutdown() {
        ...
        const disconnectPromises = Array.from(this.connections.keys()).map((connectionId) => this.disconnectConnection(connectionId, 'System shutdown').catch((error) => this.logger.warn(`Error disconnecting ${connectionId}`, {
            error: error.message,
        })));
