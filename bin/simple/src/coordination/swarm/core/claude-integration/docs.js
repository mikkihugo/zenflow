...
`;
                ...
                const title = config.title || 'Unknown Command';
    ...
    async createBackup(filePath) {
        ...
        try {
            ...
            logger.error('⚠️  Failed to create backup:', error.message);
            throw error;
        }
    }
    ...
    async mergeClaudeMd(filePath, noBackup = false) {
        try {
            ...
            logger.error('❌ Failed to merge CLAUDE.md:', error.message);
            throw error;
        }
    }
    getClaudeZenContent() {
...
Remember: **Claude Zen coordinates, Claude Code creates!** Start with \`mcp__claude-zen-swarm__swarm_init\` to enhance your development workflow.`;
    ...
    intelligentMerge(existingContent, claudeZenContent) {
    ...
    findClaudeZenSection(lines) {
        for (let i = 0; i < lines.length; i++) {
            const lineElement = lines[i];
            if (lineElement) {
                const line = lineElement?.toLowerCase();
                if ((line.includes('claude-zen') || line.includes('claude zen')) &&
