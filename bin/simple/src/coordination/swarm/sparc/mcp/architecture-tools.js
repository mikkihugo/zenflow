    ...
    validateArchitecture: {
        ...
        inputSchema: {
            ...
            properties: {
                architectureId: {
                    type: 'string',
                    description: 'Architecture ID to validate',
                },
                validationType: {
                    type: 'string',
                    enum: [
    ...
    getArchitecture: {
        ...
        inputSchema: {
            ...
            properties: {
                architectureId: {
                    type: 'string',
                    description: 'Architecture ID to retrieve',
                },
            },
            required: ['architectureId'],
        },
    ...
    updateArchitecture: {
        ...
        inputSchema: {
            ...
            properties: {
                architectureId: {
                    type: 'string',
                    description: 'Architecture ID to update',
                },
                updates: {
                    type: 'object',
    ...
    deleteArchitecture: {
        ...
        inputSchema: {
            ...
            properties: {
                architectureId: {
                    type: 'string',
                    description: 'Architecture ID to delete',
                },
            },
            required: ['architectureId'],
        },
    ...
    exportArchitecture: {
        ...
        inputSchema: {
            ...
            properties: {
                architectureId: {
                    type: 'string',
                    description: 'Architecture ID to export',
                },
                format: {
                    type: 'string',
        ...
        inputSchema: {
            ...
            properties: {
                sourceArchitectureId: {
                    type: 'string',
                    description: 'Source architecture ID to clone',
                },
                targetProjectId: {
                    type: 'string',
...
export class ArchitectureMCPToolsImpl {
    db;
    logger;
    architectureEngine;
    storageService;
    ...
    async generateArchitecture(params) {
        try {
            ...
            if (!params?.pseudocode?.algorithms ||
            ...
            const architectureId = await this.storageService.saveArchitecture(architecture, params?.projectId);
            return {
                success: true,
                architectureId,
    ...
    async generateArchitectureFromSpec(params) {
        try {
            this.logger?.info(`Generating architecture from specification with ${params?.pseudocode.length} algorithms`);
            const systemArchitecture = await this.architectureEngine.designSystemArchitecture(params?.specification, params?.pseudocode);
            const architecture = {
                id: nanoid(),
                systemArchitecture,
                componentDiagrams: await this.architectureEngine.generateComponentDiagrams(systemArchitecture),
                dataFlow: await this.architectureEngine.designDataFlow(systemArchitecture.components),
                deploymentPlan: await this.architectureEngine.planDeploymentArchitecture(systemArchitecture),
                validationResults: await this.architectureEngine.validateArchitecturalConsistency(systemArchitecture),
    ...
    async validateArchitecture(params) {
        try {
            ...
            const validation = await this.architectureEngine.validateArchitecturalConsistency(architecture.systemArchitecture);
            await this.storageService.saveValidation(params?.architectureId, validation, params?.validationType || 'general');
            return {
                success: true,
                validation,
    ...
    async updateArchitecture(params) {
        try {
            ...
            const updatedArchitecture = await this.architectureEngine.updateArchitecture(params?.architectureId, params?.updates);
            return {
                success: true,
    ...
    async deleteArchitecture(params) {
        try {
    ...
    async cloneArchitecture(params) {
        try {
            ...
            const sourceArchitecture = await this.storageService.getArchitectureById(params?.sourceArchitectureId);
            if (!sourceArchitecture) {
                throw new Error(`Source architecture not found: ${params?.sourceArchitectureId}`);
            }
            const clonedArchitecture = {
                ...sourceArchitecture,
            ...
            const newArchitectureId = await this.storageService.saveArchitecture(clonedArchitecture, params?.targetProjectId);
            return {
                success: true,
                newArchitectureId,
    ...
    jsonToYaml(obj) {
        const yamlify = (obj, depth = 0) => {
            ...
            if (Array.isArray(obj)) {
                return obj
                    .map((item) => `${indent}- ${yamlify(item, depth + 1)}`)
                    .join('\n');
            }
...
export async function createArchitectureMCPTools(db, logger) {
