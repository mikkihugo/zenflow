...
export class StrategicVisionService {
    ...
    async importStrategicDocuments(options) {
        try {
            ...
            const existingTypes = new Set(existingDocs.success && existingDocs.data?.documents
                ? existingDocs.data.documents.map((doc) => doc.type)
                : []);
            if (options.importFromFiles && options.projectPath) {
                const fileImportResults = await this.importFromFiles(options.projectId, options.projectPath, existingTypes, options.skipExistingDocuments);
            ...
            const codeImportResults = await this.importFromCodeAnnotations(options.projectId, options.projectPath || `/home/mhugo/code/${options.projectId}`, existingTypes);
    ...
    async analyzeStructuredDocuments(projectId, documents) {
        ...
        return {
            ...
            targetOutcome: visionDoc?.metadata?.target_outcome ||
                strategicGoals[0] ||
    ...
    async classifyDocumentWithLLM(filename, content) {
        try {
            ...
            const suggestedPriority = strategicRelevance > 0.7
                ? 'high'
    ...
    assessDocumentMaturity(content) {
        ...
        if (content.toLowerCase().includes('draft') ||
    ...
    generateSuggestedTitle(filename, content, documentType) {
        ...
        const typePrefix = documentType === 'vision'
            ? 'Vision:'
            : documentType === 'prd'
                ? 'Requirements:'
                : documentType === 'adr'
    ...
    async importFromCodeAnnotations(projectId, projectPath, existingTypes) {
        ...
        try {
            ...
            for (const annotationDoc of annotationDocs) {
                if (annotationDoc.content.length > 0 &&
    ...
    calculateConfidenceScore(documents) {
        ...
        if (documents.some((doc) => doc.type === 'vision'))
            score += 0.3;
        if (documents.some((doc) => doc.type === 'prd'))
            score += 0.2;
        if (documents.some((doc) => doc.type === 'epic'))
            score += 0.1;
        const hasMetadata = documents.some((doc) => doc.metadata && Object.keys(doc.metadata).length > 3);
        if (hasMetadata)
            score += 0.1;
...
//# sourceMappingURL=strategic-vision-service.js.map