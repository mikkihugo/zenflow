...
export class LLMStatsService {
    ...
    getSystemHealth() {
        const recentCalls = this.getRecentCalls(30);
    ...
    getRoutingAnalysis() {
        const recentCalls = this.getRecentCalls(60);
    ...
    calculateSystemHealth(calls) {
        const recentCalls = calls.filter((c) => Date.now() - c.timestamp.getTime() < 30 * 60 * 1000);
        ...
        const systemThroughput = recentCalls.length / 30;
        let healthScore = 100;
        healthScore -= errorRate * 50;
        healthScore -= Math.min(averageLatency / 100, 30);
        ...
        return {
            ...
            averageLatency,
            errorRate,
            resourceUtilization: Math.min(100, systemThroughput * 2),
    ...
    getProviderStatus(providerId, calls) {
        const recentCalls = calls.filter((c) => Date.now() - c.timestamp.getTime() < 60 * 60 * 1000);
