...
export class IntelligentAgentAssignor {
    agentCapabilityMap;
    fileTypeAgentMap;
    workloadTracker;
    constructor() {
    ...
    async assignOptimalAgent(context) {
        ...
        const selectedAgent = scoredAgents.reduce((best, current) => current?.score > best.score ? current : best);
    ...
    async updateAgentWorkload(agent, operation) {
    ...
    async balanceWorkload(agents) {
        ...
        const averageUtilization = workloads.reduce((sum, wl) => sum + wl.utilization, 0) / workloads.length;
        const threshold = 0.2;
        const unbalanced = workloads.filter((wl) => Math.abs(wl.utilization - averageUtilization) > threshold);
    ...
    async analyzeOperation(context) {
    ...
    classifyOperation(operation) {
        ...
        if (type.includes('analyze'))
            return 'analysis';
        if (type.includes('optimize'))
            return 'optimization';
        if (type.includes('secure'))
            return 'security';
        ...
        if (type.includes('api') || type.includes('backend'))
            return 'backend';
        if (type.includes('ml') || type.includes('ai'))
            return 'machine-learning';
        return 'general';
    }
    async assessComplexity(context) {
        let score = 0;
        const complexFileTypes = ['cpp', 'rust', 'java'];
        if (context.fileType && complexFileTypes.includes(context.fileType))
            score += 2;
        if (context.operation.description.length > 200)
            score += 1;
        ...
        if (context.requiredSkills.length > 3)
            score += 1;
        if (context.estimatedDuration > 3600)
            score += 2;
        if (score >= 5)
            return 'expert';
        if (score >= 3)
    ...
    inferRequiredSkills(fileType, operationType) {
        const skills = new Set();
        const fileTypeSkills = {
    ...
    identifySpecialRequirements(context) {
        const requirements = [];
        if (context.urgency === 'critical')
            requirements.push('high-availability');
        if (context.complexity === 'expert')
            requirements.push('senior-level');
        if (context.estimatedDuration > 7200)
            requirements.push('long-running');
    ...
    async getCandidateAgents(analysis) {
        const candidates = [];
    ...
    getAgentsByOperationType(operationType) {
        const operationAgentMap = {
            testing: [
                'unit-tester',
                'integration-tester',
                'e2e-tester',
    ...
    async scoreAgents(candidates, analysis) {
        const scoredAgents = [];
    ...
    async calculateAgentScore(agent, analysis) {
        const breakdown = {
            skillMatch: 0,
            performance: 0,
            availability: 0,
        ...
        const specializationMatch = agent.specialties.some((specialty) => analysis.requiredSkills.includes(specialty));
    ...
    calculateSkillMatch(agentCapabilities, requiredSkills) {
        if (requiredSkills.length === 0)
            return 1;
        const matches = requiredSkills.filter((skill) => agentCapabilities.some((cap) => cap.toLowerCase().includes(skill.toLowerCase())));
        return matches.length / requiredSkills.length;
    }
    generateReasoning(agent, analysis, score) {
        const reasons = [];
        ...
        if (score.breakdown.performance > 20) {
            reasons.push(`Excellent performance history (${(agent.performance.successRate * 100).toFixed(1)}% success rate)`);
        }
    ...
    async getHistoricalPerformanceScore(agentId, analysis) {
        const historicalData = await this.loadPerformanceHistory(agentId);
        const relevantHistory = historicalData?.filter((record) => record.context['fileType'] === analysis['fileType'] ||
            record.context['operationType'] === analysis['operationType']);
        if (relevantHistory.length === 0)
            return 0.5;
        const successRate = relevantHistory.filter((record) => record.success).length /
            relevantHistory.length;
        return successRate;
    }
    async estimatePerformance(agent, context) {
        const baseTime = context.estimatedDuration;
        const performanceMultiplier = 2 - agent.performance.successRate;
        return {
    ...
    meetsRequirements(agent, analysis) {
    ...
    calculateProjectedEfficiency(workloads, recommendations) {
        const currentEfficiency = workloads.reduce((sum, wl) => sum + Math.min(wl.utilization, 1), 0) /
            workloads.length;
        const improvementPotential = recommendations.reduce((sum, rec) => sum + Math.abs(rec.impact), 0) /
            recommendations.length;
        return Math.min(currentEfficiency + improvementPotential * 0.1, 1);
    }
    async loadAgentMemory(_agentId) {
        return {};
    }
    async loadAgentPreferences(_agentId) {
        return {};
    }
    async loadLearningData(_agentId) {
        return [];
    }
    async loadPerformanceHistory(_agentId) {
        return [];
    }
    async estimateOperationDuration(_operation) {
        return 300;
    }
    async persistWorkloadUpdate(_agentId, _newWorkload) {
    }
    async createAgentInfo(agentType) {
        ...
        return {
            ...
            currentWorkload: Math.random() * 50,
            maxWorkload: 100,
            performance: {
...
//# sourceMappingURL=auto-agent-assignment.js.map