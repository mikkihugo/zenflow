...
export class HookPerformanceTracker {
    ...
    async calculateQualityScore(operation, result) {
        ...
        if (result?.resourceUsage?.memoryMB <
            ...
            score += 0.1;
        if (result?.resourceUsage?.cpuPercent <
    ...
    async generateOptimizations(metrics) {
        ...
        if (metrics.resourceUsage.memoryMB >
            this.performanceThresholds.maxMemoryUsage * 0.8) {
    ...
    async generateRecommendations(metrics) {
        ...
        const memoryUsage = metrics.reduce((sum, m) => sum + m.resourceUsage.memoryMB, 0) /
            metrics.length;
    ...
    async analyzeAgentPerformance(metrics) {
        ...
        for (const [agentId, agentOps] of agentMetrics.entries()) {
            ...
            const avgQuality = agentOps.reduce((sum, op) => sum + (op.qualityScore || 0), 0) /
                agentOps.length;
            const avgSatisfaction = agentOps.reduce((sum, op) => sum + (op.userSatisfaction || 0), 0) /
                agentOps.length;
            summaries.push({
                agentId,
    ...
    calculateMemoryTrend(metrics) {
        ...
        const firstHalf = sortedMetrics.slice(0, midpoint);
        const secondHalf = sortedMetrics.slice(midpoint);
        const firstHalfAvgMemory = firstHalf.reduce((sum, m) => sum + m.resourceUsage.memoryMB, 0) /
            firstHalf.length;
        const secondHalfAvgMemory = secondHalf.reduce((sum, m) => sum + m.resourceUsage.memoryMB, 0) /
            secondHalf.length;
    ...
    async generatePredictions(trends, timeframe) {
        ...
        trends.forEach((trend) => {
            if (trend.direction !== 'stable' && trend.confidence > 0.5) {
                ...
                const predictedChange = trend.change * 1.5;
                predictions.push({
                    metric: trend.metric,
...
export class OperationPerformanceOptimizer {
    ...
    async analyzeOptimizationOpportunities(operation) {
        ...
        if (operation.type.includes('analysis') ||
