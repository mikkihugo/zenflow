export const CLAUDE_FLOW_AGENTS = {
    'Core Development': [
        'coder',
        'analyst',
        'researcher',
...
export const OUR_AGENT_CATEGORIES = {
    'Core Foundation': [
        'coder',
        'analyst',
        'researcher',
...
export function performGapAnalysis() {
    const ourAgentCount = Object.values(OUR_AGENT_CATEGORIES)
        .flat()
        .filter((agent, index, array) => array.indexOf(agent) === index).length;
    ...
    const missingCapabilities = allTheirAgents.filter((agent) => !allOurAgents.includes(agent));
    ...
    return {
        ...
        advantageRatio: Number.parseFloat((ourAgentCount / claudeFlowAgentCount).toFixed(2)),
        categoryComparison,
        missingCapabilities,
        uniqueAdvantages,
        recommendations,
...
export function auditAutoAssignmentCapabilities() {
    return {
        hasIntelligentSelection: true,
        hasFileTypeMatching: false,
        hasWorkloadBalancing: true,
