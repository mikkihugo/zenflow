...
export class AgentRegistry extends EventEmitter {
    ...
    healthCheckInterval;
    initialized = false;
    constructor(memory, namespace = 'agent-registry') {
        super();
        this.memory = memory;
    ...
    async queryAgents(query = {}) {
        ...
        return agents.filter((agent) => {
            ...
            if (query.capabilities && query.capabilities.length > 0) {
                const hasAllCapabilities = query.capabilities.every((cap) => agent.capabilities.languages?.includes(cap) ||
    ...
    getStats() {
        ...
        return {
            ...
            averageSuccessRate: agents.reduce((sum, a) => sum + a.metrics.successRate, 0) /
                agents.length || 0,
        };
    }
    ...
    async persistRegistrations() {
        const promises = Array.from(this.agents.entries()).map(([id, agent]) => this.memory.store(`${this.namespace}/agents/${id}`, agent, {
            ttl: 3600000,
            replicas: 2,
        }));
    ...
    calculateLoadFactor(metrics) {
        if (!metrics)
            return 0;
        const taskLoad = (metrics.tasksInProgress || 0) /
            Math.max(1, metrics.tasksCompleted + (metrics.tasksInProgress || 0));
        const resourceLoad = ((metrics.resourceUsage?.memory || 0) +
    ...
    calculateHealth(metrics, status) {
        ...
        const resourcePenalty = Math.max(0, ((metrics.resourceUsage?.memory || 0) +
            (metrics.resourceUsage?.cpu || 0)) /
            2 -
            0.8);
    ...
    calculateSelectionScore(agent, criteria) {
    ...
    filterByContext(candidates, criteria) {
    ...
    calculateContextScore(agent, criteria) {
        const fileTypeToAgents = this.getFileTypeMapping();
        const taskTypeToAgents = this.getTaskTypeMapping();
        let score = 0;
        if (criteria.fileType && fileTypeToAgents[criteria.fileType]) {
