...
export class AutoScalingStrategy extends EventEmitter {
    config;
    scalingHistory = [];
    lastScalingAction = new Date(0);
    currentAgentCount = 0;
    constructor(config) {
        super();
        this.config = config;
    ...
    async makeScalingDecision(metrics) {
        ...
        if (avgUtilization > this.config.scaleUpThreshold ||
            ...
            if (agentCount < this.config.maxAgents) {
                const targetCount = Math.min(this.config.maxAgents, Math.ceil(agentCount * 1.5));
                return {
                    action: 'scale_up',
                    targetCount,
        ...
        if (avgUtilization < this.config.scaleDownThreshold &&
            ...
            if (agentCount > this.config.minAgents) {
                const targetCount = Math.max(this.config.minAgents, Math.floor(agentCount * 0.8));
                return {
                    action: 'scale_down',
                    targetCount,
                    confidence: this.calculateScalingConfidence(systemLoad),
    ...
    calculateSystemLoad(metrics) {
        ...
        const avgUtilization = cpuUtilizations.reduce((sum, cpu) => sum + cpu, 0) /
    ...
    calculateScalingConfidence(systemLoad) {
        const utilizationFactor = Math.abs(systemLoad.avgUtilization - 0.7);
        const responseTimeFactor = Math.min(1, systemLoad.avgResponseTime / 5000);
        const errorFactor = Math.min(1, systemLoad.errorRate * 10);
        const confidence = (utilizationFactor + responseTimeFactor + errorFactor) / 3;
        return Math.min(1.0, Math.max(0.1, confidence));
    }
    recordScalingAction(action, reason, countChange) {
...
//# sourceMappingURL=auto-scaling-strategy.js.map