...
export class PerformanceAnalyzer extends EventEmitter {
    ...
    calculateHealthScore(metrics) {
        ...
        const factHealth = metrics.fact.cache.hitRate *
            100 *
    ...
    generatePredictions() {
        ...
        if (cpuTrend.direction === 'increasing' && cpuTrend.rate > 0) {
            const timeToCapacity = (90 - recentMetrics[recentMetrics.length - 1]?.system?.cpu?.usage) /
                cpuTrend.rate;
        ...
        return {
            capacityUtilization,
            timeToCapacity: resourceExhaustion.length > 0 ? 300 : Number.POSITIVE_INFINITY,
            resourceExhaustion,
        };
    }
    ...
    getHistoricalInsights(timeRange) {
        const relevantMetrics = timeRange
            ? this.metricsHistory.filter((m) => m.system.timestamp >= timeRange.start &&
    ...
    updateBaselines() {
        ...
        const avgCpu = recentMetrics.reduce((sum, m) => sum + m.system.cpu.usage, 0) /
            recentMetrics.length;
        this.baselineMetrics.set('cpu_usage', avgCpu);
        const avgMemory = recentMetrics.reduce((sum, m) => sum + m.system.memory.percentage, 0) /
            recentMetrics.length;
