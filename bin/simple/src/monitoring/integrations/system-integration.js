...
export class SystemIntegration extends EventEmitter {
    ...
    setupSystemHooks() {
        ...
        this.hooks.onMcpToolComplete = (toolName, duration, success, error) => {
            if (success) {
                ...
                errors.push(error);
                this.mcpMetrics.toolErrors.set(toolName, errors.slice(-10));
            }
            const latencies = this.mcpMetrics.toolLatencies.get(toolName) || [];
            latencies.push(duration);
            this.mcpMetrics.toolLatencies.set(toolName, latencies.slice(-100));
    ...
    enhanceMetrics(metrics) {
        if (this.factMetrics.queryTimes.length > 0) {
            const avgQueryTime = this.factMetrics.queryTimes.reduce((a, b) => a + b, 0) /
                this.factMetrics.queryTimes.length;
            const hitRate = this.factMetrics.cacheHits /
        ...
        if (this.swarmMetrics.consensusTimes.length > 0) {
            ...
            const avgTaskTime = totalTasks > 0 ? 5000 : metrics.swarm.tasks.averageTaskTime;
            metrics.swarm = {
                ...metrics.swarm,
                coordination: {
        ...
        const avgResponseTime = allLatencies.length > 0
