import { DALFactory } from '../database/index.ts';
...
export async function createIntegratedSystem() {
    ...
    container.register(DATABASE_TOKENS?.['ProviderFactory'], {
        type: 'singleton',
        create: () => ({
            createProvider: async (type, config) => {
    ...
    container.register(DATABASE_TOKENS?.DALFactory, {
        type: 'singleton',
        create: (c) => new DALFactory(c.resolve(CORE_TOKENS.Logger), c.resolve(CORE_TOKENS.Config), c.resolve(DATABASE_TOKENS?.['ProviderFactory'])),
    });
    ...
    return {
        ...
        async storeWithCoordination(sessionId, key, data) {
        ...
        async retrieveWithOptimization(sessionId, key) {
        ...
        async performVectorSearch(embedding, sessionId) {
            const query = {
        ...
        async getSystemHealth() {
            ...
            return {
                ...
                overall: {
                    status: memoryHealth.overall === 'healthy' &&
        ...
        async getPerformanceMetrics() {
            ...
            return {
                ...
                integration: {
                    totalOperations: (memoryStats.coordinator?.decisions?.total || 0) +
                        (databaseStats?.coordinator?.queries?.total || 0),
                    averageLatency: ((memoryStats.current?.averageLatency || 0) +
...
export async function demonstrateErrorHandling() {
    ...
    try {
        ...
        const { MemoryError, MemoryErrorClassifier } = await import('../memory/error-handling/memory-errors.ts');
        const { DatabaseError, DatabaseErrorClassifier } = await import('../database/error-handling/database-errors.ts');
...
//# sourceMappingURL=memory-database-integration.js.map