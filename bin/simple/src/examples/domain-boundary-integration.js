...
import { CommonSchemas, Domain, domainValidatorRegistry, getDomainValidator, validateCrossDomain, } from '../core/domain-boundary-validator.ts';
...
export async function coordinationDomainExample() {
    ...
    const validatedPhaseAssignment = coordValidator.validateInput(phaseAssignment, ExtendedSchemas.PhaseAssignment);
...
export async function workflowsDomainExample() {
    ...
    try {
        const validatedExecution = workflowsValidator.validateInput(workflowExecution, ExtendedSchemas.WorkflowExecution);
    ...
    const validatedEvent = workflowsValidator.validateInput(workflowEvent, ExtendedSchemas.WorkflowEvent);
...
export async function contractEnforcementExample() {
    logger.info('Running contract enforcement example');
    const coordValidator = getDomainValidator(Domain.COORDINATION);
    const crossDomainOperation = {
...
export async function complexMultiDomainExample() {
    ...
    const workflowData = validateCrossDomain(validatedPlan, ExtendedSchemas.ExecutionPlan, Domain.COORDINATION, Domain.WORKFLOWS, 'execution-plan-transfer');
    const workflowsValidator = getDomainValidator(Domain.WORKFLOWS);
    const workflowExecution = {
    ...
    const validatedExecution = workflowsValidator.validateInput(workflowExecution, ExtendedSchemas.WorkflowExecution);
    const neuralIntegrationOperation = {
...
export async function errorHandlingExample() {
    ...
    try {
        const invalidData = { id: 123, status: 'invalid-status' };
...
//# sourceMappingURL=domain-boundary-integration.js.map