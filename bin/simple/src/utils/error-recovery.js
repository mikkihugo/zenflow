...
export class ErrorRecoverySystem extends EventEmitter {
    ...
    getStrategiesForError(errorType, component) {
        return Array.from(this.strategies.values())
            .filter((strategy) => strategy.conditions.some((condition) => this.matchesCondition(condition, errorType, component)))
            .sort((a, b) => this.getSeverityWeight(a.severity) -
            this.getSeverityWeight(b.severity));
    }
    getRecoveryHistory(component, since) {
    ...
    getRecoveryStats() {
        ...
        for (const recovery of this.recoveryHistory) {
            strategiesUsed[recovery.strategy] =
                (strategiesUsed[recovery.strategy] || 0) + 1;
            componentsAffected[recovery.component] =
                (componentsAffected[recovery.component] || 0) + 1;
        }
        return {
    ...
    selectRecoveryStrategy(context) {
        const candidates = this.getStrategiesForError(context.errorType, context.component);
        const viable = candidates.filter((strategy) => {
            const severityMatch = this.getSeverityWeight(strategy.severity) >=
                this.getSeverityWeight(context.severity);
    ...
    async executeRestartAction(_action, _context) {
        await new Promise((resolve) => setTimeout(resolve, 100));
    }
    async executeRollbackAction(_action, _context) {
        await new Promise((resolve) => setTimeout(resolve, 200));
    }
    async executeFailoverAction(_action, _context) {
        await new Promise((resolve) => setTimeout(resolve, 500));
    }
    async executeScaleAction(_action, _context) {
        await new Promise((resolve) => setTimeout(resolve, 1000));
    }
    async executeNotifyAction(_action, _context) {
        await new Promise((resolve) => setTimeout(resolve, 50));
    }
    async executeRepairAction(_action, _context) {
        await new Promise((resolve) => setTimeout(resolve, 300));
    }
    matchesCondition(condition, errorType, component) {
    ...
    calculateBackoffDelay(strategy, retryCount, baseTimeout) {
        switch (strategy) {
            case 'linear':
