...
import { TemplateEngine } from '../../coordination/swarm/sparc/core/template-engine.ts';
import { SpecificationPhaseEngine } from '../../coordination/swarm/sparc/phases/specification/specification-engine.ts';
export function createSPARCTemplateCommands() {
    ...
    sparcTemplateCmd
        ...
        try {
            ...
            if (options?.template) {
                ...
                specification =
                    await specEngine.generateSpecificationFromTemplate(projectSpec);
            }
            let output;
...
function formatSpecificationAsMarkdown(specification) {
    ...
    specification.functionalRequirements.forEach((req, index) => {
        ...
        if (req.testCriteria && req.testCriteria.length > 0) {
            ...
            req.testCriteria.forEach((criteria) => {
                markdown += `- ${criteria}\n`;
            });
            markdown += `\n`;
        }
    ...
    if (specification.nonFunctionalRequirements &&
        ...
        specification.nonFunctionalRequirements.forEach((req, index) => {
            markdown += `### ${index + 1}. ${req.title}\n`;
            markdown += `**Priority:** ${req.priority}\n`;
            markdown += `**Description:** ${req.description}\n\n`;
            if (req.metrics) {
    ...
    if (specification.riskAssessment?.risks?.length > 0) {
        ...
        specification.riskAssessment.risks.forEach((risk, index) => {
    ...
    if (specification.successMetrics && specification.successMetrics.length > 0) {
        ...
        specification.successMetrics.forEach((metric, index) => {
...
//# sourceMappingURL=sparc-template-commands.js.map