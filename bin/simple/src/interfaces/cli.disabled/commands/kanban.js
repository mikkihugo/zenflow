...
export async function monitorKanbanFlow(input, flags) {
    ...
    if (flags.detailed) {
        ...
        logger.info('   Bottleneck Detector: 1,944 lines - Real-time detection engine');
...
export async function testKanbanFlow(input, flags) {
    ...
    if (flags.metricsTracker || flags.all) {
        ...
        logger.info('   ✅ Performance Engine: Optimization recommendations active');
...
export async function debugKanbanFlow(input, flags) {
...
export async function handleKanbanCommand(argv) {
