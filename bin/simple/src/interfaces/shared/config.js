...
export class ConfigurationUtils {
    ...
    static validateConfig(configOverrides) {
        const errors = [];
        if (configOverrides?.theme &&
            !['dark', 'light', 'auto'].includes(configOverrides?.theme)) {
            errors.push('Invalid theme. Must be one of: dark, light, auto');
        }
        if (configOverrides?.verbosity &&
    ...
    static getColorScheme(theme) {
        ...
        const scheme = COLOR_SCHEMES[currentTheme];
        return scheme ? { ...scheme } : { ...COLOR_SCHEMES.dark };
    }
    static updateInterfaceConfig(updates) {
