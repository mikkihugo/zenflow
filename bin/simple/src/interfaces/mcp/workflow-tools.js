...
export const monitorWorkflowTool = {
...
export const scaleWorkflowTool = {
    name: 'workflow_scale',
...
export const configureWorkflowTool = {
...
export const monitorKanbanFlowTool = {
...
export const getFlowMetricsTool = {
    name: 'flow_metrics',
...
export async function handleInitializeWorkflow(args) {
    try {
...
export async function handleMonitorWorkflow(args) {
    try {
        ...
        const result = {
            ...
            optimization: {
                ...
                recommendations: optimization.canOptimize && args.includeRecommendations
...
export async function handleScaleWorkflow(args) {
    try {
...
export async function handleTestWorkflow(args) {
    try {
        logger.info(`🧪 MCP: Running Workflow ${args.testType.toUpperCase()} Test`);
        const memoryOptimizer = createAdaptiveOptimizer();
        const systemInfo = getSystemInfo();
        const startTime = Date.now();
        await new Promise((resolve) => setTimeout(resolve, Math.min(args.duration * 100, 2000)));
...
export async function handleWorkflowToolCall(name, args) {
    switch (name) {
