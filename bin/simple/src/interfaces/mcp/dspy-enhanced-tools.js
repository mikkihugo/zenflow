...
export class DSPyEnhancedMCPTools {
    ...
    async initializeMCPPrograms() {
        const projectAnalysisProgram = await this.dspy.createProgram('project_structure: object, user_request: string, context: object -> analysis: string, recommendations: string[], priority_actions: string[], complexity_score: number', 'Analyze project structure and provide intelligent recommendations based on user intent and context');
        const codeGenerationProgram = await this.dspy.createProgram('requirements: string, existing_code: string, project_context: object -> generated_code: string, explanation: string, integration_steps: string[], testing_suggestions: string[]', 'Generate high-quality code that integrates seamlessly with existing project structure');
        const errorResolutionProgram = await this.dspy.createProgram('error_details: object, code_context: string, project_info: object -> solution: string, fix_code: string, prevention_tips: string[], confidence: number', 'Intelligently resolve errors with contextual understanding of the project');
        const workflowOptimizationProgram = await this.dspy.createProgram('current_workflow: object, user_goals: string, project_constraints: object -> optimized_workflow: object, improvement_suggestions: string[], automation_opportunities: string[]', 'Optimize development workflows for maximum efficiency and user satisfaction');
        const taskOrchestrationProgram = await this.dspy.createProgram('task_request: string, available_tools: string[], project_state: object -> task_plan: object, tool_sequence: string[], risk_assessment: object, success_prediction: number', 'Intelligently orchestrate complex tasks using available MCP tools with risk assessment');
    ...
    async analyzeProject(request) {
        ...
        try {
            ...
            const result = await this.dspy.execute(program, {
                project_structure: { projectPath, files: files?.slice(0, 50) },
    ...
    async optimizeWorkflow(request) {
        ...
        if (!program) {
            return this.createErrorResponse('Workflow optimization program not available');
        }
        try {
            ...
            this.recordToolUsage('workflow_optimization', request.parameters, null, false);
    ...
    async orchestrateTask(request) {
        ...
        if (!program) {
            return this.createErrorResponse('Task orchestration program not available');
        }
        try {
            ...
            this.recordToolUsage('task_orchestration', request.parameters, null, false);
            return this.createErrorResponse('Task orchestration failed', error);
        }
    }
    getToolStats() {
        const recentUsage = this.toolUsageHistory.filter((usage) => Date.now() - usage.timestamp.getTime() < 3600000);
    ...
    updateToolOutcome(toolName, parameters, success, actualResult) {
        const usage = this.toolUsageHistory.find((u) => u.tool === toolName &&
            JSON.stringify(u.input) === JSON.stringify(parameters) &&
            Date.now() - u.timestamp.getTime() < 300000);
        if (usage) {
    ...
    recordToolUsage(tool, input, output, success) {
    ...
    async trainProgramFromOutcome(programType, usage) {
    ...
    generateFollowupActions(priorityActions) {
    ...
    estimateTaskDuration(toolCount) {
        const minutes = Math.max(5, toolCount * 3);
        if (minutes < 60) {
...
//# sourceMappingURL=dspy-enhanced-tools.js.map