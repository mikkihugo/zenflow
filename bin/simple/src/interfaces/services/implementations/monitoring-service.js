...
export class MonitoringService extends BaseService {
    ...
    async executeOperation(operation, params, _options) {
        ...
        switch (operation) {
            case 'collect-metrics':
                return (await this.collectMetrics(params?.source));
            case 'get-metrics':
                return this.getMetrics(params?.metricName, params?.timeRange);
            case 'record-metric':
                return this.recordMetric(params?.name, params?.value, params?.timestamp);
            case 'create-alert':
                return this.createAlert(params);
            case 'update-alert':
                return this.updateAlert(params?.alertId, params?.config);
            case 'delete-alert':
                return this.deleteAlert(params?.alertId);
            case 'get-alerts':
                return this.getAlerts(params?.status);
            case 'trigger-alert':
                return (await this.triggerAlert(params?.alertId, params?.data));
            case 'get-stats':
                return this.getMonitoringStats();
            case 'clear-metrics':
                return (await this.clearMetrics(params?.olderThan));
            case 'export-metrics':
                return (await this.exportMetrics(params?.format, params?.timeRange));
            default:
                throw new Error(`Unknown monitoring operation: ${operation}`);
        }
    }
    async collectMetrics(source) {
    ...
    getMetrics(metricName, timeRange) {
        ...
        for (const [name, data] of this.metrics.entries()) {
            if (timeRange) {
                allMetrics[name] = data?.filter((m) => m.timestamp >= timeRange.start && m.timestamp <= timeRange.end);
            }
            else {
    ...
    recordMetric(name, value, timestamp) {
        ...
        const config = this.config;
        const retention = config?.metrics?.retention || 86400000;
        const cutoff = ts - retention;
        const filtered = metricArray.filter((m) => m.timestamp > cutoff);
        this.metrics.set(name, filtered);
        if (filtered.length > 10000) {
            this.metrics.set(name, filtered.slice(-5000));
        }
    ...
    async triggerAlert(alertId, data) {
        const alert = this.alerts.get(alertId);
        if (!alert) {
            throw new Error(`Alert not found: ${alertId}`);
        }
    ...
    async exportMetrics(format = 'json', timeRange) {
    ...
    startMetricsCollection() {
        const config = this.config;
        const interval = config?.metrics?.interval || 10000;
    ...
    async sendNotification(channel, alert, data) {
    ...
    estimateMemoryUsage() {
        ...
        for (const metricArray of this.metrics.values()) {
            totalSize += metricArray.length * 200;
        }
        totalSize += this.alerts.size * 500;
        return totalSize;
    }
