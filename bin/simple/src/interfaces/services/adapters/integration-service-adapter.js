...
export class IntegrationServiceAdapter {
    ...
    async validateConfig(config) {
        try {
            ...
            if (typedConfig?.safeAPI?.enabled) {
                if (typedConfig?.safeAPI?.timeout &&
                    typedConfig?.safeAPI?.timeout < 1000) {
                    errors.push('API timeout must be at least 1000ms');
                }
                if (typedConfig?.safeAPI?.retries &&
            ...
            if (typedConfig?.protocolManagement?.enabled) {
                ...
                if (typedConfig?.protocolManagement?.connectionPooling?.enabled) {
                    if (typedConfig?.protocolManagement?.connectionPooling?.maxConnections <
                        1) {
    ...
    async sendProtocolMessage(protocol, message) {
        ...
        try {
            ...
            const metrics = this.protocolMetrics.get(protocol);
            if (metrics) {
                metrics.averageLatency = (metrics.averageLatency + Date.now()) / 2;
    ...
    getConnectionPoolStatus() {
        ...
        for (const [protocol, pool] of this.connectionPool.entries()) {
            poolStatuses[protocol] = {
                ...
                maxConnections: this.config.protocolManagement?.connectionPooling?.maxConnections ||
                    50,
    ...
    calculateArchitectureOperationsRate() {
        const recentMetrics = this.architectureMetrics.filter((m) => Date.now() - m.timestamp.getTime() < 300000);
        return recentMetrics.length > 0 ? recentMetrics.length / 300 : 0;
    }
...
//# sourceMappingURL=integration-service-adapter.js.map