...
export class PerformanceOptimizer extends EventEmitter {
    ...
    calculateOptimizationConfidence(patterns, optimizations) {
        ...
        const optimizationFactor = Math.min(1, optimizations.length / 3);
    ...
    calculateAgentTaskScore(agent, task, _agentInfo, _taskInfo) {
        let score = 0;
        const capabilityMatch = task.requirements.filter((req) => agent.capabilities.includes(req) ||
            agent.specializations.includes(req)).length / task.requirements.length;
    ...
    calculateResourcePerformance(allocations) {
        if (allocations.length === 0)
            return 0;
        const avgEfficiency = allocations.reduce((sum, alloc) => sum + alloc.efficiency, 0) /
            allocations.length;
        return Math.min(1, avgEfficiency / 100);
    }
    ...
    prioritizeBottlenecks(bottlenecks) {
        return bottlenecks.sort((a, b) => b.severity * b.impact * b.frequency -
    ...
    generateLatencyOptimizations(bottlenecks) {
        return bottlenecks.map((bottleneck) => ({
            ...
            expectedReduction: bottleneck.severity * 100,
            implementation: [
                {
