...
import { isInferenceResult, isNeuralError, isTrainingResult, isWasmError, } from '../utils/type-guards.ts';
export class SafeNeuralNetwork {
    ...
    async initializeWasm() {
        try {
            ...
            this.wasmModule = {
                predict: (inputs) => inputs.map((x) => Math.tanh(x)),
    ...
    async trainEpoch(_data, _options) {
        try {
    ...
    async validateNetwork(inputs, _outputs) {
        try {
            const predictions = [];
    ...
    predictWithJavaScript(inputs) {
        return inputs.map((x) => Math.tanh(x * 0.5));
    }
    calculateConfidence(predictions) {
        return predictions.map((p) => Math.abs(p) * 0.9 + 0.1);
    }
    calculateAccuracy(predictions, expected) {
        if (predictions.length !== expected.length)
            return 0;
        let correct = 0;
...
export async function safeNeuralUsageExample() {
    const config = {
        layers: [2, 4, 1],
    ...
    for (const input of testInputs) {
        ...
        if (isInferenceResult(predictionResult)) {
            ...
            logger.error(`❌ Prediction failed for input [${input.join(', ')}]:`, predictionResult?.error?.message);
        }
    }
}
//# sourceMappingURL=safe-neural-network.js.map