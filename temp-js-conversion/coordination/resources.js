...
class ResourceManager {
    constructor(config, eventBus, logger) {
        ...
        this.locks = new Map(); // resourceId -> agentId
        this.waitQueue = new Map(); // resourceId -> queue
        this.agentResources = new Map(); // agentId -> resourceIds
    }
    async initialize() {
    ...
    async acquire(resourceId, agentId, priority = 0) {
        ...
        throw new errors_js_1.ResourceLockError(`Resource acquisition timeout for ${resourceId}`, { resourceId, agentId, timeout: this.config.resourceTimeout });
    }
