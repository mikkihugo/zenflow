...
function showAllTopics() {
    ...
    const table = new cli_table3_1.default.default({
        head: ['Topic', 'Category', 'Description'],
        style: { head: ['cyan'] }
    });
    for (const topic of HELP_TOPICS) {
        table.push([
            chalk_1.default.cyan(topic.name),
            chalk_1.default.yellow(topic.category),
            topic.description
        ]);
    }
...
async function showTopicHelp(topicName, options) {
    ...
    if (!topic) {
        ...
        const similar = HELP_TOPICS.filter(t => t.name.includes(topicName) ||
