...
async function (flags = {}) {
    ...
    console.log(chalk_1.default.gray('  --topology mesh            Set topology'));
    ...
    const getFlag = (camelCase, kebabCase, defaultValue) => {
        if (flags[kebabCase] !== undefined)
            return flags[kebabCase];
        if (flags[camelCase] !== undefined)
            return flags[camelCase];
        return defaultValue;
    };
    const config = {
...
async function spawnSwarm(args, flags) {
    ...
    try {
        ...
        try {
            ...
            if (!validQueenTypes.includes(queenType)) {
                ...
                console.error(chalk_1.default.red(`  ❌ Valid queen types: ${validQueenTypes.join(', ')}`));
                return;
            }
            // Validate topology
            ...
            if (!validTopologies.includes(topology)) {
                ...
                console.error(chalk_1.default.red(`  ❌ Valid topologies: ${validTopologies.join(', ')}`));
                return;
            }
            const finalFlags = {
    ...
    `);
      ...
      `).run(swarmId, hiveMind.config.name, objective, hiveMind.config.queenType);
        ...
        const autoSave = (0, auto_save_middleware_js_1.createAutoSaveMiddleware)(sessionId, {
    ...
    `).run(...Object.values(queenAgent));
      ...
      `).run(...Object.values(worker));
        ...
        if (flags.claude || flags.spawn) {
            ...
            console.log('\n' + chalk_1.default.blue('💡 Pro Tip:') + ' Add --claude to spawn coordinated Claude Code instances');
...
async function spawnClaudeCodeInstances(swarmId, swarmName, objective, workers, flags) {
    ...
    try {
        ...
        console.log(chalk_1.default.cyan('Swarm ID:'), swarmId);
        console.log(chalk_1.default.cyan('Objective:'), objective);
        console.log(chalk_1.default.cyan('Queen Type:'), flags.queenType || flags['queen-type'] || 'strategic');
        console.log(chalk_1.default.cyan('Topology:'), flags.topology || 'hierarchical');
        ...
        try {
            ...
            const { spawn: childSpawn, execSync } = await Promise.resolve().then(() => require('child_process'));
...
async function resumeSession(args, flags) {
    ...
    try {
        ...
        if (flags.claude || flags.spawn) {
            ...
            console.log('\n' + chalk_1.default.blue('💡 Pro Tip:') + ' Add --claude to spawn Claude Code with restored context');
...
async function getMcpWrapper() {
    const { MCPToolWrapper } = await Promise.resolve().then(() => require('./hive-mind/mcp-wrapper.js'));
