...
const sparc_modes_js_1 = require("./templates/sparc-modes.js");
...
async function initCommand(subArgs, flags) {
    ...
    try {
        ...
        // Create CLAUDE.md
        const claudeMd = initOptimized ? await (0, claude_md_js_1.createOptimizedSparcClaudeMd)() :
            initSparc ? (0, claude_md_js_1.createSparcClaudeMd)() :
                initMinimal ? (0, claude_md_js_1.createMinimalClaudeMd)() : (0, claude_md_js_1.createFullClaudeMd)();
        if (!initDryRun) {
            await node_compat_js_1.Deno.writeTextFile('CLAUDE.md', claudeMd);
        ...
        const memoryBankMd = initOptimized ? await (0, memory_bank_md_js_1.createOptimizedMemoryBankMd)() :
            initMinimal ? (0, memory_bank_md_js_1.createMinimalMemoryBankMd)() : (0, memory_bank_md_js_1.createFullMemoryBankMd)();
        if (!initDryRun) {
            await node_compat_js_1.Deno.writeTextFile('memory-bank.md', memoryBankMd);
        ...
        const coordinationMd = initOptimized ? await (0, coordination_md_js_1.createOptimizedCoordinationMd)() :
            initMinimal ? (0, coordination_md_js_1.createMinimalCoordinationMd)() : (0, coordination_md_js_1.createFullCoordinationMd)();
        if (!initDryRun) {
            await node_compat_js_1.Deno.writeTextFile('coordination.md', coordinationMd);
        ...
        if (initSparc && !initDryRun) {
            try {
                ...
                const sparcTemplates = (0, sparc_modes_js_1.createSparcModeTemplates)();
                console.log('  📁 Creating SPARC command files...');
                for (const [filename, content] of Object.entries(sparcTemplates)) {
                    try {
                        await node_compat_js_1.Deno.writeTextFile(`${sparcTargetDir}/${filename}`, content);
                ...
                const sparcModesOverview = (0, sparc_modes_js_1.createSparcModesOverview)();
                await node_compat_js_1.Deno.writeTextFile(`${sparcTargetDir}/sparc-modes.md`, sparcModesOverview);
            ...
            try {
                ...
                const swarmTemplates = (0, sparc_modes_js_1.createSwarmStrategyTemplates)();
                console.log('  📁 Creating swarm strategy files...');
                for (const [filename, content] of Object.entries(swarmTemplates)) {
                    try {
                        await node_compat_js_1.Deno.writeTextFile(`${swarmTargetDir}/${filename}`, content);
                        console.log(`    ✓ Created ${filename}`);
                    }
                    catch (err) {
                        console.log(`    ⚠️  Could not create ${filename}: ${err.message}`);
            ...
            try {
                ...
                await node_compat_js_1.Deno.writeTextFile(`${workingDir}/.claude/config.json`, JSON.stringify(configContent, null, 2));
        ...
        const agentsReadme = (0, readme_files_js_1.createAgentsReadme)();
        if (!initDryRun) {
            await node_compat_js_1.Deno.writeTextFile('memory/agents/README.md', agentsReadme);
            console.log('  ✓ Created memory/agents/README.md');
        }
        else {
            console.log('  [DRY RUN] Would create memory/agents/README.md');
        }
        const sessionsReadme = (0, readme_files_js_1.createSessionsReadme)();
        if (!initDryRun) {
            await node_compat_js_1.Deno.writeTextFile('memory/sessions/README.md', sessionsReadme);
        ...
        if (!initDryRun) {
            await node_compat_js_1.Deno.writeTextFile('memory/claude-flow-data.json', JSON.stringify(initialData, null, 2));
...
async function createInitialFiles(options, workingDir, dryRun = false) {
    if (!dryRun) {
        const claudeMd = options.sparc ? (0, claude_md_js_1.createSparcClaudeMd)() :
            options.minimal ? (0, claude_md_js_1.createMinimalClaudeMd)() : (0, claude_md_js_1.createFullClaudeMd)();
        ...
        const coordinationMd = options.minimal ? (0, coordination_md_js_1.createMinimalCoordinationMd)() : (0, coordination_md_js_1.createFullCoordinationMd)();
...
`;
        ...
        if (!dryRun) {
            ...
            try {
                ...
                const { FallbackMemoryStore } = await Promise.resolve().then(() => require('../../../memory/fallback-store.js'));
