...
async function swarmCommand(args, flags) {
    ...
    if (flags && flags.executor) {
        ...
        try {
            const { execSync, spawn } = await Promise.resolve().then(() => require('child_process'));
...
`;
    ...
    try {
        ...
        try {
            ...
            const module = await Promise.resolve(`${distPath}`).then(s => require(s));
        ...
        if (error.code === 'ERR_MODULE_NOT_FOUND' || error.code === 'ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING' || error.code === 'ERR_UNKNOWN_FILE_EXTENSION') {
            ...
            try {
                const { executeSwarm } = await Promise.resolve().then(() => require('./swarm-executor.js'));
