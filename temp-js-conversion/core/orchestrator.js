...
class SessionManager {
    constructor(terminalManager, memoryManager, eventBus, logger, config) {
        ...
        this.persistenceCircuitBreaker = (0, helpers_js_1.circuitBreaker)('SessionPersistence', { threshold: 5, timeout: 30000, resetTimeout: 60000 });
    }
    async createSession(profile) {
        try {
            // Create terminal with retry logic
            const terminalId = await (0, helpers_js_1.retry)(() => this.terminalManager.spawnTerminal(profile), { maxAttempts: 3, initialDelay: 1000 });
            // Create memory bank with retry logic
            const memoryBankId = await (0, helpers_js_1.retry)(() => this.memoryManager.createBank(profile.id), { maxAttempts: 3, initialDelay: 1000 });
            // Create session
            const session = {
...
class Orchestrator {
    constructor(config, terminalManager, memoryManager, coordinationManager, mcpServer, eventBus, logger) {
        ...
        this.healthCheckCircuitBreaker = (0, helpers_js_1.circuitBreaker)('HealthCheck', { threshold: 3, timeout: 10000, resetTimeout: 30000 });
        this.taskAssignmentCircuitBreaker = (0, helpers_js_1.circuitBreaker)('TaskAssignment', { threshold: 5, timeout: 5000, resetTimeout: 20000 });
    }
    async initialize() {
        if (this.initialized) {
    ...
    async spawnAgent(profile) {
        ...
        try {
            ...
            const session = await (0, helpers_js_1.retry)(() => this.sessionManager.createSession(profile), { maxAttempts: 3, initialDelay: 2000 });
            // Store agent profile
