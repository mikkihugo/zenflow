...
use ruv_fann::training::{Rprop, TrainingAlgorithm};
...
impl NeuralAgent {
  ...
  fn train_on_experience(
    &mut self,
    experiences: &[(Vec<f32>, Vec<f32>)],
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  println!(
    "Created {} agents with neural networks:",
    neural_agents.len()
  );
  ...
  let tasks = vec![
    ...
    Task::new(
      ...
      ruv_swarm_core::Priority::Medium,
    ),
    Task::new(
      "research_topic",
      serde_json::json!({"topic": "AI"}),
      ruv_swarm_core::Priority::Low,
    ),
  ];
  ...
  for (i, neural_agent) in neural_agents.iter_mut().enumerate() {
    ...
    let interpretation =
      interpret_output(&evaluation, &neural_agent.cognitive_pattern);

    println!(
      "  {:?} agent: {}",
      neural_agent.cognitive_pattern, interpretation
    );
  }
  ...
  match pattern {
    ...
    CognitivePattern::Divergent => {
      format!(
        ...
        output.iter().filter(|&&v| v > 0.5).count()
      )
    }
    CognitivePattern::Lateral => {
      format!(
        "Cross-domain connections: {:.0}",
        output.iter().sum::<f32>() * 10.0
      )
    }
    CognitivePattern::Systems => {
      format!(
        "System complexity score: {:.2}",
        output.iter().map(|v| v * v).sum::<f32>()
      )
    }
    CognitivePattern::Critical => {
      format!(
    ...
    CognitivePattern::Abstract => {
      format!(
        "Conceptual abstraction level: {:.1}",
        output
          .iter()
