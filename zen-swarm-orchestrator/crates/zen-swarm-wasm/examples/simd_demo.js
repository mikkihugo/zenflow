...
async function runNeuralNetworkBenchmark() {
  ...
  console.log(
    `Benchmarking neural network [${layers.join(', ')}] with ${iterations} iterations:`,
  );
...
function testSimdOperations() {
  ...
  console.log(
    `Vector addition: [${vectorSum.slice(0, 4).join(', ')}...] (expected: [3, 5, 7, 9...])`,
  );

  const scaledVector = vectorOps.vector_scale(vecA, 2.0);
  console.log(
    `Scaled vector: [${scaledVector.slice(0, 4).join(', ')}...] (expected: [2, 4, 6, 8...])`,
  );
  ...
  console.log(
    `ReLU activation: [${reluResult.join(', ')}] (expected: [0, 0, 0, 1, 2])`,
  );

  const sigmoidResult = vectorOps.apply_activation(testVec, 'sigmoid');
  console.log(
    `Sigmoid activation: [${sigmoidResult.map((x) => x.toFixed(3)).join(', ')}]`,
  );
  ...
  console.log(
    `Matrix-vector multiply: [${matVecResult.join(', ')}] (expected: [14, 32])`,
  );
  ...
  console.log(
    `Matrix-matrix multiply: [${matMulResult.join(', ')}] (expected: [19, 22, 43, 50])`,
  );
}
...
function checkBrowserSupport() {
  ...
  console.log(
    `WebAssembly SIMD: ${typeof WebAssembly !== 'undefined' && WebAssembly.validate ? 'Needs testing' : 'Unknown'}`,
  );
