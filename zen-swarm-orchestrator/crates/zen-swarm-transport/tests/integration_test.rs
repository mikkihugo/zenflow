...
async fn test_in_process_communication() {
    ...
    let correlation_id = match &request.payload {
        zen_swarm_transport::protocol::MessageType::Request { correlation_id, .. } => {
            *correlation_id
        }
    ...
    match received_response.payload {
        zen_swarm_transport::protocol::MessageType::Response {
...
async fn test_broadcast() {
    use zen_swarm_transport::in_process::InProcessTransportBuilder;
    ...
    match (&received2.payload, &received3.payload) {
        (
            zen_swarm_transport::protocol::MessageType::Broadcast { topic: topic2, .. },
            zen_swarm_transport::protocol::MessageType::Broadcast { topic: topic3, .. },
        ) => {
