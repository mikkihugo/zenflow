            ...
            |b, &receivers| {
                b.iter(|| {
                    rt.block_on(async {
                        let config = TransportConfig::default();
                        let builder =
                            zen_swarm_transport::in_process::InProcessTransportBuilder::new();
...
fn bench_shared_memory_throughput(c: &mut Criterion) {
    ...
    group.bench_function("ring_buffer_1000_messages", |b| {
        use zen_swarm_transport::shared_memory::RingBuffer;
...
fn bench_message_serialization(c: &mut Criterion) {
    ...
    use zen_swarm_transport::protocol::{BinaryCodec, JsonCodec, MessageCodec};
...
fn bench_compression(c: &mut Criterion) {
    ...
    use zen_swarm_transport::websocket::WebSocketTransport;
