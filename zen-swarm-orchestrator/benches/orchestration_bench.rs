...
use ruv_swarm::{
  ...
  task::{Task, TaskPriority, TaskType},
  topology::Topology,
};
        ...
        b.iter(|| {
          rt.block_on(async {
            ...
            let subtasks: Vec<&str> = (0..workers)
              .map(|i| {
                Box::leak(format!("Subtask {}", i).into_boxed_str()) as &str
              })
              .collect();

            let task =
              Task::new("Parallel workload", TaskType::Compute, subtasks)
                .unwrap();
      ...
      |b, task_type| {
        b.iter(|| {
          rt.block_on(async {
            ...
            let result = swarm
              .orchestrate_with_diversity(black_box(task))
              .await
              .unwrap();
            black_box(result);
          });
      ...
      |b, &depth| {
        b.iter(|| {
          rt.block_on(async {
            ...
            // Create hierarchy
            let coordinator =
              swarm.spawn_agent(AgentType::Coordinator).await.unwrap();

            let mut managers = Vec::new();
            for _ in 0..3 {
              managers
                .push(swarm.spawn_agent(AgentType::Coordinator).await.unwrap());
            }
            ...
            let task =
              Task::new("Hierarchical processing", TaskType::Compute, subtasks)
                .unwrap();

            let result = swarm
              .orchestrate_hierarchical(task, coordinator)
              .await
              .unwrap();
            black_box(result);
          });
  ...
  group.bench_function("failure_detection", |b| {
    b.iter(|| {
      rt.block_on(async {
        ...
        let result = swarm
          .orchestrate_with_recovery(black_box(task))
          .await
          .unwrap();
        black_box(result);
      });
  ...
  c.bench_function("pipeline_execution", |b| {
    b.iter(|| {
      rt.block_on(async {
        ...
        let result = swarm
          .execute_pipeline(pipeline, black_box(data))
          .await
          .unwrap();
        black_box(result);
      });
