...
async function validateMcpFunctionality() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `   WASM modules: ${Object.keys(features.wasm.modules_loaded || {}).length}`,
      );
      console.log(
        `   SIMD support: ${features.wasm.simd_support ? 'Yes' : 'No'}`,
      );
    ...
    try {
      ...
      console.log(
        `   Features: ${Object.entries(swarm.features)
          .filter(([k, v]) => v)
          .map(([k]) => k)
          .join(', ')}`,
      );
    ...
    try {
      ...
      console.log(
        `   Memory Usage: ${(status.global_metrics.memoryUsage / (1024 * 1024)).toFixed(2)}MB`,
      );
    ...
    try {
      ...
      console.log(
        `   Total Time: ${benchmarks.performance.total_benchmark_time_ms.toFixed(1)}ms`,
      );
    ...
    try {
      ...
      if (neural.available) {
        ...
        console.log(
          `   SIMD Acceleration: ${neural.simd_acceleration ? 'Yes' : 'No'}`,
        );
      }
