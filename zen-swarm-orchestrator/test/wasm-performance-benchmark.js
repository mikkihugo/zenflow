...
async function loadWasmModule() {
  const wasmPath = path.join(
    __dirname,
    '../crates/ruv-swarm-wasm/pkg/ruv_swarm_wasm_bg.wasm',
  );
...
async function benchmarkFunctionCalls() {
  ...
  results.wasmFunctionTime =
    results.jsFunctionTime + iterations * wasmCallOverhead;
...
function createNativeBaseline() {
  ...
  return {
    ...
    notes:
      'WASM typically runs 2-5x slower than native code due to sandboxing and runtime checks',
  };
}
  ...
  try {
    ...
    if (moduleSize > 500 * 1024) {
      ...
      results.recommendations.push({
        ...
        suggestion:
          'Consider using wasm-opt with -Oz flag for maximum size optimization',
      });
    }

    // 2. Initialization performance
    console.log('2. Testing initialization performance...');
    const { instance, initTime, initialMemoryPages } =
      await measureInitialization(wasmModule);
    ...
    const avgThroughput =
      memoryResults.reduce((sum, r) => sum + r.throughputMBps, 0) /
    ...
    if (!simdResults.simdAvailable) {
      results.bottlenecks.push({
        ...
        severity: 'high',
        details:
          'SIMD instructions not available, missing potential 2-4x speedup',
      });
      results.recommendations.push({
    ...
    if (nnResults.inferencePerSecond < 1000) {
      ...
      results.recommendations.push({
        category: 'algorithm',
        suggestion:
          'Consider using quantization or pruning for faster inference',
      });
    }
    ...
    results.recommendations.push({
      category: 'build',
      suggestion:
        'Enable LTO (Link Time Optimization) and single codegen unit for better optimization',
    });
    ...
    if (results.metrics.moduleLoading.loadTime > 100) {
      results.recommendations.push({
        category: 'deployment',
        suggestion:
          'Consider using WASM streaming compilation for faster startup',
      });
    }
  } catch (error) {
