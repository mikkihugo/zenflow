...
} from '../src/wasm-memory-optimizer.js';
...
async function runSIMDClaudeFlowDemo() {
  ...
  try {
    ...
    if (simdResults.supported) {
      ...
      console.log(
        `⚡ Average speedup: ${simdResults.averageSpeedup.toFixed(2)}x`,
      );
      console.log(
        `📊 Performance score: ${simdResults.performanceScore.toFixed(1)}/100`,
      );

      // Show detailed operation results
      for (const [operation, data] of Object.entries(simdResults.operations)) {
        console.log(
          `  🟢 ${operation}: ${data.averageSpeedup.toFixed(2)}x speedup`,
        );
      }
    } else {
    ...
    console.log(
      `📊 Efficiency: ${executionResult.metrics.efficiency.toFixed(1)}%`,
    );
    console.log(
      `⚡ Speedup factor: ${executionResult.metrics.speedupFactor.toFixed(2)}x`,
    );
    console.log(
      `📦 Batching compliance: ${executionResult.batchingReport.complianceScore}/100`,
    );
    ...
    if (executionResult.metrics) {
      ...
      console.log(
        `  • Parallel efficiency: ${(metrics.parallelizationRate * 100).toFixed(1)}%`,
      );
      console.log(
        `  • Theoretical speedup: ${metrics.theoreticalSequentialTime}ms`,
      );
    ...
    console.log(
      `Overall Score: ${fullBenchmarks.performanceScore.toFixed(1)}/100`,
    );
    console.log(
      `Grade: ${benchmarks.getPerformanceGrade(fullBenchmarks.performanceScore)}`,
    );
    ...
    console.log(
      `Batching Compliance Score: ${performanceReport.batching.complianceScore}/100`,
    );
    ...
    console.log(
      `Average Speedup: ${performanceReport.summary.averageSpeedup.toFixed(2)}x`,
    );
    ...
    const compatibilityResults =
      await benchmarks.benchmarkBrowserCompatibility();

    console.log(
      `Compatibility Score: ${compatibilityResults.performanceScore.toFixed(1)}/100`,
    );
    console.log('\nSupported Features:');

    for (const [feature, supported] of Object.entries(
      compatibilityResults.features,
    )) {
    ...
    const integrationSummary = {
      simdOptimization:
        simdResults.supported && simdResults.averageSpeedup > 2.0,
    ...
    const successCount =
      Object.values(integrationSummary).filter(Boolean).length;
    ...
    console.log('\n🚀 Expected Performance Improvements:');
    console.log(
      `  • SIMD operations: ${simdResults.averageSpeedup.toFixed(1)}x faster`,
    );
    console.log(
      `  • Parallel execution: ${executionResult.metrics.speedupFactor.toFixed(1)}x faster`,
    );
    console.log(`  • Memory efficiency: Optimized pooling and GC`);
    console.log(
      `  • Workflow coordination: ${performanceReport.batching.complianceScore}% compliant`,
    );
    console.log(
      `  • Overall system: ${fullBenchmarks.performanceScore.toFixed(1)}/100 performance score`,
    );
