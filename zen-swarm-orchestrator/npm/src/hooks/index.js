/**
 * Claude Code Hooks Implementation for zen-swarm
...
class RuvSwarmHooks {
  ...
  async mcpSwarmInitializedHook(args) {
    ...
    if (persistConfig) {
      const configDir = path.join(process.cwd(), '.zen-swarm');
...
---
*Generated by zen-swarm agent coordination system*
`;
...
🤖 Generated by zen-swarm agent coordination
Co-Authored-By: ${agentName} <agent@zen-swarm.ai>`;
...
EOF
  ...
  async sessionRestoreHook(args) {
    ...
    try {
      const sessionDir = path.join(process.cwd(), '.zen-swarm');
  ...
  async checkSwarmStatus() {
    try {
      ...
      const statusFile = path.join(process.cwd(), '.zen-swarm', 'status.json');
  ...
  generateSessionSummary() {
    ...
    return `# zen-swarm Session Summary
...
`;
  ...
  async getSwarmStatus() {
    try {
      const statusPath = path.join(process.cwd(), '.zen-swarm', 'status.json');
      if (
        await fs
          .access(statusPath)
          .then(() => true)
