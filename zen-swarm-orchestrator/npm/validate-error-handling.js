...
async function validateErrorHandling() {
  ...
  try {
    const { ValidationError, SwarmError, ErrorFactory } = await import(
      './src/errors.js'
    );
    ...
    if (validationError.name !== 'ValidationError')
      throw new Error('ValidationError name incorrect');
    if (validationError.getSuggestions().length === 0)
      throw new Error('No suggestions generated');

    // Test SwarmError
    const swarmError = new SwarmError('Swarm failed', 'test-id', 'init');
    if (swarmError.swarmId !== 'test-id')
      throw new Error('SwarmError context not preserved');

    // Test ErrorFactory
    const factoryError = ErrorFactory.createError('agent', 'Test agent error', {
      agentId: 'test',
    });
    if (factoryError.name !== 'AgentError')
      throw new Error('ErrorFactory not creating correct type');
  ...
  try {
    ...
    const validResult = ValidationUtils.validateParams(
      { topology: 'mesh' },
      'swarm_init',
    );
    if (validResult.topology !== 'mesh')
      throw new Error('Valid param validation failed');
    ...
    if (!errorCaught)
      throw new Error('Invalid param validation should have failed');
  ...
  try {
    ...
    const handledError = tools.handleError(
      testError,
      'test_tool',
      'test_operation',
    );
