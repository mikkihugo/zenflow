...
console.log(
  '🧪 Starting High-Availability and Load Testing for SQLite Connection Pool\n',
);
...
async function testConcurrentReads() {
  ...
  try {
    ...
    console.log(
      `✅ Completed ${CONCURRENT_CONNECTIONS} concurrent reads in ${duration}ms`,
    );
    console.log(
      `⚡ Average: ${(duration / CONCURRENT_CONNECTIONS).toFixed(2)}ms per read`,
    );
...
async function testWriteQueueUnderLoad() {
  ...
  try {
    ...
    console.log(
      `✅ Completed ${totalOperations} write operations in ${duration}ms`,
    );
    console.log(
...
async function testWorkerThreadPerformance() {
  ...
  try {
    ...
    console.log(
      `✅ Completed 20 complex queries in worker threads in ${duration}ms`,
    );
...
async function testHAPersistenceLayer() {
  ...
  try {
    ...
    console.log('📊 Persistence Stats:', {
      ...
      averageResponseTime:
        persistenceStats.averageResponseTime.toFixed(2) + 'ms',
    });
...
async function testStressTestSustainedLoad() {
  ...
  try {
    ...
    console.log(
      `❌ Errors: ${errorCount} (${((errorCount / operationCount) * 100).toFixed(2)}%)`,
    );
...
async function testConnectionRecovery() {
  ...
  try {
    ...
    const result2 = await pool.read(
      'SELECT COUNT(*) as count FROM recovery_test',
    );
    console.log(
      `✅ Post-stress operations working (${result2[0].count} total records)`,
    );
