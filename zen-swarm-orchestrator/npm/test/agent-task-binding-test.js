  ...
  try {
    ...
    for (const taskResult of orchestratedTasks) {
      try {
        ...
        console.log(
          `📊 Task ${taskResult.taskId} status: ${statusCheck.status}`,
        );
    ...
    try {
      ...
      console.log(
        `   Assigned ${multiAgentTask.assigned_agents.length} agents`,
      );
    } catch (error) {
    ...
    console.log(
      `   - Successfully orchestrated ${orchestratedTasks.length} tasks`,
    );
