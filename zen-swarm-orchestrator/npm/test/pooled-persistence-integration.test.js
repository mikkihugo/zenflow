...
async function testMCPToolsIntegration() {
  ...
  try {
    ...
    console.log(
      `📊 Active connections: ${healthResult.pool_status?.active_connections || 'N/A'}`,
    );
...
async function testRuvSwarmCoreIntegration() {
  ...
  try {
    ...
    if (ruvSwarm.persistence.constructor.name !== 'SwarmPersistencePooled') {
      throw new Error(
        'Expected SwarmPersistencePooled, got ' +
          ruvSwarm.persistence.constructor.name,
      );
    }
...
async function testPerformanceComparison() {
  ...
  try {
    ...
    console.log(
      `   - Error rate: ${finalStats.persistence_metrics?.error_rate || 'N/A'}`,
    );
    console.log(
...
async function testEnvironmentConfiguration() {
  ...
  try {
    ...
    console.log(
      `   - Readers: ${stats.pool_metrics?.available_readers || 'N/A'}`,
    );
    console.log(
      `   - Workers: ${stats.pool_metrics?.available_workers || 'N/A'}`,
    );
