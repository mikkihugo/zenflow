...
for (const { name, pattern } of timeoutPatterns) {
  ...
  if (noTimeoutMatches === null && originalMatches !== null) {
    console.log(
      `   ✅ ${name}: Successfully removed (${originalMatches.length} instances)`,
    );
    removedCount++;
  } else if (noTimeoutMatches !== null) {
    console.log(
      `   ❌ ${name}: Still found ${noTimeoutMatches.length} instances`,
    );
  } else {
...
for (const { name, pattern } of securityPatterns) {
  ...
  if (noTimeoutMatches !== null && originalMatches !== null) {
    console.log(
      `   ✅ ${name}: Preserved (${noTimeoutMatches.length} instances)`,
    );
    preservedCount++;
  } else if (noTimeoutMatches === null) {
...
console.log(`\n📊 DETAILED SCORES:`);
console.log(
  `   🔥 Timeout Removal: ${(overallScore.timeoutRemoval * 100).toFixed(1)}%`,
);
console.log(
  `   🔒 Security Preservation: ${(overallScore.securityPreservation * 100).toFixed(1)}%`,
);
console.log(
  `   ⚡ Core Functionality: ${(overallScore.corePreservation * 100).toFixed(1)}%`,
);
console.log(
  `   🏷️  Version Identification: ${(overallScore.versionIdentification * 100).toFixed(1)}%`,
);
console.log(
  `   📝 Code Quality: ${(overallScore.codeQuality * 100).toFixed(1)}%`,
);
...
runFunctionalTests()
  .then((allPassed) => {
    ...
    if (allPassed && averageScore >= 0.95) {
      ...
      console.log(
        '⚠️  RESULT: Some issues detected. Review above output for details.',
      );
    }
