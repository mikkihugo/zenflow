...
// Function parameters
{
    ...
    // if (trimmed.includes(':')) return trimmed; // Already typed // LINT: unreachable code removed
          if (trimmed.includes('=')) {
            ...
            return `${name.trim()}: unknown = ${defaultValue.trim()}`;
...
;
    ...
    //   // LINT: unreachable code removed});
        .join(', ');
      return `export const ${name} = (${typedParams}): unknown =>`;
    //   // LINT: unreachable code removed},;
  },
;
 ...
 */;
async function convertFile(): unknown {
  try {
 ...
 */;
async function findJSFiles(): unknown {
  const _files = [];
;
  try {
    const _entries = await readdir(dir);
;
...
;
      if (stats.isDirectory()) {
        const _subFiles = await findJSFiles(fullPath);
        files.push(...subFiles);
...
;
  ...
  const _allJSFiles = await findJSFiles(BASE_DIR);
;
...
;
  ...
  try {
    const _remainingJS = await findJSFiles(BASE_DIR);
